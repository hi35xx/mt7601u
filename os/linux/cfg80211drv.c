...
extern INT RtmpIoctl_rt_ioctl_siwauth(IN RTMP_ADAPTER * pAd,
				      IN VOID * pData, IN ULONG Data);

extern UCHAR P2POUIBYTE[];
BUILD_TIMER_FUNCTION(RemainOnChannelTimeout);

VOID RemainOnChannelTimeout(IN PVOID SystemSpecific1,
...
#define RESTORE_COM_CH_TIME 100
	...
	if (RTMP_TEST_FLAG
		...
		CFG80211DBG(RT_DEBUG_OFF,
			    ("RemainOnChannelTimeout Abort! pAd->Flags = %lu \n",
			     pAd->Flags));
		return;
	}
	...
	if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].Valid &&
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_NULL: P2P_CLI PWR_ACTIVE ROC_END\n"));
		CFG80211_P2pClientSendNullFrame(pAd, PWR_ACTIVE);
#ifdef CONFIG_STA_SUPPORT
		if (INFRA_ON(pAd)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_NULL: CONCURRENT STA PWR_ACTIVE ROC_END\n"));
		...
		RTMPSetTimer(&pAd->Cfg80211RemainOnChannelDurationTimer,
			     RESTORE_COM_CH_TIME);
	} else if (INFRA_ON(pAd)
		   && (pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.BW ==
		       BW_20
		       || pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth ==
		       BW_20)
		   && (pAd->LatchRfRegs.Channel != pAd->CommonCfg.Channel)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_ROC: ROC_Timeout INFRA_ON Channel(BW_20): %d\n",
...
#ifdef CONFIG_STA_SUPPORT
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_NULL: INFRA_ON PWR_ACTIVE ROC_END\n"));
		RTMPSendNullFrame(pAd, pAd->CommonCfg.TxRate,
				  (OPSTATUS_TEST_FLAG
				   (pAd, fOP_STATUS_WMM_INUSED) ? TRUE : FALSE),
				  pAd->CommonCfg.
				  bAPSDForcePowerSave ? PWR_SAVE : pAd->StaCfg.
				  Psm);
#endif				/*CONFIG_STA_SUPPORT */
		RTMPSetTimer(&pAd->Cfg80211RemainOnChannelDurationTimer,
			     RESTORE_COM_CH_TIME);
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_ROC: ROC_Timeout INFRA_ON Channel(BW_40): %d\n",
			  pAd->CommonCfg.CentralChannel));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_ROC: RemainOnChannelTimeout -- FINISH\n"));

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,34))
		if (pAd->flg_cfg_dummy_p2p_init)
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,7,2))
			cfg80211_remain_on_channel_expired(pAd->
							   dummy_p2p_net_dev->
							   ieee80211_ptr,
							   pAd->CfgChanInfo.
							   cookie,
			...
			cfg80211_remain_on_channel_expired((struct wireless_dev
							    *)(pAd->
							       dummy_p2p_net_dev),
							   pAd->CfgChanInfo.
							   cookie,
...
VOID CFG80211_P2pClientSendNullFrame(IN VOID * pAdCB, IN INT PwrMgmt)
{
	...
	if (pEntry == NULL) {
		...
		ApCliRTMPSendNullFrame(pAd,
					...
					(pEntry,
					 fCLIENT_STATUS_WMM_CAPABLE)) ? TRUE :
				       FALSE, pEntry, PwrMgmt);
		OS_WAIT(20);
	}
}
			    ...
			    IN USHORT subcmd, IN VOID * pData, IN ULONG Data)
{
	...
	case CMD_RTPRIV_IOCTL_80211_CHANNEL_LOCK:
		if (pAd->LatchRfRegs.Channel != Data) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("CFG80211_PKT: TX CHANNEL_LOCK %lu\n", Data));
			AsicSwitchChannel(pAd, Data, FALSE);
			AsicLockChannel(pAd, Data);

			DBGPRINT(RT_DEBUG_INFO,
				 ("OFF-Channel Send Packet:  %lu-%d\n", Data,
				  pAd->LatchRfRegs.Channel));
		} else
			DBGPRINT(RT_DEBUG_INFO,
				 ("OFF-Channel Channel Equal:  %lu-%d\n", Data,
				  pAd->LatchRfRegs.Channel));
		break;
	...
	case CMD_RTPRIV_IOCTL_80211_CHANNEL_RESTORE:
		if (pAd->Cfg80211RocTimerRunning == TRUE) {
			if (pAd->LatchRfRegs.Channel != pAd->CfgChanInfo.ChanId) {
				AsicSwitchChannel(pAd, pAd->CfgChanInfo.ChanId,
						  FALSE);
				AsicLockChannel(pAd, pAd->CfgChanInfo.ChanId);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("After OFF-Channel Send, restore to ROC: %d\n",
					  pAd->CfgChanInfo.ChanId));
			} else
				DBGPRINT(RT_DEBUG_TRACE,
					 ("After OFF-Channel Send, Keep in ROC: %d\n",
					  pAd->CfgChanInfo.ChanId));
		} else {
			if (pAd->LatchRfRegs.Channel != pAd->CommonCfg.Channel) {
				...
				AsicLockChannel(pAd, pAd->CommonCfg.Channel);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("After OFF-Channel Send, restore to Common: %d\n",
					  pAd->CommonCfg.Channel));
			}
			DBGPRINT(RT_DEBUG_TRACE,
				 ("After OFF-Channel Send, Keep in Common: %d\n",
				  pAd->CommonCfg.Channel));
		}
		break;
		...
		if (pData != NULL) {
			...
			if (pAd->StaCfg.WfdCfg.bSuppInsertWfdIe) {
				...
				case SUBTYPE_REASSOC_REQ:
					WfdIeBitmap =
					    (0x1 << SUBID_WFD_DEVICE_INFO) |
					    (0x1 << SUBID_WFD_ASSOCIATED_BSSID)
					    | (0x1 <<
					       SUBID_WFD_COUPLED_SINK_INFO);
					break;
...
#endif				/* WFD_SUPPORT */
			{
				...
				os_alloc_mem(NULL,
					     (UCHAR **) & pAd->pTxStatusBuf,
					     Data);
				if (pAd->pTxStatusBuf != NULL) {
					NdisCopyMemory(pAd->pTxStatusBuf, pData,
						       Data);
				...
				CFG80211DRV_PrintFrameType(pAd, "TX", pData,
							   Data);
#ifdef CONFIG_AP_SUPPORT
				//struct ieee80211_mgmt *mgmt;
	...
	case CMD_RTPRIV_IOCTL_80211_REMAIN_ON_CHAN_DUR_TIMER_INIT:
		...
		RTMPInitTimer(pAd, &pAd->Cfg80211RemainOnChannelDurationTimer,
			      GET_TIMER_FUNCTION(RemainOnChannelTimeout), pAd,
			      FALSE);
		break;

	case CMD_RTPRIV_IOCTL_80211_CHANNEL_LIST_SET:
		DBGPRINT(RT_DEBUG_INFO,
			 ("CMD_RTPRIV_IOCTL_80211_CHANNEL_LIST_SET: %lu\n",
			  Data));
		//UINT32 *pChanList = (UINT32 *) pData;
		...
		if (pChanList != NULL) {
			...
			os_alloc_mem(NULL, (UCHAR **) (&pAd->pCfg80211ChanList),
				     sizeof(UINT32 *) * Data);
			if (pAd->pCfg80211ChanList != NULL) {
				NdisCopyMemory(pAd->pCfg80211ChanList,
					       pChanList,
					       sizeof(UINT32 *) * Data);
	...
	case CMD_RTPRIV_IOCTL_80211_AP_PROBE_RSP:
		...
		if (pData != NULL) {
			...
			os_alloc_mem(NULL, (UCHAR **) & pAd->pCfg80211RrobeRsp,
				     Data);
			if (pAd->pCfg80211RrobeRsp != NULL) {
				NdisCopyMemory(pAd->pCfg80211RrobeRsp, pData,
					       Data);
				pAd->Cfg80211AssocRspLen = Data;
			} else {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("YF_AP: MEM ALLOC ERROR\n"));
				return NDIS_STATUS_FAILURE;
			}
...
BOOLEAN CFG80211DRV_PrintFrameType(IN VOID * pAdOrg,
				   IN PUCHAR preStr,
		...
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			if ((pFrame->p80211Header.FC.SubType == SUBTYPE_ACTION)
				...
				isP2pFrame = TRUE;
				   ...
				   RTMPEqualMemory(&pFrame->Octet[1],
						   P2POUIBYTE, 4)) {
				isP2pFrame = TRUE;
				switch (pFrame->Subtype) {
				case P2PACT_NOA:
...
VOID CFG80211DRV_DisableApInterface(VOID * pAdOrg)
{
#ifdef CONFIG_AP_SUPPORT
	UINT32 Value = 0;
	UCHAR ByteValue = 0;
...
VOID CFG80211DRV_OpsMgmtFrameProbeRegister(VOID * pAdOrg,
					   VOID * pData, BOOLEAN isReg)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdOrg;
	...
	while (pDevEntry != NULL) {
		if (RTMPEqualMemory
		    (pDevEntry->net_dev->dev_addr, pNewNetDev->dev_addr,
		     MAC_ADDR_LEN))
			break;
	...
	if ((pAd->Cfg80211VifDevSet.vifDevList.size > 0) && (pDevEntry != NULL)) {
		if (isReg) {
	...
	DBGPRINT(RT_DEBUG_INFO,
		 ("pAd->Cfg80211RegisterProbeReqFrame=%d[%d]\n",
		  pAd->Cfg80211RegisterProbeReqFrame,
		  pAd->Cfg80211ProbeReqCount));
}

VOID CFG80211DRV_OpsMgmtFrameActionRegister(VOID * pAdOrg,
					    VOID * pData, BOOLEAN isReg)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdOrg;
	PNET_DEV pNewNetDev = (PNET_DEV) pData;
	...
	if ((pAd->Cfg80211VifDevSet.vifDevList.size > 0) && (pDevEntry != NULL)) {
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("TYPE pDevEntry->Cfg80211RegisterActionFrame=%d[%d]\n",
			  pDevEntry->Cfg80211RegisterActionFrame,
			  pDevEntry->Cfg80211ActionCount));
	...
	DBGPRINT(RT_DEBUG_INFO,
		 ("TYPE pAd->Cfg80211RegisterActionFrame=%d[%d]\n",
...
BOOLEAN CFG80211DRV_OpsScanCheckStatus(VOID * pAdOrg, UINT8 IfType)
{
	...
	if (RTMP_CFG80211_VIF_P2P_CLI_ON(pAd) &&
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("WARN: P2P_CLIENT In Connecting & Canncel Scan with Infra Side\n"));
		return FALSE;
	}
#ifdef RT_CFG80211_SUPPORT
...
static int CFG80211DRV_UpdateTimIE(VOID * pAdOrg,
	...
	if (pAd->ApCfg.DtimCount == 0)
		*(ptr + 4) |=
		    (pAd->ApCfg.MBSSID[MAIN_MBSSID].
		     TimBitmaps[WLAN_CT_TIM_BCMC_OFFSET] & 0x01);
			   ...
			   UINT32 beacon_tail_len, BOOLEAN isAllUpdate)
{
	...
	if (isAllUpdate) {	/* Invoke From CFG80211 OPS For setting Beacon buffer */
		...
		os_alloc_mem(NULL, (UCHAR **) & pAd->beacon_tail_buf,
			     beacon_tail_len);
		if (pAd->beacon_tail_buf != NULL) {
			NdisCopyMemory(pAd->beacon_tail_buf, beacon_tail_buf,
				       beacon_tail_len);
		...
		/* 1. Update the TIM IE */
		New_Tim_Len =
		    CFG80211DRV_UpdateTimIE(pAd, pBeaconFrame,
					    pAd->ApCfg.MBSSID[MAIN_MBSSID].
					    TimIELocationInBeacon);

		/* 2. Update the AFTER TIM IE */
		if (pAd->beacon_tail_buf != NULL) {
			NdisCopyMemory(pAd->ApCfg.MBSSID[MAIN_MBSSID].
				       BeaconBuf +
				       pAd->ApCfg.MBSSID[MAIN_MBSSID].
				       TimIELocationInBeacon + New_Tim_Len,
				       pAd->beacon_tail_buf,
				       pAd->beacon_tail_len);

			beacon_len =
			    pAd->ApCfg.MBSSID[MAIN_MBSSID].
			    TimIELocationInBeacon + pAd->beacon_tail_len +
			    New_Tim_Len;
		} else {
	...
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, TRUE, FALSE, FALSE, TRUE, 0,
		      BSS0Mcast_WCID, beacon_len, PID_MGMT, 0, 0, IFS_HTTXOP,
		      FALSE, &BeaconTransmit);
	...
	for (i = 0; i < beacon_len; i += 4) {
		longValue =
		    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) +
		    (*(ptr + 3) << 24);
		RTMP_IO_WRITE32(pAd, pAd->BeaconOffset[0] + TXWISize + i,
				longValue);
		ptr += 4;
	}
	...
	if (pBeaconSync == NULL) {
		CFG80211DBG(RT_DEBUG_ERROR,
			    ("80211> UpdateBeacon - pBeaconSync == NULL\n"));
		return;
	}

	ptr =
	    (PUCHAR) (pAd->ApCfg.MBSSID[MAIN_MBSSID].BeaconBuf +
		      pAd->ApCfg.MBSSID[MAIN_MBSSID].TimIELocationInBeacon);
	if ((*(ptr + 4)) & 0x01)
...
VOID CFG80211_UpdateAssocRespExtraIe(VOID * pAdOrg,
				     UCHAR * assocresp_ies,
				     UINT32 assocresp_ies_len)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdOrg;
	PUCHAR pAssocRespBuf =
	    (PUCHAR) pAd->ApCfg.MBSSID[MAIN_MBSSID].AssocRespExtraIe;
	//PUCHAR UpdatePos;

	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: IE len = %d\n", __FUNCTION__, assocresp_ies_len));
	if (assocresp_ies_len >
	    sizeof(pAd->ApCfg.MBSSID[MAIN_MBSSID].AssocRespExtraIe)) {
		DBGPRINT(RT_DEBUG_ERROR,
...
BOOLEAN CFG80211DRV_OpsBeaconSet(VOID * pAdOrg, VOID * pData, BOOLEAN isAdd)
{
#ifdef CONFIG_AP_SUPPORT
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdOrg;
	...
	CFG80211DBG(RT_DEBUG_TRACE,
		    ("80211> CFG80211DRV_OpsBeaconSet ==> %d\n", isAdd));
	...
	wpa_ie =
	    cfg80211_find_ie(CFG_WPA_EID, pBeacon->beacon_tail,
...
#else
	ht_info =
	    cfg80211_find_ie(WLAN_EID_HT_INFORMATION, pBeacon->beacon_tail,
			     pBeacon->beacon_tail_len);
#endif
#endif
...
#ifdef CONFIG_P2P_AUTO_GO_AS_SOFTAP
	vendor_spec =
	    cfg80211_find_ie(WLAN_EID_VENDOR_SPECIFIC, pBeacon->beacon_tail,
			     pBeacon->beacon_tail_len);
#endif
#ifdef DOT11_N_SUPPORT		/*forced to enable short-GI */
	ht_cap =
	    (UCHAR *) cfg80211_find_ie(WLAN_EID_HT_CAPABILITY,
				       pBeacon->beacon_tail,
				       pBeacon->beacon_tail_len);
	if (ht_cap != NULL) {
...
#else
	ssid_ie =
	    cfg80211_parsing_ie(WLAN_EID_SSID, pBeacon->beacon_head + 36,
				pBeacon->beacon_head_len - 36);
	supp_rates_ie =
	    cfg80211_parsing_ie(WLAN_EID_SUPP_RATES, pBeacon->beacon_head + 36,
				pBeacon->beacon_head_len - 36);
	wpa_ie =
	    cfg80211_parsing_ie(WLAN_EID_WPA, pBeacon->beacon_tail,
				pBeacon->beacon_tail_len);
	...
	if (pAd->p2pForceGOAsSoftAP) {
		...
		if (vendor_spec) {
			...
			while (pVendorIE <
				...
				if (RTMPEqualMemory
				    (pVendorIE + 2 /*EID + Length */ ,
				     P2POUIBYTE, 4)) {
					VendorIELen = *(pVendorIE + 1);
					hex_dump("pDest p2p IE 2:", pVendorIE,
						 VendorIELen);
					RTMPZeroMemory(pVendorIE + 2,
						       VendorIELen);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Remove P2P IE RTMPZeroMemory done, lk added\n"));
	...
	if (isAdd) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("New AP BSSID %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (pBeacon->assocresp_ies_len && pBeacon->assocresp_ies) {
		CFG80211_UpdateAssocRespExtraIe(pAd, pBeacon->assocresp_ies,
	...
	if (isAdd) {
		...
		{
			BOOLEAN Cancelled;
			RTMPCancelTimer(&pAd->CommonCfg.BeaconUpdateTimer,
					&Cancelled);
		}
...
BOOLEAN CFG80211DRV_OpsExtraIesSet(VOID * pAdOrg)
{
	...
	CFG80211DBG(RT_DEBUG_INFO,
		    ("80211> is_wpa_supplicant_up ==> %d\n",
		     pAd->StaCfg.WpaSupplicantUP));
	os_alloc_mem(pAd, (UCHAR **) & (pAd->StaCfg.pWpsProbeReqIe), ie_len);
	if (pAd->StaCfg.pWpsProbeReqIe) {
		memcpy(pAd->StaCfg.pWpsProbeReqIe,
...
BOOLEAN CFG80211DRV_ApKeyDel(VOID * pAdOrg, VOID * pData)
{
	...
	{
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG: AsicRemoveSharedKeyEntry %d\n",
			  pKeyInfo->KeyId));
...
BOOLEAN CFG80211DRV_ApKeyAdd(VOID * pAdOrg, VOID * pData)
{
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP40
		...
		pAd->ApCfg.MBSSID[MAIN_MBSSID].WepStatus = Ndis802_11WEPEnabled;
		{
			...
			NdisMoveMemory(pSharedKey->Key, pKeyInfo->KeyBuf,
				       pKeyInfo->KeyLen);
...
//#endif                
...
#endif
			{
				...
				if (pAd->ApCfg.MBSSID[MAIN_MBSSID].
				    GroupKeyWepStatus ==
					...
					pAd->SharedKey[MAIN_MBSSID][pKeyInfo->
								    KeyId].
					    KeyLen = LEN_TK;
					NdisMoveMemory(pAd->
						       SharedKey[MAIN_MBSSID]
						       [pKeyInfo->KeyId].Key,
						       pKeyInfo->KeyBuf,
						       pKeyInfo->KeyLen);
					...
					else if (pAd->ApCfg.MBSSID[MAIN_MBSSID].
						 GroupKeyWepStatus ==
						 Ndis802_11Encryption3Enabled)
#endif
						pAd->
					...
					AsicAddSharedKeyEntry(pAd, MAIN_MBSSID,
							      pKeyInfo->KeyId,
							      &pAd->
							      SharedKey
							      [MAIN_MBSSID]
				...
				if (pKeyInfo->MAC)
					pEntry =
					    MacTableLookup(pAd, pKeyInfo->MAC);
				if (pEntry) {
#ifdef CONFIG_P2P_AUTO_GO_AS_SOFTAP
					if (pAd->ApCfg.MBSSID[MAIN_MBSSID].
					    WepStatus ==
...
VOID CFG80211DRV_P2pClientKeyAdd(VOID * pAdOrg, VOID * pData)
{
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP40
...
#endif
		{
			...
			if (pApCliEntry->WepStatus ==
			    Ndis802_11Encryption3Enabled) {
				printk
				    ("APCLI: Set AES Security Set. [%d] (GROUP) %d\n",
				     BssIdx, pKeyInfo->KeyLen);
				NdisZeroMemory(&pApCliEntry->
					       SharedKey[pKeyInfo->KeyId],
					       sizeof(CIPHER_KEY));
				pApCliEntry->SharedKey[pKeyInfo->KeyId].KeyLen =
				    LEN_TK;
				NdisMoveMemory(pApCliEntry->
					       SharedKey[pKeyInfo->KeyId].Key,
					       pKeyInfo->KeyBuf,
					       pKeyInfo->KeyLen);
				...
				AsicAddSharedKeyEntry(pAd, BssIdx,
						      pKeyInfo->KeyId,
						      &pApCliEntry->
						      SharedKey[pKeyInfo->
								KeyId]);
			...
			if (pMacEntry) {
				printk
				    ("APCLI: Set AES Security Set. [%d] (PAIRWISE) %d\n",
				     BssIdx, pKeyInfo->KeyLen);
				NdisZeroMemory(&pMacEntry->PairwiseKey,
					       sizeof(CIPHER_KEY));
				pMacEntry->PairwiseKey.KeyLen = LEN_TK;

				NdisCopyMemory(&pMacEntry->
					       PTK[OFFSET_OF_PTK_TK],
					       pKeyInfo->KeyBuf,
					       OFFSET_OF_PTK_TK);
				...
				AsicAddPairwiseKeyEntry(pAd,
							...
							&pMacEntry->
							PairwiseKey);
				RTMPSetWcidSecurityInfo(pAd, BssIdx, 0,
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP40
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_WPAPSK_Proc ==> %d, %d, %zu...\n",
			  pKeyInfo->KeyId, pKeyInfo->KeyType,
			  strlen(pKeyInfo->KeyBuf)));
...
#endif
		{
			if (pAd->StaCfg.GroupCipher ==
			    Ndis802_11Encryption2Enabled)
				IoctlSec.Alg =
				    RT_CMD_STA_IOCTL_SECURITY_ALG_TKIP;
			else if (pAd->StaCfg.GroupCipher ==
				 Ndis802_11Encryption3Enabled)
				IoctlSec.Alg =
				    RT_CMD_STA_IOCTL_SECURITY_ALG_CCMP;
...
VOID CFG80211DRV_SetP2pCliAssocIe(VOID * pAdOrg, VOID * pData, UINT ie_len)
{
	...
	if (ie_len > 0) {
		...
		os_alloc_mem(NULL,
			     (UCHAR **) & pAd->ApCfg.ApCliTab[MAIN_MBSSID].
			     pWpaAssocIe, ie_len);
		if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].pWpaAssocIe) {
			pAd->ApCfg.ApCliTab[MAIN_MBSSID].WpaAssocIeLen = ie_len;
			NdisMoveMemory(pAd->ApCfg.ApCliTab[MAIN_MBSSID].
				       pWpaAssocIe, pData,
				       pAd->ApCfg.ApCliTab[MAIN_MBSSID].
				       WpaAssocIeLen);
		} else
...
BOOLEAN CFG80211DRV_P2pClientConnect(VOID * pAdOrg, VOID * pData)
{
	...
	pAd->ApCfg.ApCliTab[MAIN_MBSSID].WpaSupplicantUP =
	...
	if (pConnInfo->PairwiseEncrypType & RT_CMD_80211_CONN_ENCRYPT_CCMP) {
		...
		Set_ApCli_EncrypType_Proc(pAd, "AES");
		   ...
		   PairwiseEncrypType & RT_CMD_80211_CONN_ENCRYPT_TKIP) {
		DBGPRINT(RT_DEBUG_TRACE, ("TKIP\n"));
		Set_ApCli_EncrypType_Proc(pAd, "TKIP");
	} else if (pConnInfo->
		   PairwiseEncrypType & RT_CMD_80211_CONN_ENCRYPT_WEP) {
	...
	if (pConnInfo->pBssid != NULL) {
		NdisZeroMemory(pAd->ApCfg.ApCliTab[0].CfgApCliBssid,
			       MAC_ADDR_LEN);
		NdisCopyMemory(pAd->ApCfg.ApCliTab[0].CfgApCliBssid,
			       pConnInfo->pBssid, MAC_ADDR_LEN);
		NdisCopyMemory(pAd->P2pCfg.Bssid, pConnInfo->pBssid,
			       MAC_ADDR_LEN);
	}
...
BOOLEAN CFG80211DRV_Connect(VOID * pAdOrg, VOID * pData)
{
	...
	if (pConnInfo->GroupwiseEncrypType & RT_CMD_80211_CONN_ENCRYPT_CCMP) {
		...
		RtmpIoctl_rt_ioctl_siwauth(pAd, &IoctlWpa, 0);
		   ...
		   GroupwiseEncrypType & RT_CMD_80211_CONN_ENCRYPT_TKIP) {
	...
	CFG80211DBG(RT_DEBUG_TRACE, ("80211> Key = %p\n", pConnInfo->pKey));
	...
	if (pConnInfo->pKey &&
		...
		/* reset key */
#ifdef RT_CFG80211_DEBUG
		hex_dump("KeyBuf=", (UINT8 *) pConnInfo->pKey,
			 pConnInfo->KeyLen);
#endif				/* RT_CFG80211_DEBUG */
...
VOID CFG80211_UnRegister(IN VOID * pAdOrg, IN VOID * pNetDev)
{
	...
	pAd->Cfg80211RegisterActionFrame = FALSE, pAd->Cfg80211ActionCount = 0;
...
VOID CFG80211_LostGoInform(IN VOID * pAdCB)
{
...
VOID CFG80211_LostApInform(IN VOID * pAdCB)
{
	...
	if (p80211CB->pCfg80211_Wdev->sme_state == CFG80211_SME_CONNECTING) {
		cfg80211_connect_result(pAd->net_dev, NULL, NULL, 0, NULL, 0,
					WLAN_STATUS_UNSPECIFIED_FAILURE,
					GFP_KERNEL);
	} else if (p80211CB->pCfg80211_Wdev->sme_state ==
		   CFG80211_SME_CONNECTED) {
		cfg80211_disconnected(pAd->net_dev, 0, NULL, 0, GFP_KERNEL);
	}
#else
...
BOOLEAN CFG80211DRV_OpsCancelRemainOnChannel(VOID * pAdOrg, UINT32 cookie)
{
	...
	if (pAd->Cfg80211RocTimerRunning == TRUE) {
		CFG80211DBG(RT_DEBUG_TRACE,
			    ("CFG_ROC : CANCEL Cfg80211RemainOnChannelDurationTimer\n"));
		RTMPCancelTimer(&pAd->Cfg80211RemainOnChannelDurationTimer,
				&Cancelled);
		pAd->Cfg80211RocTimerRunning = FALSE;
	}
//#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,34))
...
BOOLEAN CFG80211DRV_OpsRemainOnChannel(VOID * pAdOrg,
				       VOID * pData, UINT32 duration)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdOrg;
	...
	if (INFRA_ON(pAd) &&
	    ((pAd->CommonCfg.Channel != pChanInfo->ChanId)
	     || (pAd->CommonCfg.CentralChannel != pChanInfo->ChanId))
	    && ((pAd->CommonCfg.Channel == pAd->LatchRfRegs.Channel)
		|| (pAd->CommonCfg.CentralChannel ==
		    pAd->LatchRfRegs.Channel))) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_NULL: STA PWR_SAVE ROC_START\n"));
		RTMPSendNullFrame(pAd, pAd->CommonCfg.TxRate,
				  (OPSTATUS_TEST_FLAG
				   (pAd, fOP_STATUS_WMM_INUSED) ? TRUE : FALSE),
				  PWR_SAVE);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,34))
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,7,2))
	cfg80211_ready_on_channel(pAd->dummy_p2p_net_dev->ieee80211_ptr,
				  pChanInfo->cookie, pChanInfo->chan, duration,
				  GFP_KERNEL);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	cfg80211_ready_on_channel((struct wireless_dev *)(pAd->
							  dummy_p2p_net_dev),
				  pChanInfo->cookie, pChanInfo->chan,
				  pChanInfo->ChanType, duration, GFP_KERNEL);
#else
	cfg80211_ready_on_channel(pAd->dummy_p2p_net_dev, pChanInfo->cookie,
				  pChanInfo->chan, pChanInfo->ChanType,
				  duration, GFP_KERNEL);
#endif
#endif
	NdisCopyMemory(&pAd->CfgChanInfo, pChanInfo,
		       sizeof(CMD_RTPRIV_IOCTL_80211_CHAN));

	if (pAd->Cfg80211RocTimerInit == FALSE) {
		CFG80211DBG(RT_DEBUG_TRACE,
			    ("CFG80211_ROC : INIT Cfg80211RemainOnChannelDurationTimer\n"));
		RTMPInitTimer(pAd, &pAd->Cfg80211RemainOnChannelDurationTimer,
			      GET_TIMER_FUNCTION(RemainOnChannelTimeout), pAd,
			      FALSE);
	...
	if (pAd->Cfg80211RocTimerRunning == TRUE) {
		CFG80211DBG(RT_DEBUG_TRACE,
			    ("CFG80211_ROC : CANCEL Cfg80211RemainOnChannelDurationTimer\n"));
		RTMPCancelTimer(&pAd->Cfg80211RemainOnChannelDurationTimer,
				&Cancelled);
		pAd->Cfg80211RocTimerRunning = FALSE;
	}
			   ...
			   IN VOID * pWiphy, IN UCHAR * pAlpha2)
{
	...
	for (IdBand = 0; IdBand < 2; IdBand++) {
		...
		for (IdChan = 0; IdChan < ChanNum; IdChan++) {
			if (CFG80211OS_ChanInfoGet
			    (CFG80211CB, pWiphy, IdBand, IdChan, &ChanId,
			     &Power, &FlgIsRadar) == FALSE) {
			...
			for (IdPwr = 0; IdPwr < MAX_NUM_OF_CHANNELS; IdPwr++) {
				if (ChanId == pAd->TxPower[IdPwr].Channel) {
					...
					pAd->ChannelList[RecId].MaxTxPwr =
					    Power;

					/* keep DFS flag */
					if (FlgIsRadar == TRUE)
						pAd->ChannelList[RecId].DfsReq =
						    TRUE;
					else
						pAd->ChannelList[RecId].DfsReq =
						    FALSE;
					...
					/* keep DFS type */
					pAd->ChannelList[RecId].
					    RegulatoryDomain = DfsType;
		      ...
		      IN UCHAR * pFrame, IN UINT32 FrameLen, IN INT32 RSSI)
{
	...
	CFG80211OS_Scaning(pCfg80211_CB,
			   ChanId, pFrame, FrameLen, RSSI, FlgIsNMode, BW);
#endif				/* CONFIG_STA_SUPPORT */
}				/* End of CFG80211_Scaning */

static void CFG80211_UpdateBssAvgRssi(IN PBSS_ENTRY pBssEntry)
{
	...
	if (FlgIsAborted == TRUE)
		...
		for (index = 0; index < pAd->ScanTab.BssNr; index++) {
			...
			if (pAd->ScanTab.BssEntry[index].Channel > 14)
				CenFreq =
				    ieee80211_channel_to_frequency(pAd->ScanTab.
								   BssEntry
								   [index].
								   Channel,
								   IEEE80211_BAND_5GHZ);
			...
			bss =
			    cfg80211_get_bss(pWiphy, chan, pBssEntry->Bssid,
					     pBssEntry->Ssid,
					     pBssEntry->SsidLen,
					     WLAN_CAPABILITY_ESS,
					     WLAN_CAPABILITY_ESS);
					   ...
					   IN UCHAR FlgIsSuccess)
{
	...
	CFG80211OS_P2pClientConnectResultInform(pAd->ApCfg.
						ApCliTab[MAIN_MBSSID].dev,
						pBSSID, pReqIe, ReqIeLen,
						pRspIe, RspIeLen, FlgIsSuccess);
...
VOID CFG80211_ApStaDel(IN VOID * pAdCB, IN UCHAR * pMac)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdCB;
	MAC_TABLE_ENTRY *pEntry;
...
INT CFG80211_reSetToDefault(IN VOID * pAdCB)
{
	...
	if (pAd->FlgCfg80211Scanning == TRUE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("WARN: close the scan cmd in device close phase\n"));
...
VOID CFG80211_IndicateScanFail(IN VOID * pAdCB)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdCB;
	CFG80211_CB *p80211CB = pAd->pCfg80211_CB;
...
BOOLEAN CFG80211_checkScanTable(IN VOID * pAdCB)
{
	...
	bss = cfg80211_get_bss(pWiphy, NULL, pAd->ApCliMlmeAux.Bssid,
			       ...
			       pAd->ApCliMlmeAux.SsidLen, WLAN_CAPABILITY_ESS,
			       WLAN_CAPABILITY_ESS);
	if (bss) {
		DBGPRINT(RT_DEBUG_TRACE,
	...
	bss_idx =
	    BssSsidTableSearchBySSID(&pAd->ScanTab, pAd->ApCliMlmeAux.Ssid,
	...
	if (bss_idx != BSS_NOT_FOUND) {
		...
		if (pAd->ScanTab.BssEntry[bss_idx].Channel > 14)
			CenFreq =
			    ieee80211_channel_to_frequency(pBssEntry->Channel,
							   IEEE80211_BAND_5GHZ);
		else
			CenFreq =
			    ieee80211_channel_to_frequency(pBssEntry->Channel,
		...
		NdisCopyMemory(ie + 2, pAd->ApCliMlmeAux.Ssid,
			       pAd->ApCliMlmeAux.SsidLen);
		NdisCopyMemory(ie + 2 + pAd->ApCliMlmeAux.SsidLen,
			       pBssEntry->pVarIeFromProbRsp,
			       pBssEntry->VarIeFromProbeRspLen);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,0))
		bss =
		    cfg80211_inform_bss(pWiphy, chan,
					CFG80211_BSS_FTYPE_UNKNOWN,
					pAd->ApCliMlmeAux.Bssid, timestamp,
					WLAN_CAPABILITY_ESS,
					pAd->ApCliMlmeAux.BeaconPeriod, ie,
					ieLen, (pBssEntry->AvgRssi * 100),
					GFP_KERNEL);
#else
...
VOID CFG80211_ParseBeaconIE(VOID * pAdOrg, VOID * pData, const UCHAR * wpa_ie,
			    const UCHAR * rsn_ie)
{
	//PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)pAdOrg;
