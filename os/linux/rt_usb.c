...
/*	Status			= pUrb->status; */
	...
	if (Status == USB_ST_NOERROR) {
		...
		UAPSD_UnTagFrame(pAd, BulkOutPipeId,
...
#endif				/* UAPSD_SUPPORT */
#ifdef USB_BULK_BUF_ALIGMENT
		RTMP_IRQ_LOCK(&pAd->TxContextQueueLock[BulkOutPipeId],
			      IrqFlags2);
		CUR_WRITE_IDX_INC(pHTTXContext->CurtBulkIdx,
				  BUF_ALIGMENT_RINGSIZE);
		RTMP_IRQ_UNLOCK(&pAd->TxContextQueueLock[BulkOutPipeId],
				IrqFlags2);
#endif				/* USB_BULK_BUF_ALIGMENT */
...
/*	Status			= pUrb->status; */
	...
	if ((pMLMEContext->Wcid != MCAST_WCID)
		...
		MAC_TABLE_ENTRY *pEntry =
		    &pAd->MacTab.Content[pMLMEContext->Wcid];
...
#if RT_CFG80211_SUPPORT
	{
		...
		pHeader =
		    (HEADER_802_11 *) (GET_OS_PKT_DATAPTR(pPacket) +
		...
		if (pAd->
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("CFG_TX_STATUS: REAL send %d\n",
				  (pHeader->Sequence)));
			CFG80211OS_TxStatus(pAd->dummy_p2p_net_dev, 5678,
					    pAd->pTxStatusBuf,
					    pAd->TxStatusBufLen, TRUE);
