...
/*	buffer = kmalloc(MAX_INI_BUFFER_SIZE, MEM_ALLOC_FLAG); */
	...
	if (src && *src) {
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			retval =
			    RtmpOSFileRead(srcf, buffer, MAX_INI_BUFFER_SIZE);
			if (retval > 0) {
...
/*	if((pBuf = kmalloc(IW_CUSTOM_MAX_LEN, GFP_ATOMIC)) != NULL) */
	...
	if (pBuf != NULL) {
		...
		if (pAddr)
			...
			pBufPtr +=
			    sprintf(pBufPtr, "(RT2860) BSS(ra%d) ", BssIdx);
		else
...
/*			NdisAcquireSpinLock(&pAd->TxSwQueueLock); */
			...
			while (pAd->MacTab.McastPsQueue.Head) {
				...
				if (pAd->TxSwQueue[QID_AC_BE].Number <=
					...
					pEntry =
					    RemoveHeadQueue(&pAd->MacTab.
							    McastPsQueue);
					/*if(pAd->MacTab.McastPsQueue.Number) */
					if (count) {
						RTMP_SET_PACKET_MOREDATA(pEntry,
									 TRUE);
					}
					InsertHeadQueue(&pAd->
							TxSwQueue[QID_AC_BE],
...
//+++Add by shiang for debug
	if (0) {
		hex_dump("announce_802_3_packet", GET_OS_PKT_DATAPTR(pRxPkt),
...
#ifdef CONFIG_STA_SUPPORT
void STA_MonPktSend(IN RTMP_ADAPTER * pAd, IN RX_BLK * pRxBlk)
{
	PNET_DEV pNetDev;
	...
	if (IS_MT7601(pAd))
		MaxRssi =
		    ConvertToRssi(pAd, pRxBlk->pRxWI->RxWISNR2, RSSI_0,
				  pRxBlk->pRxWI->RxWISNR1,
				  pRxBlk->pRxWI->RxWIBW);
	else
#endif				/* MT7601 */
		MaxRssi = RTMPMaxRssi(pAd,
				      ConvertToRssi(pAd,
						    pRxBlk->pRxWI->RxWIRSSI0,
						    RSSI_0, 0, 0),
				      ConvertToRssi(pAd,
						    pRxBlk->pRxWI->RxWIRSSI1,
						    RSSI_1, 0, 0),
				      ConvertToRssi(pAd,
						    pRxBlk->pRxWI->RxWIRSSI2,
						    RSSI_2, 0, 0));
...
#include "../elian/lib/sccb.h"
	...
	send_monitor_packets(pNetDev, pRxPacket, pHeader, pData, DataSize,
			     L2PAD, PHYMODE, BW, ShortGI, MCS, AMPDU, STBC,
			     RSSI1, BssMonitorFlag11n, pDevName, Channel,
...
VOID RTMPFreeAdapter(IN VOID * pAdSrc)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
...
/*	MiniportMMRequest(pAd, pkt->data, pkt->len); */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (RTMP_GET_PACKET_OPMODE(pPacket)) {
			APSendPackets((NDIS_HANDLE) pAd,
				      (PPNDIS_PACKET) & pPacket, 1);
			goto done;
		}
#endif				/* P2P_SUPPORT */
		if (!INFRA_ON(pAd) && !RTMP_CFG80211_VIF_P2P_GO_ON(pAd)
			...
			STASendPackets((NDIS_HANDLE) pAd,
				       (PPNDIS_PACKET) & pPacket, 1);
	}
...
#endif				/* CONFIG_APSTA_MIXED_SUPPORT */
#ifdef SINGLE_SKU
			    && (strstr(pConfig->pCmdData, "ModuleTxpower") ==
				NULL)
#endif				/* SINGLE_SKU */
			    )
			{
...
/*	        DBGPRINT(RT_DEBUG_ERROR, ("rt28xx_ioctl can not find I/F and use default: cmd = 0x%08x\n", cmd)); */
		...
		for (index = 0; index < MAX_APCLI_NUM; index++) {
			...
			if (index == MAX_APCLI_NUM) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("rt28xx_ioctl can not find Apcli I/F\n"));
				return -ENETDOWN;
			}
		}
