...
BOOLEAN RTMP_CFG80211_VIF_ON(IN VOID * pAdSrc)
{
...
PCFG80211_VIF_DEV static RTMP_CFG80211_FindVifEntry_ByMac(IN VOID * pAdSrc,
							  IN PNET_DEV
							  pNewNetDev)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
...
PNET_DEV RTMP_CFG80211_FindVifEntry_ByType(IN VOID * pAdSrc, UINT32 devType)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
	PLIST_HEADER pCacheList = &pAd->Cfg80211VifDevSet.vifDevList;
	PCFG80211_VIF_DEV pDevEntry = NULL;
...
VOID RTMP_CFG80211_AddVifEntry(IN VOID * pAdSrc,
			       IN PNET_DEV pNewNetDev, IN UINT32 DevType)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
	...
	if (pNewVifDev) {
		...
		insertTailList(&pAd->Cfg80211VifDevSet.vifDevList,
...
VOID RTMP_CFG80211_RemoveVifEntry(IN VOID * pAdSrc, IN PNET_DEV pNewNetDev)
{
	...
	pListEntry =
	    (PLIST_ENTRY) RTMP_CFG80211_FindVifEntry_ByMac(pAd, pNewNetDev);
...
VOID RTMP_CFG80211_VirtualIF_CancelP2pClient(IN VOID * pAdSrc)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
	PLIST_HEADER pCacheList = &pAd->Cfg80211VifDevSet.vifDevList;
	PCFG80211_VIF_DEV pDevEntry = NULL;
...
static INT CFG80211_VirtualIF_Open(IN PNET_DEV dev_p)
{
	VOID *pAdSrc;
...
========================================================================
*/
static INT CFG80211_VirtualIF_Close(IN PNET_DEV dev_p)
{
	VOID *pAdSrc;
	PRTMP_ADAPTER pAd = NULL;
...
VOID RTMP_CFG80211_VirtualIF_Init(IN VOID * pAdSrc,
				  IN CHAR * pDevName, IN UINT32 DevType)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdSrc;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s ---> (%s, %s, %d)\n", __FUNCTION__, pDevName, preIfName,
		  preIfIndex));
	...
	new_dev_p =
	    RtmpOSNetDevCreate(MC_RowID, &IoctlIF, Inf, preIfIndex,
			       sizeof(PRTMP_ADAPTER), preIfName);
	...
	NdisMoveMemory(&pNetDevOps->devAddr[0], &pAd->CurrentAddress[0],
		       MAC_ADDR_LEN);
...
========================================================================
*/
INT CFG80211_VirtualIF_PacketSend(IN PNDIS_PACKET skb_p, IN PNET_DEV dev_p)
{
...
VOID RTMP_CFG80211_VirtualIF_Remove(IN VOID * pAdSrc,
				    IN PNET_DEV dev_p, IN UINT32 DevType)
{
	...
	if (dev_p) {
		if (pAd->cfg80211_ctrl.Cfg80211RocTimerRunning == TRUE) {
			CFG80211DBG(RT_DEBUG_OFF,
				    ("%s : CANCEL Cfg80211RocTimer\n",
				     __FUNCTION__));
			RTMPCancelTimer(&pAd->cfg80211_ctrl.Cfg80211RocTimer,
					&Cancelled);
...
========================================================================
*/
INT CFG80211_VirtualIF_Ioctl(IN PNET_DEV dev_p, IN OUT VOID * rq_p, IN INT cmd)
{
...
int CFG80211_PacketSend(IN PNDIS_PACKET pPktSrc,
...
static INT CFG80211_DummyP2pIf_Close(IN PNET_DEV dev_p)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0))
	{
		extern const struct ieee80211_iface_combination
		    *p_ra_iface_combinations_ap_sta;
		extern const INT ra_iface_combinations_ap_sta_num;
		wdev->wiphy->iface_combinations =
		    p_ra_iface_combinations_ap_sta;
		wdev->wiphy->n_iface_combinations =
		    ra_iface_combinations_ap_sta_num;
	}
#endif
...
static INT CFG80211_DummyP2pIf_Ioctl(IN PNET_DEV dev_p,
				     IN OUT VOID * rq_p, IN INT cmd)
{
	RTMP_ADAPTER *pAd;
...
static INT CFG80211_DummyP2pIf_PacketSend(IN PNDIS_PACKET skb_p,
					  IN PNET_DEV dev_p)
{
	return 0;
}
...
#define INF_CFG80211_DUMMY_P2P_NAME "p2p"
	...
	if (unlikely(!pWdev)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Could not allocate wireless device\n"));
		return;
	}
