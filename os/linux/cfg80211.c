...
static VOID CFG80211_RegNotifier(IN struct wiphy *pWiphy,
				 IN struct regulatory_request *pRequest);
#else
static INT32 CFG80211_RegNotifier(IN struct wiphy *pWiphy,
				  IN struct regulatory_request *pRequest);
#endif
#else
static INT32 CFG80211_RegNotifier(IN struct wiphy *pWiphy,
...
#if 0
static int CFG80211_PrivGetIfType(IN struct wiphy *pWiphy)
{
			    ...
			    IN struct cfg80211_scan_request *pRequest)
{
	...
	if (MONITOR_ON(newpAd)) {
		CFG80211DBG(RT_DEBUG_ERROR,
			    ("under monitor mode, return back!!! \n"));
		CFG80211OS_ScanEnd(pCfg80211_CB, TRUE);
		return 0;
	}
	...
	if ((pRequest->wdev->iftype != NL80211_IFTYPE_STATION) &&
		...
		CFG80211DBG(RT_DEBUG_ERROR,
			    ("80211> DeviceType Not Support Scan ==> \n"));
		CFG80211OS_ScanEnd(pCfg80211_CB, TRUE);
		return -EOPNOTSUPP;
	}
	/* End of if */
	...
	if ((pRequest->n_ssids == 0) || (!pRequest->ssids)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("80211> Wildcard SSID,n_ssids == 0 In ProbeRequest.\n"));
		Wreq.data.flags |= IW_SCAN_ALL_ESSID;
	}
	if ((pRequest->n_ssids == 1) && (pRequest->ssids->ssid_len == 0)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("80211> Wildcard SSID In ProbeRequest.\n"));
		Wreq.data.flags |= IW_SCAN_ALL_ESSID;
	} else {
		DBGPRINT(RT_DEBUG_TRACE,
	...
	if (pRequest->n_channels > 0) {
		...
		for (idx = 0; idx < pRequest->n_channels; idx++) {
			pChanList[idx] =
			    ieee80211_frequency_to_channel(pRequest->
							   channels[idx]->
							   center_freq);
		...
		RTMP_DRIVER_80211_CHANNEL_LIST_SET(pAd, pChanList,
						   pRequest->n_channels);
			    ...
			    IN struct cfg80211_scan_request *pRequest)
{
	...
	if (OPSTATUS_TEST_FLAG(newpAd, fOP_STATUS_MEDIA_STATE_CONNECTED)
	    && RTMPEqualMemory(pRequest->ssids->ssid, "DIRECT-", 7)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("80211> sta is connected ,set the BW to 20M!\n"));
...
========================================================================
*/
static int CFG80211_OpsIbssJoin(IN struct wiphy *pWiphy,
				IN struct net_device *pNdev,
...
========================================================================
*/
static int CFG80211_OpsIbssLeave(IN struct wiphy *pWiphy,
				 IN struct net_device *pNdev)
{
	VOID *pAd;
...
#else
static int CFG80211_OpsTxPwrSet(IN struct wiphy *pWiphy,
				IN enum tx_power_setting Type, IN int dBm)
{
	CFG80211DBG(RT_DEBUG_TRACE, ("80211> %s ==>\n", __FUNCTION__));
	return -EOPNOTSUPP;
}				/* End of CFG80211_OpsTxPwrSet */
...
static int CFG80211_OpsPwrMgmt(IN struct wiphy *pWiphy,
			       IN struct net_device *pNdev,
			       IN bool FlgIsEnabled, IN int Timeout)
{
	CFG80211DBG(RT_DEBUG_TRACE, ("80211> %s ==>\n", __FUNCTION__));
	return -EOPNOTSUPP;
				      ...
				      IN UINT32 Changed)
{
	...
	if (Changed & WIPHY_PARAM_RTS_THRESHOLD) {
		RTMP_DRIVER_80211_RTS_THRESHOLD_ADD(pAd,
						    (VOID *) (ULONG) (pWiphy->
								      rts_threshold));
		CFG80211DBG(RT_DEBUG_TRACE,
...
*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37))
static int CFG80211_OpsKeyGet(IN struct wiphy *pWiphy,
			      IN struct net_device *pNdev,
			      IN UINT8 KeyIdx,
			      IN bool Pairwise,
			      IN const UINT8 * pMacAddr,
...
static int CFG80211_OpsKeyDel(IN struct wiphy *pWiphy,
			      IN struct net_device *pNdev,
			      IN UINT8 KeyIdx, IN const UINT8 * pMacAddr)
#endif				/* LINUX_VERSION_CODE */
{
	VOID *pAd;
	CMD_RTPRIV_IOCTL_80211_KEY KeyInfo;
...
static int CFG80211_OpsKeyDefaultSet(IN struct wiphy *pWiphy,
				     IN struct net_device *pNdev,
				     IN UINT8 KeyIdx,
				     IN bool Unicast, IN bool Multicast)
#else
			       ...
			       IN struct cfg80211_connect_params *pSme)
{
	...
	if (pSme->ie_len > 0) {
		...
		if (pNdev->ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_CLIENT)
			RTMP_DRIVER_80211_P2PCLI_ASSSOC_IE_SET(pAd, pSme->ie,
							       pSme->ie_len);
		else
#endif
			RTMP_DRIVER_80211_GEN_IE_SET(pAd, pSme->ie,
						     pSme->ie_len);
	...
	if (pSme->ie_len > 6) {	/* EID(1) + LEN(1) + OUI(4) */
		...
		while ((pos_start - pSme->ie) < pSme->ie_len) {
			...
			len = *(pos_start + 1);
			if (*pos_start == WLAN_EID_VENDOR_SPECIFIC
			    && *(pos_start + 1) >= 4 && *(pos_start + 2) == 0x00
			    && *(pos_start + 3) == 0x50
			    && *(pos_start + 4) == 0xf2
			    && *(pos_start + 5) == 0x04) {
				CFG80211DBG(RT_DEBUG_ERROR,
					    ("ConnInfo.bWpsConnection ===> WPS IE is found\n"));
				 ...
				 IN int Idx, IN struct survey_info *pSurvey)
{
	...
	pSurvey->channel =
	    ((CFG80211_CB *) (SurveyInfo.pCfg80211))->pCfg80211_Channels;
	pSurvey->filled =
	    SURVEY_INFO_CHANNEL_TIME_BUSY | SURVEY_INFO_CHANNEL_TIME_EXT_BUSY;
				       ...
				       OUT u64 * cookie)
{
	...
	if (MONITOR_ON(newpAd)) {
		CFG80211DBG(RT_DEBUG_ERROR,
			    ("remain on channel ==>under monitor mode, return back!!!! \n"));
		return 0;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0))
	CFG80211DBG(RT_DEBUG_TRACE,
		    ("80211> Channel = %d,duration = %d\n", ChanId, duration));
#else
	CFG80211DBG(RT_DEBUG_TRACE,
		    ("80211> Channel = %d, ChannelType = %d, duration = %d\n",
			      ...
			      IN u64 * pCookie)
{
	...
	*pCookie = 5678;
	...
	RTMP_DRIVER_80211_MGMT_FRAME_SEND(pAd, (VOID *) (params->buf),
					  params->len);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0))
	if (no_cck) ;		//pAd->isCfgDeviceInP2p = TRUE;
#else
				 ...
				 struct beacon_parameters *info)
{
	...
	CFG80211DBG(RT_DEBUG_TRACE,
		    ("80211>proberesp_ies_len = %d \n",
		     info->proberesp_ies_len));
	CFG80211DBG(RT_DEBUG_TRACE,
		    ("80211>assocresp_ies_len = %d \n",
		     info->assocresp_ies_len));

	if (info->proberesp_ies_len > 0)
		RTMP_DRIVER_80211_AP_PROBE_RSP(pAd, info->proberesp_ies,
					       info->proberesp_ies_len);
#endif
			       ...
			       struct cfg80211_ap_settings *settings)
{
	...
	if (RTDebugLevel > RT_DEBUG_TRACE) {
		...
		hex_dump("proberesp_ies",
			 (UCHAR *) (settings->beacon.proberesp_ies),
			 settings->beacon.proberesp_ies_len);
		hex_dump("assocresp_ies",
			 (UCHAR *) (settings->beacon.assocresp_ies),
	...
	if (settings->chandef.chan) {
		...
		/* get channel number */
		ChanId =
		    ieee80211_frequency_to_channel(settings->chandef.chan->
						   center_freq);
		CFG80211DBG(RT_DEBUG_TRACE, ("80211> Channel = %d\n", ChanId));
		ChanInfo.ChanId = ChanId;
		...
		if (cfg80211_get_chandef_type(&settings->chandef) ==
		    NL80211_CHAN_NO_HT)
			ChanInfo.ChanType = RT_CMD_80211_CHANTYPE_NOHT;
		else if (cfg80211_get_chandef_type(&settings->chandef) ==
			 NL80211_CHAN_HT20)
			ChanInfo.ChanType = RT_CMD_80211_CHANTYPE_HT20;
		else if (cfg80211_get_chandef_type(&settings->chandef) ==
			 NL80211_CHAN_HT40MINUS)
...
static int CFG80211_OpsStaDel(struct wiphy *pWiphy,
			      struct net_device *dev, UINT8 * pMacAddr)
{
	VOID *pAd;
	MAC80211_PAD_GET(pAd, pWiphy);
...
static int CFG80211_OpsStaAdd(struct wiphy *wiphy,
			      struct net_device *dev,
			      UINT8 * mac, struct station_parameters *params)
{
	CFG80211DBG(RT_DEBUG_TRACE, ("80211> %s ==>\n", __FUNCTION__));
	return 0;
}

static int CFG80211_OpsStaChg(struct wiphy *pWiphy,
			      struct net_device *dev,
			      UINT8 * pMacAddr,
...
struct net_device *
#endif
CFG80211_OpsVirtualInfAdd(IN struct wiphy *pWiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
			  IN const char *name,
#else
			  IN char *name,
...
#endif
{
	...
	CFG80211DBG(RT_DEBUG_ERROR,
		    ("80211> %s, %s [%d]==>\n", __FUNCTION__,
		     wdev->netdev->name, wdev->netdev->ieee80211_ptr->iftype));
#else
	CFG80211DBG(RT_DEBUG_ERROR,
		    ("80211> %s, %s [%d]==>\n", __FUNCTION__, pwdev->name,
		     pwdev->ieee80211_ptr->iftype));
#endif
	MAC80211_PAD_GET(pAd, pWiphy);
...
#else
	RTMP_CFG80211_VirtualIF_Remove(pAd, pwdev,
				       pwdev->ieee80211_ptr->iftype);
#endif
	return 0;
}
...
static const struct ieee80211_iface_combination ra_iface_combinations_p2p_GO[] = {
	{
	 .num_different_channels = 1,
	 .max_interfaces = 3,
	 //.beacon_int_infra_match = true,
...
const struct ieee80211_iface_combination *p_ra_iface_combinations_p2p =
    ra_iface_combinations_p2p_GO;
const INT ra_iface_combinations_p2p_num =
ARRAY_SIZE(ra_iface_combinations_p2p_GO);
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0))
	pWdev->wiphy->iface_combinations =
	    (struct ieee80211_iface_combination *)(&ra_iface_combinations_p2p);
...
========================================================================
*/
BOOLEAN CFG80211_Register(IN VOID * pAd,
			  IN struct device * pDev,
