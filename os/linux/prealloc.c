...
int __init prealloc_init(void)
{
...
#endif
	{
		PreAllocBuffer[tx + BLK_TX0] =
		    kmalloc(sizeof(HTTX_BUFFER), GFP_KERNEL | GFP_DMA);
		if (!PreAllocBuffer[tx + BLK_TX0])
			goto fail_malloc1;

		PreAllocDmaAddr[tx + BLK_TX0] =
		    virt_to_phys(PreAllocBuffer[tx + BLK_TX0]);
	}

	//Null
	PreAllocBuffer[BLK_NULL] =
	    kmalloc(sizeof(TX_BUFFER), GFP_KERNEL | GFP_DMA);
	...
	//RX
	for (rx = 0; rx < 8; rx++) {
		PreAllocBuffer[rx + BLK_RX0] =
		    kmalloc(MAX_RXBULK_SIZE, GFP_KERNEL | GFP_DMA);
		if (!PreAllocBuffer[rx + BLK_RX0])
			goto fail_malloc3;

		PreAllocDmaAddr[rx + BLK_RX0] =
		    virt_to_phys(PreAllocBuffer[rx + BLK_RX0]);
	}

	//CMD
	PreAllocBuffer[BLK_CMD] =
	    kmalloc(CMD_RSP_BULK_SIZE, GFP_KERNEL | GFP_DMA);
