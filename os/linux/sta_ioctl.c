...
/*				HT_CAP_INFO capInfo = pIoctlScan->pBssTable[i].HtCapability.HtCapInfo; */
		...
		if (pIoctlScan->pBssTable[i].WpsIeLen > 0) {
			...
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length =
			    (pIoctlScan->pBssTable[i].WpsIeLen * 2) + 7;
			NdisMoveMemory(custom, "wps_ie=", 7);
			for (idx = 0; idx < pIoctlScan->pBssTable[i].WpsIeLen;
			     idx++)
				sprintf(custom, "%s%02x", custom,
...
static int
rt_private_set_wsc_u32_item(struct net_device *dev,
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	RTMP_STA_IoctlHandle(pAd, NULL,
			     CMD_RTPRIV_IOCTL_STA_IW_SET_WSC_U32_ITEM, 0,
...
static int
rt_private_set_wsc_string_item(struct net_device *dev,
			       struct iw_request_info *info,
			       struct iw_point *dwrq, char *extra)
{
	...
	case RTPRIV_IOCTL_MSC_CTRL:
		{
			...
			if (access_ok
			    (VERIFY_READ, wrqin->u.data.pointer,
			     wrqin->u.data.length) != TRUE)
				break;
			msc_cmd_handler(wrqin->u.data.pointer,
					wrqin->u.data.length, result, pAd);
			wrqin->u.data.length = strlen(result);
			Status =
			    copy_to_user(wrqin->u.data.pointer, result,
					 wrqin->u.data.length);
		}
		break;
#endif
