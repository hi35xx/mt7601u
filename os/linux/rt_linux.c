...
VOID RtmpDrvAllRFPrint(IN VOID * pReserved, IN UINT32 * pBuf, IN UINT32 BufLen)
{
	...
	if (IS_ERR(file_w)) {
		...
		if (file_w->f_op && file_w->f_op->write) {
			file_w->f_pos = 0;
			/* write data to file */
			file_w->f_op->write(file_w, (char *)pBuf, BufLen,
					    &file_w->f_pos);
		}
		filp_close(file_w, NULL);
	}
	...
	for (IdLoop = 0; IdLoop < NumOfChan; IdLoop++) {
		...
		if (IdLoop >= 14) {
			pChannels[IdLoop].center_freq =
			    ieee80211_channel_to_frequency(Cfg80211_Chan
							   [IdLoop],
							   IEEE80211_BAND_5GHZ);
		} else {
			pChannels[IdLoop].center_freq =
			...
			IN INT32 RSSI, IN BOOLEAN FlgIsNMode, IN UINT8 BW)
{
	...
	if (ChanId > 14)
		CenFreq =
		    ieee80211_channel_to_frequency(ChanId, IEEE80211_BAND_5GHZ);
	else
		CenFreq =
		    ieee80211_channel_to_frequency(ChanId, IEEE80211_BAND_2GHZ);
#else
			  ...
			  IN UINT32 len)
{
	...
	return cfg80211_rx_mgmt(pNetDev->ieee80211_ptr, freq, 0, frame, len, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	return cfg80211_rx_mgmt(pNetDev->ieee80211_ptr,
				freq, 0, frame, len, GFP_ATOMIC);
#else
