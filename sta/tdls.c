...
UCHAR TDLS_LLC_SNAP_WITH_CATEGORY[] =
    { 0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00, 0x89, 0x0d, PROTO_NAME_TDLS,
CATEGORY_TDLS };
...
UCHAR TDLS_GetRegulatoryClass(IN PRTMP_ADAPTER pAd,
			      IN UCHAR ChannelWidth, IN UCHAR TargetChannel)
{
	int i = 0;
	UCHAR regclass = 0;
...
static BOOLEAN TDLS_IsValidChannel(IN PRTMP_ADAPTER pAd, IN UCHAR channel)
{
	INT i;
...
static UCHAR TDLS_GetExtCh(IN UCHAR Channel, IN UCHAR Direction)
{
	CHAR ExtCh;
...
TDLS_FindDiscoveryEntry(IN PLIST_HEADER pTdlsEnList, IN PUCHAR pMacAddr)
{
	...
	while (pPeerEntry != NULL) {
		if (NdisEqualMemory
		    (pPeerEntry->Responder, pMacAddr, MAC_ADDR_LEN))
			return pPeerEntry;
				   ...
				   IN PUCHAR pMacAddr, IN BOOLEAN bConnected)
{
	...
	if (pTdlsPeer) {
		...
		os_alloc_mem(NULL, (PUCHAR *) & pTdlsPeer,
			     sizeof(TDLS_DISCOVERY_ENTRY));
		...
		if (pTdlsPeer) {
			...
			NdisMoveMemory(pTdlsPeer->Responder, pMacAddr,
				       MAC_ADDR_LEN);
			NdisGetSystemUpTime(&pTdlsPeer->InitRefTime);
...
VOID TDLS_DelDiscoveryEntryByMAC(IN PLIST_HEADER pTdlsEnList,
				 IN PUCHAR pMacAddr)
{
	PLIST_ENTRY pListEntry = NULL;
	PTDLS_DISCOVERY_ENTRY pPeerEntry = NULL;
	...
	if (pPeerEntry) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("\n !!! Delete %02x:%02x:%02x:%02x:%02x:%02x from discovery table !!!\n",
			  pPeerEntry->Responder[0], pPeerEntry->Responder[1],
			  pPeerEntry->Responder[2], pPeerEntry->Responder[3],
			  pPeerEntry->Responder[4], pPeerEntry->Responder[5]));
...
VOID TDLS_MaintainDiscoveryEntryList(IN PRTMP_ADAPTER pAd)
{
	...
	while (pPeerEntry != NULL) {
		if (pPeerEntry->bConnected) {
			if (RTMP_TIME_AFTER
			    (now_time,
			     pPeerEntry->InitRefTime +
			     (pAd->StaCfg.TdlsInfo.TdlsRssiMeasurementPeriod *
			      ((1000 * OS_HZ) / 1000)))) {
				BOOLEAN rv;
				UCHAR PeerMAC[MAC_ADDR_LEN];

				NdisMoveMemory(PeerMAC, pPeerEntry->Responder,
					       MAC_ADDR_LEN);
				...
				if (pPeerEntry->RetryCount >= 2) {
					...
					delEntryList(pTdlsDiscoveryEnList,
						     pListEntry);
					os_free_mem(NULL, pPeerEntry);
					...
					if (pPeerEntry->bConnectedFirstTime) {
						...
						rv = TDLS_InsertDiscoveryPeerEntryByMAC(pTdlsDiscoveryEnList, pPeerEntry->Responder, TRUE);
					}
			...
			if ((RTMP_TIME_AFTER
			     (now_time,
			      pPeerEntry->InitRefTime +
			      TDLS_AUTO_DISCOVERY_INTERVAL))
			    && (pPeerEntry->bFirstTime)) {
				BOOLEAN rv;

				RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
					      TdlsDiscovPeerListSemLock);
				rv = TDLS_InsertDiscoveryPeerEntryByMAC
				    (pTdlsDiscoveryEnList,
				     pPeerEntry->Responder, FALSE);
				RTMP_SEM_UNLOCK(&pAd->StaCfg.TdlsInfo.
						TdlsDiscovPeerListSemLock);
				if (rv) {
					MlmeEnqueue(pAd,
...
PTDLS_BLACK_ENTRY
TDLS_FindBlackEntry(IN PLIST_HEADER pTdlsEnList, IN PUCHAR pMacAddr)
{
	...
	while (pPeerEntry != NULL) {
		if (NdisEqualMemory
		    (pPeerEntry->MacAddr, pMacAddr, MAC_ADDR_LEN))
			return pPeerEntry;
...
VOID
TDLS_InsertBlackEntryByMAC(IN PLIST_HEADER pTdlsEnList,
			   IN PUCHAR pMacAddr, IN UCHAR CurrentState)
{
	...
	if (pTdlsBlack) {
		...
		os_alloc_mem(NULL, (PUCHAR *) & pTdlsBlack,
			     sizeof(TDLS_BLACK_ENTRY));
		...
		if (pTdlsBlack) {
			...
			NdisMoveMemory(pTdlsBlack->MacAddr, pMacAddr,
				       MAC_ADDR_LEN);
			NdisGetSystemUpTime(&pTdlsBlack->InitRefTime);
			pTdlsBlack->CurrentState = CurrentState;
	...
	if (pBlackEntry) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("\n Delete %02x:%02x:%02x:%02x:%02x:%02x from black table!!!\n",
			  pBlackEntry->MacAddr[0], pBlackEntry->MacAddr[1],
			  pBlackEntry->MacAddr[2], pBlackEntry->MacAddr[3],
			  pBlackEntry->MacAddr[4], pBlackEntry->MacAddr[5]));
...
VOID
TDLS_MaintainBlackList(IN PRTMP_ADAPTER pAd, IN PLIST_HEADER pTdlsBlackenList)
{
	PTDLS_BLACK_ENTRY pBlackEntry = NULL;
	PLIST_ENTRY pListEntry = NULL, pTempListEntry = NULL;
	ULONG now_time = 0;
	...
	while (pBlackEntry != NULL) {
		...
		} else if (pBlackEntry->CurrentState ==
			   TDLS_BLACK_TDLS_BY_TEARDOWN) {
			if (RTMP_TIME_AFTER
			    (now_time,
			     pBlackEntry->InitRefTime +
			     (pAd->StaCfg.TdlsInfo.
			      TdlsDisabledPeriodByTeardown * ((1000 * OS_HZ) /
							      1000)))) {
				RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
					      TdlsBlackListSemLock);
				pTempListEntry = pListEntry->pNext;
...
VOID TDLS_AutoSetupByRcvFrame(IN PRTMP_ADAPTER pAd, IN PHEADER_802_11 pHeader)
{
	...
	pTdlsDiscoveryEntry =
	    TDLS_FindDiscoveryEntry(pTdlsDiscoveryEnList, pHeader->Addr3);
	...
	if ((pTdlsBlackEntry == NULL) && (pTdlsDiscoveryEntry == NULL)) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("AutoSetupByRcvFrame trigger discovery request !!!\n"));
		...
		if (LinkId < MAX_NUM_OF_TDLS_ENTRY) {
			...
			if (pTdlsPeer) {
				...
				pTdlsPeer->CurrentState =
				    TDLS_DISCOVERY_TO_SETUP_DONE;
				DBGPRINT(RT_DEBUG_ERROR,
					 ("TDLS peer entry already on table !!!\n"));
			}
		} else if (rv) {
			MlmeEnqueue(pAd,
				    TDLS_STATE_MACHINE,
				    MT2_MLME_TDLS_DISCOVER_REQ,
				    MAC_ADDR_LEN, pHeader->Addr3, 0);
		}
	}
...
==========================================================================
*/
INT Set_TdlsCapableProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_TdlsCapableProc::(bTdlsCapable=%d)\n",
...
==========================================================================
*/
INT Set_TdlsSetupProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (value_offset = 0, value = rstrtok(arg, ":"); value;
	     value = rstrtok(NULL, ":")) {
	...
	if (INFRA_ON(pAd)) {
		if (pAd->StaActive.ExtCapInfo.TDLSProhibited == TRUE) {
			DBGPRINT(RT_DEBUG_OFF,
				 ("TDLS - Set_TdlsSetupProc() AP Prohibited TDLS !!!\n"));
			return FALSE;
		}
...
==========================================================================
*/
INT Set_TdlsTearDownProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR macAddr[MAC_ADDR_LEN];
	PSTRING value;
	INT value_offset;
	...
	if (INFRA_ON(pAd)) {
		...
		if (idx == -1 || idx == MAX_NUM_OF_TDLS_ENTRY) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - Set_TdlsTearDownProc() can not find or full the LinkId!\n"));
			return FALSE;
		}
		...
		if (idx >= 0) {
			...
			pAd->StaCfg.TdlsInfo.TDLSEntry[idx].Status =
			    TDLS_MODE_NONE;
			TDLS_MlmeParmFill(pAd, &MlmeTdlsReq,
					  &pAd->StaCfg.TdlsInfo.TDLSEntry[idx],
					  Reason, FALSE);
			MlmeEnqueue(pAd, TDLS_STATE_MACHINE,
				    MT2_MLME_TDLS_TEAR_DOWN,
				    sizeof(MLME_TDLS_REQ_STRUCT), &MlmeTdlsReq,
				    0);
...
==========================================================================
*/
INT Set_TdlsDiscoveryReqProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
==========================================================================
*/
INT Set_TdlsTPKLifeTimeProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT32 keyLifeTime;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsTPKLifeTimeProc::(TdlsKeyLifeTime=%d)\n",
...
INT Set_TdlsAutoLinkProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsAutoLinkProc::(TdlsAutoLink=%d)\n",
...
INT Set_TdlsRssiMeasurementPeriodProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsRssiMeasurementPeriodProc::(RssiMeasurementPeriod = %d secs)\n",
		  pObj->ioctl_if,
		  pAd->StaCfg.TdlsInfo.TdlsRssiMeasurementPeriod));
...
INT Set_TdlsAutoDiscoveryPeriodProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsAutoDiscoveryPeriodProc::(AutoDiscoveryPeriod = %d secs)\n",
		  pObj->ioctl_if,
		  pAd->StaCfg.TdlsInfo.TdlsAutoDiscoveryPeriod));
...
INT Set_TdlsAutoSetupRssiThresholdProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	pAd->StaCfg.TdlsInfo.TdlsAutoSetupRssiThreshold =
	    AutoSetupRssiThreshold;

	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsAutoSetupRssiThresholdProc::(AutoSetupRssiThreshold = %d dbm)\n",
		  pObj->ioctl_if,
		  pAd->StaCfg.TdlsInfo.TdlsAutoSetupRssiThreshold));
...
INT Set_TdlsDisabledPeriodByTeardownProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	pAd->StaCfg.TdlsInfo.TdlsDisabledPeriodByTeardown =
	    DisabledPeriodByTeardown;

	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsDisabledPeriodByTeardownProc::(DisabledPeriodByTeardown = %d secs)\n",
		  pObj->ioctl_if,
		  pAd->StaCfg.TdlsInfo.TdlsDisabledPeriodByTeardown));
...
INT Set_TdlsAutoTeardownRssiThresholdProc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	pAd->StaCfg.TdlsInfo.TdlsAutoTeardownRssiThreshold =
	    AutoTeardownRssiThreshold;

	DBGPRINT(RT_DEBUG_ERROR,
		 ("IF(ra%d) Set_TdlsAutoTeardownRssiThresholdProc::(AutoTeardownRssiThreshold = %d dbm)\n",
		  pObj->ioctl_if,
		  pAd->StaCfg.TdlsInfo.TdlsAutoTeardownRssiThreshold));
				 ...
				 IN HT_CAPABILITY_IE * pHtCapability)
{
	...
	MaxSupportedRateIn500Kbps =
	    dot11_max_sup_rate(SupportRateLens, pSupportRates, 0, NULL);
...
BOOLEAN TDLS_MsgTypeSubst(IN UCHAR TDLSActionType, OUT INT * MsgType)
{
	...
	default:
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS_MsgTypeSubst : unsupported TDLS Action Type(%d); \n",
			  TDLSActionType));
		return FALSE;
	}
...
/*
    ...
    ==========================================================================
*/
BOOLEAN TDLS_CheckTDLSframe(IN PRTMP_ADAPTER pAd,
			    IN PUCHAR pData, IN ULONG DataByteCount)
{
	...
	if (!NdisEqualMemory
	    (TDLS_LLC_SNAP_WITH_CATEGORY, pData, LENGTH_802_1_H + 2))
		return FALSE;
...
/*
	...
	==========================================================================
*/
VOID TDLS_CntlOidTDLSRequestProc(IN PRTMP_ADAPTER pAd,
				 IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("CNTL - TDLS_CntlOidTDLSRequestProc set %02x:%02x:%02x:%02x:%02x:%02x with Valid=%d, Status=%d\n",
	...
	if (Idx == -1) {	/* not found and the entry is not full */
		if (pTDLS->Valid) {
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
				if (!pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid) {
					NdisMoveMemory(&pAd->StaCfg.TdlsInfo.
						       TDLSEntry[i], pTDLS,
						       sizeof
						       (RT_802_11_TDLS_UI));
					TDLS_MlmeParmFill(pAd, &MlmeTdlsReq,
					...
					MlmeEnqueue(pAd, TDLS_STATE_MACHINE,
						    ...
						    &MlmeTdlsReq, 0);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("CNTL - TDLS setup case\n"));
					break;
				}
				DBGPRINT(RT_DEBUG_ERROR,
					 ("CNTL - TDLS  do not find vaild entry !!!!\n"));
			}
		} else
			DBGPRINT(RT_DEBUG_WARN,
				 ("CNTL - TDLS not changed in Idx = -1 (Valid=%d)\n",
				  pTDLS->Valid));
		...
		if (pTDLS->Valid) {
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
				if ((pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid)
					...
					Status < TDLS_MODE_CONNECTED)) {
					/* update mac case */
					RTMPCancelTimer(&pAd->StaCfg.TdlsInfo.
							TDLSEntry[i].Timer,
							&TimerCancelled);
					NdisMoveMemory(&pAd->StaCfg.TdlsInfo.
						       TDLSEntry[i], pTDLS,
					...
					MlmeEnqueue(pAd, TDLS_STATE_MACHINE,
						    ...
						    &MlmeTdlsReq, 0);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("CNTL - TDLS restart case\n"));
					break;
				}
			}
		} else
			DBGPRINT(RT_DEBUG_WARN,
				 ("CNTL - TDLS not changed in Idx = MAX_NUM_OF_TDLS_ENTRY (Valid=%d)\n",
				  pTDLS->Valid));
		...
		if ((!pTDLS->Valid)
			...
			RTMPCancelTimer(&pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].
					Timer, &TimerCancelled);
			NdisMoveMemory(&pAd->StaCfg.TdlsInfo.TDLSEntry[Idx],
			...
			MlmeEnqueue(pAd, TDLS_STATE_MACHINE,
				    ...
				    0);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CNTL - TDLS retry setup procedure\n"));
		} else {
			DBGPRINT(RT_DEBUG_WARN,
				 ("CNTL - TDLS not changed in entry - %d - Valid=%d, Status=%d\n",
				  Idx,
				  pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].Valid,
				  pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].Status));
		}
	}
}
...
INT TDLS_SearchLinkId(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr)
{
	...
	if (i == MAX_NUM_OF_TDLS_ENTRY) {
		if (empty == 0) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("TDLS_SearchLinkId -  not found and full\n"));
			return MAX_NUM_OF_TDLS_ENTRY;
		} else {
			DBGPRINT(RT_DEBUG_INFO,
				 ("TDLS_SearchLinkId -  not found\n"));
			return -1;
		}
	}
...
/*
	...
	==========================================================================
*/
VOID TDLS_MlmeParmFill(IN PRTMP_ADAPTER pAd,
		       IN OUT MLME_TDLS_REQ_STRUCT * pTdlsReq,
		       IN PRT_802_11_TDLS pTdls,
		       IN USHORT Reason, IN BOOLEAN IsViaAP)
{
...
/*
	...
	========================================================================
*/
VOID TDLS_FTDeriveTPK(IN PUCHAR mac_i,
		      IN PUCHAR mac_r,
		      IN PUCHAR a_nonce,
		      IN PUCHAR s_nonce,
		      IN PUCHAR bssid,
		      IN UINT key_len, OUT PUCHAR tpk, OUT PUCHAR tpk_name)
{
	...
	KDF(TPK_KEY_INPUT,
	    LEN_PMK,
	    (PUCHAR) "TDLS PMK",
	    8, context, c_len, temp_result, (key_len + 16));
...
==========================================================================
*/
BOOLEAN MlmeTdlsReqSanity(IN PRTMP_ADAPTER pAd,
			  IN VOID * Msg,
			  IN ULONG MsgLen,
			  OUT PRT_802_11_TDLS * pTDLS,
			  OUT PUINT16 pReason, OUT BOOLEAN * pIsViaAP)
{
...
==========================================================================
*/
BOOLEAN PeerTdlsSetupReqSanity(IN PRTMP_ADAPTER pAd,
			       IN VOID * Msg,
			       IN ULONG MsgLen,
			       OUT UCHAR * pToken,
			       OUT UCHAR * pSA,
	...
	*pLinkIdentLen = 0;
	...
	if (MsgLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupReqSanity --> Invaild packet length - (action header) \n"));
		return FALSE;
	}
	...
	if (RemainLen < 1) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupReqSanity --> Invaild packet length - (dialog token) \n"));
		return FALSE;
	}
	*pToken = *Ptr;
	...
	if (RemainLen < 2) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupReqSanity --> Invaild packet length - (capability) \n"));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		case IE_EXT_CAPABILITY:
			if (pEid->Len >= sizeof(EXT_CAP_INFO_ELEMENT)) {
				NdisMoveMemory(pTdlsExtCap, &pEid->Octet[0],
		...
		case IE_HT_CAP:
			if (pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED) {
				...
				{
					NdisMoveMemory(pHtCap, pEid->Octet,
						       sizeof
						       (HT_CAPABILITY_IE));
		...
		case IE_TDLS_LINK_IDENTIFIER:
			if (pEid->Len >= TDLS_ELM_LEN_LINK_IDENTIFIER) {
				NdisMoveMemory(pLinkIdent, &pEid->Octet[0],
					       sizeof(TDLS_LINK_IDENT_ELEMENT));
				*pLinkIdentLen = TDLS_ELM_LEN_LINK_IDENTIFIER;
				NdisMoveMemory(pSA, pLinkIdent->InitiatorAddr,
					       MAC_ADDR_LEN);
			}
			break;
		...
		default:
			...
			DBGPRINT(RT_DEBUG_WARN,
				 ("PeerTdlsSetupReqSanity - unrecognized EID = %d\n",
				  pEid->Eid));
			break;
		}
	...
	if (*pLinkIdentLen == 0) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupReqSanity --> Invaild packet - (link identifier) \n"));
		return FALSE;
	} else {
		if (!MAC_ADDR_EQUAL(pLinkIdent->BSSID, pAd->CommonCfg.Bssid)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupReqSanity --> It's not my BSSID\n"));
			return FALSE;
		} else
		    if (!MAC_ADDR_EQUAL
			(pLinkIdent->ResponderAddr, pAd->CurrentAddress)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupReqSanity --> It's not my MAC address\n"));
			return FALSE;
		}
	}
...
==========================================================================
*/
BOOLEAN PeerTdlsSetupRspSanity(IN PRTMP_ADAPTER pAd,
			       IN VOID * Msg,
			       IN ULONG MsgLen,
			       OUT UCHAR * pToken,
			       OUT UCHAR * pSA,
			       ...
			       OUT TDLS_LINK_IDENT_ELEMENT * pLinkIdent)
{
	...
	*pLinkIdentLen = 0;
	...
	// Message contains 802.11 header (24 bytes), LLC_SNAP (8 bytes), TDLS Action header(3 bytes) and Payload (variable)
	if (RemainLen <
	    (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupRspSanity --> Invaild packet length - (action header) \n"));
		return FALSE;
	}
	...
	if (RemainLen < 2) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupRspSanity --> Invaild packet length - (status code) \n"));
		return FALSE;
	}
	...
	if (RemainLen < 1) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupRspSanity --> Invaild packet length - (dialog token) \n"));
		return FALSE;
	}
	*pToken = *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		case IE_SUPP_RATES:
			if (pEid->Len <= MAX_LEN_OF_SUPPORTED_RATES) {
				NdisMoveMemory(SupRate, &pEid->Octet[0],
					       pEid->Len);
				*pSupRateLen = pEid->Len;
			} else
				return FALSE;
		...
		case IE_EXT_SUPP_RATES:
			if (pEid->Len <= MAX_LEN_OF_SUPPORTED_RATES) {
				NdisMoveMemory(ExtRate, &pEid->Octet[0],
					       pEid->Len);
				*pExtRateLen = pEid->Len;
			}
			break;
		...
		case IE_HT_CAP:
			if (pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED) {
				if (pEid->Len >= SIZE_HT_CAP_IE)	//Note: allow extension.!!
				{
					NdisMoveMemory(pHtCap, &pEid->Octet[0],
						       sizeof
						       (HT_CAPABILITY_IE));
					*pHtCapLen = SIZE_HT_CAP_IE;	// Nnow we only support 26 bytes.
				}
		...
		case IE_TDLS_LINK_IDENTIFIER:
			if (pEid->Len >= TDLS_ELM_LEN_LINK_IDENTIFIER) {
				NdisMoveMemory(pLinkIdent, &pEid->Octet[0],
					       sizeof(TDLS_LINK_IDENT_ELEMENT));
				*pLinkIdentLen = TDLS_ELM_LEN_LINK_IDENTIFIER;
				NdisMoveMemory(pSA, pLinkIdent->ResponderAddr,
					       MAC_ADDR_LEN);
			}
			break;
		...
		default:
			// Unknown IE, we have to pass it as variable IEs
			DBGPRINT(RT_DEBUG_WARN,
				 ("PeerTdlsSetupRspSanity - unrecognized EID = %d\n",
				  pEid->Eid));
			break;
		}
	...
	if (*pLinkIdentLen == 0) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupRspSanity --> Invaild packet - (link identifier) \n"));
		return FALSE;
	} else {
		if (!MAC_ADDR_EQUAL(pLinkIdent->BSSID, pAd->CommonCfg.Bssid)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupRspSanity --> It's not my BSSID\n"));
			return FALSE;
		} else
		    if (!MAC_ADDR_EQUAL
			(pLinkIdent->InitiatorAddr, pAd->CurrentAddress)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupRspSanity --> It's not my MAC address\n"));
			return FALSE;
		}
	}
...
==========================================================================
*/
BOOLEAN PeerTdlsSetupConfSanity(IN PRTMP_ADAPTER pAd,
				IN VOID * Msg,
				IN ULONG MsgLen,
				OUT UCHAR * pToken,
				OUT UCHAR * pSA,
				...
				OUT ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	*pAddHtInfoLen = 0;
	...
	if (RemainLen <
	    (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupConfSanity --> Invaild packet length - (action header) \n"));
		return FALSE;
	}
	// Offset to Status Code
	Ptr += (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H);
	...
	if (RemainLen < 2) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupConfSanity --> Invaild packet length - (status code) \n"));
		return FALSE;
	}
	NdisMoveMemory(pStatusCode, Ptr, 2);
	...
	if (RemainLen < 1) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupConfSanity --> Invaild packet length - (dialog token) \n"));
		return FALSE;
	}
	...
	*pToken = *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		case IE_ADD_HT2:
			if (pEid->Len >= sizeof(ADD_HT_INFO_IE)) {
				...
				*pAddHtInfoLen = SIZE_ADD_HT_INFO_IE;
				...
				DBGPRINT(RT_DEBUG_WARN,
					 ("PeerTdlsSetupConfSanity - wrong IE_ADD_HT. \n"));
			}
			break;
		...
		default:
			// Unknown IE, we have to pass it as variable IEs
			DBGPRINT(RT_DEBUG_WARN,
				 ("PeerTdlsSetupConfSanity - unrecognized EID = %d\n",
				  pEid->Eid));
			break;
		}
	...
	if (*pLinkIdentLen == 0) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsSetupConfSanity --> Invaild packet - (link identifier) \n"));
		return FALSE;
	} else {
		if (!MAC_ADDR_EQUAL(pLinkIdent->BSSID, pAd->CommonCfg.Bssid)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupConfSanity --> It's not my BSSID\n"));
			return FALSE;
		} else
		    if (!MAC_ADDR_EQUAL
			(pLinkIdent->ResponderAddr, pAd->CurrentAddress)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsSetupConfSanity --> It's not my MAC address\n"));
			return FALSE;
		}
	}
...
==========================================================================
*/
BOOLEAN PeerTdlsTearDownSanity(IN PRTMP_ADAPTER pAd,
			       IN VOID * Msg,
			       IN ULONG MsgLen,
			       OUT UCHAR * pSA,
			       ...
			       OUT TDLS_LINK_IDENT_ELEMENT * pLinkIdent)
{
	...
	*pFTLen = 0;
	...
	if (RemainLen <
	    (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsTearDownSanity --> Invaild packet length - (cation header) \n"));
		return FALSE;
	}
	...
	if (RemainLen < 2) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsTearDownSanity --> Invaild packet length - (reason code) \n"));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			// Unknown IE, we have to pass it as variable IEs
			DBGPRINT(RT_DEBUG_WARN,
				 ("PeerTdlsTearDownSanity - unrecognized EID = %d\n",
				  pEid->Eid));
			break;
		}
	...
	if (*pLinkIdentLen == 0) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsTearDownSanity --> Invaild packet - (link identifier) \n"));
		return FALSE;
	} else {
		if (!MAC_ADDR_EQUAL(pLinkIdent->BSSID, pAd->CommonCfg.Bssid)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsTearDownSanity --> It's not my BSSID\n"));
			return FALSE;
		}
		// Check if my MAC address and then find out SA
		if (!MAC_ADDR_EQUAL
		    (pAd->CurrentAddress, pLinkIdent->InitiatorAddr)) {
			if (!MAC_ADDR_EQUAL
			    (pAd->CurrentAddress, pLinkIdent->ResponderAddr)) {
				DBGPRINT_RAW(RT_DEBUG_WARN,
					     ("PeerTdlsTearDownSanity --> It's not my Address\n"));
				return FALSE;
			} else {
...
==========================================================================
*/
BOOLEAN PeerTdlsDiscovReqSanity(IN PRTMP_ADAPTER pAd,
				IN VOID * Msg,
				IN ULONG MsgLen,
				OUT UCHAR * pSA, OUT UCHAR * pToken)
{
	ULONG RemainLen = MsgLen;
	CHAR *Ptr = (CHAR *) Msg;
...
==========================================================================
*/
BOOLEAN PeerTdlsDiscovRspSanity(IN PRTMP_ADAPTER pAd,
				IN VOID * Msg,
				IN ULONG MsgLen,
				OUT UCHAR * pToken,
				OUT UCHAR * pSA,
				...
				OUT TDLS_LINK_IDENT_ELEMENT * pLinkIdent)
{
	...
	*pLinkIdentLen = 0;
	...
	if (RemainLen < (LENGTH_802_11 + 2 + 2)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovRspSanity --> Invaild packet length - (action header) \n"));
		return FALSE;
	}
	// Offset to Dialog Token
	...
	if (RemainLen < 1) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovRspSanity --> Invaild packet length - (dialog token) \n"));
		return FALSE;
	}
	*pToken = *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			// Unknown IE, we have to pass it as variable IEs
			DBGPRINT(RT_DEBUG_WARN,
				 ("PeerTdlsDiscovRspSanity - unrecognized EID = %d\n",
				  pEid->Eid));
			break;
		}
	...
	if (*pLinkIdentLen == 0) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovRspSanity --> Invaild packet - (link identifier) \n"));
		return FALSE;
	} else {
		if (!MAC_ADDR_EQUAL(pLinkIdent->BSSID, pAd->CommonCfg.Bssid)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsDiscovRspSanity --> It's not my BSSID\n"));
			return FALSE;
		} else
		    if (!MAC_ADDR_EQUAL
			(pLinkIdent->InitiatorAddr, pAd->CurrentAddress)) {
			DBGPRINT_RAW(RT_DEBUG_WARN,
				     ("PeerTdlsDiscovRspSanity --> It's not my MAC address\n"));
			return FALSE;
		}
	}
...
==========================================================================
*/
ULONG PeerTdlsBasicSanity(IN PRTMP_ADAPTER pAd,
			  IN VOID * Msg,
			  IN ULONG MsgLen,
			  IN BOOLEAN bInitiator,
			  OUT UCHAR * pToken, OUT UCHAR * pSA)
{
	ULONG RemainLen = MsgLen;
	CHAR *Ptr = (CHAR *) Msg;
...
==========================================================================
*/
VOID TDLS_SendNullFrame(IN PRTMP_ADAPTER pAd,
		...
		if ((pTDLS->Valid) && (pTDLS->Status == TDLS_MODE_CONNECTED)) {
			...
			if (pAd->CommonCfg.bAPSDForcePowerSave) {
				...
				pHeader_802_11->FC.PwrMgmt =
				    (RtmpPktPmBitCheck(pAd) == TRUE) ? 1 : 0;
			}

			pHeader_802_11->Duration =
			    pAd->CommonCfg.Dsifs + RTMPCalcDuration(pAd, TxRate,
								    14);
	...
	for (idx = 0; idx < MAX_NUM_OF_TDLS_ENTRY; idx++) {
		...
		if ((pTDLS->Valid == TRUE)
			...
			if (pAd->StaCfg.WepStatus !=
				...
				if (pEntry->TdlsKeyLifeTimeCount >=
				    pTDLS->KeyLifetime) {
#ifdef TDLS_AUTOLINK_SUPPORT
					...
					DBGPRINT(RT_DEBUG_WARN,
						 ("ageout %02x:%02x:%02x:%02x:%02x:%02x from TDLS #%d after %d-sec silence\n",
						  ...
						  pEntry->Addr[4],
						  pEntry->Addr[5], idx,
						  TDLS_ENTRY_AGEOUT_TIME));

					NdisAcquireSpinLock(&pAd->StaCfg.
							    TdlsInfo.
							    TDLSEntryLock);
					pTDLS->Token = 0;
					pTDLS->Valid = FALSE;
					pTDLS->Status = TDLS_MODE_NONE;
					NdisReleaseSpinLock(&pAd->StaCfg.
							    TdlsInfo.
							    TDLSEntryLock);
	...
	for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
		if ((pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid)
			...
			PMAC_TABLE_ENTRY pEntry =
			    &pAd->MacTab.Content[pAd->StaCfg.TdlsInfo.
						 TDLSEntry[i].MacTabMatchWCID];

			DBGPRINT(RT_DEBUG_OFF,
				 ("%02x:%02x:%02x:%02x:%02x:%02x  \n",
				  PRINT_MAC(pAd->StaCfg.TdlsInfo.TDLSEntry[i].
					    MacAddr)));
			...
			DBGPRINT(RT_DEBUG_OFF,
				 ("%-10s",
				  get_phymode_str(pEntry->HTPhyMode.field.
						  MODE)));
			DBGPRINT(RT_DEBUG_OFF,
				 ("%-6s",
...
VOID TDLS_InitChannelRelatedValue(IN PRTMP_ADAPTER pAd,
				  IN HT_CAPABILITY_IE * pHtCapability)
{
