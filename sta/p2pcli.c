...
INT ApCliIfLookUp(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr)
{
	...
	do {
		for (i = 0; i < MAX_APCLI_NUM; i++) {
			if (MAC_ADDR_EQUAL
			    (pAd->ApCfg.ApCliTab[i].CurrentAddress, pAddr)) {
				ifIndex = i;
...
VOID ApCliMgtMacHeaderInit(IN PRTMP_ADAPTER pAd,
			   ...
			   IN UCHAR SubType,
			   IN UCHAR ToDs,
			   IN PUCHAR pDA, IN PUCHAR pBssid, IN USHORT ifIndex)
{
		     ...
		     IN OUT ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	if (pAd->CommonCfg.DesiredHtPhy.AmsduEnable
		...
		CLIENT_STATUS_SET_FLAG(pApCliEntry,
	...
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.ChannelWidth =
	    ...
	    ChannelWidth;
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.GF =
	    pHtCapability->HtCapInfo.GF & pAd->CommonCfg.DesiredHtPhy.GF;

	/* Send Assoc Req with my HT capability. */
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.AMsduSize =
	    pAd->CommonCfg.DesiredHtPhy.AmsduSize;
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.MimoPs =
	    pHtCapability->HtCapInfo.MimoPs;
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.ShortGIfor20 =
	    (pAd->CommonCfg.DesiredHtPhy.ShortGIfor20) & (pHtCapability->
							  HtCapInfo.
							  ShortGIfor20);
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.ShortGIfor40 =
	    (pAd->CommonCfg.DesiredHtPhy.ShortGIfor40) & (pHtCapability->
							  HtCapInfo.
							  ShortGIfor40);
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.TxSTBC =
	    (pAd->CommonCfg.DesiredHtPhy.TxSTBC) & (pHtCapability->HtCapInfo.
						    RxSTBC);
	pAd->ApCliMlmeAux.HtCapability.HtCapInfo.RxSTBC =
	    (pAd->CommonCfg.DesiredHtPhy.RxSTBC) & (pHtCapability->HtCapInfo.
						    TxSTBC);
	pAd->ApCliMlmeAux.HtCapability.HtCapParm.MaxRAmpduFactor =
	    pAd->CommonCfg.DesiredHtPhy.MaxRAmpduFactor;
	pAd->ApCliMlmeAux.HtCapability.HtCapParm.MpduDensity =
	    pHtCapability->HtCapParm.MpduDensity;
	pAd->ApCliMlmeAux.HtCapability.ExtHtCapInfo.PlusHTC =
	    pHtCapability->ExtHtCapInfo.PlusHTC;

	if (pAd->CommonCfg.bRdg) {
		pAd->ApCliMlmeAux.HtCapability.ExtHtCapInfo.RDGSupport =
...
VOID ComposeP2PPsPoll(IN PRTMP_ADAPTER pAd, IN PAPCLI_STRUCT pApCliEntry)
{
...
VOID ComposeP2PNullFrame(IN PRTMP_ADAPTER pAd, IN PAPCLI_STRUCT pApCliEntry)
{
	...
	COPY_MAC_ADDR(pApCliEntry->NullFrame.Addr2,
...
BOOLEAN ApCliLinkUp(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
	...
	do {
		if (ifIndex < MAX_APCLI_NUM) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("!!! APCLI LINK UP - IF(apcli%d) AuthMode(%d)=%s, WepStatus(%d)=%s !!!\n",
				  ...
				  GetAuthMode(pAd->ApCfg.ApCliTab[ifIndex].
					      AuthMode),
				  pAd->ApCfg.ApCliTab[ifIndex].WepStatus,
				  GetEncryptType(pAd->ApCfg.ApCliTab[ifIndex].
						 WepStatus)));
		} else {
		...
		pMacEntry =
		    MacTableInsertEntry(pAd, (PUCHAR) (pAd->ApCliMlmeAux.Bssid),
		...
		if (pMacEntry) {
			...
			if (CFG80211_checkScanResInKernelCache
			    (pAd, pAd->P2pCfg.Bssid, pApCliEntry->CfgSsid,
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("                   BSSID = [%02x:%02x:%02x:%02x:%02x:%02x].  p2p_bssid = [%02x:%02x:%02x:%02x:%02x:%02x].\n",
			...
			if (pApCliEntry->bP2pClient) {
				...
				pMacEntry->P2pInfo.P2pClientState =
				    P2PSTATE_CLIENT_ASSOC;
				P2pIdx =
				    P2pGroupTabSearch(pAd, pMacEntry->Addr);
			...
			if ((pMacEntry->AuthMode >= Ndis802_11AuthModeWPA)
				...
				while (len > 0) {
					...
					if ((pEid->Eid == IE_WPA)
					    &&
					    (NdisEqualMemory
					     (pEid->Octet, WPA_OUI, 4))
					    && (pMacEntry->AuthMode ==
						Ndis802_11AuthModeWPA
						|| pMacEntry->AuthMode ==
						Ndis802_11AuthModeWPAPSK)) {
						NdisMoveMemory(pMacEntry->
							       RSN_IE, pVIE,
							       (pEid->Len + 2));
						pMacEntry->RSNIE_Len =
						    (pEid->Len + 2);
			...
			if (pMacEntry->RSNIE_Len == 0) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("ApCliLinkUp: root-AP has no RSN_IE \n"));
			} else {
				hex_dump("The RSN_IE of root-AP",
					 pMacEntry->RSN_IE,
					 pMacEntry->RSNIE_Len);
			}

			SupportRate(pAd->ApCliMlmeAux.SupRate,
				    pAd->ApCliMlmeAux.SupRateLen,
				    pAd->ApCliMlmeAux.ExtRate,
				    pAd->ApCliMlmeAux.ExtRateLen, &pRates,
			...
			if (pAd->ApCliMlmeAux.HtCapabilityLen != 0) {
				...
				PHT_CAPABILITY_IE pHtCapability =
				    (PHT_CAPABILITY_IE) & pAd->ApCliMlmeAux.
				    HtCapability;

				if ((pAd->ApCliMlmeAux.HtCapability.HtCapInfo.
				     GF) && (pAd->CommonCfg.DesiredHtPhy.GF)) {
				...
				if ((pHtCapability->HtCapInfo.ChannelWidth)
					...
					pMacEntry->MaxHTPhyMode.field.BW =
					    BW_40;
					pMacEntry->MaxHTPhyMode.field.ShortGI =
					    ((pAd->CommonCfg.DesiredHtPhy.
					      ShortGIfor40) & (pHtCapability->
							       HtCapInfo.
							       ShortGIfor40));
				} else {
					pMacEntry->MaxHTPhyMode.field.BW =
				...
				pMacEntry->MaxHTPhyMode.field.STBC =
				    (pHtCapability->HtCapInfo.
				     RxSTBC & (pAd->CommonCfg.DesiredHtPhy.
					       TxSTBC));
				...
				pMacEntry->MmpsMode =
				    (UCHAR) pHtCapability->HtCapInfo.MimoPs;
				pMacEntry->AMsduSize =
				    (UCHAR) pHtCapability->HtCapInfo.AMsduSize;
				...
				if (pHtCapability->ExtHtCapInfo.MCSFeedback ==
				    0x03)
					CLIENT_STATUS_SET_FLAG(pMacEntry,
			...
			if (pAd->ApCfg.ApCliTab[ifIndex].bAutoTxRateSwitch ==
			    FALSE) {
				pMacEntry->bAutoTxRateSwitch = FALSE;
				/* If the legacy mode is set, overwrite the transmit setting of this entry.      */
				RTMPUpdateLegacyTxSetting((UCHAR) pAd->ApCfg.
							  ApCliTab[ifIndex].
							  DesiredTransmitSetting.
							  field.FixedTxMode,
							  pMacEntry);
				...
				MlmeSelectTxRateTable(pAd, pMacEntry,
						      &pMacEntry->pTable,
						      &TableSize,
						      &pMacEntry->
						      CurrTxRateIndex);
			...
			if (!INFRA_ON(pAd)) {
				NdisMoveMemory(&(pAd->CommonCfg.APEdcaParm),
					       &(pAd->ApCliMlmeAux.APEdcaParm),
					       sizeof(EDCA_PARM));
			...
			if (pAd->CommonCfg.bAggregationCapable) {
				if ((pAd->CommonCfg.bPiggyBackCapable)
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Turn on Piggy-Back\n"));
				} else if (pAd->ApCliMlmeAux.
					   APRalinkIe & 0x00000001) {
					OPSTATUS_SET_FLAG(pAd,
							  fOP_STATUS_AGGREGATION_INUSED);
					CLIENT_STATUS_SET_FLAG(pMacEntry,
							       fCLIENT_STATUS_AGGREGATION_CAPABLE);
	...
	if ((result == TRUE) &&
		...
		pAd->ApCfg.ApCliTab[ifIndex].WscControl.WscState =
		    WSC_STATE_LINK_UP;
		pAd->ApCfg.ApCliTab[ifIndex].WscControl.WscStatus =
		    WSC_STATE_LINK_UP;
		pAd->ApCfg.ApCliTab[ifIndex].WscControl.WscConfStatus =
		    WSC_SCSTATE_UNCONFIGURED;
		NdisZeroMemory(pAd->ApCfg.ApCliTab[ifIndex].WscControl.
			       EntryAddr, MAC_ADDR_LEN);
		NdisMoveMemory(pAd->ApCfg.ApCliTab[ifIndex].WscControl.
			       EntryAddr, pAd->ApCliMlmeAux.Bssid,
			       MAC_ADDR_LEN);
...
VOID ApCliLinkDown(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
	...
	if (pApCliEntry->bP2pClient) {
		...
		if ((P2P_CLI_ON(pAd)) && (IS_ENTRY_APCLI(pEntry)) &&
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s::  Secured = %d.  Addr = %02x:%02x:%02x:%02x:%02x:%02x.\n",
				  __FUNCTION__, pEntry->PortSecured,
			...
			if (pAd->P2pCfg.bP2pCliReConnect == FALSE)
				...
				RTMPSetTimer(&pAd->P2pCfg.P2pCliReConnectTimer,
					     P2P_CHECK_CLIENT_TIMER);
			}
		}
	}
...
VOID ApCliIfUp(IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->FlgCfg80211Connecting != TRUE &&
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("(%s) ApCli can't startup Due to CFG80211 No connect yet.\n",
			  __FUNCTION__));
		return;
	}
#endif				/* RT_CFG80211_SUPPORT */
	...
	for (ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++) {
		...
		if (APCLI_IF_UP_CHECK(pAd, ifIndex)
			...
			if (pAd->ApCfg.ApCliTab[0].CtrlCurrState ==
			    APCLI_CTRL_DISCONNECTED)
				MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE,
					    APCLI_CTRL_JOIN_REQ, 0, NULL,
					    ifIndex);
		}
	}
			  ...
			  OUT INT * Machine, OUT INT * MsgType)
{
	...
	if (pFrame->Hdr.FC.Type == BTYPE_DATA) {
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry)
		    && pAd->ApCfg.ApCliTab[pEntry->apidx].WscControl.
		    WscConfMode == WSC_ENROLLEE) {
	...
	*pSupRateLen = pFrame->Octet[7];
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_VENDOR_SPECIFIC:
			/* find wfd ie */
			if (NdisEqualMemory(pEid->Octet, WFA_WFD_OUI, 4)) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("%s() found wfd ie in assoc response frame, it's wfd connect.\n",
					  __FUNCTION__));
			...
			if (NdisEqualMemory
			    (pEid->Octet, P2POUIBYTE, sizeof(P2POUIBYTE))
			    && (pEid->Len >= 4)) {
...
MAC_TABLE_ENTRY *ApCliTableLookUpByWcid(IN PRTMP_ADAPTER pAd,
					IN UCHAR wcid, IN PUCHAR pAddrs)
{
	/*USHORT HashIdx; */
	ULONG ApCliIndex;
...
/*
	...
	==========================================================================
 */
static inline BOOLEAN ValidApCliEntry(IN PRTMP_ADAPTER pAd, IN INT apCliIdx)
{
	BOOLEAN result;
...
BOOLEAN ApCliAllowToSendPacket(IN RTMP_ADAPTER * pAd,
			       IN PNDIS_PACKET pPacket, OUT UCHAR * pWcid)
{
	...
	apCliIdx =
	    RTMP_GET_PACKET_NET_DEVICE(pPacket) - MIN_NET_DEVICE_FOR_APCLI;
...
/*
	...
	========================================================================
*/
BOOLEAN ApCliValidateRSNIE(IN PRTMP_ADAPTER pAd,
			   IN PEID_STRUCT pEid_ptr,
			   IN USHORT eid_len, IN USHORT idx)
{
	PUCHAR pVIE;
	PUCHAR pTmp;
	UCHAR len;
		...
		case IE_WPA:
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("ApCliValidateRSNIE - RSN-WPA1 PairWiseCipher(%s), GroupCipher(%s), AuthMode(%s)\n",
				  ((WPA.bMixMode) ? "Mix" : GetEncryptType(WPA.
									   PairCipher)),
		...
		case IE_RSN:
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("ApCliValidateRSNIE - RSN-WPA2 PairWiseCipher(%s), GroupCipher(%s), AuthMode(%s)\n",
				  (WPA2.
				   bMixMode ? "Mix" : GetEncryptType(WPA2.
								     PairCipher)),
				  GetEncryptType(WPA2.GroupCipher),
	...
	if ((WPA_AuthMode == pApCliEntry->AuthMode) ||
	    ((WPA_AuthModeAux != Ndis802_11AuthModeOpen)
	     && (WPA_AuthModeAux == pApCliEntry->AuthMode))) {
	...
	/* WPA2PSK */
		 ...
		 ((WPA2_AuthModeAux != Ndis802_11AuthModeOpen)
		  && (WPA2_AuthModeAux == pApCliEntry->AuthMode))) {
		/* Check cipher suite, AP must have more secured cipher than station setting */
	...
	} else if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPA2)
				    ...
				    IN UCHAR FromWhichBSSID)
{
	...
	if ((pRxInfo->MyBss == 0) && (pRxInfo->Decrypted == 0)
	    && (pHeader->FC.Wep == 1)) {
		if (RTMPSoftDecryptionAction
		    (pAd, (PUCHAR) pHeader, 0,
			      ...
			      IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (!pEntry || !IS_ENTRY_APCLI(pEntry)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s : This Entry doesn't exist!!! \n", __FUNCTION__));
		return FALSE;
	}

	IfIdx = pEntry->MatchAPCLITabIdx;

	if (pAd->ApCfg.ApCliTab[IfIdx].GroupCipher ==
	    Ndis802_11Encryption2Enabled && KeyLen >= LEN_TKIP_GTK) {
		GTK_len = LEN_TKIP_GTK;
	} else if (pAd->ApCfg.ApCliTab[IfIdx].GroupCipher ==
		   Ndis802_11Encryption3Enabled && KeyLen >= LEN_AES_GTK) {
		GTK_len = LEN_AES_GTK;
	} else {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s : GTK is invalid (GroupCipher=%d, DataLen=%d) !!! \n",
			  __FUNCTION__, pAd->ApCfg.ApCliTab[IfIdx].GroupCipher,
			  KeyLen));
		return FALSE;
	}
	...
	NdisZeroMemory(&pAd->ApCfg.ApCliTab[IfIdx].SharedKey[DefaultKeyIdx],
		       sizeof(CIPHER_KEY));
	pAd->ApCfg.ApCliTab[IfIdx].SharedKey[DefaultKeyIdx].KeyLen = GTK_len;
	NdisMoveMemory(pAd->ApCfg.ApCliTab[IfIdx].SharedKey[DefaultKeyIdx].Key,
		       pKey, LEN_TK);
	if (GTK_len == LEN_TKIP_GTK) {
		NdisMoveMemory(pAd->ApCfg.ApCliTab[IfIdx].
			       SharedKey[DefaultKeyIdx].RxMic, pKey + 16,
			       LEN_TKIP_MIC);
		NdisMoveMemory(pAd->ApCfg.ApCliTab[IfIdx].
			       SharedKey[DefaultKeyIdx].TxMic, pKey + 24,
			       LEN_TKIP_MIC);
	}

	/* Update Shared Key CipherAlg */
	pAd->ApCfg.ApCliTab[IfIdx].SharedKey[DefaultKeyIdx].CipherAlg =
	    CIPHER_NONE;
	if (pAd->ApCfg.ApCliTab[IfIdx].GroupCipher ==
	    Ndis802_11Encryption2Enabled)
		pAd->ApCfg.ApCliTab[IfIdx].SharedKey[DefaultKeyIdx].CipherAlg =
...
VOID APCli_Init(IN PRTMP_ADAPTER pAd, IN RTMP_OS_NETDEV_OP_HOOK * pNetDevOps)
{
...
/*	RTMP_OS_NETDEV_OP_HOOK	netDevOpHook; */
	...
	for (apcli_index = 0; apcli_index < MAX_APCLI_NUM; apcli_index++) {
		...
		new_dev_p =
		    RtmpOSNetDevCreate(MC_RowID, &IoctlIF, INT_APCLI,
				       apcli_index, sizeof(PRTMP_ADAPTER),
				       INF_APCLI_DEV_NAME);
#ifdef HOSTAPD_SUPPORT
		pAd->IoctlIF = IoctlIF;
#endif				/* HOSTAPD_SUPPORT */
		...
		if (pAd->chipCap.MBSSIDMode == MBSSID_MODE1) {
			...
			pApCliEntry->CurrentAddress[ETH_LENGTH_OF_ADDRESS - 1] =
			    (pApCliEntry->
			     CurrentAddress[ETH_LENGTH_OF_ADDRESS - 1] +
			     pAd->ApCfg.BssidNum + MAX_MESH_NUM) & 0xFF;
		}

		pNetDevOps->priv_flags = INT_APCLI;	/* we are virtual interface */
		pNetDevOps->needProtcted = TRUE;
		NdisMoveMemory(pNetDevOps->devAddr,
			       &pApCliEntry->CurrentAddress[0], MAC_ADDR_LEN);
...
BOOLEAN ApCli_Open(IN PRTMP_ADAPTER pAd, IN PNET_DEV dev_p)
{
	UCHAR ifIndex;
...
BOOLEAN ApCli_Close(IN PRTMP_ADAPTER pAd, IN PNET_DEV dev_p)
{
	UCHAR ifIndex;
...
int APC_PacketSend(IN PNDIS_PACKET skb_p,
		   IN PNET_DEV dev_p, IN RTMP_NET_PACKET_TRANSMIT Func)
{
	RTMP_ADAPTER *pAd;
