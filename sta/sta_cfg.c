...
static struct {
	...
	{
	"ATETSSICALBRENABLE",
		    RT335x_Set_ATE_TSSI_CALIBRATION_ENABLE_Proc},
...
/* 
    ...
    ==========================================================================
*/
INT Set_SSID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
//yiwei cfg
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_MLME !!! MLME busy, reset MLME state machine !!! ORI ==> %lu\n",
		...
		if ((pAd->StaCfg.WpaPassPhraseLen >= 8) &&
			...
			if (pAd->StaCfg.WpaPassPhraseLen == 64) {
				AtoH((PSTRING) pAd->StaCfg.WpaPassPhrase,
				     pAd->StaCfg.PMK, 32);
			} else {
				RtmpPasswordHash((PSTRING) pAd->StaCfg.
						 WpaPassPhrase, Ssid.Ssid,
						 Ssid.SsidLength, keyMaterial);
				NdisMoveMemory(pAd->StaCfg.PMK, keyMaterial,
					       32);
			}
		}
		...
		NdisMoveMemory(pAd->MlmeAux.AutoReconnectSsid, Ssid.Ssid,
			       Ssid.SsidLength);
		...
		if (Ssid.SsidLength == MAX_LEN_OF_SSID)
			hex_dump("Set_SSID_Proc::Ssid", Ssid.Ssid,
				 Ssid.SsidLength);
		else
			DBGPRINT(RT_DEBUG_TRACE,
...
/* 
    ...
    ==========================================================================
*/
INT Set_EncrypType_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT Set_Wep_Key_Proc(IN PRTMP_ADAPTER pAdapter,
		     IN PSTRING Key, IN INT KeyLen, IN INT KeyId)
{
	int i;
	...
	default:		/*Invalid argument */
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Set_Wep_Key_Proc::Invalid argument (=%s)\n", Key));
		return FALSE;
	}
	...
	/* Set keys (into ASIC) */
	if (pAdapter->StaCfg.AuthMode >= Ndis802_11AuthModeWPA) ;	/* not support */
	else {			/* Old WEP stuff */

		AsicAddSharedKeyEntry(pAdapter,
				      0, 0, &pAdapter->SharedKey[BSS0][KeyId]);
	}
...
/* 
    ...
    ==========================================================================
*/
INT Set_WPAPSK_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	int status;
	...
	status =
	    RT_CfgSetWPAPSKKey(pAd, arg, strlen(arg), pAd->MlmeAux.Ssid,
			       pAd->MlmeAux.SsidLen, pAd->StaCfg.PMK);
	if (status == FALSE) {
		DBGPRINT(RT_DEBUG_TRACE,
...
INT Set_GroupKey_Proc(IN PRTMP_ADAPTER pAdapter,
		      IN RTMP_IOCTL_INPUT_STRUCT * pRequest)
{
	RTMP_STA_IoctlHandle(pAdapter, NULL,
			     CMD_RTPRIV_IOCTL_STA_SIOCSIWENCODEEXT, 0, pRequest,
			     0, INT_MAIN);
...
INT Set_PSMode_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	if (pAdapter->StaCfg.BssType == BSS_INFRA) {
		if ((strcmp(arg, "Max_PSP") == 0) ||
			...
			pAdapter->StaCfg.WindowsBatteryPowerMode =
			    Ndis802_11PowerModeMAX_PSP;
...
INT Set_WscSsid_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((strlen(arg) > 0) && (strlen(arg) <= MAX_LEN_OF_SSID)) {
		...
		if (ApIdx != BSS_NOT_FOUND) {
			...
			NdisMoveMemory(pWscControl->WscBssid,
				       pAd->ScanTab.BssEntry[ApIdx].Bssid,
				       MAC_ADDR_LEN);
			pAd->MlmeAux.Channel =
			    pAd->ScanTab.BssEntry[ApIdx].Channel;
		}

		hex_dump("Set_WscSsid_Proc:: WscBssid", pWscControl->WscBssid,
			 MAC_ADDR_LEN);
...
INT Set_WscMode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((WscMode == WSC_PIN_MODE) ||
	    (WscMode == WSC_PBC_MODE) || (WscMode == WSC_SMPBC_MODE)) {
		/* save wsc mode */
...
INT Set_WscPinCode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (validatePin) {
		if (pWscControl->WscRejectSamePinFromEnrollee &&
			...
			pWscControl->WscLastPinFromEnrollee =
...
INT Set_WscUUIDE_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < 16; i++) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%02x", (pWscControl->Wsc_Uuid_E[i] & 0xff)));
	}
	DBGPRINT(RT_DEBUG_TRACE, ("\n"));
	DBGPRINT(RT_DEBUG_TRACE,
...
INT Set_WscGetConf_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pAd->StaCfg.BssType == BSS_ADHOC) {
		...
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) &&
			...
			pAd->StaCfg.WscControl.WscStatus =
			    STATUS_WSC_EMPTY_IPV4_SUBMASK_LIST;
			return TRUE;
		}
		MlmeEnqueue(pAd, IWSC_STATE_MACHINE, IWSC_MT2_MLME_START, 0,
			    NULL, 0);
		RTMP_MLME_HANDLER(pAd);
		return TRUE;
	}
	...
	if (INFRA_ON(pAd)) {
		...
		WscBuildBeaconIE(pAd, pWscControl->WscConfStatus, TRUE,
				 TmpWscMode, pWscControl->WscConfigMethods,
				 BSS0, NULL, 0, STA_MODE);
	...
	if (LED_MODE(pAd) == WPS_LED_MODE_SHARE) {
		...
		RTMPSetTimer(&pWscControl->WscLEDTimer,
			     WSC_WPS_PREPOST_WIFI_LED_TIMEOUT);
	} else
	...
	if (pIWscInfo->bBlockConnection) {
		...
		for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
			...
			MlmeDeAuthAction(pAd, pEntry,
					 REASON_DISASSOC_STA_LEAVING, FALSE);
		}
...
INT Set_IWscOOB_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	sprintf(&Ssid.Ssid[0], "IWSC%02X%02X%02X", RandomByte(pAd),
		RandomByte(pAd), RandomByte(pAd));
	...
	NdisZeroMemory(pAd->StaCfg.WpaPassPhrase, 64);
	NdisMoveMemory(pAd->StaCfg.WpaPassPhrase, pWscControl->WpaPsk,
	...
	if ((pAd->StaCfg.WpaPassPhraseLen >= 8) &&
		...
		if (pAd->StaCfg.WpaPassPhraseLen == 64) {
			...
			RtmpPasswordHash((PSTRING) pAd->StaCfg.WpaPassPhrase,
	...
	MlmeEnqueue(pAd,
		    MLME_CNTL_STATE_MACHINE,
		    OID_802_11_SSID,
		    sizeof(NDIS_802_11_SSID), (VOID *) & Ssid, 0);
			     ...
			     IN PSTRING extra, IN UINT32 size)
{
	...
	sprintf(extra + strlen(extra),
		...
		"AID", "BSS", "RSSI0", "RSSI1", "RSSI2", "PhMd", "BW", "MCS",
		"SGI", "STBC");
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry))
			...
			sprintf(extra + strlen(extra), "%-4d",
				(int)pEntry->Aid);
			sprintf(extra + strlen(extra), "%-4d",
				(int)pEntry->apidx);
			sprintf(extra + strlen(extra), "%-7d",
...
INT Set_XlinkMode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF Set_XlinkMode_Proc::(PSPXlink=%d)\n",
		  pAd->StaCfg.PSPXlink));
	return TRUE;
}
#endif				/* XLINK_SUPPORT */
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPA) {
		if (pKey->KeyIndex & 0x80000000) {
			...
			NdisMoveMemory(pEntry->PairwiseKey.Key,
				       pAd->SharedKey[BSS0][0].Key, LEN_TK);
...
INT Set_WOW_Hold(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("WOW_Hold = %d, equal to %d ms\n", pAd->WOW_Cfg.nHoldTime,
		  (pAd->WOW_Cfg.nHoldTime) * 10));
...
INT Set_UsbWOWSuspend(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (Flag == TRUE) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Failed to enter WOW Suspend mode!\n"));
#else
		       ...
		       INOUT RTMP_IOCTL_INPUT_STRUCT * rq, IN INT cmd)
{
	...
	case RT_OID_802_11_COUNTRY_REGION:
		...
		/* Only avaliable when EEPROM not programming */
			...
			Status =
			    copy_from_user(&Country, wrq->u.data.pointer,
					   wrq->u.data.length);
...
/*    	    ppassphrase= kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		...
		if (ppassphrase == NULL) {
			...
			Status =
			    copy_from_user(ppassphrase, wrq->u.data.pointer,
					   wrq->u.data.length);
			...
			if (Status) {
				...
				if (ppassphrase->KeyLength < 8
				    || ppassphrase->KeyLength > 64) {
					Status = -EINVAL;
					DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_BSSID:
		if (wrq->u.data.length != sizeof(NDIS_802_11_MAC_ADDRESS))
			...
			Status =
			    copy_from_user(&Bssid, wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case RT_OID_802_11_PHY_MODE:
		if (wrq->u.data.length != sizeof(RT_802_11_PHY_MODE))
			...
			Status =
			    copy_from_user(&PhyMode, wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case RT_OID_802_11_STA_CONFIG:
		if (wrq->u.data.length != sizeof(RT_802_11_STA_CONFIG))
			...
			Status =
			    copy_from_user(&StaConfig, wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case OID_802_11_DESIRED_RATES:
		if (wrq->u.data.length != sizeof(NDIS_802_11_RATES))
			...
			Status =
			    copy_from_user(&aryRates, wrq->u.data.pointer,
					   wrq->u.data.length);
			NdisZeroMemory(pAd->CommonCfg.DesireRate,
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Set::OID_802_11_DESIRED_RATES (%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x)\n",
				  pAd->CommonCfg.DesireRate[0],
				  pAd->CommonCfg.DesireRate[1],
				  pAd->CommonCfg.DesireRate[2],
				  pAd->CommonCfg.DesireRate[3],
				  pAd->CommonCfg.DesireRate[4],
				  pAd->CommonCfg.DesireRate[5],
				  pAd->CommonCfg.DesireRate[6],
				  pAd->CommonCfg.DesireRate[7]));
		...
		if (wrq->u.data.length != sizeof(NDIS_802_11_KEY_INDEX)) {
			...
			if (KeyIdx & 0x80000000) {
				...
				if (KeyIdx >= 4) {
					...
					pAd->SharedKey[BSS0][KeyIdx].CipherAlg =
					    CIPHER_NONE;
					AsicRemoveSharedKeyEntry(pAd, 0,
								 (UCHAR)
								 KeyIdx);
				}
			}
	...
	case RT_OID_802_11_RESET_COUNTERS:
		...
		if (pAd->chipCap.FlgHwTxBfCap) {
			...
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)
				NdisZeroMemory(&pAd->MacTab.Content[i].
					       TxBFCounters,
	...
	case OID_802_11_FRAGMENTATION_THRESHOLD:
		if (wrq->u.data.length !=
			...
			Status =
			    copy_from_user(&FragThresh, wrq->u.data.pointer,
					   wrq->u.data.length);
			...
			if (FragThresh > MAX_FRAG_THRESHOLD
				...
				if (FragThresh == 0) {
					pAd->CommonCfg.FragmentThreshold =
					    MAX_FRAG_THRESHOLD;
					pAd->CommonCfg.
					    bUseZeroToDisableFragment = TRUE;
				} else
					Status = -EINVAL;
			} else
				pAd->CommonCfg.FragmentThreshold =
	...
	case RT_OID_802_11_TX_POWER_LEVEL_1:
		if (wrq->u.data.length < sizeof(ULONG))
			...
			Status =
			    copy_from_user(&PowerTemp, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (PowerTemp > 100)
	...
	case OID_802_11_NETWORK_TYPE_IN_USE:
		if (wrq->u.data.length != sizeof(NDIS_802_11_NETWORK_TYPE))
			...
			Status =
			    copy_from_user(&NetType, wrq->u.data.pointer,
					   wrq->u.data.length);
...
/*            pKey = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		...
		Status =
		    copy_from_user(pKey, wrq->u.data.pointer,
				   wrq->u.data.length);
		if ((pKey->Length != wrq->u.data.length)
			...
			} else if ((pAd->StaCfg.AuthMode == Ndis802_11AuthModeWPAPSK) || (pAd->StaCfg.AuthMode == Ndis802_11AuthModeWPA2PSK) || (pAd->StaCfg.AuthMode == Ndis802_11AuthModeWPANone)) {	/* Only for WPA PSK mode */
				NdisMoveMemory(pAd->StaCfg.PMK,
					       &pKey->KeyMaterial,
					       pKey->KeyLength);
...
/*            pRemoveKey = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		...
		Status =
		    copy_from_user(pRemoveKey, wrq->u.data.pointer,
				   wrq->u.data.length);
...
/*            kfree(pKey); */
	...
	case OID_802_11_CONFIGURATION:
		if (wrq->u.data.length != sizeof(NDIS_802_11_CONFIGURATION))
			...
			os_alloc_mem(NULL, (UCHAR **) & pConfig,
...
/*                pConfig = &Config; */
	...
	case RT_OID_802_11_SET_HT_PHYMODE:
		if (wrq->u.data.length != sizeof(OID_SET_HT_PHYMODE))
			...
			Status =
			    copy_from_user(&HT_PhyMode, wrq->u.data.pointer,
					   wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_SET_APSD_SETTING:
		if (wrq->u.data.length != sizeof(ULONG))
			...
			pAd->StaCfg.UapsdInfo.bAPSDCapable =
			    (apsd & 0x00000001) ? TRUE : FALSE;
			pAd->CommonCfg.bAPSDAC_BE =
			    ((apsd & 0x00000002) >> 1) ? TRUE : FALSE;
			pAd->CommonCfg.bAPSDAC_BK =
			    ((apsd & 0x00000004) >> 2) ? TRUE : FALSE;
			pAd->CommonCfg.bAPSDAC_VI =
			    ((apsd & 0x00000008) >> 3) ? TRUE : FALSE;
			pAd->CommonCfg.bAPSDAC_VO =
			    ((apsd & 0x00000010) >> 4) ? TRUE : FALSE;
			pAd->CommonCfg.MaxSPLength =
			    (UCHAR) ((apsd & 0x00000060) >> 5);
	...
	case RT_OID_802_11_SET_APSD_PSM:
		if (wrq->u.data.length != sizeof(ULONG))
			...
			Status =
			    copy_from_user(&pAd->CommonCfg.bAPSDForcePowerSave,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case RT_OID_802_11_SET_DLS:
		if (wrq->u.data.length != sizeof(ULONG))
			...
			Status =
			    copy_from_user(&pAd->CommonCfg.bDLSCapable,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case RT_OID_802_11_SET_DLS_PARAM:
		if (wrq->u.data.length != sizeof(RT_802_11_DLS_UI))
			...
			RTMPMoveMemory(&Dls, wrq->u.data.pointer,
	...
	case RT_OID_802_11_SET_TDLS:
		if (wrq->u.data.length != sizeof(ULONG))
			...
			Status =
			    copy_from_user(&pAd->StaCfg.TdlsInfo.bTDLSCapable,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case RT_OID_802_11_SET_TDLS_PARAM:
		if (wrq->u.data.length != sizeof(RT_802_11_TDLS_UI))
			...
			RTMPMoveMemory(&TDLS, wrq->u.data.pointer,
				       sizeof(RT_802_11_TDLS_UI));

			MlmeEnqueue(pAd,
				    MLME_CNTL_STATE_MACHINE,
				    RT_OID_802_11_SET_TDLS_PARAM,
				    sizeof(RT_802_11_TDLS), &TDLS, 0);

			DBGPRINT(RT_DEBUG_TRACE,
				 ("Set::RT_OID_802_11_SET_TDLS_PARAM \n"));
		}
		break;
#endif				/* DOT11Z_TDLS_SUPPORT */
	...
	case RT_OID_802_11_SET_WMM:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			Status =
			    copy_from_user(&pAd->CommonCfg.bWmmCapable,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_DISASSOCIATE:
		...
		if (pAd->StaCfg.WscControl.bSkipWPSTurnOffLED == FALSE) {
			...
			RTMPCancelTimer(&pAd->StaCfg.WscControl.WscLEDTimer,
					&Cancelled);
		}
#endif				/* WSC_LED_SUPPORT */
#endif				/* WSC_STA_SUPPORT */
	...
	case RT_OID_802_11_SET_IMME_BA_CAP:
		if (wrq->u.data.length != sizeof(OID_BACAP_STRUC))
			...
			Status =
			    copy_from_user(&Orde, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Orde.Policy > BA_NOTUSE) {
				...
				pAd->CommonCfg.HtCapability.HtCapInfo.MimoPs =
				    Orde.MMPSmode;
				pAd->CommonCfg.HtCapability.HtCapInfo.
				    AMsduSize = Orde.AmsduSize;
				pAd->CommonCfg.HtCapability.HtCapParm.
				    MpduDensity = Orde.MpduDensity;
			} else {
				...
				if (pAd->CommonCfg.BACapability.field.
				    RxBAWinLimit > MAX_RX_REORDERBUF)
					pAd->CommonCfg.BACapability.field.
					    RxBAWinLimit = MAX_RX_REORDERBUF;

			}

			pAd->CommonCfg.REGBACapability.word =
	...
	case RT_OID_802_11_ADD_IMME_BA:
		...
		if (wrq->u.data.length != sizeof(OID_ADD_BA_ENTRY))
			...
			Status =
			    copy_from_user(&BA, wrq->u.data.pointer,
					   wrq->u.data.length);
				...
				if (BA.IsRecipient == FALSE) {
					if (pEntry->bIAmBadAtheros == TRUE)
						pAd->CommonCfg.BACapability.
						    field.RxBAWinLimit = 0x10;

					BAOriSessionSetUp(pAd, pEntry, index, 0,
							  100, TRUE);
				} else {
...
/*				pBA = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
			...
			if (pBA == NULL) {
				...
				Status =
				    copy_from_user(pBA, wrq->u.data.pointer,
						   wrq->u.data.length);
				DBGPRINT(RT_DEBUG_TRACE,
...
/*						kfree(pBA); */
				...
				if (pBA->IsRecipient == FALSE) {
					pEntry =
					    MacTableLookup(pAd, pBA->MACAddr);
					DBGPRINT(RT_DEBUG_TRACE,
...
/*    	    pWepKey = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		...
		if (Status) {
			...
			if (KeyIdx >= 4) {
				...
				NdisZeroMemory(&pAd->SharedKey[BSS0][KeyIdx],
					       sizeof(CIPHER_KEY));
...
#ifdef WPA_SUPPLICANT_SUPPORT
					NdisZeroMemory(&pAd->StaCfg.
						       DesireSharedKey[KeyIdx],
						       sizeof(CIPHER_KEY));

					/* set key material and key length */
					pAd->StaCfg.DesireSharedKey[KeyIdx].
				...
				if ((pAd->StaCfg.WpaSupplicantUP !=
					...
					STA_PORT_SECURED(pAd);
				} else if (pAd->StaCfg.PortSecured ==
...
/*            kfree(pWepKey); */
	...
	case OID_SET_COUNTERMEASURES:
		if (wrq->u.data.length != sizeof(int))
			...
			int enabled = 0;
			Status =
			    copy_from_user(&enabled, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (enabled == 1)
	...
	case OID_802_11_DEAUTHENTICATION:
		if (wrq->u.data.length != sizeof(MLME_DEAUTH_REQ_STRUCT))
			...
			os_alloc_mem(pAd, (UCHAR **) & MsgElem,
...
/*				kfree(MsgElem); */
	...
	case OID_802_11_DROP_UNENCRYPTED:
		if (wrq->u.data.length != sizeof(int))
			...
			pAd->MacTab.Content[BSSID_WCID].PortSecured =
			    pAd->StaCfg.PortSecured;
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			Status =
			    copy_from_user(&IEEE8021x_required_keys,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			pAd->StaCfg.IEEE8021x_required_keys =
...
/*	        pPmkId = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		...
		if (pPmkId->BSSIDInfoCount == 0)
			...
			for (BssIdx = 0; BssIdx < pPmkId->BSSIDInfoCount;
				...
				for (CachedIdx = 0;
				     CachedIdx < pAd->StaCfg.SavedPMKNum;
				     CachedIdx++) {
					/* compare the BSSID */
					if (NdisEqualMemory
					    (pBssIdInfo->BSSID,
					     pAd->StaCfg.SavedPMK[CachedIdx].
					     BSSID,
					     sizeof(NDIS_802_11_MAC_ADDRESS)))
						break;
				}
...
/*			pAd->StaCfg.pWpsProbeReqIe = kmalloc(wrq->u.data.length, MEM_ALLOC_FLAG); */
		os_alloc_mem(pAd, (UCHAR **) & (pAd->StaCfg.pWpsProbeReqIe),
			     wrq->u.data.length);
		if (pAd->StaCfg.pWpsProbeReqIe) {
			Status =
			    copy_from_user(pAd->StaCfg.pWpsProbeReqIe,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
...
/*				if((pUPnPMsg = kmalloc(msgLen, GFP_KERNEL)) == NULL) */
			if (pUPnPMsg == NULL)
				...
				retVal =
				    copy_from_user(pUPnPMsg,
						   wrq->u.data.pointer, msgLen);
...
/*					kfree(pUPnPMsg); */
	...
	case RT_OID_WSC_SET_PROFILE:
		if (wrq->u.data.length != sizeof(WSC_PROFILE))
			...
			PWSC_PROFILE pWscProfile =
			    &pAd->StaCfg.WscControl.WscProfile;
			NdisZeroMemory(pWscProfile, sizeof(WSC_PROFILE));
			Status =
			    copy_from_user(pWscProfile, wrq->u.data.pointer,
					   wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_SET_CONF_MODE:	/* WPS disable, Enrollee or Registrar */
		if (wrq->u.data.length != sizeof(INT))
			...
			INT WscConfMode = 0;
			Status =
			    copy_from_user(&WscConfMode, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == 0) {
				if (WscConfMode == 2)
					WscConfMode = 4;
	...
	case RT_OID_WSC_SET_MODE:
		if (wrq->u.data.length != sizeof(INT))
			...
			Status =
			    copy_from_user(&WscMode, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == 0) {
	...
	case RT_OID_WSC_SET_PIN_CODE:
		if (wrq->u.data.length != 8)	/* PIN Code Length is 8 */
			...
			CHAR PinCode[9] = { 0 };
			Status =
			    copy_from_user(&PinCode[0], wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == 0) {
				if (Set_WscPinCode_Proc
				    (pAd, (PSTRING) & PinCode[0]) == FALSE)
					Status = -EINVAL;
			}
		}
		break;
	...
	case RT_OID_WSC_SET_CONN_BY_PROFILE_INDEX:
		if (wrq->u.data.length != sizeof(UINT))
			...
			Status =
			    copy_from_user(&wsc_profile_index,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			if (wsc_profile_index <
	...
	case RT_OID_WSC_DRIVER_AUTO_CONNECT:
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			Status =
			    copy_from_user(&pAd->StaCfg.WscControl.
					   WscDriverAutoConnect,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_SET_PASSPHRASE:
		if (wrq->u.data.length > 64 || wrq->u.data.length < 8)
			...
			Status =
			    copy_from_user(pAd->StaCfg.WscControl.WpaPsk,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			NdisZeroMemory(pAd->StaCfg.WscControl.WpaPsk, 64);
		...
		if (wrq->u.data.length != sizeof(ULONG))
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Set::OID_802_11_SHORTRETRYLIMIT (tx_rty_cfg.field.ShortRetryLimit=%d, ShortRetryLimit=%ld)\n",
				  tx_rty_cfg.field.ShortRtyLimit,
				  ShortRetryLimit));
		}
		break;
...
/*			kfree(pKey); */
	...
	case OID_802_11_WEPDEFAULTKEYID:
		...
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			Status =
			    copy_from_user(&pAd->StaCfg.DefaultKeyId,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case OID_802_11_CURRENTCHANNEL:
		...
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			STRING ChStr[5] = { 0 };
			Status =
			    copy_from_user(&ctmp, wrq->u.data.pointer,
					   wrq->u.data.length);
	...
	case OID_802_11_WAPI_PID:
		{
			...
			if (copy_from_user
			    (&pObj->wapi_pid, wrq->u.data.pointer,
			     wrq->u.data.length)) {
				Status = -EFAULT;
			} else {
	...
	case OID_802_11_PORT_SECURE_STATE:
		...
		if (wrq->u.data.length != sizeof(WAPI_PORT_SECURE_STRUCT))
			...
			Status =
			    copy_from_user(&wapi_port, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == NDIS_STATUS_SUCCESS) {
				if (INFRA_ON(pAd)) {
					if (NdisEqualMemory
					    (pAd->MlmeAux.Bssid, wapi_port.Addr,
					     MAC_ADDR_LEN)) {
	...
	case OID_802_11_UCAST_KEY_INFO:
		if (wrq->u.data.length != sizeof(WAPI_UCAST_KEY_STRUCT))
			...
			Status =
			    copy_from_user(&wapi_ukey, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == NDIS_STATUS_SUCCESS) {
				if (INFRA_ON(pAd)) {
					if (NdisEqualMemory
					    (pAd->MlmeAux.Bssid, wapi_ukey.Addr,
					     MAC_ADDR_LEN)) {
						pEntry =
						    &pAd->MacTab.
						    Content[BSSID_WCID];
						...
						WAPIInstallPairwiseKey(pAd,
								       pEntry,
								       FALSE);

						/* Start or re-start USK rekey mechanism, if necessary. */
						RTMPCancelWapiRekeyTimerAction
						    (pAd, pEntry);
						RTMPStartWapiRekeyTimerAction
						    (pAd, pEntry);

						DBGPRINT(RT_DEBUG_TRACE,
							 ("Set::OID_802_11_UCAST_KEY_INFO complete\n"));
						hex_dump("WAPI UCAST KEY",
							 pAd->StaCfg.PTK, 64);
					}
				}
			}
	...
	case OID_802_11_MCAST_KEY_INFO:
		if (wrq->u.data.length != sizeof(WAPI_MCAST_KEY_STRUCT))
			...
			Status =
			    copy_from_user(&wapi_mkey, wrq->u.data.pointer,
					   wrq->u.data.length);
			if (Status == NDIS_STATUS_SUCCESS) {
				...
				NdisMoveMemory(pAd->StaCfg.NMK, wapi_mkey.NMK,
					       16);

				/* Calculate GTK */
				RTMPDeriveWapiGTK(pAd->StaCfg.NMK,
						  pAd->StaCfg.GTK);
	...
	case RT_OID_802_11_SET_PSPXLINK_MODE:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			Status =
			    copy_from_user(&pAd->StaCfg.PSPXlink,
					   wrq->u.data.pointer,
					   wrq->u.data.length);
			/*if (pAd->StaCfg.PSPXlink)
...
INT RTMPQueryInformation(IN PRTMP_ADAPTER pAd,
			 IN OUT RTMP_IOCTL_INPUT_STRUCT * rq, IN INT cmd)
{
	...
	ULONG BssBufSize, ulInfo = 0, NetworkTypeList[4], apsd = 0, RateValue =
	    0;
	USHORT BssLen = 0;
	...
	case RT_OID_DEVICE_NAME:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, pAd->nickname,
				 wrq->u.data.length);
		break;
	case RT_OID_VERSION_INFO:
		...
		driverVersion[7] = '\0';
		if (copy_to_user
		    (wrq->u.data.pointer, &driverVersion[0],
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
...
/*            pBuf = kmalloc(BssBufSize, MEM_ALLOC_FLAG); */
		...
		for (i = 0; i < pAd->ScanTab.BssNr; i++) {
			...
			NdisMoveMemory(&pBss->MacAddress,
				       &pAd->ScanTab.BssEntry[i].Bssid,
				       MAC_ADDR_LEN);
			...
			if (pAd->ScanTab.BssEntry[i].VarIELen == 0) {
				...
				NdisMoveMemory(pBss->IEs,
					       &pAd->ScanTab.BssEntry[i].FixIEs,
					       sizeof(NDIS_802_11_FIXED_IEs));
				pPtr =
...
/*            kfree(pBssidList); */
	...
	case OID_802_3_CURRENT_ADDRESS:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CurrentAddress,
				 wrq->u.data.length);
		break;
	...
	case OID_802_11_SSID:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &Ssid,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
...
/*            pLinkStatus = (RT_802_11_LINK_STATUS *) kmalloc(sizeof(RT_802_11_LINK_STATUS), MEM_ALLOC_FLAG); */
		os_alloc_mem(pAd, (UCHAR **) & pLinkStatus,
		...
		if (pLinkStatus) {
			...
			pLinkStatus->RxByteCount =
			    pAd->RalinkCounters.ReceivedByteCount;
			pLinkStatus->TxByteCount =
			    pAd->RalinkCounters.TransmittedByteCount;
			pLinkStatus->CentralChannel =
			    pAd->CommonCfg.CentralChannel;
			wrq->u.data.length = sizeof(RT_802_11_LINK_STATUS);
			Status =
			    copy_to_user(wrq->u.data.pointer, pLinkStatus,
					 wrq->u.data.length);
/*                kfree(pLinkStatus); */
...
/*            pConfiguration = (NDIS_802_11_CONFIGURATION *) kmalloc(sizeof(NDIS_802_11_CONFIGURATION), MEM_ALLOC_FLAG); */
		os_alloc_mem(pAd, (UCHAR **) & pConfiguration,
			     sizeof(NDIS_802_11_CONFIGURATION));
		if (pConfiguration) {
			pConfiguration->Length =
			    sizeof(NDIS_802_11_CONFIGURATION);
			pConfiguration->BeaconPeriod =
			    pAd->CommonCfg.BeaconPeriod;
			pConfiguration->ATIMWindow = pAd->StaActive.AtimWin;
			MAP_CHANNEL_ID_TO_KHZ(pAd->CommonCfg.Channel,
					      pConfiguration->DSConfig);
			wrq->u.data.length = sizeof(NDIS_802_11_CONFIGURATION);
			Status =
			    copy_to_user(wrq->u.data.pointer, pConfiguration,
					 wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
...
/*				kfree(pConfiguration); */
	...
	case RT_OID_802_11_SNR_0:
		if ((pAd->StaCfg.LastSNR0 > 0)) {
			...
			Status =
			    copy_to_user(wrq->u.data.pointer, &ulInfo,
					 wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
...
/*            pStatistics = (NDIS_802_11_STATISTICS *) kmalloc(sizeof(NDIS_802_11_STATISTICS), MEM_ALLOC_FLAG); */
		...
		if (pStatistics) {
			...
			if (pAd->WlanCounters.TransmittedFragmentCount.
			    QuadPart < pAd->WlanCounters.RetryCount.QuadPart)
			...
			Status =
			    copy_to_user(wrq->u.data.pointer, pStatistics,
					 wrq->u.data.length);
/*                kfree(pStatistics); */
	...
	case RT_OID_802_11_QUERY_TXBF_TABLE:
		if (pAd->chipCap.FlgHwTxBfCap) {
			...
			pMacTab =
			    ...
			    kmalloc(sizeof(RT_802_11_TXBF_TABLE),
				    MEM_ALLOC_FLAG);
			if (pMacTab) {
				pMacTab->Num = 0;
				for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
					if (IS_ENTRY_CLIENT
					    ...
					    && (pAd->MacTab.Content[i].Sst ==
						SST_ASSOC)) {
						NdisMoveMemory(&pMacTab->
							       Entry[pMacTab->
								     Num],
							       &pAd->MacTab.
							       Content[i].
				...
				wrq->u.data.length =
				    sizeof(RT_802_11_TXBF_TABLE);
				Status =
				    copy_to_user(wrq->u.data.pointer, pMacTab,
						 wrq->u.data.length);
				kfree(pMacTab);
			} else {
				Status = -EFAULT;
...
/*            pStaConfig = (RT_802_11_STA_CONFIG *) kmalloc(sizeof(RT_802_11_STA_CONFIG), MEM_ALLOC_FLAG); */
		os_alloc_mem(pAd, (UCHAR **) & pStaConfig,
		...
		if (pStaConfig) {
			...
			pStaConfig->HwRadioStatus =
			    (pAd->StaCfg.bHwRadio == TRUE) ? 1 : 0;
			pStaConfig->Rsv1 = 0;
			pStaConfig->SystemErrorBitmap = pAd->SystemErrorBitmap;
			wrq->u.data.length = sizeof(RT_802_11_STA_CONFIG);
			Status =
			    copy_to_user(wrq->u.data.pointer, pStaConfig,
					 wrq->u.data.length);
/*                kfree(pStaConfig); */
	...
	case OID_802_11_RTS_THRESHOLD:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &RtsThresh,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_FRAGMENTATION_THRESHOLD:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &FragThresh,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_POWER_MODE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &PowerMode,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_RADIO:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &RadioState,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_INFRASTRUCTURE_MODE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &BssType,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_PREAMBLE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &PreamType,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &AuthMode,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_WEP_STATUS:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &WepStatus,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_TX_POWER_LEVEL:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.TxPower,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_NETWORK_TYPE_IN_USE:
		...
		if (WMODE_EQUAL(pAd->CommonCfg.PhyMode, WMODE_A))
			ulInfo = Ndis802_11OFDM5;
			 ...
			 (pAd->CommonCfg.PhyMode, (WMODE_G | WMODE_B))
	...
	case RT_OID_802_11_QUERY_MAP_REAL_RX_RATE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &RateValue,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::RT_OID_802_11_QUERY_LAST_RX_RATE (=%ld)\n",
			  RateValue));
	...
	case RT_OID_802_11_QUERY_EEPROM_VERSION:
		wrq->u.data.length = sizeof(ULONG);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->EepromVersion,
				 wrq->u.data.length);
		break;
	...
	case RT_OID_802_11_QUERY_NOISE_LEVEL:
		{
			...
			Status =
			    copy_to_user(wrq->u.data.pointer, &noise,
					 wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_EXTRA_INFO:
		wrq->u.data.length = sizeof(ULONG);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->ExtraInfo,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_QUERY_APSD_SETTING:
		apsd =
		    (pAd->StaCfg.UapsdInfo.
		     bAPSDCapable | (pAd->CommonCfg.bAPSDAC_BE << 1) | (pAd->
									CommonCfg.
									bAPSDAC_BK
									<< 2)
		...
		wrq->u.data.length = sizeof(ULONG);
		Status =
		    copy_to_user(wrq->u.data.pointer, &apsd,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_NEW_DRIVER:
		{
			...
			wrq->u.data.length = sizeof(UCHAR);
			Status =
			    copy_to_user(wrq->u.data.pointer, &enabled,
					 wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_QUERY_STATUS:
		...
		if (copy_to_user
		    (wrq->u.data.pointer, &pAd->StaCfg.WscControl.WscStatus,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_PIN_CODE:
		...
		if (copy_to_user
		    (wrq->u.data.pointer, &WscPinCode, wrq->u.data.length)) {
			Status = -EFAULT;
		}
		DBGPRINT(RT_DEBUG_TRACE,
...
/*			pProfile = kmalloc(sizeof(WSC_PROFILE), MEM_ALLOC_FLAG); */
		...
		WscCreateProfileFromCfg(pAd, STA_MODE, &pAd->StaCfg.WscControl,
					pProfile);
		if (copy_to_user
		    (wrq->u.data.pointer, pProfile, wrq->u.data.length)) {
			Status = -EFAULT;
		} else {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("AuthType = %s\n",
				  WscGetAuthTypeStr(pProfile->Profile[0].
						    AuthType)));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("EncrpType = %s\n",
				  WscGetEncryTypeStr(pProfile->Profile[0].
						     EncrType)));
			...
			if (pProfile->Profile[0].EncrType == WSC_ENCRTYPE_WEP) {
			    ...
			    if ((pProfile->Profile[0].EncrType ==
				 WSC_ENCRTYPE_TKIP)
...
/*			pProfile = kmalloc(sizeof(WSC_PROFILE), MEM_ALLOC_FLAG); */
		...
		RTMPZeroMemory(pProfile, sizeof(WSC_PROFILE));
		NdisMoveMemory(pProfile, &pAd->StaCfg.WscControl.WscProfile,
			       sizeof(WSC_PROFILE));
...
/*			kfree(pProfile); */
	...
	case RT_OID_WSC_UUID:
		...
		if (copy_to_user
		    (wrq->u.data.pointer,
		     &pAd->StaCfg.WscControl.Wsc_Uuid_Str[0], UUID_LEN_STR)) {
			Status = -EFAULT;
		}
		break;
	case RT_OID_WSC_MAC_ADDRESS:
		...
		wrq->u.data.length = MAC_ADDR_LEN;
		if (copy_to_user
		    (wrq->u.data.pointer, pAd->CurrentAddress,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
	...
	case RT_OID_DRIVER_DEVICE_NAME:
		...
		wrq->u.data.length = 16;
		if (copy_to_user
		    (wrq->u.data.pointer, pAd->StaCfg.dev_name,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
	case RT_OID_802_11_QUERY_HT_PHYMODE:
		os_alloc_mem(pAd, (UCHAR **) & pHTPhyMode,
			     sizeof(OID_SET_HT_PHYMODE));
		if (pHTPhyMode) {
			pHTPhyMode->PhyMode = pAd->CommonCfg.PhyMode;
			pHTPhyMode->HtMode =
			    (UCHAR) pAd->MacTab.Content[BSSID_WCID].HTPhyMode.
			    field.MODE;
			pHTPhyMode->BW =
			    (UCHAR) pAd->MacTab.Content[BSSID_WCID].HTPhyMode.
			    field.BW;
			...
			pHTPhyMode->ExtOffset =
			    ((pAd->CommonCfg.CentralChannel <
			      pAd->CommonCfg.
			      Channel) ? (EXTCHA_BELOW) : (EXTCHA_ABOVE));
			wrq->u.data.length = sizeof(OID_SET_HT_PHYMODE);
			if (copy_to_user
			    (wrq->u.data.pointer, pHTPhyMode,
			     wrq->u.data.length))
				Status = -EFAULT;
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_QUERY_DAT_HT_PHYMODE:
		...
		if (pHTPhyMode) {
			pHTPhyMode->PhyMode =
			    wmode_2_cfgmode(pAd->CommonCfg.PhyMode);
			pHTPhyMode->HtMode =
			    (UCHAR) pAd->CommonCfg.RegTransmitSetting.field.
			    HTMODE;
			pHTPhyMode->BW =
	...
	case RT_OID_802_11_MANUFACTURERNAME:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, ManufacturerNAME,
				 wrq->u.data.length);
		break;
	...
	case RT_OID_802_11_RESOURCETYPEIDNAME:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, ResourceTypeIdName,
				 wrq->u.data.length);
		break;
	...
	case OID_802_11_WEPDEFAULTKEYVALUE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, pKeyIdxValue,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_SHORTRETRYLIMIT:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ShortRetryLimit =%ld,  tx_rty_cfg.field.ShortRetryLimit=%d\n",
			  ShortRetryLimit, tx_rty_cfg.field.ShortRtyLimit));
		Status =
		    copy_to_user(wrq->u.data.pointer, &ShortRetryLimit,
				 wrq->u.data.length);
		break;
	...
	case OID_802_11_LONGRETRYLIMIT:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("LongRetryLimit =%ld,  tx_rty_cfg.field.LongRtyLimit=%d\n",
			  LongRetryLimit, tx_rty_cfg.field.LongRtyLimit));
		Status =
		    copy_to_user(wrq->u.data.pointer, &LongRetryLimit,
				 wrq->u.data.length);
		break;
	...
	case RT_OID_802_11_PRODUCTID:
		...
		snprintf((PSTRING) tmp, sizeof(tmp), "%04x %04x\n",
			 RtmpOsGetUsbDevVendorID(((POS_COOKIE) pAd->OS_Cookie)->
						 pUsb_Dev),
			 RtmpOsGetUsbDevProductID(((POS_COOKIE) pAd->
						   OS_Cookie)->pUsb_Dev));

#endif				/* RTMP_MAC_USB */
		wrq->u.data.length = strlen((PSTRING) tmp);
		Status =
		    copy_to_user(wrq->u.data.pointer, tmp, wrq->u.data.length);
		break;
	...
	case OID_802_11_CURRENTCHANNEL:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("sizeof UCHAR=%d, channel=%d \n", sizeof(UCHAR),
			  pAd->CommonCfg.Channel));
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.Channel,
				 wrq->u.data.length);
...
/*				pChListBuf = (RT_CHANNEL_LIST_INFO *) kmalloc(sizeof(RT_CHANNEL_LIST_INFO), MEM_ALLOC_FLAG); */
			os_alloc_mem(pAd, (UCHAR **) & pChListBuf,
...
/*					kfree(pChListBuf); */
	...
	case OID_802_11_GET_CHANNEL_GEOGRAPHY:
		...
		wrq->u.data.length = 1;
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.Geography,
				 1);
		DBGPRINT(RT_DEBUG_TRACE, ("Status=%d\n", Status));
		break;
#endif				/* EXT_BUILD_CHANNEL_LIST */
	...
	case OID_802_11_P2P_MODE:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->P2pCfg.Rule,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_MODE (Len=%zu, Rule=%d)\n",
	...
	case OID_802_11_P2P_DEVICE_NAME:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, pAd->P2pCfg.DeviceName,
				 pAd->P2pCfg.DeviceNameLen);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_DEVICE_NAME (Len=%lu, DeviceName=%s)\n",
	...
	case OID_802_11_P2P_LISTEN_CHANNEL:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer,
				 ...
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_LISTEN_CHANNEL (Len=%zu, Listen_Ch=%d)\n",
	...
	case OID_802_11_P2P_OPERATION_CHANNEL:
		wrq->u.data.length = sizeof(char);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->P2pCfg.GroupChannel,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_OPERATION_CHANNEL (Len=%zu, Op_Ch=%d)\n",
	...
	case OID_802_11_P2P_DEV_ADDR:
		wrq->u.data.length = 6;
		Status =
		    copy_to_user(wrq->u.data.pointer, pAd->P2pCfg.Bssid,
				 wrq->u.data.length);
	...
	case OID_802_11_P2P_CTRL_STATUS:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_MODE (Len=%lu, DeviceName=%s)\n",
			  pAd->P2pCfg.DeviceNameLen, pAd->P2pCfg.DeviceName));
		break;
	...
	case OID_802_11_P2P_GOFORM_STATUS:
		...
		NdisZeroMemory(tmpP2P, 24);
		sprintf(tmpP2P, "%s",
			decodeGroupFormationState(pP2PCtrl->
						  GoFormCurrentState));
		Status = copy_to_user(wrq->u.data.pointer, tmpP2P, 24);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::OID_802_11_P2P_MODE (Len=%lu, DeviceName=%s)\n",
	...
	case OID_802_11_P2P_SCAN_LIST:
		os_alloc_mem(NULL, (UCHAR **) & pUI_table,
		...
		if (pUI_table) {
			...
			for (i = 0; i < pAd->P2pTable.ClientNumber; i++) {
				...
				NdisMoveMemory(pUICli, pPAdCli,
					       sizeof(RT_P2P_CLIENT_ENTRY));
			}
			printk("Query::OID_802_11_P2P_SCAN_LIST\n");
			Status =
			    copy_to_user(wrq->u.data.pointer, pUI_table,
					 sizeof(RT_P2P_UI_TABLE));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Query::OID_802_11_P2P_SCAN_LIST (Len=%zu, Rule=%d)\n",
				  sizeof(char), pAd->P2pCfg.GroupOpChannel));
	...
	case RT_OID_802_11_QUERY_DLS_PARAM:
		{
			...
			os_alloc_mem(pAd, (UCHAR **) & pDlsInfo,
				     sizeof(RT_802_11_DLS_INFO));
			if (pDlsInfo == NULL)
				break;

			for (i = 0; i < MAX_NUM_OF_DLS_ENTRY; i++) {
				RTMPMoveMemory(&pDlsInfo->Entry[i],
					       &pAd->StaCfg.DLSEntry[i],
					       sizeof(RT_802_11_DLS_UI));
			}
			...
			Status =
			    copy_to_user(wrq->u.data.pointer, pDlsInfo,
					 wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
...
/*					kfree(pDlsInfo); */
	...
	case OID_802_11_WAPI_IE:
		...
		if (wrq->u.data.length != sizeof(WAPI_WIE_STRUCT))
			...
			NdisMoveMemory(wapi_ie.addr, wrq->u.data.pointer,
				       MAC_ADDR_LEN);

			pEntry = &pAd->MacTab.Content[BSSID_WCID];

			if ((NdisEqualMemory
			     (pEntry->Addr, wapi_ie.addr, MAC_ADDR_LEN))
			    && (pEntry->RSNIE_Len > 0)) {
				wapi_ie.wie_len = pEntry->RSNIE_Len;
				NdisMoveMemory(wapi_ie.wie, pEntry->RSN_IE,
					       pEntry->RSNIE_Len);
			}

			if (copy_to_user
			    (wrq->u.data.pointer, &wapi_ie,
			     wrq->u.data.length)) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: copy_to_user() fail\n",
					  __FUNCTION__));
	...
	case OID_802_11_SET_PSPXLINK_MODE:
		wrq->u.data.length = sizeof(BOOLEAN);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->StaCfg.PSPXlink,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_IWSC_SELF_IPV4:
		if (wrq->u.data.length != sizeof(UINT32)) {
			...
			Status =
			    copy_to_user(wrq->u.data.pointer,
					 &pAd->StaCfg.IWscInfo.SelfIpv4Addr,
					 sizeof(UINT32));
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_IWSC_REGISTRAR_IPV4:
		if (wrq->u.data.length != sizeof(UINT32)) {
			...
			Status =
			    copy_to_user(wrq->u.data.pointer,
					 &pAd->StaCfg.IWscInfo.RegIpv4Addr,
					 sizeof(UINT32));
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_IWSC_SMPBC_ENROLLEE_COUNT:
		if (wrq->u.data.length != sizeof(UINT8)) {
			...
			Status =
			    copy_to_user(wrq->u.data.pointer,
					 &pAd->StaCfg.IWscInfo.
					 SmpbcEnrolleeCount, sizeof(UINT8));
			DBGPRINT(RT_DEBUG_INFO,
				 ("Query::RT_OID_IWSC_SMPBC_ENROLLEE_COUNT(=0x%08x), Status = %d\n",
				  pAd->StaCfg.IWscInfo.SmpbcEnrolleeCount,
				  Status));
		}
		break;
#endif				/* IWSC_SUPPORT */
...
VOID RTMPIoctlMAC(IN RTMP_ADAPTER * pAd, IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
 ...
 next:
	if (bIsPrintAllMAC) {
		...
		os_alloc_mem(NULL, (UCHAR **) & pBufMac,
			     (((AddrEnd - AddrStart) / 4) * 15) + 100);
		if (pBufMac != NULL) {
			pBuf = pBufMac;
			for (IdAddr = AddrStart; IdAddr <= AddrEnd;
			     IdAddr += 4, pBuf++)
			...
			if (IS_RT65XX(pAd) || IS_MT7601(pAd)) {
				...
				for (IdAddr = AddrStart; IdAddr <= AddrEnd;
				     IdAddr += 4, pBuf++)
					RTMP_IO_READ32(pAd, IdAddr, pBuf);
				RtmpDrvAllMacPrint(pAd, pBufMac, AddrStart,
						   AddrEnd, 4);
			}
#endif				/* defined(RT65xx) || defined(MT7601) */
	...
	if (wrq->u.data.length > 1) {	/*No parameters. */
		...
		if (!value || !*value) {	/*Read */
			...
			if (strlen(this_char) == 4) {
				...
				if (eepAddr < 0xFFFF) {
					RT28xx_EEPROM_READ16(pAd, eepAddr,
							     eepValue);
					sprintf(msg + strlen(msg),
...
VOID RTMPIoctlRF(IN PRTMP_ADAPTER pAdapter, IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	bFromUI =
	    ((wrq->u.data.flags & RTPRIV_IOCTL_FLAG_UI) ==
	     RTPRIV_IOCTL_FLAG_UI) ? TRUE : FALSE;
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL) {
			...
			if (!value || !*value) {	/*Read */
				...
				if ((sscanf
				     ((PSTRING) this_char, "%d",
				      &(bank_Id)) == 1)
				    && (sscanf((PSTRING) value, "%d", &(rfId))
					== 1)) {

					if ((rfId <= maxRFIdx)
					    && (bank_Id <= MAC_RF_BANK)) {
						rlt_rf_read(pAdapter, bank_Id,
							    rfId, &regRF);
...
// --Alan test
 ...
 next:
	if (bIsPrintAllRF) {
		...
		if (copy_to_user
		    (wrq->u.data.pointer, "Dump to RFDump.txt",
		     wrq->u.data.length)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: copy_to_user() fail\n", __FUNCTION__));
		}
...
#ifdef RT65xx
VOID RTMPIoctlBbp32(IN RTMP_ADAPTER * pAd,
		    IN RTMP_IOCTL_INPUT_STRUCT * wrq,
		    IN CHAR * extra, IN UINT32 size)
{
	PSTRING this_char;
	PSTRING value = NULL;
 ...
 next:
	if (bIsPrintAllBBP) {
		...
		for (i = 0; i < sizeof(bbp_regs) / sizeof(RTMP_REG_PAIR); i++) {
			for (reg = bbp_regs[i].Register;
			     reg <= bbp_regs[i].Value; reg += 4) {
...
VOID RTMPIoctlBbp(IN RTMP_ADAPTER * pAd,
		  IN RTMP_IOCTL_INPUT_STRUCT * wrq,
		  IN CHAR * extra, IN UINT32 size)
{
	PSTRING this_char;
...
#endif				/* RALINK_ATE */
				RTMP_BBP_IO_READ8_BY_REG_ID(pAd, bbpId,
							    &regBBP);
...
#ifdef DOT11_N_SUPPORT
void getBaInfo(IN PRTMP_ADAPTER pAd, IN PSTRING pOutBuf, IN UINT32 size)
{
	INT i, j;
	...
	case RAIO_OFF:
		...
		if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)) {
			if (INFRA_ON(pAd) || ADHOC_ON(pAd)) {
				...
				os_alloc_mem(NULL, (UCHAR **) & pMsgElem,
					     sizeof(MLME_QUEUE_ELEM));
				if (pMsgElem) {
...
*/
INT
RtmpIoctl_rt_ioctl_siwfreq(IN RTMP_ADAPTER * pAd,
...
*/
INT
RtmpIoctl_rt_ioctl_giwfreq(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	UCHAR ch;
	ULONG m = 2412000;
...
*/
INT
RtmpIoctl_rt_ioctl_siwmode(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	switch (Data) {
...
*/
INT
RtmpIoctl_rt_ioctl_giwmode(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	if (ADHOC_ON(pAd))
...
*/
INT
RtmpIoctl_rt_ioctl_siwap(IN RTMP_ADAPTER * pAd, IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_giwap(IN RTMP_ADAPTER * pAd, IN VOID * pData, IN ULONG Data)
{
	if (INFRA_ON(pAd) || ADHOC_ON(pAd))
...
*/
INT
RtmpIoctl_rt_ioctl_siwscan(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	/*
...
/* #if defined(SIOCGIWSCAN) || defined(RT_CFG80211_SUPPORT) */
	...
	if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE))
		...
		RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_SCAN, -1,
					NULL, NULL, 0);
		return NDIS_STATUS_SUCCESS;
	}
	...
	do {
		...
		if (pConfig->FlgScanThisSsid) {
			...
			NdisMoveMemory(Ssid.Ssid, pConfig->pSsid,
				       Ssid.SsidLength);
...
========================================================================
*/
static void set_quality(IN RT_CMD_STA_IOCTL_BSS * pSignal,
			IN PBSS_ENTRY pBssEntry)
{
	...
	if (pBssEntry->Rssi >= -50)
		...
		pSignal->ChannelQuality =
		    (__u8) (24 + ((pBssEntry->Rssi + 80) * 26) / 10);
	else if (pBssEntry->Rssi >= -90)	/* between -80 ~ -90dbm */
		pSignal->ChannelQuality =
		    (__u8) ((pBssEntry->Rssi + 90) * 26) / 10;
	else
...
*/
INT
RtmpIoctl_rt_ioctl_giwscan(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SCAN_TABLE *pIoctlScan =
	    (RT_CMD_STA_IOCTL_SCAN_TABLE *) pData;
	...
	for (IdBss = 0; IdBss < pAd->ScanTab.BssNr; IdBss++) {
		HT_CAP_INFO capInfo =
		    pAd->ScanTab.BssEntry[IdBss].HtCapability.HtCapInfo;
		...
		pBssTable->FlgIsPrivacyOn =
		    CAP_IS_PRIVACY_ON(pBssEntry->CapabilityInfo);
...
*/
INT
RtmpIoctl_rt_ioctl_siwessid(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
...
/*				pSsidString = kmalloc(MAX_LEN_OF_SSID+1, MEM_ALLOC_FLAG); */
		...
		if (pSsidString) {
			...
			NdisMoveMemory(pSsidString, pSsid->pSsid,
				       pSsid->SsidLen);
...
*/
INT
RtmpIoctl_rt_ioctl_giwessid(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SSID *pSsid = (RT_CMD_STA_IOCTL_SSID *) pData;
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		...
		memcpy(pSsid->pSsid, pAd->CommonCfg.Ssid,
		       pAd->CommonCfg.SsidLen);
	}
#ifdef RTMP_MAC_USB
#ifdef WPA_SUPPLICANT_SUPPORT
...
*/
INT
RtmpIoctl_rt_ioctl_siwnickn(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_giwnickn(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_NICK_NAME *IoctlName =
	    (RT_CMD_STA_IOCTL_NICK_NAME *) pData;
...
*/
INT
RtmpIoctl_rt_ioctl_siwrts(IN RTMP_ADAPTER * pAd, IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_giwrts(IN RTMP_ADAPTER * pAd, IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_siwfrag(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_giwfrag(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
...
INT
RtmpIoctl_rt_ioctl_siwencode(IN RTMP_ADAPTER * pAd,
			     IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY *pIoctlSec =
	    (RT_CMD_STA_IOCTL_SECURITY *) pData;
	...
	if (pIoctlSec->length > 0) {
		...
		if (!(pIoctlSec->flags & RT_CMD_STA_IOCTL_SECURITY_NOKEY)) {
			...
			NdisMoveMemory(pAd->SharedKey[BSS0][keyIdx].Key,
				       pIoctlSec->pData, pIoctlSec->length);
		}
	} else {
...
*/
INT
RtmpIoctl_rt_ioctl_giwencode(IN RTMP_ADAPTER * pAd,
			     IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY *pIoctlSec =
	    (RT_CMD_STA_IOCTL_SECURITY *) pData;
	...
	if (pAd->StaCfg.WepStatus == Ndis802_11WEPDisabled) {
		...
		if (pIoctlSec->length > pAd->SharedKey[BSS0][kid - 1].KeyLen)
			pIoctlSec->length =
			    pAd->SharedKey[BSS0][kid - 1].KeyLen;
		memcpy(pIoctlSec->pData, pAd->SharedKey[BSS0][kid - 1].Key,
		       pIoctlSec->length);
...
*/
INT
RtmpIoctl_rt_ioctl_siwmlme(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data, IN UINT32 Subcmd)
{
	...
	case RT_CMD_STA_IOCTL_IW_MLME_DEAUTH:
		...
		NdisMoveMemory(pMsgElem->Msg, &DeAuthReq,
	...
	case RT_CMD_STA_IOCTL_IW_MLME_DISASSOC:
		...
		NdisMoveMemory(pMsgElem->Msg, &DisAssocReq,
			       sizeof(MLME_DISASSOC_REQ_STRUCT));
...
*/
INT
RtmpIoctl_rt_ioctl_siwauth(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY_ADV *pIoctlWpa =
	    (RT_CMD_STA_IOCTL_SECURITY_ADV *) pData;
	...
	case RT_CMD_STA_IOCTL_WPA_PAIRWISE:
		if (pIoctlWpa->value == RT_CMD_STA_IOCTL_WPA_PAIRWISE_NONE) {
			pAd->StaCfg.WepStatus = Ndis802_11WEPDisabled;
			pAd->StaCfg.PairCipher = Ndis802_11WEPDisabled;
		} else if (pIoctlWpa->value ==
			   RT_CMD_STA_IOCTL_WPA_PAIRWISE_WEP40
			   || pIoctlWpa->value ==
			   RT_CMD_STA_IOCTL_WPA_PAIRWISE_WEP104) {
			pAd->StaCfg.WepStatus = Ndis802_11WEPEnabled;
			pAd->StaCfg.PairCipher = Ndis802_11WEPEnabled;
#ifdef WPA_SUPPLICANT_SUPPORT
			pAd->StaCfg.IEEE8021X = FALSE;
			   ...
			   RT_CMD_STA_IOCTL_WPA_PAIRWISE_TKIP) {
			pAd->StaCfg.WepStatus = Ndis802_11Encryption2Enabled;
			pAd->StaCfg.PairCipher = Ndis802_11Encryption2Enabled;
		} else if (pIoctlWpa->value ==
			   RT_CMD_STA_IOCTL_WPA_PAIRWISE_CCMP) {
			pAd->StaCfg.WepStatus = Ndis802_11Encryption3Enabled;
			pAd->StaCfg.PairCipher = Ndis802_11Encryption3Enabled;
		}
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_CMD_STA_IOCTL_WPA_GROUP:
		if (pIoctlWpa->value == RT_CMD_STA_IOCTL_WPA_GROUP_NONE) {
			...
			pAd->StaCfg.GroupCipher = Ndis802_11GroupWEP40Enabled;
			   ...
			   RT_CMD_STA_IOCTL_WPA_GROUP_WEP104) {
...
*/
INT
RtmpIoctl_rt_ioctl_giwauth(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY_ADV *pIoctlWpa =
	    (RT_CMD_STA_IOCTL_SECURITY_ADV *) pData;
	...
	case RT_CMD_STA_IOCTL_WPA_AUTH_DROP_UNENCRYPTED:
		pIoctlWpa->value =
		    (pAd->StaCfg.WepStatus == Ndis802_11WEPDisabled) ? 0 : 1;
		break;

	case RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG:
		pIoctlWpa->value =
		    (pAd->StaCfg.AuthMode == Ndis802_11AuthModeShared) ? 0 : 1;
		break;

	case RT_CMD_STA_IOCTL_WPA_AUTH_WPA_ENABLED:
		pIoctlWpa->value =
		    (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPA) ? 1 : 0;
		break;
	}
...
========================================================================
*/
void fnSetCipherKey(IN PRTMP_ADAPTER pAd,
		    IN INT keyIdx,
		    IN UCHAR CipherAlg, IN BOOLEAN bGTK, IN UCHAR * pKey)
{
	...
	NdisMoveMemory(pAd->SharedKey[BSS0][keyIdx].TxMic, pKey + LEN_TK,
		       LEN_TKIP_MIC);
	NdisMoveMemory(pAd->SharedKey[BSS0][keyIdx].RxMic,
		       pKey + LEN_TK + LEN_TKIP_MIC, LEN_TKIP_MIC);
...
*/
		...
		pEntry->PairwiseKey.KeyLen = LEN_TK;
		NdisMoveMemory(pEntry->PairwiseKey.Key,
			       pAd->SharedKey[BSS0][keyIdx].Key, LEN_TK);
		NdisMoveMemory(pEntry->PairwiseKey.RxMic,
		...
		AsicAddSharedKeyEntry(pAd,
				      BSS0,
				      keyIdx, &pAd->SharedKey[BSS0][keyIdx]);
	}
#else
	/* Update group key information to ASIC Shared Key Table */
...
*/
INT
RtmpIoctl_rt_ioctl_siwencodeext(IN RTMP_ADAPTER * pAd,
				IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY *pIoctlSec =
	    (RT_CMD_STA_IOCTL_SECURITY *) pData;
	...
	if (pIoctlSec->flags == RT_CMD_STA_IOCTL_SECURITY_DISABLED) {
		...
		if (pIoctlSec->
		    ext_flags & RT_CMD_STA_IOCTL_SECURTIY_EXT_SET_TX_KEY) {
			pAd->StaCfg.DefaultKeyId = keyIdx;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s::DefaultKeyId = %d\n", __FUNCTION__,
				  pAd->StaCfg.DefaultKeyId));
		...
		case RT_CMD_STA_IOCTL_SECURITY_ALG_TKIP:
			...
			if (pIoctlSec->length == 32) {
				if (pAd->StaCfg.AuthMode ==
				    Ndis802_11AuthModeWPANone) {
					RTMPZeroMemory(pAd->StaCfg.PMK,
						       LEN_PMK);
					RTMPMoveMemory(pAd->StaCfg.PMK,
						       pIoctlSec->pData,
						       pIoctlSec->length);
				} else {
					if (pIoctlSec->
		...
		case RT_CMD_STA_IOCTL_SECURITY_ALG_CCMP:
			if (pAd->StaCfg.AuthMode == Ndis802_11AuthModeWPANone) {
				...
				if (pIoctlSec->
				    ext_flags &
				    RT_CMD_STA_IOCTL_SECURTIY_EXT_SET_TX_KEY) {
					fnSetCipherKey(pAd, keyIdx, CIPHER_AES,
						       FALSE, pIoctlSec->pData);
					if (pAd->StaCfg.AuthMode >=
					    Ndis802_11AuthModeWPA2) {
						STA_PORT_SECURED(pAd);
					}
...
*/
INT
RtmpIoctl_rt_ioctl_giwencodeext(IN RTMP_ADAPTER * pAd,
				IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_SECURITY *pIoctlSec =
	    (RT_CMD_STA_IOCTL_SECURITY *) pData;
	...
	case Ndis802_11WEPEnabled:
		...
		if (pAd->SharedKey[BSS0][idx].KeyLen > pIoctlSec->MaxKeyLen) {
			...
			pIoctlSec->pData =
			    (PCHAR) & (pAd->SharedKey[BSS0][idx].Key[0]);
		}
		break;
...
*/
INT
RtmpIoctl_rt_ioctl_siwgenie(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
#ifdef WPA_SUPPLICANT_SUPPORT
...
/*			pAd->StaCfg.pWpaAssocIe = kmalloc(wrqu->data.length, MEM_ALLOC_FLAG); */
			os_alloc_mem(NULL, (UCHAR **) & pAd->StaCfg.pWpaAssocIe,
				     length);
			if (pAd->StaCfg.pWpaAssocIe) {
				pAd->StaCfg.WpaAssocIeLen = length;
				NdisMoveMemory(pAd->StaCfg.pWpaAssocIe, pData,
					       pAd->StaCfg.WpaAssocIeLen);
				pAd->StaCfg.bRSN_IE_FromWpaSupplicant = TRUE;
				eid_ptr =
				    (PEID_STRUCT) (pAd->StaCfg.pWpaAssocIe);
				while (((UCHAR *) eid_ptr + eid_ptr->Len + 1) <
				       ((UCHAR *) pAd->StaCfg.pWpaAssocIe +
					pAd->StaCfg.WpaAssocIeLen)) {
					if (eid_ptr->Eid == IE_WPA) {
						if (NdisEqualMemory
						    (eid_ptr->Octet, WPS_OUI,
						     4)) {
							pAd->StaCfg.
							    WpaSupplicantUP |=
							    WPA_SUPPLICANT_ENABLE_WPS;
...
*/
INT
RtmpIoctl_rt_ioctl_giwgenie(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
...
/* #ifdef SIOCSIWGENIE */
	if ((pAd->StaCfg.WpaSupplicantUP & 0x7F) == WPA_SUPPLICANT_ENABLE &&
		...
		memcpy(IoctlRsnIe->pRsnIe, pAd->StaCfg.pWpaAssocIe,
		       pAd->StaCfg.WpaAssocIeLen);
	} else
		/* #endif *//* SIOCSIWGENIE */
#endif				/* NATIVE_WPA_SUPPLICANT_SUPPORT */
	{
		...
		memcpy(IoctlRsnIe->pRsnIe + 2, &pAd->StaCfg.RSN_IE[0],
...
*/
INT
RtmpIoctl_rt_ioctl_siwpmksa(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
	RT_CMD_STA_IOCTL_PMA_SA *pIoctlPmaSa =
	    (RT_CMD_STA_IOCTL_PMA_SA *) pData;
	...
	case RT_CMD_STA_IOCTL_PMA_SA_ADD:
		...
		if (CachedIdx < PMKID_NO) {
			...
			NdisMoveMemory(&pAd->StaCfg.SavedPMK[CachedIdx].
				       BSSID[0], pIoctlPmaSa->pBssid,
				       MAC_ADDR_LEN);
			NdisMoveMemory(&pAd->StaCfg.SavedPMK[CachedIdx].
				       PMKID[0], pIoctlPmaSa->pPmkid, 16);
			pAd->StaCfg.SavedPMKNum++;
		}
		/* Not found, replace the last one */
		else {
...
*/
INT
RtmpIoctl_rt_ioctl_siwrate(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_giwrate(IN RTMP_ADAPTER * pAd,
			   IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_gifhwaddr(IN RTMP_ADAPTER * pAd,
			     IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_rssi(IN RTMP_ADAPTER * pAd, IN VOID * pData, IN ULONG Data)
{
...
*/
INT
RtmpIoctl_rt_ioctl_setparam(IN RTMP_ADAPTER * pAd,
			    IN VOID * pData, IN ULONG Data)
{
	POS_COOKIE pObj;
...
*/
INT
RtmpIoctl_rt_private_set_wsc_u32_item(IN RTMP_ADAPTER * pAd,
				      IN VOID * pData, IN ULONG Data)
{
#ifdef WSC_STA_SUPPORT
	RT_CMD_STA_IOCTL_WSC_U32_ITEM *pIoctlWscU32 =
	    (RT_CMD_STA_IOCTL_WSC_U32_ITEM *) pData;
	...
	case WSC_STOP:
...
#endif				/* IWSC_SUPPORT */
		{
			...
			WscStop(pAd,
				...
				&pAd->StaCfg.WscControl);
			pAd->StaCfg.WscControl.WscConfMode = WSC_DISABLE;
			BssTableDeleteEntry(&pAd->MlmeAux.SsidBssTab,
					    pAd->MlmeAux.Bssid,
					    pAd->MlmeAux.Channel);
		}
		break;
...
*/
INT
RtmpIoctl_rt_private_set_wsc_string_item(IN RTMP_ADAPTER * pAd,
					 IN VOID * pData, IN ULONG Data)
{
#ifdef WSC_STA_SUPPORT
	RT_CMD_STA_IOCTL_WSC_STR_ITEM *pIoctlWscStr =
	    (RT_CMD_STA_IOCTL_WSC_STR_ITEM *) pData;
/*    int  Status=0; */
	...
	if ((subcmd != WSC_SET_SSID) &&
	    (subcmd != WSC_SET_PIN) &&
	    (subcmd != WSC_SET_BSSID) && (tmpProfileIndex > 7)) {
		DBGPRINT(RT_DEBUG_TRACE,
	...
	if ((subcmd != WSC_SET_SSID) &&
	    (subcmd != WSC_SET_PIN) && (subcmd != WSC_SET_BSSID))
	...
	case WSC_CREDENTIAL_SSID:
		...
		if (dataLen > 0 && dataLen <= NDIS_802_11_LENGTH_SSID) {
			pWscProfile->Profile[tmpProfileIndex].SSID.SsidLength =
			    dataLen;
			NdisZeroMemory(pWscProfile->Profile[tmpProfileIndex].
				       SSID.Ssid, NDIS_802_11_LENGTH_SSID);
			NdisMoveMemory(pWscProfile->Profile[tmpProfileIndex].
				       SSID.Ssid, extra + 2, dataLen);
		} else
	...
	case WSC_CREDENTIAL_AUTH_MODE:
		...
		if ((tmpAuth = WscGetAuthTypeFromStr(extra + 2)) != 0) {
			pWscProfile->Profile[tmpProfileIndex].AuthType =
			    tmpAuth;
		} else
			pIoctlWscStr->Status = RTMP_IO_EINVAL;
		break;
	case WSC_CREDENTIAL_ENCR_TYPE:
		...
		if ((tmpEncr = WscGetEncrypTypeFromStr(extra + 2)) != 0) {
			pWscProfile->Profile[tmpProfileIndex].EncrType =
			    tmpEncr;
		} else
			pIoctlWscStr->Status = RTMP_IO_EINVAL;
		break;
	...
	case WSC_CREDENTIAL_KEY:
		...
		if ((dataLen >= 8 && dataLen <= 64)
		    || (dataLen == 5 || dataLen == 10 || dataLen == 13
			|| dataLen == 26)) {
			pWscProfile->Profile[tmpProfileIndex].KeyLength =
			    dataLen;
			NdisZeroMemory(pWscProfile->Profile[tmpProfileIndex].
				       Key, 64);
			NdisMoveMemory(pWscProfile->Profile[tmpProfileIndex].
	...
	case WSC_CREDENTIAL_MAC:
			  ...
			  extra + 2));
		{
			...
			UINT tmp_val[6] = { 0 };
			sscanf_rv =
			    sscanf(extra + 2, "%02x:%02x:%02x:%02x:%02x:%02x",
				   &tmp_val[0], &tmp_val[1], &tmp_val[2],
				   &tmp_val[3], &tmp_val[4], &tmp_val[5]);
			if (sscanf_rv == 6) {
				int ii;
				NdisZeroMemory(pWscProfile->
					       Profile[tmpProfileIndex].MacAddr,
					       6);
				for (ii = 0; ii < 6; ii++)
					pWscProfile->Profile[tmpProfileIndex].
					    MacAddr[ii] = (UCHAR) tmp_val[ii];
			} else
				pIoctlWscStr->Status = RTMP_IO_EINVAL;
		}
		break;
...
*/
INT
RtmpIoctl_rt_private_get_statistics(IN RTMP_ADAPTER * pAd,
				    IN VOID * pData, IN ULONG Data)
{
...
#endif				/* RALINK_ATE */
		txCount =
		    (ULONG) pAd->WlanCounters.TransmittedFragmentCount.u.
		    LowPart;
	...
	sprintf(extra + strlen(extra),
		...
		(ULONG) pAd->WlanCounters.RetryCount.u.LowPart, per / 10,
		per % 10);
	plr =
	    txCount ==
	...
	sprintf(extra + strlen(extra),
		...
		(ULONG) pAd->WlanCounters.FailedCount.u.LowPart, plr / 100,
		plr % 100);
#else
	sprintf(extra + strlen(extra),
		"Tx retry count          		  = %lu\n",
	...
	sprintf(extra + strlen(extra),
		...
		(ULONG) pAd->WlanCounters.FCSErrorCount.u.LowPart, per / 10,
		per % 10);
	sprintf(extra + strlen(extra),
	...
	if (ATE_ON(pAd)) {
		if (pAd->ate.RxAntennaSel == 0) {
			sprintf(extra + strlen(extra),
				"RSSI-A                          = %ld\n",
				(LONG) (pAd->ate.LastRssi0 -
					pAd->BbpRssiToDbmDelta));
			sprintf(extra + strlen(extra),
				"RSSI-B (if available)           = %ld\n",
				(LONG) (pAd->ate.LastRssi1 -
					pAd->BbpRssiToDbmDelta));
			sprintf(extra + strlen(extra),
				"RSSI-C (if available)           = %ld\n\n",
				(LONG) (pAd->ate.LastRssi2 -
					pAd->BbpRssiToDbmDelta));
		} else {
			sprintf(extra + strlen(extra),
				"RSSI                            = %ld\n",
...
#endif				/* RALINK_ATE */
	{
		...
		sprintf(extra + strlen(extra),
			"RSSI                            = %ld %ld %ld\n",
			(LONG) (pAd->StaCfg.RssiSample.LastRssi0 -
				pAd->BbpRssiToDbmDelta),
			(LONG) (pAd->StaCfg.RssiSample.LastRssi1 -
				pAd->BbpRssiToDbmDelta),
			(LONG) (pAd->StaCfg.RssiSample.LastRssi2 -
				pAd->BbpRssiToDbmDelta));
		...
		sprintf(extra + strlen(extra),
			"RSSI-A                          = %ld\n",
			(LONG) (pAd->StaCfg.RssiSample.AvgRssi0 -
				pAd->BbpRssiToDbmDelta));
		sprintf(extra + strlen(extra),
			"RSSI-B (if available)           = %ld\n",
			(LONG) (pAd->StaCfg.RssiSample.AvgRssi1 -
				pAd->BbpRssiToDbmDelta));
		sprintf(extra + strlen(extra),
			"RSSI-C (if available)           = %ld\n\n",
			(LONG) (pAd->StaCfg.RssiSample.AvgRssi2 -
				pAd->BbpRssiToDbmDelta));
#endif				/* ENHANCED_STAT_DISPLAY */
			...
			pAd->StaCfg.WscControl.WscEnrolleePinCode);
	{
		...
		sprintf(extra + strlen(extra),
			...
			pAd->StaCfg.WscControl.
			WscDriverAutoConnect ? "Enabled" : "Disabled",
			pAd->StaCfg.WscControl.WscDriverAutoConnect);
		/* display pin code */
		...
		for (idx = 0;
		     idx < pAd->StaCfg.WscControl.WscProfile.ProfileCnt;
		     idx++) {
			...
			if (pAd->StaCfg.BssType == BSS_ADHOC) {
				...
				sprintf(extra + strlen(extra),
					...
					(pCredential->
					 RegIpv4Addr & 0xFF000000) >> 24,
					(pCredential->
					 RegIpv4Addr & 0x00FF0000) >> 16,
					(pCredential->
					 RegIpv4Addr & 0x0000FF00) >> 8,
					(pCredential->
					 RegIpv4Addr & 0x000000FF));
				sprintf(extra + strlen(extra),
					"Credential Entrollee IPv4 Addr  = %d:%d:%d:%d\n",
					(pCredential->
					 EnrIpv4Addr & 0xFF000000) >> 24,
					(pCredential->
					 EnrIpv4Addr & 0x00FF0000) >> 16,
					(pCredential->
					 EnrIpv4Addr & 0x0000FF00) >> 8,
					(pCredential->
					 EnrIpv4Addr & 0x000000FF));
				sprintf(extra + strlen(extra),
					"\nSelf IPv4 Addr                  = %d:%d:%d:%d\n",
					(pIWscInfo->
					 SelfIpv4Addr & 0xFF000000) >> 24,
					(pIWscInfo->
					 SelfIpv4Addr & 0x00FF0000) >> 16,
					(pIWscInfo->
					 SelfIpv4Addr & 0x0000FF00) >> 8,
					(pIWscInfo->SelfIpv4Addr & 0x000000FF));
				sprintf(extra + strlen(extra),
					"IPv4 Subnet Mask                = %d:%d:%d:%d\n",
					(pIWscInfo->
					 Ipv4SubMask & 0xFF000000) >> 24,
					(pIWscInfo->
					 Ipv4SubMask & 0x00FF0000) >> 16,
					(pIWscInfo->
					 Ipv4SubMask & 0x0000FF00) >> 8,
			 ...
			 IN VOID * pData, IN ULONG Data, IN USHORT priv_flags)
{
	...
	case CMD_RTPRIV_IOCTL_PARAM_SET:
		{
			RT_CMD_PARAM_SET *pCmdParam =
			    (RT_CMD_PARAM_SET *) pData;
	...
	case CMD_RTPRIV_IOCTL_BSS_LIST_GET:
		{
			RT_CMD_STA_IOCTL_BSS_LIST *pBssList =
			    (RT_CMD_STA_IOCTL_BSS_LIST *) pData;
	...
	default:
		...
		Status =
		    RTMP_COM_IoctlHandle(pAd, pRequest, Command, Subcmd, pData,
					 Data);
		break;
	}
...
INT ed_status_read(RTMP_ADAPTER * pAd)
{
	...
	if ((pAd->ed_threshold > 0) && (period_us > 0)
		...
		if (percent > pAd->ed_threshold) {
			...
			if (pAd->false_cca_stat[pAd->ed_stat_lidx] >
			    pAd->false_cca_threshold) {
				pAd->ed_false_cca_cnt++;

				if (pAd->ed_false_cca_cnt >
...
INT set_ed_false_cca_threshold(RTMP_ADAPTER * pAd, PSTRING arg)
{
	...
	pAd->false_cca_threshold =
	    false_cca_threshold > 0 ? false_cca_threshold : 0;
...
{
	unsigned long irqflags;
	UINT32 ed_stat[ED_STAT_CNT], ed_2nd_stat[ED_STAT_CNT],
	    false_cca_stat[ED_STAT_CNT];
	...
	NdisMoveMemory(&ed_2nd_stat[0], &pAd->ed_2nd_stat[0],
		       sizeof(ed_2nd_stat));
	...
	NdisMoveMemory(&trigger_stat[0], &pAd->ed_trigger_stat[0],
		       sizeof(trigger_stat));
	NdisMoveMemory(&silent_stat[0], &pAd->ed_silent_stat[0],
		       sizeof(silent_stat));
	NdisMoveMemory(&false_cca_stat[0], &pAd->false_cca_stat[0],
		       sizeof(false_cca_stat));
	RTMP_IRQ_UNLOCK(&pAd->irq_lock, irqflags);

#ifdef CONFIG_AP_SUPPORT
	DBGPRINT(RT_DEBUG_OFF,
		 ("Dump ChannelBusy Counts, ed_chk=%u, ed_sta_threshold=%u, ed_ap_threshold=%u, false_cca_threshold=%u, ChkPeriod=%dms, ED_Threshold=%d%%, HitCntForBlockTx=%d\n",
		  pAd->ed_chk, pAd->ed_sta_threshold, pAd->ed_ap_threshold,
		  pAd->false_cca_threshold, pAd->ed_chk_period,
		  pAd->ed_threshold, pAd->ed_block_tx_threshold));
#endif
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("Dump ChannelBusy Counts, ed_chk=%u, ed_ap_scaned=%u, ed_current_ch_aps=%u, ed_rssi_threshold=%d, false_cca_threshold=%u, ChkPeriod=%dms, ED_Threshold=%d%%, HitCntForBlockTx=%d\n",
		  pAd->ed_chk, pAd->ed_ap_scaned, pAd->ed_current_ch_aps,
		  pAd->ed_rssi_threshold, pAd->false_cca_threshold,
	...
	do {
		DBGPRINT(RT_DEBUG_OFF,
			 ("\t%d", (ed_2nd_stat[idx] * 100) / period_us));
		INC_RING_INDEX(idx, ED_STAT_CNT);
	} while (idx != end);
	DBGPRINT(RT_DEBUG_OFF, ("\n"));
	DBGPRINT(RT_DEBUG_OFF, ("\n==========================\n"));
