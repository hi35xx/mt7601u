				...
				OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP,
			      ...
			      (STATE_MACHINE_FUNC)
			      ApCliInvalidStateWhenDisassociate);
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP,
	...
	/* timer init */
	RTMPInitTimer(pAd, &pAd->ApCliMlmeAux.ApCliAssocTimer,
	...
	if (pAd->ApCfg.ApCliTab[ifIndex].bBlockAssoc == TRUE) {
		...
		*pCurrState = APCLI_ASSOC_IDLE;
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  ...
				  pAd->ApCliMlmeAux.Ssid, 1, &SupRateIe, 1,
...
/* 2008/12/17:KH modified to fix the low throughput of  AP-Client  on Big-Endian Platform-->	 */
		...
		if (pAd->CommonCfg.bAggregationCapable) {
			...
			if ((pAd->CommonCfg.bPiggyBackCapable)
			    && ((pAd->ApCliMlmeAux.APRalinkIe & 0x00000003) ==
				3)) {
				ULONG TmpLen;
				UCHAR RalinkIe[9] =
		...
		if (pAd->ApCliMlmeAux.APEdcaParm.bValid) {
			if (pAd->ApCfg.ApCliTab[0].UapsdInfo.bAPSDCapable
...
#ifdef RT_CFG80211_SUPPORT
		if ((pAd->ApCfg.ApCliTab[ifIndex].WpaSupplicantUP & 0x7F) ==
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen,
					  ...
					  WpaAssocIeLen,
					  pAd->ApCfg.ApCliTab[ifIndex].
					  pWpaAssocIe, END_OF_ARGS);

			FrameLen += TmpWpaAssocIeLen;

			VarIesOffset = 0;
			NdisMoveMemory(pAd->ApCfg.ApCliTab[ifIndex].ReqVarIEs +
				       VarIesOffset,
				       pAd->ApCfg.ApCliTab[ifIndex].pWpaAssocIe,
				       pAd->ApCfg.ApCliTab[ifIndex].
				       WpaAssocIeLen);
			VarIesOffset +=
			    pAd->ApCfg.ApCliTab[ifIndex].WpaAssocIeLen;
...
#ifdef WSC_AP_SUPPORT
		if (pAd->ApCfg.ApCliTab[ifIndex].WscControl.WscConfMode !=
		    WSC_DISABLE) {
			...
			if (pWscBuf != NULL) {
				...
				WscBuildAssocReqIE(&pAd->ApCfg.
						   ApCliTab[ifIndex].WscControl,
						   pWscBuf, &WscIeLen);
				    ...
				    IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (ApCliPeerAssocRspSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, &CapabilityInfo, &Status,
	     &Aid, &P2PSubelementLen, P2pSubelement, SupRate, &SupRateLen,
	     ExtRate, &ExtRateLen, &HtCapability, &AddHtInfo, &HtCapabilityLen,
	     &AddHtInfoLen, &NewExtChannelOffset, &EdcaParm, &CkipFlag)) {
		/* The frame is for me ? */
		if (MAC_ADDR_EQUAL(Addr2, pAd->ApCliMlmeAux.Bssid)) {
#ifdef RT_CFG80211_SUPPORT
			PFRAME_802_11 pFrame = (PFRAME_802_11) (Elem->Msg);
			NdisZeroMemory(pAd->ApCfg.ApCliTab[0].ResVarIEs,
				       MAX_VIE_LEN);
			pAd->ApCfg.ApCliTab[0].ResVarIELen = 0;

			pAd->ApCfg.ApCliTab[0].ResVarIELen =
			    Elem->MsgLen - 6 - sizeof(HEADER_802_11);
			NdisCopyMemory(pAd->ApCfg.ApCliTab[0].ResVarIEs,
				       &pFrame->Octet[6],
				       pAd->ApCfg.ApCliTab[0].ResVarIELen);
#endif				/* RT_CFG80211_SUPPORT */
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("APCLI_ASSOC - receive ASSOC_RSP to me (status=%d)\n",
				  Status));
			RTMPCancelTimer(&pAd->ApCliMlmeAux.ApCliAssocTimer,
					&Cancelled);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s:: recv peer ASSOC RSP from %02x:%02x:%02x:%02x:%02x:%02x.    bP2pClient = %d\n",
				  __FUNCTION__, PRINT_MAC(Addr2),
				  pApCliEntry->bP2pClient));

			if (Status == MLME_SUCCESS) {
				/* go to procedure listed on page 376 */
				ApCliAssocPostProc(pAd, Addr2, CapabilityInfo,
			       ...
			       IN ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	NdisMoveMemory(&pAd->ApCliMlmeAux.APEdcaParm, pEdcaParm,
		       sizeof(EDCA_PARM));
