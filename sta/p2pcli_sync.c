...
static VOID ApCliPeerProbeRspAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM * Elem);
			       ...
			       OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(Sm, APCLI_SYNC_IDLE, APCLI_MT2_PEER_PROBE_RSP,
			      (STATE_MACHINE_FUNC) ApCliPeerProbeRspAction);
	...
	StateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP,
			      APCLI_MT2_PEER_PROBE_RSP,
			      (STATE_MACHINE_FUNC)
			      ApCliPeerProbeRspAtJoinAction);
	StateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP,
			      APCLI_MT2_PEER_BEACON,
			      (STATE_MACHINE_FUNC)
			      ApCliPeerProbeRspAtJoinAction);
	StateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP,
			      APCLI_MT2_PROBE_TIMEOUT,
			      (STATE_MACHINE_FUNC)
					  ...
					  IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		if (Bssidx == BSS_NOT_FOUND) {
			...
			Bssidx =
			    BssTableSetEntry(pAd, &pAd->ScanTab, ie_list, -127,
					     LenVIE, pVIE);
		...
		if (matchFlag) {
			...
			if (bssidEqualFlag == FALSE) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s::  Swich Channel = %d. and STOP Scanning!!\n",
			...
			if ((ie_list->HtCapabilityLen > 0) &&
				...
				ApCliCheckHt(pAd, ifIndex,
					     ...
					     &ie_list->AddHtInfo);
				RTMPMoveMemory(&pAd->ApCliMlmeAux.AddHtInfo,
					       &ie_list->AddHtInfo,
					       SIZE_ADD_HT_INFO_IE);
				...
				if (INFRA_ON(pAd)
					...
					DBGPRINT(RT_DEBUG_OFF,
						 ("PeerBeaconAtJoinAction HT===> Channel offset = %d not match INFRA Channel offset %d .\n",
				...
				pAd->ApCliMlmeAux.ConCurrentCentralChannel =
				    pAd->CommonCfg.CentralChannel;
#endif				/* P2P_SUPPORT */
			} else
#endif				/* DOT11_N_SUPPORT */
...
BOOLEAN ApCliWaitProbRsp(IN PRTMP_ADAPTER pAd, IN USHORT ifIndex)
{
	if (ifIndex >= MAX_APCLI_NUM)
		return FALSE;

	return (pAd->ApCfg.ApCliTab[ifIndex].SyncCurrState ==
		APCLI_JOIN_WAIT_PROBE_RSP) ? TRUE : FALSE;
}
...
/*
    ...
    ==========================================================================
 */
VOID ApCliPeerProbeRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
