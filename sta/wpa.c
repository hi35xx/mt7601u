...
VOID WpaStaPairwiseKeySetting(IN PRTMP_ADAPTER pAd)
{
	...
	NdisMoveMemory(pSharedKey->TxMic, &pAd->StaCfg.PTK[48 + LEN_TKIP_MIC],
		       LEN_TKIP_MIC);
	...
	NdisMoveMemory(pEntry->PairwiseKey.TxMic,
		       &pAd->StaCfg.PTK[48 + LEN_TKIP_MIC], LEN_TKIP_MIC);
...
VOID WpaStaGroupKeySetting(IN PRTMP_ADAPTER pAd)
{
	...
	RTMP_ASIC_SHARED_KEY_TABLE(pAd,
				   BSS0, pAd->StaCfg.DefaultKeyId, pSharedKey);
...
/*
	...
	========================================================================
*/
VOID WpaSendEapolStart(IN PRTMP_ADAPTER pAd, IN PUCHAR pBssid)
{
