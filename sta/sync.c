			  ...
			  IN STATE_MACHINE * Sm, OUT STATE_MACHINE_FUNC Trans[])
{
...
#ifdef P2P_SUPPORT
	StateMachineSetAction(Sm, SYNC_IDLE, MT2_PEER_PROBE_RSP,
	...
	StateMachineSetAction(Sm, SCAN_PENDING, MT2_MLME_SCAN_REQ,
			      (STATE_MACHINE_FUNC) MlmeScanReqAction);
	StateMachineSetAction(Sm, SCAN_PENDING, MT2_PEER_BEACON,
			      (STATE_MACHINE_FUNC) PeerBeacon);
		 ...
		 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (MlmeEnqueue(pAd, SYNC_STATE_MACHINE, MT2_SCAN_TIMEOUT, 0, NULL, 0)) {
		...
		RTMPSendWirelessEvent(pAd, IW_SCAN_ENQUEUE_FAIL_EVENT_FLAG,
				      NULL, BSS0, 0);
	}
}
...
VOID MlmeForceJoinReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if ((RTMP_Usb_AutoPM_Get_Interface(pObj->pUsb_Dev, pObj->intf)) == 1) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("MlmeJoinReqAction: autopm_resume success\n"));
		RTMP_CLEAR_FLAG(pAd, fRTMP_ADAPTER_SUSPEND);
	} else if ((RTMP_Usb_AutoPM_Get_Interface(pObj->pUsb_Dev, pObj->intf))
		   == (-1)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("MlmeJoinReqAction autopm_resume fail ------\n"));
		RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_SUSPEND);
		return;
	} else
		DBGPRINT(RT_DEBUG_TRACE,
			 ("MlmeJoinReqAction: autopm_resume do nothing \n"));
	...
	if ((OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_ADVANCE_POWER_SAVE_PCIE_DEVICE))
	    && (IDLE_ON(pAd)) && (pAd->StaCfg.bRadio == TRUE)
	...
	do {
		...
		if (NStatus == NDIS_STATUS_SUCCESS) {
			...
			if ((pAd->MlmeAux.BssType == BSS_INFRA)
			    &&
			    (!MAC_ADDR_EQUAL
			     (ZERO_MAC_ADDR, pAd->StaCfg.ConnectinfoBssid))) {
				COPY_MAC_ADDR(pAd->MlmeAux.Bssid,
...
VOID MlmeForceScanReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (MlmeScanReqSanity(pAd,
		...
		if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].Valid
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_NULL: PWR_SAVE IN ForceScanStart\n"));
		...
		RTMPSendWirelessEvent(pAd, IW_SCANNING_EVENT_FLAG, NULL, BSS0,
				      0);
...
VOID MlmeScanReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (MlmeScanReqSanity(pAd,
		...
		if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].Valid
		    && RTMP_CFG80211_VIF_P2P_CLI_ON(pAd)
		    && (pAd->apcli_wfd_connect != TRUE)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_NULL: PWR_SAVE IN ScanStart\n"));
			RT_CFG80211_P2P_CLI_SEND_NULL_FRAME(pAd, PWR_SAVE);
		}
#endif				/* RT_CFG80211_SUPPORT */
		...
		if (NStatus == NDIS_STATUS_SUCCESS) {
			...
			if ((pAd->StaCfg.WscControl.WscEnProbeReqIE) &&
				...
				bHasWscIe = TRUE;
				 ...
				 (pAd->StaCfg.WscControl.WscV2Info.
				  bEnableWpsV2))
				bHasWscIe = TRUE;
			...
			if (bHasWscIe) {
				...
				if (WscBuf != NULL) {
					...
					WscBuildProbeReqIE(pAd, STA_MODE,
							   WscBuf, &WscIeLen);
...
VOID MlmeStartReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if ((MlmeStartReqSanity
	    ...
	    && (CHAN_PropertyCheck(pAd, pAd->MlmeAux.Channel, CHANNEL_NO_IBSS)
		== FALSE)) {
		/* reset all the timers */
		RTMPCancelTimer(&pAd->MlmeAux.ScanTimer, &TimerCancelled);
		RTMPCancelTimer(&pAd->MlmeAux.BeaconTimer, &TimerCancelled);
...
VOID rtmp_dbg_sanity_diff(RTMP_ADAPTER * pAd, MLME_QUEUE_ELEM * Elem)
{
	...
	UCHAR *Ssid = NULL;
	UCHAR SsidLen =
	    0, DtimCount, DtimPeriod, BcastFlag, MessageToMe, NewChannel,
	    Channel = 0, BssType;
	CF_PARM CfParm = { 0 };
	...
	if (Ssid == NULL)
		goto LabelErr;
	os_alloc_mem(NULL, (UCHAR **) & pHtCapability,
	...
	if (sanity_old != sanity_new) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("sanity mismatch, old=%d, new=%d\n", sanity_old,
			  sanity_new));
	} else {
		if (NdisCmpMemory(&ie_list->Addr2[0], &Addr2[0], MAC_ADDR_LEN)
		    != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Add2 mismatch!Old=%02x:%02x:%02x:%02x:%02x:%02x!New=%02x:%02x:%02x:%02x:%02x:%02x!\n",
				  PRINT_MAC(Addr2), PRINT_MAC(ie_list->Addr2)));
		}

		if (NdisCmpMemory(&ie_list->Bssid[0], &Bssid[0], MAC_ADDR_LEN)
		    != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Bssid mismatch!Old=%02x:%02x:%02x:%02x:%02x:%02x!New=%02x:%02x:%02x:%02x:%02x:%02x!\n",
				  PRINT_MAC(Bssid), PRINT_MAC(ie_list->Bssid)));
		}

		if (SsidLen != ie_list->SsidLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("SsidLen mismatch!Old=%d, New=%d\n", SsidLen,
				  ie_list->SsidLen));
		}

		if (NdisCmpMemory(&ie_list->Ssid[0], &Ssid[0], SsidLen) != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Ssid mismatch!Old=%s, New=%s\n", Ssid,
				  ie_list->Ssid));
		}

		if (BssType != ie_list->BssType) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("BssType mismatch!Old=%d, New=%d\n", BssType,
				  ie_list->BssType));
		}

		if (BeaconPeriod != ie_list->BeaconPeriod) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("BeaconPeriod mismatch!Old=%d, New=%d\n",
				  BeaconPeriod, ie_list->BeaconPeriod));
		}

		if (Channel != ie_list->Channel) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Channel mismatch!Old=%d, New=%d\n", Channel,
				  ie_list->Channel));
		}

		if (NewChannel != ie_list->NewChannel) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("NewChannel mismatch!Old=%d, New=%d\n",
				  NewChannel, ie_list->NewChannel));
		}

		if (NdisCmpMemory
		    (&ie_list->TimeStamp, &TimeStamp,
		     sizeof(LARGE_INTEGER)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TimeStamp mismatch!Old=%d - %d, New=%d - %d\n",
				  TimeStamp.u.LowPart, TimeStamp.u.HighPart,
				  ie_list->TimeStamp.u.LowPart,
				  ie_list->TimeStamp.u.HighPart));
		}

		if (NdisCmpMemory(&ie_list->CfParm, &CfParm, sizeof(CF_PARM)) !=
		    0) {
			DBGPRINT(RT_DEBUG_ERROR, ("CFParam mismatch!\n"));
			hex_dump("Old CFParam", (UCHAR *) & CfParm,
				 sizeof(CF_PARM));
			hex_dump("New CFParam", (UCHAR *) & ie_list->CfParm,
				 sizeof(CF_PARM));
		}

		if (AtimWin != ie_list->AtimWin) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("AtimWin mismatch!Old=%d, New=%d\n", AtimWin,
				  ie_list->AtimWin));
		}

		if (CapabilityInfo != ie_list->CapabilityInfo) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("CapabilityInfo mismatch!Old=%d, New=%d\n",
				  CapabilityInfo, ie_list->CapabilityInfo));
		}

		if (Erp != ie_list->Erp) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Erp mismatch!Old=%d, New=%d\n", Erp,
				  ie_list->Erp));
		}

		if (DtimCount != ie_list->DtimCount) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("DtimCount mismatch!Old=%d, New=%d\n",
				  DtimCount, ie_list->DtimCount));
		}

		if (DtimPeriod != ie_list->DtimPeriod) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("DtimPeriod mismatch!Old=%d, New=%d\n",
				  DtimPeriod, ie_list->DtimPeriod));
		}

		if (BcastFlag != ie_list->BcastFlag) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("BcastFlag mismatch!Old=%d, New=%d\n",
				  BcastFlag, ie_list->BcastFlag));
		}

		if (MessageToMe != ie_list->MessageToMe) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("MessageToMe mismatch!Old=%d, New=%d\n",
				  MessageToMe, ie_list->MessageToMe));
		}

		if (SupRateLen != ie_list->SupRateLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("SupRateLen mismatch!Old=%d, New=%d\n",
				  SupRateLen, ie_list->SupRateLen));
		}

		if (NdisCmpMemory
		    (&ie_list->SupRate[0], &SupRate,
		     ie_list->SupRateLen) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("SupRate mismatch!\n"));
			hex_dump("Old SupRate", (UCHAR *) & SupRate,
				 ie_list->SupRateLen);
			hex_dump("New SupRate", (UCHAR *) & ie_list->SupRate,
				 ie_list->SupRateLen);
		}

		if (ExtRateLen != ie_list->ExtRateLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("ExtRateLen mismatch!Old=%d, New=%d\n",
				  ExtRateLen, ie_list->ExtRateLen));
		}
		if (NdisCmpMemory
		    (&ie_list->ExtRate[0], &ExtRate,
		     ie_list->ExtRateLen) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("ExtRate mismatch!\n"));
			hex_dump("Old ExtRate", (UCHAR *) & ExtRate,
				 ie_list->ExtRateLen);
			hex_dump("New ExtRate", (UCHAR *) & ie_list->ExtRate,
				 ie_list->ExtRateLen);
		}

		if (CkipFlag != ie_list->CkipFlag) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("CkipFlag mismatch!Old=%d, New=%d\n",
				  CkipFlag, ie_list->CkipFlag));
		}

		if (AironetCellPowerLimit != ie_list->AironetCellPowerLimit) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("AironetCellPowerLimit mismatch!Old=%d, New=%d\n",
				  AironetCellPowerLimit,
				  ie_list->AironetCellPowerLimit));
		}
		if (NdisCmpMemory
		    (&ie_list->EdcaParm, &EdcaParm, sizeof(EDCA_PARM)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("EdcaParm mismatch!\n"));
			hex_dump("Old EdcaParm", (UCHAR *) & EdcaParm,
				 sizeof(EDCA_PARM));
			hex_dump("New EdcaParm", (UCHAR *) & ie_list->EdcaParm,
				 sizeof(EDCA_PARM));
		}

		if (NdisCmpMemory
		    (&ie_list->QbssLoad, &QbssLoad,
		     sizeof(QBSS_LOAD_PARM)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("QbssLoad mismatch!\n"));
			hex_dump("Old QbssLoad", (UCHAR *) & QbssLoad,
				 sizeof(QBSS_LOAD_PARM));
			hex_dump("New QbssLoad", (UCHAR *) & ie_list->QbssLoad,
				 sizeof(QBSS_LOAD_PARM));
		}

		if (NdisCmpMemory
		    (&ie_list->QosCapability, &QosCapability,
		     sizeof(QOS_CAPABILITY_PARM)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("QosCapability mismatch!\n"));
			hex_dump("Old QosCapability", (UCHAR *) & QosCapability,
				 sizeof(QOS_CAPABILITY_PARM));
			hex_dump("New QosCapability",
				 (UCHAR *) & ie_list->QosCapability,
				 sizeof(QOS_CAPABILITY_PARM));
		}

		if (RalinkIe != ie_list->RalinkIe) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("RalinkIe mismatch!Old=%lx, New=%lx\n",
				  RalinkIe, ie_list->RalinkIe));
		}

		if (HtCapabilityLen != ie_list->HtCapabilityLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("HtCapabilityLen mismatch!Old=%d, New=%d\n",
				  HtCapabilityLen, ie_list->HtCapabilityLen));
		}
#ifdef CONFIG_STA_SUPPORT
		if (PreNHtCapabilityLen != ie_list->PreNHtCapabilityLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("PreNHtCapabilityLen mismatch!Old=%d, New=%d\n",
				  PreNHtCapabilityLen,
				  ie_list->PreNHtCapabilityLen));
		}
#endif				/* CONFIG_STA_SUPPORT */
		if (NdisCmpMemory
		    (&ie_list->HtCapability, pHtCapability,
		     sizeof(HT_CAPABILITY_IE)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("pHtCapability mismatch!\n"));
			hex_dump("Old HtCapability", (UCHAR *) pHtCapability,
				 sizeof(HT_CAPABILITY_IE));
			hex_dump("New HtCapability",
				 (UCHAR *) & ie_list->HtCapability,
				 sizeof(HT_CAPABILITY_IE));
		}

		if (NdisCmpMemory
		    (&ie_list->ExtCapInfo, &ExtCapInfo,
		     sizeof(EXT_CAP_INFO_ELEMENT)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("ExtCapInfo mismatch!\n"));
			hex_dump("Old ExtCapInfo", (UCHAR *) & ExtCapInfo,
				 sizeof(EXT_CAP_INFO_ELEMENT));
			hex_dump("New ExtCapInfo",
				 (UCHAR *) & ie_list->ExtCapInfo,
				 sizeof(EXT_CAP_INFO_ELEMENT));
		}

		if (AddHtInfoLen != ie_list->AddHtInfoLen) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("AddHtInfoLen mismatch!Old=%d, New=%d\n",
				  AddHtInfoLen, ie_list->AddHtInfoLen));
		}

		if (NdisCmpMemory
		    (&ie_list->AddHtInfo, pAddHtInfo,
		     sizeof(ADD_HT_INFO_IE)) != 0) {
			DBGPRINT(RT_DEBUG_ERROR, ("AddHtInfo mismatch!\n"));
			hex_dump("Old AddHtInfo", (UCHAR *) pAddHtInfo,
				 sizeof(ADD_HT_INFO_IE));
			hex_dump("New AddHtInfo",
				 (UCHAR *) & ie_list->AddHtInfo,
				 sizeof(ADD_HT_INFO_IE));
		}
...
VOID PeerBeaconAtScanAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		Idx =
		    BssTableSearch(&pAd->ScanTab, &ie_list->Bssid[0],
				   ie_list->Channel);
...
#ifdef P2P_SUPPORT
		MlmeEnqueue(pAd, P2P_DISC_STATE_MACHINE, P2P_DISC_PEER_PROB_RSP,
			    Elem->MsgLen, Elem->Msg, ie_list->Channel);
#endif				/* P2P_SUPPORT */
		...
		Idx =
		    BssTableSetEntry(pAd, &pAd->ScanTab, ie_list, Rssi, LenVIE,
				     pVIE);
#ifdef DOT11_N_SUPPORT
		...
		if (INFRA_ON(pAd)
			...
			for (chListIdx = 0; chListIdx < pAd->ChannelListNum;
			     chListIdx++) {
				if (ie_list->Channel ==
				    pAd->ChannelList[chListIdx].Channel)
					break;
			}
		...
		if (Idx != BSS_NOT_FOUND) {
			...
			NdisMoveMemory(&pBssEntry->TTSF[0],
				       &Elem->TimeStamp.u.LowPart, 4);
			NdisMoveMemory(&pBssEntry->TTSF[4],
				       &Elem->TimeStamp.u.LowPart, 4);
			...
			NdisMoveMemory(pBssEntry->MacAddr, &ie_list->Addr2[0],
				       MAC_ADDR_LEN);
			...
			if ((pFrame->Hdr.FC.SubType == SUBTYPE_PROBE_RSP)
				...
				if (pBssEntry->pVarIeFromProbRsp) {
					pBssEntry->VarIeFromProbeRspLen =
					    LenVIE;
					RTMPZeroMemory(pBssEntry->
						       pVarIeFromProbRsp,
						       MAX_VIE_LEN);
					RTMPMoveMemory(pBssEntry->
						       pVarIeFromProbRsp, pVIE,
						       LenVIE);
				}
			}
		}
		...
		RT_CFG80211_SCANNING_INFORM(pAd, Idx, ie_list->Channel,
					    (UCHAR *) pFrame, Elem->MsgLen,
					    Rssi);
#endif				/* RT_CFG80211_SUPPORT */
#endif				/* LINUX */
	}
...
VOID PeerBeaconAtJoinAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
...
#ifdef P2P_SUPPORT
		if (P2P_INF_ON(pAd)) {
			P2pPeerBeaconAtJoinAction(pAd, Elem,
						  &ie_list->Bssid[0]);
		}
#endif				/* P2P_SUPPORT */
		...
		if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, &ie_list->Bssid[0])) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s():receive desired BEACON,Channel=%d\n",
			...
			if (IS_P2P_ENROLLEE(pAd)) {
				int p2pIdx;
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Recv desired Beacon, update SSID[%d][%s]\n",
					  ie_list->SsidLen, &ie_list->Ssid[0]));
				pAd->MlmeAux.SsidLen = ie_list->SsidLen;
				NdisMoveMemory(pAd->MlmeAux.Ssid, ie_list->Ssid,
					       pAd->MlmeAux.SsidLen);

				p2pIdx = P2pGroupTabSearch(pAd, ie_list->Addr2);
				if (p2pIdx < MAX_P2P_GROUP_SIZE) {
					pAd->P2pTable.Client[p2pIdx].SsidLen =
					    ie_list->SsidLen;
					NdisMoveMemory(pAd->P2pTable.
						       Client[p2pIdx].Ssid,
						       ie_list->Ssid,
						       pAd->P2pTable.
						       Client[p2pIdx].SsidLen);
				}
			}
#endif				/* P2P_SUPPORT */
			...
			if (pAd->MlmeAux.SsidLen == 0) {
				...
				Idx =
				    BssSsidTableSearch(&pAd->ScanTab,
						       ie_list->Bssid,
						       pAd->MlmeAux.Ssid,
						       pAd->MlmeAux.SsidLen,
						       ie_list->Channel);
...
#endif				/* WPA_SUPPLICANT_SUPPORT */
#ifdef WSC_STA_SUPPORT
						if ((pAd->StaCfg.WscControl.
						     WscState != WSC_STATE_OFF)
					    ) ;
					else
#endif				/* WSC_STA_SUPPORT */
					{
			...
			NdisMoveMemory(pAd->MlmeAux.SupRate, ie_list->SupRate,
				       ie_list->SupRateLen);
			RTMPCheckRates(pAd, pAd->MlmeAux.SupRate,
				       &pAd->MlmeAux.SupRateLen);
			pAd->MlmeAux.ExtRateLen = ie_list->ExtRateLen;
			NdisMoveMemory(pAd->MlmeAux.ExtRate, ie_list->ExtRate,
				       ie_list->ExtRateLen);
			RTMPCheckRates(pAd, pAd->MlmeAux.ExtRate,
				       &pAd->MlmeAux.ExtRateLen);
			...
			NdisMoveMemory(&pAd->MlmeAux.ExtCapInfo,
				       &ie_list->ExtCapInfo,
			...
			if (((ie_list->HtCapabilityLen > 0)
				...
				if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
					...
					DBGPRINT(RT_DEBUG_OFF,
						 ("%s(): VHT->center_freq_1=%d, CentralChannel=>%d\n",
						  __FUNCTION__,
						  vht_op->center_freq_1,
						  CentralChannel));
				}
...
#endif				/* DOT11_N_SUPPORT */
			{
				...
				pAd->StaActive.SupportedPhyInfo.bHtEnable =
				    FALSE;
#ifdef DOT11_VHT_AC
				pAd->StaActive.SupportedPhyInfo.bVhtEnable =
				    FALSE;
			...
			if (pAd->StaCfg.BssType == BSS_INFRA) {
				...
				if (pAd->MlmeAux.HtCapability.HtCapInfo.
				    ChannelWidth == BW_40)
					InfraAP_BW = TRUE;
				else
					InfraAP_BW = FALSE;
				...
				if (BwFallBack == 1) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("P2P STA connection to 40MHz GO, but Infra extra and P2P Group extra is different!!!\n"));
				...
				pAd->MlmeAux.ConCurrentCentralChannel =
...
VOID PeerBeacon(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		is_my_bssid =
		    MAC_ADDR_EQUAL(ie_list->Bssid,
				   pAd->CommonCfg.Bssid) ? TRUE : FALSE;
		is_my_ssid =
		    SSID_EQUAL(ie_list->Ssid, ie_list->SsidLen,
			       pAd->CommonCfg.Ssid,
		...
		Bssidx =
		    BssTableSearchWithSSID(&pAd->MlmeAux.SsidBssTab,
					   ie_list->Bssid, ie_list->Ssid,
					   ie_list->SsidLen, ie_list->Channel);
		if (Bssidx == BSS_NOT_FOUND) {
			/* discover new AP of this network, create BSS entry */
			Bssidx =
			    BssTableSetEntry(pAd, &pAd->MlmeAux.SsidBssTab,
					     ie_list, RealRssi, LenVIE, pVIE);
			if (Bssidx == BSS_NOT_FOUND) ;
			else {
				...
				NdisMoveMemory(&pBssEntry->PTSF[0],
					       &Elem->Msg[24], 4);
				NdisMoveMemory(&pBssEntry->TTSF[0],
					       &Elem->TimeStamp.u.LowPart, 4);
				NdisMoveMemory(&pBssEntry->TTSF[4],
				...
				NdisMoveMemory(pBssEntry->MacAddr,
		 ...
		 */
		Bssidx =
		    BssTableSearch(&pAd->ScanTab, ie_list->Bssid,
				   ie_list->Channel);
		if (Bssidx == BSS_NOT_FOUND) {
			/* discover new AP of this network, create BSS entry */
			Bssidx =
			    BssTableSetEntry(pAd, &pAd->ScanTab, ie_list,
					     RealRssi, LenVIE, pVIE);
		...
		if ((!is_my_bssid) && ADHOC_ON(pAd)) {
			...
			for (i = 0; i < 6; i++) {
				if (ie_list->Bssid[i] > pAd->CommonCfg.Bssid[i]) {
					...
					is_my_bssid = TRUE;
					break;
				} else if (ie_list->Bssid[i] <
					   pAd->CommonCfg.Bssid[i])
					break;
			}
		}
		...
		if (is_my_bssid) {
			...
			if ((pAd->CommonCfg.bIEEE80211H == 1) &&
				...
				for (index = 0; index < pAd->ChannelListNum;
				     index++) {
			...
			{
				if ((((pAd->StaCfg.WepStatus !=
				       Ndis802_11WEPDisabled) << 4) ^ ie_list->
				     CapabilityInfo) & 0x0010) {
...
/*			CFG80211_BeaconCountryRegionParse(pAd, pVIE, LenVIE); */
			...
			if (ADHOC_ON(pAd)
			    && (CAP_IS_IBSS_ON(ie_list->CapabilityInfo))) {
				...
				MaxSupportedRateIn500Kbps =
				    dot11_max_sup_rate(ie_list->SupRateLen,
						       &ie_list->SupRate[0],
						       ie_list->ExtRateLen,
						       &ie_list->ExtRate[0]);
							   ...
							   ADHOC_ENTRY_BEACON_LOST_TIME)))
				{
...
#ifdef IWSC_SUPPORT
					hex_dump
					    ("Another adhoc joining - Addr2",
					     ie_list->Addr2, 6);
					hex_dump
					    ("Another adhoc joining - WscPeerMAC",
					     pAd->StaCfg.WscControl.WscPeerMAC,
					     6);
					if ((NdisEqualMemory
					     (ie_list->Addr2,
					      pAd->StaCfg.WscControl.WscPeerMAC,
					      MAC_ADDR_LEN))
					    && (pAd->StaCfg.IWscInfo.
						bSendEapolStart == FALSE)
					    && (pWpsCtrl->bWscTrigger ==
						TRUE)) {
						pAd->StaCfg.IWscInfo.
						    bSendEapolStart = TRUE;
					}
#endif				/* IWSC_SUPPORT */
...
#ifdef DOT11_VHT_AC
					{
						...
						if (!ielist)
							goto LabelOK;
						NdisZeroMemory((UCHAR *) ielist,
							       sizeof
							       (IE_LISTS));

						if (ie_list->vht_cap_len
						    && ie_list->vht_op_len) {
							NdisMoveMemory(&ielist->
								       vht_cap,
								       &ie_list->
								       vht_cap_ie,
								       sizeof
					...
					if ((pAd->StaCfg.AuthMode ==
					    ...
					    && (pAd->StaCfg.IWscInfo.
						bBlockConnection == FALSE)
#endif				/* IWSC_TEST_SUPPORT */
#endif				// IWSC_SUPPORT //
					    ) {
						INT len, i;
						PEID_STRUCT pEid;
						NDIS_802_11_VARIABLE_IEs *pVIE2
						    = NULL;
						...
						hex_dump("PeerBeacon:: Addr2",
							 ie_list->Addr2,
							 MAC_ADDR_LEN);
						hex_dump
						    ("PeerBeacon:: CurrentAddress",
						     pAd->CurrentAddress,
						     MAC_ADDR_LEN);
						pEntry->bPeerHigherMAC =
						    bHigherMAC;
						if (pEntry->bPeerHigherMAC ==
						    FALSE) {
							/*
...
#ifdef IWSC_SUPPORT
					if (pEntry->bUpdateInfoFromPeerBeacon ==
					    FALSE) {
						if (StaAddMacTableEntry
						    (pAd, pEntry,
						     MaxSupportedRateIn500Kbps,
						     &ie_list->HtCapability,
				...
				if (pAd->StaCfg.IWscInfo.bSendEapolStart &&
					...
					WSC_ENROLLEE)) {
					pAd->StaCfg.IWscInfo.bSendEapolStart =
					    FALSE;
					pWpsCtrl->WscState = WSC_STATE_LINK_UP;
					pWpsCtrl->WscStatus =
					    STATUS_WSC_LINK_UP;
					NdisMoveMemory(pWpsCtrl->EntryAddr,
						       pWpsCtrl->WscPeerMAC,
						       MAC_ADDR_LEN);
			...
			if (INFRA_ON(pAd)) {
				...
				bUseShortSlot =
				    CAP_IS_SHORT_SLOT(ie_list->CapabilityInfo);
				if (bUseShortSlot !=
				...
				bUseBGProtection = (pAd->CommonCfg.UseBGProtection == 1) ||	/* always use */
				    ((pAd->CommonCfg.UseBGProtection == 0)
				     && ERP_IS_USE_PROTECTION(ie_list->Erp));
				...
				if (OPSTATUS_TEST_FLAG
				    ...
				    && ERP_IS_USE_BARKER_PREAMBLE(ie_list->
								  Erp)) {
...
/*	if ((pAd->P2pCfg.DiscCurrentState == P2P_DISC_LISTEN) && !(P2P_CLI_ON(pAd)))*/
	if (P2P_INF_ON(pAd))
		MlmeEnqueue(pAd, P2P_DISC_STATE_MACHINE, P2P_DISC_PEER_PROB_REQ,
			    Elem->MsgLen, Elem->Msg, Elem->Channel);
#endif				/* P2P_SUPPORT */
	...
	if (PeerProbeReqSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, Ssid, &SsidLen, NULL)) {
		if ((SsidLen == 0)
...
//yiwei cfg
	...
	if ((pAd->pCfg80211ChanList != NULL) && (pAd->MlmeAux.Channel != 0)) {
		...
		for (ChanId = 0; ChanId < pAd->Cfg80211ChanListLan; ChanId++) {
			if (pAd->pCfg80211ChanList[ChanId] >=
			    pAd->MlmeAux.Channel) {
				bScanDone = FALSE;
				break;
			}
...
VOID InvalidStateWhenScan(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (MlmeScanReqSanity(pAd,
		...
		MlmeEnqueue(pAd, P2P_CTRL_STATE_MACHINE, P2P_CTRL_DISC_CANL_EVT,
			    0, NULL, 0);
...
VOID CntlChannelWidth(IN PRTMP_ADAPTER pAd,
		      IN UCHAR prim_ch,
		      IN UCHAR cent_ch, IN UCHAR ch_bw, IN UCHAR sec_ch_offset)
{
	...
	if (rf_channel != 0) {
		...
		rtmp_bbp_get_agc(pAd, &pAd->BbpTuning.R66CurrentValue,
				 RX_CHAIN_0);
	}
}
#endif				/* DOT11N_DRAFT3 */
...
/*
    ...
    ==========================================================================
 */
VOID ScanCnclAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	BOOLEAN Cancelled;
