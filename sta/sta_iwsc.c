...
VOID IWSC_MlmeStartAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_MlmeStopAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_InvalidState(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_ScanDoneAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_ReConnectAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_SendActionFrame(IN PRTMP_ADAPTER pAd, IN UCHAR FrameType);

VOID IWSC_PeerAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_RoleAction(IN PRTMP_ADAPTER pAd, IN INT WscConfMode);

ULONG IWSC_SearchWpsApByPinMethod(IN PRTMP_ADAPTER pAd);

VOID IWSC_GetConfigMethod(IN PRTMP_ADAPTER pAd,
			  IN PUCHAR pVIE,
			  IN INT VIELen, OUT PUSHORT pConfigMethod);

VOID IWSC_PeerProbeRequest(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_PeerProbeResponse(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_PeerPIN(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem);

VOID IWSC_BuildDevQueryFrame(IN PRTMP_ADAPTER pAd,
			     OUT PUCHAR pOutBuf, OUT PUSHORT pIeLen);

VOID IWSC_ResetIpContent(IN PRTMP_ADAPTER pAd);
...
/*
    ...
    ==========================================================================
 */
void IWSC_StateMachineInit(IN PRTMP_ADAPTER pAd,
			   IN STATE_MACHINE * Sm,
			   OUT STATE_MACHINE_FUNC Trans[])
{
	StateMachineInit(Sm,
			 ...
			 IWSC_IDLE, IWSC_MACHINE_BASE);

	StateMachineSetAction(Sm, IWSC_IDLE, IWSC_MT2_MLME_START,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStartAction);
	StateMachineSetAction(Sm, IWSC_IDLE, IWSC_MT2_PEER_ACTION_FRAME,
			      (STATE_MACHINE_FUNC) IWSC_PeerAction);
	StateMachineSetAction(Sm, IWSC_IDLE, IWSC_MT2_MLME_SCAN_DONE,
			      (STATE_MACHINE_FUNC) IWSC_ScanDoneAction);
	StateMachineSetAction(Sm, IWSC_IDLE, IWSC_MT2_MLME_RECONNECT,
			      (STATE_MACHINE_FUNC) IWSC_InvalidState);

	StateMachineSetAction(Sm, IWSC_START, IWSC_MT2_MLME_START,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStartAction);
	StateMachineSetAction(Sm, IWSC_START, IWSC_MT2_MLME_STOP,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStopAction);
	StateMachineSetAction(Sm, IWSC_START, IWSC_MT2_PEER_PROBE_REQ,
			      (STATE_MACHINE_FUNC) IWSC_PeerProbeRequest);
	StateMachineSetAction(Sm, IWSC_START, IWSC_MT2_MLME_SCAN_DONE,
			      (STATE_MACHINE_FUNC) IWSC_ScanDoneAction);
	StateMachineSetAction(Sm, IWSC_START, IWSC_MT2_MLME_RECONNECT,
			      (STATE_MACHINE_FUNC) IWSC_ReConnectAction);

	StateMachineSetAction(Sm, IWSC_SCAN, IWSC_MT2_MLME_START,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStartAction);
	StateMachineSetAction(Sm, IWSC_SCAN, IWSC_MT2_MLME_STOP,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStopAction);
	StateMachineSetAction(Sm, IWSC_SCAN, IWSC_MT2_PEER_PROBE_RSP,
			      (STATE_MACHINE_FUNC) IWSC_PeerProbeResponse);
	StateMachineSetAction(Sm, IWSC_SCAN, IWSC_MT2_MLME_SCAN_DONE,
			      (STATE_MACHINE_FUNC) IWSC_ScanDoneAction);
	StateMachineSetAction(Sm, IWSC_SCAN, IWSC_MT2_MLME_RECONNECT,
			      (STATE_MACHINE_FUNC) IWSC_InvalidState);

	StateMachineSetAction(Sm, IWSC_WAIT_PIN, IWSC_MT2_PEER_PIN,
			      (STATE_MACHINE_FUNC) IWSC_PeerPIN);
	StateMachineSetAction(Sm, IWSC_WAIT_PIN, IWSC_MT2_MLME_STOP,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStopAction);

	StateMachineSetAction(Sm, IWSC_WAIT_JOIN, IWSC_MT2_MLME_STOP,
			      (STATE_MACHINE_FUNC) IWSC_MlmeStopAction);
...
VOID IWSC_InvalidState(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IWSC - InvalidState (state=%ld, msg_type = %ld)\n",
		  pAd->Mlme.IWscMachine.CurrState, pElem->MsgType));
}

VOID IWSC_Init(IN IN PRTMP_ADAPTER pAd)
{
	PIWSC_INFO pIWscInfo = &pAd->StaCfg.IWscInfo;

	RTMPInitTimer(pAd,
		      &pIWscInfo->IWscT1Timer,
		      GET_TIMER_FUNCTION(IWSC_T1TimerAction), pAd, FALSE);

	RTMPInitTimer(pAd,
		      &pIWscInfo->IWscT2Timer,
		      GET_TIMER_FUNCTION(IWSC_T2TimerAction), pAd, FALSE);

	RTMPInitTimer(pAd,
		      &pIWscInfo->IWscEntryTimer,
		      GET_TIMER_FUNCTION(IWSC_EntryTimerAction), pAd, FALSE);

	RTMPInitTimer(pAd,
		      &pIWscInfo->IWscDevQueryTimer,
		      GET_TIMER_FUNCTION(IWSC_DevQueryAction), pAd, FALSE);
	...
	pIWscInfo->IpConfMethod =
	    (IWSC_IPV4_ASSIGNMENT | IWSC_DHCP_IPV4 | IWSC_STATIC_IPV4);
...
VOID IWSC_Stop(IN PRTMP_ADAPTER pAd, IN BOOLEAN bSendNotification)
{
...
#endif				// IWSC_TEST_SUPPORT //
			RTMPSendWirelessEvent(pAd, IW_IWSC_T1_TIMER_TIMEOUT,
					      NULL, pWpsCtrl->EntryIfIdx, 0);
		} else
			DBGPRINT(RT_DEBUG_ERROR,
				 ("!!! IWSC_T1TimerAction - pWpsCtrl is null !!!\n"));
	} else
		DBGPRINT(RT_DEBUG_ERROR,
			 ("!!! IWSC_T1TimerAction - pAd is null !!!\n"));
}
			...
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("!!! IWSC_T2TimerAction - Become Enrollee !!!\n"));

	if (pAd) {
		pWpsCtrl = &pAd->StaCfg.WscControl;
		if (pWpsCtrl) {
			RTMPSendWirelessEvent(pAd, IW_IWSC_T2_TIMER_TIMEOUT,
					      NULL, pWpsCtrl->EntryIfIdx, 0);
			IWSC_RoleAction(pAd, WSC_ENROLLEE);
			pAd->Mlme.IWscMachine.CurrState = IWSC_SCAN;
		} else
			DBGPRINT(RT_DEBUG_ERROR,
				 ("!!! IWSC_T2TimerAction - pWpsCtrl is null !!!\n"));
	} else
		DBGPRINT(RT_DEBUG_ERROR,
			 ("!!! IWSC_T2TimerAction - pAd is null !!!\n"));
}
	...
	if (pAd) {
		...
		if (pIWscInfo) {
			...
			WscBuildProbeRespIE(pAd,
					    ...
					    DEV_PASS_ID_SMPBC,
					    pAd->StaCfg.WscControl.
					    WscConfigMethods, BSS0, NULL, 0,
					    STA_MODE);
			RTMPSendWirelessEvent(pAd, IW_IWSC_ENTRY_TIMER_TIMEOUT,
					      NULL,
					      pAd->StaCfg.WscControl.EntryIfIdx,
					      0);
		} else
			DBGPRINT(RT_DEBUG_ERROR,
				 ("!!! IWSC_EntryTimerAction - pIWscInfo is null !!!\n"));
	} else
		DBGPRINT(RT_DEBUG_ERROR,
			 ("!!! IWSC_EntryTimerAction - pAd is null !!!\n"));
}
	...
	if (pAd) {
		...
		if (pIWscInfo) {
			if (pIWscInfo->bIWscDevQueryReqTimerRunning) {
				pIWscInfo->bIWscDevQueryReqTimerRunning = FALSE;
				IWSC_SendActionFrame(pAd,
						     IWSC_DEV_QUERY_REQUEST);
			...
			if (pIWscInfo->bIWscDevQueryRspTimerRunning) {
				pIWscInfo->bIWscDevQueryRspTimerRunning = FALSE;
				IWSC_SendActionFrame(pAd,
						     IWSC_DEV_QUERY_RESPONSE);
			}
		} else
			DBGPRINT(RT_DEBUG_ERROR,
				 ("!!! IWSC_DevQueryAction - pIWscInfo is null !!!\n"));
	} else
		DBGPRINT(RT_DEBUG_ERROR,
			 ("!!! IWSC_DevQueryAction - pAd is null !!!\n"));
}
...
VOID IWSC_MlmeStartAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (pWpsCtrl->WscMode == WSC_SMPBC_MODE) {
		if (pWpsCtrl->WscConfMode == WSC_REGISTRAR) {
			RTMPSetTimer(&pIWscInfo->IWscEntryTimer,
				     IWSC_ENTRY_TIME_OUT);
			...
			if (OPSTATUS_TEST_FLAG
				...
				if ((pAd->StaCfg.IWscInfo.bSinglePIN == FALSE)
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("RandomTime = %d\n",
						  RandomTime));
					RTMPSetTimer(&pIWscInfo->IWscT2Timer,
						     (RandomTime * 100));
...
VOID IWSC_MlmeStopAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (pAd->StaCfg.IWscInfo.bSinglePIN && pAd->StaCfg.IWscInfo.bDoNotStop) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("SinglePIN registrar now, keep going!!\n"));
...
VOID IWSC_ScanDoneAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (pAd->Mlme.IWscMachine.CurrState != IWSC_SCAN) {
		if (pAd->Mlme.IWscMachine.CurrState != IWSC_IDLE)
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IWSC_ScanDoneAction:: CurrState = %ld\n",
	...
	if (pWpsCtrl->WscMode == WSC_PBC_MODE) {
		if (WscPBCExec(pAd, FALSE, pWpsCtrl) == FALSE) {
...
#ifdef IWSC_TEST_SUPPORT
				if (pAd->StaCfg.IWscInfo.IWscConfMode !=
				    WSC_ENROLLEE)
#endif				/* IWSC_TEST_SUPPORT */
				{
					IWSC_RoleAction(pAd, WSC_REGISTRAR);
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("RandomTime = %d\n",
						  RandomTime));
					RTMPSetTimer(&pAd->StaCfg.IWscInfo.
						     IWscT2Timer,
						     (RandomTime * 100));
					pAd->StaCfg.IWscInfo.
					    bIWscT2TimerRunning = TRUE;
					pAd->Mlme.IWscMachine.CurrState =
					    IWSC_START;
				}
#ifdef IWSC_TEST_SUPPORT
				else {
					RTMPSetTimer(&pWpsCtrl->WscScanTimer,
						     1000);
			...
			BOOLEAN bCancel;
			if (pWpsCtrl->WscPBCTimerRunning)
				RTMPCancelTimer(&pWpsCtrl->WscPBCTimer,
						&bCancel);
			pAd->Mlme.IWscMachine.CurrState = IWSC_START;
	...
	} else {
		{
...
#endif				// IWSC_TEST_SUPPORT //
			{
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("RandomTime = %d\n", RandomTime));
				RTMPSetTimer(&pAd->StaCfg.IWscInfo.IWscT2Timer,
					     (RandomTime * 100));
...
VOID IWSC_ReConnectAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	MlmeEnqueue(pAd,
		    ...
		    OID_802_11_SSID,
		    sizeof(NDIS_802_11_SSID), (VOID *) & pWpsCtrl->WscSsid, 0);
	RTMP_MLME_HANDLER(pAd);
	DBGPRINT(RT_DEBUG_TRACE, ("<----- %s\n", __FUNCTION__));
}

VOID IWSC_BuildSelRegStartNotification(IN PRTMP_ADAPTER pAd,
				       OUT PUCHAR pOutBuf, OUT PUSHORT pIeLen)
{
//      UCHAR                   Data[512]; // change array to pointer 
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("-----> IWSC_BuildSelRegStartNotification\n"));
	...
	templen =
	    AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData, (UINT8 *) & tempVal, 0);
	pData += templen;
	Len += templen;
	...
	/* Primary device type */
	templen =
	    AppendWSCTLV(WSC_ID_PRIM_DEV_TYPE, pData,
	...
	/* Config error */
	templen =
	    AppendWSCTLV(WSC_ID_CONFIG_ERROR, pData, (UINT8 *) & ConfigError,
			 0);
	pData += templen;
	Len += templen;
	...
	/* MAC address */
	templen =
	    AppendWSCTLV(WSC_ID_MAC_ADDR, pData, pReg->SelfInfo.MacAddr, 0);
	pData += templen;
	Len += templen;
...
VOID IWSC_BuildSelRegFinishNotification(IN PRTMP_ADAPTER pAd,
					OUT PUCHAR pOutBuf, OUT PUSHORT pIeLen)
{
//      UCHAR                   Data[512];
	UCHAR *Data = NULL;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("-----> IWSC_BuildSelRegFinishNotification\n"));
	...
	/* Simple Config State */
	templen =
	    AppendWSCTLV(WSC_ID_SC_STATE, pData,
			 (UINT8 *) & pWpsCtrl->WscConfStatus, 0);
	pData += templen;
	Len += templen;

	templen =
	    AppendWSCTLV(WSC_ID_MAC_ADDR, pData, &pAd->CurrentAddress[0], 0);
	pData += templen;
	Len += templen;
...
VOID IWSC_SendActionFrame(IN PRTMP_ADAPTER pAd, IN UCHAR FrameType)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IWSC_SendStartNotification:: allocate memory failed \n"));
		return;
	}

	if (FrameType == IWSC_DEV_QUERY_RESPONSE)
		ActHeaderInit(pAd, &ActHdr, pIWscInfo->IWscDevQueryReqMacAddr,
			      pAd->CurrentAddress, pAd->CommonCfg.Bssid);
	else
		ActHeaderInit(pAd, &ActHdr, BROADCAST_ADDR, pAd->CurrentAddress,
			      pAd->CommonCfg.Bssid);
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
			  ...
			  4, IWSC_ACTION_OUI,
			  1, &FrameType,
			  1, &pIWscInfo->DialogToken, END_OF_ARGS);

	os_alloc_mem(NULL, &pWscBuf, 512);
	if (pWscBuf != NULL) {
		NdisZeroMemory(pWscBuf, 512);
		if (FrameType == IWSC_SEL_REG_START_NOTITY)
			IWSC_BuildSelRegStartNotification(pAd, pWscBuf,
							  &WscIeLen);
		else if (FrameType == IWSC_SEL_REG_FINISH_NOTITY)
			IWSC_BuildSelRegFinishNotification(pAd, pWscBuf,
							   &WscIeLen);
...
VOID IWSC_PeerAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	 ...
	 */
	{
		...
		if (FrameType == IWSC_SEL_REG_START_NOTITY) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Receive SelRegStartNotification from %02X:%02X:%02X:%02X:%02X:%02X!\n",
				  PeerMAC[0], PeerMAC[1], PeerMAC[2],
				  PeerMAC[3], PeerMAC[4], PeerMAC[5]));
			pIWscInfo->bSelRegStart = TRUE;
			RTMPMoveMemory(pIWscInfo->RegMacAddr, PeerMAC,
				       MAC_ADDR_LEN);
			WscBuildProbeRespIE(pAd, WSC_MSGTYPE_IWSC_NOTIFIER,
					    pAd->StaCfg.WscControl.
					    WscConfStatus, TRUE, RegDPID,
					    RegCfgMethods, 0, NULL, 0,
					    STA_MODE);
		} else if (FrameType == IWSC_SEL_REG_FINISH_NOTITY) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Receive SelRegFinishNotification from %02X:%02X:%02X:%02X:%02X:%02X!\n",
				  PeerMAC[0], PeerMAC[1], PeerMAC[2],
				  PeerMAC[3], PeerMAC[4], PeerMAC[5]));
			if (NdisEqualMemory
			    (pIWscInfo->RegMacAddr, PeerMAC, MAC_ADDR_LEN)) {
				pIWscInfo->bSelRegStart = FALSE;
				NdisZeroMemory(pIWscInfo->RegMacAddr,
					       MAC_ADDR_LEN);
			}
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Receive Device Query Request from %02X:%02X:%02X:%02X:%02X:%02X!\n",
				  PeerMAC[0], PeerMAC[1], PeerMAC[2],
				  PeerMAC[3], PeerMAC[4], PeerMAC[5]));
			if (RandomVal < 20)
				RandomVal = 20;
			if (RandomVal > 50)
				RandomVal = 50;
			NdisMoveMemory(pIWscInfo->IWscDevQueryReqMacAddr,
				       PeerMAC, MAC_ADDR_LEN);
			RTMPSetTimer(&pIWscInfo->IWscDevQueryTimer,
				     RandomVal * 10);
			...
			if (pEntry == NULL) {
				...
				pEntry =
				    MacTableInsertEntry(pAd, PeerMAC, BSS0,
							OPMODE_STA, FALSE);
			}
			...
			if (pEntry) {
				...
				if ((pAd->StaCfg.AuthMode ==
				    ...
				    && (pEntry->WPA_Authenticator.WpaState <
					AS_INITPSK)) {
					INT len, i;
					...
					pEntry->WPA_Authenticator.WpaState =
					    AS_INITPSK;
					pEntry->WPA_Supplicant.WpaState =
					    AS_INITPSK;
					...
					hex_dump("IWSC_PeerAction:: PeerMAC",
						 PeerMAC, MAC_ADDR_LEN);
					hex_dump
					    ("IWSC_PeerAction:: pAd->CurrentAddress",
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Receive Device Query Response from %02X:%02X:%02X:%02X:%02X:%02X!\n",
				  PeerMAC[0], PeerMAC[1], PeerMAC[2],
				  PeerMAC[3], PeerMAC[4], PeerMAC[5]));
...
VOID IWSC_PeerProbeRequest(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (pWscCtrl->WscConfMode != WSC_REGISTRAR) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("IWSC_PeerProbeRequest:: Not Registrar now(WscConfMode = %d)\n",
	...
	if (pWscCtrl->WscMode == WSC_PIN_MODE) {
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD) {
			if (pIWscInfo->bSinglePIN == FALSE) {
				pWscCtrl->WscEnrolleePinCode =
				    WscRandomGeneratePinCode(pAd, BSS0);
				pWscCtrl->WscEnrolleePinCodeLen = 8;
				pWscCtrl->WscPinCodeLen = 8;
				pWscCtrl->WscStatus =
				    STATUS_WSC_IBSS_NEW_RANDOM_PIN;
			} else
				pWscCtrl->WscStatus = STATUS_WSC_IBSS_FIXED_PIN;

			WscGetRegDataPIN(pAd, pWscCtrl->WscEnrolleePinCode,
					 pWscCtrl);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IWSC_PeerProbeRequest:: WscPinCode = %08d\n",
				  pWscCtrl->WscPinCode));
		} else {
			if (pAd->StaCfg.IWscInfo.bLimitedUI) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IWSC_PeerProbeRequest:: Please input Peer's PIN\n"));
			}
		}
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD) {
		...
		WscBuildBeaconIE(pAd,
				 ...
				 TRUE,
				 DPID, ConfigMethod, BSS0, NULL, 0, STA_MODE);
		MakeIbssBeacon(pAd);	/* re-build BEACON frame */
		AsicEnableIbssSync(pAd);	/* copy BEACON frame to on-chip memory */
		WscBuildProbeRespIE(pAd,
				    WSC_MSGTYPE_REGISTRAR,
				    ...
				    TRUE,
				    DPID,
				    ConfigMethod, BSS0, NULL, 0, STA_MODE);
	}
...
VOID IWSC_PeerProbeResponse(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (pWscCtrl->WscConfMode != WSC_ENROLLEE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("IWSC_PeerProbeResponse:: Not Enrollee now(WscConfMode = %d)\n",
			  pWscCtrl->WscConfMode));
		return;
	}
	...
	if (pAd->StaCfg.IWscInfo.bLimitedUI) {
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD) {
			if (pIWscInfo->bSinglePIN == FALSE) {
				...
				pWscCtrl->WscPinCodeLen = 8;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IWSC_PeerProbeResponse:: WscPinCode = %d\n",
					  pWscCtrl->WscPinCode));
				pWscCtrl->WscStatus =
				    STATUS_WSC_IBSS_NEW_RANDOM_PIN;
			} else
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IWSC_PeerProbeResponse:: Please input Peer's PIN\n"));
	...
	MlmeEnqueue(pAd,
		    MLME_CNTL_STATE_MACHINE,
		    OID_802_11_SSID,
		    sizeof(NDIS_802_11_SSID), (VOID *) & pWscCtrl->WscSsid, 0);
	RTMP_MLME_HANDLER(pAd);
...
VOID IWSC_PeerPIN(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
	...
	if (ADHOC_ON(pAd)) {
		...
		NdisMoveMemory(pWscCtrl->EntryAddr, pWscCtrl->WscPeerMAC,
			       MAC_ADDR_LEN);
...
BOOLEAN IWSC_PeerEapolStart(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry)
{
	PWSC_CTRL pWpsCtrl = &pAd->StaCfg.WscControl;
	PIWSC_INFO pIWscInfo = &pAd->StaCfg.IWscInfo;
	BOOLEAN Cancelled;
	...
	if (pWpsCtrl->WscConfMode != WSC_REGISTRAR) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IWSC_PeerEapolStart:: Not Registrar now(WscConfMode = %d)\n",
	...
	if (pWpsCtrl->WscMode == WSC_PBC_MODE) {
		...
		if ((pAd->CommonCfg.WscStaPbcProbeInfo.WscPBCStaProbeCount == 1)
		    && !NdisEqualMemory(pAd->CommonCfg.WscStaPbcProbeInfo.
					StaMacAddr[0], &ZERO_MAC_ADDR[0],
					MAC_ADDR_LEN)
		    &&
		    (NdisEqualMemory
		     (pAd->CommonCfg.WscStaPbcProbeInfo.StaMacAddr[0],
		      &pEntry->Addr[0], 6) == FALSE)) {
			pAd->CommonCfg.WscPBCOverlap = TRUE;
		}
		//if (pAd->CommonCfg.WscPBCOverlap)
		{
			hex_dump("EntryAddr", pWpsCtrl->EntryAddr, 6);
			hex_dump("StaMacAddr0",
				 pAd->CommonCfg.WscStaPbcProbeInfo.
				 StaMacAddr[0], 6);
			hex_dump("StaMacAddr1",
				 pAd->CommonCfg.WscStaPbcProbeInfo.
				 StaMacAddr[1], 6);
			hex_dump("StaMacAddr2",
				 pAd->CommonCfg.WscStaPbcProbeInfo.
				 StaMacAddr[2], 6);
			hex_dump("StaMacAddr3",
				 pAd->CommonCfg.WscStaPbcProbeInfo.
				 StaMacAddr[3], 6);
		}
		if (pAd->CommonCfg.WscPBCOverlap == TRUE) {
			...
			RTMPSendWirelessEvent(pAd, IW_WSC_PBC_SESSION_OVERLAP,
					      NULL, pWpsCtrl->EntryIfIdx, 0);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IWSC_PeerEapolStart: PBC Session Overlap!\n"));
		...
		if (pEntry->bIWscSmpbcAccept == FALSE) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IWSC_PeerEapolStart:: pEntry->bIWscSmpbcAccept == FALSE\n"));
			return FALSE;
		}

		if (pWpsCtrl->EapMsgRunning == TRUE) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IWSC_PeerEapolStart:: Busy now\n"));
			return FALSE;
		}
...
VOID IWSC_RoleAction(IN PRTMP_ADAPTER pAd, IN INT WscConfMode)
{
	...
	if (WscConfMode == WSC_REGISTRAR) {
		...
		if (!pAd->StaCfg.IWscInfo.bLimitedUI)
			...
			ConfigMethod &= (~WPS_CONFIG_METHODS_KEYPAD);
		WscBuildBeaconIE(pAd, pWpsCtrl->WscConfStatus, TRUE, WscMode,
				 ConfigMethod, BSS0, NULL, 0, STA_MODE);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_REGISTRAR,
		...
		pWpsCtrl->WscState = WSC_STATE_LINK_UP;
		pWpsCtrl->WscStatus = STATUS_WSC_LINK_UP;
		RTMPSendWirelessEvent(pAd, IW_IWSC_BECOME_REGISTRAR, NULL,
				      pWpsCtrl->EntryIfIdx, 0);
	} else {
		...
		RTMPSendWirelessEvent(pAd, IW_IWSC_BECOME_ENROLLEE, NULL,
				      pWpsCtrl->EntryIfIdx, 0);
...
VOID IWSC_AddSmpbcEnrollee(IN PRTMP_ADAPTER pAd, IN PUCHAR pPeerAddr)
{
	PWSC_CTRL pWpsCtrl = &pAd->StaCfg.WscControl;
	PIWSC_INFO pIWscInfo = &pAd->StaCfg.IWscInfo;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("<----- IWSC_AddSmpbcEnrollee (WscConfMode = %d, WscStatus = %d)\n",
	...
	if (pWscPeerEntry &&
		...
		if (pEntry == NULL)
			pEntry =
			    MacTableInsertEntry(pAd, pPeerAddr, BSS0,
						OPMODE_STA, FALSE);
		if (pEntry) {
			pEntry->bIWscSmpbcAccept = TRUE;
			if ((pWpsCtrl->WscConfMode == WSC_DISABLE) &&
			    (pWpsCtrl->WscStatus == STATUS_WSC_CONFIGURED
			     || pWpsCtrl->WscStatus ==
			     STATUS_WSC_IBSS_WAIT_NEXT_SMPBC_ENROLLEE)) {
				pWpsCtrl->WscConfMode = WSC_REGISTRAR;
				...
				WscBuildProbeRespIE(pAd,
						    ...
						    DEV_PASS_ID_SMPBC,
						    pAd->StaCfg.WscControl.
						    WscConfigMethods, BSS0,
						    NULL, 0, STA_MODE);
			}
			DBGPRINT(RT_DEBUG_TRACE,
				 ("SMPBC Enrollee - %02x:%02x:%02x:%02x:%02x:%02x\n",
		...
		if (pEntry == NULL) {
			pEntry =
			    MacTableInsertEntry(pAd, pPeerAddr, BSS0,
						OPMODE_STA, FALSE);
			pWscPeerEntry =
			    WscFindPeerEntry(&pWpsCtrl->WscPeerList, pPeerAddr);
		...
		if (pEntry && !pEntry->bIWscSmpbcAccept) {
			pEntry->bIWscSmpbcAccept = TRUE;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Accept this peer enrollee - %02x:%02x:%02x:%02x:%02x:%02x\n",
				  pEntry->Addr[0], pEntry->Addr[1],
				  pEntry->Addr[2], pEntry->Addr[3],
				  pEntry->Addr[4], pEntry->Addr[5]));

			WscInsertPeerEntryByMAC(&pWpsCtrl->WscPeerList,
						pPeerAddr);
			pWscPeerEntry =
			    WscFindPeerEntry(&pWpsCtrl->WscPeerList, pPeerAddr);
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if ((pBss->WpsAP) &&
		    ...
		    ((pBss->WscDPIDFromWpsAP == DEV_PASS_ID_PIN)
		     || (pBss->WscDPIDFromWpsAP == DEV_PASS_ID_REG))) {
			return (ULONG) i;
		}
	}
			     ...
			     OUT PUCHAR pOutBuf, OUT PUSHORT pIeLen)
{
	...
	/* Manufacture */
	templen =
	    AppendWSCTLV(WSC_ID_MANUFACTURER, pData,
			 pReg->SelfInfo.Manufacturer,
			 strlen((PSTRING) pReg->SelfInfo.Manufacturer));
	pData += templen;
	Len += templen;
	...
	templen =
	    AppendWSCTLV(WSC_ID_MODEL_NAME, pData, pReg->SelfInfo.ModelName,
			 strlen((PSTRING) pReg->SelfInfo.ModelName));
	pData += templen;
	Len += templen;

	/* Model Number */
	templen =
	    AppendWSCTLV(WSC_ID_MODEL_NUMBER, pData, pReg->SelfInfo.ModelNumber,
			 strlen((PSTRING) pReg->SelfInfo.ModelNumber));
	pData += templen;
	Len += templen;

	/* Device Name */
	templen =
	    AppendWSCTLV(WSC_ID_DEVICE_NAME, pData, pReg->SelfInfo.DeviceName,
			 strlen((PSTRING) pReg->SelfInfo.DeviceName));
	pData += templen;
	Len += templen;
...
BOOLEAN IWSC_IpContentForCredential(IN PRTMP_ADAPTER pAd)
{
	PIWSC_INFO pIWscInfo = &pAd->StaCfg.IWscInfo;
	...
	if (pIWscInfo->RegDepth == 0) {
		...
		pIWscInfo->PeerIpv4Addr =
		    pIWscInfo->SelfIpv4Addr + pIWscInfo->IpDevCount;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("1) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n",
			  pIWscInfo->SelfIpv4Addr, pIWscInfo->PeerIpv4Addr));
		return TRUE;
	} else {
		...
		if (pIWscInfo->SelfIpv4Addr == 0) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IMPOSSIBLE!! pIWscInfo->SelfIpv4Addr = %d\n",
		...
		pIWscInfo->PeerIpv4Addr =
		    (pIWscInfo->SelfIpv4Addr & pIWscInfo->CurrentIpRange) +
		    (dev_count << shift_offset);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("2) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n",
			  pIWscInfo->SelfIpv4Addr, pIWscInfo->PeerIpv4Addr));
		return TRUE;
	}
}
