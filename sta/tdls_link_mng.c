...
/*
    ...
    ==========================================================================
 */
VOID TDLS_StateMachineInit(IN PRTMP_ADAPTER pAd,
			   IN STATE_MACHINE * Sm,
			   OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineInit(Sm, (STATE_MACHINE_FUNC *) Trans,
			 (ULONG) MAX_TDLS_STATE, (ULONG) MAX_TDLS_MSG,
			 (STATE_MACHINE_FUNC) Drop, TDLS_IDLE,
			 TDLS_MACHINE_BASE);

	/* the first column */
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_MLME_TDLS_SETUP_REQ,
			      (STATE_MACHINE_FUNC) TDLS_MlmeSetupReqAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_PEER_TDLS_SETUP_REQ,
			      (STATE_MACHINE_FUNC) TDLS_PeerSetupReqAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_PEER_TDLS_SETUP_RSP,
			      (STATE_MACHINE_FUNC) TDLS_PeerSetupRspAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_PEER_TDLS_SETUP_CONF,
			      (STATE_MACHINE_FUNC) TDLS_PeerSetupConfAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_MLME_TDLS_TEAR_DOWN,
			      (STATE_MACHINE_FUNC) TDLS_MlmeTearDownAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_PEER_TDLS_TEAR_DOWN,
			      (STATE_MACHINE_FUNC) TDLS_PeerTearDownAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_MLME_TDLS_DISCOVER_REQ,
			      (STATE_MACHINE_FUNC) TDLS_MlmeDiscoveryReqAction);
	StateMachineSetAction(Sm, TDLS_IDLE, MT2_PEER_TDLS_DISCOVER_REQ,
			      (STATE_MACHINE_FUNC) TDLS_PeerDiscoveryReqAction);

	for (i = 0; i < MAX_NUMBER_OF_DLS_ENTRY; i++) {
		pAd->StaCfg.TdlsInfo.TDLSEntry[i].pAd = pAd;

		RTMPInitTimer(pAd, &pAd->StaCfg.TdlsInfo.TDLSEntry[i].Timer,
			      GET_TIMER_FUNCTION(TDLS_TimeoutAction),
		       ...
		       OUT PULONG pFrameLen, IN PRT_802_11_TDLS pTDLS)
{
	...
	if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
		...
		NdisMoveMemory(CipherTmp, CipherSuiteTDLSWpa2PskAes,
			       CipherTmpLen);

		// Insert RSN_IE to outgoing frame
		MakeOutgoingFrame((pFrameBuf + *pFrameLen), &tmp,
				  CipherTmpLen, &CipherTmp, END_OF_ARGS);
	...
	if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
		...
		TDLS_InsertFTIE(pAd,
				...
				Length,
				FtIe.MICCtr,
				FtIe.MIC, FtIe.ANonce, FtIe.SNonce);

		/* Timeout Interval (7.3.2.49) */
		TDLS_InsertTimeoutIntervalIE(pAd, (pFrameBuf + *pFrameLen), pFrameLen, 2,	/* key lifetime interval */
					     KeyLifetime);

		pTDLS->KeyLifetime = KeyLifetime;
	}
	// fill Supported Regulatory Classes
	TDLS_SupportedRegulatoryClasses(pAd, (pFrameBuf + *pFrameLen),
					pFrameLen);
...
TDLS_BuildSetupResponse(IN PRTMP_ADAPTER pAd,
			...
			IN UCHAR FTLen,
			IN PUCHAR pFTIe,
			IN UCHAR TILen, IN PUCHAR pTIIe, IN UINT16 StatusCode)
{
	/* fill action code */
	TDLS_InsertActField(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
	...
	/* fill status code */
	TDLS_InsertStatusCode(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
			      StatusCode);
	...
	if (StatusCode == MLME_SUCCESS) {
		...
		// fill support rate
		TDLS_InsertSupportRateIE(pAd, (pFrameBuf + *pFrameLen),
					 pFrameLen);
		...
		// fill support channels
		TDLS_InsertSupportChannelIE(pAd, (pFrameBuf + *pFrameLen),
					    pFrameLen);
		...
		// fill WMM
		TDLS_InsertWMMIE(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
				 TRUE);
		...
		if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
			...
			TDLS_FTDeriveTPK(pTDLS->MacAddr,	/* MAC Address of Initiator */
					 pAd->CurrentAddress,	/* I am Responder */
					 pTDLS->ANonce,
					 pTDLS->SNonce,
					 pAd->CommonCfg.Bssid,
					 key_len, pTDLS->TPK, pTDLS->TPKName);
			...
			/* 7. FTIE without the IE header, with the MIC field of FTIE set to zero (82 bytes) */
			{
				...
				LinkIdentifier[1] = 18;
				NdisMoveMemory(&LinkIdentifier[2],
					       pAd->CommonCfg.Bssid, 6);
				NdisMoveMemory(&LinkIdentifier[8],
					       pTDLS->MacAddr, 6);
				NdisMoveMemory(&LinkIdentifier[14],
					       pAd->CurrentAddress, 6);
				...
				AES_CMAC(content, c_len, pTDLS->TPK, 16, mic,
					 &tmp_aes_len);
		...
		// fill 20/40 BSS Coexistence (7.3.2.61)
#ifdef DOT11N_DRAFT3
		TDLS_InsertBSSCoexistenceIE(pAd, (pFrameBuf + *pFrameLen),
					    pFrameLen);
#endif				// DOT11N_DRAFT3 //
#endif				// DOT11_N_SUPPORT //

		/* fill link identifier */
		TDLS_InsertLinkIdentifierIE(pAd, (pFrameBuf + *pFrameLen),
					    pFrameLen, pTDLS->MacAddr,
					    pAd->CurrentAddress);
	}
}
	...
	if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
		...
		*pFrameLen = *pFrameLen + tmp;
		 ...
		 */
		{
			...
			LinkIdentifier[0] = IE_TDLS_LINK_IDENTIFIER;
			LinkIdentifier[1] = 18;
			NdisMoveMemory(&LinkIdentifier[2], pAd->CommonCfg.Bssid,
				       6);
			NdisMoveMemory(&LinkIdentifier[8], pAd->CurrentAddress,
				       6);
		...
		TDLS_InsertFTIE(pAd,
				...
				pFrameLen,
				sizeof(FT_FTIE),
				ft->MICCtr, ft->MIC, ft->ANonce, ft->SNonce);
...
TDLS_BuildTeardown(IN PRTMP_ADAPTER pAd,
		   OUT PUCHAR pFrameBuf,
		   OUT PULONG pFrameLen,
		   IN PRT_802_11_TDLS pTDLS, IN UINT16 ReasonCode)
{
	/* fill action code */
	TDLS_InsertActField(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
	...
	/* fill reason code */
	TDLS_InsertReasonCode(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
			      ReasonCode);
	...
	if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
		...
		if (pTDLS->bInitiator) {
			...
			NdisMoveMemory(&LinkIdentifier[14], pAd->CurrentAddress,
				       6);
		} else {
			NdisMoveMemory(&LinkIdentifier[8], pAd->CurrentAddress,
				       6);
		...
		MakeOutgoingFrame(content, &tmp_len,
				  ...
				  2, &ReasonCode,
				  1, &pTDLS->Token, 1, &seq, END_OF_ARGS);
		c_len += tmp_len;

		/* concatenate FTIE */
		MakeOutgoingFrame(content + c_len, &tmp_len,
				  FTIe[1] + 2, FTIe, END_OF_ARGS);
		c_len += tmp_len;
...
USHORT TDLS_TPKMsg1Process(IN PRTMP_ADAPTER pAd,
			   ...
			   IN UCHAR RsnLen,
			   IN PUCHAR pFTIe,
			   IN UCHAR FTLen, IN PUCHAR pTIIe, IN UCHAR TILen)
{
	...
	if ((FTLen != (sizeof(FT_FTIE) + 2))
	    || RTMPEqualMemory(&ft->MICCtr, TdlsZeroSsid, 2) == 0
	    || (RTMPEqualMemory(ft->MIC, TdlsZeroSsid, 16) == 0)
	    || (RTMPEqualMemory(ft->ANonce, TdlsZeroSsid, 32) == 0))
...
==========================================================================
*/
USHORT TDLS_TPKMsg2Process(IN PRTMP_ADAPTER pAd,
			   IN PRT_802_11_TDLS pTDLS,
			   IN PUCHAR pRsnIe,
			   IN UCHAR RsnLen,
			   IN PUCHAR pFTIe,
			   IN UCHAR FTLen,
			   IN PUCHAR pTIIe,
			   IN UCHAR TILen, OUT PUCHAR pTPK, OUT PUCHAR pTPKName)
{
	USHORT StatusCode = MLME_SUCCESS;
	UCHAR CipherTmp[64] = { 0 };
	UCHAR CipherTmpLen = 0;
	...
	/* Validate TIIE */
	if ((TILen != 7) || (pTIIe[2] != 2)
	    || (le2cpu32(*((PULONG) (pTIIe + 3))) < TDLS_KEY_TIMEOUT))
		return TDLS_STATUS_CODE_UNACCEPTABLE_LIFETIME;
	...
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
	/* Derive TPK-KCK for MIC key, TPK-TK for direct link data */
	TDLS_FTDeriveTPK(pAd->CurrentAddress,	/* I am Initiator */
			 pTDLS->MacAddr,	/* MAC Address of Responder */
			 ft->ANonce,
			 ft->SNonce,
			 pAd->CommonCfg.Bssid, key_len, pTPK, pTPKName);
	...
	/* 7. FTIE without the IE header, with the MIC field of FTIE set to zero (82 bytes) */
	{
		...
		/* concatenate FTIE */
		MakeOutgoingFrame(content + c_len, &tmp_len,
				  (sizeof(FT_FTIE) + 2), pFTIe, END_OF_ARGS);
		c_len += tmp_len;
...
==========================================================================
*/
USHORT TDLS_TPKMsg3Process(IN PRTMP_ADAPTER pAd,
			   IN PRT_802_11_TDLS pTDLS,
			   IN PUCHAR pRsnIe,
			   IN UCHAR RsnLen,
			   IN PUCHAR pFTIe,
			   IN UCHAR FTLen, IN PUCHAR pTIIe, IN UCHAR TILen)
{
	...
	if ((FTLen != (sizeof(FT_FTIE) + 2))
	    || RTMPEqualMemory(&ft->MICCtr, TdlsZeroSsid, 2) == 0
	    || (RTMPEqualMemory(ft->SNonce, pTDLS->SNonce, 32) == 0)
	    || (RTMPEqualMemory(ft->ANonce, pTDLS->ANonce, 32) == 0))
		return REASON_FT_INVALID_FTIE;

	/* Validate TIIE */
	/* */
	if ((TILen != 7) || (pTIIe[2] != 2)
	    || (le2cpu32(*((PULONG) (pTIIe + 3))) < pTDLS->KeyLifetime))
		return TDLS_STATUS_CODE_UNACCEPTABLE_LIFETIME;
...
NDIS_STATUS
TDLS_DiscoveryRspAction(IN PRTMP_ADAPTER pAd,
			IN UCHAR PeerToken, IN PUCHAR pPeerMac)
{
	PUCHAR pOutBuffer = NULL;
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ACT - TDLS_DiscoveryRspAction() allocate memory failed \n"));
		return NStatus;
	}

	ActHeaderInit(pAd, &Frame.Hdr, pPeerMac, pAd->CurrentAddress,
		      pAd->CommonCfg.Bssid);
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
			  sizeof(FRAME_ACTION_HDR), &Frame, END_OF_ARGS);
	...
	if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
		...
		// Timeout Interval (7.3.2.49)
		TDLS_InsertTimeoutIntervalIE(pAd, (pOutBuffer + FrameLen), &FrameLen, 2,	/* key lifetime interval */
					     KeyLifetime);
	}
	// fill Supported Regulatory Classes
	TDLS_SupportedRegulatoryClasses(pAd, (pOutBuffer + FrameLen),
					&FrameLen);
...
*/
NDIS_STATUS
TDLS_SetupRequestAction(IN PRTMP_ADAPTER pAd, IN PRT_802_11_TDLS pTDLS)
{
	UCHAR TDLS_ETHERTYPE[] = { 0x89, 0x0d };
	UCHAR Header802_3[14];
	PUCHAR pOutBuffer = NULL;
	...
	MAKE_802_3_HEADER(Header802_3, pTDLS->MacAddr, pAd->CurrentAddress,
			  TDLS_ETHERTYPE);
			 ...
			 IN UCHAR TILen, IN PUCHAR pTIIe, IN UINT16 StatusCode)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ACT - TDLS_SetupResponseAction() allocate memory failed \n"));
		return NStatus;
	}
	...
	TDLS_BuildSetupResponse(pAd, pOutBuffer, &FrameLen, pTDLS, RsnLen,
				pRsnIe, FTLen, pFTIe, TILen, pTIIe, StatusCode);
			...
			IN UCHAR TILen, IN PUCHAR pTIIe, IN UINT16 StatusCode)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ACT - TDLS_SetupConfirmAction() allocate memory failed \n"));
		return NStatus;
	}
	...
	TDLS_BuildSetupConfirm(pAd, pOutBuffer, &FrameLen, pTDLS, RsnLen,
			       pRsnIe, FTLen, pFTIe, TILen, pTIIe, StatusCode);
	...
	for (idx = 0; idx < MAX_NUM_OF_TDLS_ENTRY; idx++) {
		...
		if (pTDLS->Valid && (pTDLS->Status >= TDLS_MODE_CONNECTED)) {
			...
			if (pAd->StaCfg.bRadio == TRUE) {
				if (bDirect)
					TDLS_TearDownAction(pAd, pTDLS,
							    TDLS_REASON_CODE_TEARDOWN_FOR_UNSPECIFIED_REASON,
							    TRUE);
				else
					TDLS_TearDownAction(pAd, pTDLS,
							    TDLS_REASON_CODE_TEARDOWN_DUE_TO_PEER_STA_UNREACHABLE,
							    FALSE);
			}
...
*/
VOID
TDLS_TearDownPeerLink(IN PRTMP_ADAPTER pAd,
		      IN PUCHAR pPeerAddr, IN BOOLEAN bDirect)
{
	UCHAR EntryIdx;
	BOOLEAN TimerCancelled;
	PRT_802_11_TDLS pTDLS = NULL;
	...
	for (EntryIdx = 0; EntryIdx < MAX_NUM_OF_TDLS_ENTRY; EntryIdx++) {
		...
		if ((pTDLS->Valid) && (pTDLS->Status >= TDLS_MODE_CONNECTED)
		    && MAC_ADDR_EQUAL(pPeerAddr, pTDLS->MacAddr)) {
			pTDLS->Status = TDLS_MODE_NONE;
			pTDLS->Valid = FALSE;
			RTMPCancelTimer(&pTDLS->Timer, &TimerCancelled);
...
TDLS_TearDownAction(IN PRTMP_ADAPTER pAd,
		    IN PRT_802_11_TDLS pTDLS,
		    IN UINT16 ReasonCode, IN BOOLEAN bDirect)
{
	UCHAR TDLS_ETHERTYPE[] = { 0x89, 0x0d };
	UCHAR Header802_3[14];
	PUCHAR pOutBuffer = NULL;
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ACT - TDLS_TearDownAction() allocate memory failed \n"));
		return NStatus;
	}
...
*/
VOID
TDLS_MlmeDiscoveryReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	UCHAR PeerMacAddr[MAC_ADDR_LEN];
	NDIS_STATUS NStatus = NDIS_STATUS_SUCCESS;
#ifdef TDLS_AUTOLINK_SUPPORT
	PLIST_HEADER pTdlsDiscoveryEnList =
	    &pAd->StaCfg.TdlsInfo.TdlsDiscovPeerList;
	...
	if (INFRA_ON(pAd)) {
		...
		if (NStatus != NDIS_STATUS_SUCCESS) {
#ifdef TDLS_AUTOLINK_SUPPORT
			RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
				      TdlsDiscovPeerListSemLock);
			TDLS_DelDiscoveryEntryByMAC(pTdlsDiscoveryEnList,
						    PeerMacAddr);
			RTMP_SEM_UNLOCK(&pAd->StaCfg.TdlsInfo.
					TdlsDiscovPeerListSemLock);
#endif				// TDLS_AUTOLINK_SUPPORT //
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - TDLS_MlmeDiscoveryReqAction() Build Discovery Setup Request Fail !!!\n"));
		} else {
#ifdef TDLS_AUTOLINK_SUPPORT
			pTdlsPeer =
			    TDLS_FindDiscoveryEntry(pTdlsDiscoveryEnList,
						    PeerMacAddr);
#endif				// TDLS_AUTOLINK_SUPPORT //
		}
	} else {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_MlmeDiscoveryReqAction() Build Discovery Setup Request Fail, Because STA is not connect to AP!!!\n"));
	}
...
*/
VOID
TDLS_PeerDiscoveryReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	NDIS_STATUS NStatus = NDIS_STATUS_SUCCESS;
	UCHAR Token;
	...
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + 3)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovReqSanity --> Invaild packet length - (action header) \n"));
		return;
	}
	...
	if (RemainLen < 1) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovReqSanity --> Invaild packet length - (dialog token) \n"));
		return;
	}
	...
	if ((RemainLen < 20) || (Ptr[0] != IE_TDLS_LINK_IDENTIFIER)
	    || (Ptr[1] != 18)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovReqSanity --> Invaild packet length - (link identifier) \n"));
		return;
	}
	if (!MAC_ADDR_EQUAL(Ptr + 2, pAd->CommonCfg.Bssid)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovReqSanity --> It's not my BSSID\n"));
		return;
	} else if (!MAC_ADDR_EQUAL(Ptr + 14, pAd->CurrentAddress)) {
		DBGPRINT_RAW(RT_DEBUG_WARN,
			     ("PeerTdlsDiscovReqSanity --> It's not my MAC address\n"));
		return;
	}
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_PeerDiscoveryReqAction() Build Discovery Response Fail !!!\n"));
	}
...
==========================================================================
*/
VOID TDLS_DiscoveryRspPublicAction(IN PRTMP_ADAPTER pAd,
				   ...
				   IN VOID * Msg, IN ULONG MsgLen)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("TDLS ===> TDLS_DiscoveryRspPublicAction() \n"));

	hex_dump("TDLS discovery response receive pack", Msg, MsgLen);

	if (!PeerTdlsDiscovRspSanity(pAd,
				     Msg,
				     MsgLen,
				     &Token,
				     PeerAddr,
				     ...
				     &FTLen,
				     FTIe,
				     &TILen, TIIe, &LinkIdentLen, &LinkIdent)) {
	...
	if (StatusCode == MLME_SUCCESS) {
		...
		if (pAd->StaCfg.TdlsInfo.TdlsAutoLink) {
			PLIST_HEADER pTdlsDiscovryEnList =
			    &pAd->StaCfg.TdlsInfo.TdlsDiscovPeerList;
			PTDLS_DISCOVERY_ENTRY pTdlsPeer = NULL;

			pTdlsPeer =
			    TDLS_FindDiscoveryEntry(pTdlsDiscovryEnList,
						    PeerAddr);

			if (pTdlsPeer) {
				PLIST_HEADER pTdlsDiscovEnList =
				    &pAd->StaCfg.TdlsInfo.TdlsDiscovPeerList;
				CHAR Rssi = -99;
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Discovery Response Rssi = %d !!!\n",
					  Rssi));

				if (pTdlsPeer->bConnected) {
					if ((pTdlsPeer->bConnectedFirstTime)
					    && (pTdlsPeer->RetryCount == 1)) {
						pTdlsPeer->AvgRssi0 = Rssi;
						pTdlsPeer->RetryCount--;
						pTdlsPeer->bConnectedFirstTime =
						    FALSE;
						DBGPRINT(RT_DEBUG_ERROR,
							 ("Connected first time discovery !!!\n"));
					} else {
								...
								TdlsAutoTeardownRssiThreshold))
							{
								...
								if (INFRA_ON
									    ...
									    MAX_NUM_OF_TDLS_ENTRY)
									{
										DBGPRINT
										    (RT_DEBUG_ERROR,
										     ("TDLS - can not find or full the LinkId!\n"));
									} else
									    if
									    (idx
									     >=
					...
					if (pTdlsPeer->CurrentState ==
					    TDLS_DISCOVERY_IDLE) {
						pTdlsPeer->CurrentState =
						    TDLS_DISCOVERY_FIRST_TIME;
						pTdlsPeer->AvgRssi0 = Rssi;
					} else if (pTdlsPeer->CurrentState ==
						   TDLS_DISCOVERY_FIRST_TIME) {
						if ((pTdlsPeer->AvgRssi0 >
						     pAd->StaCfg.TdlsInfo.
						     TdlsAutoSetupRssiThreshold)
						    && (Rssi >
							pAd->StaCfg.TdlsInfo.
							TdlsAutoSetupRssiThreshold))
						{
							...
							pTdlsPeer->
							    CurrentState =
							    TDLS_DISCOVERY_TO_SETUP_FAIL;
						}
					}
				}
			} else {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Can't find discovery entry on discovery table !!!\n"));
			}
		}
#endif				// TDLS_AUTOLINK_SUPPORT //
	}
...
VOID TDLS_MlmeSetupReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pAd->StaActive.ExtCapInfo.TDLSProhibited == TRUE) {
		DBGPRINT(RT_DEBUG_OFF,
			 ("TDLS - TDLS_MlmeSetupReqAction() AP Prohibited TDLS !!!\n"));
		return;
	}
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_MlmeSetupReqAction() Build Setup Request Fail !!!\n"));
	}
...
VOID TDLS_PeerSetupReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pAd->StaActive.ExtCapInfo.TDLSProhibited == TRUE) {
		DBGPRINT(RT_DEBUG_OFF,
			 ("TDLS - TDLS_PeerSetupReqAction() AP Prohibited TDLS !!!\n"));
		goto CleanUp;
	}
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("TDLS - PeerTdlsSetupReqAction() received a request from %02x:%02x:%02x:%02x:%02x:%02x\n",
		  PeerAddr[0], PeerAddr[1], PeerAddr[2], PeerAddr[3],
		  PeerAddr[4], PeerAddr[5]));
	...
	if (pAd->StaCfg.TdlsInfo.TdlsAutoLink) {
		...
		PLIST_HEADER pTdlsBlackEnList =
		    &pAd->StaCfg.TdlsInfo.TdlsBlackList;
		...
		if (pTdlsBlackEntry) {
			if (pTdlsBlackEntry->CurrentState ==
			    TDLS_BLACK_TDLS_BY_TEARDOWN)
				StatusCode = MLME_REQUEST_DECLINED;
		}
	}
#endif				// TDLS_AUTOLINK_SUPPORT //
	...
	if (StatusCode == MLME_SUCCESS) {
		for (idx = MAX_NUM_OF_TDLS_ENTRY - 1; idx >= 0; idx--) {
			...
			if (pTDLS->Valid
			    && MAC_ADDR_EQUAL(PeerAddr, pTDLS->MacAddr)) {
				if (pTDLS->Status == TDLS_MODE_WAIT_RESPONSE) {
					if (RTMPCompareMemory
					    (PeerAddr, pAd->CurrentAddress,
					     MAC_ADDR_LEN) == 2) {
						/*
						   ...
						   frame.
						 */
						RTMPCancelTimer(&(pTDLS->Timer),
								&TimerCancelled);
						pTDLS->Token = 0;
						pTDLS->Valid = FALSE;
						NdisMoveMemory(pTDLS->MacAddr,
							       PeerAddr,
							       MAC_ADDR_LEN);
						pTDLS->Status = TDLS_MODE_NONE;
						idx = -1;
						//pTDLS = pTmpTDLS;
						DBGPRINT(RT_DEBUG_ERROR,
							 ("TDLS - PeerTdlsSetupReqAction() find the same entry \n"));
					} else {
						/*
						   11.20.2 TDLS Link Establishment
						...
						bDiscard = TRUE;
						DBGPRINT(RT_DEBUG_ERROR,
							 ("TDLS - PeerTdlsSetupReqAction() find the same entry and discard it\n"));
					}
		...
		if (idx >= 0) {
			...
			if (pTDLS->Token == Token) {
				DBGPRINT(RT_DEBUG_WARN,
					 ("TDLS - PeerTdlsSetupReqAction() receive the same token TDLS request !!!\n"));
				goto CleanUp;;
			}
		}
			...
			for (idx = (MAX_NUM_OF_TDLS_ENTRY - 1); idx >= 0; idx--) {
				...
				if (!pTDLS->Valid) {
					...
					NdisMoveMemory(pTDLS->MacAddr, PeerAddr,
						       MAC_ADDR_LEN);
					DBGPRINT(RT_DEBUG_ERROR,
						 ("TDLS - PeerTdlsSetupReqAction() create a new entry \n"));
		...
		if (idx < 0) {
			...
			StatusCode = MLME_REQUEST_DECLINED;
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupReqAction() TDLSEntry table full(only can support %d TDLS session) \n",
				  MAX_NUM_OF_TDLS_ENTRY));
		} else if (pTDLS) {
			//
			...
			if (pAd->StaCfg.WepStatus !=
				...
				Result =
				    TDLS_TPKMsg1Process(pAd, pTDLS, RsnIe,
							RsnLen, FTIe, FTLen,
							TIIe, TILen);
				if (Result != MLME_SUCCESS) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("TDLS - TDLS_TPKMsg1Process() Failed, reason=%d \n",
						  Result));
			...
			if ((HtCapLen > 0)
				...
				RTMPZeroMemory(&pTDLS->HtCapability,
					       SIZE_HT_CAP_IE);
				pTDLS->HtCapabilityLen = SIZE_HT_CAP_IE;
				NdisMoveMemory(&pTDLS->HtCapability, &HtCap,
					       SIZE_HT_CAP_IE);
			} else {
			...
			NdisMoveMemory(&pTDLS->TdlsExtCap, &ExtCap,
 ...
 send_out:
	...
	TDLS_SetupResponseAction(pAd, pTDLS, RsnLen, RsnIe, FTLen, FTIe, TILen,
				 TIIe, StatusCode);
...
VOID TDLS_PeerSetupRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("TDLS - PeerTdlsSetupRspAction() received a response from %02x:%02x:%02x:%02x:%02x:%02x with StatusCode=%d\n",
		  PeerAddr[0], PeerAddr[1], PeerAddr[2], PeerAddr[3],
		  PeerAddr[4], PeerAddr[5], StatusCode));
	...
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_PeerSetupRspAction() can not find the LinkId!\n"));
		goto CleanUp;
	}
	...
	if (StatusCode != MLME_SUCCESS) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - PeerTdlsSetupRspAction() received a failed StatusCode, terminate the setup procedure \n"));
	...
	while (StatusCode == MLME_SUCCESS) {
		...
		if (pTDLS->Status != TDLS_MODE_WAIT_RESPONSE) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupRspAction() Not in TDLS_MODE_WAIT_RESPONSE STATE\n"));
			StatusCode = MLME_REQUEST_DECLINED;
			break;
		}
		...
		if (pTDLS->Token != Token) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupRspAction() Not match with Dialig Token my token = %d, peer token = %d\n",
				  pTDLS->Token, Token));
		...
		if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
			...
			// RSNIE (7.3.2.25), FTIE (7.3.2.48), Timeout Interval (7.3.2.49)
			Result =
			    TDLS_TPKMsg2Process(pAd, pTDLS, RsnIe, RsnLen, FTIe,
						FTLen, TIIe, TILen, TPK,
						TPKName);
			if (Result != MLME_SUCCESS) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("TDLS - TDLS_TPKMsg2Process() Failed, reason=%d \n",
					  Result));
				if (Result == MLME_REQUEST_DECLINED)	// if mic error , ignore
				{
					goto CleanUp;
	...
	if (StatusCode == MLME_SUCCESS) {
		...
		if (pMacEntry && IS_ENTRY_TDLS(pMacEntry))
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - MacTable Entry exist !!!\n"));
		else
			pMacEntry =
			    MacTableInsertEntry(pAd, pTDLS->MacAddr,
		...
		if (pMacEntry) {
			...
			if ((pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED)
				...
				CLIENT_STATUS_CLEAR_FLAG(pMacEntry,
							 fCLIENT_STATUS_WMM_CAPABLE);
			}
			...
			if (pAd->StaCfg.WepStatus !=
				...
				if (pMacEntry->Aid > 1) {
					...
					NdisMoveMemory(PairwiseKey.Key,
						       &pTDLS->TPK[16], LEN_TK);
			...
			StatusCode = MLME_REQUEST_DECLINED;
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupRspAction() MacTableInsertEntry failed\n"));
		}
	} else
		StatusCode = MLME_REQUEST_DECLINED;
 ...
 send_out:
	...
	if (StatusCode == MLME_SUCCESS) {
		TDLS_SetupConfirmAction(pAd, pTDLS, RsnLen, RsnIe, FTLen, FTIe,
					TILen, TIIe, StatusCode);
		...
		if (pAd->StaCfg.TdlsInfo.TdlsAutoLink) {
			...
			RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
				      TdlsDiscovPeerListSemLock);
			TDLS_InsertDiscoveryPeerEntryByMAC(pTdlsDiscovEnList,
							   pTDLS->MacAddr,
							   TRUE);
			pPeerEntry =
			    TDLS_FindDiscoveryEntry(pTdlsDiscovEnList,
						    pTDLS->MacAddr);
			if (pPeerEntry) {
				pPeerEntry->bConnectedFirstTime = TRUE;
				pPeerEntry->bConnected = TRUE;
				pPeerEntry->RetryCount = 0;
				pPeerEntry->CurrentState =
				    TDLS_DISCOVERY_TO_SETUP_DONE;
			}
			RTMP_SEM_UNLOCK(&pAd->StaCfg.TdlsInfo.
					TdlsDiscovPeerListSemLock);
			...
			RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
				      TdlsBlackListSemLock);
			TDLS_DelBlackEntryByMAC(pTdlsBlackEnList,
						pTDLS->MacAddr);
			RTMP_SEM_UNLOCK(&pAd->StaCfg.TdlsInfo.
					TdlsBlackListSemLock);
		}
		...
		if ((pTDLS->Status >= TDLS_MODE_CONNECTED)
		    && (pTDLS->Token == Token)) {
			DBGPRINT(RT_DEBUG_WARN,
				 ("TDLS - PeerTdlsSetupRspAction() receive the same token TDLS response !!!\n"));
			goto CleanUp;
		}
...
VOID TDLS_PeerSetupConfAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!PeerTdlsSetupConfSanity(pAd,
				     Elem->Msg,
				     Elem->MsgLen,
				     &Token,
				     PeerAddr,
				     ...
				     TIIe,
				     &LinkIdentLen,
				     &LinkIdent, &AddHtInfoLen, &AddHtInfo)) {
		LocalStatusCode = MLME_REQUEST_DECLINED;
	}
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("TDLS - PeerTdlsSetupConfAction() received a confirm from %02x:%02x:%02x:%02x:%02x:%02x with StatusCode=%d\n",
		  PeerAddr[0], PeerAddr[1], PeerAddr[2], PeerAddr[3],
		  PeerAddr[4], PeerAddr[5], StatusCode));
	...
	if (StatusCode != MLME_SUCCESS) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - PeerTdlsSetupConfAction() received a failed StatusCode, terminate the setup procedure \n"));
	...
	while (StatusCode == MLME_SUCCESS) {
		...
		if (pTDLS->Status != TDLS_MODE_WAIT_CONFIRM) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupConfAction() Not in TDLS_MODE_WAIT_CONFIRM STATE\n"));
			StatusCode = MLME_REQUEST_DECLINED;
			break;
		}
		// Is the same Dialog Token?
		if (pTDLS->Token != Token) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - PeerTdlsSetupConfAction() Not match with Dialig Token \n"));
			StatusCode = MLME_REQUEST_DECLINED;
			break;
		}
		...
		if (pAd->StaCfg.WepStatus != Ndis802_11EncryptionDisabled) {
			...
			// RSNIE (7.3.2.25), FTIE (7.3.2.48), Timeout Interval (7.3.2.49)
			Result =
			    TDLS_TPKMsg3Process(pAd, pTDLS, RsnIe, RsnLen, FTIe,
						FTLen, TIIe, TILen);
			if (Result != MLME_SUCCESS) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("TDLS - TPKMsg3Process() Failed, reason=%d \n",
					  Result));
				StatusCode = Result;
				break;
			}
		...
		if (StatusCode == MLME_SUCCESS) {
			...
			if ((pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED)
			    || (pAd->CommonCfg.bWmmCapable))
				NdisMoveMemory(&pTDLS->EdcaParm, &EdcaParm,
					       sizeof(EDCA_PARM));
			else
				NdisZeroMemory(&pTDLS->EdcaParm,
					       sizeof(EDCA_PARM));
	...
	if (StatusCode == MLME_SUCCESS) {
		...
		if (pMacEntry) {
			...
			/* update UAPSD */
			pTDLS->QosCapability |=
			    (pTDLS->EdcaParm.EdcaUpdateCount & 0x0f);
			...
			pTDLS->Status = TDLS_MODE_CONNECTED;
			pTDLS->ChSwitchTime =
			    pAd->StaCfg.TdlsInfo.TdlsSwitchTime;
			pTDLS->ChSwitchTimeout =
			    pAd->StaCfg.TdlsInfo.TdlsSwitchTimeout;
			...
			if (pAd->StaCfg.TdlsInfo.TdlsAutoLink) {
				...
				RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.
					      TdlsDiscovPeerListSemLock);
				TDLS_InsertDiscoveryPeerEntryByMAC
				    (pTdlsDiscovEnList, pTDLS->MacAddr, TRUE);
				pPeerEntry =
				    TDLS_FindDiscoveryEntry(pTdlsDiscovEnList,
							    pTDLS->MacAddr);
				if (pPeerEntry) {
			...
			pTDLS->Valid = FALSE;
			DBGPRINT(RT_DEBUG_ERROR,
				 ("TDLS - TDLS_PeerSetupConfAction() MacTableInsertEntry failed\n"));
		}
	} else {
		if ((pTDLS->Status >= TDLS_MODE_CONNECTED)
		    && (pTDLS->Token == Token)) {
			DBGPRINT(RT_DEBUG_WARN,
				 ("TDLS - TDLS_PeerSetupConfAction() receive the same token TDLS confirm !!!\n"));
			return;
		}
...
VOID TDLS_MlmeTearDownAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!MlmeTdlsReqSanity
	    (pAd, Elem->Msg, Elem->MsgLen, &pTDLS, &ReasonCode, &IsViaAP))
		return;
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_MlmeTearDownAction() Build Setup Request Fail !!!\n"));
	} else {
		BOOLEAN TimerCancelled;
...
VOID TDLS_PeerTearDownAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!PeerTdlsTearDownSanity(pAd,
				    ...
				    &IsInitator,
				    &ReasonCode,
				    &FTLen, FTIe, &LinkIdentLen, &LinkIdent)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - PeerTdlsTearDownAction() from %02x:%02x:%02x:%02x:%02x:%02x Sanity Check Fail\n",
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("TDLS - PeerTdlsTearDownAction() from %02x:%02x:%02x:%02x:%02x:%02x with ReasonCode=%d\n",
	...
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - PeerTdlsTearDownAction() can not find from %02x:%02x:%02x:%02x:%02x:%02x on TDLS entry !!!\n",
			  SA[0], SA[1], SA[2], SA[3], SA[4], SA[5]));
		return;
	}
	// Point to the current Link ID
	...
	if (pTDLS->bInitiator != IsInitator) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - PeerTdlsTearDownAction() pTDLS->bInitiator = %x parse = %x !!!\n",
...
#ifdef TDLS_AUTOLINK_SUPPORT
		RTMP_SEM_LOCK(&pAd->StaCfg.TdlsInfo.TdlsBlackListSemLock);
		TDLS_InsertBlackEntryByMAC(pTdlsBlackEnList, SA,
					   TDLS_BLACK_TDLS_BY_TEARDOWN);
		RTMP_SEM_UNLOCK(&pAd->StaCfg.TdlsInfo.TdlsBlackListSemLock);
...
==========================================================================
*/
VOID TDLS_TimeoutAction(IN PVOID SystemSpecific1,
			...
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("TdlsTimeout - Failed to wait for the response, terminate the setup procedure (%02x:%02x:%02x:%02x:%02x:%02x)\n",
...
==========================================================================
*/
VOID TDLS_LinkTimeoutAction(IN PVOID SystemSpecific1,
			    IN PVOID FunctionContext,
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	PRT_802_11_TDLS pTDLS = (PRT_802_11_TDLS) FunctionContext;
