...
static VOID ApCliPeerAuthRspAtSeq2Action(IN PRTMP_ADAPTER pAd,
					 IN MLME_QUEUE_ELEM * Elem);

static VOID ApCliPeerAuthRspAtSeq4Action(IN PRTMP_ADAPTER pAd,
					 IN MLME_QUEUE_ELEM * Elem);

static VOID ApCliPeerDeauthAction(IN PRTMP_ADAPTER pAd,
				  IN MLME_QUEUE_ELEM * Elem);

static VOID ApCliAuthTimeoutAction(IN PRTMP_ADAPTER pAd,
				   IN MLME_QUEUE_ELEM * Elem);

static VOID ApCliInvalidStateWhenAuth(IN PRTMP_ADAPTER pAd,
				      IN MLME_QUEUE_ELEM * Elem);

static VOID ApCliMlmeDeauthReqAction(IN PRTMP_ADAPTER pAd,
				     IN MLME_QUEUE_ELEM * Elem);
			       ...
			       OUT STATE_MACHINE_FUNC Trans[])
{
	...
	/* timer init */
	RTMPInitTimer(pAd, &pAd->ApCliMlmeAux.ApCliAuthTimer,
	...
	if (pAd->ApCfg.ApCliTab[ifIndex].bBlockAssoc == TRUE) {
		...
		*pCurrState = APCLI_AUTH_REQ_IDLE;
		...
		ApCliMgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, Addr,
				      pAd->ApCliMlmeAux.Bssid, ifIndex);
		...
		if (MAC_ADDR_EQUAL(pAd->ApCliMlmeAux.Bssid, Addr2) && Seq == 2) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("APCLI AUTH - Receive AUTH_RSP seq#2 to me (Alg=%d, Status=%d)\n",
				  Alg, Status));
			RTMPCancelTimer(&pAd->ApCliMlmeAux.ApCliAuthTimer,
					&Cancelled);

			if (Status == MLME_SUCCESS) {
				if (pAd->ApCliMlmeAux.Alg ==
