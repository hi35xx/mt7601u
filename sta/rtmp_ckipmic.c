...
VOID CKIP_key_permute(OUT UCHAR * PK,	/* output permuted key */
...
VOID RTMPCkipMicInit(IN PMIC_CONTEXT pContext, IN PUCHAR CK)
{
...
VOID RTMPMicUpdate(IN PMIC_CONTEXT pContext, IN PUCHAR pOctets, IN INT len)
{
	INT byte_position;
	ULONG val;
...
ULONG RTMPMicGetCoefficient(IN PMIC_CONTEXT pContext)
{
	...
	if ((coeff_position & 3) == 0) {
		...
		RTMPAesEncrypt(&pContext->CK[0], &aes_counter[0],
			       pContext->coefficient);
	}
...
VOID RTMPAesEncrypt(IN PUCHAR key, IN PUCHAR data, IN PUCHAR ciphertext)
{
	INT round;
	INT i;
...
/* calculate the mic */
VOID RTMPMicFinal(IN PMIC_CONTEXT pContext, OUT UCHAR digest[4])
{
	INT byte_position;
	ULONG val;
	ULONGLONG sum, utmp;
...
VOID RTMPCkipInsertCMIC(IN PRTMP_ADAPTER pAd,
			OUT PUCHAR pMIC,
			IN PUCHAR p80211hdr,
			IN PNDIS_PACKET pPacket,
			IN PCIPHER_KEY pKey, IN PUCHAR mic_snap)
{
	...
	do {
		...
		NdisGetNextBuffer(PacketInfo.pFirstBuffer,
				  &PacketInfo.pFirstBuffer);
		if (PacketInfo.pFirstBuffer) {
			NDIS_QUERY_BUFFER(PacketInfo.pFirstBuffer, &pSrcBufVA,
					  &SrcBufLen);
		} else
			break;
	} while (TRUE);
