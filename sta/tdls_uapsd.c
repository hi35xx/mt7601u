...
static VOID TDLS_UAPSD_PeerTrafficIndAction(IN PRTMP_ADAPTER pAd,
					    IN MLME_QUEUE_ELEM * pElem);
/* receive a traffic response frame */
static VOID TDLS_UAPSD_PeerTrafficRspAction(IN PRTMP_ADAPTER pAd,
					    IN MLME_QUEUE_ELEM * pElem);

/* build the traffic indication frame */
static ULONG TDLS_UAPSD_TrafficIndBuild(IN PRTMP_ADAPTER pAd,
					IN UCHAR * pPeerMac,
					OUT UCHAR * pFrameBuf,
					OUT UCHAR * pHeader802_3);
/* build the traffic indication frame payload */
static VOID TDLS_UAPSD_TrafficIndPayloadBuild(IN PRTMP_ADAPTER pAd,
					      OUT PUCHAR pFrameBuf,
					      OUT PULONG pFrameLen,
					      IN PRT_802_11_TDLS pTDLS);

/* send a traffic indication frame */
static NDIS_STATUS TDLS_UAPSD_TrafficIndSend(IN PRTMP_ADAPTER pAd,
					     IN UCHAR * pPeerMac);
/* send a traffic response frame */
static NDIS_STATUS TDLS_UAPSD_TrafficRspSend(IN PRTMP_ADAPTER pAd,
					     IN UCHAR * pPeerMac,
					     IN UCHAR PeerToken);
/* build the traffic response frame body */
static VOID TDLS_UAPSD_TrafficRspBuild(IN PRTMP_ADAPTER pAd,
				       OUT PUCHAR pFrameBuf,
				       OUT PULONG pFrameLen,
				       IN PRT_802_11_TDLS pTDLS,
				       IN UCHAR PeerToken);
...
static UINT32 TDLS_UAPSD_CmdUtilHexGet(IN CHAR ** ppArgv);
/* get argument number value */
static UINT32 TDLS_UAPSD_CmdUtilNumGet(IN CHAR ** ppArgv);
/* get argument MAC value */
static VOID TDLS_UAPSD_CmdUtilMacGet(IN CHAR ** ppArgv, IN UCHAR * pDevMac);

/* simulate to send a TDLS Setup request to a peer */
static VOID TDLS_UAPSD_CmdSimSetupReqSend(IN PRTMP_ADAPTER pAd,
					  IN INT32 Argc, IN CHAR * pArgv);

/* simulate to receive a TDLS Traffic response from a peer */
static VOID TDLS_UAPSD_CmdSimTrafficRspRcv(IN PRTMP_ADAPTER pAd,
					   IN INT32 Argc, IN CHAR * pArgv);

/* simulate to receive a TDLS Traffic indication from a peer */
static VOID TDLS_UAPSD_CmdSimTrafficIndRcv(IN PRTMP_ADAPTER pAd,
					   IN INT32 Argc, IN CHAR * pArgv);

/* simulate to change the power save of a peer */
static VOID TDLS_UAPSD_CmdSimPeerPowerSaveChg(IN PRTMP_ADAPTER pAd,
					      IN INT32 Argc, IN CHAR * pArgv);

/* simulate to change our power save */
static VOID TDLS_UAPSD_CmdSimSelfPowerSaveChg(IN PRTMP_ADAPTER pAd,
					      IN INT32 Argc, IN CHAR * pArgv);

/* simulate to send a TDLS Traffic response to a peer */
static VOID TDLS_UAPSD_CmdSimTrafficRspSnd(IN PRTMP_ADAPTER pAd,
					   IN INT32 Argc, IN CHAR * pArgv);
...
========================================================================
*/
BOOLEAN TDLS_UAPSDP_Init(IN PRTMP_ADAPTER pAd, IN STATE_MACHINE * pFSM)
{
	/* init FSM */
	StateMachineSetAction(pFSM, TDLS_IDLE, MT2_PEER_TDLS_TRAFFIC_IND,
			      (STATE_MACHINE_FUNC)
			      TDLS_UAPSD_PeerTrafficIndAction);
	StateMachineSetAction(pFSM, TDLS_IDLE, MT2_PEER_TDLS_TRAFFIC_RSP,
			      (STATE_MACHINE_FUNC)
			      TDLS_UAPSD_PeerTrafficRspAction);
...
========================================================================
*/
BOOLEAN TDLS_UAPSDP_Release(IN PRTMP_ADAPTER pAd)
{
	/* free lock */
...
========================================================================
*/
NDIS_STATUS TDLS_UAPSDP_TrafficIndSend(IN PRTMP_ADAPTER pAd,
				       IN UCHAR * pPeerMac)
{
	NDIS_STATUS NStatus;
...
========================================================================
*/
BOOLEAN TDLS_UAPSDP_AsicCanSleep(IN PRTMP_ADAPTER pAd)
{
	...
	for (IdEntry = 0; IdEntry < MAX_NUM_OF_TDLS_ENTRY; IdEntry++) {
		pTDLS =
		    (PRT_802_11_TDLS) & pAd->StaCfg.TdlsInfo.TDLSEntry[IdEntry];
...
========================================================================
*/
VOID TDLS_UAPSDP_PsmModeChange(IN PRTMP_ADAPTER pAd,
			       IN USHORT PsmOld, IN USHORT PsmNew)
{
	...
	for (IdTdls = 0; IdTdls < MAX_NUM_OF_TDLS_ENTRY; IdTdls++) {
		pTDLS =
		    (RT_802_11_TDLS *) & pAd->StaCfg.TdlsInfo.TDLSEntry[IdTdls];
...
========================================================================
*/
static VOID TDLS_UAPSD_CmdPeerInfoDisplay(IN PRTMP_ADAPTER pAd,
					  IN INT32 Argc, IN CHAR * pArgv)
{
	MAC_TABLE_ENTRY *pMacEntry;
	UCHAR PeerMac[6];
...
========================================================================
*/
static VOID TDLS_UAPSD_CmdSelfInfoDisplay(IN PRTMP_ADAPTER pAd,
					  IN INT32 Argc, IN CHAR * pArgv)
{
	...
	printk("\n         Tr/De      %d/%d    %d/%d    %d/%d    %d/%d\n",
	       ...
	       pAd->CommonCfg.TDLS_bAPSDAC_VI,
	       pAd->CommonCfg.TDLS_bAPSDAC_VO, pAd->CommonCfg.TDLS_bAPSDAC_VO);
...
========================================================================
*/
static VOID TDLS_UAPSD_CmdConfigure(IN PRTMP_ADAPTER pAd,
				    IN INT32 Argc, IN CHAR * pArgv)
{
...
========================================================================
*/
INT TDLS_Ioctl(IN PRTMP_ADAPTER pAd, IN PSTRING pArgvIn)
{
	CHAR BufCmd[3] = { 0, 0, 0 };
	CHAR *pArgv, *pParam;
	UINT32 Command;
	...
	case TDLS_UAPSD_CONFIG:	/* tdls uapsd configure */
		DBGPRINT(RT_DEBUG_TRACE, ("tdls_cmd> configure our UAPSD\n"));
...
========================================================================
*/
INT Set_TdlsUapsdProc(IN PRTMP_ADAPTER pAd, IN PSTRING pArgvIn)
{
					...
					OUT UCHAR * pHeader802_3)
{
	...
	hex_dump("TDLS UAPSD Peer Traffic Ind sending packet", pFrameBuf,
		 FrameLen);
	/*
	   11.2.1.14.1 Peer U-APSD Behavior at the PU buffer STA
					      ...
					      IN PRT_802_11_TDLS pTDLS)
{
	...
	*pFrameLen = TempLen;
	...
	TDLS_InsertActField(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
			    CATEGORY_TDLS,
			    TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION);

	/* fill Dialog Token */
	TDLS_UAPSD_DIALOG_GET(pAd, Token);
	TDLS_InsertDialogToken(pAd, (pFrameBuf + *pFrameLen), pFrameLen, Token);
					     ...
					     IN UCHAR PeerToken)
{
	...
	TDLS_UAPSD_TrafficRspBuild(pAd, pOutBuffer, &FrameLen, pTDLS,
				   PeerToken);

	hex_dump("TDLS UAPSD Peer Traffic Response sending packet", pOutBuffer,
		 FrameLen);
	...
	TDLS_UAPSD_PKT_SEND_TO_PEER(pAd, Header802_3, pOutBuffer, FrameLen,
				    pTDLS);
				       ...
				       IN UCHAR PeerToken)
{
	...
	TDLS_InsertActField(pAd, (pFrameBuf + *pFrameLen), pFrameLen,
			    CATEGORY_TDLS,
			    TDLS_ACTION_CODE_PEER_TRAFFIC_RESPONSE);
					    ...
					    IN MLME_QUEUE_ELEM * pElem)
{
	...
	hex_dump("TDLS UAPSD Peer Traffic Ind receive pack", pElem->Msg,
		 pElem->MsgLen);
	...
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("TDLS - TDLS_UAPSD_PeerTrafficIndAction() can not find the LinkId!\n"));
		return;
	}
					    ...
					    IN MLME_QUEUE_ELEM * pElem)
{
	...
	hex_dump("TDLS UAPSD Peer Traffic Response receive pack", pElem->Msg,
		 pElem->MsgLen);
	...
	PeerTdlsBasicSanity(pAd,
			    pElem->Msg,
			    pElem->MsgLen,
			    pTDLS->bInitiator, &Token, PeerAddr1);
					  ...
					  IN INT32 Argc, IN CHAR * pArgv)
{
	...
	memcpy(pElem->Msg + LENGTH_802_11 + LENGTH_802_1_H, pOutBuffer, 3);
					   ...
					   IN INT32 Argc, IN CHAR * pArgv)
{
	...
	/* fill Dialog Token */
	TDLS_UAPSD_DIALOG_GET(pAd, Token);
	TDLS_InsertDialogToken(pAd, (pOutBuffer + FrameLen), &FrameLen, Token);
