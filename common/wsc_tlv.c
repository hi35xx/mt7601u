		 ...
		 IN USHORT varlen)
{
	...
	dataLen =
	    (varlen !=
	     (USHORT) 0) ? varlen : wsc_tlv_0b[WSC_TLV_ENT(index)].len;
				 ...
				 IN INT PlainLength, IN PWSC_CTRL pWscControl)
{
	...
	PWSC_PROFILE pProfile =
	    (PWSC_PROFILE) & pAdapter->StaCfg.WscControl.WscM7Profile;
	UCHAR *pTmp;
	...
	while (PlainLength > 4) {
		...
		case WSC_ID_SSID:
			...
			RTMPMoveMemory(pProfile->Profile[0].SSID.Ssid, pData,
		...
		case WSC_ID_MAC_ADDR:
			if (!MAC_ADDR_EQUAL
			    (pData,
			     pAdapter->StaCfg.WscControl.RegData.SelfInfo.
			     MacAddr))
		...
		case WSC_ID_NW_KEY:
			...
			RTMPMoveMemory(pProfile->Profile[0].Key, pData,
...
/*
	...
	========================================================================
*/
static BOOLEAN WscProcessCredential(IN PRTMP_ADAPTER pAdapter,
				    IN PUCHAR pPlainData,
				    IN INT PlainLength,
	...
	while (PlainLength > 4) {
		...
		case WSC_ID_SSID:
			...
			if (RTMPCheckStrPrintAble((CHAR *) pData, Idx)
				...
				NdisMoveMemory(pProfile->Profile[CurrentIdx].
					       SSID.Ssid, pData,
		...
		case WSC_ID_IP_ADDR_CONF_METHOD:
			tmpVal = get_unaligned((PUSHORT) pData);
			pProfile->Profile[CurrentIdx].IpConfigMethod =
			    be2cpu16(tmpVal);
			break;
		case WSC_ID_REGISTRAR_IPV4:
			IPv4Addr = get_unaligned((PUINT32) pData);
			pProfile->Profile[CurrentIdx].RegIpv4Addr =
			    be2cpu32(IPv4Addr);
			if (CurrentIdx == 0)
				pIWscInfo->RegIpv4Addr =
				    pProfile->Profile[CurrentIdx].RegIpv4Addr;
		...
		case WSC_ID_IPV4_SUBMASK:
			...
			pProfile->Profile[CurrentIdx].Ipv4SubMask =
			    be2cpu32(IPv4SubMask);
			if (pProfile->Profile[CurrentIdx].Ipv4SubMask !=
			    IWSC_DEFAULT_IPV4_SUBMASK)
				DBGPRINT(RT_DEBUG_TRACE,
		...
		case WSC_ID_ENROLLEE_IPV4:
			IPv4Addr = get_unaligned((PUINT32) pData);
			pProfile->Profile[CurrentIdx].EnrIpv4Addr =
			    be2cpu32(IPv4Addr);
			if (CurrentIdx == 0)
		...
		case WSC_ID_IPV4_SUBMASK_LIST:
			NdisZeroMemory(&pProfile->Profile[CurrentIdx].
				       AvaIpv4SubmaskList[0],
				       sizeof(pProfile->Profile[CurrentIdx].
					      AvaIpv4SubmaskList));
			if ((WscLen == 0) || (WscLen == 4)
			    || ((WscLen % 4) != 0)) {
				...
				for (i = 0; i < AvaSubMaskListCount; i++) {
					pProfile->Profile[CurrentIdx].
					    AvaIpv4SubmaskList[i] =
					    be2cpu32(get_unaligned
						     ((PUINT32)
						      (pData + i + 1)));
				}
				...
				if (CurrentIdx == 0) {
					pIWscInfo->CurrentIpRange =
					    be2cpu32(get_unaligned
						     ((PUINT32) pData));
					pIWscInfo->AvaSubMaskListCount =
					    (UINT8) AvaSubMaskListCount;
					NdisZeroMemory(pIWscInfo->
						       AvaSubMaskList,
						       sizeof(pIWscInfo->
							      AvaSubMaskList));
					NdisMoveMemory(&pIWscInfo->
						       AvaSubMaskList[0],
						       &pProfile->
						       Profile[CurrentIdx].
						       AvaIpv4SubmaskList[0],
				...
				pProfile->Profile[CurrentIdx].
				    AvaIpv4SubmaskList[AvaSubMaskListCount] =
				    pIWscInfo->CurrentIpRange;
			}
			break;
#endif				/* IWSC_SUPPORT */
		   ...
		   IN PWSC_CTRL pWscControl, OUT VOID * pbuf)
{
	...
	templen =
	    AppendWSCTLV(WSC_ID_UUID_E, pData, &pWscControl->Wsc_Uuid_E[0], 0);
	pData += templen;
	Len += templen;
	...
	templen =
	    AppendWSCTLV(WSC_ID_AUTH_TYPE_FLAGS, pData,
	...
	templen =
	    AppendWSCTLV(WSC_ID_ENCR_TYPE_FLAGS, pData,
	...
	templen =
	    AppendWSCTLV(WSC_ID_CONN_TYPE_FLAGS, pData,
	...
	if (pWscControl->WscV2Info.bEnableWpsV2) {
		...
		if (CurOpMode == AP_MODE) {
			ConfigMethods =
			    (pWscControl->WscConfigMethods & 0x200F);
		}
#ifdef IWSC_SUPPORT
	...
	templen =
	    AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData,
	...
	/*18. RF Band */
	templen =
	    AppendWSCTLV(WSC_ID_RF_BAND, pData,
	...
	templen =
	    AppendWSCTLV(WSC_ID_ASSOC_STATE, pData,
	...
	templen =
	    AppendWSCTLV(WSC_ID_DEVICE_PWD_ID, pData,
	...
	templen =
	    AppendWSCTLV(WSC_ID_OS_VERSION, pData,
	...
	if (pWscControl->WscV2Info.bEnableWpsV2) {
		...
		if (pWscTLV->pTlvData && pWscTLV->TlvLen) {
			templen =
			    AppendWSCTLV(pWscTLV->TlvTag, pData,
	...
	if (pWscControl->WscV2Info.bEnableWpsV2 && pWscTLV->pTlvData
		...
		templen =
		    AppendWSCTLV(pWscTLV->TlvTag, pData,
				 (UINT8 *) pWscTLV->pTlvData, pWscTLV->TlvLen);
		pData += templen;
		Len += templen;
	}
#endif				/* WSC_V2_SUPPORT */
		   ...
		   IN PWSC_CTRL pWscControl, OUT VOID * pbuf)
{
	...
	RT_DH_SecretKey_Generate(pReg->Pke, sizeof(pReg->Pke),
	...
	templen =
	    AppendWSCTLV(WSC_ID_ENROLLEE_NONCE, pData, pReg->EnrolleeNonce, 0);
	pData += templen;
	Len += templen;

	/* 4. Registrar Nonce, 16 bytes */
	templen =
	    AppendWSCTLV(WSC_ID_REGISTRAR_NONCE, pData, pReg->RegistrarNonce,
			 0);
	pData += templen;
	Len += templen;
	...
	if (pWscControl->WscV2Info.bEnableWpsV2) {
		if (pWscControl->WscMode == WSC_PIN_MODE) {
			...
			ConfigMethods =
			    (pWscControl->WscConfigMethods & 0x02F0);
		}
	} else {
		ConfigMethods = (pWscControl->WscConfigMethods & 0x00FF);
	}
	...
	/* Config Error */
	templen =
	    AppendWSCTLV(WSC_ID_CONFIG_ERROR, pData,
...
/*	pHash = kmalloc(512, MEM_ALLOC_FLAG); */
	...
	if (pReg->PinCodeLen == 4)
		RT_HMAC_SHA256(pReg->AuthKey, 32, pReg->PIN, 2, TB,
			       SHA256_DIGEST_SIZE);
	else
		RT_HMAC_SHA256(pReg->AuthKey, 32, pReg->PIN, 4, TB,
			       SHA256_DIGEST_SIZE);
	...
	/* Generate PSK2 */
	if (pReg->PinCodeLen == 4)
		RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[2], 2, TB,
			       SHA256_DIGEST_SIZE);
	else
		RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[4], 4, TB,
			       SHA256_DIGEST_SIZE);
...
/*	IV_EncrData = kmalloc(512, MEM_ALLOC_FLAG); */
	...
	if ((CurOpMode == AP_MODE) &&
		...
		PlainLen +=
		    AppendWSCTLV(WSC_ID_SSID, &Plain[PlainLen],
		...
		PlainLen +=
		    AppendWSCTLV(WSC_ID_ENCR_TYPE, &Plain[PlainLen],
				 (UINT8 *) & encyType, 0);
		PlainLen +=
		    AppendWSCTLV(WSC_ID_NW_KEY_INDEX, &Plain[PlainLen],
...
/*	IV_EncrData = kmalloc(512, MEM_ALLOC_FLAG); */
	...
	if (CurOpMode == AP_MODE) {
		WscCreateProfileFromCfg(pAdapter, REGISTRAR_ACTION | AP_MODE,
					pWscControl, &pWscControl->WscProfile);
		pCredential =
		    &pAdapter->ApCfg.MBSSID[apidx].WscControl.WscProfile.
		    Profile[0];
	}
#endif				/* CONFIG_AP_SUPPORT */
	...
	if (CurOpMode == STA_MODE) {
		if (pAdapter->StaCfg.WscControl.WscProfile.ProfileCnt == 0 ||
			...
			WscCreateProfileFromCfg(pAdapter, STA_MODE, pWscControl,
						&pWscControl->WscProfile);

		pCredential =
		    &pAdapter->StaCfg.WscControl.WscProfile.Profile[0];
		NdisMoveMemory(pCredential->MacAddr, pAdapter->MlmeAux.Bssid,
			       6);
	}
#endif				/* CONFIG_STA_SUPPORT */
	...
	CerLen +=
	    AppendWSCTLV(WSC_ID_SSID, &TB[CerLen], pCredential->SSID.Ssid,
			 pCredential->SSID.SsidLength);
	CerLen +=
	    AppendWSCTLV(WSC_ID_AUTH_TYPE, &TB[CerLen], (UINT8 *) & AuthType,
			 0);
	CerLen +=
	    AppendWSCTLV(WSC_ID_ENCR_TYPE, &TB[CerLen], (UINT8 *) & EncrType,
			 0);
	CerLen +=
	    AppendWSCTLV(WSC_ID_NW_KEY_INDEX, &TB[CerLen],
			 &pCredential->KeyIndex, 0);
	CerLen +=
	    AppendWSCTLV(WSC_ID_NW_KEY, &TB[CerLen], pCredential->Key,
			 pCredential->KeyLength);
	CerLen +=
	    AppendWSCTLV(WSC_ID_MAC_ADDR, &TB[CerLen], pCredential->MacAddr, 0);
	...
	if ((CurOpMode == STA_MODE) && (pAdapter->StaCfg.BssType == BSS_INFRA)) {
		...
		if (IWSC_IpContentForCredential(pAdapter)) {
			...
			if (pIpv4SubMaskList
				...
				PlainLen +=
				    AppendWSCTLV(WSC_ID_IPV4_SUBMASK_LIST,
						 &Plain[PlainLen],
						 (UCHAR *) pIpv4SubMaskList,
						 Ipv4SubMaskListSize);
			}
		     ...
		     IN VOID * precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	...
	RT_DH_PublicKey_Generate(WPS_DH_G_VALUE, sizeof(WPS_DH_G_VALUE),
				 WPS_DH_P_VALUE, sizeof(WPS_DH_P_VALUE),
				 pWscControl->RegData.EnrolleeRandom,
				 sizeof(pWscControl->RegData.EnrolleeRandom),
	...
	while (Length > 4) {
		...
		case WSC_ID_SC_STATE:
			...
			if (CurOpMode == STA_MODE) {
				...
				pWscControl->bConfiguredAP =
				    (pReg->PeerInfo.ScState ==
				     WSC_SCSTATE_CONFIGURED) ? TRUE : FALSE;
			}
#endif				/* CONFIG_STA_SUPPORT */
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SC_STATE))] ^=
			    (1 << WSC_TLV_BYTE1(WSC_ID_SC_STATE));
		...
		case WSC_ID_MANUFACTURER:
			NdisMoveMemory(&pReg->PeerInfo.Manufacturer, pData,
				       WscLen);
			NdisMoveMemory(&pWscControl->WscPeerInfo.
				       WscPeerManufacturer, pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MANUFACTURER))] ^=
			    (1 << WSC_TLV_BYTE1(WSC_ID_MANUFACTURER));
			break;
		...
		case WSC_ID_MODEL_NUMBER:
			NdisMoveMemory(&pReg->PeerInfo.ModelNumber, pData,
				       WscLen);
			NdisMoveMemory(&pWscControl->WscPeerInfo.
				       WscPeerModelNumber, pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MODEL_NUMBER))] ^=
			    (1 << WSC_TLV_BYTE1(WSC_ID_MODEL_NUMBER));
			break;
		...
		case WSC_ID_SERIAL_NUM:
			NdisMoveMemory(&pReg->PeerInfo.SerialNumber, pData,
				       WscLen);
			NdisMoveMemory(&pWscControl->WscPeerInfo.
				       WscPeerSerialNumber, pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SERIAL_NUM))] ^=
			    (1 << WSC_TLV_BYTE1(WSC_ID_SERIAL_NUM));
			break;
		...
		case WSC_ID_DEVICE_NAME:
			NdisMoveMemory(&pReg->PeerInfo.DeviceName, pData,
				       WscLen);
			NdisMoveMemory(pWscControl->WscPeerInfo.
				       WscPeerDeviceName, pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_DEVICE_NAME))] ^=
			    (1 << WSC_TLV_BYTE1(WSC_ID_DEVICE_NAME));
			break;
		...
		case WSC_ID_DEVICE_PWD_ID:
			...
			if (WSC_DEVICEPWDID_DEFAULT == get_unaligned((PUSHORT) pData)) {
/**(PUSHORT) pData) */
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Rx WPS           DPID PIN\n"));
				pWscControl->RegData.SelfInfo.DevPwdId =
				    cpu2be16(DEV_PASS_ID_PIN);
			} else if (WSC_DEVICEPWDID_PUSH_BTN == get_unaligned((PUSHORT) pData)) {
/**(PUSHORT) pData) */
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Rx WPS           DPID PBC\n"));
				pWscControl->RegData.SelfInfo.DevPwdId =
		     ...
		     IN INT Length, IN UCHAR apidx, OUT PWSC_REG_DATA pReg)
{
	...
	INT HmacLen;
	UCHAR Hmac[8] =
	    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, KDK[32];
	...
	DH_Len = sizeof(pReg->SecretKey);
	RT_DH_SecretKey_Generate(pReg->Pkr, sizeof(pReg->Pkr),
				 WPS_DH_P_VALUE, sizeof(WPS_DH_P_VALUE),
				 pReg->EnrolleeRandom,
				 sizeof(pReg->EnrolleeRandom), pReg->SecretKey,
				 (UINT *) & DH_Len);
	...
	while (Length > 4) {
		...
		case WSC_ID_ENROLLEE_NONCE:
			...
			if (RTMPCompareMemory
			    (pReg->EnrolleeNonce, pData, WscLen) != 0) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Rx M2 Compare enrollee nonce mismatched \n"));
			}
			break;
		     ...
		     IN VOID * precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_REGISTRAR_NONCE:
			...
			if (RTMPCompareMemory
			    (pReg->RegistrarNonce, pData, WscLen) != 0)
				DBGPRINT(RT_DEBUG_TRACE,
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter) {
				...
				RTMPZeroMemory(&pAdapter->StaCfg.WscControl.
					       WscM7Profile,
					       sizeof(WSC_PROFILE));
			}
#endif				/* CONFIG_STA_SUPPORT */
...
/*	IV_DecrData = kmalloc(1024, MEM_ALLOC_FLAG); */
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			/* Parse encryption settings */
			if (WscProcessCredential
			    (pAdapter, pReg->ApEncrSettings, EncrLen,
			     pWscControl) == FALSE) {
				if (NULL != IV_DecrData)
					os_free_mem(NULL, IV_DecrData);
