...
VOID P2pSetPerstTable(IN PRTMP_ADAPTER pAd, IN PVOID pInformationBuffer)
{
	...
	for (i = 0; i < 1; i++) {
		RTMPMoveMemory(&pAd->P2pTable.PerstEntry[i],
			       &pP2pPerstTab->PerstEntry[i],
			       sizeof(RT_P2P_PERSISTENT_ENTRY));
...
VOID P2pGroupMaintain(IN PRTMP_ADAPTER pAd)
{
	...
	if (pP2PCtrl->GONoASchedule.bValid == TRUE) {
		...
		if (Data != pP2PCtrl->GONoASchedule.TsfHighByte) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("P2pGroupMaintain. Tsf MSB changed to %ld from %ld.  restart NoA . \n",
	...
	if (pP2PCtrl->p2pidxForServiceCbReq < MAX_P2P_GROUP_SIZE) {
		if (pAd->P2pTable.Client[pAd->P2pCfg.p2pidxForServiceCbReq].
		    ConfigTimeOut > 0)
			pAd->P2pTable.Client[pAd->P2pCfg.p2pidxForServiceCbReq].
			    ConfigTimeOut--;
		if (pAd->P2pTable.Client[pAd->P2pCfg.p2pidxForServiceCbReq].
		    P2pClientState == P2PSTATE_SERVICE_COMEBACK_COMMAND
		    && (pAd->P2pTable.Client[pAd->P2pCfg.p2pidxForServiceCbReq].
			ConfigTimeOut == 0)) {
	...
	if (P2P_GO_ON(pAd)) {
		for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
			pP2pEntry = &pAd->P2pTable.Client[i];
			if (pP2pEntry->P2pClientState ==
			    P2PSTATE_WAIT_GO_DISCO_ACK_SUCCESS) {
				ULONG TotalFrameLen;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("P2P  P2PSTATE_WAIT_GO_DISCO_ACK_SUCCESS \n"));

				P2PSendDevDisRsp(pAd, P2PSTATUS_SUCCESS,
						 pAd->P2pCfg.
						 LatestP2pPublicFrame.Token,
						 pAd->P2pCfg.
						 LatestP2pPublicFrame.
						 p80211Header.Addr2,
						 &TotalFrameLen);
				pP2pEntry->P2pClientState =
				...
				if ((pP2pEntry->StateCount == 0)
					...
					if (pP2pEntry->ReTransmitCnt >= 20) {
						DBGPRINT(RT_DEBUG_ERROR,
							 ("%s:: ReTransmit Probe Req. limit! stop connect this p2p device!\n",
							  __FUNCTION__));
						P2pLinkDown(pAd,
							    P2P_DISCONNECTED);
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("P2P Table : idx=%d Send Probe Req. \n",
						  i));
					P2pSendProbeReq(pAd,
							pP2pEntry->
							ListenChannel);
	...
	/* time out case. */
		 ...
		 && (pP2PCtrl->P2pCounter.GoScanBeginCounter100ms >
		     1200 /*GOSCANBEGINCOUNTER_MAX */ )) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("P2P_DO_GO_SCAN_BEGIN Timeout. BAck to idle. \n"));
		...
		for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
			...
			if ((pP2pEntry->P2pClientState >=
				...
				if ((pP2pEntry->StateCount == 0)
					...
					if (pP2pEntry->ReTransmitCnt >= 20) {
						DBGPRINT(RT_DEBUG_ERROR,
							 ("%s:: [%s] ReTransmit Probe Req. limit! stop connect this p2p device!\n",
							  __FUNCTION__,
					...
					pP2pEntry->ReTransmitCnt++;
					DBGPRINT(RT_DEBUG_ERROR,
						 ("P2P Table : idx=%d Go Nego Req Retry. \n",
						  i));
					P2pSendProbeReq(pAd,
							pP2pEntry->
							ListenChannel);
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("P2P Table : idx=%d Get Confirm Ask Success.  p2pState = %d.\n",
				...
				P2pStartAutoGo(pAd);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("P2P Table : idx=%d Get Invite Rsp Ask Success.  p2pState = %d.\n",
					  i, pP2PCtrl->P2PConnectState));
				...
				&&
				(MAC_ADDR_EQUAL
				 (pP2PCtrl->ConnectingMAC, pP2pEntry->addr))) {
				if (pP2pEntry->StateCount > 0) {
					DBGPRINT(RT_DEBUG_INFO,
						 ("pEntry[%d] State = %s.  StateCount = %d\n",
						  i,
			   ...
			   IN UCHAR P2pindex, IN UCHAR Macindex)
{
	...
	RTMPMoveMemory(pP2pEntry->InterfaceAddr, pEntry->P2pInfo.InterfaceAddr,
		       MAC_ADDR_LEN);
	/* Save the bssid with interface address. */
	RTMPMoveMemory(pP2pEntry->bssid, pEntry->P2pInfo.InterfaceAddr,
		       MAC_ADDR_LEN);
	RTMPMoveMemory(pP2pEntry->PrimaryDevType,
		       pEntry->P2pInfo.PrimaryDevType, P2P_DEVICE_TYPE_LEN);
	RTMPMoveMemory(pP2pEntry->DeviceName, pEntry->P2pInfo.DeviceName, 32);
	RTMPMoveMemory(pP2pEntry->SecondaryDevType,
		       pEntry->P2pInfo.SecondaryDevType, P2P_DEVICE_TYPE_LEN);
			   ...
			   IN UCHAR P2pindex, IN UCHAR Macindex)
{
	...
	RTMPMoveMemory(pEntry->P2pInfo.InterfaceAddr, pP2pEntry->InterfaceAddr,
		       MAC_ADDR_LEN);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("MacTab InterfaceAddr = %x %x %x %x %x %x . \n",
		  PRINT_MAC(pP2pEntry->InterfaceAddr)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("MacTab DevAddr = %x %x %x  %x %x %x. \n",
		  PRINT_MAC(pP2pEntry->addr)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("MacTab  DeviceNameLen = %ld . \n",
		  pEntry->P2pInfo.DeviceNameLen));
	RTMPMoveMemory(pEntry->P2pInfo.PrimaryDevType,
		       pP2pEntry->PrimaryDevType, P2P_DEVICE_TYPE_LEN);
	RTMPMoveMemory(pEntry->P2pInfo.DeviceName, pP2pEntry->DeviceName, 32);
	RTMPMoveMemory(pEntry->P2pInfo.SecondaryDevType,
		       pP2pEntry->SecondaryDevType, P2P_DEVICE_TYPE_LEN);
...
VOID P2pGroupTabInit(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
		...
		RTMPZeroMemory(Tab->Client[i].PrimaryDevType,
			       P2P_DEVICE_TYPE_LEN);
		RTMPZeroMemory(Tab->Client[i].SecondaryDevType,
			       P2P_DEVICE_TYPE_LEN);
...
VOID P2pGroupTabDisconnect(IN PRTMP_ADAPTER pAd, IN BOOLEAN bSendDeAuth)
{
	...
	for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
		...
		if (pP2pEntry->P2pClientState >= P2PSTATE_CLIENT_OPERATING) {
			...
			RTMPZeroMemory(pP2pEntry->PrimaryDevType,
				       P2P_DEVICE_TYPE_LEN);
			RTMPZeroMemory(pP2pEntry->SecondaryDevType,
				       P2P_DEVICE_TYPE_LEN);
			...
			IN UCHAR SsidLen, IN UCHAR DevCap, IN UCHAR GrpCap)
{
	...
	if (NdisEqualMemory(ZeroSsid, Addr, 6)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("P2pGroupTabInsert . Addr all zero Error. \n"));
		RTMP_SEM_UNLOCK(&pAd->P2pTableSemLock);
		return P2P_NOT_FOUND;
	}
	if ((Addr[0] & 0x1) == 0x1) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("P2pGroupTabInsert . Insert mcast Addr Error. \n"));
		RTMP_SEM_UNLOCK(&pAd->P2pTableSemLock);
		return P2P_NOT_FOUND;
	}
	...
	for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
		    ...
		    (RTMPEqualMemory(Tab->Client[i].addr, Addr, MAC_ADDR_LEN)))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("    P2pGroupTabInsert[%d] . Arrd[%02x:%02x:%02x:%02x:%02x:%02x] Update State = %s \n",
				  i, Addr[0], Addr[1], Addr[2], Addr[3],
				  Addr[4], Addr[5],
				  decodeP2PClientState(State)));
			Tab->Client[i].P2pClientState = State;
			if ((SsidLen > 0) && (Ssid != NULL))
				RTMPMoveMemory(Tab->Client[i].Ssid, Ssid, 32);
			Tab->Client[i].SsidLen = SsidLen;
...
/*	
	...
	==========================================================================
 */
UCHAR P2pGroupTabDelete(IN PRTMP_ADAPTER pAd, IN UCHAR p2pindex, IN PUCHAR Addr)
{
	UCHAR index = 0xff;
	PRT_P2P_CLIENT_ENTRY pP2pEntry;
	...
	if (index < MAX_P2P_GROUP_SIZE) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("P2pGroupTabDelete  index %d.  search addr[3~5] is %x %x %x\n",
			...
			IN PUCHAR pAddr, IN PWSC_CREDENTIAL pProfile)
{
	...
	if (index == P2P_NOT_FOUND) {
		for (i = 0; i < MAX_P2P_TABLE_SIZE; i++) {
			if (Tab->PerstEntry[i].bValid == FALSE) {
				...
				if (P2P_GO_ON(pAd))
					...
					Tab->PerstEntry[i].MyRule =
					    P2P_IS_CLIENT;

				RTMPMoveMemory(Tab->PerstEntry[i].Addr, pAddr,
					       MAC_ADDR_LEN);
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Perst::Registra MacAddr = %x %x %x %x %x %x \n",
					  Tab->PerstEntry[i].Addr[0],
					  Tab->PerstEntry[i].Addr[1],
					  Tab->PerstEntry[i].Addr[2],
					  Tab->PerstEntry[i].Addr[3],
					  Tab->PerstEntry[i].Addr[4],
					  Tab->PerstEntry[i].Addr[5]));
				RTMPMoveMemory(&Tab->PerstEntry[i].Profile,
					       pProfile,
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Perst::MacAddr = %02x:%02x:%02x:%02x:%02x:%02x.\n",
					  PRINT_MAC(pPerstProfile->MacAddr)));
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Perst::AuthType = 0x%x. EncrType = %d\n",
					  pPerstProfile->AuthType,
					  pPerstProfile->EncrType));
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Perst::KeyIndex = %d\n",
				...
				for (j = 0; j < 16;) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("  0x%x 0x%x 0x%x 0x%x\n",
						  pPerstProfile->Key[j],
						  pPerstProfile->Key[j + 1],
						  pPerstProfile->Key[j + 2],
						  pPerstProfile->Key[j + 3]));
					j = j + 4;
				}
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Perst::<===Key =\n"));
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 (" P2P -P2pPerstTabInsert to index = %x. Rule = %s.\n",
					  i,
					  decodeMyRule(Tab->PerstEntry[i].
						       MyRule)));
				return i;
			}
		}
		...
		RTMPMoveMemory(&Tab->PerstEntry[i].Profile, pProfile,
			       sizeof(WSC_CREDENTIAL));
		pPerstProfile = &Tab->PerstEntry[i].Profile;
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Perst::SsidLen = %d\n",
		...
		DBGPRINT(RT_DEBUG_ERROR, ("Perst::<===Key =\n"));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Perst::MacAddr = %x %x %x %x %x %x\n",
			  pPerstProfile->MacAddr[0], pPerstProfile->MacAddr[1],
			  pPerstProfile->MacAddr[2], pPerstProfile->MacAddr[3],
			  pPerstProfile->MacAddr[4],
			  pPerstProfile->MacAddr[5]));
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P -P2pPerstTabInsert update to index = %x.\n",
			  i));
		return i;
	}
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P -P2pPerstTabInsert . PerstNumber = %d.\n",
		  Tab->PerstNumber));
	return index;
...
UCHAR P2pPerstTabDelete(IN PRTMP_ADAPTER pAd, IN UCHAR * pMacList)
{
	...
	for (i = 0; i < MAX_P2P_TABLE_SIZE; i++) {
		if (Tab->PerstEntry[i].bValid == TRUE) {
			if (NdisEqualMemory
			    (Tab->PerstEntry[i].Addr, pMacList, 6)) {
				RTMPZeroMemory(&Tab->PerstEntry[i],
					       sizeof(RT_P2P_PERSISTENT_ENTRY));
				...
				if (Tab->PerstNumber > 0) {
					...
					DBGPRINT(RT_DEBUG_ERROR,
						 (" P2P - Persistent table count error. \n"));

				DBGPRINT(RT_DEBUG_ERROR,
					 (" P2P - Delete a Persistent Entry .Table Number = %d. \n",
					  Tab->PerstNumber));
			}
			...
			IN PUCHAR Addr, IN PUCHAR Bssid, IN PUCHAR InfAddr)
{
	...
	for (i = 0; i < MAX_P2P_TABLE_SIZE; i++) {
		if ((Tab->PerstEntry[i].bValid == TRUE) &&
		    ((Addr
		      && NdisEqualMemory(&Tab->PerstEntry[i].Addr, Addr,
					 MAC_ADDR_LEN)) || (Bssid
							    &&
							    NdisEqualMemory
							    (&Tab->
							     ...
							     MAC_ADDR_LEN))
		     || (InfAddr
			 && NdisEqualMemory(&Tab->PerstEntry[i].Addr, InfAddr,
					    MAC_ADDR_LEN)))
		    ) {
		       ...
		       IN PUCHAR pAddr, IN WSC_CREDENTIAL * pProfile)
{
	...
	if (bExist == FALSE) {
		...
		for (i = 0; i < 2 /*MAX_P2P_SAVECREDN_SIZE */ ; i++) {
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - P2pCrednTabInsert NEW\n"));
			Tab->TempCredential[i].bValid = TRUE;
			RTMPMoveMemory(&Tab->TempCredential[i].Profile,
				       pProfile, sizeof(WSC_CREDENTIAL));
			RTMPMoveMemory(&Tab->TempCredential[i].InterAddr, pAddr,
				       MAC_ADDR_LEN);
		}
	} else if (index < 2 /*MAX_P2P_SAVECREDN_SIZE */ ) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - P2pCrednTabInsert Update Existing\n"));
		Tab->TempCredential[index].bValid = TRUE;
		RTMPMoveMemory(&Tab->TempCredential[index].Profile, pProfile,
			       sizeof(WSC_CREDENTIAL));
		RTMPMoveMemory(&Tab->TempCredential[index].InterAddr, pAddr,
			       MAC_ADDR_LEN);
	}
}
			    ...
			    IN PUCHAR Addr, IN PUCHAR ResultIndex)
{
	...
	for (i = 0; i < 2 /*MAX_P2P_SAVECREDN_SIZE */ ; i++) {
		if ((Tab->TempCredential[i].bValid == TRUE)
		    && NdisEqualMemory(&Tab->TempCredential[i].InterAddr, Addr,
				       MAC_ADDR_LEN)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 (" Find Credential Entry - i = %d. \n", i));
			bFind = TRUE;
			*ResultIndex = i;
		}
	}
			  ...
			  IN PRT_P2P_CLIENT_ENTRY pP2pEntry, IN PUCHAR Addr)
{
	...
	case RT_P2P_DEVICE_FIND:
		...
		RTMPMoveMemory(p2p_all_info.dev_addr, pP2pEntry->addr,
			       MAC_ADDR_LEN);
		RTMPMoveMemory(p2p_all_info.dev_name, pP2pEntry->DeviceName,
			       pP2pEntry->DeviceNameLen);
		RTMPMoveMemory(p2p_all_info.pri_dev_type,
			       pP2pEntry->PrimaryDevType, P2P_DEVICE_TYPE_LEN);
		...
		RTMPMoveMemory(p2p_all_info.peer, p2p_all_info.addr,
			       MAC_ADDR_LEN);
		...
		if (pP2pEntry->WfdEntryInfo.bWfdClient) {
			p2p_all_info.wfd_devive_type =
			    pP2pEntry->WfdEntryInfo.wfd_devive_type;
			p2p_all_info.source_coupled =
			    pP2pEntry->WfdEntryInfo.source_coupled;
			p2p_all_info.session_avail =
			    pP2pEntry->WfdEntryInfo.session_avail;
			p2p_all_info.sink_coupled =
			    pP2pEntry->WfdEntryInfo.sink_coupled;
			p2p_all_info.wfd_service_discovery =
			    pP2pEntry->WfdEntryInfo.wfd_service_discovery;
			p2p_all_info.wfd_PC = pP2pEntry->WfdEntryInfo.wfd_PC;
			p2p_all_info.wfd_CP = pP2pEntry->WfdEntryInfo.wfd_CP;
			p2p_all_info.wfd_time_sync =
			    pP2pEntry->WfdEntryInfo.wfd_time_sync;
			p2p_all_info.rtsp_port =
			    pP2pEntry->WfdEntryInfo.rtsp_port;
			p2p_all_info.max_throughput =
			    pP2pEntry->WfdEntryInfo.max_throughput;
			RTMPMoveMemory(p2p_all_info.assoc_addr,
				       pP2pEntry->WfdEntryInfo.assoc_addr,
				       MAC_ADDR_LEN);
			p2p_all_info.coupled_sink_status.CoupledStat =
			    pP2pEntry->WfdEntryInfo.coupled_sink_status.
			    CoupledStat;
		}
#endif				/* WFD_SUPPORT */
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_DEVICE_FIND, NULL, &p2p_all_info,
					sizeof(RT_P2P_ALL_INFO));
		break;
	...
	case RT_P2P_RECV_PROV_REQ:
		   ...
		   group_capab=0x0 */
		{
			...
			if (Addr)
				RTMPMoveMemory(p2p_all_info.addr, Addr,
					       MAC_ADDR_LEN);
			RTMPMoveMemory(p2p_all_info.dev_addr, pP2pEntry->addr,
				       MAC_ADDR_LEN);
			RTMPMoveMemory(p2p_all_info.dev_name,
			...
			if (pP2pEntry->WfdEntryInfo.bWfdClient) {
				...
				p2p_all_info.wfd_PC =
				    pP2pEntry->WfdEntryInfo.wfd_PC;
				p2p_all_info.wfd_CP =
				    pP2pEntry->WfdEntryInfo.wfd_CP;
				p2p_all_info.wfd_time_sync =
				    pP2pEntry->WfdEntryInfo.wfd_time_sync;
				p2p_all_info.rtsp_port =
				    pP2pEntry->WfdEntryInfo.rtsp_port;
			...
			RtmpOSWrielessEventSend(pAd->p2p_dev,
						RT_WLAN_EVENT_CUSTOM,
						RT_P2P_RECV_PROV_REQ, NULL,
						&p2p_all_info,
						sizeof(RT_P2P_ALL_INFO));
		}
		break;
	...
	case RT_P2P_RECV_PROV_RSP:
		RTMPMoveMemory(p2p_prov_disc_rsp.peer, pP2pEntry->addr,
			       MAC_ADDR_LEN);
		p2p_prov_disc_rsp.config_methods = pP2pEntry->ConfigMethod;
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_RECV_PROV_RSP, NULL,
					&p2p_prov_disc_rsp,
					sizeof(RT_P2P_PROV_DISC_RESP));
		break;
	...
	case RT_P2P_RECV_INVITE_REQ:
		{
			...
			if (Addr)
				RTMPMoveMemory(p2p_dev_info.addr, Addr,
					       MAC_ADDR_LEN);
			RTMPMoveMemory(p2p_dev_info.dev_addr, pP2pEntry->addr,
				       MAC_ADDR_LEN);
			RTMPMoveMemory(p2p_dev_info.dev_name,
				       pP2pEntry->DeviceName,
				       pP2pEntry->DeviceNameLen);
			RTMPMoveMemory(p2p_dev_info.pri_dev_type,
				       pP2pEntry->PrimaryDevType,
				       P2P_DEVICE_TYPE_LEN);
			p2p_prov_disc_rsp.config_methods =
			    pP2pEntry->ConfigMethod;
			RtmpOSWrielessEventSend(pAd->p2p_dev,
						RT_WLAN_EVENT_CUSTOM,
						RT_P2P_RECV_INVITE_REQ, NULL,
						&p2p_dev_info,
						sizeof(RT_P2P_DEV_FOUND));
		}
		break;
	...
	case RT_P2P_GO_NEG_COMPLETED:
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_GO_NEG_COMPLETED, NULL, NULL, 0);
		break;

	case RT_P2P_WPS_COMPLETED:
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_WPS_COMPLETED, NULL, NULL, 0);
		break;
	...
	case RT_P2P_CONNECTED:
		 ...
		 */
		{
			...
			if (P2P_GO_ON(pAd)) {
				...
				p2p_all_info.Rule = 1;
				RTMPMoveMemory(p2p_all_info.Bssid,
					       pAd->P2PCurrentAddress,
					       MAC_ADDR_LEN);
				RTMPMoveMemory(p2p_all_info.Ssid,
					       pAd->ApCfg.MBSSID[MAIN_MBSSID].
					       Ssid,
					       pAd->ApCfg.MBSSID[MAIN_MBSSID].
					       SsidLen);
				RTMPMoveMemory(p2p_all_info.peer,
					       pP2pEntry->addr, MAC_ADDR_LEN);
				//   RTMPMoveMemory(p2p_all_info.peer, Addr, MAC_ADDR_LEN);
				RTMPMoveMemory(p2p_all_info.WpaPsk,
					       pWScControl->WpaPsk, 64);
				p2p_all_info.P2PChannel =
				...
				if (pP2pEntry->WfdEntryInfo.bWfdClient) {
					...
					RTMPMoveMemory(p2p_all_info.assoc_addr,
						       ...
						       assoc_addr,
						       MAC_ADDR_LEN);
					p2p_all_info.coupled_sink_status.
					    CoupledStat =
					    pP2pEntry->WfdEntryInfo.
					    coupled_sink_status.CoupledStat;
				}
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s::RTSP Port: %d, device_type: %d\n",
					  __FUNCTION__, p2p_all_info.rtsp_port,
					  p2p_all_info.wfd_devive_type));
#endif				/* WFD_SUPPORT */
			} else {
				pWScControl =
				...
				p2p_all_info.Rule = 2;
				RTMPMoveMemory(p2p_all_info.Bssid,
					       pAd->P2pCfg.Bssid, MAC_ADDR_LEN);
				RTMPMoveMemory(p2p_all_info.Ssid,
					       pAd->P2pCfg.SSID,
					       pAd->P2pCfg.SSIDLen);
				RTMPMoveMemory(p2p_all_info.peer, Addr,
					       MAC_ADDR_LEN);
				RTMPMoveMemory(p2p_all_info.WpaPsk,
					       pWScControl->WpaPsk, 64);
				p2p_all_info.P2PChannel =
	...
	case RT_P2P_DISCONNECTED:
		...
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_DISCONNECTED, NULL, NULL, 0);
		break;

	case RT_P2P_CONNECT_FAIL:
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_CONNECT_FAIL, NULL, NULL, 0);
		break;

	case RT_P2P_LEGACY_CONNECTED:
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_LEGACY_CONNECTED, NULL, Addr,
					MAC_ADDR_LEN);
		break;

	case RT_P2P_LEGACY_DISCONNECTED:
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_LEGACY_DISCONNECTED, NULL, Addr,
					MAC_ADDR_LEN);
		break;
	...
	case RT_P2P_AP_STA_CONNECTED:
		 ...
		 */
		RTMPZeroMemory(&p2p_ap_sta_connected,
			       sizeof(RT_P2P_AP_STA_CONNECTED_REPORT));

		if (pP2pEntry != NULL) {
			RTMPMoveMemory(p2p_ap_sta_connected.dev_addr, Addr,
				       MAC_ADDR_LEN);
	...
	case RT_P2P_AP_STA_DISCONNECTED:
		...
		RtmpOSWrielessEventSend(pAd->p2p_dev, RT_WLAN_EVENT_CUSTOM,
					RT_P2P_AP_STA_DISCONNECTED, NULL,
					&p2p_ap_sta_connected,
					sizeof(RT_P2P_AP_STA_CONNECTED_REPORT));
		break;
	}
