		     ...
		     IN MAC_TABLE_ENTRY * pEntry, IN PFRAME_ADDBA_RSP pFrame)
{
	...
	if ((Idx != 0) && (pBAEntry->TID == TID)
		...
		BarHeaderInit(pAd, &FrameBar,
			      ...
			      pAd->MacTab.Content[pBAEntry->Wcid].HdrAddr2);
#else
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
#ifdef APCLI_SUPPORT
			if (IS_ENTRY_APCLI
			    (&pAd->MacTab.Content[pBAEntry->Wcid])) {
				apidx =
				    pAd->MacTab.Content[pBAEntry->Wcid].
				    MatchAPCLITabIdx;
				BarHeaderInit(pAd, &FrameBar,
					      pAd->MacTab.Content[pBAEntry->
								  Wcid].Addr,
			...
			{
				apidx =
				    pAd->MacTab.Content[pBAEntry->Wcid].apidx;
				BarHeaderInit(pAd, &FrameBar,
					      pAd->MacTab.Content[pBAEntry->
								  Wcid].Addr,
...
#ifdef CONFIG_STA_SUPPORT
#ifdef P2P_SUPPORT
		if ((pAd->OpMode == OPMODE_STA) && IS_ENTRY_CLIENT(pEntry)
...
VOID PeerAddBAReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pMacEntry) {
		...
		{
			ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
				      pMacEntry->HdrAddr2, pMacEntry->HdrAddr3);
		}
	}
#else
#ifdef CONFIG_AP_SUPPORT
...
*/
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			data_p =
			    OS_PKT_HEAD_BUF_EXTEND(pRxPkt,
						   LENGTH_802_3 + VLAN_Size);
