...
#define PWR_CONSTRAION_TAB_SIZE \
	...
	MaxTxPwr =
	    GetRegulatoryMaxTxPwr(pAd, pAd->CommonCfg.Channel) - (CHAR) Value;
...
static PDOT11_REGULATORY_INFORMATION GetRugClassRegion(IN PSTRING pCountryCode,
						       IN UINT8 RugClass)
{
			     ...
			     OUT PUCHAR pBuf, OUT PULONG pBufLen)
{
	...
	for (loop = 0; loop < MAX_NUM_OF_REGULATORY_CLASS; loop++) {
		...
		pRguClassRegion = GetRugClassRegion((PSTRING) pAd->CommonCfg.
						    CountryCode,
						    pAd->CommonCfg.
						    RegulatoryClass[loop]);
