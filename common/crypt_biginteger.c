...
VOID BigInteger_Print(IN PBIG_INTEGER pBI)
{
	...
	if (strlen(pBI->Name) != 0)
		DEBUGPRINT("Name=%s\n", pBI->Name);
	DEBUGPRINT
	    ("AllocSize=%d, ArrayLength=%d, IntegerLength=%d, Signed=%d\n",
...
VOID BigInteger_ClearHighBits(IN PBIG_INTEGER pBI)
{
	...
	if (BIArrayIndex >= 0) {
		...
		while (value == 0) {
			ShiftIndex--;
			value =
			    UINT32_GETBYTE(pBI->pIntegerArray[BIArrayIndex],
					   ShiftIndex);
		}		/* End of while */
	}
	/* End of if */
...
VOID BigInteger_BI2Bin(IN PBIG_INTEGER pBI,
		       OUT UINT8 * pValue, OUT UINT * Length)
{
...
VOID BigInteger_Bin2BI(IN UINT8 * pValue,
		       IN UINT Length, OUT PBIG_INTEGER * pBI)
{
	INT ValueIndex, BIArrayIndex, ShiftIndex;
	UINT32 Number;
		    ...
		    OUT PBIG_INTEGER * pBI_Result)
{
	...
	if ((CompareResult ==
	     0) & ((pFirstOperand->Signed * pSecondOperand->Signed) < 0)) {
	...
	if ((pFirstOperand->Signed * pSecondOperand->Signed) > 0) {
		...
		if ((pFirstOperand->Signed == 1) & (pSecondOperand->Signed ==
						    -1)) {
			...
			if (BIArrayIndex < pTempBI2->ArrayLength) {
				if ((*pBI_Result)->
					...
					(*pBI_Result)->
					    ...
					    pIntegerArray[BIArrayIndex] -
					    pTempBI2->
					    pIntegerArray[BIArrayIndex] - Carry;
					Carry = 0;
				} else {
					(*pBI_Result)->
					    pIntegerArray[BIArrayIndex] =
		    ...
		    OUT PBIG_INTEGER * pBI_Result)
{
	...
	for (BIFirstIndex = 0; BIFirstIndex < pFirstOperand->ArrayLength;
		...
		Carry = 0;
		FirstValue =
		    (UINT64) pFirstOperand->pIntegerArray[BIFirstIndex];
		if (FirstValue == 0) {
			continue;
		} else {
			for (BISecondIndex = 0;
				...
				SecondValue =
				    ((UINT64) pSecondOperand->
				     pIntegerArray[BISecondIndex]) * FirstValue;
				Sum =
				    (UINT64) ((*pBI_Result)->
			...
			while (Carry != 0) {
				Sum =
				    (UINT64) (*pBI_Result)->
				    pIntegerArray[BIFirstIndex + BISecondIndex];
				Sum += Carry;
...
VOID BigInteger_Square(IN PBIG_INTEGER pBI, OUT PBIG_INTEGER * pBI_Result)
{
	...
	for (BIFirstIndex = 1; BIFirstIndex < (pBI->ArrayLength - 1);
		...
		for (BISecondIndex = (BIFirstIndex + 1);
			...
			Result64_2 =
			    (UINT64) Point_Of_BI[BISecondIndex] * TempValue64;
			Carry64 = (Result64_1 >> 32);
		    ...
		    OUT PBIG_INTEGER * pBI_Remainder)
{
	...
	SecondHighByte =
	    BigInteger_GetByteValue(pSecondOperand,
				    pSecondOperand->IntegerLength);
	ComputeSize =
	    (INT) pFirstOperand->IntegerLength - pSecondOperand->IntegerLength +
	    1;
	for (Index = (INT) ComputeSize; Index >= 0; Index--) {
		if (BigInteger_UnsignedCompare(*pBI_Remainder, pSecondOperand)
		    == -1)
			break;
		...
		if (((pSecondOperand->IntegerLength + Index) -
			...
			for (MulIndex = (INT) MulStart; MulIndex <= 0x101; MulIndex++) {	/* 0xFFFF / 0xFF = 0x101 */
				...
				pMulBI->pIntegerArray[ArrayIndex] =
				    ((UINT) MulIndex << (8 * ShiftIndex));
				BigInteger_Mul(pSecondOperand, pMulBI,
					       &pTempBI);
				CompareResult =
				    BigInteger_UnsignedCompare(*pBI_Remainder,
							       pTempBI);
				if (CompareResult < 1) {
					if (MulIndex > 1) {
						if (CompareResult != 0) {
						...
						BigInteger_Mul(pSecondOperand,
							       pMulBI,
							       &pTempBI);
						BigInteger_Sub(*pBI_Remainder,
							       pTempBI,
							       &pTempBI2);
						BigInteger_Copy(pTempBI2,
								pBI_Remainder);
						BigInteger_Add(*pBI_Result,
							       pMulBI,
							       &pTempBI2);
						BigInteger_Copy(pTempBI2,
								pBI_Result);
...
VOID BigInteger_Montgomery_Reduction(IN PBIG_INTEGER pBI_A,
				     ...
				     OUT PBIG_INTEGER * pBI_Result)
{
	...
	BigInteger_AllocSize(pBI_Result,
			     pBI_A->IntegerLength + pBI_P->IntegerLength + 20);
	...
	for (FirstLoop = 0; FirstLoop < LoopCount; FirstLoop++) {
		...
		for (SecondLoop = 0; SecondLoop < pBI_P->ArrayLength;
		     SecondLoop++) {
			Result64_1 =
			    ((UINT64) Point_Result[SecondLoop]) + Carry64;
			Result64_2 = (UINT64) Point_P[SecondLoop] * TempValue64;
			Carry64 = (Result64_1 >> 32);
			Result64_1 = (Result64_1 & 0xffffffffUL);
			Result64_1 = Result64_1 + Result64_2;
			Carry64 += (Result64_1 >> 32);
			Point_Result[SecondLoop] =
			    (UINT32) (Result64_1 & 0xffffffffUL);
		}		/* End of for */
		while (Carry64 != 0) {
			Result64_1 =
	...
	for (FirstLoop = 0; FirstLoop <= LoopCount; FirstLoop++) {
		(*pBI_Result)->pIntegerArray[FirstLoop] =
		    (*pBI_Result)->pIntegerArray[FirstLoop + LoopCount];
	}			/* End of for */
...
VOID BigInteger_Montgomery_ExpMod(IN PBIG_INTEGER pBI_G,
				  ...
				  OUT PBIG_INTEGER * pBI_Result)
{
	...
	AllocLength =
	    (pBI_G->IntegerLength + pBI_E->IntegerLength +
	     pBI_P->IntegerLength + 300);
	...
	if (NdisCmpMemory
	    (pBI_P->pIntegerArray, pBI_Temp1->pIntegerArray,
...
/*        pRValue = (UINT8 *) kmalloc(sizeof(UINT8)*AllocLength, GFP_ATOMIC); */
	...
	for (Index = 2; Index < SLIDING_WINDOW; Index++) {
		...
		BigInteger_Montgomery_Reduction(pBI_Temp1, pBI_P, pBI_R,
						&pBI_A[Index]);
	}			/* End of for */
	...
	for (Index = pBI_E->IntegerLength; Index > 0; Index--) {
		...
		if (Sliding_HighValue != 0) {
			BigInteger_Mul(pBI_A[Sliding_HighValue], pBI_X,
				       &pBI_Temp1);
			BigInteger_Montgomery_Reduction(pBI_Temp1, pBI_P, pBI_R,
							&pBI_X);
		}
		...
		if (Sliding_LowValue != 0) {
			BigInteger_Mul(pBI_A[Sliding_LowValue], pBI_X,
				       &pBI_Temp1);
			BigInteger_Montgomery_Reduction(pBI_Temp1, pBI_P, pBI_R,
							&pBI_X);
		}		/* End of if */
