...
NDIS_STATUS dumpSesMacTb(IN MAT_STRUCT * pMatCfg, IN int hashIdx)
{
	...
	if (hashIdx < 0) {	/* dump all. */
		startIdx = 0;
		endIdx = MAT_MAX_HASH_ENTRY_SUPPORT - 1;
	} else {		/* dump specific hash index. */
...
static PUidMacMappingEntry UidMacTableUpdate(IN MAT_STRUCT * pMatCfg,
					     IN PUCHAR pInMac,
					     IN PUCHAR pOutMac,
					     IN PUCHAR pTagInfo,
					     IN UINT16 tagLen,
					     IN UINT16 isServer)
{
	...
	if (pTagInfo && tagLen > 0) {
		...
		uIDAddByUs = 0;
		tagLen =
		    (tagLen > PPPOE_DIS_UID_LEN ? PPPOE_DIS_UID_LEN : tagLen);
	} else {
		/*
	...
	if (hashIdx < MAT_MAX_HASH_ENTRY_SUPPORT) {
		...
		while (pEntry) {
			...
			if (NdisEqualMemory(pUIDStr, pEntry->uIDStr, tagLen)
				...
				return pEntry;
			} else {	/* handle the age-out situation */
				if (RTMP_TIME_AFTER
				    (now,
				     (pEntry->lastTime +
	...
	pNewEntry =
	    (UidMacMappingEntry *) MATDBEntryAlloc(pMatCfg,
						   sizeof(UidMacMappingEntry));
	if (pNewEntry) {
		...
		if (pUidMacTable->uidHash[hashIdx] == NULL) {	/* Hash list is empty, directly assign it. */
...
static PUidMacMappingEntry UidMacTableLookUp(IN MAT_STRUCT * pMatCfg,
					     IN PUCHAR pTagInfo,
					     IN UINT16 tagLen)
{
	UINT hashIdx;
		...
		if ((pEntry->sessionID == sesID)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s(): find it! dstMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				  __FUNCTION__, pEntry->inMacAddr[0],
...
static NDIS_STATUS SesMacTableUpdate(IN MAT_STRUCT * pMatCfg,
				     IN PUCHAR inMacAddr,
				     IN UINT16 sesID, IN PUCHAR outMacAddr)
{
	UINT16 hashIdx;
...
*/
	...
	pNewEntry =
	    (SesMacMappingEntry *) MATDBEntryAlloc(pMatCfg,
						   sizeof(SesMacMappingEntry));
	if (pNewEntry != NULL) {
		...
		if (pSesMacTable->sesHash[hashIdx] == NULL) {	/* Hash list is empty, directly assign it. */
 ...
 */
static PUCHAR MATProto_PPPoEDis_Rx(IN MAT_STRUCT * pMatCfg,
				   IN PNDIS_PACKET pSkb,
				   IN PUCHAR pLayerHdr, IN PUCHAR pDevMacAdr)
{
	PUCHAR pData, pSrvMac = NULL, pCliMac = NULL, pOutMac = NULL, pInMac =
	    NULL, pTagContent = NULL, pPayloadLen;
	...
	while (leftLen) {
		...
		if (tagID == findTag && tagLen > 0) {
			...
			tagLen =
			    tagLen >
			    PPPOE_DIS_UID_LEN ? PPPOE_DIS_UID_LEN : tagLen;
			break;
		} else {
			pData += (tagLen + 4);
			leftLen -= (tagLen + 4);
...
/*			tagLen = tagLen > PPPOE_DIS_UID_LEN ? PPPOE_DIS_UID_LEN : tagLen; */
	...
	/* Now update our pppoe discovery table "UidMacTable" */
	pEntry =
	    UidMacTableUpdate(pMatStruct, pSrcMac, pDstMac, pTagContent, tagLen,
			      isServer);
...
/*			pModSkb = (PNDIS_PACKET)skb_copy(RTPKT_TO_OSPKT(pSkb), MEM_ALLOC_FLAG); */
			pModSkb =
			    (PNDIS_PACKET) OS_PKT_COPY(RTPKT_TO_OSPKT(pSkb));
		} else
...
/*		tailHead = skb_put(RTPKT_TO_OSPKT(pModSkb), (PPPOE_DIS_UID_LEN + 4)); */
		tailHead =
		    OS_PKT_TAIL_BUF_EXTEND(pModSkb, (PPPOE_DIS_UID_LEN + 4));
		if (tailHead) {
			...
			if (pEntry->isServer) {	/*Append the AC-Cookie tag info in the tail of the pppoe packet. */
				tailHead[0] = 0x01;
				tailHead[1] = 0x04;
				tailHead[2] = 0x00;
				...
				NdisMoveMemory(tailHead, pEntry->uIDStr,
					       PPPOE_DIS_UID_LEN);
			} else {	/*Append the host-uniq tag info in the tail of the pppoe packet. */
				tailHead[0] = 0x01;
				tailHead[1] = 0x03;
				tailHead[2] = 0x00;
				tailHead[3] = PPPOE_DIS_UID_LEN;
...
/*		pMatCfg->MatTableSet.UidMacTable = (VOID *)kmalloc(sizeof(UidMacMappingTable), GFP_KERNEL); */
		os_alloc_mem_suspend(NULL,
				     (UCHAR **) & (pMatCfg->MatTableSet.
						   UidMacTable),
				     sizeof(UidMacMappingTable));
		if (pMatCfg->MatTableSet.UidMacTable) {
			pUidMacTable =
			    (UidMacMappingTable *) pMatCfg->MatTableSet.
			    UidMacTable;
...
/*		pMatCfg->MatTableSet.SesMacTable = (VOID *)kmalloc(sizeof(SesMacMappingTable), GFP_KERNEL); */
		...
		if (pMatCfg->MatTableSet.SesMacTable) {
			pSesMacTable =
			    (SesMacMappingTable *) pMatCfg->MatTableSet.
			    SesMacTable;
 ...
 */
static PUCHAR MATProto_PPPoESes_Rx(IN MAT_STRUCT * pMatCfg,
				   IN PNDIS_PACKET pSkb,
				   IN PUCHAR pLayerHdr, IN PUCHAR pDevMacAdr)
{
