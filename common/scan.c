...
static INT scan_ch_restore(RTMP_ADAPTER * pAd, UCHAR OpMode)
{
	...
	if (OpMode == OPMODE_STA) {
		...
		if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].Valid
			...
			OS_WAIT(20);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_NULL: PWR_ACTIVE SCAN_END\n"));
	...
	if (OpMode == OPMODE_AP) {
		...
		if (P2P_CLI_ON(pAd) && (ApScanRunning(pAd) == TRUE)) {
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("%s::  Scan Done! reset APCLI CTRL State Machine!\n",
				  __FUNCTION__));
...
static INT scan_active(RTMP_ADAPTER * pAd, UCHAR OpMode, UCHAR ScanType)
{
	...
	if ((pAd->MlmeAux.ScanType == SCAN_P2P)
...
#ifdef P2P_APCLI_SUPPORT
	    || ((pAd->MlmeAux.ScanType == SCAN_WSC_ACTIVE)
		&& (OpMode == OPMODE_AP) && (P2P_CLI_ON(pAd)))
#endif				/* P2P_APCLI_SUPPORT */
	    ) {
		...
		MgtMacHeaderInit(pAd, &Hdr80211, SUBTYPE_PROBE_REQ, 0,
				 BROADCAST_ADDR, pP2PCtrl->CurrentAddress,
				 BROADCAST_ADDR);
		...
		MakeOutgoingFrame(frm_buf, &FrameLen,
				  ...
				  1, &SupRateIe,
				  1, &SupRateLen,
				  SupRateLen, SupRate, END_OF_ARGS);
	} else
#endif				/* P2P_SUPPORT */
	{
#ifdef CONFIG_AP_SUPPORT
	...
	if (OpMode == OPMODE_STA) {
		...
		if (bHasWscIe) {
			...
			if (pWscBuf != NULL) {
				...
				WscBuildProbeReqIE(pAd, STA_MODE, pWscBuf,
						   &WscIeLen);
	...
	if (pAd->ApCfg.ApCliTab[MAIN_MBSSID].Valid
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CFG80211_NULL: PWR_SAVE SCANNING\n"));
			...
			if ((RTMP_CFG80211_VIF_P2P_GO_ON(pAd) ||
				...
				if (pAd->MlmeAux.Channel <=
					...
					if (pAd->pCfg80211ChanList[ChanId] ==
						...
						pAd->MlmeAux.Channel =
						    pAd->
						    pCfg80211ChanList[ChanId];
						DBGPRINT(RT_DEBUG_OFF,
							 ("ScanNextChannel():Scan Only Go or wfd Channel %d!\n",
							  pAd->MlmeAux.
							  Channel));
						break;
					}
					...
					if (pAd->MlmeAux.Channel != 0) {
						DBGPRINT(RT_DEBUG_TRACE,
							 ("ScanNextChannel():Scan Go or wfd Channel Done, clear the channel list. pAd->MlmeAux.Channel=%d\n",
							  pAd->MlmeAux.
							  Channel));
						os_free_mem(NULL,
							    pAd->
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("ScanNextChannel():Scan Go or wfd Channel Done. pAd->MlmeAux.Channel=%d\n",
						  pAd->MlmeAux.Channel));
					break;
				}
			} else if (pAd->pCfg80211ChanList[ChanId] >=
				   pAd->MlmeAux.Channel) {
				pAd->MlmeAux.Channel =
				    pAd->pCfg80211ChanList[ChanId];
				break;
			}
		}
...
#ifdef CONFIG_STA_SUPPORT
#ifdef P2P_SUPPORT
	rv = ((pAd->Mlme.ApSyncMachine.CurrState ==
