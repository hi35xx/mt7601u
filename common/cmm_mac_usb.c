...
VOID RTMPFreeTxRxRingMemory(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < RX_RING_SIZE; ++i) {
		...
		RTMPFreeUsbBulkBufStruct(pAd, &pRxContext->pUrb,
					 (PUCHAR *) & pRxContext->
					 TransferBuffer, MAX_RXBULK_SIZE,
					 pRxContext->data_dma);
	}
	...
	do {
		...
		for (acidx = 0; acidx < 4; acidx++) {
			...
			for (ringidx = 0; ringidx < BUF_ALIGMENT_RINGSIZE;
				...
				pTransferBuffer =
				    pHTTXContext->TransferBuffer[ringidx];
		...
		for (i = 0; i < MGMT_RING_SIZE; i++) {
			...
			if (pMLMEContext) {
				...
				if (NULL != pMLMEContext->TransferBuffer) {
					...
					RTUSB_URB_FREE_BUFFER(pObj->pUsb_Dev,
							      sizeof(TX_BUFFER),
							      pMLMEContext->
							      TransferBuffer,
							      pMLMEContext->
	...
	do {
		...
		for (acidx = 0; acidx < 4; acidx++) {
...
#ifdef USB_BULK_BUF_PREALLOC
				pHTTXContext->TransferBuffer[ringidx] =
				    RTMPQMemAddr(sizeof(HTTX_BUFFER), ringidx,
						 &pHTTXContext->
						 data_dma[ringidx],
						 acidx + BLK_TX0);
				if (pHTTXContext->TransferBuffer[ringidx] ==
				    NULL)
					goto err;
#endif
			...
			pHTTXContext->TransferBuffer =
			    RTMPQMemAddr(sizeof(HTTX_BUFFER),
			...
			printk
			    ("[%d] pHTTXContext TransferBuffer=0x%x,DMA=0x%x\n",
			     acidx,
			     (unsigned int)(pHTTXContext->TransferBuffer),
			     pHTTXContext->data_dma);
#endif
#endif				/* USB_BULK_BUF_ALIGMENT */
		}
		...
		pNullContext->TransferBuffer =
		    RTMPQMemAddr(sizeof(TX_BUFFER), 0, &pNullContext->data_dma,
				 BLK_NULL);
...
#ifdef OLD_PRE_ALLOC
		pNullContext->TransferBuffer =
		    RTMPQMemAddr(sizeof(TX_BUFFER), &pNullContext->data_dma,
				 BLK_NULL);
		if (pNullContext->TransferBuffer == NULL)
			goto err;
#endif				/* OLD_PRE_ALLOC */
		...
		pPsPollContext->TransferBuffer =
		    RTMPQMemAddr(sizeof(TX_BUFFER), 0,
				 &pPsPollContext->data_dma, BLK_PSPOLL);
...
#ifdef OLD_PRE_ALLOC
		pPsPollContext->TransferBuffer =
		    RTMPQMemAddr(sizeof(TX_BUFFER), &pPsPollContext->data_dma,
				 BLK_PSPOLL);
		if (pPsPollContext->TransferBuffer == NULL)
			goto err;
#endif				/* OLD_PRE_ALLOC */
		...
		for (i = 0; i < (RX_RING_SIZE); i++) {
			...
			pRxContext->TransferBuffer =
			    RTMPQMemAddr(MAX_RXBULK_SIZE, 0,
					 &pRxContext->data_dma, i + BLK_RX0);
...
#ifdef OLD_PRE_ALLOC
			pRxContext->TransferBuffer =
			    RTMPQMemAddr(MAX_RXBULK_SIZE, &pRxContext->data_dma,
					 i + BLK_RX0);
			if (pRxContext->TransferBuffer == NULL)
				goto err;

			printk("[%d] pRxContext TransferBuffer=0x%x,DMA=0x%x\n",
			       i, (unsigned int)(pRxContext->TransferBuffer),
			       pRxContext->data_dma);
#endif
		}
		...
		pCmdRspEventContext->CmdRspBuffer =
		    RTMPQMemAddr(CMD_RSP_BULK_SIZE, 0,
				 &pCmdRspEventContext->data_dma, BLK_CMD);
...
#ifdef OLD_PRE_ALLOC
		pCmdRspEventContext->CmdRspBuffer =
		    RTMPQMemAddr(CMD_RSP_BULK_SIZE,
				 &pCmdRspEventContext->data_dma, BLK_CMD);
		if (pCmdRspEventContext->CmdRspBuffer == NULL)
			goto err;

		printk("pCmdRspEventContext TransferBuffer=0x%x,DMA=0x%x\n",
		       (unsigned int)(pCmdRspEventContext->CmdRspBuffer),
		       pCmdRspEventContext->data_dma);
#endif
...
//                                      printk("iversontest2  RTMPAllocUsbBulkBufStruct fail   !!!!!!!!!!!\n");
 ...
 err:
	...
	if (NULL != pPsPollContext->TransferBuffer) {
		POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
		RTUSB_URB_FREE_BUFFER(pObj->pUsb_Dev, sizeof(TX_BUFFER),
				      pPsPollContext->TransferBuffer,
				      pPsPollContext->data_dma);
...
VOID RTMPFreeTxRxRingMemory(IN PRTMP_ADAPTER pAd)
{
	...
	if (pCmdRspEventContext) {
		RTMPFreeUsbBulkBufStruct(pAd,
					 &pCmdRspEventContext->pUrb,
					 (PUCHAR *) & pCmdRspEventContext->
					 TransferBuffer, CMD_RSP_BULK_SIZE,
					 pCmdRspEventContext->data_dma);
	}
...
/*	USHORT			shortValue;*/
	...
	if ((apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_MBSSID_NUM(pAd))) {
		...
		bBcnReq =
		    BeaconTransmitRequired(pAd, apidx,
