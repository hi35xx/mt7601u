				   ...
				   IN UCHAR fragNum, OUT USHORT * freeCnt)
{
	...
	rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT) (USBDMApktLen), FALSE,
			     FIFO_EDCA, FALSE /*NextValid */ , FALSE, 1);
	...
	if (fragNum == pTxBlk->TotalFragNum) {
		...
		if (((pHTTXContext->CurWritePosition + 3906) & BULKOUT_SIZE) ==
		    BULKOUT_SIZE) {
	...
	if (fragNum == pTxBlk->TotalFragNum) {
		...
		if (bLasAlignmentsectiontRound == TRUE) {
			CUR_WRITE_IDX_INC(pHTTXContext->CurWriteIdx,
					  BUF_ALIGMENT_RINGSIZE);
				     ...
				     IN BOOLEAN bIsLast, OUT USHORT * freeCnt)
{
	...
	if (Status == NDIS_STATUS_SUCCESS) {
...
#ifdef HDR_TRANS_SUPPORT
		rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT) (dma_len), FALSE,
		 ...
		 */
		if ((pHTTXContext->CurWritePosition & BULKOUT_SIZE) ==
		    BULKOUT_SIZE) {
			CUR_WRITE_IDX_INC(pHTTXContext->CurWriteIdx,
					  BUF_ALIGMENT_RINGSIZE);
			pHTTXContext->CurWritePosition = 0;
				    ...
				    IN UCHAR frmNum, OUT USHORT * freeCnt)
{
	...
	if (frmNum == 0) {
		...
		if (Status == NDIS_STATUS_SUCCESS) {
			...
			rlt_usb_write_txinfo(pAd, pTxInfo,
					     (USHORT) (pTxBlk->Priv), FALSE,
					     FIFO_EDCA, FALSE /*NextValid */ ,
					     FALSE, 1);
			     ...
			     IN UCHAR * pNullFrame, IN UINT32 frameLen)
{
	if (pAd->NullContext.InUse == FALSE) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s - Send NULL Frame Fail Due to In Null Context using...\n",
			  __FUNCTION__));
	}
}
...
#ifdef RLT_MAC
	{
		NdisMoveMemory((VOID *) & pRxBlk->hw_rx_info[0],
			       (VOID *) pRxFceInfo, sizeof(RXFCE_INFO));
