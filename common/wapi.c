...
/* IV default value */
UCHAR AE_BCAST_PN[LEN_WAPI_TSC] =
    { 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c,
	0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c
};

UCHAR ASUE_UCAST_PN[LEN_WAPI_TSC] =
    { 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c,
	0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c
};

UCHAR AE_UCAST_PN[LEN_WAPI_TSC] =
...
static void kd_hmac_sha256(unsigned char *key,
			   ...
			   unsigned char *output, unsigned int output_len)
{
	...
	for (i = 0; output_len / SHA256_DIGEST_SIZE;
	     i++, output_len -= SHA256_DIGEST_SIZE) {
		RT_HMAC_SHA256(key, key_len, text, text_len,
	...
	if (output_len > 0)
		RT_HMAC_SHA256(key, key_len, text, text_len,
			       &output[i * SHA256_DIGEST_SIZE], output_len);
...
{
	NDIS_802_11_ENCRYPTION_STATUS cipher_mode =
...
VOID RTMPDeriveWapiGTK(IN PUCHAR nmk, OUT PUCHAR gtk_ptr)
{
	...
	kd_hmac_sha256(nmk,
		       16,
		       (UCHAR *) group_context,
		       strlen(group_context), gtk_ptr, 32);
}
...
VOID RT_SMS4_TEST(IN UINT8 test)
{
	...
	RTMPSoftEncryptSMS4(mac_hdr_qos, payload_qos, 1, 1, key, pn);
	...
	if (RTMPSoftDecryptSMS4(mac_hdr_qos,
				FALSE, &CipherKey, rcvd_data, &data_len) == 0)
				    ...
				    PSTRING tmpbuf, PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter("Wapiifname", tmpbuf, 32, buffer, TRUE)) {
		if (strlen(tmpbuf) > 0) {
			NdisMoveMemory(pInfo->wapi_ifname, tmpbuf,
				       strlen(tmpbuf));
	...
	if (RTMPGetKeyParameter("WapiAsCertPath", tmpbuf, 128, buffer, TRUE)) {
		if (strlen(tmpbuf) > 0) {
			NdisMoveMemory(pInfo->as_cert_path[0], tmpbuf,
				       strlen(tmpbuf));
	...
	for (idx = 1; idx < MAX_ID_NO; idx++) {
		...
		if (RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer, TRUE)) {
			if (strlen(tmpbuf) > 0) {
				NdisMoveMemory(pInfo->as_cert_path[idx], tmpbuf,
					       strlen(tmpbuf));
				pInfo->as_cert_path_len[idx] = strlen(tmpbuf);
				pInfo->as_cert_no++;

				DBGPRINT(RT_DEBUG_TRACE,
					 ("WapiAsCertPath%d=%s, len=%d\n",
					  idx + 1, pInfo->as_cert_path[idx],
	...
	if (RTMPGetKeyParameter("WapiCaCertPath", tmpbuf, 128, buffer, TRUE)) {
		if (strlen(tmpbuf) > 0) {
			NdisMoveMemory(pInfo->ca_cert_path, tmpbuf,
				       strlen(tmpbuf));
	...
	if (RTMPGetKeyParameter("WapiUserCertPath", tmpbuf, 128, buffer, TRUE)) {
		if (strlen(tmpbuf) > 0) {
			NdisMoveMemory(pInfo->user_cert_path, tmpbuf,
				       strlen(tmpbuf));
			pInfo->user_cert_path_len = strlen(tmpbuf);

			DBGPRINT(RT_DEBUG_TRACE,
				 ("WapiUserCertPath=%s, len=%d\n",
	...
	if (RTMPGetKeyParameter("WapiMskRekeyMethod", tmpbuf, 32, buffer, TRUE)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("WapiMskRekeyMethod=%d\n",
			  pAd->CommonCfg.wapi_msk_rekey_method));
	}

	/* WapiMskRekeyThreshold */
	if (RTMPGetKeyParameter
	    ("WapiMskRekeyThreshold", tmpbuf, 32, buffer, TRUE)) {
		pAd->CommonCfg.wapi_msk_rekey_threshold =
		    simple_strtol(tmpbuf, 0, 10);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("WapiMskRekeyThreshold=%d\n",
			  pAd->CommonCfg.wapi_msk_rekey_threshold));
	}
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			snprintf(tok_str, sizeof(tok_str), "WapiPsk%d",
				 apidx + 1);

			NdisZeroMemory(pAd->ApCfg.MBSSID[apidx].WAPIPassPhrase,
				       64);
			pAd->ApCfg.MBSSID[apidx].WAPIPassPhraseLen = 0;
			if (RTMPGetKeyParameter
			    (tok_str, tmpbuf, 65, buffer, FALSE)) {
				if (strlen(tmpbuf) >= 8 && strlen(tmpbuf) <= 64) {
					NdisMoveMemory(pAd->ApCfg.MBSSID[apidx].
						       WAPIPassPhrase, tmpbuf,
						       strlen(tmpbuf));
					pAd->ApCfg.MBSSID[apidx].
					    WAPIPassPhraseLen = strlen(tmpbuf);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("IF(ra%d) WapiPsk=(%s), len=%d\n",
						  apidx, tmpbuf,
						  strlen(tmpbuf)));
				} else {
					...
					DBGPRINT(RT_DEBUG_ERROR,
						 ("IF(ra%d) The length of WAPI PSKPassPhrase is invalid(len=%d). \n",
						  apidx, strlen(tmpbuf)));
				}
			}
		}
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (RTMPGetKeyParameter("WapiPsk", tmpbuf, 512, buffer, FALSE)) {
			...
			if (strlen(tmpbuf) >= 8 && strlen(tmpbuf) <= 64) {
				NdisMoveMemory(pAd->StaCfg.WAPIPassPhrase,
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("The length of WAPI PSKPassPhrase is invalid(len=%d). \n",
					  strlen(tmpbuf)));
			}
		}
	}
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			for (apidx = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
				...
				if (simple_strtol(macptr, 0, 10) == 0) {
					pAd->ApCfg.MBSSID[apidx].WapiPskType =
					    HEX_MODE;

					if (pAd->ApCfg.MBSSID[apidx].
					    WAPIPassPhraseLen % 2 != 0) {
						err = 1;
						DBGPRINT(RT_DEBUG_ERROR,
							 ("I/F(ra%d) The WAPI-PSK key length MUST be even in Hex mode\n",
							  apidx));
					}
				}
				/* ASCII */
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (simple_strtol(tmpbuf, 0, 10) == 0) {
				...
				if (pAd->StaCfg.WAPIPassPhraseLen % 2 != 0) {
					err = 1;
					DBGPRINT(RT_DEBUG_ERROR,
						 ("The WAPI-PSK key length MUST be even in Hex mode\n"));
				}
			}
			/* ASCII */
			else {
...
/* 
	...
	==========================================================================
*/
static VOID RTMPQueryWapiConfPerBss(IN PRTMP_ADAPTER pAd,
				    IN PWAPI_CONF wapi_conf_ptr, IN UCHAR apidx)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (pMbss->MSSIDDev != NULL) {
			...
			NdisMoveMemory(pConf->ifname, RtmpOsGetNetDevName(dev),
				       strlen(RtmpOsGetNetDevName(dev)));
			pConf->ifname_len = strlen(RtmpOsGetNetDevName(dev));
		} else {
			STRING slot_name[IFNAMSIZ];

			snprintf(slot_name, sizeof(slot_name), "ra%d", apidx);
			NdisMoveMemory(pConf->ifname, slot_name,
				       strlen(slot_name));
		...
		if (pMbss->WAPIPassPhraseLen > 0) {
			if (pMbss->WapiPskType == HEX_MODE) {
				...
				AtoH((PSTRING) pMbss->WAPIPassPhrase,
				     (PUCHAR) pConf->psk, pConf->psk_len);
			} else {
				pConf->psk_len = pMbss->WAPIPassPhraseLen;
				NdisMoveMemory(pConf->psk,
					       pMbss->WAPIPassPhrase,
					       pConf->psk_len);
			}
		}
		...
		if (pMbss->RSNIE_Len[0] > 0) {
			...
			NdisMoveMemory(&pConf->wie[2], pMbss->RSN_IE[0],
				       pMbss->RSNIE_Len[0]);
		}
	}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		NdisMoveMemory(pConf->ifname, pAd->StaCfg.dev_name,
			       strlen((PSTRING) pAd->StaCfg.dev_name));
		...
		if (pAd->StaCfg.WAPIPassPhraseLen > 0) {
			if (pAd->StaCfg.WapiPskType == HEX_MODE) {
				pConf->psk_len =
				    pAd->StaCfg.WAPIPassPhraseLen / 2;
				AtoH((PSTRING) pAd->StaCfg.WAPIPassPhrase,
				     (PUCHAR) pConf->psk, pConf->psk_len);
			} else {
				pConf->psk_len = pAd->StaCfg.WAPIPassPhraseLen;
				NdisMoveMemory(pConf->psk,
					       pAd->StaCfg.WAPIPassPhrase,
					       pConf->psk_len);
			}
		}
		...
		if (pAd->StaCfg.RSNIE_Len > 0) {
			...
			NdisMoveMemory(&pConf->wie[2], pAd->StaCfg.RSN_IE,
			    ...
			    IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	NdisMoveMemory(&pConf->comm_wapi_info, &pAd->CommonCfg.comm_wapi_info,
		       sizeof(COMMON_WAPI_INFO));
	...
	if (tmp_cnt > pAd->CommonCfg.wapi_msk_rekey_threshold) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
				...
				pAd->ApCfg.MBSSID[apidx].DefaultKeyId =
				    pAd->ApCfg.MBSSID[apidx].DefaultKeyId ==
				    0 ? 1 : 0;
				inc_iv_byte(pAd->ApCfg.MBSSID[apidx].
					    key_announce_flag, LEN_WAPI_TSC, 1);

				/* Generate NMK randomly */
				for (cnt = 0; cnt < 16; cnt++)
					pAd->ApCfg.MBSSID[apidx].NMK[cnt] =
					    RandomByte(pAd);

				RTMPDeriveWapiGTK(pAd->ApCfg.MBSSID[apidx].NMK,
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			...
			if (IS_ENTRY_CLIENT(pEntry) &&
				...
				WAPI_InternalCmdAction(pAd, pEntry->AuthMode,
						       pEntry->apidx,
						       pEntry->Addr,
						       WAI_MLME_UPDATE_MSK);
			}
				  ...
				  IN PMAC_TABLE_ENTRY pEntry)
{
	if (pEntry) {
		DBGPRINT(RT_DEBUG_TRACE,
			 (" RTMPInitWapiRekeyTimerAction : WAPI USK rekey timer (wcid-%d) \n",
			  pEntry->Aid));
		RTMPInitTimer(pAd, &pEntry->WapiUskRekeyTimer,
			      GET_TIMER_FUNCTION(RTMPWapiUskRekeyPeriodicExec),
			      pEntry, TRUE);
		pEntry->WapiUskRekeyTimerRunning = FALSE;
	} else {
		RTMPInitTimer(pAd, &pAd->CommonCfg.WapiMskRekeyTimer,
			      GET_TIMER_FUNCTION(RTMPWapiMskRekeyPeriodicExec),
			      pAd, TRUE);
...
{
	if (pEntry) {
		if ((pAd->CommonCfg.wapi_usk_rekey_method !=
		     REKEY_METHOD_DISABLE)
		    && (pAd->CommonCfg.wapi_usk_rekey_threshold > 0)) {
			/* Regularly check the timer */
			if (pEntry->WapiUskRekeyTimerRunning == FALSE) {
				RTMPSetTimer(&pEntry->WapiUskRekeyTimer,
					     WAPI_KEY_UPDATE_EXEC_INTV);

				pEntry->WapiUskRekeyTimerRunning = TRUE;
				pEntry->wapi_usk_rekey_cnt = 0;
				DBGPRINT(RT_DEBUG_TRACE,
					 (" RTMPStartWapiRekeyTimerAction : WAPI USK rekey timer is started (%d) \n",
					  pAd->CommonCfg.
					  wapi_usk_rekey_threshold));
			}
		}
	} else {
		if ((pAd->CommonCfg.wapi_msk_rekey_method !=
		     REKEY_METHOD_DISABLE)
		    && (pAd->CommonCfg.wapi_msk_rekey_threshold > 0)) {
			/* Regularly check the timer */
			if (pAd->CommonCfg.WapiMskRekeyTimerRunning == FALSE) {
				RTMPSetTimer(&pAd->CommonCfg.WapiMskRekeyTimer,
					     WAPI_KEY_UPDATE_EXEC_INTV);

				pAd->CommonCfg.WapiMskRekeyTimerRunning = TRUE;
				pAd->CommonCfg.wapi_msk_rekey_cnt = 0;
				DBGPRINT(RT_DEBUG_TRACE,
					 (" RTMPStartWapiRekeyTimerAction : WAPI MSK rekey timer is started \n"));
			}
		}
	}
				    ...
				    IN PMAC_TABLE_ENTRY pEntry)
{
	if (pEntry) {
		...
		if (pAd->CommonCfg.WapiMskRekeyTimerRunning == TRUE) {
			...
			RTMPCancelTimer(&pAd->CommonCfg.WapiMskRekeyTimer,
					&Cancelled);
			pAd->CommonCfg.wapi_msk_rekey_cnt = 0;
			pAd->CommonCfg.WapiMskRekeyTimerRunning = FALSE;
		}
	...
	if ((AuthMode == Ndis802_11AuthModeWAICERT) ||
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			MAKE_802_3_HEADER(FrameBuf,
					  pAd->ApCfg.MBSSID[apidx].Bssid, pAddr,
					  WAPI_IE);
		}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			MAKE_802_3_HEADER(FrameBuf, pAd->CurrentAddress, pAddr,
					  WAPI_IE);
		}
#endif				/* CONFIG_STA_SUPPORT */
		offset += LENGTH_802_3;
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Trigger WAPI for this sta(%02x:%02x:%02x:%02x:%02x:%02x)\n",
			  PRINT_MAC(pAddr)));
			      ...
			      IN UINT Wcid, OUT UCHAR * tx_tsc)
{
	...
	if (IS_HW_WAPI_SUPPORT(pAd)) {
		...
		for (i = 0; i < WAPI_PN_ENTRY_SIZE; i++)
			RTMP_IO_READ8(pAd, offset + i,
				      &tx_tsc[HW_IVEIV_ENTRY_SIZE + i]);

		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s : WCID(%d) ", __FUNCTION__, Wcid));
			    ...
			    PMAC_TABLE_ENTRY pEntry, BOOLEAN bAE)
{
	...
	if (IS_HW_WAPI_SUPPORT(pAd)) {
		...
		if (bAE)
			AsicUpdateWCIDIVEIV(pAd, pEntry->Aid, CONST_WAPI_PN + 1,
					    CONST_WAPI_PN);
		else
			AsicUpdateWCIDIVEIV(pAd, pEntry->Aid, CONST_WAPI_PN,
					    CONST_WAPI_PN);
		AsicUpdateWAPIPN(pAd, pEntry->Aid, CONST_WAPI_PN,
				 CONST_WAPI_PN);
			  ...
			  UINT8 BssIdx, UINT8 KeyIdx, UINT8 Wcid, PUINT8 pGtk)
{
	...
	if (IS_HW_WAPI_SUPPORT(pAd)) {
		...
		if (Wcid != 0) {
			...
			AsicUpdateWCIDIVEIV(pAd, Wcid, CONST_WAPI_PN,
					    CONST_WAPI_PN);
			AsicUpdateWAPIPN(pAd, Wcid, CONST_WAPI_PN,
					 CONST_WAPI_PN);
