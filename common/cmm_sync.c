...
UCHAR NextChannel(IN PRTMP_ADAPTER pAd, IN UCHAR channel)
{
	...
	if (pAd->MlmeAux.ScanType == SCAN_P2P_SEARCH) {
		...
		for (i = 0;
		     i < (pAd->P2pCfg.P2pProprietary.ListenChanelCount - 1);
		     i++) {
			if (CurrentChannel ==
			    pAd->P2pCfg.P2pProprietary.ListenChanel[i])
				next_channel =
				    pAd->P2pCfg.P2pProprietary.ListenChanel[i +
									    1];
		}
		P2P_INC_CHA_INDEX(pAd->P2pCfg.P2pProprietary.ListenChanelIndex,
				  pAd->P2pCfg.P2pProprietary.ListenChanelCount);
		if (next_channel == CurrentChannel) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("SYNC -  next_channel equals to CurrentChannel= %d\n",
				  next_channel));
			DBGPRINT(RT_DEBUG_INFO,
				 ("SYNC -  ListenChannel List : %d  %d  %d\n",
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("SYNC - P2P Scan return channel = %d.    Listen Channel = %d.\n",
...
VOID Handle_BSS_Width_Trigger_Events(IN PRTMP_ADAPTER pAd)
{
	ULONG Now32;
