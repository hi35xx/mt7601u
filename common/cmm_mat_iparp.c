...
static NDIS_STATUS IPMacTableUpdate(IN MAT_STRUCT * pMatCfg,
				    IN PUCHAR pMacAddr, IN UINT ipAddr)
{
	UINT hashIdx;
	IPMacMappingTable *pIPMacTable;
	...
	pNewEntry =
	    (IPMacMappingEntry *) MATDBEntryAlloc(pMatCfg,
						  sizeof(IPMacMappingEntry));
	if (pNewEntry != NULL) {
		...
		if (pIPMacTable->hash[hashIdx] == NULL) {	/* Hash list is empty, directly assign it. */
...
static PUCHAR MATProto_ARP_Rx(IN MAT_STRUCT * pMatCfg,
...
/*dumpPkt(RTPKT_TO_OSPKT(pSkb)->data, RTPKT_TO_OSPKT(pSkb)->len); */
	...
	isUcastMac =
	    ((tgtMac[0] | tgtMac[1] | tgtMac[2] | tgtMac[3] | tgtMac[4] |
	      tgtMac[5]) != 0);
...
static PUCHAR MATProto_ARP_Tx(IN MAT_STRUCT * pMatCfg,
			      IN PNDIS_PACKET pSkb,
			      IN PUCHAR pLayerHdr, IN PUCHAR pMacAddr)
{
	PUCHAR pSMac, pSIP;
		...
		if (OS_PKT_CLONED(pSkb)) {
			...
			if (newSkb) {
				if (IS_VLAN_PACKET(GET_OS_PKT_DATAPTR(newSkb)))
					pSMac =
					    (PUCHAR) (GET_OS_PKT_DATAPTR(newSkb)
...
static PUCHAR MATProto_IP_Rx(IN MAT_STRUCT * pMatCfg,
			     IN PNDIS_PACKET pSkb,
...
static PUCHAR MATProto_IP_Tx(IN MAT_STRUCT * pMatCfg,
			     IN PNDIS_PACKET pSkb,
			     IN PUCHAR pLayerHdr, IN PUCHAR pDevMacAdr)
{
	PUCHAR pSrcMac;
	...
	if (*(pLayerHdr + 9) == 0x11) {
		...
		if (srcPort == 68 && dstPort == 67) {	/*It's a DHCP packet */
			...
			bootpHdr = udpHdr + 8;
			bootpFlag =
			    OS_NTOHS(get_unaligned((PUINT16) (bootpHdr + 10)));
			DBGPRINT(RT_DEBUG_TRACE,
