...
/*	EDCA_PARM DefaultEdcaParm;*/
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
...
#ifdef WSC_STA_SUPPORT
		{
			...
			NdisZeroMemory(&pAd->StaCfg.WscControl.RegData,
				       sizeof(WSC_REG_DATA));
			...
			NdisAllocateSpinLock(pAd,
					     &pWscControl->WscPeerListSemLock);
...
#endif				/* P2P_APCLI_SUPPORT */
	{
		...
		for (j = 0; j < MAX_APCLI_NUM; j++) {
			pAd->ApCfg.ApCliTab[j].AuthMode =
			    Ndis802_11AuthModeOpen;
			pAd->ApCfg.ApCliTab[j].WepStatus =
			    Ndis802_11WEPDisabled;
			pAd->ApCfg.ApCliTab[j].bAutoTxRateSwitch = TRUE;
			pAd->ApCfg.ApCliTab[j].DesiredTransmitSetting.field.
			    MCS = MCS_AUTO;
			...
			pAd->ApCfg.ApCliTab[j].bRSN_IE_FromWpaSupplicant =
			    FALSE;
			...
			pAd->ApCfg.ApCliTab[j].WpaSupplicantUP =
			...
			RTMPZeroMemory(pAd->ApCfg.ApCliTab[j].SavedPMK,
