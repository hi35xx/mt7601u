	...
	if (RTMPGetKeyParameter("DefaultKeyID", tmpbuf, 25, buffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
				...
				if ((KeyIdx >= 1) && (KeyIdx <= 4))
					pAd->ApCfg.MBSSID[i].DefaultKeyId =
					    (UCHAR) (KeyIdx - 1);
				else
					  ...
					  IN PSTRING tmpbuf, IN PSTRING buffer)
{
	...
	/*ApCliSsid */
	if (RTMPGetKeyParameter
	    ("ApCliSsid", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, FALSE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_APCLI_NUM);
		     macptr = rstrtok(NULL, ";"), i++) {
			pApCliEntry = &pAd->ApCfg.ApCliTab[i];
	...
	/*ApCliBssid */
	if (RTMPGetKeyParameter
	    ("ApCliBssid", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_APCLI_NUM);
		     macptr = rstrtok(NULL, ";"), i++) {
			pApCliEntry = &pAd->ApCfg.ApCliTab[i];
		...
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("I/F(apcli%d) ApCli_AuthMode=%d \n", i,
				  pApCliEntry->AuthMode));
			RTMPMakeRSNIE(pAd, pApCliEntry->AuthMode,
				      pApCliEntry->WepStatus,
	...
	if (RTMPGetKeyParameter("ApCliWPAPSK", tmpbuf, 255, buffer, FALSE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			NdisMoveMemory(pApCliEntry->PSK, macptr,
				       strlen(macptr));
...
#ifdef DBG
			DBGPRINT(RT_DEBUG_TRACE,
				 ("I/F(apcli%d) PMK Material => \n", i));

			for (j = 0; j < 32; j++) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("%02x:", pApCliEntry->PMK[j]));
				if ((j % 16) == 15)
					DBGPRINT(RT_DEBUG_OFF, ("\n"));
			}
			DBGPRINT(RT_DEBUG_OFF, ("\n"));
	...
	for (idx = 0; idx < 4; idx++) {
		...
		if (RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer, TRUE)) {
			...
			/*ApCliKey1Str */
			if (RTMPGetKeyParameter
			    (tok_str, tmpbuf, 512, buffer, FALSE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     (macptr && i < MAX_APCLI_NUM);
				     macptr = rstrtok(NULL, ";"), i++) {
					pApCliEntry = &pAd->ApCfg.ApCliTab[i];
					KeyLen = strlen(macptr);
					if (((KeyType[i] == 0) && (KeyLen != 10)
					     && (KeyLen != 26))
					    || ((KeyType[i] != 0)
						&& (KeyLen != 5)
						&& (KeyLen != 13))) {
		...
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("I/F(apcli%d) Tx Mode = %d\n", i,
				  pApCliEntry->DesiredTransmitSetting.field.
				  FixedTxMode));
		}
	}
	...
	if (RTMPGetKeyParameter
	    ("ApCli_Wsc4digitPinCode", tmpbuf, 32, buffer, TRUE)) {
...
static void rtmp_read_acl_parms_from_file(IN PRTMP_ADAPTER pAd, PSTRING tmpbuf,
					  PSTRING buffer)
{
	STRING tok_str[32];
	PSTRING macptr;
	...
	for (idx = 0; idx < MAX_MBSSID_NUM(pAd); idx++) {
		...
		if (RTMPGetKeyParameter(tok_str, tmpbuf, 10, buffer, TRUE)) {
			...
			case 1:	/* Allow All, and the AccessControlList is positive now. */
				pAd->ApCfg.MBSSID[idx].AccessControlList.
				    Policy = 1;
				break;
		...
		if (RTMPGetKeyParameter
		    (tok_str, tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
			     macptr = rstrtok(NULL, ";"), i++) {
				if (strlen(macptr) != 17)	/* Mac address acceptable format 01:02:03:04:05:06 length 17 */
					continue;

				ASSERT(pAd->ApCfg.MBSSID[idx].AccessControlList.
				       Num <= MAX_NUM_OF_ACL_LIST);
				...
				if (pAd->ApCfg.MBSSID[idx].AccessControlList.
				    Num == MAX_NUM_OF_ACL_LIST) {
				...
				NdisMoveMemory(pAd->ApCfg.MBSSID[idx].
					       AccessControlList.
					       Entry[(pAd->ApCfg.MBSSID[idx].
					     ...
					     PSTRING tmpbuf, PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter("APAifsn", tmpbuf, 32, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->CommonCfg.APEdcaParm.Aifsn[i] =
			    (UCHAR) simple_strtol(macptr, 0, 10);;
	...
	if (RTMPGetKeyParameter("APCwmin", tmpbuf, 32, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->CommonCfg.APEdcaParm.Cwmin[i] =
			    (UCHAR) simple_strtol(macptr, 0, 10);;
	...
	if (RTMPGetKeyParameter("APCwmax", tmpbuf, 32, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->CommonCfg.APEdcaParm.Cwmax[i] =
			    (UCHAR) simple_strtol(macptr, 0, 10);;
	...
	if (RTMPGetKeyParameter("APTxop", tmpbuf, 32, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->CommonCfg.APEdcaParm.Txop[i] =
	...
	/*APACM*/ if (RTMPGetKeyParameter("APACM", tmpbuf, 32, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->CommonCfg.APEdcaParm.bACM[i] =
...
/*
    ...
    ========================================================================
*/
static void rtmp_read_radius_parms_from_file(IN PRTMP_ADAPTER pAd,
					     PSTRING tmpbuf, PSTRING buffer)
{
	STRING tok_str[16];
	PSTRING macptr;
	...
	if (RTMPGetKeyParameter
	    ("session_timeout_interval", tmpbuf, 32, buffer, TRUE)) {
	...
	while (RTMPGetKeyParameterWithOffset
	       ("RADIUS_Server", tmpbuf, &offset, 256, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			if (rtinet_aton(macptr, &ip_addr)
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IF(ra%d), radius_ip(seq-%d)=%s(%x)\n",
					  ...
					  macptr,
					  pAd->ApCfg.MBSSID[i].
					  radius_srv_info[srv_idx].radius_ip));
			}
		}
	}
	/* RADIUS_Port */
	...
	while (RTMPGetKeyParameterWithOffset
	       ("RADIUS_Port", tmpbuf, &offset, 128, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_MBSSID_NUM(pAd));
		     macptr = rstrtok(NULL, ";"), i++) {
	...
	memset(&count[0], 0, sizeof(count));
	while (RTMPGetKeyParameterWithOffset
	       ("RADIUS_Key", tmpbuf, &offset, 640, buffer, FALSE)) {
		if (strlen(tmpbuf) > 0)
			bUsePrevFormat = TRUE;
			...
			if (strlen(macptr) > 0
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IF(ra%d), radius_key(seq-%d)=%s, len=%d\n",
					  i, count[i],
					  pAd->ApCfg.MBSSID[i].
					  radius_srv_info[srv_idx].radius_key,
					  pAd->ApCfg.MBSSID[i].
					  radius_srv_info[srv_idx].
					  radius_key_len));
			}
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		if (RTMPGetKeyParameter(tok_str, tmpbuf, 33, buffer, FALSE)) {
			if (strlen(tmpbuf) > 0) {
				...
				NdisMoveMemory(pAd->ApCfg.MBSSID[i].NasId,
					       tmpbuf, strlen(tmpbuf));
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IF-ra%d NAS-ID=%s, len=%d\n", i,
...
static int rtmp_parse_wpapsk_buffer_from_file(IN PRTMP_ADAPTER pAd,
					      IN PSTRING buffer, IN INT BSSIdx)
{
	...
	ret =
	    RT_CfgSetWPAPSKKey(pAd, tmpbuf, len,
			       (PUCHAR) pAd->ApCfg.MBSSID[i].Ssid,
			       pAd->ApCfg.MBSSID[i].SsidLen,
			       pAd->ApCfg.MBSSID[i].PMK);
	if (ret == FALSE)
		return FALSE;
	...
	if ((len >= 8) && (len <= 64)) {
		NdisMoveMemory(pAd->ApCfg.MBSSID[i].WscControl.WpaPsk, tmpbuf,
			       len);
	...
	if (RTMPGetKeyParameter("FixedTxMode", pValueStr, 25, pInput, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			for (i = 0, Bufptr = rstrtok(pValueStr, ";");
				...
				pAd->ApCfg.MBSSID[i].DesiredTransmitSetting.
				    field.FixedTxMode =
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if ((Value >= 0 && Value <= 23) || (Value == 32)) {
				...
				pAd->ApCfg.MBSSID[MAIN_MBSSID].
				    DesiredTransmitSetting.field.MCS = Value;
#endif				/* P2P_SUPPORT */
...
#ifdef P2P_SUPPORT
				pAd->ApCfg.MBSSID[MAIN_MBSSID].
				    DesiredTransmitSetting.field.MCS = MCS_AUTO;
#endif				/* P2P_SUPPORT */
...
void RTMPSetSTAPassPhrase(RTMP_ADAPTER * pAd, PSTRING PassPh)
{
	...
	if (ret == TRUE) {
		...
		if ((strlen(PassPh) >= 8) && (strlen(PassPh) <= 64)) {
			NdisMoveMemory(pAd->StaCfg.WscControl.WpaPsk, PassPh,
				       strlen(PassPh));
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	do {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (RTMPGetKeyParameter
			    ("BssidNum", tmpbuf, 25, pBuffer, TRUE)) {
				pAd->ApCfg.BssidNum =
			...
			if (HW_BEACON_OFFSET >
			    (HW_BEACON_MAX_SIZE(pAd) / pAd->ApCfg.BssidNum)) {
				DBGPRINT(RT_DEBUG_OFF,
						...
						if (pAd->ApCfg.BssidNum > 1) {
							...
							for (i = 0, macptr =
								...
								pAd->ApCfg.
								    ...
								    [apidx].
								    SsidLen =
								    strlen((PSTRING) pAd->ApCfg.MBSSID[apidx].Ssid);
							...
							if ((strlen(tmpbuf) > 0)
								...
								pAd->ApCfg.
								    ...
								    [BSS0].
								    SsidLen =
								    strlen((PSTRING) pAd->ApCfg.MBSSID[BSS0].Ssid);
								DBGPRINT
								    (RT_DEBUG_TRACE,
								     ("SSID=%s\n",
								      pAd->
		...
		/*DFSIndoor */
		{
			PRADAR_DETECT_STRUCT pRadarDetect =
			    &pAd->CommonCfg.RadarDetect;
			PDFS_PROGRAM_PARAM pDfsProgramParam =
			    &pRadarDetect->DfsProgramParam;

			if (RTMPGetKeyParameter
			    ("DfsIndoor", tmpbuf, 10, pBuffer, TRUE)) {
				pAd->Dot11_H.bDFSIndoor =
				    (USHORT) (simple_strtol(tmpbuf, 0, 10) !=
					      0);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("DfsIndoor=%d\n",
					  pAd->Dot11_H.bDFSIndoor));
			}
			{
				...
				/*SymRoundFromCfg */
				if (RTMPGetKeyParameter
				    ("SymRoundFromCfg", tmpbuf, 10, pBuffer,
				     TRUE)) {
					pRadarDetect->SymRoundFromCfg =
					    (UCHAR) simple_strtol(tmpbuf, 0,
								  10);
				...
				/*BusyIdleFromCfg */
				if (RTMPGetKeyParameter
				    ("BusyIdleFromCfg", tmpbuf, 10, pBuffer,
				     TRUE)) {
				...
				/*DfsRssiHighFromCfg */
				if (RTMPGetKeyParameter
				    ("DfsRssiHighFromCfg", tmpbuf, 10, pBuffer,
				     TRUE)) {
				...
				/*DfsRssiLowFromCfg */
				if (RTMPGetKeyParameter
				    ("DfsRssiLowFromCfg", tmpbuf, 10, pBuffer,
				     TRUE)) {
				...
				/*DFSParamFromConfig */
				if (RTMPGetKeyParameter
				    ("DFSParamFromConfig", tmpbuf, 10, pBuffer,
				     TRUE)) {
				...
				/* DFSParam */
				for (k = 0; k < 4 * pAd->chipCap.DfsEngineNum;
				     k++) {
					STRING tok_str[32];
					INT index;
					UINT8 DfsEngineNum =
					    pAd->chipCap.DfsEngineNum;
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			/*DtimPeriod */
			if (RTMPGetKeyParameter
			    ("DtimPeriod", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*OLBCDetection */
		if (RTMPGetKeyParameter
		    ("DisableOLBC", tmpbuf, 10, pBuffer, TRUE)) {
			switch (simple_strtol(tmpbuf, 0, 10)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			/* MaxStaNum */
			if (RTMPGetKeyParameter
			    ("MaxStaNum", tmpbuf, 32, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					if (i >= pAd->ApCfg.BssidNum)
						break;

					ApCfg_Set_MaxStaNum_Proc(pAd, i,
								 macptr);
				}
			}
			...
			/* IdleTimeout */
			if (RTMPGetKeyParameter
			    ("IdleTimeout", tmpbuf, 10, pBuffer, TRUE)) {
				ApCfg_Set_IdleTimeout_Proc(pAd, tmpbuf);
			}

			/*NoForwarding */
			if (RTMPGetKeyParameter
			    ("NoForwarding", tmpbuf, 32, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					if (i >= pAd->ApCfg.BssidNum)
						break;
			...
			if (RTMPGetKeyParameter
			    ("NoForwardingBTNBSSID", tmpbuf, 10, pBuffer,
			     TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0)	/*Enable */
					pAd->ApCfg.
			...
			/*HideSSID */
			if (RTMPGetKeyParameter
			    ("HideSSID", tmpbuf, 32, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					int apidx = i;
			...
			/*StationKeepAlive */
			if (RTMPGetKeyParameter
			    ("StationKeepAlive", tmpbuf, 32, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					int apidx = i;
			...
			/*AutoChannelSelect */
			if (RTMPGetKeyParameter
			    ("AutoChannelSelect", tmpbuf, 10, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0) {	/*Enable */
					...
					if (SelAlg > 2 || SelAlg < 0) {
						pAd->ApCfg.
						    bAutoChannelAtBootup =
						    FALSE;
					} else {	/*Enable */

						pAd->ApCfg.
						    bAutoChannelAtBootup = TRUE;
			...
			/*AutoChannelSkipList */
			if (RTMPGetKeyParameter
			    ("AutoChannelSkipList", tmpbuf, 50, pBuffer,
			     FALSE)) {
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_STA_SCANED_AP_TH", tmpbuf, 32, pBuffer, TRUE)) {
			UINT8 count = simple_strtol(tmpbuf, 0, 10);
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_STA_CURRENT_CH_APS_TH", tmpbuf, 32, pBuffer,
		     TRUE)) {
			UINT8 count = simple_strtol(tmpbuf, 0, 10);
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_STA_RSSI_TH", tmpbuf, 32, pBuffer, TRUE)) {
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_ED_TH", tmpbuf, 32, pBuffer, TRUE)) {
			UINT8 count = simple_strtol(tmpbuf, 0, 10);
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_FALSE_CCA_TH", tmpbuf, 32, pBuffer, TRUE)) {
		...
		if (RTMPGetKeyParameter
		    ("EDCCA_BLOCK_CHECK_TH", tmpbuf, 32, pBuffer, TRUE)) {
			UINT8 count = simple_strtol(tmpbuf, 0, 10);
...
#endif				/* WPA_SUPPLICANT_SUPPORT */
#ifdef WAPI_SUPPORT
				else if ((strcmp(tmpbuf, "WAICERT") == 0)
					 || (strcmp(tmpbuf, "waicert") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeWAICERT;
				else if ((strcmp(tmpbuf, "WAIPSK") == 0)
					 || (strcmp(tmpbuf, "waipsk") == 0))
					pAd->StaCfg.AuthMode =
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				...
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
					...
					if ((strncmp(macptr, "NONE", 4) == 0)
					    || (strncmp(macptr, "none", 4) ==
						0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
						    Ndis802_11WEPDisabled;
					else if ((strncmp(macptr, "WEP", 3) ==
						  0)
						 || (strncmp(macptr, "wep", 3)
						     == 0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
						    Ndis802_11WEPEnabled;
					else if ((strncmp(macptr, "TKIPAES", 7)
						  == 0)
						 ||
						 (strncmp(macptr, "tkipaes", 7)
						  == 0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
						    Ndis802_11Encryption4Enabled;
					else if ((strncmp(macptr, "TKIP", 4) ==
						  0)
						 || (strncmp(macptr, "tkip", 4)
						     == 0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
						    Ndis802_11Encryption2Enabled;
					else if ((strncmp(macptr, "AES", 3) ==
						  0)
						 || (strncmp(macptr, "aes", 3)
						     == 0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
						    Ndis802_11Encryption3Enabled;
#ifdef WAPI_SUPPORT
					else if ((strncmp(macptr, "SMS4", 4) ==
						  0)
						 || (strncmp(macptr, "sms4", 4)
						     == 0))
						pAd->ApCfg.MBSSID[apidx].
						    WepStatus =
			...
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				if ((strcmp(tmpbuf, "WEP") == 0)
					...
					pAd->StaCfg.WepStatus =
					    Ndis802_11Encryption3Enabled;
#ifdef WAPI_SUPPORT
				else if ((strcmp(tmpbuf, "SMS4") == 0)
					 || (strcmp(tmpbuf, "sms4") == 0))
					pAd->StaCfg.WepStatus =
					    Ndis802_11EncryptionSMS4Enabled;
#endif				/* WAPI_SUPPORT */
				else
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			/* WpaMixPairCipher */
			if (RTMPGetKeyParameter
			    ("WpaMixPairCipher", tmpbuf, 256, pBuffer, TRUE)) {
				/*
				...
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					if (pAd->ApCfg.MBSSID[i].AuthMode !=
					    Ndis802_11AuthModeWPA1WPA2
					    && pAd->ApCfg.MBSSID[i].AuthMode !=
					    Ndis802_11AuthModeWPA1PSKWPA2PSK)
						continue;

					if (pAd->ApCfg.MBSSID[i].WepStatus !=
					    Ndis802_11Encryption4Enabled)
						continue;

					if ((strncmp
					     (macptr, "WPA_AES_WPA2_TKIPAES",
					      20) == 0)
					    ||
					    (strncmp
					     (macptr, "wpa_aes_wpa2_tkipaes",
			...
			/*RekeyMethod */
			if (RTMPGetKeyParameter
			    ("RekeyMethod", tmpbuf, 128, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					PRT_WPA_REKEY pRekeyInfo =
					...
					if ((strcmp(macptr, "TIME") == 0)
						...
						pRekeyInfo->ReKeyMethod =
						    PKT_REKEY;
					else if ((strcmp(macptr, "DISABLE") ==
						  0)
						 || (strcmp(macptr, "disable")
						     == 0))
						pRekeyInfo->ReKeyMethod =
						    DISABLE_REKEY;
					else
				...
				if (i == 1) {
					for (i = 1; i < pAd->ApCfg.BssidNum;
					     i++) {
						pAd->ApCfg.MBSSID[i].WPAREKEY.
						    ReKeyMethod =
						    pAd->ApCfg.MBSSID[0].
						    WPAREKEY.ReKeyMethod;
						DBGPRINT(RT_DEBUG_TRACE,
							 ("I/F(ra%d) ReKeyMethod=%ld\n",
							  i,
							  pAd->ApCfg.MBSSID[i].
			...
			/*RekeyInterval */
			if (RTMPGetKeyParameter
			    ("RekeyInterval", tmpbuf, 255, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
			...
			/*PMKCachePeriod */
			if (RTMPGetKeyParameter
			    ("PMKCachePeriod", tmpbuf, 255, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
				...
				if (RTMPGetKeyParameter
					...
					NdisZeroMemory(&pAd->ApCfg.MBSSID[i].
						       WscControl.
						       WscDefaultSsid,
						       sizeof
						       (NDIS_802_11_SSID));
					...
					pAd->ApCfg.MBSSID[i].WscControl.
					    WscDefaultSsid.SsidLength =
					    strlen(tmpbuf);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("WscDefaultSSID[%d]=%s\n", i,
						  pAd->ApCfg.MBSSID[i].
						  WscControl.WscDefaultSsid.
						  Ssid));
				}
			}
			...
			/*WscConfMode */
			if (RTMPGetKeyParameter
			    ("WscConfMode", tmpbuf, 10, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					INT WscConfMode =
					    simple_strtol(macptr, 0, 10);
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("I/F(ra%d) WscConfMode=%d\n",
						  i,
						  pAd->ApCfg.MBSSID[i].
						  WscControl.WscConfMode));
				}
			}

			/*WscConfStatus */
			if (RTMPGetKeyParameter
			    ("WscConfStatus", tmpbuf, 10, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					if (i >= pAd->ApCfg.BssidNum)
						break;
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("I/F(ra%d) WscConfStatus=%d\n",
						  i,
						  pAd->ApCfg.MBSSID[i].
						  WscControl.WscConfStatus));
				}
			}
			/*WscConfMethods */
			if (RTMPGetKeyParameter
			    ("WscConfMethods", tmpbuf, 32, pBuffer, TRUE)) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					if (i >= pAd->ApCfg.BssidNum)
						break;
			...
			if (RTMPGetKeyParameter
				...
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
					...
					DBGPRINT(RT_DEBUG_WARN,
						 ("WscKeyASCII=%d\n",
						  pAd->ApCfg.MBSSID[i].
						  WscControl.WscKeyASCII));
				}
			}

			if (RTMPGetKeyParameter
			    ("WscSecurityMode", tmpbuf, 50, pBuffer, TRUE)) {
				for (i = 0; i < pAd->ApCfg.BssidNum; i++)
					pAd->ApCfg.MBSSID[i].WscSecurityMode =
					    WPAPSKTKIP;
			...
			/* WCNTest */
			if (RTMPGetKeyParameter
			    ("WCNTest", tmpbuf, 10, pBuffer, TRUE)) {
				BOOLEAN bEn = FALSE;
			...
			for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
				...
				snprintf(tok_str, sizeof(tok_str),
					 "WSC_UUID_Str%d", i + 1);
				if (RTMPGetKeyParameter
				    (tok_str, tmpbuf, 40, pBuffer, FALSE)) {
					NdisMoveMemory(&pWpsCtrl->
						       Wsc_Uuid_Str[0], tmpbuf,
						       strlen(tmpbuf));
					DBGPRINT(RT_DEBUG_TRACE,
						 ("UUID_Str[%d]=%s\n", i + 1,
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
...
#ifdef DOT11Z_TDLS_SUPPORT
			if (RTMPGetKeyParameter
			    ("TDLSCapable", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0) {	/*Enable */
			...
			if (RTMPGetKeyParameter
			    ("TDLS_AutoLink", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0)	//Enable
				{
			...
			if (RTMPGetKeyParameter
			    ("TDLS_SwitchChSupp", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0)	//Enable
				{
			...
			if (RTMPGetKeyParameter
			    ("TDLS_PsmSupp", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) != 0)	//Enable
				{
			...
			//APSDAC for AC_BE, AC_BK, AC_VI, AC_VO
			if (RTMPGetKeyParameter
			    ("TDLS_APSDAC", tmpbuf, 32, pBuffer, TRUE)) {
				BOOLEAN FlgIsAcUapsdEnabled[4];

				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
					FlgIsAcUapsdEnabled[i] =
					    (BOOLEAN) simple_strtol(macptr, 0,
								    10);
				...
				pAd->CommonCfg.TDLS_bAPSDAC_BE =
				    FlgIsAcUapsdEnabled[0];
				pAd->CommonCfg.TDLS_bAPSDAC_BK =
				    FlgIsAcUapsdEnabled[1];
				pAd->CommonCfg.TDLS_bAPSDAC_VI =
				    FlgIsAcUapsdEnabled[2];
				pAd->CommonCfg.TDLS_bAPSDAC_VO =
				    FlgIsAcUapsdEnabled[3];
			}
			//MaxSPLength
			if (RTMPGetKeyParameter
			    ("TDLS_MaxSPLength", tmpbuf, 10, pBuffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			/* McastPhyMode */
			if (RTMPGetKeyParameter
			    ("McastPhyMode", tmpbuf, 32, pBuffer, TRUE)) {
			...
			/* McastMcs */
			if (RTMPGetKeyParameter
			    ("McastMcs", tmpbuf, 32, pBuffer, TRUE)) {
		...
		/* Wsc4digitPinCode = TRUE use 4-digit Pin code, otherwise 8-digit Pin code */
		if (RTMPGetKeyParameter
		    ("Wsc4digitPinCode", tmpbuf, 32, pBuffer, TRUE)) {
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
				     macptr; macptr = rstrtok(NULL, ";"), i++) {
		...
		if (RTMPGetKeyParameter
		    ("WscVendorPinCode", tmpbuf, 256, pBuffer, TRUE)) {
			PWSC_CTRL pWscContrl = NULL;
			int bSetOk;
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				pWscContrl =
				    &pAd->ApCfg.MBSSID[BSS0].WscControl;
			}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
...
#ifdef WSC_V2_SUPPORT
		if (RTMPGetKeyParameter
		    ("WscV2Support", tmpbuf, 32, pBuffer, TRUE)) {
			UCHAR bEnable;
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				for (i = 0, macptr = rstrtok(tmpbuf, ";");
					...
					bEnable =
					    (UCHAR) simple_strtol(macptr, 0,
								  10);
					pAd->ApCfg.MBSSID[i].WscControl.
					    WscV2Info.bEnableWpsV2 = bEnable;
					DBGPRINT(RT_DEBUG_TRACE,
		...
		if (RTMPGetKeyParameter
		    ("EntryLifeCheck", tmpbuf, 256, pBuffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			if (RTMPGetKeyParameter
			    ("P2P_GOIntent", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_DevName", tmpbuf, 32, pBuffer, TRUE)) {
				if ((strlen(tmpbuf) <= 32)
				    ) {
					pAd->P2pCfg.DeviceNameLen =
					    (UCHAR) strlen(tmpbuf);
					NdisZeroMemory(pAd->P2pCfg.DeviceName,
						       32);
			...
			if (RTMPGetKeyParameter
			    ("P2P_ListChannel", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_OpChannel", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_WscCfgMethod", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_ExtListenEnable", tmpbuf, 10, pBuffer,
			     TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_ExtListenPrd", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_ExtenListenInv", tmpbuf, 10, pBuffer, TRUE)) {
				pAd->P2pCfg.ExtListenInterval =
				    simple_strtol(tmpbuf, 0, 10);
				DBGPRINT(RT_DEBUG_ERROR,
					 ("P2P_ExtenListenInv=%d\n",
					  pAd->P2pCfg.ExtListenInterval));
			}

			if (RTMPGetKeyParameter
			    ("P2P_IntraBss", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_NoACnt", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_NoADuration", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_NoAInv", tmpbuf, 10, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("P2P_MaxEntry", tmpbuf, 10, pBuffer, TRUE)) {
		...
		if (RTMPGetKeyParameter
		    ("BW20_RSSI_THR0", tmpbuf, 10, pBuffer, TRUE))
			Set_BW20_RSSI_THR0_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW20_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
			Set_BW20_RSSI_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW20_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
			Set_BW20_RSSI_THR2_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW20_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
			Set_BW20_RSSI_THR3_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW40_RSSI_THR0", tmpbuf, 10, pBuffer, TRUE))
			Set_BW40_RSSI_THR0_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW40_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
			Set_BW40_RSSI_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW40_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
			Set_BW40_RSSI_THR2_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("BW40_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
			Set_BW40_RSSI_THR3_Proc(pAd, tmpbuf);

		/*Set false RSSI thredshold(for Anti-Noise RSSI-Base Dynamic VGA) */
		if (RTMPGetKeyParameter
		    ("NOISE_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
			Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("NOISE_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
			Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter
		    ("NOISE_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
			Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);

		/*1:enable dynamic vga. 0: disable dynamic vga */
		if (RTMPGetKeyParameter
		    ("DYPD_ENABLE", tmpbuf, 10, pBuffer, TRUE))
...
#ifdef WSC_INCLUDED
void rtmp_read_wsc_user_parms(PWSC_CTRL pWscControl,
			      STRING * tmpbuf, STRING * buffer)
{
	if (RTMPGetKeyParameter
	    ("WscManufacturer", tmpbuf, WSC_MANUFACTURE_LEN, buffer, TRUE)) {
	...
	/*WSC_User_ModelName */
	if (RTMPGetKeyParameter
	    ("WscModelName", tmpbuf, WSC_MODELNAME_LEN, buffer, TRUE)) {
	...
	/*WSC_User_DeviceName */
	if (RTMPGetKeyParameter
	    ("WscDeviceName", tmpbuf, WSC_DEVICENAME_LEN, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("WscModelNumber", tmpbuf, WSC_MODELNUNBER_LEN, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("WscSerialNumber", tmpbuf, WSC_SERIALNUNBER_LEN, buffer, TRUE)) {
				 ...
				 IN PSTRING tmpbuf, IN PSTRING buffer)
{
		...
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
...

			{
				...
				for (iii = 0; iii < MAX_WDS_ENTRY; iii++) {
					if (pAd->MulTestTab.WdsEntry[iii].
					    Valid == FALSE) {
						pAd->MulTestTab.WdsEntry[iii].
						    Valid = TRUE;
						pAd->MulTestTab.Size++;
						COPY_MAC_ADDR(pAd->MulTestTab.
							      WdsEntry[iii].
							      PeerWdsAddr,
							      macAddress);
						WdsTabIdx = iii;
	...
	if (RTMPGetKeyParameter("WdsEncrypType", tmpbuf, 128, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("WdsEncrypType[%d]=%d(%s)\n", i,
				  pAd->MulTestTab.WdsEntry[i].WepStatus,
				  GetEncryptType(pAd->MulTestTab.WdsEntry[i].
						 WepStatus)));
		}
	...
	if (RTMPGetKeyParameter("WdsKey", tmpbuf, 255, buffer, FALSE)) {
		...
		if (((pAd->MulTestTab.WdsEntry[0].WepStatus ==
			...
			NdisMoveMemory(&pAd->MulTestTab.WdsEntry[0].WdsKey.
				       RxMic, keyMaterial + 16, 8);
			NdisMoveMemory(&pAd->MulTestTab.WdsEntry[0].WdsKey.
				       TxMic, keyMaterial + 16, 8);
		}
		...
		if (pAd->MulTestTab.WdsEntry[0].WdsKey.KeyLen == 16) {
			for (j = 1; j < MAX_WDS_ENTRY; j++) {
				NdisMoveMemory(&pAd->MulTestTab.WdsEntry[j].
					       WdsKey,
					       &pAd->MulTestTab.WdsEntry[0].
					       WdsKey, sizeof(CIPHER_KEY));
			}
		}
			...
			if (RTMPGetKeyParameter
				...
				if (pAd->MulTestTab.WdsEntry[i].WepStatus ==
					...
					if (strlen(tmpbuf) == 5
						...
						pAd->MulTestTab.WdsEntry[i].
						    WdsKey.KeyLen = KeyLen;
					...
					/* Hex type */
						...
						KeyLen = strlen(tmpbuf);
						pAd->MulTestTab.WdsEntry[i].
						    WdsKey.KeyLen = KeyLen / 2;
						AtoH(tmpbuf,
						     pAd->MulTestTab.
						     WdsEntry[i].WdsKey.Key,
						     KeyLen / 2);
						if (KeyLen == 10)
							CipherAlg =
							    CIPHER_WEP64;
