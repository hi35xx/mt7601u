	...
	if (P2P_INF_ON(pAd)) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("SSID[%d] = %s.\n", pP2PCtrl->SSIDLen,
			  pP2PCtrl->SSID));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("NoA_Count = %d.        NoA_Duration = %ld.        NoA_Interval = %ld.        StartTime = %ld.\n",
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ExtListenPeriod = %d.        ExtListenInterval = %d.\n",
			  pP2PCtrl->ExtListenPeriod,
			  pP2PCtrl->ExtListenInterval));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Intra-Bss = %d.        \n", pP2PCtrl->bIntraBss));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ConenctMAC = %02x:%02x:%02x:%02x:%02x:%02x.\n",
			  PRINT_MAC(pP2PCtrl->ConnectingMAC)));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("p2pControl = %08x.    Persistent = %s. Invite = %s.    ClientDiscovery = %s.    IntraBss = %s.    ExtListen = %s.\n",
			  pP2PCtrl->P2pControl.word,
			  (IS_PERSISTENT_ON(pAd)) ? "ON" : "OFF",
			  (IS_INVITE_ON(pAd)) ? "ON" : "OFF",
			  (IS_CLIENT_DISCOVERY_ON(pAd)) ? "ON" : "OFF",
			  (IS_INTRA_BSS_ON(pAd)) ? "ON" : "OFF",
			  (IS_EXT_LISTEN_ON(pAd)) ? "ON" : "OFF"));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         Opps = %s.    SwNoATimer = %s.    ServiceDiscovery = %d.\n",
			  (IS_OPPS_ON(pAd)) ? "ON" : "OFF",
			  (IS_SW_NOA_TIMER(pAd)) ? "ON" : "OFF",
			  pP2PCtrl->P2pControl.field.ServiceDiscovery));
	}
...
INT Set_P2P_Device_Name(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (strlen(arg) <= 32) {
		...
		NdisMoveMemory(pAd->P2pCfg.DeviceName, arg,
			       pAd->P2pCfg.DeviceNameLen);
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Set P2P Device Name - %s", pAd->P2pCfg.DeviceName));
	}
...
INT Set_P2P_WSC_Mode(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (wscMode <= 2 && wscMode >= 1)
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("WscMode is invalid, using default and accept wsc method from peer's provision request action frame.\n"));
		pAd->P2pCfg.WscMode = WSC_PIN_MODE;	/* PIN */
		pAd->P2pCfg.Dpid = DEV_PASS_ID_NOSPEC;
		pAd->P2pCfg.ConfigMethod =
		    (WSC_CONFMET_PBC | WSC_CONFMET_KEYPAD |
		     WSC_CONFMET_DISPLAY);
	}
...
INT Set_P2P_Print_GroupTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < pAd->P2pTable.ClientNumber; i++) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         InterfaceAddr[%02x:%02x:%02x:%02x:%02x:%02x]\n",
		...
		if (pP2pEntry->WscMode == 1)
			DBGPRINT(RT_DEBUG_ERROR,
				 ("                         WscMode = PIN.        PIN = %02x %02x %02x %02x %02x %02x %02x %02x.\n",
				  pP2pEntry->PIN[0], pP2pEntry->PIN[1],
				  pP2pEntry->PIN[2], pP2pEntry->PIN[3],
				  pP2pEntry->PIN[4], pP2pEntry->PIN[5],
				  pP2pEntry->PIN[6], pP2pEntry->PIN[7]));
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("                         WscMode = ***Unknown***.\n"));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         CfgMethod = %s.        GoIntent = %d.\n",
			  decodeConfigMethod(pAd->P2pTable.Client[i].
					     ConfigMethod),
			  pAd->P2pTable.Client[i].GoIntent));
...
INT Set_P2P_Print_PersistentTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < pAd->P2pTable.PerstNumber; i++) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Table.PerstEntry[%d]: Rule = %s\n", i,
			  decodeMyRule(pAd->P2pTable.PerstEntry[i].MyRule)));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         DevAddr = %02x:%02x:%02x:%02x:%02x:%02x.\n",
			  PRINT_MAC(pAd->P2pTable.PerstEntry[i].Addr)));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         SSID[%d] = %s.\n",
			  pAd->P2pTable.PerstEntry[i].Profile.SSID.SsidLength,
			  pAd->P2pTable.PerstEntry[i].Profile.SSID.Ssid));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         MACAddr = %02x:%02x:%02x:%02x:%02x:%02x.\n",
			  PRINT_MAC(pAd->P2pTable.PerstEntry[i].Profile.
				    MacAddr)));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                         Key[%d] = %02x %02x %02x %02x  %02x %02x %02x %02x.\n",
			  pAd->P2pTable.PerstEntry[i].Profile.KeyIndex,
			  pAd->P2pTable.PerstEntry[i].Profile.Key[0],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[1],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[2],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[3],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[4],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[5],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[6],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[7]));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("                                  %02x %02x %02x %02x  %02x %02x %02x %02x.\n",
			  pAd->P2pTable.PerstEntry[i].Profile.Key[8],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[9],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[10],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[11],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[12],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[13],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[14],
			  pAd->P2pTable.PerstEntry[i].Profile.Key[15]));
	}
	return TRUE;
}
...
INT Set_P2P_Device_Discoverability_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (p2pindex < MAX_P2P_GROUP_SIZE) {
		...
		if (MyGOIdx != P2P_NOT_FOUND) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Start P2P Device Discoverability = %02x:%02x:%02x:%02x:%02x:%02x.	\n",
				  PRINT_MAC(pAd->P2pTable.Client[p2pindex].
					    addr)));
			pAd->P2pTable.Client[p2pindex].GeneralToken++;
			pAd->P2pTable.Client[MyGOIdx].P2pClientState =
...
INT Set_P2P_Connect_Dev_Addr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s:: DevMac = %02x:%02x:%02x:%02x:%02x:%02x\n", __FUNCTION__,
		  PRINT_MAC(ConnAddr)));
	...
	for (i = 0; i < pAd->P2pTable.ClientNumber; i++) {
		if (MAC_ADDR_EQUAL(pAd->P2pTable.Client[i].addr, ConnAddr) ||
		    ...
		    MAC_ADDR_EQUAL(pAd->P2pTable.Client[i].InterfaceAddr,
				   ConnAddr)) {
			p2pindex = i;
			break;
		}
...
INT Set_P2P_Provision_Dev_Addr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
 ...
 retry:
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("pAd->P2pTable.ClientNumber = %d, p2pindex = %d\n",
...
INT Set_P2P_Default_Config_Method_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((ConfigMethod >= 1) && (ConfigMethod <= 3)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s:: Default Config Method is %d.\n", __FUNCTION__,
			  ConfigMethod));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("    1: DISPLAY.    2: KEYPAD.    3. PBC\n"));
...
INT Set_P2P_QoS_NULL_Legacy_Rate_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s:: P2P QoS NULL using Legacy Rate Enable = %d.\n",
...
INT Set_P2P_CLIENT_PM_Enable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s:: P2P CLIENT PM Enable = %d.\n", __FUNCTION__,
		  pP2PCtrl->bP2pCliPmEnable));
					    ...
					    IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s:: P2P Device Discoverability Enable = %d.\n",
...
INT Set_P2P_Service_Discovery_Capable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
					     ...
					     IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s::  Send Service Discovery to DevAddr[%s]\n", __FUNCTION__,
		  arg));
	...
	if ((p2pindex < pAd->P2pTable.ClientNumber)
		...
		P2pConnectPrepare(pAd, pAddr,
				  P2PSTATE_SERVICE_DISCOVER_INIT_COMMAND);
	} else
		DBGPRINT(RT_DEBUG_ERROR, ("Table Idx out of range!\n"));
						 ...
						 IN PSTRING arg)
{
	...
	if ((p2pindex < pAd->P2pTable.ClientNumber)
		...
		pAddr = &pAd->P2pTable.Client[p2pindex].addr[0];
		P2pConnectPrepare(pAd, pAddr,
				  P2PSTATE_SERVICE_COMEBACK_COMMAND);
	} else
		DBGPRINT(RT_DEBUG_ERROR, ("Table Idx out of range!\n"));
...
INT Set_P2P_DevDiscPeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s:: P2P Device Discovery Period = %lu.\n", __FUNCTION__,
...
INT Set_P2P_PriDeviceType_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	hex_dump("Set_P2P_PriDeviceType_Proc:: PriDeviceType",
		 pP2PCtrl->DevInfo.PriDeviceType, 8);
	return TRUE;
}
...
INT Set_P2P_SecDevTypeList_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (len == 0)		//reset to zero
	{
		...
		if ((len % (8 * 2)) == 2)
		{
			...
			AtoH(arg, pP2PCtrl->DevInfo.SecDevTypList,
			     (tpylen * 8) + 1);
			hex_dump
			    ("Set_P2P_SecDevTypeList_Proc : p2p2ndDevTypList",
			     pP2PCtrl->DevInfo.SecDevTypList, (tpylen * 8) + 1);

		} else {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: arg length incorrect\n", __FUNCTION__));
			return FALSE;
		}
	} else {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: arg length is not enough!\n", __FUNCTION__));
		return FALSE;
	}
...
INT Set_P2P_Cancel_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (val == 1) {
		...
		if (!MAC_ADDR_EQUAL
		    (&ZERO_MAC_ADDR[0], &pP2PCtrl->ConnectingMAC[0])) {
			UCHAR p2pindex;

			p2pindex =
			    P2pGroupTabSearch(pAd, pP2PCtrl->ConnectingMAC);
			if (p2pindex < MAX_P2P_GROUP_SIZE) {
				if (pAd->P2pTable.Client[p2pindex].Rule ==
				    P2P_IS_GO)
					...
					pAd->P2pTable.Client[p2pindex].
					    P2pClientState = P2PSTATE_DISCOVERY;
