...
/*	
	...
	==========================================================================
 */
VOID PeerGASIntialReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
...
/*	
	...
	==========================================================================
 */
VOID PeerGASIntialRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	/*UCHAR Action = Elem->Msg[LENGTH_802_11+1]; */
	/*UCHAR StatusCode; */
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2pPeer  PeerGASIntialRspAction = %ld, \n", Elem->MsgLen));
	pDest = &Elem->Msg[0];
	for (i = 0; i < Elem->MsgLen;) {
		DBGPRINT(RT_DEBUG_TRACE,
			 (": %x %x %x %x %x %x %x %x %x %x \n", *(pDest + i),
			  *(pDest + i + 1), *(pDest + i + 2), *(pDest + i + 3),
			  *(pDest + i + 4), *(pDest + i + 5), *(pDest + i + 6),
			  *(pDest + i + 7), *(pDest + i + 8),
			  *(pDest + i + 9)));
		i = i + 10;
	}
...
/*	
	...
	==========================================================================
 */
VOID P2PPublicAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeFakeNoATlv(IN PRTMP_ADAPTER pAd,
		       IN ULONG StartTime, IN PUCHAR pOutBuffer)
{
	PUCHAR pDest;
...
/*	
	...
	==========================================================================
 */
ULONG InsertP2PGroupInfoTlv(IN PRTMP_ADAPTER pAd, IN PUCHAR pOutBuffer)
{
	...
	*(pDest + 2) = 0;	/* Set length to 0 first.  Need to update to real length in the end of this function. */
	...
	for (i = 0; i < MAX_P2P_GROUP_SIZE; i++) {
		...
		if (pClient->P2pClientState == P2PSTATE_CLIENT_WPS_DONE) {
			...
			RTMPMoveMemory(ClientInfo.DevAddr, pClient->addr,
				       MAC_ADDR_LEN);
			DBGPRINT(RT_DEBUG_INFO,
				 ("  -- InsertP2PGroupInfoTlv( ) (Mac  %02x:%02x:%02x:%02x:%02x:%02x.\n",
				  PRINT_MAC(pClient->addr)));
			RTMPMoveMemory(ClientInfo.InterfaceAddr,
				       pClient->InterfaceAddr, MAC_ADDR_LEN);
			...
			RTMPMoveMemory(ClientInfo.ConfigMethod, &config_method,
				       2);
			RTMPMoveMemory(ClientInfo.PrimaryDevType,
				       pClient->PrimaryDevType,
				       P2P_DEVICE_TYPE_LEN);
			...
			RTMPMoveMemory(ClientInfo.PrimaryDevType,
				       ...
				       P2P_DEVICE_TYPE_LEN);
			ClientInfo.Length =
			    SIZE_OF_FIXED_CLIENT_INFO_DESC - 1 +
			    pClient->DeviceNameLen + 4;
			RTMPMoveMemory(pContent, &ClientInfo,
				       SIZE_OF_FIXED_CLIENT_INFO_DESC);
			...
			*((PUSHORT) (pContent + 2)) =
			    cpu2be16(pClient->DeviceNameLen);
			RTMPMoveMemory(pContent + 4, pClient->DeviceName,
...
/*	
	...
	==========================================================================
 */
ULONG InsertP2PSubelmtTlv(IN PRTMP_ADAPTER pAd,
			  IN UCHAR SubId,
			  IN PUCHAR pInBuffer, IN PUCHAR pOutBuffer)
{
	...
	*pDest = SubId;
	...
	case SUBID_P2P_DEVICE_INFO:	/* 13 */
		*pDest =
		    17 + pP2PCtrl->DeviceNameLen + 4 +
		    (8 * pP2PCtrl->DevInfo.SecDevTypList[0]);
		*(pDest + 1) = 0;

		RTMPMoveMemory(pDest + 2, pInBuffer, MAC_ADDR_LEN);
		/* DEvice Type */
		*((PUSHORT) (pDest + 8)) = cpu2be16(pP2PCtrl->ConfigMethod);
		RTMPMoveMemory(pDest + 10, &pP2PCtrl->DevInfo.PriDeviceType[0],
			       8);

		{
			*(pDest + 18) = pP2PCtrl->DevInfo.SecDevTypList[0];
			NdisMoveMemory((pDest + 19),
				       &pP2PCtrl->DevInfo.SecDevTypList[1],
				       (8 *
					(pP2PCtrl->DevInfo.SecDevTypList[0])));
			pDest +=
			    (19 + (8 * pP2PCtrl->DevInfo.SecDevTypList[0]));
		}

		*((PUSHORT) pDest) = cpu2be16(WSC_ID_DEVICE_NAME);
		*((PUSHORT) (pDest + 2)) = cpu2be16(pP2PCtrl->DeviceNameLen);
		RTMPMoveMemory(pDest + 4, &pP2PCtrl->DeviceName[0],
			       pP2PCtrl->DeviceNameLen);

		Length =
		    20 + (8 * (pP2PCtrl->DevInfo.SecDevTypList[0])) +
		    pP2PCtrl->DeviceNameLen + 4;
		break;
	...
	case SUBID_P2P_GROUP_ID:	/* 15 */
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 (" -----Insert SUBID_P2P_GROUP_ID (Len = %ld) \n",
			  Length));
		break;
...
/*	
	...
	==========================================================================
 */
VOID InsertP2pChannelList(IN PRTMP_ADAPTER pAd,
			  IN UCHAR OpChannel,
			  OUT ULONG * ChannelListLen, OUT PUCHAR pDest)
{
	UCHAR i, pos;
	...
	for (i = 0; i < pAd->ChannelListNum; i++) {
		...
		if (LastRegClass == 0xff) {
			...
			*(pDest) = CurRegClass;
		} else if ((CurRegClass != LastRegClass)
			   && (LastRegClass != 0xff)) {
	...
	*ChannelListLen = pos;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s:: Channel List ==> output length is %ld \n", __FUNCTION__,
		  *ChannelListLen));
...
/*	
	...
	==========================================================================
 */
BOOLEAN P2pParseGroupInfoAttribute(IN PRTMP_ADAPTER pAd,
				   IN UCHAR P2pindex,
		...
		if (RTMPEqualMemory(&pEid->Octet[0], WIFIDIRECT_OUI, 4)) {
			...
			while ((Length + 3 + AttriLen) <= pEid->Len) {
				...
				case SUBID_P2P_GROUP_INFO:
					...
					DBGPRINT(RT_DEBUG_INFO,
						 ("SUBID_P2P_GROUP_INFO - Go index=%d\n",
						  P2pindex));
					while (GroupInfoLen > 0) {
						pClient =
						    (P2P_CLIENT_INFO_DESC *)
						    pData;
						...
						if ((ThisDescLen < 23)
							...
							GroupInfoLen)) {
							DBGPRINT(RT_DEBUG_ERROR,
								 ("Error parsing P2P IE group info attribute. This Client -%d/%ld\n",
								  ThisDescLen,
								  GroupInfoLen));
							break;
						}
							...
							P2PSTATE_CLIENT_DISCO_COMMAND))
						{
							...
							DBGPRINT(RT_DEBUG_TRACE,
								 ("!P2P topology changed[P2pClientState = %d] when parsing P2P IE group info attribute. Delete from index : %d\n",
								  pAd->P2pTable.
								  Client[idx].
								  P2pClientState,
								  SmallerP2Pidx));
							...
							for (i =
									...
									P2PSTATE_DISCOVERY_CLIENT))
								{
									DBGPRINT
									    (RT_DEBUG_ERROR,
									     ("!break right away because we have another connect command to continue. update topology is not so important to do right now.\n"));
									break;
								} else {
									P2pGroupTabDelete
									    (pAd,
						...
						if ((idx == P2P_NOT_FOUND)
							...
							DBGPRINT(RT_DEBUG_TRACE,
								 ("Insert a P2P Client to P2P table [%d]  .\n",
								  idx));
				...
				pP2pEid =
				    (PP2PEID_STRUCT) ((UCHAR *) pP2pEid + 3 +
						      AttriLen);
				AttriLen =
				    pP2pEid->Len[0] + pP2pEid->Len[1] * 8;
			}
...
/*	
	...
	==========================================================================
 */
VOID P2pParseNoASubElmt(IN PRTMP_ADAPTER pAd,
			IN VOID * Msg,
			IN ULONG MsgLen, IN UCHAR wcidindex, IN UINT32 Sequence)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	ULONG Length = 0;
	PP2PEID_STRUCT pP2pEid;
	...
	if (bNoAAttriExist == FALSE) {
		if (P2P_TEST_BIT(pAd->P2pCfg.CTWindows, P2P_OPPS_BIT)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Beacon and no NoA Attribute! \n"));
			P2pStopOpPS(pAd);
		}
		if ((pAd->MacTab.Content[wcidindex].P2pInfo.NoADesc[0].bValid ==
		     TRUE)) {
			DBGPRINT(RT_DEBUG_TRACE,
...
/*	
	...
	==========================================================================
 */
VOID P2pParseExtListenSubElmt(IN PRTMP_ADAPTER pAd,
			      IN VOID * Msg,
			      IN ULONG MsgLen,
...
/*	
	...
	==========================================================================
 */
VOID P2pParseManageSubElmt(IN PRTMP_ADAPTER pAd,
			   IN VOID * Msg,
			   IN ULONG MsgLen,
			   OUT UCHAR * pChannel,
			   OUT UCHAR * pNumOfP2pOtherAttribute,
			   OUT UCHAR * pTotalNumOfP2pAttribute,
			   OUT UCHAR * pMamageablity, OUT UCHAR * pMinorReason)
{
	PP2PEID_STRUCT pP2pEid;
	PEID_STRUCT pEid;
	ULONG Length;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2pParseManageSubElmt  MsgLen = %ld.  \n", MsgLen));
	...
	while ((ULONG) (pEid->Len + 2) <= LeftLength) {
		if (pEid->Eid == IE_CHANNEL_USAGE) {
			*pChannel = pEid->Octet[2];
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IE_CHANNEL_USAGE  = %x %x %x %x [ch=]%x.  \n",
				  pEid->Eid, pEid->Len, pEid->Octet[0],
				  pEid->Octet[1], pEid->Octet[2]));
		}
		/* might contains P2P IE and WPS IE.  So use if else if enough for locate  P2P IE. */
		if (RTMPEqualMemory(&pEid->Octet[0], WIFIDIRECT_OUI, 4)) {
			/* Get Request content capability */
			...
			while ((Length + 3 + AttriLen) <= pEid->Len) {
				...
				case SUBID_P2P_MINOR_REASON:
					DBGPRINT(RT_DEBUG_TRACE,
						 ("SYNC -  Has P2P SUBID_P2P_MINOR_REASON IE Minor Reason = %d.\n",
						  pP2pEid->Octet[0]));
					if (pTotalNumOfP2pAttribute != NULL)
					...
					if (pTotalNumOfP2pAttribute != NULL) {
						*pTotalNumOfP2pAttribute =
						    *pTotalNumOfP2pAttribute +
						    1;
						DBGPRINT(RT_DEBUG_TRACE,
							 ("SYNC -Ap Has P2P Manageability IE . Total P2P IE count is %d \n",
							  *pTotalNumOfP2pAttribute));
					}
...
/*	
	...
	==========================================================================
 */
VOID P2pParseSubElmt(IN PRTMP_ADAPTER pAd,
		     IN VOID * Msg,
		     IN ULONG MsgLen,
		     IN BOOLEAN bBeacon,
		     OUT USHORT * pDpid,
		     ...
		     OUT UCHAR * pInviteFlag, OUT VOID * pPrivate)
{
	ULONG Length = 0;
	PP2PEID_STRUCT pP2pEid;
	ULONG AttriLen;
	...
	while ((ULONG) (AccuIeLen) <= MsgLen) {
		if (RTMPEqualMemory(&pEid->Octet[0], WPS_OUI, 4)) {
			...
			if (bBeacon == TRUE)
				P2PParseWPSIE((PUCHAR) (&pEid->Octet),
					      (USHORT) (pEid->Len + 2), pDpid,
					      pWpsConfigMethod, pDeviceName,
					      pDeviceNameLen);
			else
				P2PParseWPSIE((PUCHAR) (&pEid->Octet),
					      (USHORT) (pEid->Len + 2), pDpid,
					      pWpsConfigMethod, NULL, NULL);
		}
		/* might contains P2P IE and WPS IE.  So use if else if enough for locate  P2P IE. */
			...
			while ((Length + 3 + AttriLen) <= AccuP2PIELen) {
				...
				pP2pEid =
				    (PP2PEID_STRUCT) ((UCHAR *) pP2pEid + 3 +
						      AttriLen);
				AttriLen =
				    pP2pEid->Len[0] + pP2pEid->Len[1] * 256;
			}
		...
		if (RTMPEqualMemory(&pEid->Octet[1], WIFIDIRECT_OUI, 4)) {
			pEid =
			    (PEID_STRUCT) ((UCHAR *) pEid +
					   (pEid->Len + pEid->Octet[0] * 256 +
					    3));
...
/*	
	...
	==========================================================================
 */
VOID P2pReceGoNegoConfirmAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 (" P2P - Recieve Confirm Confirm Confirm. -> From %02x:%02x:%02x:%02x:%02x:%02x.\n",
		  PRINT_MAC(pFrame->p80211Header.Addr2)));

	/* Get Request content capability */
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen + 7 - sizeof(P2P_PUBLIC_FRAME)), FALSE,
			&Dpid, &GroupCap, &DeviceCap, NULL, &DeviceNameLen,
			DevAddr, IfAddr, NULL, &SsidLen, (PUCHAR) & Ssid,
			&ConfigMethod, NULL, DevType, &Channel, &OpChannel,
			NULL, &Intent, &StatusCode, NULL, NULL);
	...
	if (StatusCode != 0) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - Ignore Go Negociation Confirm Status Code not Success. \n"));
		return;
	} else
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - Recieve Confirm Confirm Confirm Success. \n"));
	...
	if (index == P2P_NOT_FOUND) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - Ignore Go Negociation Confirm from Unknown device. \n"));
		...
		if ((pP2PCtrl->P2PConnectState == P2P_ANY_IN_FORMATION_AS_GO)
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - Existing peer stay in another state. = %d. return.\n",
				  pAd->P2pTable.Client[index].P2pClientState));
			return;
		}
	}
	...
	if (index < MAX_P2P_GROUP_SIZE) {
		...
		if (!NdisEqualMemory(AllZero, IfAddr, MAC_ADDR_LEN))
			RTMPMoveMemory(pP2pEntry->InterfaceAddr, IfAddr,
				       MAC_ADDR_LEN);
		DBGPRINT(RT_DEBUG_ERROR,
			 (" pP2pEntry[%d]->rule = %s. GoIntent = %d. My intent is %d. \n",
			  index, decodeMyRule(pP2pEntry->Rule),
			  pP2pEntry->GoIntent, pP2PCtrl->GoIntentIdx));
		...
		if (pP2PCtrl->P2PConnectState == P2P_ANY_IN_FORMATION_AS_CLIENT) {
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - I become ENROLLEE!GOGO Go's SsidLen = %d.!!\n",
				  pP2pEntry->SsidLen));
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - I become ENROLLEE!GOGO Go's Ssid[%d] = ",
				  pP2pEntry->SsidLen));
			...
			DBGPRINT(RT_DEBUG_ERROR, ("\n"));
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - I become ENROLLEE!!GOGO Go's Bssid = %02x:%02x:%02x:%02x:%02x:%02x!!\n",
				  PRINT_MAC(pP2PCtrl->Bssid)));
			RTMPZeroMemory(pP2PCtrl->SSID, 32);
			RTMPMoveMemory(pP2PCtrl->SSID, pP2pEntry->Ssid,
				       pP2pEntry->SsidLen);
			pP2PCtrl->SSIDLen = pP2pEntry->SsidLen;
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P - I become ENROLLEE!!GOGO Go's SSID[%d] = %s!!\n",
...
/*	
	...
	==========================================================================
 */
VOID P2pReceGoNegoRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) & Elem->Msg[0];
	UCHAR DevType[8], Ssid[32], DevAddr[6], IfAddr[6] = { 0 }
	, Channel = 0, OpChannel = 0, Intent, index;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 (" P2P -  GOGOGO Recieve Response.Response MsgLen = %ld.\n",
		  Elem->MsgLen));
	...
	/* Get Request content capability */
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, &Dpid,
			&GroupCap, &DeviceCap, DeviceName, &DeviceNameLen,
			DevAddr, IfAddr, NULL, &SsidLen, Ssid, &ConfigMethod,
			NULL, DevType, &Channel, &OpChannel, NULL, &TempIntent,
			&StatusCode, NULL, NULL);

	Intent = TempIntent >> 1;
	DBGPRINT(RT_DEBUG_TRACE,
		 (" Dev Addr = %02x:%02x:%02x:%02x:%02x:%02x. Intent = %d. My Intent = %d. \n",
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 (" P2P -StatusCode = %d. PeerIntent = %d . OpChannel = %d. My Intent = %d.\n",
	...
	if (StatusCode == P2PSTATUS_BOTH_INTENT15) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 (" P2P - Receive Status Code that both Go Intent Value is 15.  = %x \n",
			  StatusCode));
		P2pLinkDown(pAd, P2P_CONNECT_FAIL);
		return;
	} else if ((StatusCode == P2PSTATUS_REJECT_BY_USER)
		   || (StatusCode == P2PSTATUS_PASSED)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P -Go Negociation Response Status Code PASSED or being rejected  = %d \n",
			  StatusCode));
		index = P2pGroupTabSearch(pAd, DevAddr);
		if (index < MAX_P2P_GROUP_SIZE) {
			...
			MlmeEnqueue(pAd, P2P_DISC_STATE_MACHINE,
				    P2P_DISC_CANL_CMD_EVT, 0, NULL, 0);
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P -  = %s \n",
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - Ignore Go Negociation Response Status Code not Success.  = %d \n",
			  StatusCode));
		return;
	}
	...
	if (index == P2P_NOT_FOUND) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - Ignore Go Negociation Response from Unknown device. \n"));
		return;
	} else {
	...
	if (index < MAX_P2P_GROUP_SIZE) {
		...
		if (P2P_TEST_FLAG(pP2pEntry, P2PFLAG_PROVISIONED)) {
			...
			case WSC_CONFMET_KEYPAD:
				if (Dpid != DEV_PASS_ID_USER) {
					DBGPRINT(RT_DEBUG_ERROR,
						 (" P2P -3 Ignore Go Negociation Response with wrong dpid \n"));
					return;
				}
				break;
			default:
		...
		if ((pP2PCtrl->P2PConnectState == P2P_ANY_IN_FORMATION_AS_GO)
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 (" P2P -  Existing peer stay in another state. = %d. \n",
				  pAd->P2pTable.Client[index].P2pClientState));
			return;
		}
		P2PSendGoNegoConfirm(pAd, pFrame->Token, index,
				     pP2pEntry->addr);
	}
...
/*	
	...
	==========================================================================
 */
VOID P2pReceGoNegoReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	UCHAR DevType[8] = { 0 }
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("GOGOGOGO P2P - receive Go Neg Request. MsgLen = %ld \n",
		  Elem->MsgLen));
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, &Dpid,
			&GroupCap, &DeviceCap, DeviceName, &DeviceNameLen,
			DevAddr, BssidAddr, NULL, &SsidLen, NULL, &ConfigMethod,
			NULL, DevType, &Channel, &OpChannel, NULL, &TempIntent,
			&StatusCode, NULL, NULL);

	Intent = TempIntent >> 1;
	DBGPRINT(RT_DEBUG_TRACE,
		 ("    DevAddr in P2P IE = %02x %02x %02x %02x %02x %02x  \n",
		  PRINT_MAC(DevAddr)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("    Addr2 = %02x %02x %02x %02x %02x %02x  \n",
		  PRINT_MAC(pFrame->p80211Header.Addr2)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("    its bssid = %02x %02x %02x %02x %02x %02x  \n",
		  PRINT_MAC(BssidAddr)));
	DBGPRINT(RT_DEBUG_TRACE,
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2P -Peer[%d] Intent = %x . OpChannel = %x. My Intent = %x. TempIntent = %x\n",
		  index, Intent, Channel, pP2PCtrl->GoIntentIdx, TempIntent));
	if (index == P2P_NOT_FOUND) {
		DBGPRINT(RT_DEBUG_TRACE, ("P2P -1  insert\n"));
		index =
		    P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_DISCOVERY, NULL, 0,
				      DeviceCap, GroupCap);
		if (index < MAX_P2P_GROUP_SIZE) {
			pP2PCtrl->PopUpIndex = index;
			RTMPMoveMemory(pAd->P2pTable.Client[index].DeviceName,
				       DeviceName, 32);
			DBGPRINT(RT_DEBUG_ERROR,
				 ("From a unknown peer. Pop up setting windows. %d\n",
				  pP2PCtrl->PopUpIndex));
			pAd->P2pTable.Client[index].DeviceNameLen =
			    DeviceNameLen;
			pAd->P2pTable.Client[index].DeviceName[pAd->P2pTable.
							       Client[index].
							       DeviceNameLen] =
			    0x0;
	...
	if (index < MAX_P2P_GROUP_SIZE) {
		...
		if (!NdisEqualMemory(AllZero, BssidAddr, MAC_ADDR_LEN))
			RTMPMoveMemory(pP2pEntry->InterfaceAddr, BssidAddr,
				       MAC_ADDR_LEN);
		P2PPrintP2PEntry(pAd, index);

		DBGPRINT(RT_DEBUG_TRACE,
			 ("My P2P Dpid = %x.    State = %s. \n", pP2PCtrl->Dpid,
			  decodeP2PClientState(pP2pEntry->P2pClientState)));
		if (IS_P2P_PEER_CLIENT_OP(pP2pEntry)
		    || IS_P2P_PEER_WPAPSK(pP2pEntry)
		    || IS_P2P_PEER_PROVISIONING(pP2pEntry)) {
			/* Receive Go Neg Req when this peer's state is operating or doing provisioning. Delete this peer. */
			DBGPRINT(RT_DEBUG_TRACE,
				 (" P2P - Existing peer  state is %d. %s,.	Delete it.\n",
				  pP2pEntry->P2pClientState,
		...
		if (pP2pEntry->P2pClientState > P2PSTATE_GO_DONE) {
			DBGPRINT(RT_DEBUG_TRACE,
				 (" P2P - Existing peer  state is %d . %s,. return.\n",
				  pP2pEntry->P2pClientState,
				  decodeP2PClientState(pP2pEntry->
						       P2pClientState)));
			goto CleanUp;
		} else if (pP2pEntry->P2pClientState ==
			   P2PSTATE_SENT_GO_NEG_REQ) {
			if (IsP2pFirstMacSmaller
			    (pP2pEntry->addr, pP2PCtrl->CurrentAddress)) {
				DBGPRINT(RT_DEBUG_TRACE,
					 (" P2P - Dual GO Req. Existing peer  state is %s. \n",
					  decodeP2PClientState(pP2pEntry->
							       P2pClientState)));
				goto CleanUp;
			}
	...
	if ((Intent == pP2PCtrl->GoIntentIdx) && (Intent == 15)) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2pReceGoNegoReqAction -Receive a peer that has the same Go Intent index as mine. \n"));
	}
	...
	if (P2P_TEST_FLAG(pP2pEntry, P2PFLAG_PROVISIONED)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P -	Go Negociation Responce to provisioned	\n"));
...
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
	if (bSendEvent)
		P2pSendWirelessEvent(pAd, RT_P2P_DEVICE_FIND,
				     &pAd->P2pTable.Client[index],
				     pFrame->p80211Header.Addr2);
	P2pSendWirelessEvent(pAd, RT_P2P_RECV_GO_NEGO_REQ,
			     &pAd->P2pTable.Client[index],
			     pFrame->p80211Header.Addr2);
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 (" P2P - ReceDpid = %x. SentDpid %x \n", (Dpid), (SentDpid)));
	DBGPRINT(RT_DEBUG_TRACE,
		 (" P2P - ReceDpid = %s. SentDpid %s \n", decodeDpid(Dpid),
		  decodeDpid(SentDpid)));
	P2PMakeGoNegoRsp(pAd, DevAddr, SentDpid, pFrame->Token, TempIntent,
			 OpChannel, RspStatus, pOutBuffer, &TempLen);
	FrameLen = TempLen;
	pDest += TempLen;
	/* : copy req to rsp first. */

	DBGPRINT(RT_DEBUG_TRACE,
		 (" P2P - Make Go Negociation Responce Length = %ld.RspStatus = %d \n",
		  FrameLen, RspStatus));
	...
	if (RspStatus == P2PSTATUS_PASSED) {
		...
		if (pP2PCtrl->ConfigMethod != WSC_CONFMET_KEYPAD)
			MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE,
				    ...
				    sizeof(P2P_CMD_STRUCT), &P2pCmd, 0);
 ...
 */
UCHAR DevDiscCnt = 0;
VOID P2pReceDevDisReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	//PRT_P2P_CONFIG        pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	...
	/* Get Request content capability */
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, NULL,
			&GroupCap, &DeviceCap, NULL, &DeviceNameLen, devAddr,
			NULL, BssidAddr, &SsidLen, NULL, NULL, NULL, NULL, NULL,
			&OpChannel, NULL, NULL, &StatusCode, NULL, NULL);
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - ask for p2p client[%d] = %02x:%02x:%02x:%02x:%02x:%02x.\n",
	...
	if (((pMacEntry != NULL) && (pEntry != NULL)
		...
		RTMPMoveMemory(&pAd->P2pCfg.LatestP2pPublicFrame, pFrame,
			       sizeof(P2P_SAVED_PUBLIC_FRAME));
		OS_WAIT(500);
		P2PSendDevDisRsp(pAd, P2PSTATUS_SUCCESS,
				 pAd->P2pCfg.LatestP2pPublicFrame.Token,
				 pAd->P2pCfg.LatestP2pPublicFrame.p80211Header.
				 Addr2, &TotalFrameLen);
	} else {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s:: P2P -can't find p2p client .Status fail\n",
			  __FUNCTION__));
		P2PSendDevDisRsp(pAd, P2PSTATUS_IMCOMPA_PARM, pFrame->Token,
				 pFrame->p80211Header.Addr2, &TotalFrameLen);
	}
}
...
/*	
	...
	==========================================================================
 */
VOID P2pReceDevDisRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	UCHAR DevType[8], DevAddr[6], BssidAddr[6], Channel, OpChannel, index,
	    StatusCode;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("GOGO  P2P - P2pReceDevDisRspAction MsgLen = %ld ",
		  Elem->MsgLen));
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, &Dpid,
			&GroupCap, &DeviceCap, NULL, &DeviceNameLen, DevAddr,
			BssidAddr, NULL, &SsidLen, NULL, &ConfigMethod, NULL,
			DevType, &Channel, &OpChannel, NULL, &TempIntent,
			&StatusCode, NULL, NULL);
	...
	if (StatusCode == P2PSTATUS_SUCCESS) {
		...
		pAd->P2pTable.Client[ClientP2PIndex].P2pClientState =
		    P2PSTATE_CONNECT_COMMAND;
		P2pStartGroupForm(pAd,
...
/*	
	...
	==========================================================================
 */
VOID P2pReceInviteReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	UCHAR DevType[8], Ssid[32], DevAddr[6], BssidAddr[6], InfAddr[6],
	    OpChannel, index, StatusCode;
	UCHAR GroupCap, DeviceCap, DeviceNameLen;
	ULONG FrameLen;
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, NULL,
			&GroupCap, &DeviceCap, NULL, &DeviceNameLen, DevAddr,
			InfAddr, BssidAddr, &SsidLen, Ssid, &ConfigMethod, NULL,
			DevType, NULL, &OpChannel, ChannelList, NULL,
			&StatusCode, &InviteFlag, NULL);
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2pReceInviteReqAction ==>InviteFlag = %d.  %s \n",
	...
	DBGPRINT(RT_DEBUG_ERROR, ("OpChannel = %d.    \n", OpChannel));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("BssidAddr = %02x:%02x:%02x:%02x:%02x:%02x  \n",
		  PRINT_MAC(BssidAddr)));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("DevAddr = %02x:%02x:%02x:%02x:%02x:%02x  \n",
		  PRINT_MAC(DevAddr)));
	...
	{
		p2pindex =
		    P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_DISCOVERY_GO, Ssid,
				      SsidLen, DeviceCap, GroupCap);
	}
	index = P2pPerstTabSearch(pAd, DevAddr, BssidAddr, InfAddr);
	DBGPRINT(RT_DEBUG_ERROR,
		 ("perst index = %d. p2pindex = %d. \n", index, p2pindex));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("My Dpid = %d.    Dpid = %s.\n", pP2PCtrl->Dpid,
	...
	if (P2P_GO_ON(pAd)) {
		...
		if ((pP2PCtrl->p2pMaxEntry != 0)
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("P2pReceInviteReqAction ==>P2PGetEntryCnt = %d\n",
				  P2PEntryCnt));
			RspStatus = P2PSTATUS_UNABLE;
			P2PMakeInviteRsp(pAd, MyRule, pFrame->Token,
					 pFrame->p80211Header.Addr2,
					 pP2PCtrl->CurrentAddress, &OpChannel,
					 &RspStatus, &FrameLen);
			return;
		}
	}
	if ((index < MAX_P2P_TABLE_SIZE)
	    && ((InviteFlag & P2P_INVITE_FLAG_REINVOKE) ==
		P2P_INVITE_FLAG_REINVOKE))
		bReinvoke = TRUE;
	...
	if (bReinvoke == TRUE) {
		...
		P2pCheckInviteReq(pAd,
				  (pAd->P2pTable.PerstEntry[index].MyRule ==
				   P2P_IS_GO), index, ChannelList, BssidAddr,
				  OpChannel, Ssid, SsidLen, &RspStatus);
		if (RspStatus == P2PSTATUS_SUCCESS) {
			...
			if (pAd->P2pTable.PerstEntry[index].MyRule == P2P_IS_GO) {
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invite: Decide to use OpChannel = %d for group \n",
					  OpChannel));
				p2pindex =
				    P2pGroupTabInsert(pAd, DevAddr,
						      P2PSTATE_CLIENT_WPS, Ssid,
						      SsidLen, DeviceCap,
						      GroupCap);
				if (p2pindex < MAX_P2P_GROUP_SIZE) {
					pAd->P2pTable.Client[p2pindex].
					    P2pClientState =
					    P2PSTATE_WAIT_REVOKEINVITE_RSP_ACK;
				} else {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("Invite: reach my limit. Send back RspStatus = %d.  \n",
						  RspStatus));
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invite: Reinvoke as client for group \n"));
				pP2PCtrl->P2PConnectState =
				    P2P_ANY_IN_FORMATION_AS_CLIENT;
				/* Update the Client state and SSID. */
				P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_GO_WPS,
						  Ssid, SsidLen, DeviceCap,
						  GroupCap);
				...
				COPY_MAC_ADDR(pAd->ApCfg.ApCliTab[0].
					      CfgApCliBssid, pP2PCtrl->Bssid);
			}
		}
	}
	/* case 2: abNormal invitation cases:...
	 */
	else if ((InviteFlag & P2P_INVITE_FLAG_REINVOKE) ==
		 P2P_INVITE_FLAG_REINVOKE) {
	 ...
	 */
		...
		if (RTMPEqualMemory(BssidAddr, ZERO_MAC_ADDR, MAC_ADDR_LEN)) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Invite Req carry allzero Bssid, \n"));
		} else if (IS_P2P_CONNECT_IDLE(pAd)
			   || (pAd->P2pCfg.P2PConnectState == P2P_INVITE)) {
			P2pCheckInviteReqFromExisting(pAd, ChannelList, DevAddr,
						      OpChannel, Ssid, SsidLen,
						      &RspStatus);
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Invite: Got invitation from P2P Client . RspStatus = %d \n",
				  RspStatus));
			if (RspStatus == P2PSTATUS_SUCCESS) {
				RTMPZeroMemory(pP2PCtrl->SSID, MAX_LEN_OF_SSID);
				RTMPMoveMemory(pP2PCtrl->SSID, Ssid, SsidLen);
				RTMPZeroMemory(pAd->P2pTable.Client[p2pindex].
					       Ssid, MAX_LEN_OF_SSID);
				RTMPMoveMemory(pAd->P2pTable.Client[p2pindex].
					       Ssid, Ssid, SsidLen);
				pAd->P2pTable.Client[p2pindex].SsidLen =
				    SsidLen;
				COPY_MAC_ADDR(pP2PCtrl->Bssid, BssidAddr);
				COPY_MAC_ADDR(pAd->P2pTable.Client[p2pindex].
					      bssid, BssidAddr);
				COPY_MAC_ADDR(pAd->P2pTable.Client[p2pindex].
					      InterfaceAddr, BssidAddr);
				pP2PCtrl->SSIDLen = SsidLen;
				pP2PCtrl->GroupOpChannel = OpChannel;
				pAd->P2pTable.Client[p2pindex].OpChannel =
				    OpChannel;
				pAd->P2pTable.Client[p2pindex].ConfigMethod =
				    ConfigMethod;
				MyRule = P2P_IS_CLIENT;
			}
		}
...
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2p Send Invite Rsp RspStatus = %d.  \n", RspStatus));
	P2PMakeInviteRsp(pAd, MyRule, pFrame->Token, pFrame->p80211Header.Addr2,
			 pP2PCtrl->CurrentAddress, &OpChannel, &RspStatus,
			 &FrameLen);
	...
	if (((bReinvoke == FALSE) && (MyRule == P2P_IS_CLIENT)) &&
		...
		if (MyRule == P2P_IS_CLIENT) {
			...
			NdisMoveMemory(pP2PCtrl->Bssid,
				       pAd->P2pTable.PerstEntry[index].Addr,
				       MAC_ADDR_LEN);
			WscWriteConfToApCliCfg(pAd, pWscControl,
					       &pAd->P2pTable.PerstEntry[index].
					       Profile, TRUE);
			pObj->ioctl_if_type = INT_P2P;

			if ((pAd->CommonCfg.Channel != OpChannel)
			    && (!INFRA_ON(pAd))) {
...
/*	
	...
	==========================================================================
 */
VOID P2pReceInviteRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	UCHAR DevType[8], devAddr[6], BssidAddr[6] = { 0 }
	, OpChannel, index, StatusCode;
	UCHAR GroupCap, DeviceCap, DeviceNameLen;
	USHORT ConfigMethod;
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, NULL,
			&GroupCap, &DeviceCap, NULL, &DeviceNameLen, devAddr,
			NULL, BssidAddr, &SsidLen, NULL, &ConfigMethod, NULL,
			DevType, NULL, &OpChannel, ChannelList, NULL,
			&StatusCode, NULL, NULL);
	...
	if ((StatusCode == P2PSTATUS_SUCCESS)
	    || (StatusCode == P2PSTATUS_PASSED)) {
		index =
		    P2pPerstTabSearch(pAd, pFrame->p80211Header.Addr2, NULL,
				      NULL);
		if ((index < MAX_P2P_TABLE_SIZE)
			...
			if (pAd->P2pTable.PerstEntry[index].MyRule == P2P_IS_GO) {
				...
				P2pCopyPerstParmToCfg(pAd, index);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Decide to use OpChannel = %d for group \n",
					  OpChannel));
				P2pGroupTabInsert(pAd,
						  pFrame->p80211Header.Addr2,
						  P2PSTATE_CLIENT_WPS, NULL,
						  SsidLen, 0, 0);
				pP2pEntry = &pAd->P2pTable.Client[index];
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Reinvoke as client for group. OpChannel= %d\n",
					  OpChannel));
				pP2PCtrl->P2PConnectState = P2P_DO_WPS_ENROLLEE;
			...
			index =
			    P2pGroupTabSearch(pAd, pFrame->p80211Header.Addr2);
			if (index < MAX_P2P_GROUP_SIZE) {
				if (P2P_GO_ON(pAd)) {
					pP2pEntry =
					    &pAd->P2pTable.Client[index];
					...
					DBGPRINT(RT_DEBUG_ERROR,
						 ("P2pReceInviteRspAction Success.  I am GO. \n"));
				} else if (P2P_CLI_ON(pAd))
					DBGPRINT(RT_DEBUG_ERROR,
						 ("P2pReceInviteRspAction Success.  I am client. \n"));
...
/*	
	...
	==========================================================================
 */
VOID P2pReceProvisionReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PRT_P2P_CLIENT_ENTRY pP2pEntry = NULL;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	...
	ULONG FrameLen;
	UCHAR p2pindex;
	UCHAR GroupCap, DeviceCap, DevAddr[MAC_ADDR_LEN],
	    BssidAddr[MAC_ADDR_LEN], OpChannel;
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID,
			(Elem->MsgLen - LENGTH_802_11 - 8), FALSE, NULL,
			&GroupCap, &DeviceCap, DeviceName, &DeviceNameLen,
			DevAddr, NULL, BssidAddr, &SsidLen, Ssid, NULL,
			&WpsConfigMethod, DevType, NULL, &OpChannel, NULL, NULL,
			NULL, NULL, NULL);
	...
	if (P2P_GO_ON(pAd)) {
		...
		if ((pP2PCtrl->p2pMaxEntry != 0)
		    && (P2PEntryCnt >= pP2PCtrl->p2pMaxEntry)) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("P2pRece Provision ReqAction ==>P2PGetEntryCnt = %d\n",
				  P2PEntryCnt));
			P2PSendProvisionRsp(pAd, 0, pFrame->Token,
					    pFrame->p80211Header.Addr2,
					    &FrameLen);
			return;
		}
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2pRece Provision ReqAction ==>ConfigMethod = %x %s\n",
	...
	if (WpsConfigMethod == WSC_CONFMET_PBC
		...
		if (WpsConfigMethod == WSC_CONFMET_PBC) {
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("P2pRece Provision ReqAction ==> Ignore this provision request here\n"));
			return;
		}
	}
	...
	if ((WpsConfigMethod == WSC_CONFMET_DISPLAY)
		...
		if (p2pindex < MAX_P2P_GROUP_SIZE) {
			...
			if (DeviceNameLen != 0) {
				RTMPMoveMemory(&pAd->P2pTable.Client[p2pindex].
					       DeviceName[0], DeviceName, 32);
				pAd->P2pTable.Client[p2pindex].DeviceNameLen =
				    DeviceNameLen;
				pAd->P2pTable.Client[p2pindex].DeviceName[pAd->
									  P2pTable.
									  Client
									  [p2pindex].
									  DeviceNameLen]
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Update My Config Method to  %s (%d)\n",
			...
			if (pP2PCtrl->bProvAutoRsp == TRUE) {
				...
				P2P_SET_FLAG(&pAd->P2pTable.Client[p2pindex],
					     P2PFLAG_PROVISIONED);
				P2PSendProvisionRsp(pAd, WpsConfigMethod,
						    pFrame->Token,
						    pFrame->p80211Header.Addr2,
						    &FrameLen);
				if ((P2P_GO_ON(pAd))
					...
					pP2pEntry->P2pClientState =
					    P2PSTATE_GO_WPS;
					RTMPMoveMemory(pP2PCtrl->Bssid,
						       pAd->P2pTable.
						       Client[p2pindex].bssid,
						       MAC_ADDR_LEN);
		...
		P2PSendProvisionRsp(pAd, 0, pFrame->Token,
				    pFrame->p80211Header.Addr2, &FrameLen);
}
...
/*	
	...
	==========================================================================
 */
VOID P2pReceProvisionRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PRT_P2P_CLIENT_ENTRY pP2pEntry;
	PP2P_PUBLIC_FRAME pFrame = (PP2P_PUBLIC_FRAME) Elem->Msg;
	...
	if (p2pindex >= MAX_P2P_GROUP_SIZE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("P2pReceProvisionRspAction from unknown device ==> \n"));
		return;
	}
	...
	pP2pEntry->bValid = FALSE;
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2pReceProvisionRspAction from P2P_Tab[%d] = %02x:%02x:%02x:%02x:%02x:%02x ==> \n",
	...
	if (RTMPEqualMemory(Ptr, WPS_OUI, 4)) {
		/* point to Element ID */
		/*Ptr -= 2; */
		P2PParseWPSIE(Ptr, (pFrame->Length + 2), NULL, &ConfigMethod,
			      NULL, NULL);
	}

	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2pRece Provision RspAction ==>ConfigMethod = %04x. %s.\n",
		  ConfigMethod, decodeConfigMethod(ConfigMethod)));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("pAd->P2pCfg.Dpid = %d. %s.\n", pAd->P2pCfg.Dpid,
		  decodeDpid(pAd->P2pCfg.Dpid)));
	...
	if ((ConfigMethod == WSC_CONFMET_DISPLAY)
		...
		if ((ConfigMethod == WSC_CONFMET_DISPLAY)
		    && (pP2pEntry->Rule != P2P_IS_GO)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Need setting first. Pop up setting windows. %d\n",
				  pAd->P2pCfg.PopUpIndex));
			pP2pEntry->P2pClientState = P2PSTATE_DISCOVERY;
		}

		DBGPRINT(RT_DEBUG_ERROR,
			 ("Peer %d ConfigMethod = %s \n", p2pindex,
			  decodeConfigMethod(ConfigMethod)));
		if (pP2pEntry->Rule == P2P_IS_GO) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Decide to Join P2p group? when I am %s \n",
				  decodeP2PState(pAd->P2pCfg.P2PConnectState)));
			if (pAd->P2pCfg.P2PConnectState == P2P_CONNECT_IDLE) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("P2p :connecting to GO with Bssid   %02x:%02x:%02x:%02x:%02x:%02x. \n",
			...
			if (P2P_GO_ON(pAd)) {
				...
				P2pSetWps(pAd, pP2pEntry);
				DBGPRINT(RT_DEBUG_ERROR,
					 (" P2P - I am Internal REGISTRA!!    REGISTRA. !! GOGOGO\n"));
			} else {
				if ((pP2PCtrl->ConfigMethod ==
				     WSC_CONFMET_KEYPAD)
				    ||
				    ((pP2PCtrl->ConfigMethod ==
				      WSC_CONFMET_DISPLAY)
				     && bAutoGroupFormation == FALSE)) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("%s:: recv Provision Rsp. Condif Method = %04x.\n",
						  __FUNCTION__, ConfigMethod));
				} else {
					/* After Provision Success, start connect command */
					pAddr =
					    (PUCHAR) (&pAd->P2pTable.
						      Client[p2pindex].addr);
					P2pConnectPrepare(pAd, pAddr,
							  P2PSTATE_CONNECT_COMMAND);
				}
			}
		}
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
		P2pSendWirelessEvent(pAd, RT_P2P_RECV_PROV_RSP,
				     &pAd->P2pTable.Client[p2pindex],
				     pFrame->p80211Header.Addr2);
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
...
/*	
	...
	==========================================================================
 */
VOID P2pSendProbeReq(IN PRTMP_ADAPTER pAd, IN UCHAR Channel)
{
	PUCHAR pOutBuffer;
	ULONG FrameLen;
	NDIS_STATUS NStatus;
	...
	if (Elem) {
		...
		P2PMakeProbe(pAd, Elem, Channel, SUBTYPE_PROBE_REQ, pOutBuffer,
			     &FrameLen);
		if (FrameLen > 0)
			MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
		MlmeFreeMemory(pAd, pOutBuffer);
...
/*	
	...
	==========================================================================
 */
VOID P2PSendGoNegoConfirm(IN PRTMP_ADAPTER pAd,
			  IN UCHAR Token, IN UCHAR idx, IN PUCHAR Addr1)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	ULONG FrameLen;
	ULONG TempLen;
	...
	StatusCode = 0;
	TempLen =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_STATUS, &StatusCode, pDest);
	pDest += TempLen;
	FrameLen += TempLen;
	P2pLen += TempLen;
	/* :  .
	 */
	TempLen =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CAP, pP2PCtrl->P2pCapability,
				pDest);
	FrameLen += TempLen;
	pDest += TempLen;
	P2pLen += TempLen;
	...
	if (pP2pEntry->Rule == P2P_IS_CLIENT) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s:: P2P - I become Internal REGISTRA!!	  REGISTRA. !! GOGOGO\n",
			  __FUNCTION__));
		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OP_CHANNEL, &Channel,
					pDest);
		FrameLen += TempLen;
		pDest += TempLen;
		P2pLen += TempLen;
		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CHANNEL_LIST, &Channel,
					pDest);
		FrameLen += TempLen;
		pDest += TempLen;
		P2pLen += TempLen;
		...
		pP2PCtrl->SSIDLen = pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen;
		{
			...
			tmpGroupID[0] =
			    6 + pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen;
			tmpGroupID[1] = 0;
			RTMPMoveMemory(&tmpGroupID[2], pP2PCtrl->CurrentAddress,
				       6);
			RTMPMoveMemory(&tmpGroupID[8],
				       &pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid,
				       pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen);
			TempLen =
			    InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID,
						(PUCHAR) (&tmpGroupID), pDest);
		}
		FrameLen += TempLen;
		pDest += TempLen;
		P2pLen += TempLen;
		...
		P2pLen += TempLen;
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s:: P2P - I become ENROLLEE!!	ENROLLEE.!!\n",
			  __FUNCTION__));
/*
...
/*	
	...
	==========================================================================
 */
VOID P2PSendDevDisReq(IN PRTMP_ADAPTER pAd,
		      IN PUCHAR Addr1,
		      IN PUCHAR Bssid,
		      IN PUCHAR ClientAddr1, OUT PULONG pTotalFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame;
	ULONG P2pIeLen = 0;
	...
	*pTotalFrameLen = 0;
	...
	pFrame = (PP2P_PUBLIC_FRAME) pOutBuffer;
	Ptr = (PUCHAR) pFrame;
	ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
		      pP2PCtrl->CurrentAddress, Bssid);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2P - P2PSendDevDisReq  TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
		  PRINT_MAC(Addr1)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2P - P2PSendDevDisReq  Bssid %02x:%02x:%02x:%02x:%02x:%02x. \n",
		  PRINT_MAC(Bssid)));
	...
	*pTotalFrameLen = 38;
	...
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_DEVICE_ID, ClientAddr1, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("P2P - P2PSendDevice Discovery Req  . TO %02x:%02x:%02x:%02x:%02x:%02x.  *pTotalFrameLen = %ld. \n",
...
/*	
	...
	==========================================================================
 */
VOID P2PSendDevDisRsp(IN PRTMP_ADAPTER pAd,
		      IN UCHAR RspStatus,
		      IN UCHAR Token,
		      IN PUCHAR Addr1, OUT PULONG pTotalFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame;
	ULONG P2pIeLen = 0;
	...
	*pTotalFrameLen = 0;
	...
	Ptr = (PUCHAR) pFrame;
	ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
		      pP2PCtrl->CurrentAddress, pP2PCtrl->CurrentAddress);
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PSendDevDisRsp TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
		  PRINT_MAC(Addr1)));
	pFrame->Category = CATEGORY_PUBLIC;
	pFrame->Action = ACTION_WIFI_DIRECT;
	/* point to OUI */
	...
	*pTotalFrameLen += Length;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PSendDevice Discovery Response. TO %02x:%02x:%02x:%02x:%02x:%02x.   \n",
		  PRINT_MAC(Addr1)));
...
/*	
	...
	==========================================================================
 */
VOID P2PSendProvisionReq(IN PRTMP_ADAPTER pAd,
			 ...
			 IN PUCHAR Addr1, OUT PULONG pTotalFrameLen)
{
	...
	*pTotalFrameLen = 0;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PMakeProvisionReq  . TO %02x:%02x:%02x:%02x:%02x:%02x.  ConfigMethod = %s \n",
		  PRINT_MAC(Addr1), decodeConfigMethod(ConfigMethod)));
	pFrame = (PP2P_PUBLIC_FRAME) pOutBuffer;
	Ptr = (PUCHAR) pFrame;
	ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
		      pP2PCtrl->CurrentAddress, Addr1);
	pFrame->Category = CATEGORY_PUBLIC;
	pFrame->Action = ACTION_WIFI_DIRECT;
	/* point to OUI */
	Ptr += (sizeof(HEADER_802_11) + 2);
	...
	*pTotalFrameLen += Length;
	...
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_DEVICE_INFO,
				pP2PCtrl->CurrentAddress, pDest);
	pDest += Length;
	WpsIeLen += Length;
	...
	*pTotalFrameLen += Length;
	...
	if (index != P2P_NOT_FOUND) {
		...
		pP2pEntry = &pAd->P2pTable.Client[index];
		DBGPRINT(RT_DEBUG_TRACE,
			 ("P2P - P2PMakeProvisionReq   MyRule=%s, PeerRule=%d \n",
		...
		if (pP2pEntry->Rule == P2P_IS_GO)	// peer is GO
		{
			...
			RTMPMoveMemory(&tmpGroupID[8], &pP2pEntry->Ssid[0],
				       pP2pEntry->SsidLen);
			Length =
			    InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID,
						(PUCHAR) (&tmpGroupID), pDest);
...
/*	
	...
	==========================================================================
 */
VOID P2PSendProvisionRsp(IN PRTMP_ADAPTER pAd,
			 IN USHORT ConfigMethod,
			 IN UCHAR Token,
			 IN PUCHAR Addr1, OUT PULONG pTotalFrameLen)
{
	...
	*pTotalFrameLen = 0;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PMakeProvisionRsp  . TO %02x:%02x:%02x:%02x:%02x:%02x.  ConfigMethod = %x(%s) \n",
		  PRINT_MAC(Addr1), ConfigMethod,
		  decodeConfigMethod(ConfigMethod)));
	pFrame = (PP2P_PUBLIC_FRAME) pOutBuffer;
	Ptr = (PUCHAR) pFrame;
	ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeGoNegoConfirm(IN PRTMP_ADAPTER pAd,
			  IN PUCHAR Addr1,
			  IN UCHAR Token,
			  IN PUCHAR pOutBuffer, OUT PULONG pTotalFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	...
	ActHeaderInit(pAd, (PHEADER_802_11) pOutBuffer, Addr1,
		      pP2PCtrl->CurrentAddress, pP2PCtrl->PortCfg.Bssid);
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeGoNegoRsp(IN PRTMP_ADAPTER pAd,
		      IN PUCHAR Addr1,
		      IN USHORT SentDpid,
		      IN UCHAR Token,
		      IN UCHAR TempIntent,
		      ...
		      IN PUCHAR pOutBuffer, OUT PULONG pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	RealIntent =
	    (pP2PCtrl->GoIntentIdx << 1) + ((TempIntent & 0x1) == 1 ? 0 : 1);

	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OWNER_INTENT, &RealIntent,
				pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/* attach subelementID= 4. */
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CONFIG_TIMEOUT,
				&pP2PCtrl->ConfigTimeout[0], pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	if (INFRA_ON(pAd))
		Length =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OP_CHANNEL,
					&pAd->CommonCfg.Channel, pDest);
	else
		Length =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OP_CHANNEL,
					&pP2PCtrl->GroupChannel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/*  attach subelementID= 9. */
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_INTERFACE_ADDR,
				pP2PCtrl->CurrentAddress, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CHANNEL_LIST,
				&pP2PCtrl->GroupChannel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeGoNegoReq(IN PRTMP_ADAPTER pAd,
		      IN UCHAR index,
		      IN PUCHAR Addr1,
		      IN PUCHAR pOutBuffer, OUT PULONG pTotalFrameLen)
{
	...
	*pTotalFrameLen = 0;
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 (" %s - P2PMakeGoNegoReq  TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
		  __FUNCTION__, Addr1[0], Addr1[1], Addr1[2], Addr1[3],
		  Addr1[4], Addr1[5]));
	...
	*pTotalFrameLen += Length;
	...
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OWNER_INTENT, &FilledIntent,
				pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	*pTotalFrameLen += Length;
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_LISTEN_CHANNEL,
				&pP2PCtrl->ListenChannel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	*pTotalFrameLen += Length;
	...
	if (IS_P2P_SUPPORT_EXT_LISTEN(pAd)) {
		/*  attach subelementID= 17. */
		Length =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_EXT_LISTEN_TIMING, NULL,
					pDest);
		pDest += Length;
		P2pIeLen += Length;
		*pTotalFrameLen += Length;
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeInviteReq(IN PRTMP_ADAPTER pAd,
		      IN UCHAR MyRule,
		      IN UCHAR InviteFlag,
		      IN PUCHAR Addr1,
		      IN PUCHAR Bssid, OUT PULONG pTotalFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame;
	ULONG P2pIeLen = 0;
	...
	*pTotalFrameLen = 0;
	...
	ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
		      pP2PCtrl->CurrentAddress, Bssid);
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PMakeInviteReq  TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
		  PRINT_MAC(Addr1)));
	pFrame->Category = CATEGORY_PUBLIC;
	pFrame->Action = ACTION_WIFI_DIRECT;
	/* point to OUI */
	...
	*pTotalFrameLen += Length;
	...
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_INVITE_FLAG, &InviteFlag, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/* attach subelementID= 17. */
	...
	/* attach subelementID= 7. */
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_BSSID,
				&pP2PCtrl->CurrentAddress[0], pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/* attach subelementID= 11. */
	...
	*pTotalFrameLen += Length;
	/* attach subelementID= 11. */
	{
		...
		tmpGroupID[1] = 0;
		RTMPMoveMemory(&tmpGroupID[2], pP2PCtrl->CurrentAddress, 6);
		RTMPMoveMemory(&tmpGroupID[8], &pP2PCtrl->SSID[0],
			       pP2PCtrl->SSIDLen);
		Length =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID,
					(PUCHAR) (&tmpGroupID), pDest);
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeInviteRsp(IN PRTMP_ADAPTER pAd,
		      IN UCHAR MyRule,
		      IN UCHAR Token,
		      IN PUCHAR Addr1,
		      IN PUCHAR Bssid,
		      IN PUCHAR OpChannel,
		      IN PUCHAR Status, OUT PULONG pTotalFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME pFrame;
	ULONG P2pIeLen = 0;
	...
	*pTotalFrameLen = 0;
	...
	if (MyRule == P2P_IS_CLIENT)
		...
		ActHeaderInit(pAd, &pFrame->p80211Header, Addr1,
			      pP2PCtrl->CurrentAddress, Bssid);
	DBGPRINT(RT_DEBUG_ERROR,
		 ("P2P - P2PMakeInviteRsp MyRule = %s. TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
	...
	*pTotalFrameLen += Length;
	...
	if ((MyRule == P2P_IS_GO) && (*Status == P2PSTATUS_SUCCESS)) {
		Length =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_BSSID, Bssid,
					pDest);
		pDest += Length;
		P2pIeLen += Length;
		*pTotalFrameLen += Length;
	...
	/* attach subelementID= 7. */
	Length =
	    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CHANNEL_LIST, OpChannel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	/* attach subelementID= 17. */
	*pTotalFrameLen += Length;
...
/*	
	...
	==========================================================================
 */
BOOLEAN PeerP2pProbeReqSanity(IN PRTMP_ADAPTER pAd,
			      IN VOID * Msg,
			      IN ULONG MsgLen,
			      OUT PUCHAR pAddr2,
			      OUT CHAR Ssid[],
			      ...
			      OUT ULONG * WpsIELen, OUT PUCHAR pWpsIE)
{
	...
	*Peerip = 0;
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_VENDOR_SPECIFIC:
			...
			if (NdisEqualMemory(pEid->Octet, WPS_OUI, 4)
			    && (pEid->Len > 4)) {
				RTMPMoveMemory(pWpsIE, &pEid->Eid,
					       pEid->Len + 2);
				*WpsIELen = pEid->Len + 2;
			}
			if (NdisEqualMemory(pEid->Octet, P2POUIBYTE, 4)
			    && (pEid->Len > 4)) {
				...
				DBGPRINT(RT_DEBUG_INFO,
					 (" !FIND!!!!!!===>P2P - PeerP2pProbeReq P2P IE Len = %ld.   %s\n",
					  *P2PSubelementLen,
					  decodeP2PState(pP2PCtrl->
							 P2PConnectState)));
			}
			break;
		}
	...
	if (RateLen <= 4) {
		brc = FALSE;
		DBGPRINT(RT_DEBUG_INFO,
			 ("Ignore Probe Request that is 11b-only.\n"));
	}
	/* Check P2P's Probe Response validatity. */
	if (*P2PSubelementLen <= 4) {
		brc = FALSE;
		DBGPRINT(RT_DEBUG_INFO,
			 ("Ignore Probe Request that doesn't have P2P IE.\n"));
	}
...
/*	
	...
	==========================================================================
 */
BOOLEAN PeerP2pBeaconSanity(IN PRTMP_ADAPTER pAd,
			    IN VOID * Msg,
			    IN ULONG MsgLen,
			    OUT PUCHAR pAddr2,
			    OUT CHAR Ssid[],
	...
	*Peerip = 0;
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_VENDOR_SPECIFIC:
			...
			} else if (NdisEqualMemory(pEid->Octet, P2POUIBYTE, 4)
				   && (pEid->Len >= 4)) {
				/*
...
/*	
	...
	==========================================================================
 */
BOOLEAN PeerP2pProbeRspSanity(IN PRTMP_ADAPTER pAd,
			      IN VOID * Msg,
			      IN ULONG MsgLen,
			      OUT PUCHAR pAddr2,
			      OUT CHAR Ssid[],
		...
		*(pP2PIeConLen + 1) = (UCHAR) (P2PIeConLen / 256);
		...
		for (idx = 0; idx < (*P2PSubelementLen);) {
			DBGPRINT_RAW(RT_DEBUG_INFO,
				      ...
				      *(pP2pSubelement + idx),
				      *(pP2pSubelement + idx + 1),
				      *(pP2pSubelement + idx + 2),
				      *(pP2pSubelement + idx + 3)
				      , *(pP2pSubelement + idx + 4),
				      *(pP2pSubelement + idx + 5),
				      *(pP2pSubelement + idx + 6),
				      *(pP2pSubelement + idx + 7)
				      , *(pP2pSubelement + idx + 8),
				      *(pP2pSubelement + idx + 9),
				      *(pP2pSubelement + idx + 10),
				      *(pP2pSubelement + idx + 11)));
...
VOID P2pPeerBeaconAtJoinAction(IN PRTMP_ADAPTER pAd,
			       IN MLME_QUEUE_ELEM * Elem, IN PUCHAR Bssid)
{
	/*PRT_P2P_CONFIG        pP2PCtrl = &pAd->P2pCfg; */
	...
	P2pParseManageSubElmt(pAd,
			      ...
			      &Channel,
			      &NumOfOtherP2pAttri,
			      &TotalNumOfP2pAttribute, &P2pManageability, NULL);
	...
	if (P2pManageability != 0xff) {
		DBGPRINT(RT_DEBUG_INFO,
			 ("SYNC - Receive desired BEACON with valid P2pManageability= %x\n",
			  P2pManageability));
		DBGPRINT(RT_DEBUG_INFO,
			 ("SYNC -NumOfOtherP2pAttri %d . Channel = %d. \n",
			  NumOfOtherP2pAttri, Channel));
		RTMPMoveMemory(pAd->P2pCfg.P2pManagedParm.ManageAPBSsid, Bssid,
			       MAC_ADDR_LEN);
		...
		if ((P2P_TEST_BIT(P2pManageability, P2PMANAGED_ENABLE_BIT))
		    && (NumOfOtherP2pAttri == 0)) {
			DBGPRINT(RT_DEBUG_INFO,
	...
	pAd->P2pCfg.P2pManagedParm.TotalNumOfP2pAttribute =
	    TotalNumOfP2pAttribute;

	DBGPRINT(RT_DEBUG_INFO,
		 ("SYNC - BEACON P2pManageability= %x. TotalNumOfP2pAttribute = %d. OtherP2pAttri = %d\n",
		  P2pManageability, TotalNumOfP2pAttribute,
		  NumOfOtherP2pAttri));
...
/*	
	...
	==========================================================================
 */
VOID PeerP2pBeaconProbeRspAtScan(IN PRTMP_ADAPTER pAd,
...
/*	IN UCHAR	WorkingChannel) */
{
	...
	if (PeerP2pProbeRspSanity(pAd,
		...
		if (!(NdisEqualMemory(Ssid, &WILDP2PSSID[0], WILDP2PSSIDLEN))) {
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("= %c %c %c %c %c %c%c \n", Ssid[0], Ssid[1],
				  Ssid[2], Ssid[3], Ssid[4], Ssid[5], Ssid[6]));
			goto CleanUp;
		} else
			DBGPRINT(RT_DEBUG_INFO,
		...
		if (bBeacon == FALSE) {
			P2pParseSubElmt(pAd, (PVOID) P2pSubelement,
					P2PSubelementLen, bBeacon, &Dpid,
					&GroupCap, &DeviceCap, DeviceName,
					&DeviceNameLen, DevAddr, NULL, NULL,
					NULL, NULL, &ConfigMethod, NULL,
					DevType, &Channel, &OpChannel, NULL,
					&Intent, &StatusCode, NULL, NULL);
		} else
			P2pParseSubElmt(pAd, (PVOID) P2pSubelement,
					P2PSubelementLen, bBeacon, &Dpid,
					&GroupCap, &DeviceCap, DeviceName,
					&DeviceNameLen, DevAddr, NULL, NULL,
					NULL, NULL, NULL, NULL, DevType,
					&Channel, &OpChannel, NULL, &Intent,
					&StatusCode, NULL, NULL);
		...
		if (index == P2P_NOT_FOUND) {
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("2 New From Addr2 = %02x:%02x:%02x:%02x:%02x:%02x. bBeacon = %d . GroupCap = %x \n",
				  PRINT_MAC(Addr2), bBeacon, GroupCap));
			DBGPRINT(RT_DEBUG_ERROR,
				 ("2 New From DevAddr = %02x:%02x:%02x:%02x:%02x:%02x. bBeacon = %d . GroupCap = %x \n",
		...
		if (index < MAX_P2P_GROUP_SIZE) {
			...
			if ((bBeacon == TRUE) &&
				...
				if ((pAd->P2pTable.ClientNumber < 10)
				    || (((pAd->P2pTable.ClientNumber % 4) == 3)
					&& (pAd->P2pTable.ClientNumber >= 10)))
					bChangeState = TRUE;
				pP2pEntry->Rule = P2P_IS_GO;
				pP2pEntry->P2pClientState =
				    P2PSTATE_DISCOVERY_GO;
				DBGPRINT(RT_DEBUG_ERROR,
					 ("3 From DevAddr = %02x:%02x:%02x:%02x:%02x:%02x.	Change rule to GO GroupCap = %x \n",
					  PRINT_MAC(DevAddr), GroupCap));
			} else
			    if ((pP2pEntry->P2pClientState ==
				 P2PSTATE_DISCOVERY_GO)
				&& ((GroupCap & GRPCAP_OWNER) != GRPCAP_OWNER)) {
				if ((pAd->P2pTable.ClientNumber < 10)
				    || (((pAd->P2pTable.ClientNumber % 4) == 3)
				...
				pP2pEntry->P2pClientState = P2PSTATE_DISCOVERY;
				DBGPRINT(RT_DEBUG_ERROR,
					 ("4 From DevAddr = %02x:%02x:%02x:%02x:%02x:%02x.	Change rule to Device. GroupCap = %x \n",
					  PRINT_MAC(DevAddr), GroupCap));
			}
			...
			if (DeviceNameLen != 0) {
				RTMPMoveMemory(&pP2pEntry->DeviceName[0],
					       DeviceName, 32);
			...
			if (bBeacon == TRUE) {
				...
				RTMPMoveMemory(pP2pEntry->Ssid, Ssid, 32);
				pP2pEntry->SsidLen = SsidLen;
				RTMPMoveMemory(pP2pEntry->bssid, Addr2,
					       MAC_ADDR_LEN);
				RTMPMoveMemory(pP2pEntry->InterfaceAddr, Addr2,
					       MAC_ADDR_LEN);
				...
				if ((GroupCap & GRPCAP_OWNER) != GRPCAP_OWNER) {
					/* Print for debug. */
					DBGPRINT(RT_DEBUG_ERROR,
						 ("P2p : One P2P device[%d] send out Beacon. But group owner %x bit not set ? \n",
						  index, GroupCap));
				}
			}
			...
			if ((pP2pEntry->P2pClientState <= P2PSTATE_GO_DONE)) {
				if ((DeviceCap & DEVCAP_INVITE) ==
				    DEVCAP_INVITE)
					P2P_SET_FLAG(pP2pEntry,
						     P2PFLAG_INVITE_ENABLED);
				...
				if (pP2pEntry->P2pClientState !=
					...
					DBGPRINT(RT_DEBUG_INFO,
						 ("P2p : DevAddr[%02x:%02x:%02x:%02x:%02x:%02x] State = %s.\n",
						  PRINT_MAC(DevAddr),
						  decodeP2PClientState
						  (pP2pEntry->P2pClientState)));
					DBGPRINT(RT_DEBUG_INFO,
						 ("P2p : GrpCap=%x. DevCap=%x. ConfigMethod= %x.\n",
						  GroupCap, DeviceCap,
						  pP2pEntry->ConfigMethod));

					MlmeEnqueue(pAd,
						    P2P_GO_FORM_STATE_MACHINE,
						    P2P_START_COMMUNICATE_CMD_EVT,
						    sizeof(P2P_CMD_STRUCT),
						    &P2pCmd, ClientState);
					RTMP_MLME_HANDLER(pAd);
					goto CleanUp;
				}
				if (pAd->P2pCfg.ConnectingIndex <
				    MAX_P2P_GROUP_SIZE) {
					if (MAC_ADDR_EQUAL
					    (pAd->P2pTable.Client[index].addr,
					     &pAd->P2pCfg.ConnectingMAC[0])) {
						DBGPRINT(RT_DEBUG_INFO,
							 ("    From : %02x:%02x:%02x:%02x:%02x:%02x, state = %s\n",
							  ...
							  (pP2pEntry->
							   P2pClientState)));
						DBGPRINT(RT_DEBUG_INFO,
							 ("    P2PTab[%d] Addr : %02x:%02x:%02x:%02x:%02x:%02x\n",
							  index,
							  PRINT_MAC(pP2pEntry->
								    addr)));
						DBGPRINT(RT_DEBUG_INFO,
							 ("    ConenctIdx = %d, Addr : %02x:%02x:%02x:%02x:%02x:%02x\n",
							  pAd->P2pCfg.
							  ConnectingIndex,
							  PRINT_MAC(pAd->P2pCfg.
								    ConnectingMAC)));

						P2pConnectAfterScan(pAd,
								    bBeacon,
								    index);
					}
				}
...
/*	
	...
	==========================================================================
 */
VOID PeerP2pBeacon(IN PRTMP_ADAPTER pAd,
		   IN PUCHAR pAddr2,
		   IN MLME_QUEUE_ELEM * Elem, IN LARGE_INTEGER TimeStamp)
{
	...
	if (!(IS_P2P_CLI_ENTRY(pMacEntry) && IS_ENTRY_APCLI(pMacEntry))) {
		DBGPRINT(RT_DEBUG_INFO,
			 ("1 PeerP2pBeaconProbeRspAtScan failed :  wcid = %d. not ValidAsP2P. Bug!please check. \n",
			  Elem->Wcid));
		DBGPRINT(RT_DEBUG_INFO,
			 ("pMacEntry Addr = %02x:%02x:%02x:%02x:%02x:%02x.\n",
	...
	if (P2pSubelement == NULL) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("3 PeerP2pBeaconProbeRspAtScan::1Allocate memory size(=1024) failed\n"));
		goto CleanUp;
	}
	...
	if (PeerP2pBeaconSanity(pAd,
				...
				Ssid,
				&SsidLen,
				&Peerip, &P2PSubelementLen, P2pSubelement)) {
		/* Parse the power managemenr parameters in here. */
		pP2PCtrl->GONoASchedule.LastBeaconTimeStamp =
		    TimeStamp.u.LowPart;
		P2pParseNoASubElmt(pAd, P2pSubelement, P2PSubelementLen,
				   Elem->Wcid, pFrame->Hdr.Sequence);
		...
		if (P2P_TEST_BIT(pAd->P2pCfg.CTWindows, P2P_OPPS_BIT)) {
			...
			RTMPDeQueuePacket(pAd, FALSE, NUM_OF_TX_RING,
					  MAX_TX_PROCESS);
			if (((pAd->P2pCfg.CTWindows & 0x7f) > 0)
			    && ((pAd->P2pCfg.CTWindows & 0x7f) < 80)) {
				DBGPRINT(RT_DEBUG_INFO,
					 ("%s::  set P2P CTWindows timer.\n",
					  __FUNCTION__));
				RTMPSetTimer(&pAd->P2pCfg.P2pCTWindowTimer,
					     (pAd->P2pCfg.CTWindows & 0x7f));
			}
		}
	}
...
/*	
	...
	==========================================================================
 */
BOOLEAN PeerBeaconParseRalinkIE(IN PRTMP_ADAPTER pAd,
				IN VOID * Msg,
				IN ULONG MsgLen,
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_VENDOR_SPECIFIC:
			if (NdisEqualMemory(pEid->Octet, RALINK_OUI, 3)
			    && (pEid->Len == 5)) {
				RTMPMoveMemory(pRalinkIE, &pEid->Eid,
					       sizeof(RALINKIP_IE));
				brc = TRUE;
...
/*	
	...
	==========================================================================
 */
VOID PeerP2pProbeReq(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
/*	IN BOOLEAN	bSendRsp)  */
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
...
//QQ TBD, p2p widi need insert p2p entry in probe req. due to p2p0 only in listen state
		...
		if (index < MAX_P2P_GROUP_SIZE) {
			DBGPRINT(RT_DEBUG_INFO, ("%s / %s\n",
						 decodeP2PClientState(pAd->
								      P2pTable.
								      Client
								      [index].
								      P2pClientState),
						 decodeP2PState(pAd->P2pCfg.
								P2PConnectState)));
			if ((pAd->P2pTable.Client[index].P2pClientState >
			     P2PSTATE_DISCOVERY_UNKNOWN)
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("(%d) P2P Peer Probe Req from %02x %02x %02x %02x %02x %02x	\n",
		...
		if (bSendRsp == TRUE) {
			...
			P2PMakeProbe(pAd, Elem, Channel, SUBTYPE_PROBE_RSP,
				     pOutBuffer, &FrameLen);
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("Got P2P Peer %d Probe Req . Send probe response back  len=%ld \n",
				  index, FrameLen));
			MlmeFreeMemory(pAd, pOutBuffer);
		}
		...
		if (index < MAX_P2P_GROUP_SIZE) {
			P2pParseSubElmt(pAd, P2pSubelement, P2PSubelementLen,
					FALSE, NULL, &GroupCap, &DeviceCap,
					NULL, NULL, DevAddr, NULL, NULL,
					&SsidLen, Ssid, NULL, NULL,
					pAd->P2pTable.Client[index].
					PrimaryDevType,
					&pAd->P2pTable.Client[index].
					ListenChannel,
					&pAd->P2pTable.Client[index].OpChannel,
					NULL,
					&pAd->P2pTable.Client[index].GoIntent,
					&StatusCode, NULL, NULL);
			if (pAd->P2pTable.Client[index].P2pClientState ==
			    P2PSTATE_CONNECT_COMMAND) {
				P2P_CMD_STRUCT P2pCmd;
...
/*	
	...
	==========================================================================
 */
VOID P2pReceiveEapNack(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM pElem)
{
}

VOID P2pMakeProbeRspWSCIE(IN PRTMP_ADAPTER pAd,
			  OUT PUCHAR pOutBuf, OUT PULONG pIeLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	...
	*(pData + 4) =
	    (pAd->P2pCfg.
	     bConfiguredAP ? WSC_SCSTATE_CONFIGURED : WSC_SCSTATE_UNCONFIGURED);
	pData += 5;
	Len += 5;
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(0x0010);
	...
	// 5. Manufacture
	templen =
	    AppendWSCTLV(WSC_ID_MANUFACTURER, pData, pAd->P2pCfg.Manufacturer,
			 pAd->P2pCfg.ManufacturerLen);
	pData += templen;
	Len += templen;
	...
	/* 6. Model Name */
	templen =
	    AppendWSCTLV(WSC_ID_MODEL_NAME, pData, pAd->P2pCfg.ModelName,
			 pAd->P2pCfg.ModelNameLen);
	pData += templen;
	Len += templen;

	/* 7. Model Number */
	templen =
	    AppendWSCTLV(WSC_ID_MODEL_NUMBER, pData, pAd->P2pCfg.ModelNumber,
			 pAd->P2pCfg.ModelNumberLen);
	pData += templen;
	Len += templen;

	/* 8. Serial Number */
	templen =
	    AppendWSCTLV(WSC_ID_SERIAL_NUM, pData, pAd->P2pCfg.SerialNumber,
			 pAd->P2pCfg.SerialNumberLen);
	pData += templen;
	Len += templen;
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(pAd->P2pCfg.DeviceNameLen);
	NdisMoveMemory((pData + 4), pAd->P2pCfg.DeviceName,
	...
	*((PUSHORT) (pData + 4)) = cpu2be16(0x0188);
	 ...
	 */
	{
		...
		*(pData + 4) = pDevInfo->RfBand;
		...
		if ((pAd->ApCfg.MBSSID[BSS0].WscControl.WscSelReg)
		    && (pAd->P2pCfg.Dpid == DEV_PASS_ID_PBC)) {
			if (P2P_GO_ON(pAd)) {
				/* 14. Selected Registrar */
				*((PUSHORT) pData) =
				    cpu2be16(WSC_ID_SEL_REGISTRAR);
				*((PUSHORT) (pData + 2)) = cpu2be16(0x0001);
				...
				*((PUSHORT) pData) =
				    cpu2be16(WSC_ID_SEL_REG_CFG_METHODS);
				*((PUSHORT) (pData + 2)) = cpu2be16(0x0002);
...
/*
	...
	========================================================================
*/
VOID P2pMakeProbeReqIE(IN PRTMP_ADAPTER pAd,
		       OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen)
{
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(0x0001);
	*(pData + 4) =
	    ((pAd->StaCfg.WscControl.WscConfMode ==
	      WSC_REGISTRAR) ? WSC_MSGTYPE_AP_WLAN_MGR :
	     WSC_MSGTYPE_ENROLLEE_OPEN_8021X);
	pData += 5;
	Len += 5;
	...
	*((PUSHORT) pData) =
	    ((pAd->StaCfg.WscControl.WscConfMode ==
	      WSC_REGISTRAR) ? cpu2be16(WSC_ID_UUID_R) :
	     cpu2be16(WSC_ID_UUID_E));
	*((PUSHORT) (pData + 2)) = cpu2be16(0x0010);
	NdisMoveMemory((pData + 4), pDevInfo->Uuid, 16);
	pData += 20;
	Len += 20;
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(pP2PCtrl->DeviceNameLen);
	NdisMoveMemory((pData + 4), pP2PCtrl->DeviceName,
		       pP2PCtrl->DeviceNameLen);
...
/*	
	...
	==========================================================================
 */
VOID P2PMakeProbe(IN PRTMP_ADAPTER pAd,
		  IN MLME_QUEUE_ELEM * Elem,
		  IN UCHAR DsChannel,
		  IN USHORT SubType, OUT PUCHAR pDest, OUT ULONG * pFrameLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	...
	if (SubType == SUBTYPE_PROBE_RSP) {
		...
		MgtMacHeaderInit(pAd, &ProbeRspHdr, SUBTYPE_PROBE_REQ, 0,
				 BROADCAST_ADDR,
#ifdef P2P_SUPPORT
				 pP2PCtrl->CurrentAddress,
#endif				/* P2P_SUPPORT */
	...
	NdisMoveMemory(tmpSupRate, pAd->CommonCfg.SupRate,
	...
	if (DsChannel == 0)
		DBGPRINT(RT_DEBUG_ERROR,
			 ("SYNC - P2PMakeProbeRsp. DsChannel = is 0 !!!!!\n"));
	...
	if (SubType == SUBTYPE_PROBE_RSP) {
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  ...
				  1, &SupRateIe,
				  1, &SupRateLen,
				  SupRateLen, tmpSupRate, END_OF_ARGS);
	} else {
		SsidLen = WILDP2PSSIDLEN;
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  sizeof(HEADER_802_11), &ProbeRspHdr,
	...
	if ((DsChannel > 0) && (DsChannel <= 14)) {
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
				  1, &DsIe,
				  1, &DsLen, 1, &DsChannel, END_OF_ARGS);
		FrameLen += TmpLen;
	}
...
/*	
	...
	==========================================================================
 */
VOID P2pMakeP2pIE(IN PRTMP_ADAPTER pAd,
		  IN UCHAR PacketType, OUT PUCHAR pOutBuf, OUT PULONG pIeLen)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	...
	*pIeLen = 0;
	...
	if (PacketType == SUBTYPE_ASSOC_RSP) {
		...
		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_STATUS, &Status, pData);
		Len += TempLen;
		pData += TempLen;

		if (IS_P2P_SUPPORT_EXT_LISTEN(pAd)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("P2pMakeP2pIE (PacketType = %d)  insert SUBID_P2P_EXT_LISTEN_TIMING  .\n",
				  PacketType));
			TempLen =
			    InsertP2PSubelmtTlv(pAd,
						SUBID_P2P_EXT_LISTEN_TIMING,
						NULL, pData);
			Len += TempLen;
			pData += TempLen;
		}
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("<----- P2pMakeP2pIE For managed STA. (Len = %ld) \n",
			  Len));
		*(pOutBuf + 1) = (Len - 2);
		*pIeLen = Len;
		return;
	...
	if ((pP2PCtrl->P2pManagedParm.TotalNumOfP2pAttribute > 0) &&
		...
		if ((!P2P_TEST_BIT(P2pCapability[1], GRPCAP_CROSS_CONNECT))
		    && RTMPEqualMemory(pP2PCtrl->Bssid,
				       pAd->P2pCfg.P2pManagedParm.ManageAPBSsid,
				       MAC_ADDR_LEN)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("pAd->P2pCfg.P2pManagedParm.APP2pManageability = %x \n",
				  pAd->P2pCfg.P2pManagedParm.
				  APP2pManageability));
			...
			if ((pAd->P2pCfg.P2pManagedParm.APP2pManageability !=
			     0xff)
			    &&
			    (P2P_TEST_BIT
			     (pAd->P2pCfg.P2pManagedParm.APP2pManageability,
			      P2PMANAGED_ENABLE_BIT))
			    &&
			    (!P2P_TEST_BIT
			     (pAd->P2pCfg.P2pManagedParm.APP2pManageability,
			      P2PMANAGED_ICS_ENABLE_BIT))) {
				P2pCapability[1] &= (~GRPCAP_CROSS_CONNECT);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Turn off Corss Conenct bit in Assoc Req. %x \n",
					  (~GRPCAP_CROSS_CONNECT)));
			}
		}

		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_CAP, P2pCapability,
					pData);
		Len += TempLen;
		pData += TempLen;
		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_INTERFACE,
					pP2PCtrl->CurrentAddress, pData);
		Len += TempLen;
		pData += TempLen;
		/* this is managed infra STA connect to Infra AP. So Add P2P Interface. then this's all. return here. */
		DBGPRINT(RT_DEBUG_ERROR,
			 ("<-----P2pMakeP2pIE For managed STA. (Len = %ld PacketType = %d.) \n",
			  Len, PacketType));
		*(pOutBuf + 1) = (Len - 2);
		*pIeLen = Len;
		return;
	...
	if (PacketType == SUBTYPE_PROBE_REQ) {
		...
		TempLen =
		    InsertP2PSubelmtTlv(pAd, SUBID_P2P_LISTEN_CHANNEL, pBuf,
					pData);
		Len += TempLen;
		pData += TempLen;
		...
		if (pP2PCtrl->Rule == P2P_IS_GO) {
			...
			TempLen =
			    InsertP2PSubelmtTlv(pAd, SUBID_P2P_OP_CHANNEL, pBuf,
						pData);
			Len += TempLen;
			pData += TempLen;
		}
	...
	if ((PacketType == SUBTYPE_PROBE_RSP)
	    || (PacketType == SUBTYPE_ASSOC_REQ)
	    || (PacketType == SUBTYPE_REASSOC_REQ)) {
		if (PacketType == SUBTYPE_ASSOC_REQ) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("	P2pMakeP2pIE (PacketType = %d)	insert SUBID_P2P_DEVICE_INFO. DeviceNameLen = %ld.\n",
...
/*	
	...
	==========================================================================
 */
VOID P2PParseWPSIE(IN PUCHAR pWpsData,
		   IN USHORT WpsLen,
		   OUT PUSHORT Dpid,
		   OUT PUSHORT ConfigMethod,
		   OUT PUCHAR DeviceName, OUT UCHAR * DeviceNameLen)
{
	USHORT Length;
	PUCHAR pData;
	USHORT WscType, WscLen;
			...
			if (DeviceName != NULL) {
				...
				DBGPRINT(RT_DEBUG_INFO,
					 ("%s :  DeviceName = %c%c%c%c%c ...\n",
					  __FUNCTION__, DeviceName[0],
					  DeviceName[1], DeviceName[2],
					  DeviceName[3], DeviceName[4]));
			}
			break;
...
/*	
	...
	==========================================================================
 */
VOID GoPeerDisassocReq(IN PRTMP_ADAPTER pAd, IN PUCHAR Addr2)
{
...
/*	
	...
	==========================================================================
 */
ULONG P2pUpdateGroupBeacon(IN PRTMP_ADAPTER pAd, IN ULONG StartPosition)
{
...
/*	
	...
	==========================================================================
 */
ULONG P2pUpdateNoABeacon(IN PRTMP_ADAPTER pAd, IN UCHAR apidx,
			 ...
			 IN PUCHAR pDest)
{
	...
	if (pP2PCtrl->GONoASchedule.bValid == TRUE) {
		...
		*(pDest + 11) = pP2PCtrl->GONoASchedule.Count;
		/* Duration */
		RTMPMoveMemory((pDest + 12), &pP2PCtrl->GONoASchedule.Duration,
			       4);
		/* Interval */
		RTMPMoveMemory((pDest + 16), &pP2PCtrl->GONoASchedule.Interval,
			       4);
		RTMPMoveMemory((pDest + 20), &pP2PCtrl->GONoASchedule.StartTime,
			       4);
...
/*	
	...
	==========================================================================
 */
ULONG P2pUpdateNoAProbeRsp(IN PRTMP_ADAPTER pAd, IN PUCHAR pInbuffer)
{
	PUCHAR pDest;
	...
	if (pAd->P2pCfg.GONoASchedule.bValid == TRUE) {
		...
		*(pDest + 11) = pAd->P2pCfg.GONoASchedule.Count;
		/* Duration */
		RTMPMoveMemory((pDest + 12),
			       &pAd->P2pCfg.GONoASchedule.Duration, 4);
		/* Interval */
		RTMPMoveMemory((pDest + 16),
			       &pAd->P2pCfg.GONoASchedule.Interval, 4);
		RTMPMoveMemory((pDest + 20),
			       &pAd->P2pCfg.GONoASchedule.StartTime, 4);
...
/*	
	...
	==========================================================================
 */
VOID P2pUpdateBssBeacon(IN PRTMP_ADAPTER pAd,
			IN PUCHAR pCapability, IN PUCHAR pIpReqP2ptabindex)
{
}

VOID GOUpdateBeaconFrame(IN PRTMP_ADAPTER pAd)
{
...
/*	
	...
	==========================================================================
 */
VOID P2pMakeBssBeacon(IN PRTMP_ADAPTER pAd)
{
}
...
*/
VOID P2pCheckInviteReq(IN PRTMP_ADAPTER pAd,
		       IN BOOLEAN bIAmGO,
		       IN UCHAR index,
		       IN PUCHAR ChannelList,
		       IN PUCHAR BssidAddr,
		       IN UCHAR OpChannel,
		       IN PUCHAR Ssid, IN UCHAR SsidLen, IN UCHAR * pRspStatus)
{
	UCHAR i;
	*pRspStatus = P2PSTATUS_SUCCESS;
	...
	if (!RTMPEqualMemory
	    (BssidAddr, pAd->P2pTable.PerstEntry[index].Addr, MAC_ADDR_LEN)
	    && (bIAmGO == FALSE)) {
	...
	if ((SsidLen > 0)
	    &&
	    (!RTMPEqualMemory
	     (pAd->P2pTable.PerstEntry[index].Profile.SSID.Ssid, Ssid,
	      SsidLen))) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Ssid1[%d] = %s.  \n", SsidLen, Ssid));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Ssid2[%d] = %s.  \n",
			  pAd->P2pTable.PerstEntry[index].Profile.SSID.
			  SsidLength,
...
*/
VOID P2pCheckInviteReqFromExisting(IN PRTMP_ADAPTER pAd,
				   IN PUCHAR ChannelList,
				   IN PUCHAR BssidAddr,
				   IN UCHAR OpChannel,
				   IN PUCHAR Ssid,
				   IN UCHAR SsidLen, IN UCHAR * pRspStatus)
{
	UCHAR i;
	*pRspStatus = P2PSTATUS_SUCCESS;
	/* Check if have Common Channels. */
	if (FALSE == P2pCheckChannelList(pAd, ChannelList)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 (" P2P - no common channel. Channel list is : %d...\n",
			  ChannelList[0]));
		*pRspStatus = P2PSTATUS_NO_CHANNEL;
		return;
	}
...
UCHAR ChannelToClass(IN UCHAR Channel, IN UCHAR Country)
{
	UCHAR ReturnClass = 1;
