...
VOID Adhoc_RTMPGetTxTscFromAsic(IN PRTMP_ADAPTER pAd,
	...
	if (pEntry) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 (" PortSecured(%d), WpaState(%d), AuthMode(%d), PMKID_CacheIdx(%d) \n",
			  pEntry->PortSecured, pAuthenticator->WpaState,
	...
	*((USHORT *) & EapolKeyInfo) = cpu2le16(*((USHORT *) & EapolKeyInfo));
	...
	if (MsgType != EAPOL_PAIR_MSG_1) {
		...
		if (EapolKeyInfo.KeyDescVer == KEY_DESC_TKIP) {	/* TKIP */
			RT_HMAC_MD5(p4WayProfile->PTK, LEN_PTK_KCK,
				    (PUCHAR) pMsg, eapol_len, mic,
				    MD5_DIGEST_SIZE);
		} else if (EapolKeyInfo.KeyDescVer == KEY_DESC_AES) {	/* AES */
			RT_HMAC_SHA1(p4WayProfile->PTK, LEN_PTK_KCK,
				     (PUCHAR) pMsg, eapol_len, digest,
				     SHA1_DIGEST_SIZE);
			...
			if ((peerKeyInfo.KeyAck == 1)
				...
				if ((peerKeyInfo.Secure == 0)
					...
					if (peerKeyInfo.KeyMic == 0) {
						Adhoc_PeerPairMsg1Action(pAd,
									 pEntry,
									 Elem);
					} else {
						Adhoc_PeerPairMsg3Action(pAd,
									 pEntry,
									 Elem);
					}
					...
					if (peerKeyInfo.KeyType == PAIRWISEKEY) {
						...
						Adhoc_PeerGroupMsg1Action(pAd,
									  pEntry,
									  Elem);
					}
				}
			} else if (pEntry->WPA_Authenticator.WpaState >=
				   AS_INITPSK) {
					...
					if (peerKeyInfo.Secure == 0
						...
						if (CONV_ARRARY_TO_UINT16
						    (pEapol_packet->KeyDesc.
						     KeyDataLen) == 0) {
							Adhoc_PeerPairMsg4Action
							    (pAd, pEntry, Elem);
						} else {
							Adhoc_PeerPairMsg2Action
							    (pAd, pEntry, Elem);
						}
					} else if (peerKeyInfo.Secure == 1
						   && peerKeyInfo.KeyType ==
...
/*
    ...
    ==========================================================================
*/
VOID Adhoc_WpaStart4WayHS(IN PRTMP_ADAPTER pAd,
			  IN MAC_TABLE_ENTRY * pEntry, IN ULONG TimeInterval)
{
	...
	if (pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK) {
		...
		RT_HMAC_SHA1(pAd->StaCfg.PMK, PMK_LEN, PMK_key, 20, digest,
			     LEN_PMKID);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("<=== Adhoc_WpaStart4WayHS: send Msg1 of 4-way \n"));
			      ...
			      IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (ADHOC_ON(pAd)
	    && (pEntry->WPA_Supplicant.WpaState == AS_PTKINITDONE)) {
	...
	if (Adhoc_PeerWpaMessageSanity
	    (pAd, pMsg1, MsgLen, EAPOL_PAIR_MSG_1, pSupplicant,
	     pEntry) == FALSE)
		return;
	...
	/* Store ANonce */
	NdisMoveMemory(pSupplicant->ANonce, pMsg1->KeyDesc.KeyNonce,
		       LEN_KEY_DESC_NONCE);
	...
	WpaDerivePTK(pAd,
		     ...
		     pSupplicant->ANonce,
		     pEntry->Addr,
		     pSupplicant->SNonce, pCurrentAddr, PTK, LEN_PTK);
	...
	Adhoc_ConstructEapolMsg(pEntry, group_cipher, EAPOL_PAIR_MSG_2, 0,	/* DefaultKeyIdx */
				...
				NULL,	/* GTK */
				(UCHAR *) rsnie_ptr,
				rsnie_len, pSupplicant, pEapolFrame);
			      ...
			      IN MLME_QUEUE_ELEM * Elem)
{
	...
	/* Store SNonce */
	NdisMoveMemory(pAuthenticator->SNonce, pMsg2->KeyDesc.KeyNonce,
		       LEN_KEY_DESC_NONCE);
	...
	if (Adhoc_PeerWpaMessageSanity
	    (pAd, pMsg2, MsgLen, EAPOL_PAIR_MSG_2, pAuthenticator,
	     pEntry) == FALSE)
		return;
	...
	do {
		...
		RTMPSetTimer(&pAuthenticator->MsgRetryTimer,
			      ...
			      IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Adhoc_PeerWpaMessageSanity
	    (pAd, pMsg3, MsgLen, EAPOL_PAIR_MSG_3, pSupplicant,
	     pEntry) == FALSE)
		return;
	...
	/* Double check ANonce */
	if (!NdisEqualMemory
	    (pSupplicant->ANonce, pMsg3->KeyDesc.KeyNonce,
	     LEN_KEY_DESC_NONCE)) {
		return;
	}
			      ...
			      IN MLME_QUEUE_ELEM * Elem)
{
	...
	do {
		...
		if (Adhoc_PeerWpaMessageSanity
		    (pAd, pMsg4, MsgLen, EAPOL_PAIR_MSG_4, pAuthenticator,
		     pEntry) == FALSE)
			break;
			       ...
			       IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Adhoc_PeerWpaMessageSanity
	    (pAd, pGroup, MsgLen, EAPOL_GROUP_MSG_1, pSupplicant,
	     pEntry) == FALSE)
		return;

	/* Save Replay counter, it will use to construct message 2 */
	NdisMoveMemory(pSupplicant->ReplayCounter,
		       pGroup->KeyDesc.ReplayCounter, LEN_KEY_DESC_REPLAY);
...
VOID Adhoc_Wpa4WayComplete(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	compare_address =
	    NdisCmpMemory(pAd->CurrentAddress, pEntry->Addr, MAC_ADDR_LEN);
	if ((compare_address > 0)
	    && (pAuthenticator->WpaState == AS_PTKINITDONE)) {
		NdisMoveMemory(pEntry->PTK, pAuthenticator->PTK, 64);
	} else if ((compare_address < 0)
		   && (pSupplicant->WpaState == AS_PTKINITDONE)) {
	...
	if (pEntry) {
		...
		pWscPeerEntry =
		    WscFindPeerEntry(&pWpsCtrl->WscConfiguredPeerList,
				     pEntry->Addr);
		if (pWscPeerEntry) {
			WscDelListEntryByMAC(&pWpsCtrl->WscConfiguredPeerList,
					     pEntry->Addr);
		}
			     ...
			     OUT PEAPOL_PACKET pMsg)
{
	...
	*(USHORT *) (pKeyInfo) = cpu2le16(*(USHORT *) (pKeyInfo));
	...
	NdisMoveMemory(pKeyDesc->ReplayCounter, p4WayProfile->ReplayCounter,
		       LEN_KEY_DESC_REPLAY);
	...
	if ((MsgType <= EAPOL_PAIR_MSG_3)
	    || ((!bWPA2 && (MsgType == EAPOL_GROUP_MSG_1))))
		NdisMoveMemory(pKeyDesc->KeyNonce, KeyNonce,
			       LEN_KEY_DESC_NONCE);
	...
	Adhoc_ConstructEapolKeyData(pEntry,
				    ...
				    MsgType,
				    DefaultKeyIdx,
				    GTK, RSNIE, RSNIE_Len, p4WayProfile, pMsg);
