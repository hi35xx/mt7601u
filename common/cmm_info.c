		...
		if ((WMODE_CAP_5G(pAd->CommonCfg.PhyMode))
			...
			for (i = 0; i < pAd->ChannelListNum; i++) {
				if (pAd->ChannelList[i].Channel == Channel) {
					if (pAd->ChannelList[i].
					    RemainingTimeForUse > 0) {
						DBGPRINT(RT_DEBUG_ERROR,
...
INT Set_WscGenPinCode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pObj->ioctl_if_type == INT_P2P) {
		...
		pApWscControl->WscEnrolleePinCodeLen =
		    pWscControl->WscEnrolleePinCodeLen;
		pApWscControl->WscEnrolleePinCode =
		    pWscControl->WscEnrolleePinCode;
	}
#endif				/* P2P_SUPPORT */
...
/*	UCHAR       apidx = pObj->ioctl_if;*/
	...
	if (pObj->ioctl_if_type == INT_P2P) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_WscVendorPinCode_Proc() for p2p(%d)\n", apidx));
		return TRUE;
	}
#endif				/* P2P_SUPPORT */
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				/*if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[ifIndex].SharedKey[i].BssId, pKey->BSSID)) */
				{
					...
					pAd->ApCfg.ApCliTab[ifIndex].
					    SharedKey[i].KeyLen = 0;
					pAd->ApCfg.ApCliTab[ifIndex].
					    SharedKey[i].CipherAlg =
					    CIPHER_NONE;
			       ...
			       IN UCHAR CipherAlg, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (pEntry && (IS_ENTRY_WDS(pEntry) || IS_ENTRY_MESH(pEntry)))
			WCIDAttri =
			    (BssIdx << 4) | (CipherAlg << 1) | PAIRWISEKEYTABLE;
		else if ((pEntry)
			...
			WCIDAttri =
			    (BssIdx << 4) | (CipherAlg << 1) | SHAREDKEYTABLE;
	}
#endif				/* CONFIG_AP_SUPPORT */
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC)) {
			...
			sprintf(msg + strlen(msg), "%-6d",
				RateIdToMbps[pAd->MacTab.Content[i].
					     CurrTxRate]);
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		pAd->ApCfg.MBSSID[pObj->ioctl_if].DesiredTransmitSetting.field.
		    MCS = HtMcs;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_HtMcs_Proc::(HtMcs=%d) for ra%d\n",
			  pAd->ApCfg.MBSSID[pObj->ioctl_if].
			  DesiredTransmitSetting.field.MCS, pObj->ioctl_if));
	}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
...
INT Set_HtRxStream_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((Value <= 3) && (Value >= 1)
	    && (Value <= pAd->Antenna.field.RxPath))
...
INT Show_HtMcs_Proc(IN PRTMP_ADAPTER pAd, OUT PSTRING pBuf, IN ULONG BufLen)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	    snprintf(pBuf, BufLen, "\t%u",
		     pAd->ApCfg.MBSSID[pObj->ioctl_if].DesiredTransmitSetting.
		     field.MCS);
#endif				/* CONFIG_AP_SUPPORT */
...
#ifdef WSC_STA_SUPPORT
INT Show_WpsPbcBand_Proc(IN PRTMP_ADAPTER pAd,
			 OUT PSTRING pBuf, IN ULONG BufLen)
{
	...
	default:
		snprintf(pBuf, BufLen, "\tUnknow Value(%d)",
			 pAd->StaCfg.WscControl.WpsApBand);
		break;
	}
	return 0;
}
...
INT Show_MacTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
...
#endif				/* !DPA_S */
		{
			...
			printk("\t\t\t\t%-10s",
			       get_phymode_str(pEntry->MaxHTPhyMode.field.
					       MODE));
			printk("%-6s",
...
#if defined(DPA_S) && defined(P2P_SUPPORT)
			printk("%-32s%\n",
			       bFound ? &pAd->P2pTable.Client[index].
			       DeviceName[0] : "N/A");
#endif				/* DPA_S */
			printk("\n");
		}
...
#ifdef WSC_STA_SUPPORT
INT Show_WpsManufacturer_Proc(IN PRTMP_ADAPTER pAd,
			      OUT PSTRING pBuf, IN ULONG BufLen)
{
...
INT Show_WpsModelName_Proc(IN PRTMP_ADAPTER pAd,
			   OUT PSTRING pBuf, IN ULONG BufLen)
{
...
INT Show_WpsDeviceName_Proc(IN PRTMP_ADAPTER pAd,
			    OUT PSTRING pBuf, IN ULONG BufLen)
{
...
INT Show_WpsModelNumber_Proc(IN PRTMP_ADAPTER pAd,
			     OUT PSTRING pBuf, IN ULONG BufLen)
{
...
INT Show_WpsSerialNumber_Proc(IN PRTMP_ADAPTER pAd,
			      OUT PSTRING pBuf, IN ULONG BufLen)
{
...
INT Set_ReadITxBf_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	maxCarriers =
	    profData.
	    fortyMHz ? PROFILE_MAX_CARRIERS_40 : PROFILE_MAX_CARRIERS_20;
...
INT Set_FalseCCA_THR1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT16 Value;
...
INT Set_FalseCCA_THR2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT16 Value;
...
INT Set_BW20_RSSI_THR0_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_20 Rssi Thr0=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw20RssiThr0));
	return TRUE;
}

INT Set_BW20_RSSI_THR1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_20 Rssi Thr1=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw20RssiThr1));
	return TRUE;
}

INT Set_BW20_RSSI_THR2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_20 Rssi Thr2=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw20RssiThr2));
	return TRUE;
}

INT Set_BW20_RSSI_THR3_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_20 Rssi Thr3=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw20RssiThr3));
	return TRUE;
}

INT Set_BW40_RSSI_THR0_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_40 Rssi Thr0=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw40RssiThr0));
	return TRUE;
}

INT Set_BW40_RSSI_THR1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_40 Rssi Thr1=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw40RssiThr1));
	return TRUE;
}

INT Set_BW40_RSSI_THR2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_40 Rssi Thr2=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw40RssiThr2));
	return TRUE;
}

INT Set_BW40_RSSI_THR3_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(BW_40 Rssi Thr3=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.Bw40RssiThr3));
	return TRUE;
}

INT Set_RSSI_NOISE_THR1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(Rssi Thr1=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.NoiseRssiThr1));
	return TRUE;
}

INT Set_RSSI_NOISE_THR2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF %s::(Rssi Thr2=%d)\n", __FUNCTION__,
		  pAd->CommonCfg.NoiseRssiThr2));
	return TRUE;
}

INT Set_RSSI_NOISE_THR3_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT16 Value;
...
*/
#ifdef DYNAMIC_PD_SUPPORT
INT Set_Dynamic_PD_Enable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT16 Value;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF Set_Dynamic_PD_Enable_Proc::(Enable =%d)\n",
		     ...
		     IN PSTRING pName, IN PSTRING pBuf, IN UINT32 MaxLen)
{
	...
	for (PRTMP_PRIVATE_STA_SHOW_CFG_VALUE_PROC =
		...
		if (!strcmp(pName, PRTMP_PRIVATE_STA_SHOW_CFG_VALUE_PROC->name)) {
			if (PRTMP_PRIVATE_STA_SHOW_CFG_VALUE_PROC->
			    show_proc(pAd, pBuf, MaxLen))
				Status = -EINVAL;
			break;	/*Exit for loop. */
		}
	}
