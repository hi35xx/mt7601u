...
VOID WscDelWPARetryTimer(IN PRTMP_ADAPTER pAd);
...
BOOLEAN WscCheckNonce(IN PRTMP_ADAPTER pAdapter,
		      IN MLME_QUEUE_ELEM * Elem,
		      IN BOOLEAN bFlag, IN PWSC_CTRL pWscControl);

VOID WscEapActionDisabled(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl);

VOID WscGetConfigErrFromNack(IN PRTMP_ADAPTER pAdapter,
			     IN MLME_QUEUE_ELEM * pElem,
			     OUT USHORT * pConfigError);

INT WscSetAuthMode(IN PRTMP_ADAPTER pAd,
		   IN UCHAR CurOpMode, IN UCHAR apidx, IN PSTRING arg);

INT WscSetEncrypType(IN PRTMP_ADAPTER pAd,
		     IN UCHAR CurOpMode, IN UCHAR apidx, IN PSTRING arg);

VOID WscSendNACK(IN PRTMP_ADAPTER pAdapter,
		 IN MAC_TABLE_ENTRY * pEntry, IN PWSC_CTRL pWscControl);

static INT wsc_write_dat_file_thread(IN ULONG data);

#ifdef CONFIG_STA_SUPPORT
VOID WscLinkDown(IN PRTMP_ADAPTER pAd);
#endif				/* CONFIG_STA_SUPPORT */

VOID WscDelListEntryByMAC(PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr);

NDIS_802_11_AUTHENTICATION_MODE WscGetAuthMode(IN USHORT authFlag);

NDIS_802_11_WEP_STATUS WscGetWepStatus(IN USHORT encryFlag);
		    ...
		    UCHAR * uuidAscStr, int apIdx, BOOLEAN bUseCurrentTime)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (apIdx >= MIN_NET_DEVICE_FOR_P2P_GO)
			NdisMoveMemory(&uuid_t.node[0],
				       &pAd->ApCfg.MBSSID[MAIN_MBSSID].Bssid[0],
				       sizeof(uuid_t.node));
		else
#endif				/* P2P_SUPPORT */
			NdisMoveMemory(&uuid_t.node[0], &pAd->CurrentAddress[0],
				       sizeof(uuid_t.node));
	}
#endif				/* CONFIG_STA_SUPPORT */
...
VOID WscEAPOLStartAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Rejected by IWSC SM. Ignore EAPOL-Start.\n"));
			return;
		}
#endif				// IWSC_SUPPORT //
	...
	if (pWpsCtrl->WscPeerList.size != 0) {
		...
		if (NdisEqualMemory
		    (pEntry->Addr, pWscPeer->mac_addr, MAC_ADDR_LEN) == FALSE) {
			DBGPRINT(RT_DEBUG_TRACE,
	...
	if ((pEntry->Receive_EapolStart_EapRspId == 0) ||
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("WscEAPOLStartAction(ra%d) - send EAP-Req(Id) to %02x:%02x:%02x:%02x:%02x:%02x\n",
			  pEntry->apidx, pEntry->Addr[0], pEntry->Addr[1],
	...
	if (CurOpMode == AP_MODE) {
		if (!bUPnPMsg) {
			...
			if (pEntry && IS_ENTRY_APCLI(pEntry))
				pWscControl =
				    &pAdapter->ApCfg.ApCliTab[apidx].WscControl;
			else
#endif				/* APCLI_SUPPORT */
				pWscControl =
				    &pAdapter->ApCfg.MBSSID[apidx].WscControl;
		} else {
			int i;
			for (i = 0; i < MAX_MBSSID_NUM(pAdapter); i++) {
				if (NdisEqualMemory
				    (pAdapter->ApCfg.MBSSID[i].Bssid, MacAddr,
				     MAC_ADDR_LEN)) {
					apidx = i;
					break;
				}
	...
	if (MsgType == WSC_MSG_EAP_REG_RSP_ID) {
		...
		if (bUPnPMsg) {
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE)
				...
				if ((pAdapter->CommonCfg.WscStaPbcProbeInfo.
				     WscPBCStaProbeCount == 1)
				    && !NdisEqualMemory(pAdapter->CommonCfg.
							WscStaPbcProbeInfo.
							StaMacAddr[0],
							&ZERO_MAC_ADDR[0],
							MAC_ADDR_LEN)
				    &&
				    (NdisEqualMemory
				     (pAdapter->CommonCfg.WscStaPbcProbeInfo.
				      StaMacAddr[0], &pWscControl->EntryAddr[0],
				      6) == FALSE)) {
					pAdapter->CommonCfg.WscPBCOverlap =
					    TRUE;
				}
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE) &&
				...
				RTMPSendWirelessEvent(pAdapter,
						      IW_WSC_PBC_SESSION_OVERLAP,
						      NULL,
						      (pWscControl->
						       EntryIfIdx & 0x0F), 0);
				DBGPRINT(RT_DEBUG_TRACE,
...
#endif				/* CONFIG_AP_SUPPORT */
			if ((pWscControl->
				     WscConfMode & WSC_PROXY_REGISTRAR) != 0) {
...
#ifdef CONFIG_AP_SUPPORT
				if (CurOpMode == AP_MODE) {
					WscSendUPnPConfReqMsg(pAdapter,
							      (pWscControl->
							       EntryIfIdx &
							       0x0F),
							      (PUCHAR)
		...
		if (pWscControl->WscState >= WSC_STATE_WAIT_EAPFAIL) {
...
#ifdef P2P_SUPPORT
				if (pWscControl->WscStatus ==
				    STATUS_WSC_CONFIGURED)
					pAdapter->P2pCfg.WscState =
					    WSC_STATE_CONFIGURED;
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
				P2pSendWirelessEvent(pAdapter,
						     RT_P2P_WPS_COMPLETED, NULL,
						     NULL);
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
#endif				/* P2P_SUPPORT */
				/* Bring apcli interface down first */
				if (pEntry && IS_ENTRY_APCLI(pEntry)
				    && pAdapter->ApCfg.ApCliTab[BSS0].Enable ==
				    TRUE) {
#ifdef P2P_SUPPORT
					UCHAR P2pIdx =
					    P2pGroupTabSearch(pAdapter,
							      pEntry->Addr);
#endif				/* P2P_SUPPORT */
					pAdapter->ApCfg.ApCliTab[pEntry->apidx].
					    Enable = FALSE;
			...
			if (CurOpMode == STA_MODE) {
				...
				if ((pAdapter->OpMode == OPMODE_STA) &&
				    ...
				    (pAdapter->StaCfg.WscControl.WscConfMode ==
				     WSC_ENROLLEE))
					pAdapter->StaCfg.IWscInfo.bReStart =
					    TRUE;
#endif				/* IWSC_SUPPORT */
				pWscControl->WscConfMode = WSC_DISABLE;
			...
			if (pAdapter->StaCfg.BssType == BSS_ADHOC) {
				...
				WscSendEapolStart(pAdapter,
						  pWscControl->WscPeerMAC,
						  STA_MODE);
			}
#endif				/* IWSC_SUPPORT */
		...
		if ((pWscControl->WscConfMode & WSC_REGISTRAR) != 0) {
			...
			if (!bUPnPMsg
				...
				WscEapRegistrarAction(pAdapter, Elem, MsgType,
						      pEntry, pWscControl);
			rv = 1;
		}
#ifdef CONFIG_AP_SUPPORT
		...
		if (((pWscControl->WscConfMode & WSC_REGISTRAR) != 0)
		   ...
		   MsgType == WSC_MSG_M2D ||
		   MsgType == WSC_MSG_M4 ||
		   MsgType == WSC_MSG_M6 || MsgType == WSC_MSG_M8) {
		BOOLEAN bNonceMatch =
		...
		if (!bUPnPMsg &&
			...
			if ((pWscControl->WscState == WSC_STATE_WAIT_M8) &&
				...
				if (IS_PERSISTENT_ON(pAdapter)
				 ...
				 && (pWscControl->WscConfStatus ==
				     WSC_SCSTATE_CONFIGURED)
				 && (pAdapter->ApCfg.MBSSID[apidx].WepStatus ==
...
/*	if( (WscData = kmalloc(WSC_MAX_DATA_LEN, GFP_ATOMIC)) == NULL) */
		...
		if (pWscControl->RegData.ReComputePke == 1) {
			...
			for (idx = 0; idx < 192; idx++)
				pWscControl->RegData.EnrolleeRandom[idx] =
				    RandomByte(pAdapter);
			RT_DH_PublicKey_Generate(WPS_DH_G_VALUE,
						 sizeof(WPS_DH_G_VALUE),
						 WPS_DH_P_VALUE,
	...
	case WSC_MSG_M2:
		...
		if (pWscControl->WscState == WSC_STATE_SENT_M1 ||
			...
			NdisMoveMemory(pWscControl->RegData.PeerInfo.MacAddr,
				       pWscControl->EntryAddr, 6);
			if ((rv =
			     ProcessMessageM2(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      (pWscControl->EntryIfIdx & 0x0F),
					      &pWscControl->RegData))) {
				goto Fail;
	...
	case WSC_MSG_M2D:
		...
		if (pWscControl->WscState == WSC_STATE_SENT_M1 ||
		    pWscControl->WscState == WSC_STATE_RX_M2D) {
			if ((rv =
			     ProcessMessageM2D(pAdapter, Elem->Msg,
					       Elem->MsgLen,
					       &pWscControl->RegData)))
				goto Fail;
			...
			if (CurOpMode == AP_MODE) {
				...
				pWscControl->RegData.SelfInfo.ConfigError =
				    WSC_ERROR_NO_ERROR;
				DataLen =
				    BuildMessageNACK(pAdapter, pWscControl,
						     WscData);
				RTMPSendWirelessEvent(pAdapter,
						      IW_WSC_SEND_NACK, NULL,
						      (pWscControl->
						       EntryIfIdx & 0x0F), 0);
			} else {
				...
				DataLen =
				    BuildMessageACK(pAdapter, pWscControl,
						    WscData);
				RTMPSendWirelessEvent(pAdapter, IW_WSC_SEND_ACK,
						      NULL,
						      (pWscControl->
						       EntryIfIdx & 0x0F), 0);
			}
	...
	case WSC_MSG_M4:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M4) {
			...
			if ((rv =
			     ProcessMessageM4(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData))) {
#ifdef CONFIG_AP_SUPPORT
#ifdef WSC_V2_SUPPORT
	...
	case WSC_MSG_M6:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M6) {
			...
			if ((rv =
			     ProcessMessageM6(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData))) {
#ifdef CONFIG_AP_SUPPORT
#ifdef WSC_V2_SUPPORT
				 ...
				 */
				if (pEntry) {
					RTMP_SEM_LOCK(&pWscControl->
						      WscPeerListSemLock);
					WscDelListEntryByMAC(&pWscControl->
							     WscPeerList,
							     pEntry->Addr);
					RTMP_SEM_UNLOCK(&pWscControl->
							WscPeerListSemLock);
				}
			}
		}
	...
	case WSC_MSG_M8:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M8) {
			...
			if ((rv =
			     ProcessMessageM8(pAdapter, Elem->Msg, Elem->MsgLen,
					      pWscControl)))
				goto Fail;
			else {
				OpCode |= WSC_OPCODE_DONE;
				DataLen =
				    BuildMessageDONE(pAdapter, pWscControl,
						     WscData);
				...
				if (CurOpMode == STA_MODE) {
					pWscControl->WscState =
					    WSC_STATE_WAIT_EAPFAIL;
					pWscControl->WscStatus =
					    STATUS_WSC_EAP_RSP_DONE_SENT;
				}
#endif				/* CONFIG_STA_SUPPORT */
				RTMPSendWirelessEvent(pAdapter,
						      IW_WSC_SEND_DONE, NULL,
						      (pWscControl->
						       EntryIfIdx & 0x0F), 0);
			}
		}
...
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
			P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_COMPLETED,
					     NULL, NULL);
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
#endif				/* P2P_SUPPORT */
		}
	...
	if (OpCode > WSC_OPCODE_UPNP_MASK)
		bUPnPStatus =
		    WscSendUPnPMessage(pAdapter,
				       (pWscControl->EntryIfIdx & 0x0F),
				       WSC_OPCODE_UPNP_DATA,
				       WSC_UPNP_DATA_SUB_NORMAL, WscData,
	...
	if (rv) {
		...
		if (CurOpMode == STA_MODE) {
			...
			if (pWscControl->WscSsid.SsidLength) {
				pAdapter->MlmeAux.AutoReconnectSsidLen =
				    pWscControl->WscSsid.SsidLength;
				NdisZeroMemory(&pAdapter->MlmeAux.
					       AutoReconnectSsid[0],
					       MAX_LEN_OF_SSID);
				NdisMoveMemory(&pAdapter->MlmeAux.
					       AutoReconnectSsid[0],
	...
	if (rv == 1) {
		...
		if ((pAdapter->OpMode == OPMODE_STA)
			...
			if (pAdapter->StaCfg.IWscInfo.bIWscT1TimerRunning) {
				pAdapter->StaCfg.IWscInfo.bIWscT1TimerRunning =
				    FALSE;
				RTMPCancelTimer(&pAdapter->StaCfg.IWscInfo.
						IWscT1Timer, &Cancelled);
			}
		...
		if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
			...
			/*RTMPCancelTimer(&pAdapter->P2pCfg.P2pWscTimer, &Cancelled); */
			if (P2P_GO_ON(pAdapter)
			    && (pWscControl->EntryIfIdx != BSS0) && pEntry) {
				UCHAR P2pIdx = P2P_NOT_FOUND;
				P2pIdx =
				    P2pGroupTabSearch(pAdapter, pEntry->Addr);
				if (P2pIdx != P2P_NOT_FOUND) {
					PRT_P2P_CLIENT_ENTRY pP2pEntry =
					    &pAdapter->P2pTable.Client[P2pIdx];
					// Update p2p Entry's state.
					pP2pEntry->P2pClientState =
					    P2PSTATE_CLIENT_WPS_DONE;
				}
			...
			pAdapter->P2pCfg.ExtListenInterval = 0;
			pAdapter->P2pCfg.ExtListenPeriod = 0;
			if (IS_PERSISTENT_ON(pAdapter) && pEntry
			    && (pEntry->bP2pClient == TRUE)) {
				UCHAR P2pIdx = P2P_NOT_FOUND;
				P2pIdx =
				    P2pGroupTabSearch(pAdapter, pEntry->Addr);
				...
				if ((P2pIdx != P2P_NOT_FOUND)
					...
					P2pPerstTabInsert(pAdapter,
							  ...
							  Profile[0]);
					// this is a persistent connection.
					pAdapter->P2pCfg.ExtListenInterval =
					    P2P_EXT_LISTEN_INTERVAL;
					pAdapter->P2pCfg.ExtListenPeriod =
					    P2P_EXT_LISTEN_PERIOD;
				}
			}
#endif				//P2P_SUPPORT //
...
/*#endif */
...
#endif				/* APCLI_SUPPORT */
				{
					RTMPSetTimer(&pWscControl->
						     WscUpdatePortCfgTimer,
						     1000);
					pWscControl->
					    WscUpdatePortCfgTimerRunning = TRUE;
				}
			...
			if (CurOpMode == STA_MODE) {
				...
				if (pWscControl->WscDriverAutoConnect != 0) {
					pAdapter->StaCfg.bAutoConnectByBssid =
					    TRUE;
...
#ifdef IWSC_SUPPORT
					if ((pAdapter->StaCfg.BssType ==
					     BSS_ADHOC)
					    && (pAdapter->StaCfg.IWscInfo.
						bIWscDevQueryReqTimerRunning ==
						FALSE)) {
						pAdapter->StaCfg.IWscInfo.
						    bIWscDevQueryReqTimerRunning
						    = TRUE;
						RTMPSetTimer(&pAdapter->StaCfg.
							     IWscInfo.
							     IWscDevQueryTimer,
							     200);
					}
...
/*	if ((WscData = kmalloc(WSC_MAX_DATA_LEN, GFP_ATOMIC)) == NULL) */
	...
	if (Elem->MsgType == WSC_EAPOL_UPNP_MSG) {	/* WSC message send from UPnP. */
		switch (MsgType) {
		case WSC_MSG_M2:
		case WSC_MSG_M4:
		case WSC_MSG_M6:
			   ...
			   IN PWSC_CTRL pWscControl)
{
	...
	if (bUPnPMsg) {
		if (MsgType == WSC_MSG_M1) {	/* It's a M1 message, we may need to initialize our state machine. */
...
/*	if( (WscData = kmalloc(WSC_MAX_DATA_LEN, GFP_ATOMIC)) == NULL) */
	...
	case WSC_MSG_M1:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M1) {
			...
			/* Process M1 */
			if ((rv =
			     ProcessMessageM1(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData)))
				goto Fail;
			else {
	...
	case WSC_MSG_M3:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M3) {
			...
			if ((rv =
			     ProcessMessageM3(pAdapter, Elem->Msg, Elem->MsgLen,
					      &pWscControl->RegData)))
				goto Fail;
			else {
				OpCode |= WSC_OPCODE_MSG;
				DataLen =
				    BuildMessageM4(pAdapter, pWscControl,
						   WscData);
				pWscControl->WscStatus = STATUS_WSC_EAP_M4_SENT;
				/* Change the state to next one */
				pWscControl->WscState = WSC_STATE_WAIT_M5;
				RTMPSendWirelessEvent(pAdapter, IW_WSC_SEND_M4,
						      NULL,
						      (pWscControl->
						       EntryIfIdx & 0x0F), 0);
			}
	...
	case WSC_MSG_M5:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M5) {
			...
			if ((rv =
			     ProcessMessageM5(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData)))
				goto Fail;
			else {
	...
	case WSC_MSG_M7:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M7) {
			...
			if ((rv =
			     ProcessMessageM7(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData)))
				goto Fail;
			else {
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_DONE) {
			...
			if (CurOpMode == AP_MODE) {
				...
				if (P2P_GO_ON(pAdapter) &&
				    (pWscControl->WscConfStatus ==
				     WSC_SCSTATE_UNCONFIGURED)
				    && (pWscControl ==
					&pAdapter->ApCfg.MBSSID[MAIN_MBSSID].
					WscControl))
			...
			if (CurOpMode == STA_MODE) {
				...
				if ((pAdapter->StaCfg.BssType == BSS_ADHOC) &&
					...
					pAdapter->StaCfg.IWscInfo.
					    IWscSmpbcAcceptCount--;
					pWscControl->WscStatus =
					    STATUS_WSC_CONFIGURED;

					if (pEntry && pEntry->bIWscSmpbcAccept) {
						pEntry->bIWscSmpbcAccept =
						    FALSE;
						WscDelListEntryByMAC
						    (&pWscControl->WscPeerList,
						     pEntry->Addr);
	...
	if (OpCode > WSC_OPCODE_UPNP_MASK)
		bUPnPStatus =
		    WscSendUPnPMessage(pAdapter,
				       ...
				       DataLen, Elem->TimeStamp.u.LowPart,
				       Elem->TimeStamp.u.HighPart,
				       &pWscControl->EntryAddr[0], CurOpMode);
	else if (OpCode > 0 && OpCode < WSC_OPCODE_UPNP_MASK) {
#ifdef WSC_V2_SUPPORT
		pWscControl->WscTxBufLen = 0;
		pWscControl->pWscCurBufIdx = NULL;
		...
		if (pWscControl->bWscFragment
			...
			pWscControl->pWscCurBufIdx =
			    (pWscControl->pWscTxBuf + pWscControl->WscFragSize);
		}
#endif				/* WSC_V2_SUPPORT */
#ifdef CONFIG_AP_SUPPORT
		if (CurOpMode == AP_MODE) {
	...
	if (bUPnPMsg) {
		if (pWscControl->WscState == WSC_STATE_SENT_M2D) {	/*After M2D, reset the status of State Machine. */
 ...
 Fail:
	...
	if (rv) {
		...
		if (bUPnPMsg) {
			...
			DataLen =
			    BuildMessageNACK(pAdapter, pWscControl, WscData);
#ifdef CONFIG_AP_SUPPORT
			if (CurOpMode == AP_MODE) {
			...
			if (CurOpMode == STA_MODE) {
				if (ADHOC_ON(pAdapter))
					WscSendMessage(pAdapter,
						       WSC_OPCODE_NACK, WscData,
						       DataLen, pWscControl,
						       STA_MODE, EAP_CODE_REQ);
				else
					WscSendMessage(pAdapter,
						       WSC_OPCODE_NACK, WscData,
						       DataLen, pWscControl,
		...
		if (INFRA_ON(pAdapter) ||
			...
			if (CurOpMode == STA_MODE) {
				...
				if ((pWscControl->bConfiguredAP)
					...
					RTMPMoveMemory(&pWscControl->WscProfile,
						       &pWscControl->
						       WscM7Profile,
						       sizeof(pWscControl->
							      WscM7Profile));

				WscWriteConfToPortCfg(pAdapter, pWscControl,
						      &pWscControl->WscProfile.
						      Profile[0], TRUE);
...
/*#ifdef KTHREAD_SUPPORT */
		...
		if (pEntry && (pAdapter->StaCfg.BssType == BSS_ADHOC))
			MacTableDeleteEntry(pAdapter, pEntry->Aid,
					    pEntry->Addr);
#endif				/* IWSC_SUPPORT */
		return;
	}
...
VOID WscTimeOutProcess(IN PRTMP_ADAPTER pAd,
			   ...
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (FunctionContext == 0) {
		...
		pEntry =
		    MacTableLookup(pWscControl->pAd, pWscControl->EntryAddr);
	}
		...
		if (pWscControl->Wsc2MinsTimerRunning) {
			...
			if (CurOpMode == AP_MODE) {
				...
				if ((pWscControl->EntryIfIdx & 0x0F) <
				    pAd->ApCfg.BssidNum) {
					WscBuildBeaconIE(pWscControl->pAd,
							 IsAPConfigured, FALSE,
							 0, 0,
							 (pWscControl->
							  EntryIfIdx & 0x0F),
							 NULL, 0, CurOpMode);
					WscBuildProbeRespIE(pWscControl->pAd,
							    WSC_MSGTYPE_AP_WLAN_MGR,
							    IsAPConfigured,
							    FALSE, 0, 0,
							    pWscControl->
							    EntryIfIdx, NULL, 0,
							    CurOpMode);
					APUpdateBeaconFrame(pWscControl->pAd,
							    pWscControl->
							    EntryIfIdx & 0x0F);
				}
				if ((pWscControl->WscConfMode & WSC_PROXY) == 0) {	/* Proxy mechanism is disabled */
					pWscControl->WscState = WSC_STATE_OFF;
				}
			}
#endif				/* CONFIG_AP_SUPPORT */
			...
			if (CurOpMode == STA_MODE) {
				...
				RTMPZeroMemory(pAd->MlmeAux.AutoReconnectSsid,
					       MAX_LEN_OF_SSID);
				pAd->MlmeAux.AutoReconnectSsidLen =
				    pAd->CommonCfg.SsidLen;
				RTMPMoveMemory(pAd->MlmeAux.AutoReconnectSsid,
					       pAd->CommonCfg.Ssid,
					       pAd->CommonCfg.SsidLen);
				if (INFRA_ON(pAd)
			  ...
			  IN PWSC_CTRL pWscControl, IN UCHAR apidx)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		NdisMoveMemory(pWscControl->RegData.SelfInfo.MacAddr,
			       pAdapter->ApCfg.MBSSID[apidx].Bssid, 6);
	}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE)
	...
	if (CurOpMode == AP_MODE) {
		if (!RTMP_TEST_FLAG(pWscControl, 0x04))
			NdisMoveMemory(&pWscControl->RegData.SelfInfo.
				       DeviceName, AP_WSC_DEVICE_NAME,
				       sizeof(AP_WSC_DEVICE_NAME));
	}
#endif				/* CONFIG_AP_SUPPORT */
	...
	if (CurOpMode == STA_MODE) {
		if (!RTMP_TEST_FLAG(pWscControl, 0x04)) {
			NdisMoveMemory(&pWscControl->RegData.SelfInfo.
				       DeviceName, STA_WSC_DEVICE_NAME,
				       sizeof(STA_WSC_DEVICE_NAME));
		}
	}
#endif				/* CONFIG_STA_SUPPORT */
	...
	if (CurOpMode == AP_MODE)
		NdisMoveMemory(pWscControl->RegData.SelfInfo.Ssid,
			       pAdapter->ApCfg.MBSSID[apidx].Ssid,
			       pAdapter->ApCfg.MBSSID[apidx].SsidLen);
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE)
		NdisMoveMemory(pWscControl->RegData.SelfInfo.Ssid,
			       pAdapter->CommonCfg.Ssid,
	...
	if (CurOpMode == AP_MODE) {
		...
		if (pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI) {
			NdisMoveMemory(&pWscControl->RegData.SelfInfo.
				       PriDeviceType,
				       pAdapter->P2pCfg.DevInfo.PriDeviceType,
				       8);
		} else
#endif				/* P2P_SUPPORT */
		if (pWscControl->EntryIfIdx & MIN_NET_DEVICE_FOR_APCLI)
			NdisMoveMemory(&pWscControl->RegData.SelfInfo.
				       PriDeviceType, &STA_Wsc_Pri_Dev_Type[0],
				       8);
		else
			NdisMoveMemory(&pWscControl->RegData.SelfInfo.
				       PriDeviceType, &AP_Wsc_Pri_Dev_Type[0],
				       8);
	}
#endif				/* CONFIG_AP_SUPPORT */
...
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE)
		NdisMoveMemory(&pWscControl->RegData.SelfInfo.PriDeviceType,
...
/*
	...
	========================================================================
*/
VOID WscSendEapolStart(IN PRTMP_ADAPTER pAdapter,
		       IN PUCHAR pBssid, IN UCHAR CurOpMode)
{
...
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE) {
		if (pAdapter->StaCfg.WscControl.WscState >=
		    WSC_STATE_WAIT_WSC_START)
			return;
	}
#endif				/* CONFIG_STA_SUPPORT */
	...
	if (CurOpMode == AP_MODE) {
		MAKE_802_3_HEADER(Header802_3,
				  pBssid,
				  &pAdapter->ApCfg.ApCliTab[0].
				  CurrentAddress[0], EAPOL);
	}
#endif				/* APCLI_SUPPORT */
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE)
		MAKE_802_3_HEADER(Header802_3, pBssid,
				  &pAdapter->CurrentAddress[0], EAPOL);
#endif				/* CONFIG_STA_SUPPORT */
	...
	if (CurOpMode == AP_MODE) {
		/* Update WSC status */
		pAdapter->ApCfg.ApCliTab[0].WscControl.WscStatus =
		    STATUS_WSC_EAPOL_START_SENT;
		pAdapter->ApCfg.ApCliTab[0].WscControl.WscState =
		    WSC_STATE_WAIT_REQ_ID;
		if (!pAdapter->ApCfg.ApCliTab[0].WscControl.EapolTimerRunning) {
			pAdapter->ApCfg.ApCliTab[0].WscControl.
			    EapolTimerRunning = TRUE;
			RTMPSetTimer(&pAdapter->ApCfg.ApCliTab[0].WscControl.
				     EapolTimer, WSC_EAPOL_START_TIME_OUT);
		}
	}
#endif				/* APCLI_SUPPORT */
#endif				/* CONFIG_AP_SUPPORT */
	...
	if (CurOpMode == STA_MODE) {
		...
		if (!pAdapter->StaCfg.WscControl.EapolTimerRunning) {
			...
			RTMPSetTimer(&pAdapter->StaCfg.WscControl.EapolTimer,
				     2000);
		}
	}
#endif				/* CONFIG_STA_SUPPORT */
...
VOID WscSendEapRspId(IN PRTMP_ADAPTER pAdapter,
		     IN PMAC_TABLE_ENTRY pEntry, IN PWSC_CTRL pWscControl)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		if (INFRA_ON(pAdapter)) {
			MAKE_802_3_HEADER(Header802_3,
					  &pAdapter->CommonCfg.Bssid[0],
					  &pAdapter->CurrentAddress[0], EAPOL);
		} else {
			MAKE_802_3_HEADER(Header802_3,
					  &pWscControl->WscPeerMAC[0],
					  &pAdapter->CurrentAddress[0], EAPOL);
		}
...
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE)
		RTMPToWirelessSta((PRTMP_ADAPTER) pWscControl->pAd,
			  ...
			  IN INT Status, IN UINT eventID, IN UCHAR CurOpMode)
{
	...
	WscSendUPnPMessage(pAd, apIdx, WSC_OPCODE_UPNP_MGMT,
			   WSC_UPNP_MGMT_SUB_CONFIG_REQ, &pData[0], 39, eventID,
			   0, NULL, CurOpMode);
...
int WscSendUPnPMessage(IN PRTMP_ADAPTER pAd,
		       ...
		       IN INT dataLen,
		       IN UINT eventID,
		       IN UINT toIPAddr, IN PUCHAR pMACAddr, IN UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		if (devIfIdx & MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (bssIdx >= MAX_APCLI_NUM)
				return FALSE;
			pDevAddr =
			    &pAd->ApCfg.ApCliTab[bssIdx].CurrentAddress[0];
		} else
#endif				/* APCLI_SUPPORT */
		{
		...
		while (leftLen) {
			...
			pNLMsgHdr->segLen =
			    (leftLen >
			     ...
			     IWEVCUSTOM_PAYLOD_MAX_LEN : leftLen);
			...
			if (firstSeg) {
				...
				copyLen =
				    (pNLMsgHdr->segLen - RTMP_WSC_MSG_HDR_LEN);
				if (((pWscMsgHdr->
				      msgSubType &
			...
			RtmpOSWrielessEventSend(pAd->net_dev,
						...
						RT_WSC_UPNP_EVENT_FLAG, NULL,
						pBuf,
						pNLMsgHdr->segLen +
						sizeof(RTMP_WSC_NLMSG_HDR));
		}
...
VOID WscSendMessage(IN PRTMP_ADAPTER pAdapter,
		    IN UCHAR OpCode,
		    IN PUCHAR pData,
		    IN INT Len,
		    IN PWSC_CTRL pWscControl, IN UCHAR OpMode, IN UCHAR EapType)
{
	/* Inb-EAP Message */
	UCHAR Header802_3[14];
	...
	if (pData && (Len > 0)) {
		if (pWscControl->bWscFragment && (pWscControl->bWscFirstOne)) {
			...
			MakeOutgoingFrame(pOutBuffer, &TmpLen,
					  ...
					  LF_Len, &MsgLen, END_OF_ARGS);

			FrameLen += TmpLen;

			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  Len, pData, END_OF_ARGS);
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  sizeof(IEEE8021X_FRAME), &Ieee_8021x,
				  sizeof(EapFrame), &EapFrame,
				  sizeof(WscFrame), &WscFrame, END_OF_ARGS);
...
VOID WscBuildBeaconIE(IN PRTMP_ADAPTER pAd,
...
/*	UCHAR 			Data[256]; */
	...
	if ((CurOpMode == AP_MODE) && pWpsCtrl->bSetupLock) {
		// AP Setup Lock
		templen =
		    AppendWSCTLV(WSC_ID_AP_SETUP_LOCKED, pData,
	...
	if (b_selRegistrar) {
		...
		templen =
		    AppendWSCTLV(WSC_ID_SEL_REGISTRAR, pData,
		...
		templen =
		    AppendWSCTLV(WSC_ID_SEL_REG_CFG_METHODS, pData,
				 (UINT8 *) & tempVal, 0);
		pData += templen;
		Len += templen;
	}
...
#ifdef IWSC_SUPPORT
	if ((pAd->StaCfg.BssType == BSS_ADHOC) && (CurOpMode == STA_MODE)) {
		UCHAR respType;

		// Connection Type Flag ESS
		templen =
		    AppendWSCTLV(WSC_ID_CONN_TYPE, pData,
		...
		if ((pWpsCtrl->WscMode == WSC_SMPBC_MODE) &&
			...
			templen =
			    AppendWSCTLV(WSC_ID_ENTRY_ACCEPTABLE, pData,
					 (UINT8 *) & bEntryAcceptable, 0);
			pData += templen;
			Len += templen;
			...
			templen =
			    AppendWSCTLV(WSC_ID_REGISTRATON_READY, pData,
					 (UINT8 *) & bRegistrationReady, 0);
			pData += templen;
			Len += templen;
		}
		...
		templen =
		    AppendWSCTLV(WSC_ID_IP_ADDR_CONF_METHOD, pData,
				 (UINT8 *) & tempVal, 0);
		pData += templen;
		Len += templen;
#ifdef IWSC_TEST_SUPPORT
		...
		templen =
		    AppendWSCTLV(WSC_ID_RESP_TYPE, pData, (UINT8 *) & respType,
				 0);
		pData += templen;
		Len += templen;
#endif				/* IWSC_TEST_SUPPORT */
	...
	if (pWpsCtrl->WscV2Info.bEnableWpsV2) {
		...
		WscGenV2Msg(pWpsCtrl,
			    b_selRegistrar,
			    pAuthorizedMACs, AuthorizedMACsLen, &pData, &Len);
...
#ifdef CONFIG_AP_SUPPORT
	if (CurOpMode == AP_MODE) {
		memcpy(pAd->ApCfg.MBSSID[apidx].WscIEBeacon.Value, &ieHdr,
		       sizeof(WSC_IE_HEADER));
		memcpy(pAd->ApCfg.MBSSID[apidx].WscIEBeacon.Value +
		       sizeof(WSC_IE_HEADER), Data, Len);
		pAd->ApCfg.MBSSID[apidx].WscIEBeacon.ValueLen =
		    sizeof(WSC_IE_HEADER) + Len;
	}
#endif				/* CONFIG_AP_SUPPORT */

#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE) {
		memcpy(pAd->StaCfg.WpsIEBeacon.Value, &ieHdr,
		       sizeof(WSC_IE_HEADER));
		memcpy(pAd->StaCfg.WpsIEBeacon.Value + sizeof(WSC_IE_HEADER),
		       Data, Len);
...
VOID WscBuildProbeRespIE(IN PRTMP_ADAPTER pAd,
			 IN UCHAR respType,
			 IN UCHAR scState,
			 IN BOOLEAN b_selRegistrar,
			 IN USHORT devPwdId,
...
/*	UCHAR 			Data[512]; */
...
#ifdef IWSC_SUPPORT
		if ((CurOpMode == STA_MODE)
		    && pAd->StaCfg.IWscInfo.bSelRegStart) {
			templen =
			    AppendWSCTLV(WSC_ID_MAC_ADDR, pData,
					 pAd->StaCfg.IWscInfo.RegMacAddr, 0);
			pData += templen;
			Len += templen;
			pAd->StaCfg.IWscInfo.bSelRegStart = FALSE;
		}
	...
	templen =
	    AppendWSCTLV(WSC_ID_RESP_TYPE, pData, (UINT8 *) & respType, 0);
	pData += templen;
	Len += templen;
...
#ifdef CONFIG_AP_SUPPORT
	if (CurOpMode == AP_MODE) {
		memcpy(pAd->ApCfg.MBSSID[apidx & 0xF].WscIEProbeResp.Value,
		       &ieHdr, sizeof(WSC_IE_HEADER));
		memcpy(pAd->ApCfg.MBSSID[apidx & 0xF].WscIEProbeResp.Value +
		       sizeof(WSC_IE_HEADER), Data, Len);
		pAd->ApCfg.MBSSID[apidx & 0xF].WscIEProbeResp.ValueLen =
		    sizeof(WSC_IE_HEADER) + Len;
	}
#endif				/* CONFIG_AP_SUPPORT */
...
#ifdef CONFIG_STA_SUPPORT
	if (CurOpMode == STA_MODE) {
		memcpy(pAd->StaCfg.WpsIEProbeResp.Value, &ieHdr,
		       sizeof(WSC_IE_HEADER));
		memcpy(pAd->StaCfg.WpsIEProbeResp.Value + sizeof(WSC_IE_HEADER),
		       Data, Len);
...
/*
	...
	========================================================================
*/
VOID WscSendEapFail(IN PRTMP_ADAPTER pAd,
		    IN PWSC_CTRL pWscControl, IN BOOLEAN bSendDeAuth)
{
	...
	if (CurOpMode == AP_MODE) {
		MAKE_802_3_HEADER(Header802_3,
				  &pWscControl->EntryAddr[0],
				  &pAd->ApCfg.MBSSID[apidx].Bssid[0], EAPOL);
	}
#endif				/* CONFIG_AP_SUPPORT */
...
VOID WscBuildAssocRespIE(IN PRTMP_ADAPTER pAd,
			 IN UCHAR ApIdx,
			 IN UCHAR Reason, OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen)
{
	WSC_IE_HEADER ieHdr;
/*	UCHAR 			Data[512] = {0}; */
	UCHAR *Data = NULL;
	...
	UINT8 tempVal = 0;
	PWSC_REG_DATA pReg =
	    (PWSC_REG_DATA) & pAd->ApCfg.MBSSID[ApIdx].WscControl.RegData;
...
VOID WscSelectedRegistrar(IN PRTMP_ADAPTER pAd,
			  IN PUCHAR pReginfo, IN UINT Length, IN UCHAR apidx)
{
	PUCHAR pData;
	INT IsAPConfigured;
		...
		case WSC_ID_VENDOR_EXT:
			...
			if (pWscCtrl->WscV2Info.bEnableWpsV2 && (WscLen > 0)) {
				...
				if (pAuthorizedMACs) {
					...
					WscParseV2SubItem
					    (WFA_EXT_ID_AUTHORIZEDMACS, pData,
					     WscLen, pAuthorizedMACs,
					     &AuthorizedMACsLen);
				}
			}
#endif				/* WSC_V2_SUPPORT */
	...
	if (wsc_sel_reg == 0x01) {
		...
		if ((AuthorizedMACsLen == 6)
		    &&
		    (NdisEqualMemory
		     (pAuthorizedMACs, BROADCAST_ADDR, MAC_ADDR_LEN) == FALSE)
		    &&
		    (NdisEqualMemory
		     (pAuthorizedMACs, ZERO_MAC_ADDR, MAC_ADDR_LEN) == FALSE)
...
/*	UCHAR			OutMsgBuf[512];		// buffer to create message contents */
	...
	templen =
	    AppendWSCTLV(WSC_ID_ASSOC_STATE, pData, (UINT8 *) & tempVal, 0);
	pData += templen;
	Len += templen;
	...
	if (pAd->StaCfg.WscControl.WscV2Info.bEnableWpsV2) {
		...
		NdisZeroMemory(pData, 32 + 4);
		templen =
		    AppendWSCTLV(WSC_ID_MODEL_NAME, pData,
				 pReg->SelfInfo.ModelName,
				 strlen((PSTRING) pReg->SelfInfo.ModelName));
		pData += templen;
		Len += templen;
...
#ifdef APCLI_SUPPORT
		if ((CurOpMode == AP_MODE)
		    && (pAdapter->ApCfg.ApCliTab[BSS0].CtrlCurrState ==
...
UINT WscRandomGen4digitPinCode(IN PRTMP_ADAPTER pAd)
{
	...
	iPin =
	    RandomByte2(pAd) * 256 * 256 + RandomByte2(pAd) * 256 +
	    RandomByte2(pAd);
	iPin = iPin % 10000;
...
UINT WscRandomGeneratePinCode(IN PRTMP_ADAPTER pAd, IN UCHAR apidx)
{
	...
	iPin =
	    RandomByte(pAd) * 256 * 256 + RandomByte(pAd) * 256 +
	    RandomByte(pAd);
...
VOID WscInformFromWPA(IN PMAC_TABLE_ENTRY pEntry)
{
	...
	if (MAC_ADDR_EQUAL
		...
		RTMPCancelTimer(&pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.
				EapolTimer, &Cancelled);
		pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.EapolTimerRunning =
		    FALSE;
		pEntry->bWscCapable = FALSE;
		pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscState =
		    WSC_STATE_CONFIGURED;
...
VOID WscDelWPARetryTimer(IN PRTMP_ADAPTER pAd)
{
	...
	pEntry =
	    MacTableLookup(pAd, pAd->ApCfg.MBSSID[apidx].WscControl.EntryAddr);
	     ...
	     IN PWSC_CTRL pWscControl)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		NdisZeroMemory(&pAd->MlmeAux.AutoReconnectSsid[0],
			       MAX_LEN_OF_SSID);
		NdisMoveMemory(&pAd->MlmeAux.AutoReconnectSsid[0],
			       &pAd->MlmeAux.Ssid[0], pAd->MlmeAux.SsidLen);
	}
#endif				/* CONFIG_STA_SUPPORT */
...
VOID WscInit(IN PRTMP_ADAPTER pAd, IN BOOLEAN bFromApCli, IN UCHAR BssIndex)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (bFromApCli)
			pWscControl =
			    &pAd->ApCfg.ApCliTab[BssIndex & 0x0F].WscControl;
		else
#endif				/* APCLI_SUPPORT */
			pWscControl =
			    &pAd->ApCfg.MBSSID[BssIndex & 0x0F].WscControl;
	}
#endif				/* CONFIG_AP_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (BssIndex >= MIN_NET_DEVICE_FOR_P2P_GO) {
			...
			pWscControl =
			    &pAd->ApCfg.ApCliTab[MAIN_MBSSID].WscControl;
		} else
#endif				/* P2P_SUPPORT */
		{
			pWscControl = &pAd->StaCfg.WscControl;
...
#endif				/* APCLI_SUPPORT */
			{
				pAd->ApCfg.MBSSID[BssIndex & 0x0F].WscIEBeacon.
				    ValueLen = 0;
				pAd->ApCfg.MBSSID[BssIndex & 0x0F].
				    WscIEProbeResp.ValueLen = 0;
			}
		}
	} else
#endif				/* CONFIG_AP_SUPPORT */
	{
		...
		if (pWscControl->WscConfMode == WSC_DISABLE) {
			if (BssIndex >= MIN_NET_DEVICE_FOR_P2P_GO) {
				pAd->ApCfg.MBSSID[MAIN_MBSSID].WscIEBeacon.
				    ValueLen = 0;
				pAd->ApCfg.MBSSID[MAIN_MBSSID].WscIEProbeResp.
				    ValueLen = 0;
			}
			return;
		}
			   ...
			   IN PWSC_CREDENTIAL pCredential, IN BOOLEAN bEnrollee)
{
	...
	if (bEnrollee || (CurOpMode == AP_MODE)) {
...
#ifdef P2P_SUPPORT
			if (P2P_GO_ON(pAd)) {
				NdisZeroMemory(pAd->P2pCfg.SSID,
					       MAX_LEN_OF_SSID);
				pAd->P2pCfg.SSIDLen =
				    pCredential->SSID.SsidLength;
				NdisMoveMemory(pAd->P2pCfg.SSID,
					       pCredential->SSID.Ssid,
					       pAd->P2pCfg.SSIDLen);
			}
#endif				/* P2P_SUPPORT */
		...
		if (CurOpMode == STA_MODE) {
			...
			NdisZeroMemory(pAd->MlmeAux.Ssid,
				       NDIS_802_11_LENGTH_SSID);
			NdisMoveMemory(pAd->MlmeAux.Ssid,
				       pCredential->SSID.Ssid,
				       pAd->MlmeAux.SsidLen);
			if (!NdisEqualMemory
			    (pCredential->MacAddr, pAd->CurrentAddress,
			     MAC_ADDR_LEN)) {
				NdisZeroMemory(pAd->MlmeAux.Bssid,
					       MAC_ADDR_LEN);
				NdisMoveMemory(pAd->MlmeAux.Bssid,
					       pCredential->MacAddr,
					       MAC_ADDR_LEN);
			}
		}
		...
		if (pCredential->
			...
			if (!
			    (pCredential->
			     EncrType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES)))
			{
				DBGPRINT(RT_DEBUG_TRACE,
...
#ifdef CONFIG_STA_SUPPORT
					if (CurOpMode == STA_MODE) {
						RTMPZeroMemory(pAd->StaCfg.
							       WpaPassPhrase,
							       64);
						RTMPMoveMemory(pAd->StaCfg.
							       WpaPassPhrase,
							       pCredential->Key,
							       pWscControl->
							       WpaPskLen);
		...
		if (CurOpMode == AP_MODE) {
			...
			NdisMoveMemory(pWscControl->WpaPsk, pCredential->Key,
				       pWscControl->WpaPskLen);
			/* Copy SSID */
			NdisZeroMemory(pAd->ApCfg.MBSSID[CurApIdx].Ssid,
				       MAX_LEN_OF_SSID);
			...
			if (pWscControl->WscKeyASCII == 0) {
				AtoH((PSTRING) pWscControl->WpaPsk,
				     pAd->ApCfg.MBSSID[CurApIdx].PMK, 32);
			} else {
				...
				RtmpPasswordHash((PSTRING) pWscControl->WpaPsk,
						 ...
						 pAd->ApCfg.MBSSID[CurApIdx].
						 SsidLen, keyMaterial);
				NdisMoveMemory(pAd->ApCfg.MBSSID[CurApIdx].PMK,
					       keyMaterial, 32);
			}
		}
	}
...
VOID WscWriteSsidToDatFile(IN PRTMP_ADAPTER pAd,
			   IN PSTRING pTempStr,
			   IN BOOLEAN bNewFormat, IN UCHAR CurOpMode)
{
	...
	if (bNewFormat == FALSE) {
		...
		if (CurOpMode == STA_MODE) {
			UINT profile_idx =
			    pAd->StaCfg.WscControl.WscProfile.ApplyProfileIdx;
...
VOID WscWriteWpaPskToDatFile(IN PRTMP_ADAPTER pAd,
			     IN PSTRING pTempStr, IN BOOLEAN bNewFormat)
{
#ifdef CONFIG_AP_SUPPORT
	UCHAR apidx;
#endif				/* CONFIG_AP_SUPPORT */
...
VOID WscGetRegDataPIN(IN PRTMP_ADAPTER pAdapter,
		      IN UINT PinCode, IN PWSC_CTRL pWscControl)
{
	...
	if (pWscControl->WscPinCode == 0) {
		...
		if (pWscControl->WscPinCodeLen == 4) {
			UCHAR temp4PIN[5] = { 0 };
			snprintf((PSTRING) temp4PIN, sizeof(temp4PIN), "%04u",
				 pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, temp4PIN, 4);
			pWscControl->RegData.PinCodeLen = 4;
		} else {
			snprintf((PSTRING) tempPIN, sizeof(tempPIN), "%08u",
				 pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, tempPIN, 8);
			pWscControl->RegData.PinCodeLen = 8;
		}
...
VOID WscEapActionDisabled(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter) {
		if (pWscControl->EntryIfIdx & MIN_NET_DEVICE_FOR_APCLI)
			WscSendMessage(pAdapter, WSC_OPCODE_NACK, WscData,
	...
	if (CurOpMode == AP_MODE) {
		...
		if ((strcmp(arg, "WEPAUTO") == 0)
			...
			pAd->ApCfg.MBSSID[apidx].AuthMode =
			    Ndis802_11AuthModeOpen;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: Unknow AuthMode (%s), set AuthMode to OPEN\n",
				  __FUNCTION__, arg));
		}
		...
		pAd->ApCfg.MBSSID[apidx].PortSecured =
		    WPA_802_1X_PORT_NOT_SECURED;
	...
	if (CurOpMode == AP_MODE) {
		...
		if ((strcmp(arg, "NONE") == 0) || (strcmp(arg, "none") == 0))
			...
			pAd->ApCfg.MBSSID[apidx].WepStatus =
			    Ndis802_11WEPDisabled;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: Unknow EncrypType (%s), set EncrypType to NONE\n",
				  __FUNCTION__, arg));
		}

		if (pAd->ApCfg.MBSSID[apidx].WepStatus >=
		    Ndis802_11Encryption2Enabled)
			pAd->ApCfg.MBSSID[apidx].DefaultKeyId = 0;

		/*RTMPMakeRSNIE(pAd, pAd->ApCfg.MBSSID[apidx].AuthMode, pAd->ApCfg.MBSSID[apidx].WepStatus, apidx); */
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(ra%d) %s::(EncrypType=%d)\n", apidx, __FUNCTION__,
			  pAd->ApCfg.MBSSID[apidx].WepStatus));
...
BOOLEAN WscBssWpsIESearchForPBC(PRTMP_ADAPTER pAd,
				PWSC_CTRL pWscControl,
				PBSS_ENTRY pInBss,
				UUID_BSSID_CH_INFO ApUuidBssid[],
				INT VarIeLen, PUCHAR pVar)
{
	INT j = 0, Len = 0, idx = 0;
		...
		while (Len > 0) {
			...
			if ((pAd->OpMode == OPMODE_STA) &&
				...
				if (be2cpu16(pWscIE->Type) == WSC_ID_MAC_ADDR) {
					...
					RTMPMoveMemory(mac_addr, (pData + 4),
						       MAC_ADDR_LEN);
					if (NdisCmpMemory
					    (pInBss->MacAddr, mac_addr,
					     MAC_ADDR_LEN)) {
						bFound = FALSE;
						break;
					}
		...
		if ((bFound == TRUE) && (bSelReg == TRUE)) {
			...
			if (pWscControl->WscPBCBssCount > 0) {
				for (j = 0; j < pWscControl->WscPBCBssCount;
				     j++) {
					if (RTMPCompareMemory
					    (&ApUuidBssid[j].Uuid[0],
					     &TmpInfo.Uuid[0], 16) == 0) {
						if (RTMPCompareMemory
						    (&TmpInfo.Uuid[0], zeros16,
						     16) != 0) {
							/*
...
/*	UUID_BSSID_CH_INFO	ApUuidBssid[8]; */
	...
	if (pWscControl->WscPBCBssCount == 1) {
		...
		RTMPMoveMemory(pWscControl->WscPeerMAC, ApUuidBssid[0].MacAddr,
			       MAC_ADDR_LEN);
#endif				/* CONFIG_STA_SUPPORT */
...
#ifdef APCLI_SUPPORT
		if (CurOpMode == AP_MODE) {
			NdisZeroMemory(pAd->ApCfg.ApCliTab[BSS0].CfgSsid,
				       MAX_LEN_OF_SSID);
			NdisMoveMemory(pAd->ApCfg.ApCliTab[BSS0].CfgSsid,
				       ApUuidBssid[0].Ssid,
				       ApUuidBssid[0].SsidLen);
			pAd->ApCfg.ApCliTab[BSS0].CfgSsidLen =
			    (UCHAR) ApUuidBssid[0].SsidLen;
		}
#endif				/* APCLI_SUPPORT */
	...
	if (pWscControl->WscMode == WSC_SMPBC_MODE)
		DBGPRINT(RT_DEBUG_OFF,
			 ("WscPBCBssTableSort : Total %d SMPBC Registrar Found\n",
			  pWscControl->WscPBCBssCount));
	else
#endif				/* IWSC_SUPPORT */
		DBGPRINT(RT_DEBUG_OFF,
		     ...
		     INOUT PUCHAR pKey, INOUT PUSHORT pKeyLen)
{
	...
	if (pWscControl->WscKeyASCII == 0) {
		...
		for (idx = 0; idx < 32; idx++) {
			...
			snprintf((PSTRING) & tmpStrB[0], 3, "%02x",
				 tempRandomByte);
...
VOID WscCreateProfileFromCfg(IN PRTMP_ADAPTER pAd,
			     IN UCHAR OpMode,
			     IN PWSC_CTRL pWscControl,
			     OUT PWSC_PROFILE pWscProfile)
{
	...
	if (CurOpMode == AP_MODE) {
		if ((OpMode & 0x0F) == AP_MODE) {
			...
			if (((pWscControl->WscConfStatus ==
				...
				authType =
				    WscGetAuthType(pAd->ApCfg.MBSSID[apidx].
						   AuthMode);
				encyType =
				    WscGetEncryType(pAd->ApCfg.MBSSID[apidx].
						    WepStatus);
			}
			...
			apidx = apidx & 0x0F;
			authType =
			    WscGetAuthType(pAd->ApCfg.ApCliTab[apidx].AuthMode);
			encyType =
			    WscGetEncryType(pAd->ApCfg.ApCliTab[apidx].
					    WepStatus);
	...
	if ((WSC_SCSTATE_UNCONFIGURED == pWscControl->WscConfStatus) &&
		...
		if ((OpMode & 0x0F) == STA_MODE) {
			...
			if (pAd->StaCfg.IWscInfo.IWscDefaultSecurity == 1) {
			...
			} else if (pAd->StaCfg.IWscInfo.IWscDefaultSecurity ==
				   2) {
				UCHAR idx;
				CHAR tempRandomByte;
				authType = WSC_AUTHTYPE_OPEN;
				...
				for (idx = 0; idx < 13; idx++) {
					tempRandomByte =
					    RandomByte(pAd) % 94 + 33;
					sprintf((PSTRING) pCredential->Key +
						idx, "%c", tempRandomByte);
				}
		...
		case WSC_ENCRTYPE_WEP:
			...
			if (((OpMode & 0x0F) == AP_MODE
			     || (OpMode & 0x0F) == STA_MODE)
			    && pAd->SharedKey[apidx][WepKeyId].KeyLen) {
				INT i;
				for (i = 0;
				     i < pAd->SharedKey[apidx][WepKeyId].KeyLen;
				     i++) {
					snprintf((PSTRING) pCredential->Key, 64,
						 "%s%02x", pCredential->Key,
						 pAd->
						 SharedKey[apidx][WepKeyId].
						 Key[i]);
				}
				pCredential->KeyLength =
				    pAd->SharedKey[apidx][WepKeyId].KeyLen * 2;
			}
#ifdef CONFIG_AP_SUPPORT
#ifdef APCLI_SUPPORT
			else if ((OpMode == AP_CLIENT_MODE)
				 && (pAd->ApCfg.ApCliTab[apidx].
				     SharedKey[WepKeyId].KeyLen)
				 && (CurOpMode == AP_MODE)) {
				INT i;
				for (i = 0;
				     i <
				     pAd->ApCfg.ApCliTab[apidx].
				     SharedKey[WepKeyId].KeyLen; i++) {
					snprintf((PSTRING) pCredential->Key, 64,
						 "%s%02x", pCredential->Key,
						 pAd->ApCfg.ApCliTab[apidx].
						 SharedKey[WepKeyId].Key[i]);
				}
				pCredential->KeyLength =
				    pAd->SharedKey[apidx][WepKeyId].KeyLen * 2;
			}
		...
		case (WSC_ENCRTYPE_AES | WSC_ENCRTYPE_TKIP):
			...
			memcpy(pCredential->Key,
			       pWscControl->WpaPsk, pWscControl->WpaPskLen);
			break;
		}
	}
	...
	if (CurOpMode == STA_MODE) {
		...
		NdisMoveMemory(pCredential->SSID.Ssid, pAd->CommonCfg.Ssid,
			       pAd->CommonCfg.SsidLen);
			    ...
			    IN BOOLEAN bEnrollee)
{
	...
	CurApIdx = (pWscControl->EntryIfIdx & 0x0F);
	{
		...
		Set_ApCli_AuthMode_Proc(pAd,
					WscGetAuthTypeStr(pCredential->
							  AuthType));
		Set_ApCli_EncrypType_Proc(pAd,
					  WscGetEncryTypeStr(pCredential->
							     EncrType));
	...
	if (P2P_CLI_ON(pAd)) {
		...
		NdisMoveMemory(pAd->P2pCfg.SSID, pCredential->SSID.Ssid,
			       pAd->P2pCfg.SSIDLen);
	}
#endif				/* P2P_SUPPORT */
...
{
	int HeaderLen =
	    LENGTH_802_11 + LENGTH_802_1_H + sizeof(IEEE8021X_FRAME) +
	    sizeof(EAP_FRAME);
	PHEADER_802_11 pHeader;
	PMAC_TABLE_ENTRY pEntry = NULL;
	...
	if (CurOpMode == 0xFF) {
		DBGPRINT(RT_DEBUG_WARN,
			 ("Unkown OpMode (CurOpMode=0x%02x)\n", CurOpMode));
		return;
	} else
	...
	if ((Elem->MsgType == WSC_EAPOL_UPNP_MSG) && (Elem->MsgLen > HeaderLen)) {	/*The WSC msg from UPnP daemon */
		PUCHAR pData;
		UCHAR MacAddr[MAC_ADDR_LEN] = { 0 };
		...
		StateMachinePerformAction(pAd, &pAd->Mlme.WscMachine, Elem,
					  pAd->Mlme.WscMachine.CurrState);
	else if (pEntry && (Elem->MsgType == WSC_EAPOL_PACKET_MSG)) {	/* WSC_STATE_MACHINE can service only one station at one time */
		PSTRING pData;
		...
		if (pEapFrame->Code == EAP_CODE_FAIL) {	/* EAP-Fail */
			STRING fail_data[] = "EAP_FAIL";
			...
			StateMachinePerformAction(pAd, &pAd->Mlme.WscMachine,
						  ...
						  CurrState);
			return;
		} else if ((pEapFrame->Code == EAP_CODE_REQ) && (pEapFrame->Type == EAP_TYPE_ID)) {	/* EAP-Req (Identity) */
			STRING id_data[] = "hello";
			...
			StateMachinePerformAction(pAd, &pAd->Mlme.WscMachine,
						  ...
						  CurrState);
			return;
		} else if ((pEapFrame->Code == EAP_CODE_REQ) && (pEapFrame->Type == EAP_TYPE_WSC)) {	/* EAP-Req (Messages) */
		...
		if (IS_ENTRY_CLIENT(pEntry) &&
			...
			if (strstr(pData, "SimpleConfig")) {
				...
				NdisZeroMemory(pWpsCtrl->EntryAddr,
					       MAC_ADDR_LEN);
				pWpsCtrl->EapolTimerRunning = FALSE;
				RTMPCancelTimer(&pWpsCtrl->EapolTimer,
						&Cancelled);
				return;
			}
		} else {
	...
	if (pAd->StaCfg.WscControl.WscConfMode != WSC_DISABLE) {
		...
		if (pAd->StaCfg.BssType == BSS_INFRA) {
			...
			if (pAd->StaCfg.IWscInfo.bReStart) {
				...
				if (pAd->StaCfg.WscControl.WscStatus !=
				    STATUS_WSC_CONFIGURED) {
					Ssid.SsidLength =
					    pAd->CommonCfg.SsidLen;
					NdisZeroMemory(Ssid.Ssid,
						       MAX_LEN_OF_SSID);
			...
			MlmeEnqueue(pAd, IWSC_STATE_MACHINE, IWSC_MT2_MLME_STOP,
				    0, NULL, 0);
			RTMP_MLME_HANDLER(pAd);
		}
#endif				/* IWSC_SUPPORT */
			   ...
			   IN PUCHAR pSsid, IN UCHAR SsidLen, IN INT WscMode)
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if (SSID_EQUAL(pSsid, SsidLen, pBss->Ssid, pBss->SsidLen) &&
		    pBss->WpsAP &&
		    ((pBss->WscDPIDFromWpsAP == DesiredDPID)
		     || (DesiredDPID == DEV_PASS_ID_PIN))) {
			if ((pWscControl->WpsApBand ==
			     PREFERRED_WPS_AP_PHY_TYPE_5_G_FIRST)
			    && (pBss->Channel <= 14))
	...
	if (pWscStaPbcProbeInfo->WscPBCStaProbeCount > 1) {
		...
		for (i = 0; i < MAX_PBC_STA_TABLE_SIZE; i++) {
			...
			if (pWscStaPbcProbeInfo->Valid[i] &&
			    RTMP_TIME_AFTER(now,
					    pWscStaPbcProbeInfo->ReciveTime[i] +
					    120 * OS_HZ)) {
				NdisZeroMemory(&
					       (pWscStaPbcProbeInfo->
						StaMacAddr[i][0]),
...
VOID WscPBC_DPID_FromSTA(IN PRTMP_ADAPTER pAd, IN PUCHAR pMacAddr)
{
	...
	if (pWscStaPbcProbeInfo->WscPBCStaProbeCount == 0)
		...
		for (tab_idx = 0; tab_idx < MAX_PBC_STA_TABLE_SIZE; tab_idx++) {
			if (NdisEqualMemory
			    (pMacAddr, pWscStaPbcProbeInfo->StaMacAddr[tab_idx],
			     MAC_ADDR_LEN)) {
		...
		for (tab_idx = 0; tab_idx < MAX_PBC_STA_TABLE_SIZE; tab_idx++) {
			if (RTMP_TIME_AFTER
			    (now,
			     pWscStaPbcProbeInfo->ReciveTime[tab_idx] +
			     120 * OS_HZ)
			    || NdisEqualMemory(pWscStaPbcProbeInfo->
					       StaMacAddr[tab_idx],
					       &ZERO_MAC_ADDR[0],
					       MAC_ADDR_LEN)) {
				if (pWscStaPbcProbeInfo->Valid[tab_idx] ==
				    FALSE) {
					Index = tab_idx;
					bAddEntry = TRUE;
					break;
					...
					pWscStaPbcProbeInfo->
					    ReciveTime[tab_idx] = now;
					NdisMoveMemory(pWscStaPbcProbeInfo->
						       StaMacAddr[tab_idx],
	...
	if (bAddEntry) {
		...
		NdisMoveMemory(pWscStaPbcProbeInfo->StaMacAddr[Index], pMacAddr,
			       MAC_ADDR_LEN);
	}
}

void WscWriteConfToDatFile(IN PRTMP_ADAPTER pAd, IN UCHAR CurOpMode)
{
	char *cfgData = 0;
	...
	if (CurOpMode == AP_MODE) {
		...
		snprintf((PSTRING) WepKeyName, sizeof(WepKeyName),
			 "Key%dStr%d=",
			 pAd->ApCfg.MBSSID[apidx].DefaultKeyId + 1, apidx + 1);
		snprintf((PSTRING) WepKeyFormatName, sizeof(WepKeyFormatName),
			 "Key%dType=",
	...
	if (CurOpMode == STA_MODE) {
		...
		snprintf(WepKeyName, sizeof(WepKeyName), "Key%dStr=",
			 pAd->StaCfg.DefaultKeyId + 1);
		snprintf(WepKeyFormatName, sizeof(WepKeyFormatName),
			 "Key%dType=", pAd->StaCfg.DefaultKeyId + 1);
	}
#endif				/* CONFIG_STA_SUPPORT */
		...
		for (;;) {
			...
			if ((size_t) (offset - cfgData) < fileLen) {
				...
				if ((strncmp(pTempStr, "SSID=", strlen("SSID="))
				     == 0)
				    || strncmp(pTempStr, "SSID1=",
					       strlen("SSID1=")) == 0
				    || strncmp(pTempStr, "SSID2=",
					       strlen("SSID2=")) == 0
				    || strncmp(pTempStr, "SSID3=",
					       strlen("SSID3=")) == 0
				    || strncmp(pTempStr, "SSID4=",
					       strlen("SSID4=")) == 0) {
					...
					WscWriteSsidToDatFile(pAd, pTempStr,
					 ...
					 (pTempStr, "NetworkType=",
					  strlen("NetworkType=")) == 0) {
					NdisZeroMemory(pTempStr, 512);
					if (pAd->StaCfg.BssType == BSS_ADHOC)
					...
					if (CurOpMode == STA_MODE) {
						USHORT auth_flag =
						    WscGetAuthType(pAd->StaCfg.
								   AuthMode);
						snprintf(pTempStr, 512, "%s%s",
							 pTempStr,
							 WscGetAuthTypeStr
							 (auth_flag));
					}
				    ...
				    if (strncmp
					(pTempStr, "WscConfMode=",
					 strlen("WscConfMode=")) == 0) {
					snprintf(pTempStr, 512, "WscConfMode=");
#ifdef CONFIG_AP_SUPPORT
					for (index = 0;
					     index < pAd->ApCfg.BssidNum;
					     index++) {
						pWscControl =
				    ...
				    if (strncmp
					(pTempStr, "WscConfStatus=",
					 strlen("WscConfStatus=")) == 0) {
					snprintf(pTempStr, 512,
						 "WscConfStatus=");
#ifdef CONFIG_AP_SUPPORT
					for (index = 0;
				    ...
				    if (strncmp
					(pTempStr, "DefaultKeyID=",
					 strlen("DefaultKeyID=")) == 0) {
					NdisZeroMemory(pTempStr, 512);
					snprintf(pTempStr, 512,
						 "DefaultKeyID=");
#ifdef CONFIG_AP_SUPPORT
					...
					pCredentail =
					    &pAd->ApCfg.MBSSID[apidx].
					    WscControl.WscProfile.Profile[0];
					if (pAd->ApCfg.MBSSID[apidx].
					    WepStatus == Ndis802_11WEPEnabled) {
						UCHAR idx = 0, KeyType[4] =
						    { 0 };
					...
					if (pAd->ApCfg.MBSSID[apidx].
					 ...
					 (pTempStr, (PSTRING) WepKeyFormatName)
					 && (CurOpMode == STA_MODE)) {
					pCredentail =
					    &pWscControl->WscProfile.
					    Profile[pWscControl->WscProfile.
						    ApplyProfileIdx];
				    ...
				    if (rtstrstr(pTempStr, (PSTRING) WepKeyName)
					&& (CurOpMode == STA_MODE)) {
					pCredentail =
					    &pWscControl->WscProfile.
					    Profile[pWscControl->WscProfile.
...
void WscWriteConfToAR9File(IN PRTMP_ADAPTER pAd, IN UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		snprintf((PSTRING) WepKeyName, sizeof(WepKeyName), "Key%dStr1=",
			 pAd->ApCfg.MBSSID[MAIN_MBSSID].DefaultKeyId + 1);
		snprintf((PSTRING) WepKeyFormatName, sizeof(WepKeyFormatName),
			 "Key%dType=",
			 pAd->ApCfg.MBSSID[MAIN_MBSSID].DefaultKeyId + 1);
	}
		...
		/*for (;;) */
		{
			...
			NdisZeroMemory(pDatStr, 4096);
			{
...

				{
					...
					NdisMoveMemory(pDatStr + datoffset,
						       pTempStr, offset);
					datoffset += offset;
				}
				...
				if (CurOpMode == AP_MODE) {
					for (index = 1; index <= 4; index++) {
						snprintf(WepKeyFormatName,
							 sizeof
							 (WepKeyFormatName),
							 "Key%dType=", index);
						/*if (rtstrstr(pTempStr, WepKeyFormatName)) */
						{
							NdisZeroMemory(pTempStr,
								       512);
							offset = 0;
							NdisMoveMemory(pTempStr,
								       WepKeyFormatName,
...
BOOLEAN WscThreadExit(RTMP_ADAPTER * pAd)
{
	...
	if ((pAd->OpMode == OPMODE_AP)
		...
		for (ap_idx = 0; ap_idx < MaxBssidNum; ap_idx++) {
			PWSC_CTRL pWpsCtrl =
			    &pAd->ApCfg.MBSSID[ap_idx].WscControl;
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		NdisFreeSpinLock(&pAd->StaCfg.WscControl.
				 WscConfiguredPeerListSemLock);
#endif				/* IWSC_SUPPORT */
	}
#endif				/* CONFIG_STA_SUPPORT */
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER * pAd)
{
	...
	status =
	    RtmpOSTaskAttach(pTask, wsc_write_dat_file_thread,
			     (ULONG) & pAd->wscTask);
...
*/
#ifdef CONFIG_AP_SUPPORT
extern INT Set_AP_WscMode_Proc(IN PRTMP_ADAPTER pAd, IN PUCHAR arg);
extern INT Set_AP_WscGetConf_Proc(IN PRTMP_ADAPTER pAd, IN PUCHAR arg);
#endif				/* CONFIG_AP_SUPPORT */
	...
	default:
		{
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: Incorrect encryption types (%d)\n",
				  __FUNCTION__,
				  pWscControl->WscProfile.Profile[currentIdx].
				  EncrType));
			ASSERT(FALSE);
			break;
		}
		...
		if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
			...
			if (DevicePasswordID == DEV_PASS_ID_PBC) {	/* Check for PBC value */
				...
				if ((pAd->OpMode == OPMODE_AP)
					...
					UCHAR ap_idx = 0;
					for (ap_idx = 0;
					     ap_idx < pAd->ApCfg.BssidNum;
					     ap_idx++) {
						if (NdisEqualMemory
						    (Fr->Hdr.Addr1,
						     pAd->ApCfg.MBSSID[ap_idx].
						     Bssid, MAC_ADDR_LEN))
							break;
					}
				...
				if (pWscCtrl &&
					...
					RTMP_SEM_LOCK(&pWscCtrl->
						      WscPeerListSemLock);
					WscInsertPeerEntryByMAC(&pWscCtrl->
								WscPeerList,
								Fr->Hdr.Addr2);
					RTMP_SEM_UNLOCK(&pWscCtrl->
							WscPeerListSemLock);
				}
			}
#ifdef IWSC_SUPPORT
		...
		if (pAd->StaCfg.BssType == BSS_ADHOC) {
			...
			if ((be2cpu16(pWscIE->Type) == WSC_ID_CONFIG_METHODS) &&
				...
				RTMP_SEM_LOCK(&pWscCtrl->
					      WscConfiguredPeerListSemLock);
				pWscPeerEntry =
				    WscFindPeerEntry(&pWscCtrl->
						     WscConfiguredPeerList,
						     Fr->Hdr.Addr2);
				if (pWscPeerEntry == NULL) {
					NdisMoveMemory(&PeerConfigMethod,
						       pData + 4,
						       sizeof
						       (PeerConfigMethod));
					PeerConfigMethod =
					...
					MlmeEnqueue(pAd, IWSC_STATE_MACHINE,
						    IWSC_MT2_PEER_PROBE_REQ,
						    sizeof(USHORT),
						    &PeerConfigMethod, 0);
					RTMP_MLME_HANDLER(pAd);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("PeerProbeReqSanity : Add this peer: %02x:%02x:%02x:%02x:%02x:%02x\n",
						  PRINT_MAC(Fr->Hdr.Addr2)));
				}
				RTMP_SEM_UNLOCK(&pWscCtrl->
						WscConfiguredPeerListSemLock);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("PeerProbeReqSanity : PeerConfigMethod = 0x%04x\n",
					  PeerConfigMethod));
			}
		}
#endif				/* IWSC_SUPPORT */
...
PWSC_PEER_ENTRY WscFindPeerEntry(PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr)
{
	...
	while (pPeerEntry != NULL) {
		if (NdisEqualMemory
		    (pPeerEntry->mac_addr, pMacAddr, MAC_ADDR_LEN))
			return pPeerEntry;
...
VOID WscInsertPeerEntryByMAC(PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr)
{
	PWSC_PEER_ENTRY pWscPeer = NULL;
	...
	if (pWscPeer) {
		...
		os_alloc_mem(NULL, (UCHAR **) & pWscPeer,
			     sizeof(WSC_PEER_ENTRY));
		if (pWscPeer) {
...
INT WscStaShowPeerList(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pWscEnList->size != 0) {
		...
		while (pPeerEntry != NULL) {
			printk
			    ("MAC:%02x:%02x:%02x:%02x:%02x:%02x\tReveive Time:%lu\n",
			     pPeerEntry->mac_addr[0], pPeerEntry->mac_addr[1],
			     pPeerEntry->mac_addr[2], pPeerEntry->mac_addr[3],
			     pPeerEntry->mac_addr[4], pPeerEntry->mac_addr[5],
			     pPeerEntry->receive_time);
			pListEntry = pListEntry->pNext;
			pPeerEntry = (PWSC_PEER_ENTRY) pListEntry;
		}
...
VOID WscMaintainPeerList(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWpsCtrl)
{
	...
	while (pPeerEntry != NULL) {
		if (RTMP_TIME_AFTER
		    (now_time, pPeerEntry->receive_time + (30 * OS_HZ))) {
