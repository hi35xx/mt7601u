			...
			IN BOOLEAN bFromApcli, IN UCHAR apidx)
{
	...
	if (apidx >= MIN_NET_DEVICE_FOR_P2P_GO)
		NdisMoveMemory(&macAddr[0], pAd->P2PCurrentAddress,
			       MAC_ADDR_LEN);
...
static UCHAR RT_CfgMbssWirelessModeMaxGet(RTMP_ADAPTER * pAd)
{
	...
	if (mode_str) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s(): Combined WirelessMode = %s(0x%x)\n",
			  __FUNCTION__, mode_str, wmode));
		os_free_mem(pAd, mode_str);
	}
	return wmode;
...
//#endif /* CONFIG_PM */        
	...
	case CMD_RTPRIV_IOCTL_FREQ_LIST_GET:
		{
			...
			for (i = 1; i <= pAd->ChannelListNum; i++) {
				m = 2412000;
				MAP_CHANNEL_ID_TO_KHZ(pAd->ChannelList[i - 1].
						      Channel, m);
				(*pFreq) = m;
				pFreq++;
			}
	...
	case CMD_RTPRIV_IOCTL_VIRTUAL_INF_UP:
		/* interface up */
		{
			...
			UINT32 ret;
			DBGPRINT(RT_DEBUG_ERROR,
				 ("================> UP : RTMP_SEM_EVENT_WAIT(%s)\n",
				  DRIVER_ROLE));
			RTMP_SEM_EVENT_WAIT(&pAd->android_resume_lock, ret);
			if (ret != 0) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s : UP reg_atomic get failed(ret=%d)\n",
					  DRIVER_ROLE, ret));
				return NDIS_STATUS_FAILURE;
			}
			DBGPRINT(RT_DEBUG_TRACE,
				 ("================> %s : UP: VIRTUAL_IF_NUM == %lu\n",
				  DRIVER_ROLE, VIRTUAL_IF_NUM(pAd)));
			if (VIRTUAL_IF_NUM(pAd) == 0) {

				if (!RTMP_TEST_FLAG
				    (pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)) {
					if (pInfConf->
					    rt28xx_open(pAd->net_dev) != 0) {
						DBGPRINT(RT_DEBUG_TRACE,
							 (" %s : rt28xx_open return fail!\n",
							  DRIVER_ROLE));
						RTMP_SEM_EVENT_UP(&pAd->
								  android_resume_lock);
						return NDIS_STATUS_FAILURE;
					}
					VIRTUAL_IF_INC(pAd);
				...
				pAd->ApCfg.MBSSID[MAIN_MBSSID].bBcnSntReq =
				    TRUE;
#endif				/* RT_CFG80211_SUPPORT */
				{
					extern VOID APMakeAllBssBeacon(IN
								       PRTMP_ADAPTER
								       pAd);
					extern VOID APUpdateAllBeaconFrame(IN
									   PRTMP_ADAPTER
									   pAd);
					APMakeAllBssBeacon(pAd);
					APUpdateAllBeaconFrame(pAd);
	...
	case CMD_RTPRIV_IOCTL_VIRTUAL_INF_DOWN:
		/* interface down */
		{
			...
			RT_CMD_INF_UP_DOWN *pInfConf =
			    (RT_CMD_INF_UP_DOWN *) pData;
			DBGPRINT(RT_DEBUG_ERROR,
				 ("================> DOWN : RTMP_SEM_EVENT_WAIT(%s)\n",
				  DRIVER_ROLE));
			RTMP_SEM_EVENT_WAIT(&pAd->android_resume_lock, ret);
			if (ret != 0) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s : DOWN reg_atomic get failed(ret=%d)\n",
					  DRIVER_ROLE, ret));
				return NDIS_STATUS_FAILURE;
			}
			DBGPRINT(RT_DEBUG_TRACE,
				 ("================> %s : DOWN : VIRTUAL_IF_NUM == %lu\n",
				  DRIVER_ROLE, VIRTUAL_IF_NUM(pAd)));
//                      VIRTUAL_IF_DEC(pAd);
		...
		/* get statistics */
		{
			...
			if (pAd->OpMode == OPMODE_STA) {
				...
				for (index = 0; index < MAX_MBSSID_NUM(pAd);
				     index++) {
					if (pAd->ApCfg.MBSSID[index].MSSIDDev ==
				...
				pStats->rx_bytes =
				    pAd->ApCfg.MBSSID[index].ReceivedByteCount;
				pStats->tx_bytes =
				    pAd->ApCfg.MBSSID[index].
				    TransmittedByteCount;
				pStats->rx_errors =
				    pAd->ApCfg.MBSSID[index].RxErrorCount;
				pStats->tx_errors =
				    pAd->ApCfg.MBSSID[index].TxErrorCount;
	...
	case CMD_RTPRIV_IOCTL_INF_IW_STATUS_GET:
		/* get wireless statistics */
		{
				...
				if ((pStats->priv_flags == INT_APCLI)
					...
					INT ApCliIdx =
					    ApCliIfLookUp(pAd,
							  ...
							  dev_addr);
					if ((ApCliIdx >= 0)
					    && VALID_WCID(pAd->ApCfg.
							  ApCliTab[ApCliIdx].
							  MacTabWCID))
						pMacEntry =
						    &pAd->MacTab.Content[pAd->
									 ApCfg.
									 ApCliTab
									 [ApCliIdx].
									 MacTabWCID];
			...
			if (CurOpMode == OPMODE_AP) {
				if (pMacEntry != NULL)
					pStats->qual =
					    ((pMacEntry->ChannelQuality * 12) /
					     10 + 10);
				else
					pStats->qual =
					    ((pAd->Mlme.ChannelQuality * 12) /
					     10 + 10);
			}
#endif				/* CONFIG_AP_SUPPORT */
...
#ifdef CONFIG_AP_SUPPORT
			pStats->noise =
			    RTMPMaxRssi(pAd, pAd->ApCfg.RssiSample.AvgRssi0,
	...
	case CMD_RTPRIV_IOCTL_SET_ANDROID_RESUME_LOCK:
		{
			...
			if (Data) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("=========================================>  %s : USB DISCONNECT LOCK\n",
					  DRIVER_ROLE));
				RTMP_SEM_EVENT_WAIT(&pAd->android_resume_lock,
						    ret);
				if (ret != 0) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("================> %s: reg_atomic get failed(ret=%d)\n",
						  DRIVER_ROLE, ret));
					return NDIS_STATUS_FAILURE;
				}
			} else {
	...
	case CMD_RTPRIV_IOCTL_SET_SUSPEND_FLAG:
		{
			...
			printk
			    ("==================================> %s : CMD_RTPRIV_IOCTL_SET_SUSPEND_FLAG %d\n",
	...
	case CMD_RTPRIV_IOCTL_SET_INIT_FLAG:
		if (Data == 0) {
			...
			RTMP_SET_EXT_FLAG(pAd, fRTMP_ADAPTER_EXT_INIT_ONGOING);
		}
		break;
	}
