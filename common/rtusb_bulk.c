		     ...
		     IN UCHAR BulkOutPipeId, IN usb_complete_t Func)
{
	...
	if (pTxContext->bAggregatible) {
		...
		RTUSB_FILL_TX_BULK_URB(pUrb,
				       pObj->pUsb_Dev,
				       pChipCap->
				       WMM0ACBulkOutAddr[BulkOutPipeId], pSrc,
		...
		RTUSB_FILL_TX_BULK_URB(pUrb,
				       ...
				       WMM0ACBulkOutAddr[BulkOutPipeId], pSrc,
				       pTxContext->BulkOutSize, Func,
				       pTxContext, pTxContext->data_dma);
	}
}
...
#ifdef USB_BULK_BUF_ALIGMENT
	pSrc =
	    &pTxContext->TransferBuffer[idx]->field.WirelessPacket[pTxContext->
								   NextBulkOutPosition];
#else
	pSrc =
			    ...
			    IN UCHAR BulkOutPipeId, IN UCHAR Index)
{
	...
	pWirelessPkt =
	    &pHTTXContext->TransferBuffer[idx]->field.WirelessPacket[0];
#else
...
/*				printk("data bulk out bLasAlignmentsectiontRound \n");*/
				...
				break;
				 ...
				 ||
				 ((ThisBulkSize & pAd->
				   BulkOutDataSizeLimit[BulkOutPipeId]) ==
...
*/
	...
	if (bLasAlignmentsectiontRound == TRUE) {
		CUR_WRITE_IDX_INC(pHTTXContext->NextBulkIdx,
...
VOID RTUSBBulkOutNullFrame(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->BulkOutPending[0] == TRUE)
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("RTUSBBulkOutNullFrame: IN STOP_TX STATUS\n"));
		return;
	}
