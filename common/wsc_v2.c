...
BOOLEAN WscAppendV2SubItem(IN UCHAR SubID,
			   IN PUCHAR pData,
			   IN UCHAR DataLen,
...
VOID WscOnOff(IN PRTMP_ADAPTER pAd, IN INT ApIdx, IN BOOLEAN bOff)
{
	PWSC_V2_INFO pWpsV2Info =
	    &pAd->ApCfg.MBSSID[ApIdx & 0x0F].WscControl.WscV2Info;
	if (bOff) {
		/*
		...
		if (pAd->ApCfg.MBSSID[ApIdx & 0x0F].WscControl.WscConfMode !=
		    WSC_DISABLE) {
			INT IsAPConfigured;
			IsAPConfigured =
			    pAd->ApCfg.MBSSID[ApIdx & 0x0F].WscControl.
			    WscConfStatus;
			  ...
			  IN INT ApIdx, IN PUCHAR pMacAddr)
{
	...
	for (i = 0; i < pACL->Num; i++) {
		if (NdisEqualMemory
		    (pACL->Entry[i].Addr, pMacAddr, MAC_ADDR_LEN))
			bFound = TRUE;
	}
	if (bFound == FALSE) {
			 ...
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	WscBuildBeaconIE(pAd,
			 ...
			 FALSE,
			 0,
			 0, (pWscControl->EntryIfIdx & 0xF), NULL, 0, AP_MODE);
	WscBuildProbeRespIE(pAd,
			    WSC_MSGTYPE_AP_WLAN_MGR,
			    pWscControl->WscConfStatus,
			    FALSE,
			    0, 0, pWscControl->EntryIfIdx, NULL, 0, AP_MODE);
		    ...
		    OUT UCHAR ** pOutBuf, OUT INT * pOutBufLen)
{
	...
	if (pWscV2Msg) {
		...
		templen =
		    AppendWSCTLV(WSC_ID_VENDOR_EXT, (*pOutBuf),
		       ...
		       IN PWSC_CTRL pWscControl, IN PMAC_TABLE_ENTRY pEntry)
{
	...
	if (IS_ENTRY_CLIENT(pEntry)) {
		...
		WscSendMessage(pAdapter, WSC_OPCODE_FRAG_ACK, NULL, 0,
			       pWscControl, AP_CLIENT_MODE, EAP_CODE_REQ);
}
