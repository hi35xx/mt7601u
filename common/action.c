...
extern VOID P2pPeerGoNegoReqAction(IN PRTMP_ADAPTER pAd,
				   IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerGoNegoRspAction(IN PRTMP_ADAPTER pAd,
				   IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerGoNegoConfirmAction(IN PRTMP_ADAPTER pAd,
				       IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerProvisionReqAction(IN PRTMP_ADAPTER pAd,
				      IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerProvisionRspAction(IN PRTMP_ADAPTER pAd,
				      IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerDeviceDiscRspAction(IN PRTMP_ADAPTER pAd,
				       IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerInvitesReqAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerInvitesRspAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM * Elem);

extern VOID P2pPeerDevDiscoverReqAction(IN PRTMP_ADAPTER pAd,
					IN MLME_QUEUE_ELEM * Elem);
#endif				/* P2P_SUPPORT */
				  ...
				  IN UCHAR apidx, IN UCHAR InfoReq)
{
	...
	ActHeaderInit(pAd, &Frame.Hdr, pAddr1, pAd->ApCfg.MBSSID[apidx].Bssid,
		      pAd->ApCfg.MBSSID[apidx].Bssid);
		...
		for (i = 0; i < pAd->CommonCfg.TriggerEventTab.EventANo; i++) {
			if (pAd->CommonCfg.TriggerEventTab.EventA[i].bValid ==
				...
				if (pAd->CommonCfg.TriggerEventTab.EventA[i].
					...
					pAd->CommonCfg.TriggerEventTab.
					    EventA[i].bValid = FALSE;
				}
				DBGPRINT(RT_DEBUG_ERROR,
...
VOID PeerPublicAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	case ACTION_BSS_2040_COEXIST:	/* Format defined in IEEE 7.4.7a.1 in 11n Draf3.03 */
		{
			...
			pBssCoexistIe =
			    (BSS_2040_COEXIST_IE *) (&pCoexistInfo->
						     BssCoexistIe);
#ifdef CONFIG_AP_SUPPORT
#ifdef P2P_SUPPORT
	...
	case ACTION_WIFI_DIRECT:
		...
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
		    NdisEqualMemory(&(Elem->Msg[LENGTH_802_11 + 2]),
				    IWSC_ACTION_OUI, 4)) {
			MlmeEnqueue(pAd, IWSC_STATE_MACHINE,
				    IWSC_MT2_PEER_ACTION_FRAME, Elem->MsgLen,
				    Elem->Msg, 0);
			RTMP_MLME_HANDLER(pAd);
			break;
		}
...
#ifdef P2P_SUPPORT
		DBGPRINT(RT_DEBUG_TRACE,
			 ("<---- Public. Vendor OUI+type = %08lx \n",
			  *(PULONG) & Elem->Msg[LENGTH_802_11 + 2]));
#ifdef RT_BIG_ENDIAN
		if (SWAP32(*(PUINT32) & Elem->Msg[LENGTH_802_11 + 2]) ==
		    P2P_OUI)
#else
...
#endif				/* RT_BIG_ENDIAN */
		{
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("!!!! Public action frames with Vendor specific OUI = WFAP2P.Subtype = %d. \n",
				  Subtype));
			...
			if (pAd->P2pCfg.P2pCounter.bStartScan) {
				...
				MlmeEnqueue(pAd, P2P_CTRL_STATE_MACHINE,
					    P2P_CTRL_DISC_CANL_EVT, 0, NULL, 0);
				RTMP_MLME_HANDLER(pAd);
			}
			switch (Subtype) {
			case GO_NEGOCIATION_REQ:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s : recv Group Nego. Req\n",
					  __FUNCTION__));
				P2pPeerGoNegoReqAction(pAd, Elem);
				break;
			case GO_NEGOCIATION_RSP:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s : recv Group Nego. Rsp\n",
					  __FUNCTION__));
				P2pPeerGoNegoRspAction(pAd, Elem);
				break;
			case GO_NEGOCIATION_CONFIRM:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s : recv Group Nego. Confirm\n",
					  __FUNCTION__));
			...
			default:
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Unknown Public action frames with Vendor specific OUI = WFAP2P.Subtype = %d \n",
					  Subtype));
				break;
			}
		}
	...
	case ACTION_TDLS_DISCOVERY_RSP:
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			if (INFRA_ON(pAd)) {
				TDLS_DiscoveryRspPublicAction(pAd, Elem,
							      Elem->Msg,
							      Elem->MsgLen);
			}
		}
						   ...
						   IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (VALID_WCID(Elem->Wcid)) {
		...
		ActHeaderInit(pAd, &HTINFOframe.Hdr, pAddr, pEntry->HdrAddr2,
			      pEntry->HdrAddr3);
	}
#else
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (IS_ENTRY_APCLI(&pAd->MacTab.Content[Elem->Wcid])) {
			apidx =
			    pAd->MacTab.Content[Elem->Wcid].MatchAPCLITabIdx;
	...
	case NOTIFY_BW_ACTION:
		...
		if (Elem->Msg[LENGTH_802_11 + 2] == 0)	/* 7.4.8.2. if value is 1, keep the same as supported channel bandwidth. */
			...
			pEntry->HTPhyMode.field.BW =
			    pEntry->MaxHTPhyMode.field.BW & pAd->CommonCfg.
