...
NDIS_STATUS MlmeInit(IN PRTMP_ADAPTER pAd)
{
	...
	do {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			TDLS_StateMachineInit(pAd, &pAd->Mlme.TdlsMachine,
					      pAd->Mlme.TdlsFunc);
#endif				/* DOT11Z_TDLS_SUPPORT */

#ifdef WSC_STA_SUPPORT
#ifdef IWSC_SUPPORT
			IWSC_StateMachineInit(pAd, &pAd->Mlme.IWscMachine,
					      pAd->Mlme.IWscFunc);
#endif				/* IWSC_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			RTMPInitTimer(pAd,
				      ...
				      GET_TIMER_FUNCTION
				      (APQuickResponeForRateUpExec), pAd,
				      FALSE);
		...
		ASSERT(P2P_CTRL_FUNC_SIZE ==
		       P2P_CTRL_MAX_EVENTS * P2P_CTRL_MAX_STATES);
		P2PCtrlStateMachineInit(pAd, &pAd->P2pCfg.P2PCtrlMachine,
					pAd->P2pCfg.P2PCtrlFunc);

		/* P2P Discovery State Machine */
		ASSERT(P2P_DISC_FUNC_SIZE ==
		       P2P_DISC_MAX_EVENTS * P2P_DISC_MAX_STATES);
		P2PDiscoveryStateMachineInit(pAd, &pAd->P2pCfg.P2PDiscMachine,
					     pAd->P2pCfg.P2PDiscFunc);

		/* P2P Group Formation State Machine */
		ASSERT(P2P_GO_FORM_FUNC_SIZE ==
		       P2P_GO_NEGO_MAX_EVENTS * P2P_GO_FORM_MAX_STATES);
		P2PGoFormationStateMachineInit(pAd,
					       &pAd->P2pCfg.P2PGoFormMachine,
					       pAd->P2pCfg.P2PGoFormFunc);

		/* P2P Action Frame State Machine */
		ASSERT(P2P_ACTION_FUNC_SIZE == MAX_P2P_MSG * MAX_P2P_STATE);
		P2PStateMachineInit(pAd, &pAd->P2pCfg.P2PActionMachine,
				    pAd->P2pCfg.P2PActionFunc);

		/* P2P CTWindows timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pCTWindowTimer,
			      GET_TIMER_FUNCTION(P2PCTWindowTimer), pAd, FALSE);
		/* P2P SwNOA timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pSwNoATimer,
			      GET_TIMER_FUNCTION(P2pSwNoATimeOut), pAd, FALSE);
		/* P2P Presence Absent timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pPreAbsenTimer,
			      GET_TIMER_FUNCTION(P2pPreAbsenTimeOut), pAd,
			      FALSE);
		/* P2P WSC Timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pWscTimer,
			      GET_TIMER_FUNCTION(P2pWscTimeOut), pAd, FALSE);
		/* P2P Re-Transmit Action Frame Timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pReSendTimer,
			      GET_TIMER_FUNCTION(P2pReSendTimeOut), pAd, FALSE);
		/* P2P CLIENT Re-Connect Timer */
		RTMPInitTimer(pAd, &pAd->P2pCfg.P2pCliReConnectTimer,
			      GET_TIMER_FUNCTION(P2pCliReConnectTimeOut), pAd,
			      FALSE);
...
VOID MlmeHalt(IN PRTMP_ADAPTER pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (pAd->StaCfg.WscControl.WscProfileRetryTimerRunning) {
			pAd->StaCfg.WscControl.WscProfileRetryTimerRunning =
			    FALSE;
			RTMPCancelTimer(&pAd->StaCfg.WscControl.
					WscProfileRetryTimer, &Cancelled);
		}
#endif				/* WSC_STA_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (pAd->ApCfg.ApQuickResponeForRateUpTimerRunning == TRUE)
			RTMPCancelTimer(&pAd->ApCfg.
					ApQuickResponeForRateUpTimer,
					&Cancelled);
		...
		if (pAd->ApCfg.ApCliTab[BSS0].WscControl.
		    WscProfileRetryTimerRunning) {
			pAd->ApCfg.ApCliTab[BSS0].WscControl.
			    WscProfileRetryTimerRunning = FALSE;
			RTMPCancelTimer(&pAd->ApCfg.ApCliTab[BSS0].WscControl.
					WscProfileRetryTimer, &Cancelled);
		}
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
...
#endif				/* P2P_SUPPORT */
#ifdef RT_CFG80211_SUPPORT
			     || (pAd->cfg80211_ctrl.isCfgInApMode ==
	...
	if (pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE == 0) {
		...
		realavgrssi =
		    (pAd->RxAnt.
		     Pair1AvgRssi[pAd->RxAnt.Pair1PrimaryRxAnt] >> 3);
...
#if 0
		printk
		    ("rssi diff %d/%d/%d, packet num %d, lock %d, period %d\n",
		     realavgrssi - pAd->RxAnt.Pair1LastAvgRssi, realavgrssi,
		     pAd->RxAnt.Pair1LastAvgRssi, pAd->CommonCfg.nPacket_Cnt,
		     pAd->CommonCfg.bEvalLock, chk_period_cnt);
#endif

		if (pAd->chipCap.FlgSwBasedPPAD
...
#if 0
		printk
		    ("\x1b[31m%s: FlgSwBasedPPAD %d, RxAntDiversityCfg %d, bAntEvalEnable %d\x1b[m\n",
		     __FUNCTION__, pAd->chipCap.FlgSwBasedPPAD,
		     pAd->CommonCfg.RxAntDiversityCfg,
		     pAd->CommonCfg.bAntEvalEnable);
		printk
		    ("\x1b[32m%s: pri ant %d, rssi th %d, miss cnt %d, miss th %d\x1b[m\n",
		     __FUNCTION__,
		...
		if (((pAd->CommonCfg.bSWRxAntDiversity)
		     && (pAd->CommonCfg.RxAntDiversityCfg ==
			 ANT_SW_DIVERSITY_ENABLE) && (!pAd->EepromAccess))
		    || (pAd->CommonCfg.bAntEvalEnable))
			AsicAntennaSelect(pAd, pAd->MlmeAux.Channel);
		else if (pAd->CommonCfg.RxAntDiversityCfg == ANT_FIX_ANT0
			 || pAd->CommonCfg.RxAntDiversityCfg == ANT_FIX_ANT1) {
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			    APAsicAntennaAvg(pAd, pAd->RxAnt.Pair1PrimaryRxAnt,
					     &realavgrssi);
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
			realavgrssi =
...
#ifdef P2P_SUPPORT
	if (P2P_INF_ON(pAd))
		P2pPeriodicExec(SystemSpecific1, FunctionContext,
...
//              return;
	...
	if (P2P_CLI_ON(pAd) || P2P_GO_ON(pAd)
		...
		if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
			...
			if (pAd->CommonCfg.RxAntDiversityCfg !=
			    ANT_DIVERSITY_DISABLE) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("[%s]Close Antenna diversity\n",
					  __FUNCTION__));
				pAd->CommonCfg.RxAntDiversityCfg =
				    ANT_DIVERSITY_DISABLE;
...
VOID STAMlmePeriodicExec(PRTMP_ADAPTER pAd)
{
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) &&
		...
		if (NdisEqualMemory
			...
			UCHAR p2pindex;
			p2pindex =
			    P2pGroupTabSearch(pAd, pAd->P2pCfg.ConnectingMAC);
			if (p2pindex != P2P_NOT_FOUND) {
				if (pAd->P2pTable.Client[p2pindex].
				    P2pClientState > P2PSTATE_DISCOVERY_UNKNOWN)
					bCheckBeaconLost = FALSE;
			}
		}
#endif				/* P2P_SUPPORT */
		...
		if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_IDLE_RADIO_OFF)) {
			...
			if ((pAd->Antenna.field.TxPath == 1) &&
				...
				if ((pAd->RalinkCounters.
				    ...
				    && (pAd->RalinkCounters.
					OneSecOsTxCount[QID_AC_BE] < 50)
				    && (pAd->RalinkCounters.
					OneSecOsTxCount[QID_AC_VI] >= 1000)) {
...
/*===========================================================================*/
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_P2P_GO) {
			...
			auto_rate_cur_p =
			    &pAd->ApCfg.MBSSID[idx].bAutoTxRateSwitch;
			HtMcs =
			    pAd->ApCfg.MBSSID[idx].DesiredTransmitSetting.field.
			    MCS;
			break;
		}
#endif				/* P2P_SUPPORT */
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (idx < MAX_APCLI_NUM) {
				...
				pMaxHtPhy =
				    &pAd->ApCfg.ApCliTab[idx].MaxHTPhyMode;
				pMinHtPhy =
				    &pAd->ApCfg.ApCliTab[idx].MinHTPhyMode;

				auto_rate_cur_p =
				    &pAd->ApCfg.ApCliTab[idx].bAutoTxRateSwitch;
				HtMcs =
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if ((apidx < pAd->ApCfg.BssidNum) &&
				...
				pMaxHtPhy =
				    &pAd->ApCfg.MBSSID[apidx].MaxHTPhyMode;
				pMinHtPhy =
				    &pAd->ApCfg.MBSSID[apidx].MinHTPhyMode;

				auto_rate_cur_p =
				    &pAd->ApCfg.MBSSID[apidx].bAutoTxRateSwitch;
				HtMcs =
...
#ifdef MCAST_RATE_SPECIFIC
		{
			...
			if (memcmp
				...
				memmove(&pAd->CommonCfg.MCastPhyMode,
					...
					HTPhyMode, sizeof(HTTRANSMIT_SETTING));
			}
		}
...
VOID MlmeUpdateHtTxRates(IN RTMP_ADAPTER * pAd, IN UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (idx < MAX_APCLI_NUM) {
				pDesireHtPhy =
				    &pAd->ApCfg.ApCliTab[idx].DesiredHtPhyInfo;
				pActiveHtPhy =
				    &pAd->ApCfg.ApCliTab[idx].DesiredHtPhyInfo;
				pHtPhy = &pAd->ApCfg.ApCliTab[idx].HTPhyMode;
				pMaxHtPhy =
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if ((apidx < pAd->ApCfg.BssidNum)
			    && (apidx < HW_BEACON_MAX_NUM)) {
				pDesireHtPhy =
				    &pAd->ApCfg.MBSSID[apidx].DesiredHtPhyInfo;
				pActiveHtPhy =
				    &pAd->ApCfg.MBSSID[apidx].DesiredHtPhyInfo;
				pHtPhy = &pAd->ApCfg.MBSSID[apidx].HTPhyMode;
				pMaxHtPhy =
			...
			case IE_WAPI:
				if (NdisEqualMemory
					...
					snprintf((PSTRING) pBss->WapiIE.IE,
						 sizeof(pBss->WapiIE.IE),
						 "wapi_ie=%02x%02x", pEid->Eid,
						 pEid->Len);
					for (idx = 0; idx < pEid->Len; idx++) {
						snprintf((PSTRING) pBss->WapiIE.
							 IE,
							 sizeof(pBss->WapiIE.
								IE), "%s%02x",
							 pBss->WapiIE.IE,
		...
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE) {
						   ...
						   fOP_AP_STATUS_MEDIA_STATE_CONNECTED))
			{
				if (MAC_ADDR_EQUAL
				    ...
				    || SSID_EQUAL(pAd->Cfg_pending_Ssid,
						  pAd->Cfg_pending_SsidLen,
						  ie_list->Ssid,
						  ie_list->SsidLen)
#endif				/* RT_CFG80211_SUPPORT */
				    ) {
					...
					NdisZeroMemory(pAd->Cfg_pending_Ssid,
						       MAX_LEN_OF_SSID);
#endif				/* RT_CFG80211_SUPPORT */
				}
				return Idx;
		      ...
		      OUT BSS_TABLE * OutTab, IN CHAR Ssid[], IN UCHAR SsidLen)
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if ((pInBss->BssType == pAd->StaCfg.BssType) &&
			...
			NdisMoveMemory(pAd->MlmeAux.Ssid, pInBss->Ssid,
				       pInBss->SsidLen);
		...
		case IE_WAPI:
			...
			while (Count > 0) {
				...
				if (TmpCipher > pBss->WAPI.PairCipher) {
					/* Move the lower cipher suite to PairCipherAux */
					pBss->WAPI.PairCipherAux =
					    pBss->WAPI.PairCipher;
			   ...
			   IN VOID * Msg, IN UCHAR Signal, IN UCHAR OpMode)
{
...
#endif				/* P2P_SUPPORT */
	{
		...
		do {
			...
			if (!MAC_ADDR_EQUAL
			    (pFrame->Hdr.Addr1, pAd->CurrentAddress) &&
#ifdef P2P_SUPPORT
			    !P2P_GO_ON(pAd) &&
#endif				/* P2P_SUPPORT */
			...
			if (!MAC_ADDR_EQUAL
			    ...
			    && MAC_ADDR_EQUAL(pAd->ApCliMlmeAux.Bssid,
					      pFrame->Hdr.Addr2)) {
				if (ApCliMsgTypeSubst
				    (pAd, pFrame, &Machine, &MsgType))
					break;
			} else {
				if (APMsgTypeSubst
				    (pAd, pFrame, &Machine, &MsgType))
			       ...
			       IN MLME_QUEUE_ELEM * Elem, IN ULONG CurrState)
{
	...
	if (S->TransFunc[(CurrState) * S->NrMsg + Elem->MsgType - S->Base])
		(*
		 (S->
		  TransFunc[(CurrState) * S->NrMsg + Elem->MsgType -
			    S->Base])) (pAd, Elem);
}
			  ...
			  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (((pAd->CommonCfg.bSWRxAntDiversity)
...
#endif				/* RELEASE_EXCLUDE */
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			APAsicAntennaAvg(pAd, pAd->RxAnt.Pair1SecondaryRxAnt,
					 &realrssi);
			if ((recv != 0)
			    && (realrssi >= (pAd->RxAnt.Pair1LastAvgRssi + 1)))
				bSwapAnt = TRUE;
		}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		...
		if (pAd->CommonCfg.bAntEvalEnable) {
			...
			rssi_diff =
			    (pAd->RxAnt.
			     Pair1AvgRssi[pAd->RxAnt.
					  Pair1SecondaryRxAnt] >> 3) -
			    (pAd->RxAnt.
			     Pair1AvgRssi[pAd->RxAnt.Pair1PrimaryRxAnt] >> 3);
			cck_snr_diff =
			    pAd->RxAnt.CCK_SNR[pAd->RxAnt.Pair1SecondaryRxAnt] -
			    pAd->RxAnt.CCK_SNR[pAd->RxAnt.Pair1PrimaryRxAnt];
			ofdm_snr_diff =
			    pAd->RxAnt.OFDM_SNR[pAd->RxAnt.
						Pair1SecondaryRxAnt] -
			    pAd->RxAnt.OFDM_SNR[pAd->RxAnt.Pair1PrimaryRxAnt];
			crc_diff =
			    pAd->RxAnt.CRC_Cnt[pAd->RxAnt.Pair1SecondaryRxAnt] -
			    pAd->RxAnt.CRC_Cnt[pAd->RxAnt.Pair1PrimaryRxAnt];

			/* if two antenns all received OFDM packets, please OFDM SNR */
			if (pAd->RxAnt.OFDM_Cnt[0] > 0
			    && pAd->RxAnt.OFDM_Cnt[1] > 0) {
				snr_diff = ofdm_snr_diff;
				/* only one packet need to add delta to cancel the noise */
				if (pAd->RxAnt.OFDM_Cnt[0] == 1
				    || pAd->RxAnt.OFDM_Cnt[1] == 1)
					snr_diff -= 2;
			} else {
				snr_diff = cck_snr_diff;
				/* only one packet need to add delta to cancel the noise */
				if (pAd->RxAnt.CCK_Cnt[0] == 1
				    || pAd->RxAnt.CCK_Cnt[1] == 1)
					snr_diff -= 1;
			...
			/* only one packet need to add delta to cancel the noise */
			if ((pAd->RxAnt.OFDM_Cnt[0] + pAd->RxAnt.CCK_Cnt[0]) ==
			    1
			    || (pAd->RxAnt.OFDM_Cnt[1] +
				pAd->RxAnt.CCK_Cnt[1]) == 1)
				rssi_diff -= 3;
		...
		if (bSwapAnt == TRUE) {
			...
			pAd->RxAnt.Pair1LastAvgRssi =
			    (pAd->RxAnt.
			     Pair1AvgRssi[pAd->RxAnt.Pair1SecondaryRxAnt] >> 3);
			...
			if (pAd->CommonCfg.bAntEvalEnable) {
				...
				RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R152,
							    &BBPValue);
				BBPValue &= ~0x80;
				BBPValue |= ant;	/* set ant */
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R152,
							     BBPValue);
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("AsicRxAntEvalAction::After Eval(fix in #%d), <%d, %d>, RcvPktNumWhenEvaluate=%ld\n",
				  pAd->RxAnt.Pair1PrimaryRxAnt, last_rssi,
				  realrssi, recv));
		}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		DBGPRINT(RT_DEBUG_TRACE,
			 ("AsicRxAntEvalAction::After Eval(fix in #%d), <%d, %d>, RcvPktNumWhenEvaluate=%ld\n",
			  pAd->RxAnt.Pair1PrimaryRxAnt,
			  (pAd->RxAnt.Pair1AvgRssi[0] >> 3),
			  (pAd->RxAnt.Pair1AvgRssi[1] >> 3),
			  pAd->RxAnt.RcvPktNumWhenEvaluate));
#endif				/* CONFIG_STA_SUPPORT */
	} else
#endif				/* ANT_DIVERSITY_SUPPORT */
	{
					   ...
					   IN PMAC_TABLE_ENTRY pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		...
		if (IS_ENTRY_CLIENT(pEntry))
			result =
			    pAd->ApCfg.MBSSID[pEntry->apidx].bAutoTxRateSwitch;
#ifdef APCLI_SUPPORT
...
UCHAR RTMPStaFixedTxMode(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		...
		if (IS_ENTRY_CLIENT(pEntry))
			tx_mode =
			    (UCHAR) pAd->ApCfg.MBSSID[pEntry->apidx].
			    DesiredTransmitSetting.field.FixedTxMode;
#ifdef APCLI_SUPPORT
		else if (IS_ENTRY_APCLI(pEntry))
			tx_mode =
			    (UCHAR) pAd->ApCfg.ApCliTab[pEntry->
							MatchAPCLITabIdx].
			    DesiredTransmitSetting.field.FixedTxMode;
#endif				/* APCLI_SUPPORT */
	}
...
VOID RTMPAcsRssi(IN PRTMP_ADAPTER pAd, IN RSSI_SAMPLE * pRssi)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("!![%s]pRssi->OldAcsRssi[%d] = %d\n", __FUNCTION__, 0,
		  pRssi->OldAcsRssi[0] >> 8));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("!!pRssi->OldAcsRssi[%d] = %d\n", 1,
		  pRssi->OldAcsRssi[1] >> 8));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("!!pRssi->OldAcsRssi[%d] = %d\n", 2,
		  pRssi->OldAcsRssi[2] >> 8));
#endif
}
#endif				/*DYNAMIC_PD_SUPPORT */
