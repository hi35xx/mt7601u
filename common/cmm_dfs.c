...
static NewDFSTable NewDFSTable1[] = {
	{
	 ...
	 NEW_DFS_FCC,
	 {
	  ...
	  {3, 2, 200, 20, 150, 900, 2200, 50, 1000, 999999999, 200, 0,
	   999999999},
	  }
	 },
...
static void dfs_sw_init(IN PRTMP_ADAPTER pAd);
...
static BOOLEAN DfsEventDataFetch(IN PRTMP_ADAPTER pAd,
				 IN PUCHAR pData, OUT PDFS_EVENT pDfsEvent);

static VOID DfsCheckBusyIdle(IN PRTMP_ADAPTER pAd);

static BOOLEAN DfsChannelCheck(IN PRTMP_ADAPTER pAd, IN UINT8 DfsChannel);

static VOID ChannelSelectOnRadarDetection(IN PRTMP_ADAPTER pAd);

static BOOLEAN DfsEventDrop(IN PRTMP_ADAPTER pAd, IN PDFS_EVENT pDfsEvent);
...
static inline BOOLEAN NewRadarDetectionMcuStart(PRTMP_ADAPTER pAd)
{
	 ...
	 */
	return AsicSendCommandToMcu(pAd, DFS_ONOFF_MCU_CMD, 0xff, 0x01, 0x01,
				    FALSE);
}
...
static inline BOOLEAN NewRadarDetectionMcuStop(PRTMP_ADAPTER pAd)
{
	/*
	   8051 firmware don't care parameter Token, Arg0 and Arg1
	 */
	return AsicSendCommandToMcu(pAd, DFS_ONOFF_MCU_CMD, 0xff, 0x01, 0x00,
				    FALSE);
}

#ifdef RTMP_MAC_USB
static VOID SwCheckDfsEventWithFw(IN PRTMP_ADAPTER pAd);
#endif				/*RTMP_MAC_USB  */
...
INT Set_DfsLowerLimit_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PRADAR_DETECT_STRUCT pRadarDetect = &pAd->CommonCfg.RadarDetect;
...
INT Set_DfsUpperLimit_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PRADAR_DETECT_STRUCT pRadarDetect = &pAd->CommonCfg.RadarDetect;
...
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
	...
	if (simple_strtol(arg, 0, 10) == 0) {
		NewRadarDetectionStart(pAd);
		   ...
		   && (simple_strtol(arg, 0, 10) <=
		       pAd->CommonCfg.RadarDetect.EnabledChMask)) {
...
INT Set_Fcc5Thrd_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_ChBusyThrd_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	int i;
	PUCHAR p1 = arg, p2;
	ULONG value;
...
INT Set_RssiThrd_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	int i;
	PUCHAR p1 = arg, p2;
	ULONG value;
...
INT Set_Ch0LErr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_DeclareThres_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_CheckLoop_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_MaxPeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_PeriodErr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_Ch0HErr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_Ch1Shift_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_Ch2Shift_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
#ifdef DFS_DEBUG
INT Set_CEPrintDebug_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	int i;
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
...
INT Set_RadarSim_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
	int i = 0, id = 0;
	...
	for (i = 0; i < 10; i++) {
		pDfsSwParam->DFS_W[id][pDfsSwParam->dfs_w_idx[id]].counter =
		    pDfsSwParam->dfs_w_counter;
		pDfsSwParam->DFS_W[id][pDfsSwParam->dfs_w_idx[id]].timestamp =
		    dfs_data[2 * i + 1];
		pDfsSwParam->DFS_W[id][pDfsSwParam->dfs_w_idx[id]].width =
		    dfs_data[2 * i];
...
INT Set_EventExpire_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_PROGRAM_PARAM pDfsProgramParam =
	    &pAd->CommonCfg.RadarDetect.DfsProgramParam;
	...
	if ((SubStr = strchr(StrBuf, '_')) != NULL) {
		*SubStr = '\0';
		...
		pDfsProgramParam->RadarEventExpire[dfs_channel] =
		    EventExpiration;
...
INT Set_CEPrint_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam =
	    &pAd->CommonCfg.RadarDetect.DfsSwParam;
	...
	if ((id >= 0) && (id <= 3)) {
		...
		for (i = 0; i < NEW_DFS_DBG_PORT_ENT_NUM; i++) {
			printk("Cnt=%u, w= %u  Time=%u\n",
			       (unsigned int)(pDfsSwParam->DFS_W[id][i].
					      counter),
	...
	if ((id >= 10) && (id < 13)) {
		...
		for (i = 0; i < NEW_DFS_MPERIOD_ENT_NUM; i++) {
			printk("T=%u, w1= %u(%u), w2= %u(%u)\n",
			       ...
			       pDfsSwParam->DFS_T[id][i].width,
			       pDfsSwParam->DFS_T[id][i].idx,
			       pDfsSwParam->DFS_T[id][i].width2,
			       pDfsSwParam->DFS_T[id][i].idx2);
	...
	if (id == 5) {
		...
		for (i = 0; i < NEW_DFS_DBG_PORT_ENT_NUM; i++) {
			printk
			    ("Cnt=%u, w= %u  Time=%lu,  start_idx = %u end_idx = %u\n",
			     (unsigned int)(pDfsSwParam->CE_DebugCh0[i].
					    counter),
			     (unsigned int)pDfsSwParam->CE_DebugCh0[i].width,
			     pDfsSwParam->CE_DebugCh0[i].timestamp,
			     pDfsSwParam->CE_DebugCh0[i].start_idx,
			     pDfsSwParam->CE_DebugCh0[i].end_idx);
		}

		for (i = 0; i < NEW_DFS_MPERIOD_ENT_NUM; i++) {
			printk("T=%lu, w1= %u(%u), w2= %u(%u)\n",
			       (pDfsSwParam->CE_TCh0[i].period),
			       pDfsSwParam->CE_TCh0[i].width,
			       pDfsSwParam->CE_TCh0[i].idx,
			       pDfsSwParam->CE_TCh0[i].width2,
			       pDfsSwParam->CE_TCh0[i].idx2);
		}
...
INT Set_RfReg_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT Show_BlockCh_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < pAd->ChannelListNum; i++) {
		if (pAd->ChannelList[i].RemainingTimeForUse != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Ch%d: RemainingTimeForUse:%d sec;\n",
				  pAd->ChannelList[i].Channel,
				  pAd->ChannelList[i].RemainingTimeForUse));
		}
	}
...
void NewRadarDetectionStart(PRTMP_ADAPTER pAd)
{
	...
	if (NewRadarDetectionMcuStart(pAd) == FALSE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("NewRadarDetectionMcuStart failed.\n"));
	}
#endif				/* RTMP_MAC_USB */
...
VOID NewRadarDetectionStop(IN PRTMP_ADAPTER pAd)
{
	...
	if (NewRadarDetectionMcuStop(pAd) == FALSE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("NewRadarDetectionMcuStop failed.\n"));
	}
#endif				/* RTMP_MAC_USB */
...
int SWRadarCheck(IN PRTMP_ADAPTER pAd, USHORT id)
{
	...
	USHORT Total, SwIdxPlus =
	    ENTRY_PLUS(pDfsSwParam->sw_idx[id], 1, NEW_DFS_DBG_PORT_ENT_NUM);
	UCHAR CounterToCheck;

	if (!DFS_CHECK_FLAGS(pAd, pRadarDetect) || (SwIdxPlus == pDfsSwParam->hw_idx[id]))	/* no entry to process */
		return 0;
	...
	if (pDfsSwParam->hw_idx[id] > SwIdxPlus)
		...
		Total =
		    pDfsSwParam->hw_idx[id] + NEW_DFS_DBG_PORT_ENT_NUM -
		    SwIdxPlus;

	if (Total > NEW_DFS_DBG_PORT_ENT_NUM)
		pDfsSwParam->pr_idx[id] =
		    ENTRY_PLUS(pDfsSwParam->sw_idx[id], MAX_PROCESS_ENTRY,
			       NEW_DFS_DBG_PORT_ENT_NUM);
	else
		pDfsSwParam->pr_idx[id] =
		    ENTRY_PLUS(pDfsSwParam->sw_idx[id], Total,
			       NEW_DFS_DBG_PORT_ENT_NUM);

	start_idx =
	    ENTRY_PLUS(pDfsSwParam->pr_idx[id], 1, NEW_DFS_DBG_PORT_ENT_NUM);
	...
	if (pAd->CommonCfg.RDDurRegion != CE) {
		...
		for (i = end_idx; i > start_idx; i--) {
			pCurrent =
			    &pDfsSwParam->DFS_W[id][i & NEW_DFS_DBG_PORT_MASK];
					...
					if (id == 0) {
						if (widthsum < 600)
							...
							pDfsSwParam->
							    ...
							    widthsum >>
							    pDfsSwParam->
							    dfs_width_diff_ch2_Shift;
					} else if (id == 1)
						pDfsSwParam->dfs_width_diff =
						    widthsum >> pDfsSwParam->
						    dfs_width_diff_ch1_Shift;
					else if (id == 2)
						pDfsSwParam->dfs_width_diff =
						    widthsum >> pDfsSwParam->
						    dfs_width_diff_ch2_Shift;

					if ((pAd->Dot11_H.RDMode ==
					     RD_SILENCE_MODE)
					    ||
					    (PERIOD_MATCH
					     (p1->width, pCurrent->width,
					      pDfsSwParam->dfs_width_diff))) {
						if (p1->timestamp >=
						    pCurrent->timestamp)
							period =
		...
		for (i = end_idx; i > start_idx; i--) {
			...
			if (pCurrent->start_idx != 0xffff) {
				...
				for (idx[0] = pCurrent->start_idx;
				     idx[0] <= pCurrent->end_idx; idx[0]++) {

					pCE_T = &pDfsSwParam->DFS_T[id][idx[0]];

					p2 = &pDfsSwParam->DFS_W[id][pCE_T->
								     idx2];
					...
					for (idx[1] = p2->start_idx;
					     idx[1] <= p2->end_idx; idx[1]++) {
...

										{
											     ...
											     NEW_DFS_END)
											{
												...
												if (pDFSValidRadar->TLow) {
													if ((T1 > (pDFSValidRadar->TLow - pDFSValidRadar->TMargin)) && (T1 < (pDFSValidRadar->THigh + pDFSValidRadar->TMargin))) {
														radar_detected
														    =
														    1;
													}
												} else {
													if ((T1 > (pDFSValidRadar->T - pDFSValidRadar->TMargin)) && (T1 < (pDFSValidRadar->T + pDFSValidRadar->TMargin))) {
														radar_detected
														    =
														    1;
														break;
					...
					if (id <= 2) {
						if (IS_FCC_RADAR_1
							...
							if (PeriodMatched_fcc1 >
							    3) {
								DBGPRINT
								    (RT_DEBUG_TRACE,
								     ("PeriodMatched_fcc1 = %d (%d)\n",
								      PeriodMatched_fcc1,
								      id));
								radar_detected =
								    1;
					...
					if ((pRadarDetect->MCURadarRegion ==
					     NEW_DFS_JAP) && (id <= 2)) {
						if (IS_W56_RADAR_3
						    ((pAd->CommonCfg.
						      HtCapability.HtCapInfo.
							...
							if (PeriodMatched_w56_3
							    > 3) {
								DBGPRINT
								    (RT_DEBUG_TRACE,
								     ("PeriodMatched_w56_3 = %d (%d)\n",
								      PeriodMatched_w56_3,
								      id));
								radar_detected =
								    1;
					...
					if ((pRadarDetect->MCURadarRegion ==
					     NEW_DFS_JAP_W53) && (id <= 2)
					    &&
					    IS_W53_RADAR_2((pAd->CommonCfg.
							    HtCapability.
							    HtCapInfo.
							    ChannelWidth ==
							    BW_40), T[0])) {
						int loop, idx1,
						    PeriodMatched_W56_2 = 0;
		...
		for (j = i - 1; j > start_idx; j--) {
			...
			if (pAd->CommonCfg.HtCapability.HtCapInfo.
				...
				if ((pAd->Dot11_H.RDMode == RD_SILENCE_MODE) ||
					...
					if ((period >= (33333 - 20))
					    && (period <=
						pDfsSwParam->dfs_max_period))
						//if ((period >= (10000 - 2)) && (period <= pDfsSwParam->dfs_max_period))
					{

						/* add in period table */
						pDfsSwParam->
						    DFS_T[id][pDfsSwParam->
							      dfs_t_idx[id]].
						    idx =
						    (i & NEW_DFS_DBG_PORT_MASK);
						pDfsSwParam->
						    DFS_T[id][pDfsSwParam->
							      dfs_t_idx[id]].
						    width = pCurrent->width;
						pDfsSwParam->
						    DFS_T[id][pDfsSwParam->
							      dfs_t_idx[id]].
						    idx2 =
						    (j & NEW_DFS_DBG_PORT_MASK);
						pDfsSwParam->
						    DFS_T[id][pDfsSwParam->
							      dfs_t_idx[id]].
						    width2 = p1->width;
...
#endif
									printk
									    ("Radar Detected(CE), W=%d, T=%d (%d), period matched=%d\n",
									     (unsigned
									      int)
									     pCE_T->
									     width,
...
static BOOLEAN StagerRadarCheck(IN PRTMP_ADAPTER pAd, UINT8 dfs_channel)
{
	UINT T1 = 0, T2 = 0, T3 = 0, T_all = 0, F1, F2, F3 = 0, Fmax =
	    0, freq_diff_min, freq_diff_max;
	...
	if (Fmax > 295 && Fmax <= 405) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("StagerRadarCheck failed, T1=%d, T2=%d, T3=%d\n", T1,
			  T2, T3));
		return FALSE;
	}
	...
	if (dfs_stg2 == 1) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("StagerRadarCheck freq_diff_min=%d freq_diff_max=%d \n",
			  freq_diff_min, freq_diff_max));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("StagerRadarCheck dfs_stg2, dff=%d  \n", freq_diff));
		...
		if ((freq_diff >= freq_diff_min)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("StagerRadarCheck failed, F1=%d, F2=%d\n", F1,
				  F2));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("stg2 fail on S/W Freq confirmed\n"));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("StagerRadarCheck freq_diff_min=%d freq_diff_max=%d \n",
			  freq_diff_min, freq_diff_max));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("StagerRadarCheck dfs_stg3, dff_1=%d, dff_2=%d \n",
			  freq_diff_1, freq_diff_2));

		if ((freq_diff_1 >= freq_diff_min)
		    && (freq_diff_1 <= freq_diff_max)
		    && (freq_diff_2 >= freq_diff_min)
		    && (freq_diff_2 <= freq_diff_max))
			return TRUE;	/* S/W check success */
		else {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("StagerRadarCheck failed, F1=%d, F2=%d, F3=%d\n",
				  F1, F2, F3));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("stg3 fail on S/W Freq confirmed\n"));
			return FALSE;	/* S/W check fail */
		}
	}
...
static BOOLEAN ChirpRadarCheck(IN PRTMP_ADAPTER pAd)
{
	...
	if (delta <= (12 * (1 << 20))) {	/* 12 sec */
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ChirpRadarCheck failed, discard previous detection.\n"));
...
static VOID DfsCheckBusyIdle(IN PRTMP_ADAPTER pAd)
{
	...
	busy_delta =
	    pRadarDetect->busy_time -
	    pRadarDetect->ch_busy_sta[pRadarDetect->ch_busy_sta_index];
	idle_delta =
	    pRadarDetect->idle_time -
	    pRadarDetect->ch_idle_sta[pRadarDetect->ch_busy_sta_index];
	...
	if ((pRadarDetect->ch_idle_sum >> pRadarDetect->ch_busy_idle_ratio) <
	    pRadarDetect->ch_busy_sum) {

		if (!
		    (pRadarDetect->McuRadarDebug & RADAR_DEBUG_DONT_CHECK_BUSY))
			pRadarDetect->ch_busy = 1;
	} else {
		if (!
		    (pRadarDetect->
		     McuRadarDebug & RADAR_DEBUG_DONT_CHECK_RSSI)) {
			if ((pAd->ApCfg.RssiSample.AvgRssi0)
			    && (pAd->ApCfg.RssiSample.AvgRssi0 >
				pRadarDetect->DfsRssiHigh))
				pRadarDetect->ch_busy = 2;
			else if ((pAd->ApCfg.RssiSample.AvgRssi0)
				 && (pAd->ApCfg.RssiSample.AvgRssi0 <
...
static BOOLEAN DfsChannelCheck(IN PRTMP_ADAPTER pAd, IN UINT8 DfsChannel)
{
	...
	for (i = 0; i < pAd->chipCap.DfsEngineNum; i++) {
		...
		if (DfsChannel & (0x1 << i)) {
			...
			if (DfsSwCheckOnHwDetection(pAd, pDFS2Table, i, T, W) ==
			    FALSE)
				continue;
...
VOID NewRadarDetectionProgram(PRTMP_ADAPTER pAd, pNewDFSTable pDFS2Table)
{
	...
	for (idx = 0; idx < DfsEngineNum; idx++) {
		if ((pRadarDetect->DFSParamFromConfig & (0x1 << idx))
		    && pDfsProgramParam->
		    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) + idx].valid) {
			pDFS2Table->entry[idx].mode =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].mode;
			pDFS2Table->entry[idx].avgLen =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].avgLen;
			pDFS2Table->entry[idx].ELow =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].ELow;
			pDFS2Table->entry[idx].EHigh =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].EHigh;
			pDFS2Table->entry[idx].WLow =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].WLow;
			pDFS2Table->entry[idx].WHigh =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].WHigh;
			pDFS2Table->entry[idx].EpsilonW =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].EpsilonW;
			pDFS2Table->entry[idx].TLow =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].TLow;
			pDFS2Table->entry[idx].THigh =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].THigh;
			pDFS2Table->entry[idx].EpsilonT =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].EpsilonT;
			pDFS2Table->entry[idx].BLow =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].BLow;
			pDFS2Table->entry[idx].BHigh =
			    pDfsProgramParam->
			    NewDFSTableEntry[(TalbeIdx * DfsEngineNum) +
					     idx].BHigh;

			DBGPRINT(RT_DEBUG_TRACE,
				 ("TalbeIdx = %d; idx = %d; DFSParam = %2d; %3d; %3d; %3d; %3d; %4d; %3d; %6lu; %7lu; %4d; %2lu; %2lu\n",
				  TalbeIdx, idx, pDFS2Table->entry[idx].mode,
				  pDFS2Table->entry[idx].avgLen,
				  pDFS2Table->entry[idx].ELow,
				  pDFS2Table->entry[idx].EHigh,
				  pDFS2Table->entry[idx].WLow,
...

	{
		...
		/* Input Control 1 */
		bbp_val =
		    (pDfsProgramParam->Rx_PE_Mask << 16) | (pDfsProgramParam->
							    Packet_End_Mask);
		...
		for (idx = 0; idx < DfsEngineNum; idx++) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("write DFS Channle[%d] configuration \n",
				  idx));
			...
			bbp_val =
			    ((pDFS2Table->entry[idx].
			      EHigh & 0x0fff) << 16) | (pDFS2Table->entry[idx].
							ELow & 0x0fff);
			RTMP_BBP_IO_WRITE32(pAd, DFS_R5, bbp_val);

			/* DFS Period Low */
			RTMP_BBP_IO_WRITE32(pAd, DFS_R7,
					    pDFS2Table->entry[idx].TLow);

			/* DFS Period High */
			RTMP_BBP_IO_WRITE32(pAd, DFS_R9,
					    pDFS2Table->entry[idx].THigh);
			...
			bbp_val =
			    ((pDFS2Table->entry[idx].
			      WHigh & 0x0fff) << 16) | (pDFS2Table->entry[idx].
							WLow & 0x0fff);
...
BOOLEAN DfsSwCheckOnHwDetection(IN PRTMP_ADAPTER pAd,
				...
				IN UINT8 DfsChannel,
				IN ULONG RadarPeriod, IN ULONG RadarWidth)
{
	BOOLEAN bRadarCheck = TRUE;
	if (!RadarPeriod || !RadarWidth) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Block eception on zero RadarPeriod or RadarWidth\n"));
		return FALSE;
	}
...
static VOID ChannelSelectOnRadarDetection(IN PRTMP_ADAPTER pAd)
{
		...
		if (pAd->CommonCfg.Channel == pAd->ChannelList[i].Channel) {
			if (pAd->CommonCfg.HtCapability.HtCapInfo.
				...
				if ((pAd->ChannelList[i].Channel >> 2) & 1) {
					if ((pAd->ChannelList[i + 1].Channel -
					     pAd->ChannelList[i].Channel) ==
					    4) {
						DBGPRINT(RT_DEBUG_TRACE,
							 ("Find extend channel = %u\n",
							  pAd->ChannelList[i +
									   1].
							  Channel));
						pAd->ChannelList[i +
								 1].
						    RemainingTimeForUse = 1800;
					}
				} else {
	...
	if (pAd->CommonCfg.RDDurRegion == JAP_W56) {
		for (i = 0; i < pAd->ChannelListNum; i++) {
			pAd->CommonCfg.Channel =
			    APAutoSelectChannel(pAd, FALSE);
			if ((pAd->CommonCfg.Channel >= 100)
			    && (pAd->CommonCfg.Channel <= 140))
				break;
		}
		...
		for (i = 0; i < pAd->ChannelListNum; i++) {
			pAd->CommonCfg.Channel =
			    APAutoSelectChannel(pAd, FALSE);
			if ((pAd->CommonCfg.Channel >= 36)
			    && (pAd->CommonCfg.Channel <= 60))
				break;
		}
	} else
	...
	if (pAd->Dot11_H.RDMode == RD_NORMAL_MODE) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s(): Error! Unexpected radar state.\n",
			  __FUNCTION__));
	}
	pRadarDetect->radarDeclared = 0;
}
...
static BOOLEAN DfsEventDrop(IN PRTMP_ADAPTER pAd, IN PDFS_EVENT pDfsEvent)
{
	...
	if (pDfsEvent->EngineId == 0x01 || pDfsEvent->EngineId == 0x02) {
		if (pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth == BW_40) {
			...
			TimeDiff =
			    (pDfsEvent->TimeStamp - pPreDfsEvent->TimeStamp);
		...
		if (TimeDiff < pDfsSwParam->EvtDropAdjTime &&
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s(): EngineId = %x,  Width = %u, TimeStamp = %u\n",
				  __FUNCTION__,
				  pDfsEvent->EngineId,
				  pDfsEvent->Width, pDfsEvent->TimeStamp));
			RetVal = TRUE;
		}
	}
...
static void dfs_sw_init(PRTMP_ADAPTER pAd)
{
	...
	if (pAd->CommonCfg.RDDurRegion == CE) {
		...
		pDfsSwParam->dfs_declare_thres =
		    CE_STAGGERED_RADAR_DECLARE_THRES;
...
void modify_table1(PRTMP_ADAPTER pAd, ULONG idx, ULONG value)
{
	...
	for (x = 0; x < DfsEngineNum; x++) {
		printk("Channel %lu\n", x);
		printk
		    ("\t\tmode(%02lu)=%d, M(%02lu)=%03d, EL(%02lu)=%03d EH(%02lu)=%03d, WL(%02lu)=%03d WH(%02lu)=%04d, eW(%02lu)=%02d\n\t\tTL(%02lu)=%05u TH(%02lu)=%06u, eT(%02lu)=%03d, BL(%02lu)=%u, BH(%02lu)=%u\n",
		     ...
		     (x * 16 + 0xa),
		     (unsigned int)pDFS2Table->entry[x].EpsilonT,
...
void modify_table2(PRTMP_ADAPTER pAd, ULONG idx, ULONG value)
{
	...
	while (pDFSValidRadar->type != NEW_DFS_END) {
		if (pDFSValidRadar->type & pRadarDetect->MCURadarRegion) {
			...
			printk("wl:wh = %d:%d  ", pDFSValidRadar->WLow,
			       pDFSValidRadar->WHigh);
			printk("W = %u  --- ", pDFSValidRadar->W);
			printk("W Margin = %d\n", pDFSValidRadar->WMargin);
			printk("Tl:Th = %d:%d  ",
			       (unsigned int)pDFSValidRadar->TLow,
			       (unsigned int)pDFSValidRadar->THigh);
...
VOID NewUsbTimerCB_Radar(IN PRTMP_ADAPTER pAd)
{
	...
	if (!pRadarDetect->bDfsSwDisable && (pRadarDetect->RadarTimeStampLow & 0x1) &&	/* 200ms */
	    !pRadarDetect->ch_busy)
	...
	if (channel & pRadarDetect->EnabledChMask) {
		radarDeclared =
		    DfsChannelCheck(pAd, channel & pRadarDetect->EnabledChMask);
	}
...
static VOID SwCheckDfsEventWithFw(IN PRTMP_ADAPTER pAd)
{
	...
	for (BBP127TableIdx = 0; BBP127TableIdx < 6; BBP127TableIdx++) {
		RTUSBMultiRead(pAd, BBPR127TABLE_OWNERID + BBP127TableIdx,
			       &BBPR127_OWERID, 1);
		...
		RTUSBMultiRead(pAd,
			       BBPR127TABLE_OFFSET + (BBP127TableIdx * 512),
			       BBPR127_Table, DFS_EVENT_BUFFER_SIZE);

		OwenerIDValue = 0x01;
		RTUSBMultiWrite_OneByte(pAd,
					BBPR127TABLE_OWNERID + BBP127TableIdx,
					&OwenerIDValue);
		...
		if (pRadarDetect->McuRadarDebug & RADAR_DEBUG_SHOW_RAW_EVENT) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\n=============0x%02x==============\n",
				  BBP127TableIdx));
			if (IS_RT3572(pAd)) {
				DFS_EVENT_BUFF_PRINT(1, BBPR127_Table,
						     DFS_EVENT_BUFFER_SIZE);
			} else {
				DFS_EVENT_BUFF_PRINT(0, BBPR127_Table,
						     DFS_EVENT_BUFFER_SIZE);
			}
		}
		...
		for (k = 0; k < NumEvent; k++) {
			...
			if (IS_RT3572(pAd)) {
				pTableOffset =
				    (BBPR127_Table + 1) + (DFS_EVENT_SIZE * k);
			} else {
				pTableOffset =
				    BBPR127_Table + (DFS_EVENT_SIZE * k);
			}

			if (DfsEventDataFetch(pAd, pTableOffset, &DfsEvent) ==
			    FALSE)
				break;
