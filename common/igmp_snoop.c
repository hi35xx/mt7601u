...
static inline void initFreeEntryList(IN PMULTICAST_FILTER_TABLE
				     ...
				     IN PLIST_HEADER pList)
{
	...
	for (i = 0; i < FREE_MEMBER_POOL_SIZE; i++)
		insertTailList(pList,
			       (PLIST_ENTRY) & (pMulticastFilterTable->
						freeMemberPool[i]));

	return;
}

static inline PMEMBER_ENTRY AllocaGrpMemberEntry(IN PMULTICAST_FILTER_TABLE
						 pMulticastFilterTable)
{
	...
	pMemberEntry =
	    (PMEMBER_ENTRY) removeHeadList(&pMulticastFilterTable->
					   freeEntryList);
...
static inline VOID FreeGrpMemberEntry(IN PMULTICAST_FILTER_TABLE
				      pMulticastFilterTable,
...
static VOID DeleteIgmpMemberList(IN PMULTICAST_FILTER_TABLE
				 pMulticastFilterTable, IN PLIST_HEADER pList);
...
/*	*ppMulticastFilterTable = kmalloc(sizeof(MULTICAST_FILTER_TABLE), MEM_ALLOC_FLAG); */
	...
	if (*ppMulticastFilterTable == NULL) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s unable to alloc memory for Multicase filter table, size=%d\n",
		...
		if (pMulticastFilterTable->Content[i].Valid == TRUE) {
			...
			DBGPRINT(RT_DEBUG_OFF,
				 ("IF(%s) entry #%d, type=%s, GrpId=(%02x:%02x:%02x:%02x:%02x:%02x) memberCnt=%d\n",
				  RTMP_OS_NETDEV_GET_DEVNAME(pEntry->net_dev),
				  i, (pEntry->type == 0 ? "static" : "dynamic"),
					...
					IN MulticastFilterEntryType type)
{
	...
	if ((pEntry =
	     MulticastFilterTableLookup(pMulticastFilterTable, pGrpId, dev))) {
		  ...
		  IN PUCHAR pSrcMacAddr, IN PUCHAR pIpHeader, IN PNET_DEV pDev)
{
	...
	if (isIgmpPkt(pDstMacAddr, pIpHeader)) {
		...
		case IGMP_V2_MEMBERSHIP_REPORT:	/* IGMP version 2 membership report. */
			...
			MulticastFilterTableInsertEntry(pAd, GroupMacAddr,
							pSrcMacAddr, pDev,
							MCAT_FILTER_DYNAMIC);
			break;
		...
		case IGMP_LEAVE_GROUP:	/* IGMP version 1 and version 2 leave group. */
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IGMP Group=%02x:%02x:%02x:%02x:%02x:%02x\n",
				  ...
				  GroupMacAddr[2], GroupMacAddr[3],
				  GroupMacAddr[4], GroupMacAddr[5]));
			MulticastFilterTableDeleteEntry(pAd, GroupMacAddr,
							pSrcMacAddr, pDev);
			break;
	...
	if ((pMemberEntry =
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s Member Mac=%02x:%02x:%02x:%02x:%02x:%02x\n",
			  __FUNCTION__, pMemberEntry->Addr[0],
			 ...
			 IN PUCHAR pMemberAddr, IN PNET_DEV pDev)
{
	...
	for (i = 0; i < MAX_LEN_OF_MULTICAST_FILTER_TABLE; i++) {
		...
		if (pEntry->Valid == TRUE) {
			if (pMemberAddr != NULL) {
				RTMP_SEM_LOCK(&pMulticastFilterTable->
					      MulticastFilterTabLock);
				DeleteIgmpMember(pMulticastFilterTable,
						 &pEntry->MemberList,
						 pMemberAddr);
				RTMP_SEM_UNLOCK(&pMulticastFilterTable->
						MulticastFilterTabLock);
			}

			if ((pEntry->type == MCAT_FILTER_DYNAMIC)
			    && (IgmpMemberCnt(&pEntry->MemberList) == 0))
				MulticastFilterTableDeleteEntry(pAd,
								pEntry->Addr,
								pMemberAddr,
								pDev);
		}
...
INT Set_IgmpSn_Enable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s:: %s\n", __FUNCTION__,
		  Enable ==
		  TRUE ? "Enable IGMP Snooping" : "Disable IGMP Snooping"));
...
INT Set_IgmpSn_AddEntry_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	pDev =
	    (ifIndex ==
	     MAIN_MBSSID) ? (pAd->net_dev) : (pAd->ApCfg.MBSSID[ifIndex].
					      MSSIDDev);
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		...
		if (strlen(thisChar) == 17) {	/*Mac address acceptable format 01:02:03:04:05:06 length 17 */
			...
			for (i = 0, value = rstrtok(thisChar, "."); value;
				...
				IpAddr[i] =
				    (UCHAR) simple_strtol(value, NULL, 10);
				i++;
			}
		...
		if ((bGroupId == 1) && IS_MULTICAST_MAC_ADDR(Addr))
			MulticastFilterTableInsertEntry(pAd, GroupId, NULL,
							pDev,
							MCAT_FILTER_STATIC);
		/* Group-Member must be a UCAST address. */
		else if ((bGroupId == 0) && !IS_MULTICAST_MAC_ADDR(Addr))
			MulticastFilterTableInsertEntry(pAd, GroupId, Addr,
							pDev,
							MCAT_FILTER_STATIC);
		else {
			DBGPRINT(RT_DEBUG_TRACE,
...
INT Set_IgmpSn_DelEntry_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		...
		if (memberCnt > 0)
			MulticastFilterTableDeleteEntry(pAd, (PUCHAR) GroupId,
							Addr, pDev);
	...
	if (memberCnt == 0)
		MulticastFilterTableDeleteEntry(pAd, (PUCHAR) GroupId, NULL,
						pDev);
			     ...
			     OUT PMULTICAST_FILTER_TABLE_ENTRY * ppGroupEntry)
{
	if (IS_MULTICAST_MAC_ADDR(pSrcBufVA)) {
		...
		if (ntohs(*((UINT16 *) (pIpHeader))) == ETH_P_IPV6)
			IgmpMldPkt =
			    IPv6MulticastFilterExcluded(pSrcBufVA, pIpHeader);
		else
			 ...
			 IN UINT8 UserPriority, IN PNET_DEV pNetDev)
{
	...
	if (IgmpPktInGroup == IGMP_IN_GROUP) {
		...
		if (pMemberEntry != NULL) {
			...
			pMacEntry =
			    APSsPsInquiry(pAd, pMemberAddr, &Sst, &Aid, &PsMode,
					  &Rate);
			bContinue = TRUE;
		}
		...
		for (MacEntryIdx = 1; MacEntryIdx < MAX_NUMBER_OF_MAC;
		     MacEntryIdx++) {
			...
			if (pMacEntry && IS_ENTRY_CLIENT(pMacEntry)
			    ...
			    &&
			    (!NdisEqualMemory
			     (src_addr, pMacEntry->Addr, MAC_ADDR_LEN))) {
	...
	while (bContinue == TRUE) {
		if (pMacEntry && (Sst == SST_ASSOC)
			...
			if ((pSkbClone)
				...
				if (IgmpPktInGroup == IGMP_IN_GROUP) {
					...
					src_addr += 6;
					for (MacEntryIdx = pMacEntry->Aid + 1;
					     MacEntryIdx < MAX_NUMBER_OF_MAC;
					     MacEntryIdx++) {
						pMemberAddr =
						    pAd->MacTab.
						    Content[MacEntryIdx].Addr;
						pMacEntry =
						    APSsPsInquiry(pAd,
								  pMemberAddr,
								  &Sst, &Aid,
			...
			if (PsMode == PWR_SAVE) {
				APInsertPsQueue(pAd, pSkbClone, pMacEntry,
						QueIdx);
			} else {
				/* insert the pkt to TxSwQueue. */
				if (pAd->TxSwQueue[QueIdx].Number >=
				    pAd->TxSwQMaxLen) {
#ifdef BLOCK_NET_IF
		...
		if (IgmpPktInGroup == IGMP_IN_GROUP) {
			...
			if (pMemberEntry != NULL) {
				pMemberAddr = pMemberEntry->Addr;
				pMacEntry =
				    APSsPsInquiry(pAd, pMemberAddr, &Sst, &Aid,
						  &PsMode, &Rate);
				bContinue = TRUE;
			} else
				bContinue = FALSE;
			...
			for (MacEntryIdx = pMacEntry->Aid + 1;
			     MacEntryIdx < MAX_NUMBER_OF_MAC; MacEntryIdx++) {
				pMemberAddr =
				    pAd->MacTab.Content[MacEntryIdx].Addr;
				pMacEntry =
				    APSsPsInquiry(pAd, pMemberAddr, &Sst, &Aid,
						  &PsMode, &Rate);
				if (pMacEntry && IS_ENTRY_CLIENT(pMacEntry)
	...
	if (isMldPkt(pDstMacAddr, pIpHeader, &MldType, &pMldHeader) == TRUE) {
		...
		case MLD_V1_LISTENER_REPORT:
			...
			ConvertMulticastIP2MAC(pGroupIpAddr,
					       ...
					       ETH_P_IPV6);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Group Id=%02x:%02x:%02x:%02x:%02x:%02x\n",
				  GroupMacAddr[0], GroupMacAddr[1],
				  GroupMacAddr[2], GroupMacAddr[3],
				  GroupMacAddr[4], GroupMacAddr[5]));
			MulticastFilterTableInsertEntry(pAd, GroupMacAddr,
		...
		case MLD_V2_LISTERNER_REPORT:	/* IGMP version 3 membership report. */
			...
			for (i = 0; i < numOfGroup; i++) {
				...
				pGroup +=
				    (4 + IPV6_ADDR_LEN + (numOfSources * 16) +
				     AuxDataLen);
			}
			break;
