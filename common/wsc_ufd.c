...
BOOLEAN WscPassXmlDeclare(INOUT STRING ** pXmlData)
{
	STRING *ptr;
...
BOOLEAN WscGetXmlSSID(IN STRING * pXmlData, OUT NDIS_802_11_SSID * pSsid)
{
...
BOOLEAN WscGetXmlAuth(IN STRING * pXmlData, OUT USHORT * pAuthType)
{
	STRING *ptr, *pBuffer = pXmlData;
	STRING AuthStr[10] = { 0 };
	UINT AuthStrLen = 0;
...
BOOLEAN WscGetXmlEncr(IN STRING * pXmlData, OUT USHORT * pEncrType)
{
	STRING *ptr, *pBuffer = pXmlData;
	STRING EncrStr[10] = { 0 };
	UINT EncrStrLen = 0;
...
BOOLEAN WscGetXmlKey(IN STRING * pXmlData,
		     OUT UCHAR * pKey, OUT USHORT * pKeyLen)
{
	STRING *ptr, *pBuffer = pXmlData;
	UINT KeyLen = 0;
...
BOOLEAN WscGetXmlKeyIndex(IN STRING * pXmlData, OUT UCHAR * pKeyIndex)
{
	STRING *ptr, *pBuffer = pXmlData;
...
BOOLEAN WscReadProfileFromUfdFile(IN RTMP_ADAPTER * pAd,
				  IN UCHAR ApIdx, IN PSTRING pUfdFileName)
{
	PWSC_CREDENTIAL pCredential =
	    &pAd->ApCfg.MBSSID[ApIdx].WscControl.WscProfile.Profile[0];
	...
	if (IS_FILE_OPEN_ERR(file_r)) {
		...
		if (pXmlData == NULL) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("pXmlData kmalloc fail. (fileLen = %zu)\n",
				  fileLen));
			return FALSE;
		}
		...
		if (rv != fileLen) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("RtmpOSFileRead fail, fileLen = %zu\n",
				  fileLen));
			RtmpOSFSInfoChange(&osFSInfo, FALSE);
			goto ReadErr;
		}
	...
	if (WscPassXmlDeclare(&pXmlData)) {
		...
		if (WscGetXmlKey
		    (pXmlData, pCredential->Key, &pCredential->KeyLength)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Credential.Key = %s (%d)\n",
				 ...
				 IN UCHAR ApIdx, IN PSTRING pUfdFileName)
{
	...
	if (IS_FILE_OPEN_ERR(file_w)) {
		...
		if ((offset =
		     rtstrstr(pXmlTemplate, (PSTRING) XML_GUID_MARK)) != NULL) {
			RtmpOSFileWrite(file_w, (PSTRING) pXmlTemplate,
					(int)(offset - pXmlTemplate));
			RtmpOSFileWrite(file_w, (PSTRING) & Guid_Str[0],
					(int)UUID_LEN_STR);
			pXmlTemplate = offset + strlen(XML_GUID_MARK);
		}

		if ((offset =
		     rtstrstr(pXmlTemplate,
			      (PSTRING) XML_AP_GUID_MARK)) != NULL) {
			RtmpOSFileWrite(file_w, (PSTRING) pXmlTemplate,
					(int)(offset - pXmlTemplate));
			RtmpOSFileWrite(file_w,
					(PSTRING) & pWscControl->
					Wsc_Uuid_Str[0], (int)UUID_LEN_STR);
			pXmlTemplate = offset + strlen(XML_AP_GUID_MARK);
		}
		if ((offset =
		     rtstrstr(pXmlTemplate, (PSTRING) XML_SSID_MARK)) != NULL) {
			RtmpOSFileWrite(file_w, (PSTRING) pXmlTemplate,
					(int)(offset - pXmlTemplate));
			RtmpOSFileWrite(file_w,
					(PSTRING) & pCredential->SSID.Ssid[0],
					(int)pCredential->SSID.SsidLength);
			pXmlTemplate = offset + strlen(XML_SSID_MARK);
		}
		if ((offset =
		     rtstrstr(pXmlTemplate, (PSTRING) XML_AUTH_MARK)) != NULL) {
			RtmpOSFileWrite(file_w, (PSTRING) pXmlTemplate,
					(int)(offset - pXmlTemplate));
			for (PWSC_UFD_AUTH_TYPE = WSC_UFD_AUTH_TYPE;
		...
		if ((offset =
		     rtstrstr(pXmlTemplate, (PSTRING) XML_ENCR_MARK)) != NULL) {
			bFound = FALSE;
			RtmpOSFileWrite(file_w, (PSTRING) pXmlTemplate,
					(int)(offset - pXmlTemplate));
			for (PWSC_UFD_ENCR_TYPE = WSC_UFD_ENCR_TYPE;
		...
		if ((offset =
		     rtstrstr(pXmlTemplate, (PSTRING) XML_KEY_MARK)) != NULL) {
			if (pCredential->EncrType != WSC_ENCRTYPE_NONE) {
				...
				RtmpOSFileWrite(file_w, (PSTRING) XML_ENCR_END,
