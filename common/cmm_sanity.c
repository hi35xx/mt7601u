			...
			if (SubType == SUBTYPE_BEACON) {
				...
				if (P2P_CLI_ON(pAd)
					...
					GetTimBit((PCHAR) pEid,
						  pAd->ApCliMlmeAux.Aid,
						  &TimLen, pBcastFlag,
						  pDtimCount, pDtimPeriod,
						  pMessageToMe);
				}
#endif				/* P2P_SUPPORT */
				if (INFRA_ON(pAd)
		...
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			...
			if (bSelReg) {
				...
				NdisMoveMemory(&pAd->StaCfg.WscControl.
					       ...
					       WscPeerMAC[0], MAC_ADDR_LEN);
				hex_dump
				    ("PeerBeaconAndProbeRspSanity - WscPeerMAC",
	...
	if (Sanity != 0x7) {
		...
		if (bFoundIWscIe && (pAd->StaCfg.BssType == BSS_ADHOC)) {
			...
			if ((pWscCtrl->WscConfMode == WSC_ENROLLEE) &&
				...
				NdisZeroMemory(&pWscCtrl->WscSsid,
					       sizeof(NDIS_802_11_SSID));
				if ((*pSsidLen) <= 32 && (*pSsidLen) != 0) {
					pWscCtrl->WscSsid.SsidLength =
					    *pSsidLen;
					NdisMoveMemory(pWscCtrl->WscSsid.Ssid,
						       Ssid,
						       pWscCtrl->WscSsid.
						       SsidLength);
					PeerConfigMethod =
					    be2cpu16(PeerConfigMethod);
					MlmeEnqueue(pAd, IWSC_STATE_MACHINE,
						    IWSC_MT2_PEER_PROBE_RSP,
						    sizeof(USHORT),
						    &PeerConfigMethod, 0);
				}
			}
	...
	*SsidLen = Fr->Octet[1];
...
#endif				/* CONFIG_STA_SUPPORT */
#endif				/* IWSC_SUPPORT */
					WscCheckPeerDPID(pAd, Fr, eid_data,
							 eid_len);

#ifdef CONFIG_AP_SUPPORT
				if (PeerWscIeLen >= 512)
					DBGPRINT(RT_DEBUG_ERROR,
						 ("APPeerProbeReqSanity : PeerWscIeLen = %d (>= 512)\n",
						  PeerWscIeLen));
				if (pPeerWscIe && (PeerWscIeLen < 512)) {
					NdisMoveMemory(pPeerWscIe +
						       PeerWscIeLen,
	...
	if (pPeerWscIe && (PeerWscIeLen > 0)) {
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			if (NdisEqualMemory
			    (Addr1, pAd->ApCfg.MBSSID[apidx].Bssid,
			     MAC_ADDR_LEN))
				break;
		}
		...
		if ((pAd->ApCfg.MBSSID[apidx].WscControl.
		     WscConfMode & WSC_PROXY) != WSC_DISABLE) {
			int bufLen = 0;
			PUCHAR pBuf = NULL;
