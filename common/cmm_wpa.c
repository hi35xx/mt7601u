		...
		if ((pEntry->Sst == SST_ASSOC)
			...
			if (peerKeyInfo.KeyAck == 1) {
				...
				if ((peerKeyInfo.KeyMic == 1) &&
				    ...
				    (pEapol_packet->KeyDesc.KeyDataLen[1] ==
				     12)) {
		       ...
		       IN PUCHAR pData, IN UINT DataLen, IN BOOLEAN bClearFrame)
{
	...
	do {
		...
		if (IS_ENTRY_APCLI(pEntry)) {
			...
			RTMP_SET_PACKET_NET_DEVICE_APCLI(pPacket,
							 pEntry->
							 MatchAPCLITabIdx);
...
#endif				/* CONFIG_AP_SUPPORT */
		{
			...
			if (IS_P2P_GO_ENTRY(pEntry)) {
				RTMP_SET_PACKET_NET_DEVICE_P2P(pPacket,
							       MAIN_MBSSID);
				RTMP_SET_PACKET_OPMODE(pPacket, OPMODE_AP);
			}
#endif				/* P2P_SUPPORT */
		    ...
		    IN MAC_TABLE_ENTRY * pEntry, IN ULONG TimeInterval)
{
...
#endif				/* P2P_SUPPORT */
	{
		...
		if (pEntry->apidx < pAd->ApCfg.BssidNum) {
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("[ERROR]WPAStart4WayHS : The apidx(%d) is invalid.\n",
				  pEntry->apidx));
			return;
		}
...
#ifdef WSC_AP_SUPPORT
	if (MAC_ADDR_EQUAL
	    (pEntry->Addr, pAd->ApCfg.MBSSID[apidx].WscControl.EntryAddr)
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2)
		...
		NdisMoveMemory(&pEapolFrame->KeyDesc.KeyData[6],
			       ...
			       BSSIDInfo[pEntry->PMKID_CacheIdx].PMKID,
			       LEN_PMKID);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMK,
			       &pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			       BSSIDInfo[pEntry->PMKID_CacheIdx].PMK, PMK_LEN);

		pEapolFrame->KeyDesc.KeyData[1] = 0x14;	/* 4+LEN_PMKID */
		INC_UINT16_TO_ARRARY(pEapolFrame->KeyDesc.KeyDataLen,
				     6 + LEN_PMKID);
...
#endif				/* P2P_SUPPORT */
	{
		...
		if ((pEntry->AuthMode == Ndis802_11AuthModeWPA)
			...
			if ((pAd->ApCfg.MBSSID[apidx].AuthMode ==
				...
				rsnie_ptr =
				    &pAd->ApCfg.MBSSID[apidx].RSN_IE[1][0];
			} else {
				rsnie_len =
				    pAd->ApCfg.MBSSID[apidx].RSNIE_Len[0];
				rsnie_ptr =
				    &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][0];
			}
		}
	}
#endif				/* CONFIG_AP_SUPPORT */
			...
			IN MAC_TABLE_ENTRY * pEntry, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (IS_P2P_CLI_ENTRY(pEntry) && (pEntry->bP2pClient)) {
		if (pAd->P2pCfg.bP2pCliReConnectTimerRunning) {
			...
			RTMPCancelTimer(&pAd->P2pCfg.P2pCliReConnectTimer,
					&Cancelled);
		}
		RTMPCancelTimer(&pAd->P2pCfg.P2pWscTimer, &Cancelled);
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
		P2pSendWirelessEvent(pAd, RT_P2P_AP_STA_CONNECTED, NULL,
				     pEntry->Addr);
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA2 ||
			...
			if (pEntry->AuthMode == Ndis802_11AuthModeWPA2) {
				...
				NdisMoveMemory(&PMK_key[8],
					       ...
					       Bssid, MAC_ADDR_LEN);
				NdisMoveMemory(&PMK_key[14], pEntry->Addr,
					       MAC_ADDR_LEN);
				RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].
					     PMK, PMK_LEN, PMK_key, 20, digest,
					     SHA1_DIGEST_SIZE);
				RTMPAddPMKIDCache(pAd, pEntry->apidx,
						  pEntry->Addr, digest,
				...
				if (pEntry->bP2pClient) {
					...
					p2pIndex =
					    P2pGroupTabSearch(pAd,
							      pEntry->Addr);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("pEntry->bP2pClient: p2pIndex = %d, %02X-%02X-%02X-%02X-%02X-%02X\n",
						  p2pIndex,
						  PRINT_MAC(pEntry->Addr)));
					...
					P2pSendWirelessEvent(pAd,
							     RT_P2P_AP_STA_CONNECTED,
							     pP2pEntry,
							     pEntry->Addr);
#endif				/* RT_P2P_SPECIFIC_WIRELESS_EVENT */
#endif
					pAd->P2pCfg.bPeriodicListen = TRUE;
					RTMPCancelTimer(&pAd->P2pCfg.
							P2pWscTimer,
							&Cancelled);
				} else {
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
...
#ifdef DOT1X_SUPPORT
			pRSN_Cap->field.PreAuth =
			    (pMbss->PreAuth == TRUE) ? 1 : 0;
#endif				/* DOT1X_SUPPORT */
		}
	}
#endif				/* CONFIG_AP_SUPPORT */
		   ...
		   IN UINT AuthMode, IN UINT WepStatus, IN UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_P2P_GO) {
			...
			NdisZeroMemory(pAd->ApCfg.MBSSID[MAIN_MBSSID].RSN_IE[0],
				       MAX_LEN_OF_RSNIE);
			NdisZeroMemory(pAd->ApCfg.MBSSID[MAIN_MBSSID].RSN_IE[1],
				       MAX_LEN_OF_RSNIE);

			/* Pointer to the first RSNIE context */
			rsnielen_cur_p =
			    &pAd->ApCfg.MBSSID[MAIN_MBSSID].RSNIE_Len[0];
			pRsnIe = pAd->ApCfg.MBSSID[MAIN_MBSSID].RSN_IE[0];

			/* Pointer to the secondary RSNIE context */
			rsnielen_ex_cur_p =
			    &pAd->ApCfg.MBSSID[MAIN_MBSSID].RSNIE_Len[1];
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (apcliIfidx < MAX_APCLI_NUM) {
				...
				NdisZeroMemory(pAd->ApCfg.ApCliTab[apcliIfidx].
					       RSN_IE, MAX_LEN_OF_RSNIE);
				rsnielen_cur_p =
	...
	*rsnielen_cur_p = p_offset;
	...
	if ((pAd->OpMode == OPMODE_AP)
		...
		if (bMixRsnIe) {
			/* 1. insert cipher suite */
			RTMPMakeRsnIeCipher(pAd, Wpa2Ie, WepStatus, apidx,
					    bMixCipher, FlexibleCipher,
					    pRsnIe_ex, &s_offset);

			/* 2. insert AKM */
			RTMPMakeRsnIeAKM(pAd, Wpa2Ie, AuthMode, apidx,
					 pRsnIe_ex, &s_offset);

			/* 3. insert capability */
			RTMPMakeRsnIeCap(pAd, Wpa2Ie, apidx, pRsnIe_ex,
					 &s_offset);

			/* Update the RSNIE length */
			*rsnielen_ex_cur_p = s_offset;

			hex_dump("The secondary RSNIE", pRsnIe_ex,
				 (*rsnielen_ex_cur_p));
		}
	}
#endif				/* CONFIG_AP_SUPPORT */
			      ...
			      IN BOOLEAN bWPA2, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (IS_ENTRY_APCLI(pEntry)) {
		...
		if (!APCliInstallSharedKey
		    (pAd, GTK, GTKLEN, DefaultIdx, pEntry)) {
			return FALSE;
		}
	}
#endif				/* APCLI_SUPPORT */
	...
	*ext_len = 0;
	...
	if (CipherAlg == CIPHER_SMS4) {
		...
		RTMPSoftEncryptSMS4(pHdr,
				    pSrcBufData,
				    SrcBufLen, KeyIdx, pKey->Key, pKey->TxTsc);
#ifdef RT_BIG_ENDIAN
		RTMPFrameEndianChange(pAd, pHdr, DIR_READ, FALSE);
#endif
		*ext_len = LEN_WPI_MIC;
...
const UCHAR *RTMPFindWPSIE(const UCHAR * Ies, INT32 Len)
{
	...
	if (!Ies || !Len)
		return NULL;
	while ((Ptr =
		RTMPFindIE(WLAN_EID_VENDOR_SPECIFIC, Ptr,
			   Len - (Ptr - Ies))) != NULL) {
