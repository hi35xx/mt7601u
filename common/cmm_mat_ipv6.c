...
static inline BOOLEAN needUpdateIPv6MacTB(UCHAR * pMac,
				      ...
				      IN PUCHAR pMacAddr, IN PCHAR pIPv6Addr)
{
	...
	while (pEntry) {
		...
		if (NdisEqualMemory(pIPv6Addr, pEntry->ipv6Addr, IPV6_ADDR_LEN)) {
			...
			return TRUE;
		} else {	/* handle the aging-out situation */
			if (RTMP_TIME_AFTER
			    (now,
			     (pEntry->lastTime + MAT_TB_ENTRY_AGEOUT_TIME))) {
	...
	pNewEntry =
	    (IPv6MacMappingEntry *) MATDBEntryAlloc(pMatCfg,
						    sizeof
						    (IPv6MacMappingEntry));
	if (pNewEntry != NULL) {
		...
		if (pIPv6MacTable->hash[hashIdx] == NULL) {	/* Hash list is empty, directly assign it. */
...
static PUCHAR MATProto_IPv6_Rx(IN MAT_STRUCT * pMatCfg,
	...
	case ICMPV6_MSG_TYPE_ROUTER_SOLICITATION:
		...
		if (!(IS_UNSPECIFIED_IPV6_ADDR(pIPv6Hdr->srcAddr))) {
			while (leftLen > sizeof(RT_ICMPV6_OPTION_HDR)) {
				pOptHdr =
				    (RT_ICMPV6_OPTION_HDR *) (pLayerHdr +
							      offset);
				if (pOptHdr->len == 0)
...
static PUCHAR MATProto_IPv6_Tx(IN MAT_STRUCT * pMatCfg,
			       IN PNDIS_PACKET pSkb,
			       IN PUCHAR pLayerHdr, IN PUCHAR pDevMacAdr)
{
	...
	needUpdate =
	    needUpdateIPv6MacTB(pSrcMac, (RT_IPV6_ADDR *) (&pIPv6Hdr->srcAddr));
	if (needUpdate)
	...
	while (nextProtocol != IPV6_NEXT_HEADER_ICMPV6 &&
		...
		if (IPv6ExtHdrHandle
		    ((RT_IPV6_EXT_HDR *) (pLayerHdr + offset), &nextProtocol,
		     &offset) == FALSE) {
			DBGPRINT(RT_DEBUG_TRACE,
	...
	case IPV6_NEXT_HEADER_ICMPV6:
		newSkb =
		    ICMPv6_Handle_Tx(pMatCfg, pSkb, pLayerHdr, pDevMacAdr,
				     offset);
		break;
...
VOID getIPv6MacTbInfo(IN MAT_STRUCT * pMatCfg,
		      IN char *pOutBuf, IN ULONG BufLen)
{
	IPv6MacMappingTable *pIPv6MacTable;
	IPv6MacMappingEntry *pHead;
	int startIdx, endIdx;
