...
/*
	...
	========================================================================
*/
void AP_QueuePsActionPacket(IN PRTMP_ADAPTER pAd,
...
/*		(FlgIsDeltsFrame == 0))*/
	if (pMacEntry->bAPSDDeliverEnabledPerAC[MgmtQid]) {
		...
		if (!pMacEntry->bAPSDAllAC) {
			...
			DuplicatePkt =
			    RTMP_DUPLICATE_PACKET(pAd, pPacket,
						  pMacEntry->apidx);
		} else
#endif				/* UAPSD_CC_FUNC_PS_MGMT_TO_LEGACY */
		{
		...
		if (pMacEntry->bAPSDAllAC) {
			...
			WLAN_MR_TIM_BIT_SET(pAd, pMacEntry->apidx,
					    pMacEntry->Aid);
		} else {
#ifdef UAPSD_CC_FUNC_PS_MGMT_TO_LEGACY
			/* duplicate one packet to legacy PS queue */
			     ...
			     IN BOOLEAN FlgDataQForce, IN BOOLEAN FlgIsLocked)
{
...
#ifdef CONFIG_AP_SUPPORT
	pHeader_802_11 =
	    (PHEADER_802_11) (pSrcBufVA + TXINFO_SIZE + TXWISize + TSO_SIZE);
	...
	if ((pEntry != NULL) &&
		 ...
		 */
		DBGPRINT(RT_DEBUG_TRACE,
			 ("STA in ps mode, queue the mgmt frame\n"));
		...
		if ((pHeader_802_11->FC.Type == BTYPE_MGMT) &&
		    (pHeader_802_11->FC.SubType == SUBTYPE_ACTION)) {
			FRAME_ADDBA_REQ *pFrameBa =
			    (FRAME_ADDBA_REQ *) pHeader_802_11;
				     ...
				     IN UCHAR QueIdx, IN PNDIS_PACKET pPacket)
{
	...
	if (P2P_INF_ON(pAd)
		...
		if (MAC_ADDR_EQUAL
		    (pHeader_802_11->Addr2, pAd->P2pCfg.CurrentAddress)
			...
			bss_idx =
			    BssTableSearch(&pAd->ScanTab, pHeader_802_11->Addr1,
	...
	if (pHeader_802_11->FC.Type == BTYPE_CNTL) {	/* must be PS-POLL */
		...
		if (pHeader_802_11->Addr1[0] & 0x01) {	/* MULTICAST, BROADCAST */
			...
			if ((pHeader_802_11->FC.SubType == SUBTYPE_PROBE_RSP)
			    ...
			    if ((pHeader_802_11->FC.SubType == SUBTYPE_DEAUTH)
				&& (MacTableLookup(pAd, pHeader_802_11->Addr1)
				    == NULL)) {
		...
		while (1) {
			if ((RTMP_TEST_FLAG
			    ...
			    || OPSTATUS_TEST_FLAG(pAd,
						  fRTMP_ADAPTER_DISABLE_DATA_BULK_OUT)
...
#define ENTRY_RETRY_INTERVAL	(100 * OS_HZ / 1000)
			...
			if (pTxBlk->
			    TxFrameType & (TX_RALINK_FRAME | TX_AMSDU_FRAME)) {
			...
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				...
				if (RTMP_GET_PACKET_OPMODE(pPacket))
					Status =
					    APHardTransmit(pAd, pTxBlk, QueIdx);
				else
#endif				/* P2P_SUPPORT */
				if (!INFRA_ON(pAd)
					    &&
					    !RTMP_CFG80211_VIF_P2P_GO_ON
					    (pAd)
					    && pAd->cfg80211_ctrl.
					    isCfgInApMode ==
				...
				IN ULONG DataSize, IN UCHAR OpMode)
{
	...
	if ((FromWhichBSSID < pAd->ApCfg.BssidNum)
		...
		VLAN_Size =
		    (pAd->ApCfg.MBSSID[FromWhichBSSID].VLAN_VID !=
		     0) ? LENGTH_802_1Q : 0;
		...
		VLAN_Size =
		    (pAd->
		     WdsTab.WdsEntry[FromWhichBSSID -
				     MIN_NET_DEVICE_FOR_WDS].VLAN_VID !=
		     0) ? LENGTH_802_1Q : 0;
	}
...
/*				kfree(Elem);*/
...
#endif				/* P2P_SUPPORT */
		{
			...
			if (FromWhichBSSID >= MIN_NET_DEVICE_FOR_P2P_GO)
				WhichBSSID =
				    FromWhichBSSID - MIN_NET_DEVICE_FOR_P2P_GO;
#endif				/* P2P_SUPPORT */
			if (pRemovedLLCSNAP) {
			...
			MBSS_VLAN_INFO_GET(pAd, VLAN_VID, VLAN_Priority,
					   WhichBSSID);

#ifdef WDS_VLAN_SUPPORT
			if (VLAN_VID == 0)	/* maybe WDS packet */
				WDS_VLAN_INFO_GET(pAd, VLAN_VID, VLAN_Priority,
						  FromWhichBSSID);
#endif				/* WDS_VLAN_SUPPORT */

			RT_VLAN_8023_HEADER_COPY(pAd, VLAN_VID, VLAN_Priority,
						 Header802_3, LENGTH_802_3,
						 pPayload, FromWhichBSSID,
						 TPID);
		}
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
...
//---Add by shiang for debug
	...
	if (IS_PKT_OPMODE_AP(pRxBlk)) {
		RTMP_AP_802_11_REMOVE_LLC_AND_CONVERT_TO_802_3(pRxBlk,
							       Header802_3);
	} else {
...
//---Add by shiang for debug
	...
	if (IS_PKT_OPMODE_AP(pRxBlk)) {
		...
		AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pRxPacket,
						    FromWhichBSSID);
	} else {
		//printk("FromWhichBSSID: %d\n", FromWhichBSSID);
		ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pRxPacket,
						 FromWhichBSSID);
	}
#else
#ifdef CONFIG_AP_SUPPORT
			      ...
			      IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (IS_PKT_OPMODE_AP(pRxBlk)) {
		...
		pPacket2 =
		    duplicate_pkt(get_netdev_from_bssid(pAd, FromWhichBSSID),
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	    AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pRxBlk->pRxPacket,
						FromWhichBSSID);
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
#ifdef P2P_SUPPORT
...
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		    AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pPacket2,
							FromWhichBSSID);
#endif				/* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
#ifdef P2P_SUPPORT
...
#ifdef P2P_SUPPORT
BOOLEAN RxDoneInterruptHandle(IN RTMP_ADAPTER * pAd)
{
...
//#endif // CONFIG_AP_SUPPORT //
	...
	while (1) {
		...
		if (pHeader->FC.SubType == SUBTYPE_ACTION) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("ACTION COME-IN !! TYPE = %d, IsReg = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
				  pHeader->FC.SubType,
				  pAd->Cfg80211RegisterActionFrame,
				  PRINT_MAC(pHeader->Addr2)));

			CFG80211DRV_PrintFrameType(pAd, "RX", (PUCHAR) pHeader,
						   pRxWI->RxWIMPDUByteCnt);
		}
#if 0
		if (pHeader->FC.SubType == SUBTYPE_BEACON &&
		    NdisEqualMemory(pAd->ApCfg.ApCliTab[0].CfgApCliBssid,
				    pHeader->Addr2, MAC_ADDR_LEN)) {
			printk
			    ("CfgApCliBssid: %02x:%02x:%02x:%02x:%02x:%02x, %02x:%02x:%02x:%02x:%02x:%02x\n",
			     PRINT_MAC(pAd->ApCfg.ApCliTab[0].CfgApCliBssid),
			     PRINT_MAC(pHeader->Addr2));
		}
#endif
		if ((pHeader->FC.Type == BTYPE_MGMT)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Deauth: %02x:%02x:%02x:%02x:%02x:%02x, %02x:%02x:%02x:%02x:%02x:%02x\n",
				  PRINT_MAC(pHeader->Addr1),
				  PRINT_MAC(pHeader->Addr2)));
		}

		if ((pHeader->FC.Type == BTYPE_DATA) &&
		    ((pHeader->FC.SubType == SUBTYPE_NULL_FUNC)
		     || (pHeader->FC.SubType == SUBTYPE_QOS_NULL))) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("NULL_FRAME: %02x:%02x:%02x:%02x:%02x:%02x\n",
				  PRINT_MAC(pHeader->Addr2)));
		}
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)
			...
			if (NdisEqualMemory
			    (pAd->P2PCurrentAddress, pHeader->Addr1,
			     MAC_ADDR_LEN)
			    || (pHeader->FC.SubType == SUBTYPE_PROBE_REQ)
			    || (pHeader->FC.SubType == SUBTYPE_BEACON
				&& NdisEqualMemory(pAd->ApCfg.ApCliTab[0].
						   CfgApCliBssid,
						   pHeader->Addr2,
						   MAC_ADDR_LEN))
			    || NdisEqualMemory(pAd->ApCliMlmeAux.Bssid,
					       pHeader->Addr2, MAC_ADDR_LEN)) {
				bReschedule =
				    APHandleRxDonePacket(pAd, pRxPacket,
							 pRxBlk);
			} else {
				bReschedule =
				    STAHandleRxDonePacket(pAd, pRxPacket,
							  pRxBlk);
			}
		} else {
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (!INFRA_ON(pAd) && !RTMP_CFG80211_VIF_P2P_GO_ON(pAd)
		    ...
		    RT_CMD_80211_IFTYPE_AP) {
			bReschedule =
			    APHandleRxDonePacket(pAd, pRxPacket, pRxBlk);
		} else {
			bReschedule =
			    STAHandleRxDonePacket(pAd, pRxPacket, pRxBlk);
		}
		}
#else
...
BOOLEAN CmdRspEventCallbackHandle(PRTMP_ADAPTER pAd, PUCHAR pRspBuffer)
{
	...
	if (pFceInfo->self_gen) {
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("Self-generated CMD Packet with event type=%d\n",
			  pFceInfo->evt_type));
		...
		if (pEntry) {
			...
			case MODE_HTMIX:
				...
				HTPhyMode.field.ShortGI =
				    pAd->MulTestTab.WdsEntry[WdsTabIdx].
				    HTPhyMode.field.ShortGI;
				HTPhyMode.field.BW =
				    pAd->MulTestTab.WdsEntry[WdsTabIdx].
				    HTPhyMode.field.BW;
				...
				if (pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED) {
					...
					pEntry->MaxHTPhyMode.field.STBC =
					    (pAd->CommonCfg.HtCapability.
					     HtCapInfo.RxSTBC & (pAd->CommonCfg.
								 DesiredHtPhy.
								 TxSTBC));
					if (pAd->CommonCfg.HtCapability.
					    HtCapParm.MpduDensity < 5)
						pEntry->MpduDensity = 5;
					else
						pEntry->MpduDensity =
						    pAd->CommonCfg.HtCapability.
						    HtCapParm.MpduDensity;
					pEntry->MaxRAmpduFactor =
					    pAd->CommonCfg.HtCapability.
					    HtCapParm.MaxRAmpduFactor;
					pEntry->MmpsMode =
					    (UCHAR) pAd->CommonCfg.HtCapability.
					    HtCapInfo.MimoPs;
					pEntry->AMsduSize =
					    (UCHAR) pAd->CommonCfg.HtCapability.
					    HtCapInfo.AMsduSize;
			...
			if (pAd->MulTestTab.WdsEntry[WdsTabIdx].
			    bAutoTxRateSwitch == FALSE) {
				pEntry->HTPhyMode.field.MCS =
				    pAd->MulTestTab.WdsEntry[WdsTabIdx].
				    DesiredTransmitSetting.field.MCS;
			...
			pAd->MulTestTab.WdsEntry[WdsTabIdx].MacTabMatchWCID =
			    (UCHAR) pEntry->Aid;
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("MacTableInsertWDSEntry - allocate entry #%d, Total= %d\n",
...
VOID AsicUpdateMulTestRxWCIDTable(IN PRTMP_ADAPTER pAd)
{
	...
	for (index = 0; index < MAX_WDS_ENTRY; index++) {
		...
		MacTableInsertMulTestEntry(pAd,
					   pAd->MulTestTab.WdsEntry[index].
					   PeerWdsAddr, index);
	}
...
VOID AsicUpdateMulTestEncryption(IN PRTMP_ADAPTER pAd, IN UCHAR wcid)
{
	...
	do {
		...
		if (pAd->MulTestTab.WdsEntry[pEntry->MatchWDSTabIdx].Valid !=
		    TRUE)
			break;

		WdsIdex = pEntry->MatchWDSTabIdx;

		if (((pAd->MulTestTab.WdsEntry[WdsIdex].WepStatus ==
		      Ndis802_11Encryption1Enabled)
		     || (pAd->MulTestTab.WdsEntry[WdsIdex].WepStatus ==
			...
			if (pAd->MulTestTab.WdsEntry[WdsIdex].WepStatus ==
			    Ndis802_11Encryption1Enabled)
				DefaultKeyId =
				    pAd->MulTestTab.WdsEntry[pEntry->
							     MatchWDSTabIdx].
				    KeyIdx;
			...
			NdisMoveMemory((PUCHAR) (&pEntry->PairwiseKey),
				       (PUCHAR) (&pAd->MulTestTab.
						 WdsEntry[WdsIdex].WdsKey),
				       sizeof(CIPHER_KEY));

			DBGPRINT(RT_DEBUG_ERROR,
				 ("pEntry->PairwiseKey.CipherAlg = (%d)\n",
			...
			if (pEntry->WepStatus == Ndis802_11Encryption1Enabled) {
				if (pAd->MulTestTab.WdsEntry[WdsIdex].WdsKey.
				    CipherAlg == CIPHER_WEP64)
					pEntry->PairwiseKey.CipherAlg =
					    CIPHER_WEP64;
				else if (pAd->MulTestTab.WdsEntry[WdsIdex].
					 WdsKey.CipherAlg == CIPHER_WEP128)
					pEntry->PairwiseKey.CipherAlg =
					    CIPHER_WEP128;
				else {
					DBGPRINT(RT_DEBUG_ERROR,
			...
			if (CLIENT_STATUS_TEST_FLAG
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("===> SW_ENC ON(wcid=%d) \n",
					  pEntry->Aid));
				NdisZeroMemory(pEntry->PairwiseKey.TxTsc,
					       LEN_WPA_TSC);
				NdisZeroMemory(pEntry->PairwiseKey.RxTsc,
					       LEN_WPA_TSC);
			} else
#endif				/* SOFT_ENCRYPT */
			{
