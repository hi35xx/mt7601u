...
static VOID ATEWriteTxWI(IN PRTMP_ADAPTER pAd, IN TXWI_STRUC * pTxWI, IN BOOLEAN FRAG, IN BOOLEAN InsTimestamp, IN BOOLEAN AMPDU, IN BOOLEAN Ack, IN BOOLEAN NSeq,	/* HW new a sequence. */
			 ...
			 IN UCHAR MIMOps,
			 IN UCHAR Txopmode,
			 IN BOOLEAN CfAck, IN HTTRANSMIT_SETTING Transmit)
{
...
static VOID ATEWriteTxInfo(IN PRTMP_ADAPTER pAd,
			   ...
			   IN BOOLEAN bWiv,
			   IN UCHAR QueueSel,
			   IN UCHAR NextValid, IN UCHAR TxBurst)
{
#ifdef RLT_MAC
...
INT ATESetUpFrame(IN PRTMP_ADAPTER pAd, IN UINT32 TxIdx)
{
	...
	if (pNullContext->InUse == FALSE) {
...
#ifdef RT_BIG_ENDIAN
		RTMPFrameEndianChange(pAd, (PUCHAR) & (pAd->NullFrame),
				      DIR_READ, FALSE);
#endif				/* RT_BIG_ENDIAN */
		...
		RTMPZeroMemory(&pAd->NullContext.TransferBuffer->field.
			       WirelessPacket[0], TX_BUFFER_NORMSIZE);
		pTxInfo =
		    (TXINFO_STRUC *) & pAd->NullContext.TransferBuffer->field.
		    WirelessPacket[0];
		...
		if (pATEInfo->bQATxStart == TRUE) {
			...
			NdisMoveMemory(pTxInfo, &(pATEInfo->TxInfo),
				       sizeof(pATEInfo->TxInfo));
		} else
#endif				/* RALINK_QA */
		{
			/* Avoid to exceed the range of WirelessPacket[]. */
			ASSERT(pATEInfo->TxLength <=
			       (MAX_FRAME_SIZE - 34 /* == 2312 */ ));

			/* pTxInfo->TxInfoPktLen will be updated to include padding later */
			ATEWriteTxInfo(pAd, pTxInfo,
				       (USHORT) (TXWISize + pATEInfo->TxLength)
				       , TRUE, EpToQueue[MGMTPIPEIDX], FALSE,
				       FALSE);
		...
		if (pATEInfo->bQATxStart == TRUE) {
			...
			ATEWriteTxWI(pAd, pTxWI, pATEInfo->TxWI.TxWIFRAG,
				     pATEInfo->TxWI.TxWITS,
				     pATEInfo->TxWI.TxWIAMPDU,
				     pATEInfo->TxWI.TxWIACK,
				     pATEInfo->TxWI.TxWINSEQ,
				     pATEInfo->TxWI.TxWIBAWinSize, BSSID_WCID,
				     pATEInfo->TxWI.
				     TxWIMPDUByteCnt /* include 802.11 header */
				     ,
				     pATEInfo->TxWI.TxWIPacketId, 0,
				     pATEInfo->TxWI.TxWITXOP /*IFS_HTTXOP */ ,
				     pATEInfo->TxWI.TxWICFACK
				      /*FALSE*/, TxHTPhyMode);
		} else {
			TxHTPhyMode.field.BW = pATEInfo->TxWI.TxWIBW;
			TxHTPhyMode.field.ShortGI = pATEInfo->TxWI.TxWIShortGI;
			...
			ATEWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, FALSE
				     /* No ack required. */ , FALSE, 0,
				     BSSID_WCID, pATEInfo->TxLength,
				     0, 0, IFS_HTTXOP, FALSE, TxHTPhyMode);
		}

		hex_dump("ATE",
			 pAd->NullContext.TransferBuffer->field.WirelessPacket,
			 24);
		dump_txinfo(pAd, pTxInfo);
		dumpTxWI(pAd, pTxWI);

		RTMPMoveMemory(&pAd->NullContext.TransferBuffer->field.
			       WirelessPacket[TXINFO_SIZE + TXWISize],
		...
		if (pATEInfo->bQATxStart == TRUE) {
			...
			if ((pATEInfo->PLen != 0) && (pATEInfo->DLen != 0)) {
				for (pos = 0; pos < pATEInfo->DLen;
				     pos += pATEInfo->PLen) {
					RTMPMoveMemory(pDest, pATEInfo->Pattern,
						       pATEInfo->PLen);
					pDest += pATEInfo->PLen;
				}
			}
			TransferBufferLength =
			    ...
			    pATEInfo->TxWI.TxWIMPDUByteCnt;
		} else
#endif				/* RALINK_QA */
		{
			for (pos = 0;
			     pos < (pATEInfo->TxLength - sizeof(HEADER_802_11));
			     pos++) {
			...
			TransferBufferLength =
			    TXINFO_SIZE + TXWISize + pATEInfo->TxLength;
		}
		...
		ASSERT((padLen <=
			(RTMP_PKT_TAIL_PADDING -
			 4 /* 4 == MaxBulkOutsize alignment padding */ )));
	...
	RTMPFrameEndianChange(pAd,
			      (((PUCHAR) pTxInfo) + TXWISize + TXINFO_SIZE),
			      DIR_WRITE, FALSE);
	RTMPDescriptorEndianChange((PUCHAR) pTxInfo, TYPE_TXINFO);
#endif				/* RT_BIG_ENDIAN */

	hex_dump("ATE TX",
		 &pAd->NullContext.TransferBuffer->field.WirelessPacket[0],
		 TXWISize + TXINFO_SIZE);
...
INT ATEResetBulkOut(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pATEInfo->Mode == ATE_TXFRAME)
		...
		/* Init Tx context descriptor. */
		RTUSBInitTxDesc(pAd, pNullContext,
				0 /* pAd->bulkResetPipeid */ ,
				(usb_complete_t)
