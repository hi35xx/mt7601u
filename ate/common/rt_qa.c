	 ...
	 */
		...
		if (atemode == ATE_TXCONT) {
			if (pATEInfo->TxMethod == TX_METHOD_0) {
				...
				ATE_BBP_STOP_CTS_TX_MODE(pAd, BBP_R22,
							 &BbpData);
			}
		}
...
static VOID RTMP_IO_READ_BULK(PRTMP_ADAPTER pAd, UCHAR * dst, UINT32 offset,
			      UINT32 len)
{
	UINT32 i, Value = 0;
	UCHAR *pDst;
...
VOID CalNoiseLevel(PRTMP_ADAPTER pAd, UCHAR channel, INT32 RSSI[3][10])
{
	...
	UCHAR Org_BBP66value = 0, Org_BBP69value = 0, Org_BBP70value = 0, data =
	    0;
	USHORT LNA_Gain = 0;
	INT32 column = 0;
		...
		for (column = 0; column < 10; column++) {
			...
			if (pAd->Antenna.field.RxPath >= 2) {	/* 2R */
				...
				if (BbpR51Rssi1 == 0) {
					...
					RSSI1 =
					    (INT32) (-12 - BbpR51Rssi1 -
						     LNA_Gain - Rssi1Offset);
				}
				RSSI[1][column] = RSSI1;
			}
			...
			if (pAd->Antenna.field.RxPath >= 3) {	/* 3R */
				...
				if (BbpR52Rssi2 == 0)
					...
					RSSI2 =
					    (INT32) (-12 - BbpR52Rssi2 -
						     LNA_Gain - Rssi2Offset);
...
static VOID ATE_BBPRead(IN PRTMP_ADAPTER pAd,
			IN UCHAR offset, IN UCHAR * pValue)
{
	if (ATE_ON(pAd)) {
...
static INT ResponseToGUI(IN struct ate_racfghdr *pRaCfg,
			 ...
			 IN INT Length, IN INT Status)
{
	(pRaCfg)->length = OS_HTONS((Length));
	(pRaCfg)->status = OS_HTONS((Status));
	(pwrq)->u.data.length =
	    sizeof((pRaCfg)->magic_no) + sizeof((pRaCfg)->command_type)
	...
	if (copy_to_user
	    ((pwrq)->u.data.pointer, (UCHAR *) (pRaCfg),
	     (pwrq)->u.data.length)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RACFG command(0x%04x)[magic number(0x%08x)] is done\n",
			  OS_NTOHS(pRaCfg->command_id),
			  OS_NTOHL(pRaCfg->magic_no)));
	}
				      ...
				      IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 2,
		      NDIS_STATUS_SUCCESS);
				...
				IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 4,
		      NDIS_STATUS_SUCCESS);
				     ...
				     IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + (len << 2),
		      NDIS_STATUS_SUCCESS);
				  ...
				  IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 1,
		      NDIS_STATUS_SUCCESS);
				     ...
				     IN struct ate_racfghdr *pRaCfg)
{
	...
	for (bbp_reg_index = 0; bbp_reg_index < pAd->chipCap.MaxNumOfBbpId + 1;
	     bbp_reg_index++) {
		pRaCfg->data[bbp_reg_index] = 0;
		ATE_BBPRead(pAd, bbp_reg_index, &pRaCfg->data[bbp_reg_index]);
	}

	ResponseToGUI(pRaCfg, wrq,
		      sizeof(pRaCfg->status) + pAd->chipCap.MaxNumOfBbpId + 1,
		      NDIS_STATUS_SUCCESS);
					...
					IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq,
		      sizeof(pRaCfg->status) + (sizeof(INT32) * 3 * 10),
		      NDIS_STATUS_SUCCESS);
				    ...
				    IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 60,
		      NDIS_STATUS_SUCCESS);
				 ...
				 IN struct ate_racfghdr *pRaCfg)
{
	...
	if (OS_NTOHS(pRaCfg->length) != 0) {
...
#ifdef RT_BIG_ENDIAN
		RTMPDescriptorEndianChange((PUCHAR) & pATEInfo->TxInfo,
					   TYPE_TXINFO);
#endif				/* RT_BIG_ENDIAN */
#endif				/* RTMP_MAC_USB */
		...
		NdisMoveMemory(&pATEInfo->TxCount, pRaCfg->data + TXWISize + 2,
			       4);
...
/*		p = (USHORT *)(&pRaCfg->data[TXWISize + TxInfoSize + 2]); */
		...
		NdisMoveMemory(&pATEInfo->Header,
			       pRaCfg->data + (TXWISize + TxInfoSize + 2 * 3),
			       pATEInfo->HLen);
		...
		NdisMoveMemory(&pATEInfo->Pattern,
			       pRaCfg->data + (TXWISize + TxInfoSize + 2 * 3) +
			       pATEInfo->HLen, pATEInfo->PLen);
		pATEInfo->DLen =
		    pATEInfo->TxWI.TxWIMPDUByteCnt - pATEInfo->HLen;
					   ...
					   IN struct ate_racfghdr *pRaCfg)
{
	...
	memcpy_exl(pAd, &pRaCfg->data[4],
		   (UCHAR *) & pAd->WlanCounters.RetryCount.u.LowPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[8],
		   (UCHAR *) & pAd->WlanCounters.FailedCount.u.LowPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[12],
		   (UCHAR *) & pAd->WlanCounters.RTSSuccessCount.u.LowPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[16],
		   (UCHAR *) & pAd->WlanCounters.RTSFailureCount.u.LowPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[20],
		   (UCHAR *) & pAd->WlanCounters.ReceivedFragmentCount.QuadPart,
		   4);
	memcpy_exl(pAd, &pRaCfg->data[24],
		   (UCHAR *) & pAd->WlanCounters.FCSErrorCount.u.LowPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[28],
		   (UCHAR *) & pAd->Counters8023.RxNoBuffer, 4);
	memcpy_exl(pAd, &pRaCfg->data[32],
		   (UCHAR *) & pAd->WlanCounters.FrameDuplicateCount.u.LowPart,
		   4);
	memcpy_exl(pAd, &pRaCfg->data[36],
		   (UCHAR *) & pAd->RalinkCounters.OneSecFalseCCACnt, 4);
	...
	if (pATEInfo->RxAntennaSel == 0) {
		...
		ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 52,
			      NDIS_STATUS_SUCCESS);
	} else {
		INT32 RSSI0 = 0;
		...
		RSSI0 = (INT32) (pATEInfo->LastRssi0 - pAd->BbpRssiToDbmDelta);
		memcpy_exl(pAd, &pRaCfg->data[40], (UCHAR *) & RSSI0, 4);
		ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 44,
			      NDIS_STATUS_SUCCESS);
	}
					     ...
					     IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + len,
		      NDIS_STATUS_SUCCESS);
					      ...
					      IN struct ate_racfghdr *pRaCfg)
{
	...
	memcpy_exs(pAd, (UCHAR *) buffer + offset, (UCHAR *) pRaCfg->data + 2,
		   len);

	if ((offset + len) <= EEPROM_SIZE) {
		rt_ee_write_bulk(pAd, (USHORT *) (((UCHAR *) buffer) + offset),
				 offset, len);
	} else {
					...
					IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + 3,
		      NDIS_STATUS_SUCCESS);
			     ...
			     IN pRACFGHDR pRaCfg)
{
	...
	while (TableIndex <
	       (sizeof(RACFG_CMD_TABLES) / sizeof(RACFG_CMD_TABLE))) {
		int cmd_index = 0;
		cmd_index = Command_Id - RACFG_CMD_TABLES[TableIndex].cmdOffset;
		if ((cmd_index >= 0)
		    && (cmd_index < RACFG_CMD_TABLES[TableIndex].cmdSetSize)) {
			...
			if (pCmdSet[cmd_index] != NULL)
				Status =
				    (*pCmdSet[cmd_index]) (pAd, wrq, pRaCfg);
			break;
		}
		TableIndex++;
	}
	...
	if (pATEInfo->PassiveMode) {
		...
		for (entry = 0;
		     entry < sizeof(allowedCmds) / sizeof(allowedCmds[0]);
		     entry++) {
		...
		if (Command_Id == RACFG_CMD_BBP_WRITE8) {
			...
			if (offset == BBP_R27
			    || (offset >= BBP_R174 && offset <= BBP_R182))
				allowCmd = TRUE;
		}
...
INT RtmpDoAte(IN PRTMP_ADAPTER pAd,
	      IN RTMP_IOCTL_INPUT_STRUCT * wrq, IN PSTRING wrq_name)
{
	INT32 Status = NDIS_STATUS_SUCCESS;
	struct ate_racfghdr *pRaCfg;
	UINT32 ATEMagicNum;

	os_alloc_mem_suspend(pAd, (UCHAR **) & pRaCfg,
	...
	Status =
	    copy_from_user((PUCHAR) pRaCfg, wrq->u.data.pointer,
			   wrq->u.data.length);
...
VOID ATE_QA_Statistics(IN RTMP_ADAPTER * pAd,
		       IN RXWI_STRUC * pRxWI,
		       IN RXINFO_STRUC * pRxInfo, IN PHEADER_802_11 pHeader)
{
	PATE_INFO pATEInfo = &(pAd->ate);
