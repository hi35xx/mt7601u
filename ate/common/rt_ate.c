...
UCHAR TemplateFrame[32] =
	...
	0x00, 0xAA, 0xBB, 0x12, 0x34, 0x56, 0x00, 0x11, 0x22, 0xAA, 0xBB, 0xCC,
	    0x00, 0x00,
...
CHAR ATEGetDesiredTSSI(IN PRTMP_ADAPTER pAd)
{
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s::Currently not support singledriver.\n", __FUNCTION__));
	return 0;
...
CHAR ATEGetDesiredTSSI(IN PRTMP_ADAPTER pAd)
{
	...
	} else if ((pATEInfo->TxWI.TxWIPHYMODE == MODE_HTMIX)
		   || (pATEInfo->TxWI.TxWIPHYMODE == MODE_HTGREENFIELD)) {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: desiredTSSI = %d, Latest Tx setting: MODE = %d, MCS = %d, STBC = %d\n",
		  __FUNCTION__, desiredTSSI, pATEInfo->TxWI.TxWIPHYMODE,
		  pATEInfo->TxWI.TxWIMCS, pATEInfo->TxWI.TxWISTBC));
	...
	if (pAd->TxPowerCtrl.bInternalTxALC == TRUE) {
		{
				...
				if ((pATEInfo->Channel >= 1)
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("%s: bExtendedTssiMode = %d, original desiredTSSI = %d, CentralChannel = %d, PerChTxPwrOffset = %d\n",
						  ...
						  pATEInfo->Channel,
						  pAd->TxPowerCtrl.
						  PerChTxPwrOffset[pATEInfo->
								   Channel]));

					desiredTSSI +=
					    pAd->TxPowerCtrl.
					    PerChTxPwrOffset[pATEInfo->Channel];
				}
			}
			...
			if (IS_RT5350(pAd)) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("MT7601 Bata driver not support RT30xxReadRFRegister() and RT30xxWriteRFRegister().\n"));
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: desiredTSSI = %d, currentTSSI = %d, idxTxPowerTable = %d, {RF_TX_ALC = %d, MAC_PowerDelta = %d}\n",
				  __FUNCTION__, desiredTSSI, currentTSSI,
				  pAd->TxPowerCtrl.idxTxPowerTable,
				  pTxPowerTuningEntry->RF_TX_ALC,
				  pTxPowerTuningEntry->MAC_PowerDelta));
		}
	...
	for (index = 0; index < maxTxPwrCnt; index++) {
		if (TxPwr[index] != 0xffffffff) {
			for (inner_index = 0; inner_index < 8; inner_index++) {
				...
				TxPwr[index] =
				    (TxPwr[index] &
				     ~(0x0000000F << inner_index *
				       4)) | (Value << inner_index * 4);
			}
			...
			if (IS_RT2883(pAd) || IS_RT3883(pAd)) {
				if (index == 5) {
					RTMP_IO_WRITE32(pAd, TX_PWR_CFG_5,
							TxPwr[index]);
				} else if (index == 6) {
					RTMP_IO_WRITE32(pAd, TX_PWR_CFG_6,
							TxPwr[index]);
				} else {
					RTMP_IO_WRITE32(pAd,
							TX_PWR_CFG_0 +
...
CHAR ATEConvertToRssi(IN PRTMP_ADAPTER pAd, IN CHAR Rssi, IN UCHAR RssiNumber)
{
...
VOID ATESampleRssi(IN RTMP_ADAPTER * pAd, IN RXWI_STRUC * pRxWI)
{
	...
	if (pRxWI->RxWIRSSI0 != 0) {
		pATEInfo->LastRssi0 =
		    ATEConvertToRssi(pAd, (CHAR) pRxWI->RxWIRSSI0, RSSI_0);
		pATEInfo->AvgRssi0X8 =
		    (pATEInfo->AvgRssi0X8 - pATEInfo->AvgRssi0) +
		    pATEInfo->LastRssi0;
	...
	if (pRxWI->RxWIRSSI1 != 0) {
		pATEInfo->LastRssi1 =
		    ATEConvertToRssi(pAd, (CHAR) pRxWI->RxWIRSSI1, RSSI_1);
		pATEInfo->AvgRssi1X8 =
		    (pATEInfo->AvgRssi1X8 - pATEInfo->AvgRssi1) +
		    pATEInfo->LastRssi1;
	...
	if (pRxWI->RxWIRSSI2 != 0) {
		pATEInfo->LastRssi2 =
		    ATEConvertToRssi(pAd, (CHAR) pRxWI->RxWIRSSI2, RSSI_2);
		pATEInfo->AvgRssi2X8 =
		    (pATEInfo->AvgRssi2X8 - pATEInfo->AvgRssi2) +
		    pATEInfo->LastRssi2;
...
VOID rt_ee_write_bulk(PRTMP_ADAPTER pAd, USHORT * Data, USHORT offset,
		      USHORT length)
{
	USHORT pos;
	USHORT value;
...
VOID DefaultATEAsicSetTxRxPath(IN PRTMP_ADAPTER pAd)
{
...
==========================================================================
*/
VOID DefaultATEAsicSwitchChannel(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 Value = 0;
	...
	if (Channel <= 14) {
		...
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R62,
					    (0x37 - GET_LNA_GAIN(pAd)));
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R63,
					    (0x37 - GET_LNA_GAIN(pAd)));
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R64,
					    (0x37 - GET_LNA_GAIN(pAd)));
...
static VOID BbpHardReset(IN PRTMP_ADAPTER pAd)
{
	UINT32 MacData = 0;
...
INT ATETxPwrHandler(IN PRTMP_ADAPTER pAd, IN char index)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
========================================================================
*/
static VOID SetJapanFilter(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UCHAR BbpData = 0;
	...
	if ((pATEInfo->TxWI.TxWIPHYMODE == MODE_CCK)
	    && (pATEInfo->Channel == 14) && (pATEInfo->TxWI.TxWIBW == BW_20)) {
...
========================================================================
*/
VOID ATEDisableAsicProtect(IN PRTMP_ADAPTER pAd)
{
...
==========================================================================
*/
VOID ATEAPStop(IN PRTMP_ADAPTER pAd)
{
	BOOLEAN Cancelled;
	UINT32 Value = 0;
	INT apidx = 0;
...
static NDIS_STATUS ATESTART(IN PRTMP_ADAPTER pAd)
{
	 ...
	 */
		...
		if (atemode == ATE_TXCONT) {
			...
			if (pATEInfo->TxMethod == TX_METHOD_1) {
				BbpSoftReset(pAd);
				RTMP_IO_WRITE32(pAd, RA_TX_PIN_CFG,
						Default_TX_PIN_CFG);
			}
		}
...
/*	pAd->ContinBulkIn = FALSE; */
	...
	if (pATEInfo->PeriodicTimer.State == TRUE) {
		...
		RTMPInitTimer(pAd, &pAd->ate.PeriodicTimer,
			      GET_TIMER_FUNCTION(ATEPeriodicExec), pAd, TRUE);
...
static NDIS_STATUS TXCARR(IN PRTMP_ADAPTER pAd)
{
	...
	if (pChipStruct->bBBPStoreTXCARR == TRUE) {
		...
		for (bbp_index = 0; bbp_index < ATE_BBP_REG_NUM; bbp_index++)
			ATE_BBP_IO_READ8_BY_REG_ID(pAd, bbp_index,
						   &restore_BBP[bbp_index]);
	}
#ifdef RTMP_MAC_USB
	/* Disable Rx */
		...
		if ((IS_RT3350(pAd)) || (IS_RT3352(pAd))) {
			if ((pATEInfo->TxWI.MCS == 7)
				...
				if (pATEInfo->Channel == 7) {
					...
					ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49,
								   &BbpData);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("BBP R49 = 0x%02x\n",
						  BbpData));
...
==========================================================================
*/
static INT ATESetAntennaTxPower(IN PRTMP_ADAPTER pAd,
				IN PSTRING arg, IN INT Antenna)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	CHAR TxPower;
...
VOID ATEAsicTemperCompensation(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	...
	if (pATEInfo->TxWI.TxWIBW == BW_20) {
		if (pATEInfo->Channel <= 14) {
			...
			for (powerIndex = 0;
			     powerIndex < MAX_TXPOWER_ARRAY_SIZE;
			     powerIndex++) {
				if (pAd->Tx20MPwrCfgABand[powerIndex] ==
				    0xffffffff)
					continue;
	...
	/* If bandwidth = 40M, set RF Reg4 bit 21 = 0. */
		...
		if (pATEInfo->Channel <= 14) {
			...
			for (powerIndex = 0;
			     powerIndex < MAX_TXPOWER_ARRAY_SIZE;
			     powerIndex++) {
				if (pAd->Tx40MPwrCfgABand[powerIndex] ==
				    0xffffffff)
					continue;
				...
				RTMP_IO_WRITE32(pAd,
						TX_PWR_CFG_0 + powerIndex * 4,
						pAd->
						Tx40MPwrCfgABand[powerIndex]);
				RtmpOsMsDelay(5);
			}

			if ((pATEInfo->TxWI.TxWIPHYMODE >= 2)
			    && (pATEInfo->TxWI.TxWIMCS == 7)) {
				value = 0x28;
				ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R67,
							    value);
			}
		}
...
INT Set_ATE_TX_MODE_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (IS_RT3350(pAd)) {
		if (pATEInfo->TxWI.TxWIPHYMODE == MODE_CCK) {
			...
			if (rf_value == 0xff)
				rf_value = 0x4F;
			ATE_RF_IO_WRITE8_BY_REG_ID(pAd, rf_offset,
						   (UCHAR) rf_value);
...
INT Set_ATE_Load_E2P_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (value > 0) {
		...
		do {
			...
			FileLength =
			    RtmpOSFileRead(srcf, (PSTRING) WriteEEPROM,
					   EEPROM_SIZE);
	...
	if (value == 0 || value == 1) {
		...
		DBGPRINT(RT_DEBUG_WARN, ("Divider Cal Done:\n"
					 ...
					 (UCHAR) (divParams.gBeg[0] -
						  divParams.gEnd[0]) * 360 /
					 256,
					 (UCHAR) (divParams.gBeg[1] -
						  divParams.gEnd[1]) * 360 /
					 256,
					 (UCHAR) (divParams.gBeg[0] -
						  divParams.gBeg[1]) * 360 /
					 256,
...
==========================================================================
*/
static BOOLEAN rtmp_ate_txbf_cal_valid_ch(IN RTMP_ADAPTER * pAd,
					  IN UCHAR channel)
{
	BOOLEAN bValidCh;
...
INT Set_ATE_Show_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("Addr2=%02x:%02x:%02x:%02x:%02x:%02x\n", pATEInfo->Addr2[0],
		  pATEInfo->Addr2[1], pATEInfo->Addr2[2], pATEInfo->Addr2[3],
		  pATEInfo->Addr2[4], pATEInfo->Addr2[5]));
	DBGPRINT(RT_DEBUG_OFF,
		 ("Addr3=%02x:%02x:%02x:%02x:%02x:%02x\n", pATEInfo->Addr3[0],
		  pATEInfo->Addr3[1], pATEInfo->Addr3[2], pATEInfo->Addr3[3],
...
#else
	DBGPRINT(RT_DEBUG_OFF,
		 ("ATE=APSTOP, APSTART, TXCONT, TXCARR, TXCARS, TXFRAME, RXFRAME\n"));
#endif				/* CONFIG_RT2880_ATE_CMD_NEW */
...
#ifdef RTMP_TEMPERATURE_COMPENSATION
	DBGPRINT(RT_DEBUG_OFF,
		 ("ATEREADEXTSSI, start advanced temperature TSSI calibration.\n"));
#endif				/* RTMP_TEMPERATURE_COMPENSATION */
...
#if defined(RT3350) || defined(RT3352)
INT RT335x2_Set_ATE_TSSI_CALIBRATION_ENABLE_Proc(IN PRTMP_ADAPTER pAd,
						 IN PSTRING arg)
{
...
CHAR InsertTssi(UCHAR InChannel, UCHAR Channel0, UCHAR Channel1, CHAR Tssi0,
		CHAR Tssi1)
{
	CHAR InTssi;
...
INT RT335xATETssiCalibrationExtend(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	NdisCopyMemory(TssiRefPerChannel, pAd->ate.TssiRefPerChannel,
	...
	for (CurrentChannel = 2; CurrentChannel < 7; CurrentChannel++)
		TssiRefPerChannel[CurrentChannel - 1] =
		    InsertTssi(CurrentChannel, 1, 7, TssiRefPerChannel[0],
			       TssiRefPerChannel[6]);

	/* insert channel 8 to 12 TSSI values */
	for (CurrentChannel = 8; CurrentChannel < 13; CurrentChannel++)
		TssiRefPerChannel[CurrentChannel - 1] =
		    InsertTssi(CurrentChannel, 7, 13, TssiRefPerChannel[6],
			       TssiRefPerChannel[12]);
	...
	for (CurrentChannel = 1; CurrentChannel <= 14; CurrentChannel++) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Channel %d, TSSI= 0x%x, TssiDelta=0x%x\n",
	...
	EEPData &= 0x00ff;
	EEPData |=
	    (TssiDeltaPerChannel[0] << 8) | (TssiDeltaPerChannel[1] << 12);
	...
	if (pAd->bUseEfuse) {
		if (pAd->bFroceEEPROMBuffer)
			...
			eFuseWrite(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_1 - 1,
				   (PUCHAR) (&EEPData), 2);
	} else
#endif				/* RTMP_EFUSE_SUPPORT */
	{
	...
	for (CurrentChannel = 3; CurrentChannel <= 14; CurrentChannel += 4) {
		...
		if (pAd->bUseEfuse) {
			if (pAd->bFroceEEPROMBuffer)
				NdisMoveMemory(&
					       (pAd->
						EEPROMImage[(EEPROM_TX_POWER_OFFSET_OVER_CH_3 + ((CurrentChannel - 3) / 2))]), (PUCHAR) (&EEPData), 2);
			else
				eFuseWrite(pAd,
	...
	if (pAd->bUseEfuse) {
		if (pAd->bFroceEEPROMBuffer)
			NdisMoveMemory(&(pAd->EEPROMImage[EEPROM_TSSI_ENABLE]),
				       (PUCHAR) (&EEPData), 2);
		else
			eFuseWrite(pAd, EEPROM_TSSI_ENABLE, (PUCHAR) (&EEPData),
				   2);
	} else
#endif				/* RTMP_EFUSE_SUPPORT */
	{
	...
	if (pAd->bUseEfuse) {
		if (pAd->bFroceEEPROMBuffer)
			NdisMoveMemory(&
				       (pAd->
					EEPROMImage[EEPROM_TSSI_MODE_EXTEND]),
				       (PUCHAR) (&EEPData), 2);
		else
			eFuseWrite(pAd, EEPROM_TSSI_MODE_EXTEND,
				   (PUCHAR) (&EEPData), 2);
	} else
#endif				/* RTMP_EFUSE_SUPPORT */
	{
	...
	NdisCopyMemory(pAd->ate.TssiRefPerChannel, TssiRefPerChannel,
		       CFG80211_NUM_OF_CHAN_2GHZ);
...
#ifdef MT7601
INT Set_ATE_Read_Temperature_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	CHAR Temperature;
...
NDIS_STATUS ATEInit(IN PRTMP_ADAPTER pAd)
{
	...
	NdisZeroMemory((PUCHAR) & (pATEInfo->TssiRefPerChannel),
		       CFG80211_NUM_OF_CHAN_2GHZ);
	NdisZeroMemory((PUCHAR) & (pATEInfo->TssiDeltaPerChannel),
		       CFG80211_NUM_OF_CHAN_2GHZ);
#endif				/* defined(RT3350) || defined(RT3352) */
#endif				/* RTMP_INTERNAL_TX_ALC */
	/* Default TXCONT/TXCARR/TXCARS mechanism is TX_METHOD_1 */
	pATEInfo->TxMethod = TX_METHOD_1;
	if ((IS_RT2070(pAd) || IS_RT2860(pAd) || IS_RT2872(pAd)
	     || IS_RT2883(pAd))) {
...
NDIS_STATUS ATEBBPWriteWithRxChain(IN RTMP_ADAPTER * pAd,
				   IN UCHAR bbpId,
				   IN CHAR bbpVal, IN RX_CHAIN_IDX rx_ch_idx)
{
	UCHAR idx = 0, val = 0;
	...
	while (rx_ch_idx != 0) {
		...
		if (rx_ch_idx & 0x01) {
			...
			val =
			    (val & (~0x60) /* clear bit5 and bit6 */ ) | (idx <<
									  5);
#ifdef RTMP_MAC_USB
			if (IS_USB_INF(pAd)) {
...
INT Set_ADCDump_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR BBP_R21_Ori = 0, BBP_R60_Ori = 0, BBP_R142_ORI = 0, BBP_R143_ORI =
	    0;
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			for (i = 0; i < 0x1000; i++) {
				...
				sprintf(msg, "%d %d %d %d %d %d\n",
					...
					SMMValued.field.LOW_BYTE0,
					PKTValue1d.field.BYTE3,
					PKTValue1d.field.BYTE2,
					PKTValue1d.field.BYTE1,
					PKTValue1d.field.BYTE0);
				sprintf(msg1, "%d %d %d %d %d %d\n",
					SMMValued.field.LOW_BYTE1,
					SMMValued.field.LOW_BYTE0,
					PKTValue2d.field.BYTE3,
					PKTValue2d.field.BYTE2,
					PKTValue2d.field.BYTE1,
					PKTValue2d.field.BYTE0);

				retval =
				    RtmpOSFileWrite(srcf, (PSTRING) msg,
						    strlen(msg));
				retval =
				    RtmpOSFileWrite(srcf, (PSTRING) msg1,
						    strlen(msg1));
			}
		}
	} else {
	...
	if (ATE_ON(pAd)) {
		...
		if (pATEInfo->bRxFER == 1) {
			...
			ate_print(KERN_EMERG
				  "ATEPeriodicExec: Rx packet cnt = %d/%d\n",
