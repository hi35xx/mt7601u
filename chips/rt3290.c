...
REG_PAIR RT3290_BBPRegTable[] = {
	...
	,
	{BBP_R73, 0x13}
	,			/* Rx AGC SQ Acorr threshold */
	{BBP_R75, 0x46}
	,			/* Rx high power VGA offset for LNA offset */
	{BBP_R76, 0x28}
	,			/* Rx medium power VGA offset for LNA offset */
	{BBP_R77, 0x59}
	,
	{BBP_R82, 0x62}
	,
...
UCHAR RT3290_NUM_BBP_REG_PARMS =
    (sizeof(RT3290_BBPRegTable) / sizeof(REG_PAIR));
...
UCHAR RT3290_NUM_MAC_REG_PARMS =
    (sizeof(RT3290_MACRegTable) / sizeof(RTMP_REG_PAIR));
...
CHAR RT3290_desiredTSSIOverHTUsingSTBCExt[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET +
					  1][8];
...
static BOOLEAN RT3290_Asic_Get_TSSI_Info(IN RTMP_ADAPTER * pAd,
...
/*
   ...
   Parameters
    ...
    Return Value:
	  Rounding result
*/
LONG Rounding(IN PRTMP_ADAPTER pAd,
	      IN LONG Integer, IN LONG Fraction, IN LONG DenominatorOfTssiRatio)
{
	LONG temp = 0;

	DBGPRINT(RT_DEBUG_INFO,
		 ("%s: Integer = %d, Fraction = %d, DenominatorOfTssiRatio = %d\n",
		  __FUNCTION__, (INT) Integer, (INT) Fraction,
...
CHAR desire_tssi_calc(IN RTMP_ADAPTER * pAd,
		      IN UCHAR ch,
		      IN CHAR tb_idx,
		      IN UCHAR tssi_base, IN CHAR tssi_ub, IN CHAR tssi_lb)
{
	UCHAR index;
	CHAR DesiredTssi;
	...
	index =
	    GET_TSSI_RATE_TABLE_INDEX(tb_idx +
				      pAd->TxPowerCtrl.PerChTxPwrOffset[ch]);
	DesiredTssi =
	    (SHORT) Rounding(pAd,
			     (tssi_base * RT3290_TssiRatioTable[index][0] /
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s DesiredTssi = 0x%02x\n", print_string, DesiredTssi));

	return DesiredTssi;
}

VOID RT3290_InitDesiredTSSITable(IN PRTMP_ADAPTER pAd)
{
	INT i = 0;
	CHAR DesiredTssi = 0;
	CHAR BWPowerDelta = 0;
	...
	if ((Value & 0xFF) == 0xFF) {	/* 20M/40M BW Power Delta for 2.4GHz band */
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Don't considerate 20M/40M BW Delta Power since EEPROM is not calibrated.\n"));
	} else {
		if ((Value & 0xC0) == 0xC0)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("20/40M BW Delta Power is not enabled, Value = 0x%x\n",
				  Value));
		}
	}
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("TSSIBase = 0x%x, TxPowerOFDM54 = 0x%x, bExtendedTssiMode = %d\n",
	...
	if (bExtendedTssiMode == TRUE) {
		...
		RT28xx_EEPROM_READ16(pAd,
				     (EEPROM_TX_POWER_OFFSET_OVER_CH_1 - 1),
				     TxPwrOffset.word);
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_3,
				     TxPwrOffset.word);
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_7,
				     TxPwrOffset.word);
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_11,
				     TxPwrOffset.word);
		...
		for (i = 1; i <= NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET; i++) {
			if ((pAd->TxPowerCtrl.PerChTxPwrOffset[i] & 0x08) == 0x00)	/* Positive number */
				pAd->TxPowerCtrl.PerChTxPwrOffset[i] =
				    (pAd->TxPowerCtrl.
				     PerChTxPwrOffset[i] & ~0xF8);
			else	/* 0x08: Negative number */
				pAd->TxPowerCtrl.PerChTxPwrOffset[i] =
				    (pAd->TxPowerCtrl.
				     PerChTxPwrOffset[i] | 0xF0);
		}
		...
		RTMPZeroMemory(pAd->TxPowerCtrl.PerChTxPwrOffset,
			       sizeof(pAd->TxPowerCtrl.PerChTxPwrOffset));
	}
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s():EEPROM[0x%02x]=0x%x, TxPwr[]=%02x:%02x, idx[]=%d:%d, MCS=%d:%d!\n",
		  __FUNCTION__, EEPROM_CCK_MCS0_MCS1, Value, TxPower[0],
		  TxPower[1], table_idx[0], table_idx[1], MCS_0, MCS_3));
	for (ch = 1; ch <= NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET; ch++) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s: ch = %d\n", __FUNCTION__, ch));
		for (i = MCS_0; i <= MCS_3; i += 2) {
			DesiredTssi =
			    desire_tssi_calc(pAd, ch, table_idx[i / 2],
					     TSSIBase, 0x7C, 0x0);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\tMCS[%d-%d]:DesiredTssi[0x%02x]\n", i,
				  i + 1, DesiredTssi));
			RT3290_desiredTSSIOverCCKExt[ch][i] =
			    (CHAR) DesiredTssi;
			RT3290_desiredTSSIOverCCKExt[ch][i + 1] =
			    (CHAR) DesiredTssi;
		}
	}
	...
	for (ch = 1; ch <= NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET; ch++) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: ch = %d, RT3290_desiredTSSIOverCCK:\n",
			  __FUNCTION__, ch));
		for (mcs_idx = 0; mcs_idx < 4; mcs_idx++)
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\t[%d] = %d\n", mcs_idx,
				  RT3290_desiredTSSIOverCCKExt[ch][mcs_idx]));
	}
	...
	for (i = 0; i < sizeof(eeprom_ofdm) / sizeof(UCHAR); i++) {
		...
		TxPower[1] = ((Value >> 8) & 0x000F);
		table_idx[0] =
		    TxPower[0] - TxPowerOFDM54 + TSSI_RATIO_TABLE_OFFSET;
		table_idx[1] =
		    TxPower[1] - TxPowerOFDM54 + TSSI_RATIO_TABLE_OFFSET;
		start_mcs = i * 4;
		end_mcs = ((i + 1) * 4) - 1;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s():EEPROM[0x%02x]=0x%x, TxPwr[]=%02x:%02x, idx[]=%d:%d, MCS=%d:%d!\n",
			  __FUNCTION__, ee_addr, Value, TxPower[0], TxPower[1],
			  table_idx[0], table_idx[1], start_mcs, end_mcs));
		for (ch = 1; ch <= NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET; ch++) {
			for (mcs_idx = start_mcs; mcs_idx <= end_mcs;
			     mcs_idx += 2) {
				DesiredTssi =
				    desire_tssi_calc(pAd, ch,
						     table_idx[(mcs_idx -
								start_mcs) / 2],
						     TSSIBase, 0x7C, 0x0);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("ch=%d, index=%d, DesiredTssi=0x%02x\n",
					  ch, index, DesiredTssi));

				RT3290_desiredTSSIOverOFDMExt[ch][mcs_idx] =
				    (CHAR) DesiredTssi;
				RT3290_desiredTSSIOverOFDMExt[ch][mcs_idx + 1] =
				    (CHAR) DesiredTssi;
			}
		}
	}
			...
			for (mcs_idx = start_mcs; mcs_idx <= end_mcs;
				...
				DesiredTssi =
				    desire_tssi_calc(pAd, ch,
						     ...
						     TSSIBase, 0x7C, 0x0);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: ch = %d(BW:20MHz), index = %d, DesiredTssi = 0x%02x\n",
					  __FUNCTION__, ch, index,
					  DesiredTssi));

				RT3290_desiredTSSIOverHTExt[ch][mcs_idx] =
				    (CHAR) DesiredTssi;
				RT3290_desiredTSSIOverHTExt[ch][mcs_idx + 1] =
				    (CHAR) DesiredTssi;
				...
				DesiredTssi =
				    desire_tssi_calc(pAd, ch,
						     table_idx[(mcs_idx -
								start_mcs) /
							       2] +
						     BWPowerDelta, TSSIBase,
						     0x7C, 0x0);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: ch = %d(BW:40MHz), index = %d, DesiredTssi = 0x%02x\n",
					  __FUNCTION__, ch, index,
					  DesiredTssi));
				RT3290_desiredTSSIOverHT40Ext[ch][mcs_idx] =
				    (CHAR) DesiredTssi;
				RT3290_desiredTSSIOverHT40Ext[ch][mcs_idx + 1] =
				    (CHAR) DesiredTssi;
			}
		}
	}
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: ------------------------------------------------------\n",
		  __FUNCTION__));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: The desired TSSI over HT using STBC\n", __FUNCTION__));
			...
			for (mcs_idx = start_mcs; mcs_idx <= end_mcs;
				...
				DesiredTssi =
				    desire_tssi_calc(pAd, ch,
						     ...
						     TSSIBase, 0x7C, 0x0);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: ch = %d, index = %d, DesiredTssi = 0x%02x\n",
					  __FUNCTION__, ch, index,
					  DesiredTssi));

				RT3290_desiredTSSIOverHTUsingSTBCExt[ch]
				    [mcs_idx] = (CHAR) DesiredTssi;
				RT3290_desiredTSSIOverHTUsingSTBCExt[ch][mcs_idx
									 + 1] =
				    (CHAR) DesiredTssi;
			}
		}
	...
	for (ch = 1; ch <= NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET; ch++) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: ch = %d, desiredTSSIOverHTUsingSTBC\n",
			  __FUNCTION__, ch));
		for (mcs_idx = 0; mcs_idx < 8; mcs_idx++)
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\t[%d] = %d\n", mcs_idx,
	...
	Value = (Value & 0x00FF);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: EEPROM_TSSI_GAIN_AND_ATTENUATION = 0x%X\n", __FUNCTION__,
		  Value));
...
/*
   ...
   Return Value:
	  Success or failure
*/
BOOLEAN RT3290_GetDesiredTssiAndCurrentTssi(IN PRTMP_ADAPTER pAd,
					    ...
					    IN OUT PCHAR pCurrentTssi)
{
	...
	if ((BbpR47 & 0x04) == 0x04) {	/* The TSSI INFO is not ready. */
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: BBP TSSI INFO is not ready. (BbpR47 = 0x%X)\n",
			  __FUNCTION__, BbpR47));
		return FALSE;
	}

	if ((pAd->CommonCfg.CentralChannel >= 1)
	    && (pAd->CommonCfg.CentralChannel <= 14))
		ch = pAd->CommonCfg.CentralChannel;
	else {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: Incorrect channel #%d, set as default #%d\n",
	...
	if ((RateInfo & 0x03) == MODE_CCK) {	/* CCK */
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: CCK, cckTssiInfo.field.Rate = %d\n",
			  __FUNCTION__, cckTssiInfo.field.Rate));
		if (((cckTssiInfo.field.Rate >= 4) && (cckTssiInfo.field.Rate <= 7)) || (cckTssiInfo.field.Rate > 11)) {	/* boundary verification */
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: incorrect MCS: cckTssiInfo.field.Rate = %d\n",
				  __FUNCTION__, cckTssiInfo.field.Rate));
		...
		*pDesiredTssi =
		    RT3290_desiredTSSIOverCCKExt[ch][cckTssiInfo.field.Rate];
	} else if ((RateInfo & 0x03) == MODE_OFDM) {	/* OFDM */
		int rate_idx;

		UCHAR bbp_ofdm_rate[] =
		    { 0xb /* 6M */ , 0xf /* 9M */ , 0xa /* 12M */ ,
		...
		*pDesiredTssi =
		    RT3290_desiredTSSIOverOFDMExt[ch][ofdmTssiInfo.field.Rate];
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: HT, htTssiInfo.PartA.field.STBC = %d, htTssiInfo.PartB.field.MCS = %d\n",
			  __FUNCTION__, htTssiInfo.PartA.field.STBC,
			  htTssiInfo.PartB.field.MCS));

		if ((htTssiInfo.PartB.field.MCS < 0) || (htTssiInfo.PartB.field.MCS > 7)) {	/* boundary verification */
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: incorrect MCS: htTssiInfo.PartB.field.MCS = %d\n",
		...
		if (htTssiInfo.PartA.field.STBC == 0) {
			if ((htTssiInfo.PartB.field.BW == BW_40) &&
			    ((htTssiInfo.PartB.field.MCS == MCS_5)
			     || (htTssiInfo.PartB.field.MCS == MCS_6)
			     || (htTssiInfo.PartB.field.MCS == MCS_7))) {
				*pDesiredTssi =
				    RT3290_desiredTSSIOverHT40Ext[ch]
				    [htTssiInfo.PartB.field.MCS];
			} else {
				*pDesiredTssi =
				    RT3290_desiredTSSIOverHTExt[ch][htTssiInfo.
								    PartB.field.
								    MCS];
			}
		} else {
			*pDesiredTssi =
			    RT3290_desiredTSSIOverHTUsingSTBCExt[ch][htTssiInfo.
								     PartB.
								     field.MCS];
		}
	}
...
VOID RT3290_AsicTxAlcGetAutoAgcOffset(IN PRTMP_ADAPTER pAd,
				      IN PCHAR pDeltaPwr,
				      ...
				      IN PCHAR pDeltaPowerByBbpR1)
{
	const TX_POWER_TUNING_ENTRY_STRUCT *TxPowerTuningTable =
	    pAd->chipCap.TxPowerTuningTable_2G;
	...
	if (pAd->TxPowerCtrl.bInternalTxALC == TRUE) {
		if ((pAd->Mlme.OneSecPeriodicRound % 4 == 0)
		    && (*pDeltaPowerByBbpR1 == 0)) {
			if (RT3290_GetDesiredTssiAndCurrentTssi
			    (pAd, &desiredTssi, &currentTssi) == FALSE) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: Incorrect desired TSSI or current TSSI\n",
					  __FUNCTION__));

				/* Tx power adjustment over RF */
				RT30xxReadRFRegister(pAd, RF_R49,
						     (PUCHAR) (&RFValue));
				RFValue =
				    ((RFValue & ~0x3F) | pAd->TxPowerCtrl.
				     RF_TX_ALC);
				if ((RFValue & 0x3F) > 0x27) {	/* The valid range of the RF R49 (<5:0>tx0_alc<5:0>) is 0x00~0x27 */
					RFValue = ((RFValue & ~0x3F) | 0x27);
				}
				RT30xxWriteRFRegister(pAd, RF_R49,
						      (UCHAR) (RFValue));
				...
				TuningTableIndex =
				    ...
				    +
				    pAd->TxPower[pAd->CommonCfg.CentralChannel -
						 1].Power;
#else
				    + pAd->TxPower[pAd->CommonCfg.Channel -
						   1].Power;
				...
				pTxPowerTuningEntry =
				    &TxPowerTuningTable[TuningTableIndex +
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: desiredTSSI = %d, currentTSSI = %d, idxTxPowerTable = %d, TuningTableIndex = %d, {RF_TX_ALC = 0x%X, MAC_PowerDelta = %d}\n",
					  __FUNCTION__, desiredTssi,
					  currentTssi,
					  pAd->TxPowerCtrl.idxTxPowerTable,
					  TuningTableIndex,
					  pTxPowerTuningEntry->RF_TX_ALC,
			...
			RFValue =
			    ((RFValue & ~0x3F) | pAd->TxPowerCtrl.RF_TX_ALC);
			if ((RFValue & 0x3F) > 0x27) {	/* The valid range of the RF R49 (<5:0>tx0_alc<5:0>) is 0x00~0x1F */
				RFValue = ((RFValue & ~0x3F) | 0x27);
			}
...
#ifdef RALINK_ATE
INT RT3290_ATETssiCalibration(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR inputDAC;
	UINT i = 0;
	...
	if (!(pAd->TxPowerCtrl.bInternalTxALC)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Not support TSSI calibration since not 5390 chip or EEPROM not set!!!\n"));
		return FALSE;
	}
	...
	if (pAd->bUseEfuse) {
		if (pAd->bFroceEEPROMBuffer)
			NdisMoveMemory(&
				       (pAd->
					EEPROMImage[EEPROM_TSSI_OVER_OFDM_54]),
				       (PUCHAR) (&EEPData), 2);
		else
			eFuseWrite(pAd, EEPROM_TSSI_OVER_OFDM_54,
				   (PUSHORT) (&EEPData), 2);
	}
...
CHAR RT3290_RTATEInsertTssi(UCHAR InChannel, UCHAR Channel0, UCHAR Channel1,
			    CHAR Tssi0, CHAR Tssi1)
{
	...
	if ((TssiDelta > 0)
	    && (((InChannelDelta * TssiDelta * 10) / ChannelDelta) % 10 >= 5)) {
		InTssi = Tssi0 + ((InChannelDelta * TssiDelta) / ChannelDelta);
		InTssi += 1;
	} else if ((TssiDelta < 0)
		   && (((InChannelDelta * TssiDelta * 10) / ChannelDelta) %
		       10 <= -5)) {
		InTssi = Tssi0 + ((InChannelDelta * TssiDelta) / ChannelDelta);
		InTssi -= 1;
	} else {
...
UCHAR RT3290_RTATEGetTssiByChannel(PRTMP_ADAPTER pAd, UCHAR Channel)
{
	...
	RT28xx_EEPROM_READ16(pAd, EEPROM_G_TX_PWR_OFFSET + Channel - 1,
			     ChannelPower);
	pAd->ate.TxPower0 = (UCHAR) (ChannelPower & 0xff);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Channel %d, Calibrated Tx.Power0= 0x%x\n", Channel,
		  pAd->ate.TxPower0));
...
CHAR RT3290_GetPowerDeltaFromTssiRatio(CHAR TssiOfChannel, CHAR TssiBase)
{
	...
	DBGPRINT(RT_DEBUG_WARN,
		 ("TssiOfChannel = %d, TssiBase = %d, TssiRatio = %ld\n",
	...
	DBGPRINT(RT_DEBUG_WARN,
		 ("MinTssiDeltaIndex = %d, MinTssiDelta = %ld, PowerDelta = %d\n",
		  MinTssiDeltaIndex, MinTssiDelta, PowerDelta));

	return (PowerDelta);
}

INT RT3290_ATETssiCalibrationExtend(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR inputData;
	...
	if ((pAd->TxPowerCtrl.bInternalTxALC) && (pAd->TxPowerCtrl.bExtendedTssiMode))
		...
		UCHAR RFValue;
		CHAR TssiRefPerChannel[14 + 1], PowerDeltaPerChannel[14 + 1],
		    TssiBase;
		USHORT EEPData;
		...
		CurrentChannel = 7;
		TssiRefPerChannel[CurrentChannel] =
		    RT3290_RTATEGetTssiByChannel(pAd, CurrentChannel);
		TssiBase = TssiRefPerChannel[CurrentChannel];
		PowerDeltaPerChannel[CurrentChannel] =
		    RT3290_GetPowerDeltaFromTssiRatio(TssiRefPerChannel
						      [CurrentChannel],
						      TssiBase);
		...
		for (CurrentChannel = 2; CurrentChannel < 7; CurrentChannel++)
			PowerDeltaPerChannel[CurrentChannel] =
			    RT3290_RTATEInsertTssi(CurrentChannel, 1, 7,
						   PowerDeltaPerChannel[1],
						   PowerDeltaPerChannel[7]);
		...
		for (CurrentChannel = 8; CurrentChannel < 13; CurrentChannel++)
			PowerDeltaPerChannel[CurrentChannel] =
			    RT3290_RTATEInsertTssi(CurrentChannel, 7, 13,
						   PowerDeltaPerChannel[7],
						   PowerDeltaPerChannel[13]);
		...
		for (CurrentChannel = 1; CurrentChannel <= 14; CurrentChannel++) {
			DBGPRINT(RT_DEBUG_WARN,
				 ("Channel %d, PowerDeltaPerChannel= 0x%x\n",
				  CurrentChannel,
				  PowerDeltaPerChannel[CurrentChannel]));
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_1 - 1,
				     EEPData);
		EEPData &= 0x00ff;
		EEPData |=
		    (PowerDeltaPerChannel[1] << 8) | (PowerDeltaPerChannel[2] <<
						      12);
		RT28xx_EEPROM_WRITE16(pAd, EEPROM_TX_POWER_OFFSET_OVER_CH_1 - 1,
				      EEPData);

		for (CurrentChannel = 3; CurrentChannel <= 14;
		     CurrentChannel += 4) {
			...
			EEPData =
			    (PowerDeltaPerChannel[CurrentChannel + 3] << 12) |
			    (PowerDeltaPerChannel[CurrentChannel + 2] << 8) |
			    (PowerDeltaPerChannel[CurrentChannel + 1] << 4) |
			    PowerDeltaPerChannel[CurrentChannel];
...
/*
	...
	==========================================================================
 */
VOID RT3290LoadRFNormalModeSetup(IN PRTMP_ADAPTER pAd) {
...
/*
	...
	==========================================================================
 */
VOID RT3290LoadRFSleepModeSetup(IN PRTMP_ADAPTER pAd) {
/*	UCHAR RFValue;*/
/*	UINT32 MACValue;*/
	UCHAR rfreg;
...
/*
	...
	==========================================================================
 */
VOID RT3290ReverseRFSleepModeSetup(IN PRTMP_ADAPTER pAd,
...
VOID RT3290HaltAction(IN PRTMP_ADAPTER pAd) {
...
VOID RT3290_AsicResetBbpAgent(IN PRTMP_ADAPTER pAd) {
	// TODO:
}
...
VOID NICInitRT3290RFRegisters(IN PRTMP_ADAPTER pAd) {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: Initialize the RF registers to the default values",
		  __FUNCTION__));

	/* Initialize RF register to default value */
	for (i = 0; i < RT3290_NUM_RF_REG_PARMS; i++) {
		if (IS_RT3290LE(pAd)
		    && (RT3290_RFRegTable[i].Register == RF_R25))
...
========================================================================
*/
VOID NICInitRT3290MacRegisters(IN RTMP_ADAPTER * pAd) {
	// TODO: shiang, Fix me
...
========================================================================
*/
VOID NICInitRT3290BbpRegisters(IN PRTMP_ADAPTER pAd) {
...
VOID RT3290_AsicAntennaDefaultReset(IN struct _RTMP_ADAPTER *pAd,
				    IN union _EEPROM_ANTENNA_STRUC *pAntenna) {
	pAntenna->word = 0;
	pAntenna->field.RfIcType = 0xf;
	pAntenna->field.TxPath = 1;
	pAntenna->field.RxPath = 1;
} VOID RT3290_AsicSetRxAnt(IN PRTMP_ADAPTER pAd, IN UCHAR Ant) {
...
VOID RT3290_ChipBBPAdjust(IN RTMP_ADAPTER * pAd) {
	// TODO: shiang, Fix me

}

#ifdef CONFIG_STA_SUPPORT
VOID RT3290_NetDevNickNameInit(IN struct _RTMP_ADAPTER *pAd) {
	snprintf((PSTRING) pAd->nickname, sizeof(pAd->nickname), "RT3290STA");
} VOID RT3290_RxSensitivityTuning(IN PRTMP_ADAPTER pAd) {
...
UCHAR RT3290_ChipAGCAdjust(IN PRTMP_ADAPTER pAd,
			   IN CHAR Rssi, IN UCHAR OrigR66Value) {
	UCHAR R66 = 0x1C + 2 * GET_LNA_GAIN(pAd);
	UCHAR r83_val;
...
VOID RT3290_ChipAGCInit(IN PRTMP_ADAPTER pAd, IN UCHAR BandWidth) {
	UCHAR R66 = 0x1C + 2 * GET_LNA_GAIN(pAd);
...
/*
	...
	========================================================================
*/
VOID RTMPRT3290ReadTxPwrPerRate(IN PRTMP_ADAPTER pAd) {
	// TODO: shiang, Fix me
...
/*
	...
	========================================================================
*/
VOID RTMPRT3290ReadChannelPwr(IN PRTMP_ADAPTER pAd) {
	// TODO: shiang, Fix me

}

/* ATE will also call this function to set GPIO, channel=36 to set low and channel=1 to set high */
VOID RTMPRT3290ABandSel(IN UCHAR Channel) {
	// TODO: shiang, Fix me

}

VOID RT3290_ChipSetBW(IN RTMP_ADAPTER * pAd, IN UCHAR bandwidth) {

}

VOID RT3290_ChipLDOCoreLvAdjust(IN RTMP_ADAPTER * pAd, IN BOOLEAN bHalt) {
...
VOID RT3290_ChipSwitchChannel(IN PRTMP_ADAPTER pAd,
	...
	for (index = 0; index < MAX_NUM_OF_CHANNELS; index++) {
		if (Channel == pAd->TxPower[index].Channel) {
			...
			if (IS_RT3090A(pAd)) {
				Tx0FinePowerCtrl =
				    pAd->TxPower[index].Tx0FinePowerCtrl;
				Tx1FinePowerCtrl =
				    pAd->TxPower[index].Tx1FinePowerCtrl;
			}
	...
	for (index = 0; index < NUM_OF_3020_CHNL; index++) {
		if (Channel == FreqItems3020[index].Channel) {
			...
			if (pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration == TRUE && INFRA_ON(pAd)) {	// Update the frequency offset from the adaptive frequency offset
			...
			if ((!bScan) && (pAd->CommonCfg.BBPCurrentBW == BW_40))
				TxRxh20M =
				    ((pAd->Mlme.CaliBW40RfR24 & 0x20) >> 5);
			else
				TxRxh20M =
				    ((pAd->Mlme.CaliBW20RfR24 & 0x20) >> 5);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02x, K=0x%02x, R=0x%02x\n",
				  __FUNCTION__, Channel, pAd->RfIcType, TxPwer,
...
VOID MlmeAntSelection(IN PRTMP_ADAPTER pAd,
...
*/
INT RT3290_eeprom_access_grant(IN RTMP_ADAPTER * pAd, IN BOOLEAN bGetCtrl) {
	ULONG btFunInfo = 0;
...
*/
VOID RTMP_MAC_PWRSV_EN(IN PRTMP_ADAPTER pAd,
...
VOID RT3290_LinkDown(IN RTMP_ADAPTER * pAd) {
	WLAN_FUN_CTRL_STRUC WlanFunCtrl = {
	.word = 0};
	CMB_CTRL_STRUC CmbCtrl = {
	.word = 0};
	...
	if ((CmbCtrl.field.LDO3_EN == TRUE)
	    && (CmbCtrl.field.LDO_CORE_LEVEL == 2) && !IS_RT3290LE(pAd)) {
		// use 1.2v
		CmbCtrl.field.LDO_CORE_LEVEL = 0;
		RTMP_IO_WRITE32(pAd, CMB_CTRL, CmbCtrl.word);
	}
...
VOID RT3290_LinkUp(IN RTMP_ADAPTER * pAd) {
	UCHAR Value;
...
/*
	...
	========================================================================
*/
VOID RTMPEnableWlan(IN PRTMP_ADAPTER pAd, IN BOOLEAN bOn, IN BOOLEAN bResetWLAN) {
	WLAN_FUN_CTRL_STRUC WlanFunCtrl = {
	.word = 0};
	CMB_CTRL_STRUC CmbCtrl;
	...
	if (bOn == TRUE) {
		...
		do {
			RTMP_IO_FORCE_READ32(pAd, WPDMA_GLO_CFG, &GloCfg.word);
			if ((GloCfg.field.RxDMABusy == 0)
			    || (GloCfg.word == 0xFFFFFFFF))
				break;
	...
	do {
		...
		if (bOn == TRUE) {
			...
			if (index >= 100) {
				...
				RTMP_IO_FORCE_READ32(pAd, WPDMA_GLO_CFG,
						     &GloCfg.word);
			...
			if (bResetWLAN) {
				RTMP_IO_FORCE_WRITE32(pAd, WLAN_FUN_CTRL,
						      pAd->WlanFunCtrl.
						      word | 0x8);
				RTMPusecDelay(2);
			}
...
========================================================================
*/
VOID RT3290_Init(IN PRTMP_ADAPTER pAd) {
	// TODO: shiang, Fix me
