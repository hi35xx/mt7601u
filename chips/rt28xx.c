...
VOID RT28xx_ChipSwitchChannel(IN PRTMP_ADAPTER pAd,
			      IN UCHAR Channel, IN BOOLEAN bScan)
{
	...
	case RFIC_2750:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("SwitchChannel#%d(RF=%d, Pwr0=%lu, Pwr1=%lu, %dT) to , R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
	...
	if (Channel <= 14) {
		...
		/* Rx High power VGA offset for LNA select */
		{
			if (pAd->NicConfig2.field.ExternalLNAForG) {
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R82,
							     0x62);
		...
		/* Rx High power VGA offset for LNA select */
		bbpValue =
		    (pAd->NicConfig2.field.ExternalLNAForA) ? 0x46 : 0x50;
