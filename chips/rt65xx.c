...
/*
	...
	========================================================================
*/
VOID ral_wlan_chip_onoff(IN RTMP_ADAPTER * pAd,
...
VOID dump_bw_info(RTMP_ADAPTER * pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tTx/Rx BandwidthCtrl(CORE_R1[4:3])=%d(%s MHz)\n", bw,
		  bw_str[bw]));

	DBGPRINT(RT_DEBUG_TRACE, ("[AGC_R0]\n"));
	prim_ch_idx = ((agc_r0 & 0x300) >> 8) & 0xff;
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tPrimary Channel Idx(AGC_R0[9:8])=%d\n", prim_ch_idx));
	decode_cap = ((agc_r0 & 0x7000) >> 12);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tDecodeBWCap(AGC_R0[14:12])=%d(%s MHz Data)\n", decode_cap,
		  decode_str[decode_cap]));
