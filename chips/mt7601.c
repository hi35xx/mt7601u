...
UCHAR MT7601_EFUSE_DEFAULT_BIN[] = {
	0x01, 0x76, 0x00, 0x0c, 0x00, 0x0c, 0x43, 0x26, 0x60, 0x40, 0xff, 0xff,
	    0xff, 0xff, 0xff, 0xff,
	0x01, 0x02, 0x8f, 0x14, 0x01, 0x76, 0x00, 0x00, 0x4a, 0x00, 0x01, 0x00,
	    0x80, 0x50, 0x08, 0x00,
	0x03, 0x00, 0x02, 0x00, 0x00, 0x00, 0xff, 0x02, 0x00, 0x20, 0x40, 0xff,
	    0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0x11, 0xff, 0x04, 0x28, 0xff, 0xff, 0x2c, 0x01,
	    0xff, 0xff, 0x99, 0x99,
	0x8c, 0x88, 0xff, 0x07, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	    0x00, 0x00, 0x00, 0x00,
	...
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	    0xff, 0xff, 0xff, 0xff,
	0x20, 0xf9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	    0x00, 0x00, 0x05, 0x05,
	0x03, 0x03, 0x04, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x02, 0x00,
	    0x00, 0x00, 0x02, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	    0xff, 0xff, 0xff, 0xff,
};
...
static UCHAR MT7601_NUM_MAC_REG_PARMS =
    (sizeof(MT7601_MACRegTable) / sizeof(RTMP_REG_PAIR));
...
static UCHAR MT7601_BBP_InitRegTb_Size =
    (sizeof(MT7601_BBP_InitRegTb) / sizeof(RTMP_REG_PAIR));
...
const UCHAR MT7601_BBP_BW20RegTb_Size =
    (sizeof(MT7601_BBP_BW20RegTb) / sizeof(RTMP_REG_PAIR));
...
const UCHAR MT7601_BBP_BW40RegTb_Size =
    (sizeof(MT7601_BBP_BW40RegTb) / sizeof(RTMP_REG_PAIR));
...
const static UCHAR MT7601_BBP_CommonRegTb_Size =
    (sizeof(MT7601_BBP_CommonRegTb) / sizeof(RTMP_REG_PAIR));
...
const UCHAR MT7601_BBP_HighTempBW20RegTb_Size =
    (sizeof(MT7601_BBP_HighTempBW20RegTb) / sizeof(RTMP_REG_PAIR));
...
const UCHAR MT7601_BBP_HighTempBW40RegTb_Size =
    (sizeof(MT7601_BBP_HighTempBW40RegTb) / sizeof(RTMP_REG_PAIR));
...
const static UCHAR MT7601_BBP_HighTempCommonRegTb_Size =
    (sizeof(MT7601_BBP_HighTempCommonRegTb) / sizeof(RTMP_REG_PAIR));
...
const UCHAR MT7601_BBP_LowTempBW20RegTb_Size =
    (sizeof(MT7601_BBP_LowTempBW20RegTb) / sizeof(RTMP_REG_PAIR));
...
UCHAR MT7601_BBP_LowTempBW40RegTb_Size =
    (sizeof(MT7601_BBP_LowTempBW40RegTb) / sizeof(RTMP_REG_PAIR));

static RTMP_REG_PAIR MT7601_BBP_LowTempCommonRegTb[] = {
	{BBP_R178, 0xFF},	// For CCK CH14 OBW
};

const static UCHAR MT7601_BBP_LowTempCommonRegTb_Size =
    (sizeof(MT7601_BBP_LowTempCommonRegTb) / sizeof(RTMP_REG_PAIR));
...
static UINT32 MT7601_RF_Central_RegTb_Size =
    (sizeof(MT7601_RF_Central_RegTb) / sizeof(BANK_RF_REG_PAIR));
...
static UINT32 MT7601_RF_Channel_RegTb_Size =
    (sizeof(MT7601_RF_Channel_RegTb) / sizeof(BANK_RF_REG_PAIR));
...
static UINT32 MT7601_RF_VGA_RegTb_Size =
    (sizeof(MT7601_RF_VGA_RegTb) / sizeof(BANK_RF_REG_PAIR));
...
UINT32 NUM_OF_MT7601_CHNL =
    (sizeof(MT7601_Frequency_Plan) / sizeof(MT7601_FREQ_ITEM));
...
/*
	...
	========================================================================
*/
VOID MT7601_WLAN_ChipOnOff(IN RTMP_ADAPTER * pAd,
			   IN BOOLEAN bOn, IN BOOLEAN bResetWLAN)
{
	...
	DBGPRINT(RT_DEBUG_INFO,
		 ("==>%s(): OnOff:%d, pAd->WlanFunCtrl:0x%x, Reg-WlanFunCtrl=0x%x\n",
...
//1012 BBP CR
	AndesBBPRandomWrite(pAd, 4,
			    BBP_R158, 0x8D,
			    BBP_R159, 0xFC, BBP_R158, 0x8C, BBP_R159, 0x4C);
	...
	AndesBBPRandomWrite(pAd, 2, BBP_R158, 0x8D, BBP_R159, 0xE0);
...
//      UCHAR Temperature;
	...
	pAd->chipCap.CurrentTemperature =
	    (pAd->chipCap.CurrentTemperBbpR49 -
	     pAd->chipCap.TemperatureRef25C) * MT7601_E2_TEMPERATURE_SLOPE;
...
#ifdef DPD_CALIBRATION_SUPPORT
	/* DPD-Calibration */
	AndesCalibrationOP(pAd, ANDES_CALIBRATION_DPD,
			   pAd->chipCap.CurrentTemperature);
#endif				/* DPD_CALIBRATION_SUPPORT */
...
========================================================================
*/
VOID InitFce(PRTMP_ADAPTER pAd)
{
...
//      UINT32 IdReg;
	...
	AndesRFRandomWritePair(pAd, MT7601_RF_Central_RegTb,
			       MT7601_RF_Central_RegTb_Size);

	AndesRFRandomWritePair(pAd, MT7601_RF_Channel_RegTb,
			       MT7601_RF_Channel_RegTb_Size);

	AndesRFRandomWritePair(pAd, MT7601_RF_VGA_RegTb,
			       MT7601_RF_VGA_RegTb_Size);
...
========================================================================
*/
static VOID NICInitMT7601BbpRegisters(IN PRTMP_ADAPTER pAd)
{
//      INT IdReg;
	...
	AndesBBPRandomWritePair(pAd, MT7601_BBP_InitRegTb,
				MT7601_BBP_InitRegTb_Size);

	return;
}

static VOID MT7601_AsicAntennaDefaultReset(IN struct _RTMP_ADAPTER *pAd,
...
#ifdef DOT11_N_SUPPORT
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s() : %s, ChannelWidth=%d, Channel=%d, ExtChanOffset=%d(%d) \n",
...
/*
	...
	==========================================================================
 */
UCHAR MT7601_ChipAGCAdjust(IN PRTMP_ADAPTER pAd,
			   IN CHAR Rssi, IN UCHAR OrigR66Value)
{
	...
	if (pAd->LatchRfRegs.Channel <= 14) {	/* BG band */
		...
		if (AGC_AUX_VGA != Orig_AGC_AUX_VGA) {
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x87);
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196,
						     AGC_AUX_VGA);
		}
...
VOID MT7601_ChipAGCInit(IN PRTMP_ADAPTER pAd, IN UCHAR BandWidth)
{
	UCHAR R66 = 0x14;
	CHAR lan_gain = GET_LNA_GAIN(pAd);
...
static VOID MT7601_ChipSwitchChannel(struct _RTMP_ADAPTER *pAd,
				     UCHAR Channel, BOOLEAN bScan)
{
...
//      INT IdReg;
...
#endif				/* RTMP_INTERNAL_TX_ALC */
	{
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("SkuBasePwr = 0x%x,  DefaultTargetPwr = 0x%x, ChannelPwrAdj 0x13b4: 0x%x\n",
...
#ifdef RTMP_INTERNAL_TX_ALC
	if (pAd->TxPowerCtrl.bInternalTxALC)
		UpdateSkuRatePwr(pAd, Channel, pAd->CommonCfg.BBPCurrentBW,
				 TxPwer);
	else
#endif				/* RTMP_INTERNAL_TX_ALC */
		UpdateSkuRatePwr(pAd, Channel, pAd->CommonCfg.BBPCurrentBW,
...
//      UINT32 Value, 
	...
	if (Stage == SUSPEND_RADIO_OFF) {
		...
		if (MT7601DisableTxRx(pAd, DOT11POWERSAVE) ==
		    STATUS_UNSUCCESSFUL) {
...
*/
	...
	AndesRFRandomWrite(pAd, 2,
			   RF_BANK0, RF_R04, 0x0A, RF_BANK0, RF_R05, 0x20);
	...
	if (pAd->Mlme.bSendNullFrameAfterWareUp == TRUE) {
		...
		RTMPSendNullFrame(pAd,
				  ...
				  (pAd->CommonCfg.bWmmCapable & pAd->CommonCfg.
				   APEdcaParm.bValid),
				  pAd->CommonCfg.
...
//      BOOLEAN bUseDefault = TRUE;
	...
	if ((tx_pwr1 == 0x0) || (tx_pwr1 > max_tx1_pwr)) {
		tx_pwr1 = 0x20;
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: EEPROM 0xD0 Error! Use Default Target Power = 0x%x\n",
			  __FUNCTION__, tx_pwr1));
	}
	...
	for (i = 0; i < 7; i++) {
		...
		if ((Power.field.Byte0 <= max_tx1_pwr)
		    && (Power.field.Byte0 >= 0))
			tx_pwr1 = Power.field.Byte0;

		if ((Power.field.Byte1 <= max_tx1_pwr)
		    || (Power.field.Byte1 >= 0))
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: TxPower[%d].Power = 0x%02X, TxPower[%d].Power = 0x%02X\n",
			  __FUNCTION__, i * 2, pAd->TxPower[i * 2].Power,
			  i * 2 + 1, pAd->TxPower[i * 2 + 1].Power));
...
VOID MT7601_AsicExtraPowerOverMAC(IN PRTMP_ADAPTER pAd)
{
...
#if defined(RTMP_INTERNAL_TX_ALC) || defined(SINGLE_SKU_V2)
VOID MT7601_InitPAModeTable(IN PRTMP_ADAPTER pAd)
{
	INT32 PAMode;
	UINT32 Value = 0;
	UINT16 index, offset;
	...
	for (index = 0, offset = 0; index < 16; index++, offset += 2) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("PAModeHT[%d] = %d\n", index,
			  pAd->chipCap.PAModeHT[index]));
	}

}
#endif				/* defined(RTMP_INTERNAL_TX_ALC) || defined(SINGLE_SKU_V2) */

VOID MT7601_InitTemperatureCompensation(IN PRTMP_ADAPTER pAd)
{
	UINT32 Value = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("pAd->TemperatureRef25C = 0x%x\n",
		  pAd->chipCap.TemperatureRef25C));

}

VOID MT7601_TemperatureCompensation(IN PRTMP_ADAPTER pAd)
{
	RTMP_CHIP_CAP *pChipCap = &pAd->chipCap;
	...
	if (pAd->CommonCfg.bDynaPDEnable == TRUE) {
		DBGPRINT(RT_DEBUG_INFO,
			 ("Forced to disable TemperatureCompensation when DynaPDEnable=1.\n"));
		return;
	}
...
VOID MT7601AsicTemperatureCompensation(IN PRTMP_ADAPTER pAd,
				       IN BOOLEAN bPowerOn)
{
	INT32 CurrentTemper;
//      INT IdReg;
...
#ifdef DPD_CALIBRATION_SUPPORT
	/* DPD Calibration */
	if (((CurrentTemper - pChipCap->TemperatureDPD) > 450)
	    || ((CurrentTemper - pChipCap->TemperatureDPD) < -450)) {
		pChipCap->TemperatureDPD = CurrentTemper;

		/* DPD-Calibration */
		AndesCalibrationOP(pAd, ANDES_CALIBRATION_DPD,
				   pChipCap->TemperatureDPD);
	...
	if (CurrentTemper > high_temp_cr_threshold)	// (60 - 25) * 10 = 350
	{
		if (pChipCap->TemperatureMode != TEMPERATURE_MODE_HIGH) {
			...
			AndesBBPRandomWritePair(pAd,
						MT7601_BBP_HighTempCommonRegTb,
						MT7601_BBP_HighTempCommonRegTb_Size);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s::Change to TEMPERATURE_MODE_HIGH\n",
				  __FUNCTION__));
		}
	} else if (CurrentTemper > low_temp_cr_threshold)	// ( 0 - 25 ) * 10
	{
		if (pChipCap->TemperatureMode != TEMPERATURE_MODE_NORMAL) {
			...
			AndesBBPRandomWritePair(pAd, MT7601_BBP_CommonRegTb,
						MT7601_BBP_CommonRegTb_Size);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s::Change to TEMPERATURE_MODE_NORMAL\n",
				  __FUNCTION__));
		}
	} else {
		if (pChipCap->TemperatureMode != TEMPERATURE_MODE_LOW) {
			...
			AndesBBPRandomWritePair(pAd,
						MT7601_BBP_LowTempCommonRegTb,
						MT7601_BBP_LowTempCommonRegTb_Size);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s::Change to TEMPERATURE_MODE_LOW\n",
				  __FUNCTION__));
		}
	}
...
VOID MT7601_TssiDcGainCalibration(RTMP_ADAPTER * pAd)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (i == 1)
			...
			tssi0_db_hvga =
			    lin2dBd(tssi_linear - pTxALCData->TssiDC0_HVGA);
...
//      UINT16 index, offset;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("TSSI slope = 0x%x, offset[0] = 0x%x, offset[1] = 0x%x, offset[2] = 0x%x, TSSI_USE_HVGA = %x\n",
		  pTxALCData->TssiSlope, pTxALCData->MT7601_TSSI_OFFSET[0],
		  pTxALCData->MT7601_TSSI_OFFSET[1],
		  pTxALCData->MT7601_TSSI_OFFSET[2],
		  pTxALCData->TSSI_USE_HVGA));

	RT28xx_EEPROM_READ16(pAd, EEPROM_TX0_TSSI_OFFSET, Value);
	pTxALCData->MT7601_TSSI_T0_Delta_Offset =
	    ((CHAR) (Value & 0xFF)) * 1024;
	DBGPRINT(RT_DEBUG_TRACE,
		 ("TSSI T0 Delta offset = %d\n",
		  pTxALCData->MT7601_TSSI_T0_Delta_Offset));

	RTMP_IO_READ32(pAd, TX_ALC_CFG_1, &Value);
	init_offset =
	    ((pTxALCData->TSSI0_DB * pTxALCData->TssiSlope) +
	     pTxALCData->MT7601_TSSI_OFFSET[1]) / 4096;
...
BOOLEAN MT7601_GetTemperatureCompensationParam(IN PRTMP_ADAPTER pAd,
...
VOID MT7601_AsicTxAlcGetAutoAgcOffset(IN PRTMP_ADAPTER pAd,
				      IN PCHAR pDeltaPwr,
...
//      UCHAR BBPReg;
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0) {
		...
		if (MT7601_GetTemperatureCompensationParam
...
INT MT7601_Bootup_Read_Temperature(IN PRTMP_ADAPTER pAd, OUT CHAR * Temperature)
{
...
INT MT7601_Read_Temperature(IN PRTMP_ADAPTER pAd, OUT CHAR * Temperature)
{
	UCHAR BBPReg = 0xff;
	int i;

#ifdef RTMP_INTERNAL_TX_ALC
	if ((pAd->chipCap.TxALCData.TssiTriggered == 1)
...
#ifdef MICROWAVE_OVEN_SUPPORT
VOID MT7601_AsicMeasureFalseCCA(IN PRTMP_ADAPTER pAd)
{
	UINT32 reg;
	/* restore LAN gain */
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Stored_BBP_R65=%x RF_BANK5_R06: %x RF_BANK5_R07: %x @%s \n",
		  pAd->CommonCfg.MO_Cfg.Stored_BBP_R65,
		  pAd->CommonCfg.MO_Cfg.Stored_RF_B5_R6,
		  pAd->CommonCfg.MO_Cfg.Stored_RF_B5_R7, __FUNCTION__));
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R65,
				     pAd->CommonCfg.MO_Cfg.Stored_BBP_R65);
	/* restore RF LPF threshold */
	rlt_rf_write(pAd, RF_BANK5, RF_R06,
		     pAd->CommonCfg.MO_Cfg.Stored_RF_B5_R6);
	rlt_rf_write(pAd, RF_BANK5, RF_R07,
		     pAd->CommonCfg.MO_Cfg.Stored_RF_B5_R7);
...
VOID MT7601_AsicMitigateMicrowave(IN PRTMP_ADAPTER pAd)
{
	UINT8 RegValue;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("BBP_R65=%x ,RF_BANK5_R06 and RF_BANK5_R07: 0x3F, @%s \n",
...
========================================================================
*/
CHAR MT7601_get_rssi(IN RTMP_ADAPTER * pAd)
{
	UINT32 rssi = 0;
	UINT16 i = 0;
	...
	case STA_TYPE:
		 /*STA*/ if (pAppAd->MBSSID[MAIN_MBSSID].StaCount < 2) {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("2[%s]:StaCount=%d,Avg Rssi=%d,pAd->OpMode=%d\n",
		  __FUNCTION__, pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount, rssi,
		  pAd->OpMode));
...
========================================================================
*/
VOID MT7601_dynamic_vga_tuning(IN RTMP_ADAPTER * pAd)
{
	//RTMP_CHIP_CAP *pChipCap         = &pAd->chipCap;
  ...
  /*--->STA or P2P_GC or P2P GO(only one client)*/
	if (pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount < 2) {
		...
		if (pre_R75_Value != init_R75_Value) {
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R75,
						     init_R75_Value);
		}
		...
		if (pre_R76_Value != init_R76_Value) {
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R76,
						     init_R76_Value);
		}
	}
	/*P2P GO, or AP, StaCount >= 2 */
		...
		if ((pAd->CommonCfg.BBPCurrentBW == BW_20
		     && rssi > RSSI_FOR_20MHZ_SENSIBILITY)
		    || (pAd->CommonCfg.BBPCurrentBW == BW_40
			&& rssi > RSSI_FOR_40MHZ_SENSIBILITY)) {
			MT7601_Periodic_Monitor_FalseCCA(pAd, rssi,
							 &init_main_vga,
							 &init_aux_vga);
		} else {
	...
	if (pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount >= 2) {	/*P2P GO, or AP, StaCount >= 2 */
		...
		if (pre_reg_val != pAd->CommonCfg.InitR68) {
			...
			if (pAd->CommonCfg.InitR68)
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R68,
							     pAd->CommonCfg.
							     InitR68);
		}
	}
	...
	if (temp_count % 2 == 0) {
		temp_count = 1;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[%s]========> rssi(%d), BW(%s), OneSecFalseCCACnt(%d)\n",
			  __FUNCTION__, rssi,
			  pAd->CommonCfg.BBPCurrentBW == 0 ? "20" : "40",
...
/*0. Dynamic vga is disable*/
   ...
   /*--->STA or P2P_GC or P2P GO(only one client)*/
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[%s]main_vga(0x%x),aux_vga(0x%x),rssi(%d),StaCount(%d)\n",
		  __FUNCTION__, init_main_vga, init_aux_vga, rssi,
		  pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount));
				      ...
				      UINT32 * main_vga, UINT32 * aux_vga)
{
	...
	if (FCCACnt > FCCA_THR1)	//Defaut: 1800
	{
		...
		*main_vga =
		    (*main_vga >
		     (Init_main_vga + 0x10)) ? (Init_main_vga +
						0x10) : *main_vga;

		*aux_vga += 0x04;	//-2dBm
		*aux_vga =
		    (*aux_vga >
		     (Init_aux_vga + 0x10)) ? (Init_aux_vga + 0x10) : *aux_vga;
	} else if (FCCACnt < FCCA_THR2)	//Default: 200
	{
		*main_vga -= 0x04;	//+2dBm
		*main_vga =
		    (*main_vga < Init_main_vga) ? Init_main_vga : *main_vga;
...
VOID MT7601_dynamic_vga_reset_default(IN RTMP_ADAPTER * pAd)
{
	UCHAR regBBP = 0x0;
	...
	if (regBBP != pAd->CommonCfg.InitR66) {
		if (pAd->CommonCfg.InitR66) {
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66,
						     pAd->CommonCfg.InitR66);
			rtmp_bbp_set_agc(pAd, pAd->CommonCfg.InitR66,
					 RX_CHAIN_ALL);
		}
	}
	...
	if (pAd->CommonCfg.InitR195) {
		...
		if (regBBP != pAd->CommonCfg.InitR195) {
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195,
						     pAd->CommonCfg.InitR195);
		}
	}
