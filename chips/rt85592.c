...
static UCHAR RT85592_FreqPlan_Sz =
    (sizeof(RT85592_FreqPlan_Xtal40M) / sizeof(RT8592_FREQ_ITEM));
...
//
VOID RT85592ReadTxPwrPerRate(IN PRTMP_ADAPTER pAd)
{
...
static UCHAR RT85592_NUM_MAC_REG_PARMS =
    (sizeof(RT85592_MACRegTable) / sizeof(RTMP_REG_PAIR));
...
static UCHAR RT85592_BBP_GBand_Tb_Size =
    (sizeof(RT85592_BBPRegTb_GBand) / sizeof(RTMP_REG_PAIR));
...
static UCHAR RT85592_BBP_ABand_Tb_Size =
    (sizeof(RT85592_BBPRegTb_ABand) / sizeof(RTMP_REG_PAIR));
...
INT RT85592_ReadChannelPwr(RTMP_ADAPTER * pAd)
{
	...
	for (ss_idx = 0; ss_idx < 2; ss_idx++) {
		for (i = 0; i < 7; i++) {
			idx = i * 2;
			RT28xx_EEPROM_READ16(pAd, ss_offset_g[ss_idx] + idx,
					     Power.word);

			tx_pwr1 = tx_pwr2 = DEFAULT_RF_TX_POWER;
			if ((Power.field.Byte0 <= 0x27)
			    && (Power.field.Byte0 >= 0))
				tx_pwr1 = Power.field.Byte0;
			if ((Power.field.Byte1 <= 0x27)
			    || (Power.field.Byte1 >= 0))
...

	{
		...
		for (i = 0; i < 6; i++) {
			...
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TX_PWR_OFFSET + idx,
					     Power.word);
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TX2_PWR_OFFSET + idx,
					     Power2.word);
			...
			if ((Power2.field.Byte0 < 0x20)
			    && (Power2.field.Byte0 >= 0))
				pAd->TxPower[idx + choffset + 0].Power2 =
				    Power2.field.Byte0;

			if ((Power2.field.Byte1 < 0x20)
			    && (Power2.field.Byte1 >= 0))
				pAd->TxPower[idx + choffset + 1].Power2 =
				    Power2.field.Byte1;
		}
...
/*
	...
	==========================================================================
 */
static VOID RT85592ReverseRFSleepModeSetup(IN PRTMP_ADAPTER pAd,
...
// TODO: shiang-8592, clear RF_CSR_REG busy bit first and try to work-around it in RT6856 platform
	{
		...
		if (reg_val.word != 0) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s(): Before Init RF, RF_CSR_CFG.word = 0x%x, RF_CSR_CFG.Kick=%d\n",
				  __FUNCTION__, reg_val.word,
				  reg_val.field.RF_CSR_KICK));
			RTMP_IO_WRITE32(pAd, RF_CSR_CFG, 0);
			RTMP_IO_READ32(pAd, RF_CSR_CFG, &reg_val.word);
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s(): After write RF_CSR_CFG, read value back=0x%x\n",
				  __FUNCTION__, reg_val.word));
		}
	}
// TODO: ---End
...
static VOID RT85592_AsicAntennaDefaultReset(IN struct _RTMP_ADAPTER *pAd,
...
INT set_freq_plan(RTMP_ADAPTER * pAd, UCHAR Channel)
{
	...
	for (idx = 0; idx < RT85592_FreqPlan_Sz; idx++) {
		if (Channel == RT85592_FreqPlan_Xtal40M[idx].Channel) {
			...
			RFValue &= (~0x0c);
			RFValue |=
			    (RT85592_FreqPlan_Xtal40M[idx].mod & 0x3) << 2;
			RT30xxWriteRFRegister(pAd, RF_R11, RFValue);

			RT30xxReadRFRegister(pAd, RF_R09, &RFValue);
			RFValue &= (~0xe0);
			RFValue |=
			    (RT85592_FreqPlan_Xtal40M[idx].mod >> 2) << 5;
			RT30xxWriteRFRegister(pAd, RF_R09, RFValue);
			break;
		}
	}
...
VOID RT85592_ChipSwitchChannel(struct _RTMP_ADAPTER *pAd,
			       UCHAR Channel, BOOLEAN bScan)
{
	...
	if ((pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration == TRUE)
		...
		RFValue =
		    ((RFValue & ~0x7F) |
		     (pAd->FreqCalibrationCtrl.AdaptiveFreqOffset & 0x7F));
	else
...
// TODO: End---
	...
	if (Channel <= 14) {
		...
		if (WINDOWS) {
			TX_ATTENUATION_CTRL_STRUC TxAttenuationCtrl = {.word =
				    0 };
...
#ifdef CONFIG_STA_SUPPORT
static VOID RT85592_NetDevNickNameInit(IN struct _RTMP_ADAPTER *pAd)
{
...
VOID RT85592_Init(RTMP_ADAPTER * pAd)
{
	...
	pChipCap->bbpRegTbSize =
	    (sizeof(RT85592_BBPRegTb) / sizeof(RTMP_REG_PAIR));
