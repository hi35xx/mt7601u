...
REG_PAIR BBPRegTable[] = {
	...
	,			/* fix rssi issue */
	{BBP_R66, 0x38}
	,			/* Also set this default value to pAd->BbpTuning.R66CurrentValue at initial */
	{BBP_R68, 0x0B}
	,			/* improve Rx sensitivity. */
	{BBP_R69, 0x12}
	,
	{BBP_R70, 0xa}
	,			/* BBP_R70 will change to 0x8 in ApStartUp and LinkUp for rt2860C, otherwise value is 0xa */
	{BBP_R73, 0x10}
	,
	{BBP_R81, 0x37}
	,
	...
	,
	{BBP_R84, 0x99}
	,			/* 0x19 is for rt2860E and after. This is for extension channel overlapping IOT. 0x99 is for rt2860D and before */
	{BBP_R86, 0x00}
	,			/* middle range issue, Rory @2008-01-28         */
	{BBP_R91, 0x04}
	,			/* middle range issue, Rory @2008-01-28 */
	{BBP_R92, 0x00}
	,			/* middle range issue, Rory @2008-01-28 */
	{BBP_R103, 0x00}
	,			/* near range high-power issue, requested from Gary @2008-0528 */
	{BBP_R105, 0x05}
	,			/* 0x05 is for rt2860E to turn on FEQ control. It is safe for rt2860D and before, because Bit 7:2 are reserved in rt2860D and before. */
#ifdef DOT11_N_SUPPORT
	{BBP_R106, 0x35}
	,			/* Optimizing the Short GI sampling request from Gray @2009-0409 */
...
NDIS_STATUS AsicBBPWriteWithRxChain(IN RTMP_ADAPTER * pAd,
				    IN UCHAR bbpId,
		...
		if (rx_ch_idx & 0x01) {
			...
			if ((IS_USB_INF(pAd)) &&
			    (RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R27, val) ==
			     STATUS_SUCCESS)) {
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, bbpId,
							     bbpVal);
			}
#endif				/* RTMP_MAC_USB */
...
NDIS_STATUS AsicBBPReadWithRxChain(IN RTMP_ADAPTER * pAd,
				   IN UCHAR bbpId,
				   IN CHAR * pBbpVal, IN RX_CHAIN_IDX rx_ch_idx)
{
	UCHAR idx, val;
...
INT rtmp_cfo_track(RTMP_ADAPTER * pAd, MAC_TABLE_ENTRY * pEntry, INT lastClient)
{
	...
	if (IS_RT3883(pAd)) {
		...
		} else if ((lastClient < MAX_LEN_OF_MAC_TABLE)
			   && (lastClient >= 1) && pAd->CommonCfg.CFOTrack < 8
...
#ifdef MT7601
NDIS_STATUS MT7601_BBP_write(IN PRTMP_ADAPTER pAd,
...
NDIS_STATUS MT7601_BBP_read(IN RTMP_ADAPTER * pAd,
			    IN UCHAR regID, IN UCHAR * pValue)
{
	BBP_CSR_CFG_STRUC BbpCsr = { {0}
	};
	UINT i = 0, k = 0;
