...
VOID RTMPSetHT(IN RTMP_ADAPTER * pAd, IN OID_SET_HT_PHYMODE * pHTPhyMode)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("RTMPSetHT : AMsduSize = %d, MimoPs = %d, MpduDensity = %d, MaxRAmpduFactor = %d\n",
		  rt_ht_cap->AmsduSize, rt_ht_cap->MimoPs,
		  rt_ht_cap->MpduDensity, rt_ht_cap->MaxRAmpduFactor));
...
/*
	...
	========================================================================
*/
VOID RTMPSetIndividualHT(IN RTMP_ADAPTER * pAd, IN UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_P2P_GO) {
			...
			pDesired_ht_phy =
			    &pAd->ApCfg.MBSSID[idx].DesiredHtPhyInfo;
			DesiredMcs =
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (idx < MAX_APCLI_NUM) {
				...
				encrypt_mode =
				    pAd->ApCfg.ApCliTab[idx].WepStatus;
...
VOID RTMPDisableDesiredHtInfo(IN PRTMP_ADAPTER pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			RTMPZeroMemory(&pAd->ApCfg.MBSSID[apidx].
				       DesiredHtPhyInfo, sizeof(RT_PHY_INFO));
		}
#ifdef APCLI_SUPPORT
		for (apidx = 0; apidx < MAX_APCLI_NUM; apidx++) {
...
#endif				/* P2P_SUPPORT */
		RTMPZeroMemory(&pAd->StaCfg.DesiredHtPhyInfo,
			       sizeof(RT_PHY_INFO));
	}
#endif				/* CONFIG_STA_SUPPORT */
