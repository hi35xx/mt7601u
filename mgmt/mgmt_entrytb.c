...
/*	ULONG	addr;*/
	...
	for (i = FirstWcid; i < MAX_LEN_OF_MAC_TABLE; i++) {	/* skip entry#0 so that "entry index == AID" for fast lookup */
		...
		if (IS_ENTRY_NONE(&pAd->MacTab.Content[i])) {
			...
			RTMPInitTimer(pAd, &pEntry->EnqueueStartForPSKTimer,
				      GET_TIMER_FUNCTION
				      (EnqueueStartForPSKExec), pEntry, FALSE);
...
#ifdef APCLI_SUPPORT
				if (IS_ENTRY_APCLI(pEntry)) {
					COPY_MAC_ADDR(pEntry->HdrAddr2,
						      pAd->ApCfg.
						      ApCliTab[pEntry->apidx].
						      CurrentAddress);
	...
	if (pEntry) {
		...
		if (pAd->StaCfg.BssType == BSS_ADHOC) {
			pWscPeerEntry =
			    WscFindPeerEntry(&pAd->StaCfg.WscControl.
					     WscPeerList, pEntry->Addr);
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry)) {
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr)) {
...
#ifdef ADHOC_WPA2PSK_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				RTMPReleaseTimer(&pEntry->WPA_Authenticator.
			...
			if (IS_ENTRY_CLIENT(pEntry)
				...
				RTMPReleaseTimer(&pEntry->RetryTimer,
						 &Cancelled);
				...
				if ((PmkCacheIdx =
				     RTMPSearchPMKIDCache(pAd, pEntry->apidx,
							  pEntry->Addr)) !=
				    -1) {
					RTMPDeletePMKIDCache(pAd, pEntry->apidx,
							     PmkCacheIdx);
				}
#endif				/* DOT1X_SUPPORT */
...
#ifdef RT_CFG80211_SUPPORT
				{
					...
					if ((pAd->Cfg80211VifDevSet.vifDevList.
					     size > 0)
					    &&
					    ((pNetDev =
					      RTMP_CFG80211_FindVifEntry_ByType
					      (pAd,
					       RT_CMD_80211_IFTYPE_P2P_GO)) !=
					     NULL))
						CFG80211OS_DelSta(pNetDev,
								  pEntry->Addr);
					else
						CFG80211OS_DelSta(pAd->net_dev,
								  pEntry->Addr);
				}
#endif
			}
			...
			if (IS_ENTRY_CLIENT(pEntry)) {
				PWSC_CTRL pWscControl =
				    ...
				    WscControl;
				if (MAC_ADDR_EQUAL
				    (pEntry->Addr, pWscControl->EntryAddr)) {
					/*
...
//                      NdisZeroMemory(pEntry, sizeof(MAC_TABLE_ENTRY));
			...
			if (P2P_GO_ON(pAd)) {
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("MacTableDeleteEntry1 - Total= %d. p2pEntry = %d.\n",
...
VOID MacTableReset(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i])) {
			...
			RTMPReleaseTimer(&pAd->MacTab.Content[i].
					 EnqueueStartForPSKTimer, &Cancelled);
#ifdef CONFIG_STA_SUPPORT
#ifdef ADHOC_WPA2PSK_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				RTMPReleaseTimer(&pAd->MacTab.Content[i].
...
#endif				/* ADHOC_WPA2PSK_SUPPORT */
#endif				/* CONFIG_STA_SUPPORT */
			pAd->MacTab.Content[i].EnqueueEapolStartTimerRunning =
			    EAPOL_START_DISABLE;
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		for (apidx = MAIN_MBSSID; apidx < pAd->ApCfg.BssidNum; apidx++) {
			...
			RTMPCancelTimer(&pAd->ApCfg.MBSSID[apidx].WscControl.
					EapolTimer, &Cancelled);
			pAd->ApCfg.MBSSID[apidx].WscControl.EapolTimerRunning =
			    FALSE;
			NdisZeroMemory(pAd->ApCfg.MBSSID[apidx].WscControl.
