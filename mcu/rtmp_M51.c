...
#if 0
NDIS_STATUS isMCUNeedToLoadFIrmware(IN PRTMP_ADAPTER pAd)
{
...
NDIS_STATUS isMCUnotReady(IN PRTMP_ADAPTER pAd)
{
	NDIS_STATUS Status = NDIS_STATUS_SUCCESS;
	ULONG Index;
	UINT32 MacReg;
	...
	if (IS_RT65XX(pAd)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s(): Ignore for MCU status check for 6590 now!\n",
			  __FUNCTION__));
		return Status;
	}
#endif				/* RT65xx */
	...
	if (IS_MT7601(pAd)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s(): Ignore for MCU status check for MT7601 now!\n",
			  __FUNCTION__));
		return Status;
	}
#endif				/* MT7601 */
...
/*	pFirmwareImage = kmalloc(MAX_FIRMWARE_IMAGE_SIZE, MEM_ALLOC_FLAG);*/
	...
	if (flg_default_firm_use == FALSE) {
		do {
			...
			FileLength =
			    RtmpOSFileRead(srcf, pFirmwareImage,
					   MAX_FIRMWARE_IMAGE_SIZE);
			if (FileLength != MAX_FIRMWARE_IMAGE_SIZE) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: error file length (=%d) in RT2860AP.BIN\n",
				...
				for (i = 0; i < (MAX_FIRMWARE_IMAGE_SIZE - 2);
				     i++, ptr++)
				...
				if ((pFirmwareImage[MAX_FIRMWARE_IMAGE_SIZE - 2]
					...
					DBGPRINT(RT_DEBUG_ERROR,
						 ("%s: CRC = 0x%02x 0x%02x "
						  ...
						  __FUNCTION__,
						  pFirmwareImage
						  [MAX_FIRMWARE_IMAGE_SIZE - 2],
						  pFirmwareImage
						  [MAX_FIRMWARE_IMAGE_SIZE - 1],
						  (UCHAR) (crc >> 8),
						  (UCHAR) (crc)));
					NICLF_DEFAULT_USE();
					break;
	...
	if (isMCUNeedToLoadFIrmware(pAd) == NDIS_STATUS_SUCCESS) {
		...
		if (pAd->WOW_Cfg.bWOWFirmware == TRUE) {
			UCHAR ver =
			    FirmwareImage[FIRMWAREIMAGEV3_LENGTH + 0xFFD];
			UINT16 sum;

			NdisCopyMemory(&sum,
				       &FirmwareImage[FIRMWAREIMAGEV3_LENGTH +
						      0xFFE], 2);
		...
		{
			UCHAR ver =
			    FirmwareImage[FIRMWAREIMAGEV2_LENGTH + 0xFFD];
			UINT16 sum;

			NdisCopyMemory(&sum,
				       &FirmwareImage[FIRMWAREIMAGEV2_LENGTH +
						      0xFFE], 2);
			printk("%s: ver %x/%x, sum %x/%x, mac %x\n",
			       __FUNCTION__, FVer, ver, FCS, sum, MacReg1);
			if (FVer != ver || FCS != sum)
		...
		if (Equal == FALSE) {
			DBGPRINT(RT_DEBUG_OFF,
				 ("%s: WOW stops to go into 4K ram codes ...\n",
				  __FUNCTION__));
	...
	{
#ifdef RTMP_MAC_USB
		if ((Version != 0x2860) && (Version != 0x2872) && (Version != 0x3070)) {	/* Use Firmware V2. */
			/*printk("KH:Use New Version,part2\n"); */
			pFirmwareImage =
			    (PUCHAR) & FirmwareImage[FIRMWAREIMAGEV1_LENGTH];
		...
		if ((Version != 0x2860) && (Version != 0x2872)
		    && (Version != 0x3070)) {
	...
	if (isMCUNeedToLoadFIrmware(pAd) || Equal == FALSE) {
		...
		RTUSB_VendorRequest(pAd, USBD_TRANSFER_DIRECTION_OUT,
				    DEVICE_VENDOR_REQUEST_OUT, 0x01, 0x8, 0,
				    NULL, 0);
	}
#else
			     ...
			     IN UCHAR Arg1, IN BOOLEAN FlgIsNeedLocked)
{
	...
	if (IS_RT3290(pAd)) {
		if (Command == WAKE_MCU_CMD) {
			...
			if (i >= 1000) {
				DBGPRINT(RT_DEBUG_ERROR,
					 (" %s: [boundary]DMA Tx keeps busy.  %d\n",
					  __FUNCTION__, i));
			}
		}
	}
