		   ...
		   IN BOOLEAN CfAck, IN HTTRANSMIT_SETTING * pTransmit)
{
	...
	if (pMac && pAd->chipCap.FlgHwTxBfCap) {
		if (pMac->TxSndgType == SNDG_TYPE_NDP
		    || pMac->TxSndgType == SNDG_TYPE_SOUNDING || pTxWI->eTxBF)
			pTxWI->TxWISTBC = 0;
	}
#endif				/* TXBF_SUPPORT */
...
VOID RTMPWriteTxWI_Data(RTMP_ADAPTER * pAd, TXWI_STRUC * pTxWI, TX_BLK * pTxBlk)
{
	...
	if (pTxBlk->NeedTrans)
		...
		pTxWI->TxWIMPDUByteCnt =
		    pTxBlk->MpduHeaderLen + pTxBlk->SrcBufLen;
	...
	if ((pAd->force_amsdu == TRUE)
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("%s():f_amsdu=%d, pTxBlk->TxFrameType=%d, pMacEntry->TXBAbitmap=0x%x, pTxBlk->UserPriority=%d, cond_match=%d!\n",
			  __FUNCTION__, pAd->force_amsdu, pTxBlk->TxFrameType,
			  pMacEntry->TXBAbitmap, pTxBlk->UserPriority,
			  ((pMacEntry->
			    TXBAbitmap & (1 << pTxBlk->UserPriority)) !=
			   0) ? TRUE : FALSE));
		...
		if ((pAd->force_amsdu == TRUE)
			...
			&&
			((pMacEntry->
			  TXBAbitmap & (1 << pTxBlk->UserPriority)) != 0)))
			amsdu_in_ampdu = TRUE;
	}
#endif				/* WFA_VHT_PF */
...
#endif				/* WFA_VHT_PF */
		pTxWI->TxWIAMPDU =
		    ((pTxBlk->TxFrameType == TX_AMPDU_FRAME) ? TRUE : FALSE);
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ETxBF in RTMPWriteTxWI_Data(): sending normal sounding, eTxBF=%d\n",
			  pTxWI->eTxBF));
		pTxWI->iTxBF = 0;
...
#ifdef MFB_SUPPORT
		if (pMacEntry && (pMacEntry->mrqCnt > 0)
		    && (pMacEntry->toTxMrq == TRUE))
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP
	    || pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING || pTxWI->eTxBF)
		pTxWI->TxWISTBC = 0;
#endif				/* TXBF_SUPPORT */
	...
	if (pTxBlk->QueIdx == 0) {
		...
		pAd->DiagStruct.TxMcsCnt[pAd->DiagStruct.ArrayCurIdx][pTxWI->
								      MCS]++;
	}
#endif				/* DBG_DIAGNOSE */
			 ...
			 INOUT TXWI_STRUC * pTxWI, IN TX_BLK * pTxBlk)
{
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING) {
		...
		pTxWI->iTxBF = 0;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ETxBF in RTMPWriteTxWI_Cache(): sending normal sounding, eTxBF=%d\n",
			  pTxWI->eTxBF));
	} else if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP) {
		if (pTxBlk->TxNDPSndgMcs >= 16)
			pTxWI->NDPSndRate = 2;
