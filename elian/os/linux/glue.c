...
{
	printk
	    ("[MSC] done: AM=%d, ssid=%s, pwd=%s, user=%s, cust_data_len=%d, cust_data=%s,\n",
...
int msc_proc_create(void *priv)
{
	...
	if (entry == NULL) {
		...
		entry =
		    proc_create_data(MSC_PROC_NAME, S_IRUSR | S_IWUSR, NULL,
				     &msc_proc_fops, priv);
#else
...
static int msc_proc_show(struct seq_file *seq, void *offset)
{
	...
	for (i = 0; i < SMTCNResult.msc_cust_tlv_len; i++)
		sprintf(SMTCNResult.msc_cust_tlv_str + i * 3, "%02X-",
			SMTCNResult.msc_cust_tlv[i]);
	SMTCNResult.msc_cust_tlv_str[i * 3 - 1] = 0;
	printf
	    ("MSC_CORE: AM=%d, ssid=%s, pwd=%s, user=%s, cust_data_len=%d, cust_data=%s\n",
	     test_result->auth_mode, test_result->ssid, test_result->pwd,
	     test_result->user, test_result->cust_data_len,
	     test_result->cust_data);
	seq_printf(seq,
		   "SSID\t: %s\n" "PASSWORD\t: %s\n" "AUTHMODE\t: %d\n"
		   "TLV_HEX\t: %s\n", SMTCNResult.msc_ssid, SMTCNResult.msc_pwd,
		   SMTCNResult.msc_authmode, SMTCNResult.msc_cust_tlv_str);
#endif
	seq_printf(seq,
