...
int sc_plt_enter_monitor_mode(struct monitor_info *info, void *priv)
{
	...
	MSC_DBG
	    ("[MSC] enter monitor mode: filter:0x%x, chan_id:%d, width:%d, chan_flags:0x%x, monitor_priv=%p, priv=%p\n",
...
char *sc_plt_get_la(void *priv)
{
	...
	MSC_DBG("[MSC] get local address %p : %x:%x:%x:%x:%x:%x\n",
		priv, pAd->CurrentAddress[0] & 0xff,
		pAd->CurrentAddress[1] & 0xff, pAd->CurrentAddress[2] & 0xff,
		pAd->CurrentAddress[3] & 0xff, pAd->CurrentAddress[4] & 0xff,
		pAd->CurrentAddress[5] & 0xff);
	return (char *)pAd->CurrentAddress;
}

void sc_plt_aes128_decrypt(unsigned char *cipher_blk,
			   unsigned int cipher_blk_size, unsigned char *key,
			   unsigned int key_len, unsigned char *plain_blk,
			   unsigned int *plain_blk_size)
{
	return RT_AES_Decrypt(cipher_blk, cipher_blk_size, key, key_len,
			      plain_blk, plain_blk_size);
}
