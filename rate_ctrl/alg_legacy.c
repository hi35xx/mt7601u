...
VOID APMlmeDynamicTxRateSwitching(RTMP_ADAPTER * pAd)
{
	...
	CHAR Rssi, TmpIdx = 0;
	ULONG TxTotalCnt, TxErrorRatio =
	    0, TxSuccess, TxRetransmit, TxFailCount;
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pAd->MacTab.Size == 1) {
			...
			if (pEntry->Aid >= 1 && pEntry->Aid <= 8) {
				...
				HwTxCnt =
				    pEntry->fifoTxSucCnt + pEntry->fifoTxRtyCnt;
				if (HwTxCnt)
					HwErrRatio =
					    (pEntry->fifoTxRtyCnt * 100) /
					    HwTxCnt;
				else
					HwErrRatio = 0;
				...
				DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
					 ("%s():Aid:%d, MCS:%d, CuTxRaIdx=%d,TxErrRatio(Hw:%ld-%ld%%, Sw:%ld-%ld%%)\n",
					  ...
					  pEntry->CurrTxRateIndex, HwTxCnt,
					  HwErrRatio, TxTotalCnt,
					  TxErrorRatio));
		...
		pEntry->LastTxPER =
		    (TxTotalCnt == 0 ? 0 : (UCHAR) TxErrorRatio);
		...
		if ((pCurrTxRate->Mode <= MODE_CCK)
			...
			while (TmpIdx < TableSize) {
				pTmpTxRate =
				    PTX_RA_LEGACY_ENTRY(pTable, TmpIdx);
				if (pEntry->
				    SupportCCKMCS[pTmpTxRate->CurrMCS] ==
				    TRUE) {
					UpRateIdx = TmpIdx;
					break;
				}
			...
			while (TmpIdx < TableSize) {
				pTmpTxRate =
				    PTX_RA_LEGACY_ENTRY(pTable, TmpIdx);
				if (pEntry->
				    SupportOFDMMCS[pTmpTxRate->CurrMCS] ==
				    TRUE) {
					UpRateIdx = TmpIdx;
					break;
				}
			...
			if ((CurrRateIdx > 0)
				...
				while (TmpIdx < TableSize) {
					pTmpTxRate =
					    PTX_RA_LEGACY_ENTRY(pTable, TmpIdx);
					if (pEntry->
					    SupportHTMCS[pTmpTxRate->CurrMCS] ==
					    TRUE) {
						UpRateIdx = TmpIdx;
						break;
					}
		...
		if (pAd->CommonCfg.DebugFlags & DBF_SHOW_RA_LOG)
			MlmeRALog(pAd, pEntry, RAL_OLD_DRS, TxErrorRatio,
				  TxTotalCnt);
		...
		MlmeOldRateAdapt(pAd, pEntry, CurrRateIdx, UpRateIdx,
				 DownRateIdx, TrainUp, TrainDown, TxErrorRatio);
				 ...
				 IN PVOID SystemSpecific3)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		/* Debug option: Concise RA log */
		if (pAd->CommonCfg.DebugFlags & DBF_SHOW_RA_LOG)
			MlmeRALog(pAd, pEntry, RAL_QUICK_DRS, TxErrorRatio,
				  TxTotalCnt);
#endif				/* DBG_CTRL_SUPPORT */
		...
		do {
			 ...
			 */
			if ((pEntry->LastTimeTxRateChangeAction ==
			     RATE_NO_CHANGE)
#ifdef DBG_CTRL_SUPPORT
			    && (pAd->CommonCfg.
				DebugFlags & DBF_FORCE_QUICK_DRS) == 0
#endif				/* DBG_CTRL_SUPPORT */
			    )
...
// TODO: gaa - use different criterion for train up in Old RA?
			...
			} else if (pEntry->LastSecTxRateChangeAction ==
				   RATE_DOWN) {
...
VOID MlmeDynamicTxRateSwitching(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
...
#ifdef NEW_RATE_ADAPT_SUPPORT
		if (ADAPT_RATE_TABLE(pTable)) {
			MlmeDynamicTxRateSwitchingAdapt(pAd, i, TxSuccess,
							TxRetransmit,
							TxFailCount);
			continue;
		}
		...
		if ((pAd->MacTab.Size == 1) || IS_ENTRY_DLS(pEntry)) {
			...
			if (IS_RT3290(pAd) &&
			    ((AccuTxTotalCnt > 150)
			     || (pAd->AntennaDiversityState == 1))
			    && (pAd->CommonCfg.BBPCurrentBW == BW_40)) {
				WLAN_FUN_CTRL_STRUC WlanFunCtrl = {.word = 0 };
				RTMP_IO_READ32(pAd, WLAN_FUN_CTRL,
					       &WlanFunCtrl.word);

				if ((WlanFunCtrl.field.WLAN_EN == TRUE) &&
				    (WlanFunCtrl.field.PCIE_APP0_CLK_REQ ==
				     FALSE)) {
					WlanFunCtrl.field.PCIE_APP0_CLK_REQ =
					    TRUE;
				...
				MlmeAntSelection(pAd, AccuTxTotalCnt,
						 TxErrorRatio, TxSuccess,
			...
			DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
				 ("DRS:Aid=%d, TxSuccess=%ld, TxRetransmit=%ld, TxFailCount=%ld \n",
				  pEntry->Aid, TxSuccess, TxRetransmit,
				  TxFailCount));
		...
		if (AGS_IS_USING(pAd, pTable)) {
			...
			MlmeDynamicTxRateSwitchingAGS(pAd, pEntry, pTable,
						      TableSize,
						      &AGSStatisticsInfo,
						      InitTxRateIdx);
		...
		if (TxTotalCnt <= 15) {
			...
			/* Select the Tx rate based on the RSSI */
			TxRateIdx =
			    MlmeSelectTxRate(pAd, pEntry, mcs, Rssi,
					     RssiOffset);
				  ...
				  IN PVOID SystemSpecific3)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pAd->MacTab.Size == 1) {
			...
			if (pAd->chipCap.FlgHwFifoExtCap) {
				if (pEntry->Aid >= 1 && pEntry->Aid <= 8) {
					...
					regAddr =
					    WCID_TX_CNT_0 + (pEntry->Aid -
							     1) * 4;
					RTMP_IO_READ32(pAd, regAddr,
						       &wcidTxCnt.word);

					HwTxCnt =
					    wcidTxCnt.field.succCnt +
					    wcidTxCnt.field.reTryCnt;
					if (HwTxCnt)
						HwErrRatio =
						    (wcidTxCnt.field.reTryCnt *
						     100) / HwTxCnt;
		...
		if (AGS_IS_USING(pAd, pTable)) {
			...
			StaQuickResponeForRateUpExecAGS(pAd, pEntry, pTable,
							TableSize,
							&AGSStatisticsInfo,
							InitTxRateIdx);
		...
		if (bTrainUp) {
			...
			if ((CurrRateIdx != UpRateIdx)
				...
				pEntry->LastSecTxRateChangeAction = RATE_UP;
				  ...
				  || (TxErrorRatio > TrainUp))
#ifdef DBG_CTRL_SUPPORT
				 &&
				 ((pAd->CommonCfg.
				   DebugFlags & DBF_NO_BF_AWARE_RA) == 0)
#endif				/* DBG_CTRL_SUPPORT */
			    ) {
				...
				} else if (pEntry->eTxBfEnCond > 0
					   || pEntry->iTxBfEn) {
					RTMP_RA_LEGACY_TB *pUpRate =
					    PTX_RA_LEGACY_ENTRY(pTable,
								UpRateIdx);
					RTMP_RA_LEGACY_TB *pCurrTxRate =
					    PTX_RA_LEGACY_ENTRY(pTable,
								CurrRateIdx);
