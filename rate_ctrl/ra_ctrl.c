...
*/
VOID MlmeGetSupportedMcs(IN PRTMP_ADAPTER pAd,
...
VOID MlmeClearTxQuality(IN MAC_TABLE_ENTRY * pEntry)
{
#ifdef TXBF_SUPPORT
	if (pEntry->phyETxBf || pEntry->phyITxBf)
		NdisZeroMemory(pEntry->BfTxQuality,
			       sizeof(pEntry->BfTxQuality));
	else
#endif				/*  TXBF_SUPPORT */
...
VOID MlmeClearAllTxQuality(IN MAC_TABLE_ENTRY * pEntry)
{
#ifdef TXBF_SUPPORT
...
VOID MlmeDecTxQuality(IN MAC_TABLE_ENTRY * pEntry, IN UCHAR rateIndex)
{
#ifdef TXBF_SUPPORT
	if (pEntry->phyETxBf || pEntry->phyITxBf) {
...
VOID MlmeSetTxQuality(IN MAC_TABLE_ENTRY * pEntry,
...
USHORT MlmeGetTxQuality(IN MAC_TABLE_ENTRY * pEntry, IN UCHAR rateIndex)
{
#ifdef TXBF_SUPPORT
	if (pEntry->phyETxBf || pEntry->phyITxBf)
		     ...
		     IN RTMP_RA_LEGACY_TB * pTxRate)
{
	...
	if (pEntry->pTable == RateTableVht2S) {
		...
		pEntry->HTPhyMode.field.MCS =
		    pAdaptTbEntry->
		    CurrMCS | ((pAdaptTbEntry->dataRate - 1) << 4);
	}
#endif				/* NEW_RATE_ADAPT_SUPPORT */
	...
	if (pEntry->pTable == Ags2x2VhtRateTable) {
		...
		pEntry->HTPhyMode.field.MCS =
		    pAgsTbEntry->CurrMCS | (pAgsTbEntry->Nss << 4);
	}
#endif				/* AGS_SUPPORT */
#endif				/* DOT11_VHT_AC */
...
#ifdef DBG_CTRL_SUPPORT
			   && (pAd->CommonCfg.
			       DebugFlags & DBF_DISABLE_20MHZ_MCS1) == 0
#endif				/* DBG_CTRL_SUPPORT */
		    ) {
...
#ifdef DBG_CTRL_SUPPORT
			   && (pAd->CommonCfg.
			       DebugFlags & DBF_ENABLE_20MHZ_MCS8)
#endif				/* DBG_CTRL_SUPPORT */
		    ) {
		   ...
		   IN MAC_TABLE_ENTRY * pEntry, IN RTMP_RA_LEGACY_TB * pTxRate)
{
	...
	if (!pAd || !pEntry || !pTxRate || !pEntry->pTable
	    || (pEntry->EntryType == 0)) {
		if (!pEntry) {
			...
			DBGPRINT(1,
				 ("pAd%sNULL, pEntry%sNULL, pTxRate%sNULL, pTable%sNULL, pEntry->EntryType=%d\n",
				  pAd ? "!=" : "==", pEntry ? "!=" : "==",
				  pTxRate ? "!=" : "==",
				  pEntry->pTable ? "!=" : "==",
				  pEntry->EntryType));
	...
	if (ADHOC_ON(pAd)) {
...
#ifdef DOT11_N_SUPPORT
		if ((pAd->CommonCfg.RegTransmitSetting.field.HTMODE ==
		     HTMODE_GF)
...
#ifdef DBG_CTRL_SUPPORT
			    && (pAd->CommonCfg.
				DebugFlags & DBF_DISABLE_20MHZ_MCS0) == 0
#endif				/* DBG_CTRL_SUPPORT */
			    ) {
				/*  Map HT Duplicate to 20MHz MCS0 */
				pEntry->HTPhyMode.field.BW = BW_20;
		...
		if (OperationMode != 0xffff)
			AsicUpdateProtect(pAd, OperationMode, ALLN_SETPROTECT,
					  TRUE,
			   ...
			   IN UCHAR * pTableSize, IN UCHAR * pInitTxRateIdx)
{
	do {
		...
		if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
			...
			INT mcs_idx, ss = 0;
			for (mcs_idx = 0; mcs_idx < MAX_LEN_OF_VHT_RATES;
			     mcs_idx++) {
		...
		if ((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) && (pEntry->HTCapability.MCSSet[0] != 0x00) && (pEntry->HTCapability.MCSSet[1] != 0x00) && (((pAd->Antenna.field.TxPath == 3) && (pEntry->HTCapability.MCSSet[2] == 0x00)) || (pAd->CommonCfg.TxStream == 2))) {	/* 11BGN 2S AP */
#ifdef AGS_SUPPORT
			if (SUPPORT_AGS(pAd)) {
				*ppTable = AGS2x2HTRateTable;
			} else
		...
		if ((pEntry->HTCapability.MCSSet[0] != 0x00) && ((pEntry->HTCapability.MCSSet[1] == 0x00) || (pAd->CommonCfg.TxStream == 1))) {	/* 11N 1S AP */
#ifdef AGS_SUPPORT
			if (SUPPORT_AGS(pAd))
				*ppTable = AGS1x1HTRateTable;
			else
		...
		if ((pEntry->HTCapability.MCSSet[0] != 0x00) && (pEntry->HTCapability.MCSSet[1] != 0x00) && (pAd->CommonCfg.TxStream == 2)) {	/* 11N 2S AP */
#ifdef AGS_SUPPORT
			if (SUPPORT_AGS(pAd))
				*ppTable = AGS2x2HTRateTable;
			else
		       ...
		       IN CHAR mcs[], IN CHAR Rssi, IN CHAR RssiOffset)
{
	...
	if (pTable == RateTableVht2S) {
		...
		if (IS_MT7601(pAd)) {
			//printk("RSSI=%d\n", Rssi);
			if (mcs[7] >= 0 && (Rssi > (-64 + RssiOffset))
			    && (pEntry->SupportHTMCS[MCS_7]))
				TxRateIdx = mcs[7];
			else if (mcs[6] >= 0 && (Rssi > (-66 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_6]))
				TxRateIdx = mcs[6];
			else if (mcs[5] >= 0 && (Rssi > (-70 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_5]))
				TxRateIdx = mcs[5];
			else if (mcs[4] >= 0 && (Rssi > (-73 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_4]))
				TxRateIdx = mcs[4];
			else if (mcs[3] >= 0 && (Rssi > (-77 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_3]))
				TxRateIdx = mcs[3];
			else if (mcs[2] >= 0 && (Rssi > (-79 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_2]))
				TxRateIdx = mcs[2];
			else if (mcs[1] >= 0 && (Rssi > (-81 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_1]))
				TxRateIdx = mcs[1];
			else if (mcs[1] >= 0 && (Rssi > (-83 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_0]))
				TxRateIdx = mcs[0];
#ifdef CONFIG_STA_SUPPORT
			else {
				if ((mcs[0] > 1)
				    && pEntry->SupportCCKMCS[MCS_1])
					TxRateIdx = 1;
				else
					TxRateIdx = mcs[0];
			}
	...
	{			/*  Legacy mode */
		if (mcs[7] >= 0 && (Rssi > -70)
			...
			if (mcs[3] >= 0 && (Rssi > -85)
			    && (pEntry->SupportCCKMCS[MCS_3]))
				TxRateIdx = mcs[3];
			else if (mcs[2] >= 0 && (Rssi > -87)
				 && (pEntry->SupportCCKMCS[MCS_2]))
				TxRateIdx = mcs[2];
			   ...
			   && (pEntry->SupportOFDMMCS[MCS_3]))
			TxRateIdx = mcs[3];
		else if (mcs[2] >= 0 && (Rssi > -87)
			 && (pEntry->SupportOFDMMCS[MCS_2]))
			TxRateIdx = mcs[2];
		else if (mcs[1] >= 0 && (Rssi > -90)
			 && (pEntry->SupportOFDMMCS[MCS_1]))
			TxRateIdx = mcs[1];
		else
			TxRateIdx = mcs[0];
	}
	       ...
	       IN ULONG TxErrorRatio, IN ULONG TxTotalCnt)
{
	...
	UINT TxCount =
	    ...
	    pEntry->TxBFCounters.TxFailCount + ETxCount + ITxCount;
	ULONG bfRatio = 0;
#endif				/*  TXBF_SUPPORT */
	...
	if (TxTotalCnt != 0 || raLogType == RAL_QUICK_DRS
		...
		if (pEntry->StreamModeMACReg != 0) {
			...
			RTMP_IO_READ32(pAd, pEntry->StreamModeMACReg + 4,
				       &streamWord);
		...
		if ((TxCount - pEntry->LastTxCount) > 0) {
			if (pEntry->HTPhyMode.field.eTxBF)
				bfRatio =
				    100 * (ETxCount -
					   pEntry->LastETxCount) / (TxCount -
								    pEntry->
								    LastTxCount);
			else if (pEntry->HTPhyMode.field.iTxBF)
				bfRatio =
				    100 * (ITxCount -
					   pEntry->LastITxCount) / (TxCount -
								    pEntry->
								    LastTxCount);
		}
		...
		if ((pEntry->HTPhyMode.field.eTxBF
			...
			DBGPRINT_RAW(RT_DEBUG_ERROR,
				     ("%s[%d]: M=%d %c%c%c%c%c PER=%ld%% TP=%ld BF=%ld%% ",
				      ...
				      MODE_CCK ? 'C' : (pEntry->HTPhyMode.field.
							ShortGI ? 'S' : 'L'),
				      pEntry->HTPhyMode.field.BW ? '4' : '2',
				      stbc ? 'S' : 's', csd ? 'C' : 'c',
				      pEntry->HTPhyMode.field.
				      eTxBF ? 'E' : (pEntry->HTPhyMode.field.
						     iTxBF ? 'I' : '-'),
...
VOID MlmeCheckRDG(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry)
{
	...
	if (((pTable == RateSwitchTable11BGN3S) ||
		...
		pTempTxRate =
		    (RTMP_RA_LEGACY_TB
		     *) (&pTable[(pEntry->CurrTxRateIndex + 1) * TableStep]);
...
VOID txbf_rate_adjust(RTMP_ADAPTER * pAd, MAC_TABLE_ENTRY * pEntry)
{
	...
	if (ADAPT_RATE_TABLE(pTable))
		pNextTxRate =
		    (RTMP_RA_LEGACY_TB *) PTX_RA_GRP_ENTRY(pTable,
							   pEntry->
							   CurrTxRateIndex);
	else
#endif				/*  NEW_RATE_ADAPT_SUPPORT */
		pNextTxRate =
		    PTX_RA_LEGACY_ENTRY(pTable, pEntry->CurrTxRateIndex);
...
VOID MlmeNewTxRate(RTMP_ADAPTER * pAd, MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pEntry == NULL || pTable == NULL || (ULONG) pTable < 0x1000) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("pEntry->pTable address no valid, so do nothing here\n"));
		return;
	}

	rtmp_get_rate_from_rate_tb(pEntry->pTable, pEntry->CurrTxRateIndex,
				   &tx_rate);
	/*  Get pointer to CurrTxRate entry */
#ifdef NEW_RATE_ADAPT_SUPPORT
	if (ADAPT_RATE_TABLE(pTable))
	...
	if (pNextTxRate == NULL) {
		DBGPRINT_RAW(RT_DEBUG_ERROR,
			     ("pNextTxRate is set as invalid, return\n"));
		return;
	}
	// end
	...
	if (pAd->CommonCfg.StreamMode != 0 && pEntry->StreamModeMACReg != 0) {
		...
		mcsDisable = (pEntry->HTPhyMode.field.MCS < 16) &&
		    (pAd->CommonCfg.
		     StreamModeMCS & (1 << pEntry->HTPhyMode.field.MCS)) == 0
	...
	if (HtCapabilityLen) {
		...
		if (OpMode == OPMODE_AP) {
			...
			if (IS_ENTRY_APCLI(pEntry))
				pDesired_ht_phy =
				    &pAd->ApCfg.ApCliTab[pEntry->apidx].
				    DesiredHtPhyInfo;
			else
#endif				/* APCLI_SUPPORT */
				pDesired_ht_phy =
...
INT Set_RateAlg_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT32 ra_alg;
