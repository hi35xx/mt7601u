	...
	while (1) {
		...
		if (PTX_RA_GRP_ENTRY(pEntry->pTable, UpRateIdx)->CurrMCS == 32) {
			...
			if (pEntry->MaxHTPhyMode.field.BW != BW_40 ||
				...
				UpRateIdx =
				    PTX_RA_GRP_ENTRY(pEntry->pTable,
						     UpRateIdx)->upMcs1;
				pEntry->mcsGroup = 1;
				break;
			}
		...
		if (PTX_RA_GRP_ENTRY(pEntry->pTable, UpRateIdx)->ShortGI &&
			...
			MlmeSetTxQuality(pEntry, UpRateIdx,
		...
		if ((MlmeGetTxQuality(pEntry, UpRateIdx) > 0)
		    && (pEntry->mcsGroup > 0))
			pEntry->mcsGroup--;
		else
			break;
...
*/
UCHAR MlmeSelectDownRate(IN RTMP_ADAPTER * pAd,
			 IN PMAC_TABLE_ENTRY pEntry, IN UCHAR CurrRateIdx)
{
	UCHAR DownRateIdx =
	    PTX_RA_GRP_ENTRY(pEntry->pTable, CurrRateIdx)->downMcs;
...
#ifdef DBG_CTRL_SUPPORT
			    && ((pAd->CommonCfg.DebugFlags & DBF_DISABLE_CCK) ==
				0)
#endif				/* DBG_CTRL_SUPPORT */
			    )
				break;
		...
		for (idx = 0; idx < RATE_TABLE_SIZE(pTable); idx++) {
			...
			if (pCurrTxRate->CurrMCS == MCS_0
				...
				mcs[6] = idx;
				 ...
				 && pCurrTxRate->dataRate == 1)
				mcs[7] = idx;
			else if (pCurrTxRate->CurrMCS == MCS_0
				 && pCurrTxRate->dataRate == 2)
				mcs[8] = idx;
			else if (pCurrTxRate->CurrMCS == MCS_1
				 && pCurrTxRate->dataRate == 2)
				mcs[9] = idx;
			    ...
			    IN CHAR mcs[], IN CHAR Rssi, IN CHAR RssiOffset)
{
	...
	} else if ((pTable == RateSwitchTable11BGN2S) || (pTable == RateSwitchTable11BGN2SForABand) || (pTable == RateSwitchTable11N2S) || (pTable == RateSwitchTable11N2SForABand)) {	/*  N mode with 2 stream */
...
*/
static ULONG MlmeRAEstimateThroughput(IN RTMP_ADAPTER * pAd,
				      IN MAC_TABLE_ENTRY * pEntry,
				      IN RTMP_RA_GRP_TB * pCurrTxRate,
				      IN ULONG TxErrorRatio)
{
	...
	if (pCurrTxRate->CurrMCS == MCS_32) {
		...
		if ((pAd->CommonCfg.DebugFlags & DBF_DISABLE_20MHZ_MCS0) == 0)
		   ...
		   || (pCurrTxRate->Mode == MODE_HTGREENFIELD)) {
		if (pEntry->MaxHTPhyMode.field.BW == BW_40
#ifdef DBG_CTRL_SUPPORT
			 ...
			 IN ULONG NewTxOkCount, IN ULONG TxErrorRatio)
{
	...
	if (100 * NewTxOkCount <
	    pAd->CommonCfg.TrainUpLowThrd * pEntry->LastTxOkCount)
		return TRUE;

	if (100 * NewTxOkCount >
	    pAd->CommonCfg.TrainUpHighThrd * pEntry->LastTxOkCount)
		return FALSE;

	newTP =
	    MlmeRAEstimateThroughput(pAd, pEntry, pCurrTxRate, TxErrorRatio);
	oldTP =
	    MlmeRAEstimateThroughput(pAd, pEntry,
				     PTX_RA_GRP_ENTRY(pEntry->pTable,
						      pEntry->lastRateIdx),
				     pEntry->LastTxPER);
		      ...
		      IN ULONG TrainDown, IN ULONG TxErrorRatio)
{
	...
	if (TxErrorRatio >= TrainDown) {
		...
		pLastNonBfRate =
		    PTX_RA_GRP_ENTRY(pTable, pEntry->lastNonBfRate);
		...
		if ((phyRateLimit20 != 0)
		    && (pUpRate->dataRate >= phyRateLimit20))
			return;

		/*  If UpRate is good then train up in current BF state */
		if ((CurrRateIdx != UpRateIdx)
		    && (MlmeGetTxQuality(pEntry, UpRateIdx) <= 0) && bTrainUp) {
...
#endif				/* DBG_CTRL_SUPPORT */
		{
			...
			if (pEntry->phyETxBf || pEntry->phyITxBf) {
				...
				if ((CurrRateIdx != UpRateIdx) &&
				    MlmeTxBfAllowed(pAd, pEntry,
						    (RTMP_RA_LEGACY_TB *)
						    pUpRate)) {
					if (pEntry->BfTxQuality[UpRateIdx])
						pEntry->
						    BfTxQuality[UpRateIdx]--;
				...
				if (pEntry->LastSecTxRateChangeAction ==
				    RATE_NO_CHANGE
				    && MlmeTxBfAllowed(pAd, pEntry,
						       (RTMP_RA_LEGACY_TB *)
						       pCurrTxRate)) {
...
VOID APQuickResponeForRateUpExecAdapt(	/* actually for both up and down */
					     IN PRTMP_ADAPTER pAd, IN ULONG idx)
{
	PUCHAR pTable;
	UCHAR CurrRateIdx;
	...
	if (pAd->MacTab.Size == 1) {
		...
		if (TxTotalCnt)
			TxErrorRatio =
			    ((TxRetransmit + TxFailCount) * 100) / TxTotalCnt;
		...
		if ((pEntry->Aid >= 1) && (pEntry->Aid <= 8)) {
			...
			if (HwTxCnt)
				HwErrRatio =
				    (pEntry->fifoTxRtyCnt * 100) / HwTxCnt;
			else
				HwErrRatio = 0;

			DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
				 ("%s():Aid:%d, MCS:%d, TxErrRation(Hw:0x%lx-0x%lx, Sw:0x%lx-%lx)\n",
				  __FUNCTION__, pEntry->Aid,
				  pEntry->HTPhyMode.field.MCS, HwTxCnt,
	...
	if ((pEntry->LastTimeTxRateChangeAction == RATE_NO_CHANGE)
		...
		ratio =
		    (RA_INTERVAL -
		     pAd->ra_fast_interval) / pAd->ra_fast_interval;
	...
	if (pEntry->LastSecTxRateChangeAction == RATE_UP) {
		...
		if (useOldRate) {
			...
			RTMP_RA_GRP_TB *pLastTxRate =
			    PTX_RA_GRP_ENTRY(pTable, pEntry->lastRateIdx);
	...
	rateChanged = (pEntry->CurrTxRateIndex != CurrRateIdx) ||
	    (pEntry->phyETxBf != CurrPhyETxBf)
	    || (pEntry->phyITxBf != CurrPhyITxBf);
...
VOID APMlmeDynamicTxRateSwitchingAdapt(RTMP_ADAPTER * pAd, ULONG i)
{
	...
	MlmeNewRateAdapt(pAd, pEntry, UpRateIdx, DownRateIdx, TrainUp,
...
#ifdef CONFIG_STA_SUPPORT
VOID StaQuickResponeForRateUpExecAdapt(IN PRTMP_ADAPTER pAd,
	...
	if ((pAd->MacTab.Size == 1) || (IS_ENTRY_DLS(pEntry))) {
		...
		if (pEntry->Aid >= 1 && pEntry->Aid <= 8) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s():TxErrRatio(Aid:%d, MCS:%d, Hw:0x%x-0x%x, Sw:0x%x-%x)\n",
				  __FUNCTION__, pEntry->Aid,
				  pEntry->HTPhyMode.field.MCS, HwTxCnt,
				  HwErrRatio, TxTotalCnt, TxErrorRatio));
...
INT Set_RateTable_Proc(RTMP_ADAPTER * pAd, PSTRING arg)
{
	...
	if (i == MAX_LEN_OF_MAC_TABLE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Set_RateTable_Proc: Empty MAC Table\n"));
		return FALSE;
	}
	...
	if (*arg != ':') {
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("Set_RateTable_Proc::%d:%d:%d\n", itemNo, rtIndex,
			  value));
	}

	DBGPRINT(RT_DEBUG_OFF, ("%d, 0x%02x, %d, %d, %d, %d, %d, %d, %d, %d\n",
				pRateEntry[0], pRateEntry[1], pRateEntry[2],
				pRateEntry[3], pRateEntry[4], pRateEntry[5],
				pRateEntry[6], pRateEntry[7], pRateEntry[8],
				pRateEntry[9]));
