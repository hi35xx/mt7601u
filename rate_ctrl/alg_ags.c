				   ...
				   IN UCHAR InitTxRateIdx)
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %d, TxSuccess = %d, TxRetransmit = %d, TxFailCount = %d, TxErrorRatio = %d\n",
		  __FUNCTION__, pAGSStatisticsInfo->AccuTxTotalCnt,
	...
	pCurrTxRate =
	    (RTMP_RA_AGS_TB
	     *) (&pTable[(CurrRateIdx + 1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
		...
		if (pTable == AGS3x3HTRateTable) {
			...
			DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
				     ("AGS> Current Group:%d, Select UpRateIdx=%d in group %d\n",
				      pEntry->AGSCtrl.MCSGroup, UpRateIdx,
				      next_grp));
	...
	if ((pAGSStatisticsInfo->RSSI > -65)
		...
		TrainDown =
		    (pCurrTxRate->TrainDown + (pCurrTxRate->TrainDown >> 1));
	} else
	...
	if (pAGSStatisticsInfo->AccuTxTotalCnt <= 15) {
		...
		if (pTable == AGS3x3HTRateTable) {
			if (MCS[23]
			    && (pAGSStatisticsInfo->RSSI > (-67 + RssiOffset)))
				TxRateIdx = MCS[23];
			else if (MCS[22]
			...
			if (pTable == Ags2x2VhtRateTable)
				RssiOffset =
				    AgsRssiOffsetTable[1][pAd->CommonCfg.
							  BBPCurrentBW];
#endif				/* DOT11_VHT_AC */
			...
			if (MCS
			    [15
			     && (pAGSStatisticsInfo->RSSI > (-69 + RssiOffset)))
			    TxRateIdx = MCS[15];
			    else
			    if (MCS
				[14
					...
					if (MCS
						...
						if (MCS
						     ...
						     *) (&pTable[(pEntry->
								  ...
								  1) *
								 SIZE_OF_AGS_RATE_TABLE_ENTRY]);
						    MlmeSetTxRate(pAd, pEntry,
								  pNextTxRate);
						    RTMPZeroMemory(pEntry->
								   TxQuality,
								   (sizeof
								    (USHORT) *
								    (MAX_TX_RATE_INDEX
								     + 1)));
						    RTMPZeroMemory(pEntry->PER,
								   (sizeof
								    (UCHAR) *
								    (MAX_TX_RATE_INDEX
								     + 1)));
						    ...
						    DBGPRINT_RAW(RT_DEBUG_INFO |
								 ...
								 ("%s: AGS: TrainUp:%d, TrainDown:%d\n",
								  ...
								  TrainDown));
						    do {
						    BOOLEAN bTrainUpDown = FALSE; DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s: AGS: TxQuality[CurrRateIdx(%d)] = %d, UpPenalty:%d\n", __FUNCTION__, CurrRateIdx, pEntry->TxQuality[CurrRateIdx], pEntry->TxRateUpPenalty)); if (pAGSStatisticsInfo->TxErrorRatio >= TrainDown) {	/* Poor quality */
						    ...
						    AGS_TX_QUALITY_WORST_BOUND;}
						    else
						    if (pAGSStatisticsInfo->TxErrorRatio <= TrainUp) {	/* Good quality */
						    bTrainUpDown = TRUE;
						    ...
						    if (pEntry->AGSCtrl.MCSGroup > 0) {	/* even if TxErrorRatio > TrainUp */
		...
		/* update error ratio for current MCS */
						    ...
						    if ((CurrRateIdx != UpRateIdx) && (pEntry->TxQuality[UpRateIdx] <= 0)) {	/* Upgrade Tx rate */
						    pEntry->CurrTxRateIndex =
						    UpRateIdx;
						    pEntry->
						    LastSecTxRateChangeAction =
							...
							&& (pEntry->
							    LastSecTxRateChangeAction
							    == RATE_DOWN)) {
						    DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s: AGS: --TX rate from %d to %d\n", __FUNCTION__, CurrRateIdx, pEntry->CurrTxRateIndex)); pEntry->LastSecTxRateChangeAction = RATE_DOWN; pEntry->TxRateUpPenalty = 0;	/* No penalty */
						    pEntry->TxQuality[pEntry->
								      CurrTxRateIndex]
						    = 0;
						    pEntry->PER[pEntry->
								CurrTxRateIndex]
	...
	/* RDG threshold control for the infrastructure mode only */
							...
							((pAd->RalinkCounters.
							  ...
							  OneSecTransmittedByteCount)
							 >= (50 * 1024))) {
						    RTMP_IO_READ32(pAd, TX_LINK_CFG, &TxLinkCfg.word); TxLinkCfg.field.TxRDGEn = 0; RTMP_IO_WRITE32(pAd, TX_LINK_CFG, TxLinkCfg.word); RTMP_IO_READ32(pAd, TXOP_THRES_CFG, &TxopThCfg.word); TxopThCfg.field.RDG_IN_THRES = 0xFF;	/* Similar to diable Rx RDG */
						    TxopThCfg.field.
						    RDG_OUT_THRES = 0x00;
						    RTMP_IO_WRITE32(pAd,
								    TXOP_THRES_CFG,
...
*/
						    ...
						    CurrRateIdx = TableSize - 1;
						    UpRateIdx = DownRateIdx =
						    pEntry->AGSCtrl.lastRateIdx;
						    pCurrTxRate =
						    (RTMP_RA_AGS_TB
						     *) (&pTable[(CurrRateIdx +
								  1) *
								 SIZE_OF_AGS_RATE_TABLE_ENTRY]);
						    if ((pAGSStatisticsInfo->
							 RSSI > -65)
							&& (pCurrTxRate->Mode >=
							    MODE_HTMIX)) {
						    TrainUp =
						    ...
						    ratio = 4; if (pAGSStatisticsInfo->TxErrorRatio >= TrainDown)	/* Poor quality */
						    pEntry->
						    TxQuality[CurrRateIdx] =
						    AGS_TX_QUALITY_WORST_BOUND;
						    pEntry->PER[CurrRateIdx] =
						    ...
						    if ((pEntry->LastSecTxRateChangeAction == 2) && (CurrRateIdx != UpRateIdx))	// Tx rate up
						    {
...
// Don't quick check within train down case
						    ...
						    switch (pEntry->AGSCtrl.
							    MCSGroup) {
case 3:
UpRateIdx = pCurrTxRate->upMcs3; break; case 2:
UpRateIdx = pCurrTxRate->upMcs2; break; case 1:
UpRateIdx = pCurrTxRate->upMcs1; break; case 0:
UpRateIdx = CurrRateIdx; break; default:
						    {
						    DBGPRINT_RAW(RT_DEBUG_INFO |
								 DBG_FUNC_RA,
						    ...
						    DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s: QuickAGS: ++TX rate from %d to %d\n", __FUNCTION__, CurrRateIdx, pEntry->CurrTxRateIndex)); pEntry->TxRateUpPenalty = 0; pEntry->TxQuality[pEntry->CurrTxRateIndex] = 0;	/*restore the TxQuality from max to 0 */
						    RTMPZeroMemory(pEntry->PER,
								   sizeof(UCHAR)
								   *
								   (MAX_TX_RATE_INDEX
						    ...
						    if ((pEntry->CurrTxRateIndex != CurrRateIdx) && (pEntry->LastSecTxRateChangeAction == 1)) {	/* Tx rate down */
						    DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s: QuickAGS: --TX rate from %d to %d\n", __FUNCTION__, CurrRateIdx, pEntry->CurrTxRateIndex)); pEntry->TxRateUpPenalty = 0;	/* No penalty */
						    pEntry->TxQuality[pEntry->
								      CurrTxRateIndex]
						    = 0;
						    pEntry->PER[pEntry->
...
*/
						    ...
						    (IN RTMP_ADAPTER * pAd,
						     IN INT idx) {
						    UCHAR * pTable, TableSize =
						    0;
						    MAC_TABLE_ENTRY * pEntry =
						    &pAd->MacTab.Content[idx];
						    UCHAR UpRateIdx =
						    0, DownRateIdx =
						    0, CurrRateIdx = 0;
						    RTMP_RA_AGS_TB *
						    ...
						    pNextTxRate = NULL;
						    BOOLEAN bTxRateChanged =
						    TRUE, bUpgradeQuality =
						    FALSE;
						    UCHAR TrainUp =
						    0, TrainDown = 0, next_grp;
						    CHAR RssiOffset = 0;
						    ULONG TxTotalCnt,
						    TxErrorRatio = 0;
						    ULONG TxSuccess,
						    TxRetransmit, TxFailCount;
						    AGS_STATISTICS_INFO
						    AGSStatisticsInfo = {
						    0};
						    DBGPRINT_RAW(RT_DEBUG_TRACE,
						    ...
						    TxTotalCnt =
						    TxRetransmit + TxSuccess +
						    TxFailCount; if (TxTotalCnt)
						    TxErrorRatio =
						    ((TxRetransmit +
						      TxFailCount) * 100) /
						    TxTotalCnt;}
						    ...
						    DBGPRINT(RT_DEBUG_INFO |
							     DBG_FUNC_RA,
							     ("%s():Aid:%d, MCS:%d, TxErrRatio(Hw:0x%lx-0x%lx, Sw:0x%lx-%lx)\n",
							      __FUNCTION__,
							      pEntry->Aid,
							      pEntry->HTPhyMode.
							      field.MCS,
						    ...
						    AGSStatisticsInfo.
						    AccuTxTotalCnt = TxTotalCnt;
						    AGSStatisticsInfo.
						    TxTotalCnt = TxTotalCnt;
						    AGSStatisticsInfo.
						    TxSuccess = TxSuccess;
						    AGSStatisticsInfo.
						    TxRetransmit = TxRetransmit;
						    AGSStatisticsInfo.
						    TxFailCount = TxFailCount;
						    DBGPRINT(RT_DEBUG_INFO |
							     DBG_FUNC_RA,
...
UpRateIdx = pCurrTxRate->upMcs3; break; case 2:
UpRateIdx = pCurrTxRate->upMcs2; break; case 1:
UpRateIdx = pCurrTxRate->upMcs1; break; default:
						    {
						    DBGPRINT_RAW(RT_DEBUG_ERROR,
								 ("%s: AGS: Incorrect MCS group, pEntry->AGSCtrl.MCSGroup = %d\n",
								  __FUNCTION__,
...
UpRateIdx = pCurrTxRate->upMcs1; break; default:
						    ...
						    DBGPRINT_RAW(RT_DEBUG_INFO |
								 ...
								 ("%s: AGS: TrainUp:%d, TrainDown:%d\n",
								  ...
								  TrainDown));
						    do {
						    BOOLEAN bTrainUpDown = FALSE; DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s: AGS: TxQuality[CurrRateIdx(%d)] = %d, UpPenalty:%d\n", __FUNCTION__, CurrRateIdx, pEntry->TxQuality[CurrRateIdx], pEntry->TxRateUpPenalty)); if (AGSStatisticsInfo.TxErrorRatio >= TrainDown) {	/* Poor quality */
						    /* error ratio too high, do rate down */
						    DBGPRINT_RAW(RT_DEBUG_INFO |
								 DBG_FUNC_RA,
								 ("%s: AGS: (DOWN) TxErrorRatio >= TrainDown\n",
						    ...
						    if (AGSStatisticsInfo.TxErrorRatio <= TrainUp) {	/* Good quality */
						    bTrainUpDown = TRUE;
						    bUpgradeQuality = TRUE;
						    DBGPRINT_RAW(RT_DEBUG_INFO |
								 DBG_FUNC_RA,
	...
	/* Tx rate fast train up/down */
						    if ((bTxRateChanged == TRUE)
							&& (!pAd->ApCfg.
							    ApQuickResponeForRateUpTimerRunning))
						    {
						    RTMPSetTimer(&pAd->ApCfg.
...
*/
						    ...
						    AGS_STATISTICS_INFO
						    AGSStatisticsInfo = {
						    0}; ULONG TxTotalCnt, TxErrorRatio = 0; ULONG TxSuccess, TxRetransmit, TxFailCount; DBGPRINT_RAW(RT_DEBUG_TRACE, ("QuickAGS: ---> %s\n", __FUNCTION__)); pAd->ApCfg.ApQuickResponeForRateUpTimerRunning = FALSE; pEntry = &pAd->MacTab.Content[idx];	/* point to information of the individual station */
						    pTable = pEntry->pTable;
						    TableSize = pTable[0];
						    if (pAd->MacTab.Size == 1) {
						    TX_STA_CNT1_STRUC StaTx1;
						    ...
						    ratio = 4; if (AGSStatisticsInfo.TxErrorRatio >= TrainDown)	/* Poor quality */
						    pEntry->
						    TxQuality[CurrRateIdx] =
						    AGS_TX_QUALITY_WORST_BOUND;
						    pEntry->PER[CurrRateIdx] =
