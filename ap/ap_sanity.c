...
BOOLEAN PeerAssocReqCmmSanity(RTMP_ADAPTER * pAd,
			      BOOLEAN isReassoc,
			      VOID * Msg, INT MsgLen, IE_LISTS * ie_lists)
{
	CHAR *Ptr;
	...
	while (((UCHAR *) eid_ptr + eid_ptr->Len + 1) < ((UCHAR *) Fr + MsgLen)) {
		...
		case IE_SUPP_RATES:
			if ((eid_ptr->Len <= MAX_LEN_OF_SUPPORTED_RATES) &&
				...
				UCHAR RateDefault[8] =
				    { 0x82, 0x84, 0x8b, 0x96, 0x12, 0x24, 0x48,
			 0x6c };
		...
		case IE_EXT_SUPP_RATES:
			if (eid_ptr->Len + ie_lists->SupportedRatesLen <=
				...
				NdisMoveMemory(&ie_lists->
					       SupportedRates[ie_lists->
							      SupportedRatesLen],
					       eid_ptr->Octet, eid_ptr->Len);
				ie_lists->SupportedRatesLen += eid_ptr->Len;
			} else {
		...
		case IE_WPA2:
...
#ifdef P2P_SUPPORT
			if (NdisEqualMemory
			    (eid_ptr->Octet, P2POUIBYTE, sizeof(P2POUIBYTE))
			    && (eid_ptr->Len >= 4)) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 (" ! ===>P2P - PeerAssocReqSanity  P2P IE Len becomes = %lu.   %s\n",
					  ie_lists->P2PSubelementLen,
			...
			if (NdisEqualMemory(eid_ptr->Octet, WPS_OUI, 4)) {
				...
				if ((pAd->ApCfg.MBSSID[pEntry->apidx].
				     WscControl.WscV2Info.bWpsEnable)
				    || (pAd->ApCfg.MBSSID[pEntry->apidx].
					WscControl.WscV2Info.bEnableWpsV2 ==
					FALSE))
			...
			if (pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode <
			...
			if (!NdisEqualMemory
			    (eid_ptr->Octet, WPA1_OUI, sizeof(WPA1_OUI))
			...
			if ( /*(eid_ptr->Len <= MAX_LEN_OF_RSNIE) && */
				...
				hex_dump("Received RSNIE in Assoc-Req",
					 (UCHAR *) eid_ptr, eid_ptr->Len + 2);
		...
		case IE_WAPI:
			if ((pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode !=
			    ...
			    && (pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode !=
...
BOOLEAN APPeerAuthSanity(IN PRTMP_ADAPTER pAd,
			 ...
			 OUT PUCHAR pAddr2,
			 OUT USHORT * Alg,
			 OUT USHORT * Seq, OUT USHORT * Status, CHAR * ChlgText)
{
	PFRAME_802_11 Fr = (PFRAME_802_11) Msg;
	...
	if (*Alg == AUTH_MODE_OPEN) {
		...
		if (*Seq == 1 || *Seq == 4) {
			...
			NdisMoveMemory(ChlgText, &Fr->Octet[8],
				       CIPHER_TEXT_LEN);
			return TRUE;
		} else {
			DBGPRINT(RT_DEBUG_TRACE,
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("APPeerAuthSanity fail - wrong algorithm (=%d)\n",
			  *Alg));
		return FALSE;
	}
