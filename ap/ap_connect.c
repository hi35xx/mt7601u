...
/*
	...
	==========================================================================
*/
BOOLEAN BeaconTransmitRequired(IN RTMP_ADAPTER * pAd,
			       IN INT apidx, IN MULTISSID_STRUCT * pMbss)
{
...
VOID APMakeBssBeacon(RTMP_ADAPTER * pAd, INT apidx)
{
	...
	RTMPWriteTxWI(pAd, &pAd->BeaconTxWI, FALSE, FALSE, TRUE, FALSE, FALSE,
		      TRUE, 0, BSS0Mcast_WCID, FrameLen, PID_MGMT, 0, 0,
	...
	reg_base =
	    pAd->BeaconOffset[pAd->ApCfg.MBSSID[apidx].BcnBufIdx] + TXWISize;
...
VOID APUpdateBeaconFrame(RTMP_ADAPTER * pAd, INT apidx)
{
	...
	*(ptr + 4) = TimFirst;
	...
	if (pAd->ApCfg.DtimCount == 0)
		*(ptr + 4) |=
		    (pMbss->TimBitmaps[WLAN_CT_TIM_BCMC_OFFSET] & 0x01);
	...
	if (((pAd->ApCfg.MBSSID[apidx].WscControl.WscConfMode >= 1)
	     && (pAd->ApCfg.MBSSID[apidx].WscIEBeacon.ValueLen))) {
		bHasWpsIE = TRUE;
	}
...
/*		USHORT	b2lTmp, b2lTmp2; // no use */
	...
	if (pMbss->bWmmCapable) {
	  ...
	  0x01, 0, 0 };
		...
		NdisMoveMemory(AIFSN, pAd->ApCfg.BssEdcaParm.Aifsn,
			       sizeof(AIFSN));
	...
	if (pAd->FlgQloadEnable != 0) {
		FrameLen +=
		    QBSS_LoadElementAppend(pAd, pBeaconFrame + FrameLen);
	}
	...
	if (WMODE_CAP_N(PhyMode) && (pMbss->DesiredHtPhyInfo.bHtEnable)) {
		...
		if (pAd->bBroadComHT == TRUE) {
			...
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  ...
					  END_OF_ARGS);
#else
			NdisMoveMemory(&HtCapabilityTmp, &pComCfg->HtCapability,
				       HtLen);
			*(USHORT *) (&HtCapabilityTmp.HtCapInfo) =
			    SWAP16(*(USHORT *) (&HtCapabilityTmp.HtCapInfo));
#ifdef UNALIGNMENT_SUPPORT
			...
			*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
			...
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  ...
					  END_OF_ARGS);
#else
			NdisMoveMemory(&addHTInfoTmp, &pComCfg->AddHTInfo,
				       HtLen1);
			*(USHORT *) (&addHTInfoTmp.AddHtInfo2) =
			    SWAP16(*(USHORT *) (&addHTInfoTmp.AddHtInfo2));
			*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
	...
	if (P2P_GO_ON(pAd)) {
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &P2pTmpLen,
				  ...
				  1, &P2pCapId,
				  2, &P2pCapIdLen,
				  2, &pAd->P2pCfg.P2pCapability, END_OF_ARGS);
		FrameLen += P2pTmpLen;

		MakeOutgoingFrame(pBeaconFrame + FrameLen, &P2pTmpLen,
				  1, &P2pDevId,
				  2, &P2pDevIdLen,
				  6, &pAd->P2pCfg.CurrentAddress, END_OF_ARGS);
		FrameLen += P2pTmpLen;
...
#endif				/* A_BAND_SUPPORT */
	RTMPWriteTxWI(pAd, &pAd->BeaconTxWI, FALSE, FALSE, TRUE, FALSE, FALSE,
		      TRUE, 0, 0xff, FrameLen, PID_MGMT, QID_MGMT, 0,
		      IFS_HTTXOP, FALSE, &BeaconTransmit);
...
/*
    ...
    ==========================================================================
*/
static UCHAR GetBcnNum(IN PRTMP_ADAPTER pAd)
{
	int i;
	int NumBcn;
...
VOID APMakeAllBssBeacon(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < HW_BEACON_MAX_COUNT(pAd); i++) {
		for (j = 0; j < TXWISize; j += 4) {
			RTMP_CHIP_UPDATE_BEACON(pAd, pAd->BeaconOffset[i] + j,
						0, 4);
		}
	}
