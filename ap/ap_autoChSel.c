...
VOID UpdateChannelInfo(IN PRTMP_ADAPTER pAd,
		       IN int ch_index, IN ChannelSel_Alg Alg)
{
	if (pAd->pChannelInfo != NULL) {
		...
		if (Alg == ChannelAlgCCA) {
			...
			pAd->pChannelInfo->FalseCCA[ch_index] =
			    RxStaCnt1.field.FalseCca;
		}
...
static inline VOID AutoChannelSkipListSetDirty(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < pAd->ApCfg.AutoChannelSkipListNum; i++) {
		UCHAR channel_idx =
		    GetChIdx(pAd, pAd->ApCfg.AutoChannelSkipList[i]);
	...
	if (pAd->CommonCfg.bIEEE80211H) {
		...
		for (i = 0; i < pAd->ChannelListNum; i++) {
			...
			if (AutoChannelSkipListCheck
			    (pAd, pAd->ChannelList[i].Channel) == TRUE)
				continue;
		...
		ch = pAd->ChannelList[RandomByte2(pAd) %
				      pAd->ChannelListNum].Channel;
		if (ch == 0)
			ch = FirstChannel(pAd);
	}
...
#define CCA_THRESHOLD (100)
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Channel %d : Dirty = %ld, False CCA = %u, Busy Time = %u, Skip Channel = %s\n",
			  ...
			  0,
#endif				/* AP_QLOAD_SUPPORT */
			  (pChannelInfo->SkipList[channel_idx] ==
			   TRUE) ? "TRUE" : "FALSE"));
	}
	DBGPRINT(RT_DEBUG_TRACE,
		 ("=====================================================\n"));
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++) {
		...
		if (pChannelInfo->FalseCCA[channel_idx] <= CCA_THRESHOLD) {
			 ...
			 */
			if ((pAd->CommonCfg.RegTransmitSetting.field.BW ==
			     BW_40)
			...
			if (pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40) {
				if (pAd->pChannelInfo->IsABand) {
					if (((channel_idx +
						...
						dirtyness +=
						    pChannelInfo->
						    dirtyness[ChOffsetIdx];
					}
				} else {
					UCHAR ExChannel_idx = 0;
					if (pAd->ChannelList[channel_idx].
					    Channel == 14) {
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++) {
		ch = pAd->ChannelList[(base + channel_idx) %
				      pAd->ChannelListNum].Channel;
...
static inline UCHAR SelectClearChannelApCnt(IN PRTMP_ADAPTER pAd)
{
	...
	for (channel_index = 0; channel_index < pAd->ChannelListNum;
		...
		if (pChannelInfo->ApCnt[channel_index] > 0) {
			...
			if (pChannelInfo->IsABand) {
				...
				if (pAd->CommonCfg.RegTransmitSetting.field.
				    BW == BW_40) {
					if (Channel > 14) {
						if ((Channel == 36)
						    || (Channel == 44)
	...
	for (channel_index = 0; channel_index < pAd->ChannelListNum;
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Channel %d : Dirty = %ld, ApCnt=%ld, Busy Time = %d, Skip Channel = %s\n",
			  ...
			  0,
#endif				/* AP_QLOAD_SUPPORT */
			  (pChannelInfo->SkipList[channel_index] ==
			   TRUE) ? "TRUE" : "FALSE"));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("=====================================================\n"));
	...
	for (dirty = 30; dirty <= 32; dirty++) {
		...
		if (candidate_num) {
			for (channel_index = 0;
			     channel_index < pAd->ChannelListNum;
			     channel_index++) {
				...
				if (candidate[channel_index]
					...
					if ((pAd->CommonCfg.RegTransmitSetting.
					     field.BW == BW_40)
					    &&
					    (BW40_ChannelCheck
					     (pAd->ChannelList[channel_index].
	...
	for (channel_index = 0; channel_index < pAd->ChannelListNum;
	     channel_index++) {
		final_channel =
		    pAd->ChannelList[(base + channel_index) %
				     pAd->ChannelListNum].Channel;
			   ...
			   IN UCHAR ExtChOffset, IN CHAR Rssi)
{
	...
	Idx =
	    AutoChBssSearchWithSSID(pAd, pBssid, (PUCHAR) Ssid, SsidLen,
				    ChannelNo);
	if (Idx == BSS_NOT_FOUND) {
		if (pBssInfoTab->BssNr >= MAX_LEN_OF_BSS_TABLE)
			return BSS_NOT_FOUND;
		Idx = pBssInfoTab->BssNr;
		AutoChBssEntrySet(&pBssInfoTab->BssEntry[Idx], pBssid, Ssid,
				  SsidLen, ChannelNo, ExtChOffset, Rssi);
		pBssInfoTab->BssNr++;
	} else {
...
/*	pAd->pBssInfoTab = (PBSSINFO)kmalloc(sizeof(BSSINFO), GFP_ATOMIC); */
	...
	if (pAd->pBssInfoTab)
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s Fail to alloc memory for pAd->pBssInfoTab",
			  __FUNCTION__));
...
void CheckPhyModeIsABand(IN PRTMP_ADAPTER pAd)
{
	...
	pAd->pChannelInfo->IsABand =
	    (WMODE_CAP_5G(pAd->CommonCfg.PhyMode)) ? TRUE : FALSE;
...
UCHAR APAutoSelectChannel(IN PRTMP_ADAPTER pAd, IN ChannelSel_Alg Alg)
{
	...
	if ((Alg == ChannelAlgRandom) && (pAd->pChannelInfo->IsABand == TRUE)) {	/*for Dfs */
		...
		for (i = 0; i < pAd->ChannelListNum; i++) {
			AsicSwitchChannel(pAd, pAd->ChannelList[i].Channel,
					  TRUE);
...
#ifdef AP_QLOAD_SUPPORT
			if (QLOAD_DOES_ALARM_OCCUR(pAd)) {	/* QLOAD ALARM, ever alarm from QLOAD module */
