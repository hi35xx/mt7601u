...
/*
    ...
    ==========================================================================
*/
BOOLEAN RTMPCheckMcast(IN PRTMP_ADAPTER pAd,
		       IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (eid_ptr->Len >= 6) {
		...
		if (eid_ptr->Eid == IE_WPA) {
			...
			else if (pAd->ApCfg.MBSSID[apidx].AuthMode ==
				 Ndis802_11AuthModeWPA1PSKWPA2PSK)
				pEntry->AuthMode = Ndis802_11AuthModeWPAPSK;

			if (NdisEqualMemory
			    (&eid_ptr->Octet[6],
			     &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][6], 4))
				return TRUE;
			...
			if (NdisEqualMemory
			    (&eid_ptr->Octet[2],
			     &pAd->ApCfg.MBSSID[apidx].RSN_IE[IE_Idx][2], 4))
				return TRUE;
		}
	}
...
/*
    ...
    ==========================================================================
*/
BOOLEAN RTMPCheckUcast(IN PRTMP_ADAPTER pAd,
		       IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY * pEntry)
{
	PUCHAR pStaTmp;
	USHORT Count;
	...
	if (eid_ptr->Eid == IE_WPA2) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("[ERROR]RTMPCheckUcast : invalid IE=%d\n",
			  eid_ptr->Eid));
		return FALSE;
	}
	...
	if (eid_ptr->Len >= 16) {
		if (eid_ptr->Eid == IE_WPA) {
			if (pAd->ApCfg.MBSSID[apidx].WepStatus == Ndis802_11Encryption4Enabled) {	/* multiple cipher (TKIP/CCMP) */

				while (Count > 0) {
					/* TKIP */
					if (MIX_CIPHER_WPA_TKIP_ON
...
/*
    ...
    ==========================================================================
*/
BOOLEAN RTMPCheckAUTH(IN PRTMP_ADAPTER pAd,
		      IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY * pEntry)
{
	PUCHAR pStaTmp;
	USHORT Count;
	...
	if (eid_ptr->Len >= 16) {
		if (eid_ptr->Eid == IE_WPA) {
			while (Count > 0) {
				if (RTMPCheckAKM
				    (pStaTmp,
				     &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][0], 0))
					return TRUE;
		     ...
		     IN PUCHAR pRsnIe, IN UCHAR rsnie_len)
{
	...
	if ((eid_ptr->Len + 2) != rsnie_len) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("[ERROR]APValidateRSNIE : the len is invalid !!!\n"));
	...
	if (StatusCode != MLME_SUCCESS) {
		...
		RTMPSendWirelessEvent(pAd, IW_RSNIE_SANITY_FAIL_EVENT_FLAG,
				      pEntry->Addr, 0, 0);
		DBGPRINT(RT_DEBUG_ERROR,
...
VOID HandleCounterMeasure(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pAd->ApCfg.CMTimerRunning == TRUE) {
		...
		/* renew GTK */
		GenRandom(pAd, pAd->ApCfg.MBSSID[pEntry->apidx].Bssid,
		  ...
		  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)) {
		...
		case Ndis802_11AuthModeWPA2PSK:
			...
			if (pEntry->ReTryCounter >
			    (GROUP_MSG1_RETRY_TIMER_CTR + 1)) {
			...
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pMbss->WPAREKEY.ReKeyMethod == TIME_REKEY)
	    && (pMbss->REKEYCOUNTER < 0xffffffff))
	...
	if (RTMPEqualMemory((pPacket + 12), EAPOL, 2)) {
		...
		case EAP_CODE_SUCCESS:
			if ((pEntry->AuthMode >= Ndis802_11AuthModeWPA)
				...
				pEntry->WpaState = AS_PTKINITDONE;
				pAdapter->ApCfg.MBSSID[pEntry->apidx].
				    PortSecured = WPA_802_1X_PORT_SECURED;
				pEntry->PortSecured = WPA_802_1X_PORT_SECURED;
#ifdef WSC_AP_SUPPORT
				if (pAdapter->ApCfg.MBSSID[pEntry->apidx].
				    WscControl.WscConfMode != WSC_DISABLE)
		       ...
		       IN PUCHAR pAddr, IN UCHAR * PMKID, IN UCHAR * PMK)
{
	...
	if ((CacheIdx = RTMPSearchPMKIDCache(pAd, apidx, pAddr)) != -1) {
		NdisGetSystemUpTime(&
				    (pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				     BSSIDInfo[CacheIdx].RefreshTime));
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			       BSSIDInfo[CacheIdx].PMKID, PMKID, LEN_PMKID);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			       BSSIDInfo[CacheIdx].PMK, PMK, PMK_LEN);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RTMPAddPMKIDCache update %02x:%02x:%02x:%02x:%02x:%02x cache(%d) from IF(ra%d)\n",
			  pAddr[0], pAddr[1], pAddr[2], pAddr[3], pAddr[4],
			  pAddr[5], CacheIdx, apidx));
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		if (!pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].Valid) {
			pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].Valid =
			    TRUE;
			NdisGetSystemUpTime(&
					    (pAd->ApCfg.MBSSID[apidx].
					     PMKIDCache.BSSIDInfo[i].
					     RefreshTime));
			COPY_MAC_ADDR(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				      BSSIDInfo[i].MAC, pAddr);
			NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				       BSSIDInfo[i].PMKID, PMKID, LEN_PMKID);
			NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				       BSSIDInfo[i].PMK, PMK, PMK_LEN);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("RTMPAddPMKIDCache add %02x:%02x:%02x:%02x:%02x:%02x cache(%d) from IF(ra%d)\n",
				  pAddr[0], pAddr[1], pAddr[2], pAddr[3],
				  pAddr[4], pAddr[5], i, apidx));
			break;
		}
	}
	...
	if (i == MAX_PMKID_COUNT) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RTMPAddPMKIDCache(IF(%d) Cache full\n", apidx));
		for (i = 0; i < MAX_PMKID_COUNT; i++) {
			if (pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].
			    Valid) {
				if (((timestamp == 0) && (idx == 0))
				    || ((timestamp != 0)
					&& timestamp <
					pAd->ApCfg.MBSSID[apidx].PMKIDCache.
					BSSIDInfo[i].RefreshTime)) {
					timestamp =
					    pAd->ApCfg.MBSSID[apidx].PMKIDCache.
					    BSSIDInfo[i].RefreshTime;
					idx = i;
		...
		NdisGetSystemUpTime(&
				    (pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				     BSSIDInfo[idx].RefreshTime));
		COPY_MAC_ADDR(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			      BSSIDInfo[idx].MAC, pAddr);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			       BSSIDInfo[idx].PMKID, PMKID, LEN_PMKID);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
			       BSSIDInfo[idx].PMK, PMK, PMK_LEN);
		DBGPRINT(RT_DEBUG_TRACE,
...
INT RTMPSearchPMKIDCache(IN PRTMP_ADAPTER pAd, IN INT apidx, IN PUCHAR pAddr)
{
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		if ((pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].Valid)
		    && MAC_ADDR_EQUAL(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.
				      BSSIDInfo[i].MAC, pAddr)) {
			DBGPRINT(RT_DEBUG_TRACE,
	...
	if (i == MAX_PMKID_COUNT) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RTMPSearchPMKIDCache - IF(%d) not found\n", apidx));
		return -1;
	}
			  ...
			  IN UCHAR apidx, OUT PUCHAR pTxTsc)
{
	...
	if (apidx >= MAX_MBSSID_NUM(pAd)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RTMPGetTxTscFromAsic : invalid apidx(%d)\n", apidx));
		return;
	}
	...
	if (pAd->ApCfg.MBSSID[apidx].GroupKeyWepStatus == Ndis802_11Encryption3Enabled) {	/* AES */
		*pTxTsc = IvEiv[0];
		*(pTxTsc + 1) = IvEiv[1];
		*(pTxTsc + 2) = IvEiv[4];
		*(pTxTsc + 3) = IvEiv[5];
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("RTMPGetTxTscFromAsic : WCID(%d) TxTsc 0x%02x-0x%02x-0x%02x-0x%02x-0x%02x-0x%02x \n",
		  Wcid, *pTxTsc, *(pTxTsc + 1), *(pTxTsc + 2), *(pTxTsc + 3),
		  *(pTxTsc + 4), *(pTxTsc + 5)));
...
VOID RTMPHandleSTAKey(IN PRTMP_ADAPTER pAd,
		      IN PMAC_TABLE_ENTRY pEntry, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if ((pEntry->WpaState != AS_PTKINITDONE)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Not expect calling STAKey hand shaking here"));
		return;
	}
...
/*        Offset += 2; */
	...
	NdisMoveMemory((PUCHAR) & peerKeyInfo,
		       (PUCHAR) & pSTAKey->KeyDesc.KeyInfo, sizeof(KEY_INFO));
	*((USHORT *) & peerKeyInfo) = cpu2le16(*((USHORT *) & peerKeyInfo));
	/*Benson add 20081016 <-- */

	/* Check Replay Counter */
	if (!RTMPEqualMemory
	    (pSTAKey->KeyDesc.ReplayCounter, pEntry->R_Counter,
	     LEN_KEY_DESC_REPLAY)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Replay Counter Different in STAKey handshake!! \n"));
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Receive : %d %d %d %d  \n",
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Current : %d %d %d %d  \n", pEntry->R_Counter[4],
			  pEntry->R_Counter[5], pEntry->R_Counter[6],
			  pEntry->R_Counter[7]));
		return;
	}
	...
	if (peerKeyInfo.KeyMic && peerKeyInfo.Secure && peerKeyInfo.Request) {	/*Benson add for big-endian 20081016 --> */
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("STAKey Initiator: %02x:%02x:%02x:%02x:%02x:%02x\n",
			  pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
			  pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5]));
	}

	MICMsgLen = pSTAKey->Body_Len[1] | ((pSTAKey->Body_Len[0] << 8)
					    && 0xff00);
	MICMsgLen += LENGTH_EAPOL_H;
	if (MICMsgLen > (Elem->MsgLen - LENGTH_802_11 - LENGTH_802_1_H)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Receive wrong format EAPOL packets \n"));
		return;
	}
	...
	WpaDerivePTK(pAd, temp, temp, pAd->ApCfg.MBSSID[pEntry->apidx].Bssid,
		     temp, pEntry->Addr,
	...
	if (pEntry->WepStatus == Ndis802_11Encryption2Enabled) {
		RT_HMAC_MD5(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK,
			    LEN_PTK_KCK, (PUCHAR) pSTAKey, MICMsgLen, mic,
			    MD5_DIGEST_SIZE);
	} else {
		RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK,
			     LEN_PTK_KCK, (PUCHAR) pSTAKey, MICMsgLen, digest,
			     SHA1_DIGEST_SIZE);
	...
	if (!RTMPEqualMemory(rcv_mic, mic, LEN_KEY_DESC_MIC)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("MIC Different in STAKey handshake!! \n"));
		return;
	} else
		DBGPRINT(RT_DEBUG_TRACE,
			 ("MIC VALID in STAKey handshake!! \n"));
	...
	do {
		...
		MAKE_802_3_HEADER(Header802_3, pDaEntry->Addr,
				  pAd->ApCfg.MBSSID[pDaEntry->apidx].Bssid,
				  EAPOL);
...
/*        NdisMoveMemory(pDaEntry->R_Counter, pAd->ApCfg.R_Counter, sizeof(pDaEntry->R_Counter)); */
		NdisMoveMemory(pOutPacket->KeyDesc.ReplayCounter,
			       pDaEntry->R_Counter, LEN_KEY_DESC_REPLAY);
		...
		if (pDaEntry->WepStatus == Ndis802_11Encryption3Enabled) {
			...
			NdisMoveMemory(&Key_Data[14], pEntry->PairwiseKey.Key,
				       LEN_AES_TK);
		} else {
			Key_Data[1] = 0x2E;	/* 4+2+6+32(OUI+Reserved+STAKey_MAC_Addr+STAKey) */
			NdisMoveMemory(&Key_Data[14], pEntry->PairwiseKey.Key,
				       LEN_TK);
			NdisMoveMemory(&Key_Data[14 + LEN_TK],
				       pEntry->PairwiseKey.TxMic, LEN_TKIP_MIC);
		...
		NdisZeroMemory(temp, 64);
		NdisMoveMemory(temp, "IEEE802.11 WIRELESS ACCESS POINT", 32);
		WpaDerivePTK(pAd, temp, temp,
			     pAd->ApCfg.MBSSID[pEntry->apidx].Bssid, temp, DA,
			     pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK);
		...
		NdisMoveMemory(pOutPacket->KeyDesc.KeyData, Key_Data,
			       key_length);
		...
		*(USHORT *) (&pOutPacket->KeyDesc.KeyInfo) =
		...
		if (pDaEntry->WepStatus == Ndis802_11Encryption3Enabled) {
			RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK,
				     LEN_PTK_KCK, pOutBuffer, FrameLen, digest,
				     SHA1_DIGEST_SIZE);
			NdisMoveMemory(pOutPacket->KeyDesc.KeyMic, digest,
				       LEN_KEY_DESC_MIC);
		} else {
			RT_HMAC_MD5(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK,
				    LEN_PTK_KCK, pOutBuffer, FrameLen, mic,
				    MD5_DIGEST_SIZE);
			NdisMoveMemory(pOutPacket->KeyDesc.KeyMic, mic,
				       LEN_KEY_DESC_MIC);
		}

		RTMPToWirelessSta(pAd, pDaEntry, Header802_3, LENGTH_802_3,
				  (PUCHAR) pOutPacket,
...
/*	union iwreq_data wrqu; */
	...
	if (report) {		/*station reports a mic error to this ap. */
		...
		snprintf(buf, sizeof(buf), "%s(keyid=%d %scast addr=%s)", tag,
			 keyix,
			 IEEE80211_IS_MULTICAST(pHeader->
						Addr1) ? "broad" : "uni",
			    ...
			    IN PAPCLI_STRUCT pApCliEntry)
{
	...
	MAKE_802_3_HEADER(Header802_3, pBssid, &pApCliEntry->CurrentAddress[0],
			  EAPOL);
	...
	RTMPToWirelessSta((PRTMP_ADAPTER) pAd, pMacEntry,
			  Header802_3, LENGTH_802_3, (PUCHAR) & Packet, 4,
			  TRUE);
...
VOID ApcliSendAssocIEsToWpaSupplicant(IN RTMP_ADAPTER * pAd, IN UINT ifIndex)
{
	...
	if ((pAd->ApCfg.ApCliTab[ifIndex].ReqVarIELen + 17) <= IW_CUSTOM_MAX) {
		...
		NdisMoveMemory(custom + 17,
			       pAd->ApCfg.ApCliTab[ifIndex].ReqVarIEs,
