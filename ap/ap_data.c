...
static VOID APFindCipherAlgorithm(IN PRTMP_ADAPTER pAd, IN TX_BLK * pTxBlk);

#ifdef DOT11_N_SUPPORT
VOID RTMP_BASetup(IN PRTMP_ADAPTER pAd,
		  IN PMAC_TABLE_ENTRY pMacEntry, IN UINT8 UserPriority)
{
	if (pMacEntry && (pMacEntry->NoBADataCountDown == 0)
	    && IS_HT_STA(pMacEntry)) {
		...
		if (((pMacEntry->TXBAbitmap & (1 << UserPriority)) == 0)
			...
			BAOriSessionSetUp(pAd, pMacEntry, UserPriority, 0, 10,
					  FALSE);
		}
	}
}
...
static inline BOOLEAN ApAllowToSendPacket(IN RTMP_ADAPTER * pAd,
					  ...
					  OUT UCHAR * pWcid)
{
...
#ifdef CLIENT_WDS
	{
		PUCHAR pEntryAddr;
		pEntry =
		    APSsPsInquiry(pAd, pSrcBufVA, &Sst, &Aid, &PsMode, &Rate);
		if ((pEntry == NULL)
		    && (pEntryAddr =
			CliWds_ProxyLookup(pAd, pSrcBufVA)) != NULL) {
			pEntry =
			    APSsPsInquiry(pAd, pEntryAddr, &Sst, &Aid, &PsMode,
					  &Rate);
		}
	}
#else
...
VOID APSendPackets(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt)
{
	...
	for (Index = 0; Index < pkt_cnt; Index++) {
		...
		if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >=
		    MIN_NET_DEVICE_FOR_CFG80211_VIF_P2P_GO)
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);
		else if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >=
			 MIN_NET_DEVICE_FOR_CFG80211_VIF_AP)
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);
		else
		...
		if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >=
			...
			allowToSend =
			    ApCliAllowToSendPacket(pAd, pPacket, &wcid);
		else
#endif				/* APCLI_SUPPORT */
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);
...
NDIS_STATUS APSendPacket(RTMP_ADAPTER * pAd, PNDIS_PACKET pPacket)
{
	...
	if (SrcBufLen <= 14) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("APSendPacket --> Ndis Packet buffer error !!!\n"));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		return (NDIS_STATUS_FAILURE);
	}
	...
	UserPriority = 0;
	QueIdx = QID_AC_BE;
	if (RTMPCheckEtherType
	    (pAd, pPacket, pMacEntry, OPMODE_AP, &UserPriority,
	     &QueIdx) == FALSE) {
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		return NDIS_STATUS_FAILURE;
	}
	...
	if (IS_ENTRY_CLIENT(pMacEntry) || (Wcid == MCAST_WCID)) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("I/F(ra%d) APSendPacket --> Drop unknow packet !!!\n",
			  apidx));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		return NDIS_STATUS_FAILURE;
	}
	...
	if (pAd->ApCfg.IgmpSnoopEnable) {
		...
		if (IS_ENTRY_WDS(pMacEntry))
			FromWhichBSSID =
			    pMacEntry->MatchWDSTabIdx + MIN_NET_DEVICE_FOR_WDS;
	...
	if (pMacEntry
	    && (IS_ENTRY_WDS(pMacEntry) || IS_ENTRY_APCLI(pMacEntry)
		|| IS_ENTRY_MESH(pMacEntry))) {
	...
	/* M/BCAST frames are put to PSQ as long as there's any associated STA in power-save mode */
		...
		if (pAd->MacTab.McastPsQueue.Number >=
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("M/BCAST PSQ(=%ld) full, drop it!\n",
				  pAd->MacTab.McastPsQueue.Number));
			return NDIS_STATUS_FAILURE;
		} else {
			RTMP_IRQ_LOCK(&pAd->irq_lock, IrqFlags);
			InsertHeadQueue(&pAd->MacTab.McastPsQueue,
					PACKET_TO_QUEUE_ENTRY(pPacket));
			RTMP_IRQ_UNLOCK(&pAd->irq_lock, IrqFlags);
...
static inline VOID APFindCipherAlgorithm(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bApCliPacket)) {
		...
		if (RTMP_GET_PACKET_EAPOL(pTxBlk->pPacket)) {
			...
			if (CipherAlg)
				pKey =
				    &pApCliEntry->SharedKey[pApCliEntry->
							    DefaultKeyId];
	...
	if (pMbss->WepStatus == Ndis802_11EncryptionSMS4Enabled) {
		...
		if ((RTMP_GET_PACKET_EAPOL(pTxBlk->pPacket)) ||
#ifdef DOT1X_SUPPORT
		    ((pMbss->WepStatus == Ndis802_11Encryption1Enabled)
		     && (pMbss->IEEE8021X == TRUE)) ||
#endif				/* DOT1X_SUPPORT */
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bClearEAPFrame)) {
			...
			if (P2P_INF_ON(pAd) && P2P_GO_ON(pAd))
				CipherAlg =
				    pAd->SharedKey[apidx + 1][KeyIdx].CipherAlg;
			else
...
static inline VOID APBuildCommon802_11Header(RTMP_ADAPTER * pAd,
					     TX_BLK * pTxBlk)
{
...
static inline PUCHAR AP_Build_ARalink_Frame_Header(IN RTMP_ADAPTER * pAd,
						   IN TX_BLK * pTxBlk)
{
...
#ifdef UAPSD_SUPPORT
		if (CLIENT_STATUS_TEST_FLAG
		    (pTxBlk->pMacEntry, fCLIENT_STATUS_APSD_CAPABLE)
		    ) {
			/* 
...
static inline PUCHAR AP_Build_AMSDU_Frame_Header(IN RTMP_ADAPTER * pAd,
						 IN TX_BLK * pTxBlk)
{
	UCHAR *pHeaderBufPtr;
	HEADER_802_11 *pHeader_802_11;
	...
	*(pHeaderBufPtr + 1) = 0;
	...
	if (pTxBlk->pMacEntry && pAd->chipCap.FlgHwTxBfCap) {
		...
		if (pMacEntry->TxSndgType >= SNDG_TYPE_SOUNDING) {
			...
			if (pMacEntry->TxSndgType == SNDG_TYPE_SOUNDING) {
				...
				if (pAd->CommonCfg.ETxBfNoncompress == 0 &&
				    (pMacEntry->HTCapability.TxBFCap.ExpComBF >
				     0))
		...
		if ((pMacEntry->HTCapability.ExtHtCapInfo.MCSFeedback >=
		     MCSFBK_MRQ) && (pTxBlk->TxSndgPkt == SNDG_TYPE_DISABLE)) {
			if (bHTCPlus == FALSE) {
				bHTCPlus = TRUE;
		...
		if (pAd->CommonCfg.HtCapability.ExtHtCapInfo.MCSFeedback >=
		    MCSFBK_MRQ && pMacEntry->toTxMfb == 1) {
			if (bHTCPlus == FALSE) {
				NdisZeroMemory(pHeaderBufPtr,
					       sizeof(HT_CONTROL));
				bHTCPlus = TRUE;
...
VOID AP_AMPDU_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
		...
		*(pHeaderBufPtr + 1) = 0;
		...
		if ((pAd->CommonCfg.bRdg == TRUE)
		    ...
		    (CLIENT_STATUS_TEST_FLAG
		     (pTxBlk->pMacEntry, fCLIENT_STATUS_RDG_CAPABLE))
#ifdef TXBF_SUPPORT
	...
	if (pAd->CommonCfg.DebugFlags & DBF_DBQ_TXFRAME)
		dbQueueEnqueueTxFrame((UCHAR *) (&pTxBlk->
						 HeaderBuf[TXINFO_SIZE]),
...
*/
	...
	if ((pMacEntry->isCached)
		...
		pWiBufPtr =
		    (PUCHAR) (&pTxBlk->HeaderBuf[TXINFO_SIZE + TXWISize]);
...
VOID AP_AMSDU_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
		...
		if (frameNum == 0) {
			pHeaderBufPtr =
			    AP_Build_AMSDU_Frame_Header(pAd, pTxBlk);
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bApCliPacket)) {
			pApCliEntry =
			    &pAd->ApCfg.ApCliTab[pTxBlk->pMacEntry->
						 MatchAPCLITabIdx];
			if (pApCliEntry->Valid)
				NdisMoveMemory(&subFrameHeader[6],
					       pApCliEntry->CurrentAddress, 6);
		}
#endif				/* APCLI_SUPPORT */
...
#endif				/* WAPI_SUPPORT */
		{
			...
			if (pTxBlk->pMacEntry->WapiUskRekeyTimerRunning
			    ...
			    REKEY_METHOD_PKT)
				pTxBlk->pMacEntry->wapi_usk_rekey_cnt +=
				    totalMPDUSize;
#endif				/* WAPI_SUPPORT */
...
VOID AP_Legacy_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	if (pTxBlk->TxFrameType == TX_MCAST_FRAME) {
		...
		if (pAd->CommonCfg.WapiMskRekeyTimerRunning &&
			...
			pAd->CommonCfg.wapi_msk_rekey_cnt +=
			    (pTxBlk->SrcBufLen);
		}
#endif				/* WAPI_SUPPORT */
	}
...
VOID AP_ARalink_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
		...
		if (frameNum == 0) {	/* For first frame, we need to create the 802.11 header + padding(optional) + RA-AGG-LEN + SNAP Header */
			...
			pHeaderBufPtr =
			    AP_Build_ARalink_Frame_Header(pAd, pTxBlk);
		...
		if ((pRxInfo->U2M)
			...
			DBGPRINT(RT_DEBUG_OFF,
				 ("%s(): pRxInfo:Crc=%d, CipherErr=%d, U2M=%d, Wcid=%d\n",
				  ...
				  pRxInfo->CipherErr, pRxInfo->U2M, Wcid));
			return NDIS_STATUS_FAILURE;
		}
	} else {
		return NDIS_STATUS_SUCCESS;
				 ...
				 IN ULONG Wcid, IN PHEADER_802_11 pHeader)
{
	...
	if (Wcid >= MAX_LEN_OF_MAC_TABLE) {
		...
		DBGPRINT(RT_DEBUG_WARN,
			 ("%s():Rx a frame from %02x:%02x:%02x:%02x:%02x:%02x with WCID(%ld) > %d\n",
			  __FUNCTION__, PRINT_MAC(pHeader->Addr2), Wcid,
...
*/
/*static UCHAR is_on; */
VOID detect_wmm_traffic(IN RTMP_ADAPTER * pAd,
...
#ifdef RTMP_MAC_USB
				RTEnqueueInternalCmd(pAd,
						     CMDTHREAD_AP_ADJUST_EXP_ACK_TIME,
						     NULL, 0);
#endif				/* RTMP_MAC_USB */
				pAd->is_on = 1;
			}
...
#ifdef RTMP_MAC_USB
				RTEnqueueInternalCmd(pAd,
						     CMDTHREAD_AP_RECOVER_EXP_ACK_TIME,
						     NULL, 0);
#endif				/* RTMP_MAC_USB */
				pAd->is_on = 0;
			}
...
VOID dynamic_tune_be_tx_op(RTMP_ADAPTER * pAd, ULONG nonBEpackets)
{
	...
	if (pAd->CommonCfg.bEnableTxBurst
#ifdef DOT11_N_SUPPORT
	    || pAd->CommonCfg.bRdg || pAd->CommonCfg.bRalinkBurstMode
#endif				/* DOT11_N_SUPPORT */
	    ) {
		...
		if (
			...
			if (RTMP_TEST_FLAG
			    (pAd, fRTMP_ADAPTER_DYNAMIC_BE_TXOP_ACTIVE)) {
				RTMP_IO_READ32(pAd, EDCA_AC0_CFG, &RegValue);

				if (RTMP_TEST_FLAG
				    (pAd, fRTMP_ADAPTER_RALINK_BURST_MODE)) {
					RegValue =
					    pAd->CommonCfg.RestoreBurstMode;
					RTMP_CLEAR_FLAG(pAd,
							fRTMP_ADAPTER_RALINK_BURST_MODE);
				}

				if (RTMP_TEST_FLAG
				    (pAd, fRTMP_ADAPTER_RDG_ACTIVE)) {
					TX_LINK_CFG_STRUC TxLinkCfg;

					RTMP_IO_READ32(pAd, TX_LINK_CFG,
						       &TxLinkCfg.word);
					TxLinkCfg.field.TxRDGEn = 0;
					RTMP_IO_WRITE32(pAd, TX_LINK_CFG,
							TxLinkCfg.word);
				...
				if (pAd->CommonCfg.APEdcaParm.Txop[QID_AC_VO] !=
				    102) {
			...
			if (RTMP_TEST_FLAG
			    (pAd, fRTMP_ADAPTER_DYNAMIC_BE_TXOP_ACTIVE) == 0) {
				/* enable AC0(BE) TX_OP */
...
VOID APRxDErrorHandle(RTMP_ADAPTER * pAd, RX_BLK * pRxBlk)
{
	...
	if (pRxInfo->U2M && pRxInfo->CipherErr) {
		if (pRxWI->RxWIWirelessCliID < MAX_LEN_OF_MAC_TABLE) {
			...
			if (VALID_WCID(Wcid))
				pEntry =
				    ApCliTableLookUpByWcid(pAd, Wcid,
							   pHeader->Addr2);
			else
				pEntry = MacTableLookup(pAd, pHeader->Addr2);
			...
			if (pEntry && IS_ENTRY_APCLI(pEntry)) {
				...
				if (pRxInfo->CipherErr == 2) {
					...
					if (pAd->ApCfg.
					    ApCliTab[pEntry->MatchAPCLITabIdx].
					    WpaSupplicantUP)
...
#endif				/* APCLI_SUPPORT */
			{
				...
				if ((pEntry->WepStatus ==
					...
					{
						RTMP_HANDLE_COUNTER_MEASURE(pAd,
									    pEntry);
					}
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Rx u2me Cipher Err(MPDUsize=%d, WCID=%d, CipherErr=%d)\n",
			  pRxWI->RxWIMPDUByteCnt, pRxWI->RxWIWirelessCliID,
			  pRxInfo->CipherErr));
...
BOOLEAN APCheckVaildDataFrame(IN RTMP_ADAPTER * pAd, IN RX_BLK * pRxBlk)
{
	PHEADER_802_11 pHeader = pRxBlk->pHeader;
	RXWI_STRUC *pRxWI = pRxBlk->pRxWI;
	...
	do {
		...
		if ((pHeader->FC.FrDs == 0)
		    ...
		    (APCheckClass2Class3Error
		     (pAd, pRxWI->RxWIWirelessCliID, pHeader)))
			break;	/* give up this frame */
...
BOOLEAN APCheckTkipMICValue(IN PRTMP_ADAPTER pAd,
			    IN MAC_TABLE_ENTRY * pEntry, IN RX_BLK * pRxBlk)
{
	...
	if (RTMPTkipCompareMICValue(pAd,
		...
		if (IS_ENTRY_APCLI(pEntry)
		    && pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
		    WpaSupplicantUP) {
...
VOID APHandleRxMgmtFrame(IN PRTMP_ADAPTER pAd, IN RX_BLK * pRxBlk)
{
	RXINFO_STRUC *pRxInfo = pRxBlk->pRxInfo;
	RXWI_STRUC *pRxWI = pRxBlk->pRxWI;
	PHEADER_802_11 pHeader = pRxBlk->pHeader;
	...
	if ((pHeader->FC.SubType == SUBTYPE_PROBE_REQ) ||
	    ((pHeader->FC.SubType == SUBTYPE_ACTION) &&
	     (CFG80211DRV_PrintFrameType
	      (pAd, "P2P_CHECK", (PUCHAR) pHeader,
	       pRxWI->RXWI_O.MPDUtotalByteCnt) == TRUE))) {
		UINT32 freq;
		...
		if (pAd->Cfg80211VifDevSet.vifDevList.size > 0) {
			if ((pNetDev =
			    ...
			    != NULL) {
				if (CFG80211OS_RxMgmt
				    (pNetDev, freq, (PUCHAR) pHeader,
				     pRxWI->RXWI_O.MPDUtotalByteCnt)) {
					DBGPRINT(RT_DEBUG_INFO,
						 ("VIF AP GO RtmpOsCFG80211RxMgmt OK!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
					...
					DBGPRINT(RT_DEBUG_INFO,
						 ("VIF AP GO RtmpOsCFG80211RxMgmt FAIL!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
						  pHeader->FC.SubType, freq,
						  PRINT_MAC(pHeader->Addr2)));
			}
		...
		if (((pHeader->FC.SubType == SUBTYPE_PROBE_REQ)
			...
			//pAd->net_dev
			if (CFG80211OS_RxMgmt
			    (pAd->dummy_p2p_net_dev, freq, (PUCHAR) pHeader,
			     pRxWI->RXWI_O.MPDUtotalByteCnt)) {
				DBGPRINT(RT_DEBUG_INFO,
					 ("MAIN AP RtmpOsCFG80211RxMgmt OK!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
					  pHeader->FC.SubType, freq,
					  PRINT_MAC(pHeader->Addr2)));
				RELEASE_NDIS_PACKET(pAd, pRxPacket,
						    NDIS_STATUS_SUCCESS);
				...
				DBGPRINT(RT_DEBUG_INFO,
					 ("MAIN AP RtmpOsCFG80211RxMgmt FAIL!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
					  pHeader->FC.SubType, freq,
					  PRINT_MAC(pHeader->Addr2)));
			}
		}
	...
	do {
...
#ifdef MT7601
		if (IS_MT7601(pAd))
			if (RTMPSpoofedMgmtDetection
			    (pAd, pHeader, pRxWI->RxWISNR2, 0, 0,
			     pRxWI->RxWISNR1)) {
		...
		if (pHeader->FC.SubType == SUBTYPE_ACTION) {
			...
			if (pEntry != NULL)
				APPsIndicate(pAd, pHeader->Addr2, pEntry->Aid,
					     pHeader->FC.PwrMgmt);
		...
		if (IS_MT7601(pAd)) {
			REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->RxWIWirelessCliID,
						  ...
						  pRxWI->RxWISNR2, 0, 0, 0,
						  pRxWI->RxWISNR1, OPMODE_AP);
		} else
#endif				/* MT7601 */
			REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->RxWIWirelessCliID,
						  pHeader,
...
VOID APHandleRxControlFrame(IN PRTMP_ADAPTER pAd, IN RX_BLK * pRxBlk)
{
	PHEADER_802_11 pHeader = pRxBlk->pHeader;
	PNDIS_PACKET pRxPacket = pRxBlk->pRxPacket;
	...
	case SUBTYPE_BLOCK_ACK_REQ:
		{
			RXWI_STRUC *pRxWI = pRxBlk->pRxWI;
			CntlEnqueueForRecv(pAd, pRxWI->RxWIWirelessCliID,
					   (pRxWI->RxWIMPDUByteCnt),
			    ...
			    IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (pRxBlk->DataSize < (LENGTH_802_1_H + LENGTH_EAPOL_H)) {
		...
		CheckPktSanity = FALSE;
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Can't find SNAP_802_1H parameter.\n"));
	...
	if (IS_ENTRY_APCLI(pEntry)) {
		eapcode =
		    ApcliWpaCheckEapCode(pAd, pRxBlk->pData, pRxBlk->DataSize,
					 LENGTH_802_1_H);
		printk("eapcode=%d\n", eapcode);
		if (pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
		    WpaSupplicantUP
		    && pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
		    IEEE8021X == TRUE && (EAP_CODE_SUCCESS == eapcode)) {
			PUCHAR Key;
			UCHAR CipherAlg;
			int idx = 0;
			int BssIdx =
			...
			if (pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
				...
				if (pAd->ApCfg.
					...
					RTMP_ASIC_SHARED_KEY_TABLE(pAd, BssIdx,
								   idx,
								   &pAd->ApCfg.
								   ApCliTab
								   [pEntry->
					...
					pAd->ApCfg.ApCliTab[pEntry->
							    MatchAPCLITabIdx].
					    SharedKey[idx].CipherAlg =
					    CipherAlg;
					pAd->ApCfg.ApCliTab[pEntry->
							    MatchAPCLITabIdx].
					    SharedKey[idx].KeyLen =
					    pAd->ApCfg.ApCliTab[pEntry->
								MatchAPCLITabIdx].
	...
	if ((pEntry) && (pEntry->WpaState < AS_INITPMK) &&
		...
		if ((pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscConfMode !=
		     WSC_DISABLE)
		    &&
		    (!MAC_ADDR_EQUAL
...
VOID APRxDataFrameAnnounce(IN RTMP_ADAPTER * pAd,
			   IN MAC_TABLE_ENTRY * pEntry,
			   IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe
		...
		if (pEntry && (IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_WDS(pEntry))
			...
			PUCHAR pData =
			    NdisEqualMemory(SNAP_802_1H, pRxBlk->pData,
					    6) ? (pRxBlk->pData +
						  6) : pRxBlk->pData;
		...
		if (pEntry && (IS_ENTRY_CLIENT(pEntry))
			...
			if (IS_MULTICAST_MAC_ADDR(pRxBlk->pHeader->Addr3)
				...
				pMbss->mcPktsRx++;
				   ...
				   || IS_BROADCAST_MAC_ADDR(pRxBlk->pHeader->
							    Addr1)) {
				pMbss->bcPktsRx++;
			} else {
				pMbss->ucPktsRx++;
...
#endif				/* DOT11_N_SUPPORT */
		{
			...
			APRxEAPOLFrameIndicate(pAd, pEntry, pRxBlk,
					       FromWhichBSSID);
		}
	}
}
				    ...
				    IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe_Hdr_Trns
	    (pAd, pEntry, pRxBlk->pTransData, pRxBlk->TransDataSize,
	     FromWhichBSSID)) {
#ifdef WAPI_SUPPORT
		/* report to upper layer if the received frame is WAI frame */
		if (RTMPCheckWAIframe(pRxBlk->pData, pRxBlk->DataSize)) {
		...
		if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMPDU)
		    && (pAd->CommonCfg.bDisableReordering == 0)) {
			Indicate_AMPDU_Packet_Hdr_Trns(pAd, pRxBlk,
						       FromWhichBSSID);
		} else
#endif				/* DOT11_N_SUPPORT */
		{
...
*/
VOID APHandleRxDataFrame(IN PRTMP_ADAPTER pAd, IN RX_BLK * pRxBlk)
{
	RXINFO_STRUC *pRxInfo = pRxBlk->pRxInfo;
	RXWI_STRUC *pRxWI = pRxBlk->pRxWI;
...
//+++Add by shiang for debug
	if (0 /*!(pRxInfo->Mcast || pRxInfo->Bcast) */ ) {
		...
		hex_dump("DataFrameHeader", (UCHAR *) pHeader,
...
//---Add by shiangf for debug
	...
	if (IS_MT7601(pAd))
		if (pFmeCtrl->FrDs == 1 &&
		    (RTMPReplayAttackDetection
		     (pAd, pHeader->Addr2, pRxWI->RxWISNR2, 0, 0,
		      pRxWI->RxWISNR1, pRxWI->RxWIBW) == TRUE)) {
			goto err;
		} else
#endif				/* MT7601 */
		if (pFmeCtrl->FrDs == 1 &&
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1)) {
		do {
			...
			if (pEntry != NULL) {
				...
				bWdsPacket = TRUE;
				CliWds_ProxyTabUpdate(pAd, pEntry->Aid,
						      pHeader->Octet);
				break;
			}
#endif				/* CLIENT_WDS */
	...
	/* handle APCLI. */
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry)) {
			if (!
			    (pEntry
			     && APCLI_IF_UP_CHECK(pAd,
						  pEntry->MatchAPCLITabIdx))) {
				goto err;
			}
			FromWhichBSSID =
			    pEntry->MatchAPCLITabIdx + MIN_NET_DEVICE_FOR_APCLI;
		...
		/* Increase received byte counter per BSS */
		if (pHeader->FC.FrDs == 0 &&
		    pRxInfo->U2M && FromWhichBSSID < pAd->ApCfg.BssidNum) {
			MULTISSID_STRUCT *pMbss = pEntry->pMbss;
			if (pMbss != NULL) {
				pMbss->ReceivedByteCount +=
				    pRxWI->RxWIMPDUByteCnt;
		...
		if (IS_P2P_GO_ENTRY(pEntry)) {
			FromWhichBSSID =
			    pEntry->apidx + MIN_NET_DEVICE_FOR_P2P_GO;
		}
#endif				/* P2P_SUPPORT */
	}
	...
	if (!pEntry->bIAmBadAtheros && (pFmeCtrl->Retry)
		...
		if (pAd->CommonCfg.IOTestParm.bRTSLongProtOn == FALSE)
			RTMP_UPDATE_PROTECT(pAd, 8, ALLN_SETPROTECT, FALSE,
					    FALSE);
...
#ifdef RT65xx
		pEntry->LastRxRate =
		    (ULONG) ((pRxWI->RxWIMCS) + (pRxWI->RxWIBW << 7) +
	...
	OldPwrMgmt =
	    APPsIndicate(pAd, pHeader->Addr2, pEntry->Aid, pFmeCtrl->PwrMgmt);
#ifdef UAPSD_SUPPORT
	if (pFmeCtrl->PwrMgmt) {
		if ((CLIENT_STATUS_TEST_FLAG
	...
	if ((pFmeCtrl->SubType & 0x04) && (pFmeCtrl->Order == 0)) {	/* bit 2 : no DATA */
		...
		if (pFmeCtrl->FrDs == 0 &&
		    pRxInfo->U2M && pRxWI->RxWIBSSID < pAd->ApCfg.BssidNum) {
...
//---Add by shiangf for debug
	...
	if (pRxPacket) {
		...
		RTMP_SET_PACKET_IF(pRxPacket, FromWhichBSSID);
#endif				/* IKANOS_VX_1X0 */
		APRxDataFrameAnnounce_Hdr_Trns(pAd, pEntry, pRxBlk,
					       FromWhichBSSID);
	} else {
		/* just return */
		/* because RTMPDeFragmentDataFrame() will release rx packet, */
...
BOOLEAN APRxDoneInterruptHandle(RTMP_ADAPTER * pAd)
{
	...
	while (1) {
		...
		if (pFceInfo->info_type != 0) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Dump the RxD, RxFCEInfo and RxInfo:\n"));
...
//+++Add by shiang for debug
		if (0) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("==>%s():Dump the RxD, RxFCEInfo and RxInfo:\n",
				  __FUNCTION__));
			hex_dump("hw_rx_info", &rxblk.hw_rx_info[0],
				 sizeof(rxblk.hw_rx_info));
			hex_dump("RxD", (UCHAR *) pRxD, sizeof(RXD_STRUC));
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Dump the RxWI and RxPacket:\n"));
			dump_rxwi(pAd, pRxWI);
			hex_dump("RxPacket", (UCHAR *) pHeader,
				 pRxWI->RxWIMPDUByteCnt);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("<==%s():Finish dump!\n", __FUNCTION__));
		}
//---Add by shiang for debug
		...
		if (pAd->CommonCfg.DebugFlags & DBF_DBQ_RXWI)
			dbQueueEnqueueRxFrame(pData, (UCHAR *) pHeader,
						...
						pAd->ate.soundingSNR[2] =
						    (Nc <
						     3) ? 0 : (CHAR) ((pRxBlk->
								      ...
								      [10]);
						pAd->ate.sounding = 2;
						pAd->ate.soundingRespSize =
						    pRxBlk->DataSize;
						for (i = 0;
						     i < pRxBlk->DataSize
						     && i <
						     MAX_SOUNDING_RESPONSE_SIZE;
						     i++)
							pAd->ate.
							    soundingResp[i] =
							    pRxBlk->pData[i];
		...
		if (APCheckRxError(pAd, pRxInfo, pRxWI->RxWIWirelessCliID) !=
		    NDIS_STATUS_SUCCESS) {
		...
		case BTYPE_DATA:
			if (pRxInfo->U2M) {
				...
				if (pRxWI->RxWIMCS < 24) {	/* 3*3 */
					pAd->DiagStruct.RxDataCnt[pAd->
								  DiagStruct.
								  ArrayCurIdx]++;
					pAd->DiagStruct.RxMcsCnt[pAd->
								 DiagStruct.
								 ArrayCurIdx]
					    [pRxWI->RxWIMCS]++;
				}
...
BOOLEAN APHandleRxDonePacket(IN RTMP_ADAPTER * pAd,
			     IN PNDIS_PACKET pRxPacket, IN RX_BLK * pRxBlk)
{
	RXD_STRUC *pRxD;
	RXINFO_STRUC *pRxInfo;
	RXWI_STRUC *pRxWI;
		...
		if ((pAd->chipCap.FlgHwTxBfCap)
			...
			if ((((pHeader->FC.SubType == SUBTYPE_ACTION)
				...
				if (pAd->ate.sounding == 1) {
					...
					for (i = 0;
					     i < pRxBlk->DataSize
					     && i < MAX_SOUNDING_RESPONSE_SIZE;
					     i++)
						pAd->ate.soundingResp[i] =
						    pRxBlk->pData[i];
				}
			} else {
				if (pHeader->FC.Order)
...
#ifdef DBG_DIAGNOSE
			if (pRxWI->RxWIMCS < 24) {
				pAd->DiagStruct.RxDataCnt[pAd->DiagStruct.
							  ArrayCurIdx]++;
				pAd->DiagStruct.RxMcsCnt[pAd->DiagStruct.
							 ArrayCurIdx][pRxWI->
								      RxWIMCS]++;
			}
#endif				/* DBG_DIAGNOSE */
					 ...
					 IN ULONG FromWhichBSSID)
{
		...
		if (pEntry && pEntry->Sst == SST_ASSOC) {
			...
			if (FromWhichBSSID == pEntry->apidx) {	/* STAs in same SSID */
				...
				if (pAd->ApCfg.IsolateInterStaTrafficBTNBSSID ==
				    1 || ((FromWhichBSSID < MAX_MBSSID_NUM(pAd))
					  && (FromWhichBSSID <
					      HW_BEACON_MAX_NUM)
					  && (pAd->ApCfg.MBSSID[pEntry->apidx].
					      VLAN_VID !=
					      pAd->ApCfg.MBSSID[FromWhichBSSID].
					      VLAN_VID)))
	...
	if (bDirectForward) {
		...
		pForwardPacket =
		    RTMP_DUPLICATE_PACKET(pAd, pPacket, FromWhichBSSID);
...

		{
			...
			if (pEntry && (pEntry->apidx != 0))
				RTMP_SET_PACKET_NET_DEVICE_MBSSID
				    (pForwardPacket, pEntry->apidx);

			/* send bc/mc frame back to the same bss */
			if (!pEntry)
				RTMP_SET_PACKET_NET_DEVICE_MBSSID
				    (pForwardPacket, FromWhichBSSID);

			RTMP_SET_PACKET_WCID(pForwardPacket,
					     pEntry ? pEntry->Aid : MCAST_WCID);
...
VOID ApCliRTMPSendNullFrame(IN PRTMP_ADAPTER pAd,
			    ...
			    PMAC_TABLE_ENTRY pMacEntry, IN USHORT PwrMgmt)
{
	UCHAR NullFrame[48];
	ULONG Length;
