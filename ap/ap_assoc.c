				    ...
				    IN IE_LISTS * ie_list)
{
	...
	if (ie_list->ht_cap_len && WMODE_CAP_N(pAd->CommonCfg.PhyMode)) {
		assoc_ht_info_debugshow(pAd, pEntry, ie_list->ht_cap_len,
		...
		if ((ie_list->vht_cap_len) &&
			...
			assoc_vht_info_debugshow(pAd, pEntry, &ie_list->vht_cap,
						 NULL);
		}
#endif				/* DOT11_VHT_AC */
	} else
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tAuthMode=%d, WepStatus=%d, WpaState=%d, GroupKeyWepStatus=%d\n",
		  pEntry->AuthMode, pEntry->WepStatus, pEntry->WpaState,
					  ...
					  IN UCHAR MaxSupportedRate)
{
	...
	if ((ie_list->ht_cap_len != 0) &&
...
#ifdef TXBF_SUPPORT
		supportsETxBF =
		    clientSupportsETxBF(pAd, &ie_list->HtCapability.TxBFCap);
#endif				/* TXBF_SUPPORT */
		...
		pEntry->MpduDensity =
		    ie_list->HTCapability.HtCapParm.MpduDensity;
		pEntry->MaxRAmpduFactor =
		    ie_list->HTCapability.HtCapParm.MaxRAmpduFactor;
		pEntry->MmpsMode =
		    (UCHAR) ie_list->HTCapability.HtCapInfo.MimoPs;
		pEntry->AMsduSize =
		    (UCHAR) ie_list->HTCapability.HtCapInfo.AMsduSize;
		...
		if (pAd->CommonCfg.bRdg
		    && ie_list->HTCapability.ExtHtCapInfo.RDGSupport)
			CLIENT_STATUS_SET_FLAG(pEntry,
...
//yiwei cfg
			...
			StatusCode =
			    update_associated_mac_entry(pAd, pEntry, ie_list,
							MaxSupportedRate);
		}
...
/*
 ...
 ========================================================================
*/
static BOOLEAN IAPP_L2_Update_Frame_Send(IN PRTMP_ADAPTER pAd,
				  ...
				  IN BOOLEAN isReassoc)
{
	...
	if (!PeerAssocReqCmmSanity
	    (pAd, isReassoc, Elem->Msg, Elem->MsgLen, ie_list))
		goto LabelOK;
...
#endif				/* WSC_V2_SUPPORT */
	{
		if (pEntry->apidx < pAd->ApCfg.BssidNum) {
			if (MAC_ADDR_EQUAL
			    (pEntry->Addr, wdev->WscControl.EntryAddr)) {
				BOOLEAN Cancelled;
				RTMPZeroMemory(wdev->WscControl.EntryAddr,
					       MAC_ADDR_LEN);
				RTMPCancelTimer(&wdev->WscControl.EapolTimer,
						&Cancelled);
	...
	StatusCode =
	    APBuildAssociation(pAd, pEntry, ie_list, MaxSupportedRate, &Aid);
...
//---Add by shiang for debug
	...
	if ((ie_list->ht_cap_len > 0) && WMODE_CAP_N(pAd->CommonCfg.PhyMode)) {
		...
		NdisMoveMemory(&HtCapabilityRsp, &pAd->CommonCfg.HtCapability,
			       ie_list->ht_cap_len);
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
				  ...
				  END_OF_ARGS);
#else
		NdisMoveMemory(&HtCapabilityTmp, &HtCapabilityRsp,
			       ie_list->ht_cap_len);
		...
		*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
		...
		if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
		    (pAd->CommonCfg.Channel > 14) && (ie_list->vht_cap_len)) {
			FrameLen +=
			    build_vht_ies(pAd, pOutBuffer + FrameLen,
					  SUBTYPE_ASSOC_RSP);
		}
#endif				/* DOT11_VHT_AC */
	}
...
/* in cfg80211 mode, wpa_supplicant may provide WPS IE */
	...
	if (pEntry->bWscCapable) {
		...
		if (pWscBuf) {
			...
			WscBuildAssocRespIE(pAd, pEntry->apidx, 0, pWscBuf,
					    &WscIeLen);
	...
	if (ie_list->P2PSubelementLen > 0) {
		...
		UCHAR GroupCap = 0xff, DeviceCap = 0xff, DevAddr[6] = { 0 }
		, DeviceType[8], DeviceName[32], DeviceNameLen = 0;
		PUCHAR pData;
		USHORT Dpid, ConfigMethod;

		pEntry->bP2pClient = TRUE;
		pEntry->P2pInfo.P2pClientState = P2PSTATE_CLIENT_ASSOC;
		P2pParseSubElmt(pAd, (PVOID) ie_list->P2pSubelement,
				ie_list->P2PSubelementLen, FALSE, &Dpid,
				&GroupCap, &DeviceCap, DeviceName,
				&DeviceNameLen, DevAddr, NULL, NULL, NULL, NULL,
				&ConfigMethod, &ConfigMethod, DeviceType, NULL,
				NULL, NULL, NULL, (UCHAR *) & StatusCode, NULL,
				NULL);
		...
		if (P2pIdx == P2P_NOT_FOUND)
			P2pIdx =
			    P2pGroupTabInsert(pAd, DevAddr,
					      P2PSTATE_DISCOVERY_CLIENT, NULL,
					      0, 0, 0);
		...
		if (P2pIdx != P2P_NOT_FOUND) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("DeviceNameLen = %d, DeviceName = %c %c %c %c %c %c %c %c\n",
				  DeviceNameLen, DeviceName[0], DeviceName[1],
				  DeviceName[2], DeviceName[3], DeviceName[4],
				  DeviceName[5], DeviceName[6], DeviceName[7]));

			/* update P2P Interface Address */
			RTMPMoveMemory(pAd->P2pTable.Client[P2pIdx].
				       InterfaceAddr, pEntry->Addr,
				       MAC_ADDR_LEN);
		...
		if (TRUE) {
			...
			if ((pAd->Cfg80211VifDevSet.vifDevList.size > 0) &&
			    ((pNetDev =
			     ...
			     != NULL)) {
				printk
				    ("CFG : GO NOITFY THE CLIENT ASSOCIATED\n");
				CFG80211OS_NewSta(pNetDev, ie_list->Addr2,
						  (PUCHAR) Elem->Msg,
						  Elem->MsgLen);
			} else {
				printk
				    ("CFG : NO NOITFY THE CLIENT ASSOCIATED\n");
				CFG80211OS_NewSta(pAd->net_dev, ie_list->Addr2,
						  (PUCHAR) Elem->Msg,
						  Elem->MsgLen);
...
/* enqueue a EAPOL_START message to trigger EAP state machine doing the authentication */
		...
		/*else if (isReassoc && */
			...
			if (((CacheIdx =
			    ...
			    (RTMPEqualMemory
			     (pPmkid,
			      &wdev->PMKIDCache.BSSIDInfo[CacheIdx].PMKID,
			      LEN_PMKID))) {
				/* Enqueue a EAPOL-start message with the pEntry for WPAPSK State Machine */
				if ((pEntry->EnqueueEapolStartTimerRunning ==
				     EAPOL_START_DISABLE
			...
			if (pEntry->WepStatus == Ndis802_11WEPEnabled) {
				...
				KeyIdx = wdev->DefaultKeyId;
				CipherAlg =
				    pAd->SharedKey[pEntry->apidx][KeyIdx].
				    CipherAlg;
...
/*
    ...
    ==========================================================================
 */
VOID APPeerAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
...
/*
    ...
    ==========================================================================
 */
VOID APPeerReassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
...
/*
    ...
    ==========================================================================
 */
VOID APPeerDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!PeerDisassocReqSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, &SeqNum, &Reason))
		return;

	DBGPRINT(RT_DEBUG_OFF,
		 ("ASSOC - receive DIS-ASSOC(seq-%d) request from %02x:%02x:%02x:%02x:%02x:%02x, reason=%d\n",
		  SeqNum, Addr2[0], Addr2[1], Addr2[2], Addr2[3], Addr2[4],
		  Addr2[5], Reason));
...
VOID MbssKickOutStas(IN PRTMP_ADAPTER pAd, IN INT apidx, IN USHORT Reason)
{
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pEntry && IS_ENTRY_CLIENT(pEntry) && pEntry->apidx == apidx)
			APMlmeKickOutSta(pAd, pEntry->Addr, pEntry->Aid,
					 Reason);
	}
	...
	if (Aid < MAX_LEN_OF_MAC_TABLE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ASSOC - MLME disassociates %02x:%02x:%02x:%02x:%02x:%02x; Send DISASSOC request\n",
			  pStaAddr[0], pStaAddr[1], pStaAddr[2], pStaAddr[3],
...
/*
    ...
    ==========================================================================
 */
VOID APMlmeDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
