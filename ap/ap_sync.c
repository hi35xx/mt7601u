			    ...
			    OUT STATE_MACHINE_FUNC Trans[])
{
	...
	RTMPInitTimer(pAd, &pAd->MlmeAux.APScanTimer,
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		if ((pAd->ApCfg.MBSSID[apidx].MSSIDDev != NULL) &&
		    !(RTMP_OS_NETDEV_STATE_RUNNING
		      (pAd->ApCfg.MBSSID[apidx].MSSIDDev))) {
		...
		if (((SsidLen == 0) && (!pAd->ApCfg.MBSSID[apidx].bHideSsid)) ||
		    ...
		    ((SsidLen == 32)
		     && MAC_ADDR_EQUAL(Addr3, pAd->ApCfg.MBSSID[apidx].Bssid)
		     && (pAd->ApCfg.MBSSID[apidx].bHideSsid == 0)) ||
#endif				/* WSC_AP_SUPPORT */
#ifdef P2P_SUPPORT
		    (NdisEqualMemory(Ssid, &WILDP2PSSID[0], WILDP2PSSIDLEN)) ||
#endif				/* P2P_SUPPORT */
		    ((SsidLen == pAd->ApCfg.MBSSID[apidx].SsidLen)
		     && NdisEqualMemory(Ssid, pAd->ApCfg.MBSSID[apidx].Ssid,
					(ULONG) SsidLen))) ;
		else
		...
		if (P2P_GO_ON(pAd)) {
			...
			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  ...
					  CapabilityInfo, 1, &SsidIe, 1,
					  &pAd->ApCfg.MBSSID[apidx].SsidLen,
					  pAd->ApCfg.MBSSID[apidx].SsidLen,
					  pAd->ApCfg.MBSSID[apidx].Ssid, 1,
					  &SupRateIe, 1, &SupRateLen,
					  SupRateLen, &SupRate, 1, &DsIe, 1,
					  &DsLen, 1, &Channel, END_OF_ARGS);
...
#endif				/* P2P_SUPPORT */
		{
			...
			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  ...
					  pAd->ApCfg.MBSSID[apidx].Ssid, 1,
					  &SupRateIe, 1, &SupRateLen,
					  SupRateLen, pAd->CommonCfg.SupRate, 1,
					  &DsIe, 1, &DsLen, 1,
					  &pAd->CommonCfg.Channel, END_OF_ARGS);
		}
		...
		if (WMODE_CAP_N(PhyMode) &&
			...
			if (pAd->CommonCfg.bExtChannelSwitchAnnouncement
				...
				build_ext_channel_switch_ie(pAd,
							    &HtExtChannelSwitchIe);
				MakeOutgoingFrame(pOutBuffer + FrameLen,
						  &TmpLen,
						  sizeof
		...
		if (pAd->ApCfg.MBSSID[apidx].AuthMode < Ndis802_11AuthModeWPA) ;	/* enough information */
		else if ((pAd->ApCfg.MBSSID[apidx].AuthMode ==
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 1,
					  ...
					  pAd->ApCfg.MBSSID[apidx].RSN_IE[0], 1,
					  &RSNIe2, 1,
					  &pAd->ApCfg.MBSSID[apidx].
					  RSNIE_Len[1],
		...
		if (pAd->ApCfg.MBSSID[apidx].bWmmCapable) {
			UCHAR i;
			UCHAR WmeParmIe[26] =
			    { IE_VENDOR_SPECIFIC, 24, 0x00, 0x50, 0xf2, 0x02,
		  0x01, 0x01, 0, 0 };
			WmeParmIe[8] =
			    pAd->ApCfg.BssEdcaParm.EdcaUpdateCount & 0x0f;
#ifdef UAPSD_SUPPORT
			UAPSD_MR_IE_FILL(WmeParmIe[8],
					 &pAd->ApCfg.MBSSID[apidx].UapsdInfo);
#endif				/* UAPSD_SUPPORT */
		...
		if (pAd->FlgQloadEnable != 0) {
			FrameLen +=
			    QBSS_LoadElementAppend(pAd, pOutBuffer + FrameLen);
		}
		...
		if (WMODE_CAP_N(PhyMode) &&
			...
			if (pAd->bBroadComHT == TRUE) {
				...
				UCHAR BROADCOM_HTC[4] =
				    { 0x0, 0x90, 0x4c, 0x33 };
				UCHAR BROADCOM_AHTINFO[4] =
				    { 0x0, 0x90, 0x4c, 0x34 };
				...
				*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
				...
				MakeOutgoingFrame(pOutBuffer + FrameLen,
						  ...
						  END_OF_ARGS);
#else
				NdisMoveMemory(&addHTInfoTmp,
					       &pAd->CommonCfg.AddHTInfo,
					       AddHtLen);
				*(USHORT *) (&addHTInfoTmp.AddHtInfo2) =
				    SWAP16(*(USHORT *)
					   (&addHTInfoTmp.AddHtInfo2));
				*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
				    SWAP16(*(USHORT *)
					   (&addHTInfoTmp.AddHtInfo3));
		...
		if ((pAd->ApCfg.MBSSID[apidx].WscControl.WscConfMode >
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &WscTmpLen,
					  ...
					  pAd->ApCfg.MBSSID[apidx].
					  WscIEProbeResp.Value, END_OF_ARGS);
			FrameLen += WscTmpLen;
		}
#endif				/* WSC_AP_SUPPORT */
		...
		if (P2P_GO_ON(pAd)) {
			...
			os_alloc_mem(NULL, (UCHAR **) & P2pSubelement,
				     MAX_VIE_LEN);
...
*/
			...
			if (PeerP2pProbeReqSanity
			    (pAd, Elem->Msg, Elem->MsgLen, Addr2, Ssid,
			     &SsidLen, &Peerip, &P2PSubelementLen,
			     P2pSubelement, &WpsLen, WpsIE)) {
				ULONG P2PIeLen;
				PUCHAR ptr;
				ptr = pOutBuffer + FrameLen;
				P2pMakeP2pIE(pAd, SUBTYPE_PROBE_RSP, ptr,
					     &P2PIeLen);
				FrameLen += P2PIeLen;
			}
			if (P2pSubelement)
...
VOID APPeerBeaconAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		RTMPConflictSsidDetection(pAd, (PUCHAR) ie_list->Ssid,
					  ie_list->SsidLen, (CHAR) Elem->Rssi0,
					  (CHAR) Elem->Rssi1,
					  (CHAR) Elem->Rssi2, Elem->AntSel);
#endif				/* IDS_SUPPORT */
...
*/
		...
		if (Elem->Wcid < MAX_LEN_OF_MAC_TABLE) {
			...
			if (pEntry && IS_ENTRY_APCLI(pEntry)
			    && (pEntry->MatchAPCLITabIdx < MAX_APCLI_NUM)) {
				pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
				    ApCliRcvBeaconTime = pAd->Mlme.Now32;
		...
		if (
			   ...
			   (NdisEqualMemory
			    (pAd->ApCfg.ApCliTab[0].CfgApCliBssid,
			     ie_list->Bssid, MAC_ADDR_LEN))) {
			MlmeEnqueue(pAd, APCLI_SYNC_STATE_MACHINE,
				    APCLI_MT2_PEER_BEACON, Elem->MsgLen,
				    Elem->Msg, 0);
		} else {
			PeerP2pBeacon(pAd, ie_list->Addr2, Elem,
				      ie_list->TimeStamp);
			if (ie_list->MessageToMe) {
				MiniportMMRequest(pAd, 0,
						  (PUCHAR) & pAd->ApCfg.
						  ApCliTab[0].PsPollFrame,
						  sizeof(PSPOLL_FRAME));
			}
		}
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		if (ie_list->Channel == pAd->ApCfg.AutoChannel_Channel) {
			if (AutoChBssSearchWithSSID
				...
				pAd->pChannelInfo->ApCnt[pAd->ApCfg.
...
VOID APMlmeScanReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (MlmeScanReqSanity
	    (pAd, Elem->Msg, Elem->MsgLen, &BssType, (PCHAR) Ssid, &SsidLen,
	     &ScanType)) {
...
{
	return (pAd->Mlme.ApSyncMachine.CurrState ==
		AP_SCAN_LISTEN) ? TRUE : FALSE;
}
#endif				/* AP_SCAN_SUPPORT */
...
UCHAR get_regulatory_class(IN PRTMP_ADAPTER pAd)
{
	int i = 0;
	UCHAR regclass = 0;
