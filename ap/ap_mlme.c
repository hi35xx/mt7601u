	...
	if (pAd->CommonCfg.bHTProtect) {
		...
		if (pAd->CommonCfg.IOTestParm.bRTSLongProtOn == FALSE) {
			AsicUpdateProtect(pAd,
					  (USHORT) pAd->CommonCfg.AddHTInfo.
		...
		if (pEntry &&
			...
			if ((MAC_ADDR_EQUAL
			     (pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.
			      EntryAddr, pEntry->Addr)
			     || MAC_ADDR_EQUAL(pAd->ApCfg.MBSSID[pEntry->apidx].
				...
				*Machine = WSC_STATE_MACHINE;
				...
				EAPCode =
				    *((UCHAR *) pFrame + LENGTH_802_11 +
				      LENGTH_802_1_H + 4);
				Return =
				    WscMsgTypeSubst(EAPType, EAPCode, MsgType);
			}
		}
#endif				/* WSC_AP_SUPPORT */
		if (!Return) {
		...
		*Machine = ACTION_STATE_MACHINE;
		...
		if ((pFrame->Octet[0] & 0x7F) == MT2_ACT_VENDOR) {	/*  Sometimes Sta will return with category bytes with MSB = 1, if they receive catogory out of their support */
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Vendor Action frame OUI= 0x%lx\n",
				  *(PULONG) & pFrame->Octet[1]));
			/* Now support WFA P2P */
			if (RTMPEqualMemory(&pFrame->Octet[1], P2POUIBYTE, 4)
			    && (P2P_INF_ON(pAd))) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Vendor Action frame P2P OUI= 0x%lx\n",
					  *(PULONG) & pFrame->Octet[1]));
...
/*
    ...
    ========================================================================
*/
VOID APAsicRxAntEvalTimeout(PRTMP_ADAPTER pAd)
{
...
/*
    ...
    ========================================================================
*/
VOID APAsicAntennaAvg(IN PRTMP_ADAPTER pAd,
		      IN UCHAR AntSelect, IN SHORT * RssiAvg)
{
	SHORT realavgrssi;
	LONG realavgrssi1;
