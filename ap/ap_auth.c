...
static VOID APMlmeDeauthReqAction(IN PRTMP_ADAPTER pAd,
				  IN MLME_QUEUE_ELEM * Elem);

static VOID APPeerDeauthReqAction(IN PRTMP_ADAPTER pAd,
				  IN MLME_QUEUE_ELEM * Elem);

static VOID APPeerAuthReqAtIdleAction(IN PRTMP_ADAPTER pAd,
				      IN MLME_QUEUE_ELEM * Elem);
...
static VOID APPeerAuthSimpleRspGenAndSend(IN PRTMP_ADAPTER pAd,
					  ...
					  IN USHORT Alg,
					  IN USHORT Seq, IN USHORT StatusCode);
				      ...
				      IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (P2P_GO_ON(pAd) && (pAd->P2pCfg.bStopAuthRsp == TRUE)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s:: GO update not complete, stop Auth Rsp.\n",
			  __FUNCTION__));
		return;
	}
#endif				/* P2P_SUPPORT */
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry)) {
		...
		if (!RTMPEqualMemory
		    (Addr1, pAd->ApCfg.MBSSID[pEntry->apidx].Bssid,
		     MAC_ADDR_LEN)) {
	...
	if (!ApCheckAccessControlList(pAd, Addr2, apidx)) {
		...
		RTMPSendWirelessEvent(pAd, IW_MAC_FILTER_LIST_EVENT_FLAG, Addr2,
				      apidx, 0);
	...
	if ((Alg == AUTH_MODE_OPEN) &&
		...
		if (!pEntry)
			pEntry =
			    MacTableInsertEntry(pAd, Addr2, apidx, OPMODE_AP,
						TRUE);
		...
		if (pEntry) {
			...
			APPeerAuthSimpleRspGenAndSend(pAd, pRcvHdr, Alg,
						      Seq + 1, MLME_SUCCESS);
				    ...
				    IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pEntry && MAC_ADDR_EQUAL(Addr2, pAd->ApMlmeAux.Addr)) {
		if ((pRcvHdr->FC.Wep == 1) &&
		    NdisEqualMemory(Chtxt, pAd->ApMlmeAux.Challenge,
				    CIPHER_TEXT_LEN)) {
			/* Successful */
			APPeerAuthSimpleRspGenAndSend(pAd, pRcvHdr, Alg,
						      Seq + 1, MLME_SUCCESS);
		     ...
		     IN ULONG Wcid, IN PHEADER_802_11 pHeader)
{
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry)) {
		...
		if (NdisEqualMemory(pAd->CurrentAddress, bssid, MAC_ADDR_LEN) ==
		    0)
			return;
	}
