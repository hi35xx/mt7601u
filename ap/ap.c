...
NDIS_STATUS APInitialize(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_MBSSID_NUM(pAd); i++)
		RTMPInitTimer(pAd, &pAd->ApCfg.MBSSID[i].REKEYTimer,
			      GET_TIMER_FUNCTION(GREKEYPeriodicExec), pAd,
			      TRUE);
...
VOID APStartUp(IN PRTMP_ADAPTER pAd)
{
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		if (pMbss->MSSIDDev != NULL) {
			NdisMoveMemory(RTMP_OS_NETDEV_GET_PHYADDR
				       (pMbss->MSSIDDev), pMbss->Bssid,
				       MAC_ADDR_LEN);
		}
		...
		if (pMbss->WepStatus == Ndis802_11WEPEnabled) {
			...
			for (idx = 0; idx < SHARE_KEY_NUM; idx++) {
				...
				if (pAd->SharedKey[apidx][idx].KeyLen > 0) {
					...
					AsicAddSharedKeyEntry(pAd, apidx, idx,
							      &pAd->
							      SharedKey[apidx]
							      [idx]);
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if ((pEntry->Sst != SST_ASSOC)
			...
			if (NdisEqualMemory
				...
				NdisZeroMemory(pAd->ApCfg.MBSSID[pEntry->apidx].
					       WscControl.EntryAddr,
					       MAC_ADDR_LEN);
		...
		if ((pAd->ApCfg.MBSSID[pEntry->apidx].StationKeepAliveTime > 0)
			...
			if (pEntry->StationKeepAliveCount++ == 0) {
				if (pEntry->PsMode == PWR_SAVE) {
					...
					ApEnqueueNullFrame(pAd, pEntry->Addr,
							   pEntry->CurrTxRate,
							   pEntry->Aid,
							   pEntry->apidx,
							   bQosNull, TRUE, 0);
...
VOID APUpdateOperationMode(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->ApCfg.LastNoneHTOLBCDetectTime + (5 * OS_HZ)) > pAd->Mlme.Now32) {	/* non HT BSS exist within 5 sec */
		...
		AsicUpdateProtect(pAd,
				  pAd->CommonCfg.AddHTInfo.AddHtInfo2.
				  OperaionMode, ALLN_SETPROTECT, FALSE, TRUE);
	}
	...
	if ((pAd->CommonCfg.AddHTInfo.AddHtInfo2.OperaionMode == 0)
		...
		AsicUpdateProtect(pAd,
				  pAd->CommonCfg.AddHTInfo.AddHtInfo2.
				  OperaionMode, (ALLN_SETPROTECT), TRUE,
	...
	pAd->CommonCfg.AddHTInfo.AddHtInfo2.NonGfPresent =
	    pAd->MacTab.fAnyStationNonGF;
}
#endif				/* DOT11_N_SUPPORT */
...
VOID APUpdateCapabilityAndErpIe(IN PRTMP_ADAPTER pAd)
{
	...
	if (bUseBGProtection !=
		...
		if (bUseBGProtection) {
			...
			AsicUpdateProtect(pAd, OperationMode, (OFDMSETPROTECT),
					  FALSE, bNonGFExist);
		} else {
...
VOID ApUpdateAccessControlList(IN PRTMP_ADAPTER pAd, IN UCHAR Apidx)
{
	...
	for (MacIdx = 0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++) {
		...
		Matched = FALSE;
		for (AclIdx = 0;
		     AclIdx < pAd->ApCfg.MBSSID[Apidx].AccessControlList.Num;
		     AclIdx++) {
			if (MAC_ADDR_EQUAL
		...
		if ((Matched == FALSE)
			...
			if (pAd->MacTab.Content[MacIdx].Sst == SST_ASSOC) {
				...
				MgtMacHeaderInit(pAd, &DisassocHdr,
						 SUBTYPE_DISASSOC, 0,
						 pAd->MacTab.Content[MacIdx].
						 Addr,
#ifdef P2P_SUPPORT
						 pAd->ApCfg.MBSSID[pAd->MacTab.
								   Content
								   [MacIdx].
								   apidx].Bssid,
#endif				/* P2P_SUPPORT */
...
/*		if ((PID & 0x3f) < WDS_PAIRWISE_KEY_OFFSET) // send to client */
		{
			MgtMacHeaderInit(pAd, pNullFr, SUBTYPE_NULL_FUNC, 0,
					 pAddr,
#ifdef P2P_SUPPORT
			...
			MiniportMMRequest(pAd,
					  MapUserPriorityToAccessCategory[7],
		...
		if (index < pAd->ChannelListNum) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("5.0GHz: Found CtrlCh idx(%d) from the ChList, ExtCh=%s, PriCh=[Idx:%d, CH:%d], SecCh=[Idx:%d, CH:%d], effected Ch=[CH:%d~CH:%d]!\n",
				  ...
				  pCoexChRange->primaryCh,
				  pAd->ChannelList[pCoexChRange->primaryCh].
				  Channel, pCoexChRange->secondaryCh,
				  pAd->ChannelList[pCoexChRange->secondaryCh].
				  Channel,
				  pAd->ChannelList[pCoexChRange->effectChStart].
				  Channel,
				  pAd->ChannelList[pCoexChRange->effectChEnd].
				  Channel));
			return TRUE;
		} else {
...
VOID APOverlappingBSSScan(IN RTMP_ADAPTER * pAd)
{
	...
	for (index = chStartIdx; index <= chEndIdx; index++) {
		...
		if ((pAd->ChannelList[index].
		     bEffectedChannel & (EFFECTED_CH_PRIMARY |
					 EFFECTED_CH_LEGACY))
		    && (index != curPriChIdx)) {
			needFallBack = TRUE;
			DBGPRINT(RT_DEBUG_TRACE,
		...
		if ((pAd->ChannelList[index].
		     bEffectedChannel & EFFECTED_CH_SECONDARY)
		    && (index != curSecChIdx)) {
			needFallBack = TRUE;
			DBGPRINT(RT_DEBUG_TRACE,
				...
				IN MAC_TABLE_ENTRY * pEntry, IN UINT8 cmd)
{
	...
	UCHAR RalinkIe[9] =
	    { 221, 7, 0x00, 0x0c, 0x43, 0x00, 0x00, 0x00, 0x00 };
	...
	if (pEntry) {
		...
		NdisMoveMemory(s_addr, pAd->ApCfg.MBSSID[apidx].Bssid,
			       MAC_ADDR_LENGTH);
		s_addr[0] |= 0x80;
	}
	...
	MAKE_802_3_HEADER(FrameBuf,
			  pAd->ApCfg.MBSSID[apidx].Bssid, s_addr, EAPOL_IE);
	offset += LENGTH_802_3;
			       ...
			       IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA)
	    ...
	    || (pAd->ApCfg.MBSSID[apidx].IEEE8021X == TRUE)) {
		/* Init the frame buffer */
		NdisZeroMemory(FrameBuf, frame_len);
		...
		/* Prepare the 802.3 header */
		MAKE_802_3_HEADER(FrameBuf, pAd->ApCfg.MBSSID[apidx].Bssid,
				  pEntry->Addr, EAPOL);
		offset += LENGTH_802_3;
