...
{
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans, MAX_DLS_STATE,
...
========================================================================
*/
VOID APPeerDlsReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * pElem)
{
	...
	if (!pDAEntry)
		...
		Status = MLME_DEST_STA_NOT_IN_QBSS;
		 ...
		 && !CLIENT_STATUS_TEST_FLAG(pDAEntry,
...
/*	pOutBuffer = kmalloc(MAX_LEN_OF_MLME_BUFFER, MEM_ALLOC_FLAG); */
	...
	if (Status == MLME_SUCCESS) {
		...
		MakeOutgoingFrame(pOutBuffer, (ULONG *) & FrameLen,
				  ...
				  1, &Category,
				  1, &Action,
				  2, &Status, 6, SA, 6, DA, END_OF_ARGS);
	}
...
========================================================================
*/
VOID APPeerDlsRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * pElem)
{
	PMAC_TABLE_ENTRY pDAEntry, pSAEntry;
	UCHAR DA[MAC_ADDR_LEN], SA[MAC_ADDR_LEN];
	UINT16 CapabilityInfo;
...
/*	pOutBuffer = kmalloc(MAX_LEN_OF_MLME_BUFFER, MEM_ALLOC_FLAG); */
	...
	NdisMoveMemory(Fr->Hdr.Addr1, DA, MAC_ADDR_LEN);
	NdisMoveMemory(Fr->Hdr.Addr2, pAd->ApCfg.MBSSID[pSAEntry->apidx].Bssid,
		       MAC_ADDR_LEN);
	NdisMoveMemory(Fr->Hdr.Addr3, SA, MAC_ADDR_LEN);
...
========================================================================
*/
VOID APPeerDlsTearDownAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * pElem)
{
	PMAC_TABLE_ENTRY pDAEntry, pSAEntry;
	UCHAR DA[MAC_ADDR_LEN], SA[MAC_ADDR_LEN];
	UINT16 ReasonCode;
